...
class RunMapModel extends \think\Model
{
    ...
    public function edit($data)
    {
        ...
        $data_i["status"] = $data["status"];
        $data_i["last_execute_time"] = $data_i["create_time"] = time();
        \think\Db::name("run_maping")
            ->where("host_id", $data["host_id"])
            ->where("active_type", $data["active_type"])
            ->update($data_i);
        return true;
    }
    public function getAllPage($data, $page, $limit, $orderby, $sorting, $supplier_id = 0)
    {
        ...
        $where = function (\think\db\Query $query) use (
            $keywords,
            $from_type,
            $status,
            $user,
        ...
        if (empty($supplier_id)) {
            $list = \think\Db::name("run_maping")
                ...
                ->field("r.*,h.domain,p.name,h.dedicatedip")
                ->where($where)
    ...
    {
        $value = \intval($status);
        return \think\Db::name("run_maping")
            ->where("id", $ID)
            ->update(["status" => $value, "last_execute_time" => time()]);
    }
    ...
    public function cronGetCountTrend($start_time = 0, $end_time = 0, $status = null)
    {
        ...
        $query_sql = \think\Db::name("run_croning")
            ->field("unique_tab,datetime")
            ->where($where)
            ->group("datetime,unique_tab")
            ->buildSql();
        $res = \think\Db::table($query_sql)
            ->alias("x")
            ->field("count(unique_tab) as sum,datetime")
            ->group("datetime")
            ->order("datetime", "asc")
            ->select()
            ->toArray();
    ...
    public function cronGetCountList($datetime, $status = null)
    {
        ...
        $query_sql = \think\Db::name("run_croning")
            ->field("cron_type,unique_tab")
            ->where($where)
            ->group("cron_type,unique_tab")
            ->buildSql();
        $res = \think\Db::table($query_sql)
            ->alias("x")
            ->field("count(cron_type) as sum,cron_type")
            ->group("cron_type")
            ->select()
            ->toArray();
    ...
    {
        $value = \intval($status);
        return \think\Db::name("run_croning")
            ->where("unique_tab", $unique_tab)
            ->update(["status" => $value]);
    }
}
