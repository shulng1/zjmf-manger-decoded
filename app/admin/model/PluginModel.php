...
class PluginModel extends \think\Model
{
    ...
    {
        if (
            !in_array($type, ["gateways", "addons", "certification", "firewall", "oauth", "sms", "automatic", "mail"])
        ) {
            return false;
        }
        if ($type == "gateways") {
    ...
    public function getPluginsMeun($type = "addons", $lang = "chinese")
    {
        ...
        array_unshift($plugins, [
            ...
            "title" => $title,
            "url" => "",
            "menu" => [["name" => $name, "url" => "/" . adminAddress() . "/plugins?languagesys={$languagesys}"]],
        ]);
        return $plugins;
    }
    ...
    private function pluginsHomeMenu($menu, $pluginName = "")
    {
        ...
        $nav = [
            "name" => trim($menu["name"] ?: ""),
            "url" => !empty($menu["url"]) ? shd_addon_url($menu["url"], [], true) : "",
            "fa_icon" => $menu["fa_icon"] ?: "",
            "lang" => json_encode($menu["lang"]),
            "pid" => $menu["pid"] ?: 0,
            "order" => 0,
            "nav_type" => 3,
            "relid" => 1,
