...
class InterflowKeywordModel extends \think\Model
{
    ...
    public function getAllKeyword($in = true, $param = ["type", ["general_execute", "system_execute"]])
    {
        ...
        $all_kw = $query
            ->where("status", "on")
            ->column("matching_text,matching_text_preg,func_id,keyword,title,system_bind", "id");
        return $all_kw ?: [];
    }
    /**
     * 根据ip与消息体，获得host信息
    ...
    {
        $uid = \think\Db::name("interflow_bots")
            ->alias("a")
            ->leftJoin("interflow_clients b", "a.i_type = b.i_type")
            ->where("a.port", $accounts["port"])
            ->where("b.i_account", $accounts["account"])
            ->value("b.uid");
        if (empty($uid)) {
            return false;
        }
    ...
    {
        $query = $this->alias("a")
            ->leftJoin("interflow_func b", "a.id = b.func_id")
            ->field("a.id,a.keyword,b.func_assembly")
            ->where("a.status", "on");
