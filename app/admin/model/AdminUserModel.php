...
class AdminUserModel extends \think\Model
{
    ...
    public function get_auth_role($uid)
    {
        if ($uid > 1) {
            ...
            $role = \think\Db::name("role_user ru")
                ->leftJoin("role r", "r.id = ru.role_id")
                ->where($where)
                ->field("r.auth_role")
                ->find();
        } else {
            $role = \think\Db::name("auth_rule")
                ->field("id")
                ->order("pid", "ASC")
                ->order("id", "ASC")
                ->select()
    ...
    public function get_rule($uid)
    {
        ...
        if ($uid > 1 && $user["role_id"] > 1) {
            $where["ru.user_id"] = $uid;
            $role = \think\Db::name("role_user ru")
                ->leftJoin("auth_access aa", "aa.role_id = ru.role_id")
                ->leftJoin("auth_rule ar", "aa.rule_id = ar.id")
                ->where($where)
                ->field("ar.*")
                ->order("order")
                ->order("id", "ASC")
            ...
            foreach ($role as $key => $v) {
                $rolecount = \think\Db::name("role_user ru")
                    ->leftJoin("auth_access aa", "aa.role_id = ru.role_id")
                    ->leftJoin("auth_rule ar", "aa.rule_id = ar.id")
                    ->where($where)
                    ->where("ar.pid", $v["id"])
