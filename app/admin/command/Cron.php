...
class Cron extends \think\console\Command
{
    ...
    public function dailyCronJob($config)
    {
        $this_time = time();
        if (
            ($this_time - $config["last_dailycron_invocation_time"] ?? 0) < 86400 ||
            date("G") < $config["cron_day_start_time"]
        ) {
            return null;
        }
        ...
        foreach ($cancels as $cancel) {
            ...
            if ($host["domainstatus"] != "Deleted") {
                ...
                if ($result["status"] == 200) {
                    ...
                    $data_i[
                        "description"
                    ] = " 用户提交 - 删除 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                    if ($is_zjmf) {
                ...
                if ($result["status"] == 200) {
                    \think\Db::name("cancel_requests")
                        ->where("id", $cancel["id"])
                        ->update(["delete_time" => time(), "status" => 1]);
                } else {
                    \think\Db::name("cancel_requests")
                        ->where("id", $cancel["id"])
                        ->update(["status" => 2]);
    ...
    public function hostInfo()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.uid,a.productid,a.domainstatus,a.regdate,a.dcimid,b.welcome_email,b.type,a.billingcycle,b.pay_type,b.name,a.nextduedate,a.billingcycle,a.dedicatedip,a.domain,a.username,a.password,a.os,a.assignedips,a.create_time,a.stream_info,b.api_type,b.zjmf_api_id,b.upstream_pid,b.server_group",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->where("a.domainstatus", "=", "Pending")
            ->where("b.api_type", "=", "resource")
            ->where("a.regdate", "<", time() - 300)
            ->select()
            ->toArray();
        $curl_multi_data = [];
        if (!empty($host)) {
    ...
    public function host($config)
    {
        ...
        if (!empty($config["cron_host_suspend"])) {
            ...
            $host = \think\Db::name("host")
                ->field("id,uid,overideautosuspend,overidesuspenduntil")
                ->whereIn("domainstatus", "Active")
                ->where("nextduedate", ">", 0)
                ->where("nextduedate", "<=", time() - $time * 24 * 3600)
            ...
            if (!empty($host[0])) {
                foreach ($host as $v) {
                    ...
                    if ($result["status"] == 200) {
                        ...
                        active_log_final(
                            "产品到期暂停 - 暂停产品 Host ID:" . $v["id"] . "失败,失败原因:" . $result["msg"],
                            $v["uid"],
                            5,
                        );
                        $data_i["host_id"] = $v["id"];
                                ...
                                if ($v["nextduedate"] <= time() - $time * 24 * 3600) {
                                    ...
                                    if ($result1["status"] == 200) {
                                        ...
                                        \think\Db::name("cancel_requests")
                                            ...
                                            ->update(["delete_time" => time(), "status" => 1]);
                                        active_log_final(
                                            "产品到期后删除 - 删除到期产品 Host ID:" . $v["id"] . "成功",
                                            $v["uid"],
                                            5,
                                        );
                                        $data_i["host_id"] = $v["id"];
                                        ...
                                        $data_i[
                                            "description"
                                        ] = "产品到期删除 Host ID:{$v["id"]}失败。原因:{$result["msg"]}";
                                        $data_i["active_type_param"] = [$v["id"], "cron"];
                                        $is_zjmf = $model_host->isZjmfApi($v["id"]);
                                        if ($is_zjmf) {
                                            $logic_run_map->saveMap($data_i, 0, 400, 4);
    ...
    public function invoice($config)
    {
        ...
        if (!empty($recharge)) {
            ...
            if ($ids) {
                ...
                if ($res) {
                    active_log_final(
                        "未付款充值账单 - User ID:" . $res["uid"] . "删除未付款充值账单" . implode(", ", $str) . "成功",
                        $res["uid"],
                        5,
                    );
                } else {
                    active_log_final(
                        "未付款充值账单 - User ID:" . $res["uid"] . "删除未付款充值账单" . implode(", ", $str) . "失败",
                        $res["uid"],
                        5,
                    );
                }
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.id,a.uid,a.initiative_renew")
            ->leftJoin("clients b", "a.uid=b.id")
            ->whereIn("a.domainstatus", "Active,Suspended")
            ->where("a.nextduedate", "<=", strtotime(date("Y-m-d")) + $days * 24 * 3600 + 86400)
            ->where("a.billingcycle", "<>", "free")
            ->where("a.billingcycle", "<>", "onetime")
        ...
        if ($config["cron_invoice_pay_email"] > 0 && $config["cron_invoice_unpaid_email"] > 0) {
            ...
            $host_second = \think\Db::name("host")
                ->alias("a")
                ->field("a.id,b.initiative_renew,a.uid,p.name,a.domain,a.nextinvoicedate,a.create_time,a.nextduedate")
                ->leftJoin("clients b", "a.uid=b.id")
                ->join("products p", "p.id=a.productid")
                ->whereIn("a.domainstatus", "Active,Suspended")
                ->where("a.nextduedate", ">=", strtotime(date("Y-m-d")) + $day * 24 * 3600)
                ->where("a.nextduedate", "<=", strtotime(date("Y-m-d")) + $day * 24 * 3600 + 86400 - 1)
                ->where("a.billingcycle", "<>", "free")
                ->where("a.billingcycle", "<>", "onetime")
                ->where("a.billingcycle", "<>", "ontrial")
                ->select()
            ...
            if (!empty($host_second[0])) {
                ...
                foreach ($host_second as $vv) {
                    if (!cancelRequest($vv["id"])) {
                        ...
                        $client = check_type_is_use(
                            $message_template_type[strtolower("second_renew_product_reminder")],
                            $vv["uid"],
                            $sms,
                        );
                        if ($client) {
                            $params = [
                                ...
                                "hostname" => $vv["domain"],
                                "product_end_time" => date("Y-m-d H:i:s", $vv["nextduedate"]),
                                "product_terminate_time" => date("Y-m-d H:i:s", $vv["nextduedate"]),
                            ];
                            $sms->sendSms(
                                $message_template_type[strtolower("second_renew_product_reminder")],
    ...
    private function invoiceDueSend($config, $times = 0)
    {
        ...
        $host = \think\Db::name("invoices")
            ->alias("b")
            ->field(
                "b.id,c.email,c.phone_code,c.phonenumber,b.total,d.suffix,b.uid,f.id as hostid,p.name,f.domain,e.create_time",
            )
            ->leftJoin("clients c", "b.uid=c.id")
            ->leftJoin("currencies d", "d.id = c.currency")
            ->leftJoin("orders e", "e.invoiceid = b.id")
            ->leftJoin("host f", "e.id = f.orderid")
            ->leftJoin("products p", "p.id = f.productid")
            ->withAttr("total", function ($value, $data) {
                return $value . $data["suffix"];
            })
            ->where("b.status", "Unpaid")
            ->where("b.due_time", "<=", $before_day_end_time)
            ->where("b.due_email_times", $times)
        ...
        if (!empty($host[0])) {
            ...
            foreach ($host as $v) {
                ...
                if ($v["hostid"]) {
                    ...
                    $sms = new \app\common\logic\Sms();
                    $client = check_type_is_use(
                        $message_template_type[strtolower($times_zh . "_Invoice_Payment_Reminder")],
                        $host["uid"],
                        $sms,
                    );
                    if ($client) {
                        $params = [
                            "product_first_time" => date("Y-m-d", $host["create_time"]),
                            "product_name" => $host["name"],
                            "hostname" => $v["domain"],
                        ];
                        $sms->sendSms(
                            $message_template_type[strtolower($times_zh . "_Invoice_Payment_Reminder")],
                    ...
                    if ($result) {
                        ...
                        \think\Db::name("invoices")
                            ->where("id", $v["id"])
                            ->where("delete_time", 0)
                            ->where("due_email_times", $times)
                            ->setInc("due_email_times");
                        $this->ad_log(12, 0, 1);
                    } else {
                        active_log_final(
                            "账单逾期提醒 - User ID:" .
    ...
    public function ticket($config)
    {
        ...
        if ($hour > 0) {
            $tickets = \think\Db::name("ticket")
                ...
                ->where("status", "<>", 4)
                ->where("status", 2)
                ->where("last_reply_time", ">", 0)
                ->where("last_reply_time", "<=", time() - $hour * 3600)
                ->select()
                ->toArray();
            $id = array_column($tickets, "id") ?: [];
            if (!empty($id)) {
                $result = \think\Db::name("ticket")
                    ->whereIn("id", $id)
    ...
    {
        $invoiceids = \think\Db::name("invoices")
        ...
        if ($cron_order_unpaid_time_high == 1 && $day > 0) {
            ...
            $orders = \think\Db::name("orders")
                ...
                ->field("a.id,a.uid,b.id as invoiceid,b.create_time")
                ->whereBetweenTime("a.create_time", $before_day_start_time, $before_day_end_time)
                ->where("b.status", "Unpaid")
                ->where("a.delete_time", 0)
                ->where("b.delete_time", 0)
                ->where("b.type", "<>", "credit_limit")
                ->select()
                ->toArray();
                ...
                if ($action == "Delete") {
                    ...
                    try {
                        ...
                        foreach ($invoiceids as $invoiceid) {
                            $invoice_status = \think\Db::name("invoices")
                                ->where("delete_time", 0)
                                ->where("id", $invoiceid)
                                ->value("status");
                        ...
                        \think\Db::name("host")
                            ->whereIn("id", array_column($hostids, "rel_id"))
    ...
    public function unCertifi($config)
    {
        ...
        if (!empty($use) && $day >= 0 && $certifi_open == 1) {
            ...
            $hosts = \think\Db::name("host")
                ->whereIn("domainstatus", "Active")
                ->where("regdate", "<=", $before_day_end_time)
                ->select()
                ->toArray();
            $hids = [];
            $logic_run_map = new \app\common\logic\RunMap();
            ...
            foreach ($hosts as $host) {
                ...
                if (!checkCertify($uid)) {
                    ...
                    if ($res["status"] == 200) {
                        ...
                        $message_template_type = array_column(config("message_template_type"), "id", "name");
                        $tmp = \think\Db::name("host")
                            ->field("uid,productid,domain,billingcycle,regdate")
                            ->where("id", $host["id"])
                            ->find();
                        $product = \think\Db::name("products")->field("name")->where("id", $tmp["productid"])->find();
                        $sms = new \app\common\logic\Sms();
                        $client = check_type_is_use(
                            $message_template_type[strtolower("uncertify_reminder")],
                            $tmp["uid"],
                            $sms,
                        );
                        if ($client && $tmp["billingcycle"] != "free") {
                            $params = [
                                ...
                                "hostname" => $tmp["domain"],
                                "product_end_time" => date("Y-m-d H:i:s", $tmp["regdate"]),
                            ];
                            $sms->sendSms(
                                $message_template_type[strtolower("uncertify_reminder")],
                                $client["phone_code"] . $client["phonenumber"],
                        ...
                        active_log_final(
                            "未实名客户产品暂停 - 暂停 Host ID:{$host["id"]}的产品失败,失败原因:" . $res["msg"],
                            $host["uid"],
                            5,
                        );
                        $data_i["host_id"] = $host["id"];
    ...
    public function updateCommission()
    {
        $rows = \think\Db::name("invoices")
            ...
            ->leftJoin("orders o", "i.id=o.invoiceid")
            ->field(
                "i.status,i.type,i.subtotal,i.paid_time,c.username,o.id,o.uid,c.id as uid,o.status,o.create_time,o.invoiceid,o.amount,o.payment,cu.prefix,cu.suffix",
            )
            ->where("i.delete_time", 0)
            ->where("i.status", "=", "Paid")
            ->where("i.is_aff", 0)
            ->where("i.use_credit_limit=0 OR (use_credit_limit=1 AND i.invoice_id>0)")
            ->where(function (\think\db\Query $query) {
                    ...
                    try {
                        ...
                        $res = \think\Db::name("invoices")
                            ...
                            ->update([
                                "is_aff" => 1,
                                "aff_sure_time" => time(),
                                "aff_commission" => $rows[0]["commission"],
                                "aff_commmission_bates" => $rows[0]["commission_bates"],
                                "aff_commmission_bates_type" => $rows[0]["commission_bates_type"],
                            ]);
                        foreach ($rows[0]["child"] as $k => $val) {
                            $res = \think\Db::name("invoice_items")
                                ->where("id", $val["inid"])
                                ->update([
                                    "is_aff" => 1,
                                    "aff_sure_time" => time(),
                                    "aff_commission" => $val["commission"],
                                    "aff_commmission_bates" => $val["commission_bates"],
                                    "aff_commmission_bates_type" => $val["commission_bates_type"],
                                ]);
                        }
                        $res = \think\Db::name("affiliates")
                            ->where("id", $affi["id"])
                            ->update([
                                "balance" => bcadd($rows[0]["commission"], $affi["balance"], 2),
                                "updated_time" => time(),
                            ]);
                        \think\Db::commit();
                        active_log_final(
                            "更新用户可提现余额余额: " .
                                $affi["balance"] .
                                " 改为 :" .
                                bcadd($rows[0]["commission"], $affi["balance"], 2),
                            $affi["uid"],
                            5,
                        );
                    } catch (\Exception $e) {
    ...
    public function autoReply()
    {
        ...
        foreach ($ticket as $key => $value) {
            $ticket_deliver = \think\Db::name("ticket_deliver")
                ->alias("a")
                ...
                ->field("a.is_open_auto_reply,a.bz,a.id")
            ...
            if ($td["is_open_auto_reply"] == 1) {
                ...
                if ($value["create_time"] + $td["minutes"] <= time()) {
                    ...
                    active_log_final(
                        sprintf(
                            "自动回复工单成功#User ID:%d - Ticket ID:%s - %s",
                            $value["uid"],
                            $value["tid"],
                            $value["title"],
                        ),
    ...
    public function dcimFlowCls()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.dcimid,a.regdate,a.nextduedate,c.secure,c.hostname,c.username,c.password,c.port,c.accesshash,d.bill_type,d.flow_remind,e.percent,e.send_email",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("servers c", "a.serverid=c.id")
            ->leftJoin("dcim_servers d", "a.serverid=d.serverid")
            ->leftJoin("dcim_percent e", "a.id=e.hostid")
            ->where("b.type", "dcim")
            ->whereIn("b.api_type", ["", "normal"])
            ->where("c.server_type", "dcim")
            ->where("a.dcimid", ">", 0)
            ->where("b.config_option1", "rent")
            ->where("a.domainstatus=\"Active\" OR a.domainstatus=\"Suspended\"")
            ...
            if ($v["bill_type"] == "last_30days") {
                ...
                while ($i < $max) {
                    ...
                    if ($target == $date) {
                        ...
                        if ($result["status"] == 200) {
                            ...
                            $data_i[
                                "description"
                            ] = "DCIM流量 - 周期重置 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                ...
                if ($date == $month) {
                    ...
                    if ($result["status"] == 200) {
                        ...
                        $data_i[
                            "description"
                        ] = "DCIM流量 - 自然月重置 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                        $logic_run_map->saveMap($data_i, 0, 100, 7);
                        $this->ad_log(7, 7, 0, $v["id"]);
                    }
                }
    ...
    public function updateDcimOs()
    {
        ...
        foreach ($server_info as $v) {
            ...
            $data[$v["id"]] = [
                "url" => $url . "/index.php?m=api&a=getAllMirrorOs",
                "data" => ["username" => $v["username"], "password" => aesPasswordDecode($v["password"]) ?? ""],
            ];
        }
        if (!empty($data)) {
            ...
            foreach ($res as $k => $v) {
                if ($v["http_code"] == 200 && $v["data"]["status"] == "success") {
                    \think\Db::name("dcim_servers")
                        ->where("serverid", $k)
                        ->update(["os" => json_encode($v["data"]["data"]) ?? ""]);
                }
            }
        }
    }
    public function updateDcimArea()
    {
        ...
        foreach ($server_info as $v) {
            ...
            $data[$v["id"]] = [
                "url" => $url . "/index.php?m=api&a=getArea",
                "data" => ["username" => $v["username"], "password" => aesPasswordDecode($v["password"]) ?? ""],
            ];
        }
        if (!empty($data)) {
            ...
            foreach ($res as $k => $v) {
                if (
                    ...
                    \think\Db::name("dcim_servers")
                        ->where("serverid", $k)
                        ->update(["area" => json_encode($v["data"]) ?? ""]);
                }
            }
        }
    }
    public function updateHostFlow()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.uid,a.serverid,a.dcimid,a.regdate,c.secure,c.hostname,c.username,c.password,c.port,c.accesshash,d.bill_type,d.flow_remind,e.percent,e.send_email",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("servers c", "a.serverid=c.id")
            ->leftJoin("dcim_servers d", "a.serverid=d.serverid")
                ...
                if (!empty($flow_remind)) {
                    ...
                    if ($over) {
                        if (!empty($v["percent"]) && $v["percent"] == $set_over_percent && $v["send_email"] == 1) {
                            ...
                            if ($r) {
                                $template_name = \think\Db::name("email_templates")
                                    ->where("id", $template)
                                    ->value("name");
                            ...
                            if (!empty($is_set)) {
                                \think\Db::name("dcim_percent")
                                    ->where("hostid", $v["id"])
                                    ->update(["percent" => $set_over_percent, "send_email" => 0]);
                            } else {
                                \think\Db::name("dcim_percent")->insert([
                                    "hostid" => $v["id"],
                                    "percent" => $set_over_percent,
                                    "send_email" => 0,
                                ]);
                            }
                        }
    ...
    public function updateDcimCloudHostFlow()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.uid,a.serverid,a.dcimid,a.regdate,a.domainstatus,a.suspendreason,c.hostname,c.username,c.password,c.secure,c.port,c.accesshash",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("servers c", "a.serverid=c.id")
            ->where("b.type", "dcimcloud")
            ...
            ->whereIn("a.domainstatus", "Active,Suspended")
            ->where("a.nextduedate=0 OR a.nextduedate>" . $time)
            ->page($dcimcloud_hostflow, $pagelimit)
            ->select()
            ->toArray();
        ...
        foreach ($host as $v) {
            if (!empty($token[$v["serverid"]])) {
                ...
                $data[$v["id"]] = [
                    "url" => $url . "/v1/net_info?host_id=" . $v["dcimid"],
                    "data" => [],
                    "header" => ["access-token: " . $token[$v["serverid"]]],
                ];
            }
        }
            ...
            if ($v["status"] == 200 && $v["http_code"] == 200 && !empty($v["data"]["meta"])) {
                ...
                if ($host_status[$k]["domainstatus"] == "Active" && $v["data"]["info"]["30_day"]["float"] > 100) {
                    ...
                    if ($result["status"] == 200) {
                        ...
                        $data_i[
                            "description"
                        ] = " 流量使用超额 - 暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                        if ($is_zjmf) {
                            $logic_run_map->saveMap($data_i, 0, 400, 2);
                        }
                        if (!$is_zjmf) {
                ...
                if (
                    $host_status[$k]["domainstatus"] == "Suspended" &&
                    $v["data"]["info"]["30_day"]["float"] < 100 &&
                    (explode("-", $host_status[$k]["suspendreason"])[0] == "用量超额" ||
                        explode("-", $host_status[$k]["suspendreason"])[0] == "flow")
                ) {
                    $host_obj->is_admin = true;
                    ...
                    if ($result["status"] == 200) {
                        $data_i[
                            "description"
                        ] = " 满足超额解除暂停条件 - 解除暂停 Host ID:{$data_i["host_id"]}的产品成功";
                        if ($is_zjmf) {
                        ...
                        $data_i[
                            "description"
                        ] = " 满足超额解除暂停条件 - 解除暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                        if ($is_zjmf) {
    ...
    public function dcimCloudFlowCls()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.productid,a.dcimid,a.domainstatus,a.regdate,a.nextduedate,c.secure,c.hostname,c.username,c.password,c.port,c.accesshash,d.bill_type,d.flow_remind,e.percent,e.send_email",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("servers c", "a.serverid=c.id")
            ->leftJoin("dcim_servers d", "a.serverid=d.serverid")
        ...
        foreach ($host as $v) {
            ...
            if (!isset($config_option_ids[$v["productid"]])) {
                ...
                if (empty($gids)) {
                    ...
                    $config_option_ids[$v["productid"]] =
                        \think\Db::name("product_config_options")
                            ->whereIn("gid", $gids)
                            ->whereLike("option_name", "traffic_bill_type|%")
                            ->value("id") ?? 0;
                }
            }
            $traffic_bill_type_config_options = [];
            if (!empty($config_option_ids[$v["productid"]])) {
                $optionid = \think\Db::name("host_config_options")
                    ->where("relid", $v["id"])
                    ->where("configid", $config_option_ids[$v["productid"]])
                    ->value("optionid");
                if (!empty($optionid)) {
                    if (isset($options[$optionid])) {
                        $traffic_bill_type_config_options = $options[$optionid];
                    } else {
                        $traffic_bill_type_config_options = \think\Db::name("product_config_options_sub")
                            ->where("id", $optionid)
                            ->value("option_name");
            ...
            if ($traffic_bill_type_config_options == "last_30days") {
                ...
                while ($i < $max) {
                    ...
                    if ($target == $date) {
                        ...
                        if ($result["status"] == 200) {
                            ...
                            $data_i[
                                "description"
                            ] = "魔方云流量 - 周期重置 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                ...
                if ($date == $month) {
                    ...
                    if ($result["status"] == 200) {
                        ...
                        $data_i[
                            "description"
                        ] = "魔方云流量 - 自然月重置 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                        $logic_run_map->saveMap($data_i, 0, 100, 8);
                        $this->ad_log(8, 8, 0, $v["id"]);
                    }
                }
    ...
    public function syncUpstreamProductInfo1()
    {
        ...
        if (!empty($products[0])) {
            ...
            foreach ($products as $product) {
                ...
                if (
                    $res["data"]["products"] &&
                    $res["data"]["products"]["location_version"] > $product["upstream_version"]
                ) {
                    if ($product["upstream_price_type"] == "percent") {
                        $percent[] = $pid;
                        $product_model = new \app\common\model\ProductModel();
                        $res = $product_model->syncProductInfo(
                            $pid,
                            $product["api_type"],
                            $product["zjmf_api_id"],
                        ...
                        $product_pricings = \think\Db::name("pricing")
                            ...
                            ->select()
                            ->toArray();
                        $configoptions_pricings = \think\Db::name("pricing")
                            ->alias("a")
                            ->field("a.*,f.type")
                            ->leftJoin("product_config_options_sub b", "a.relid = b.id")
                            ->leftJoin("product_config_options c", "b.config_id = c.id")
                            ->leftJoin("product_config_links d", "c.gid = d.gid")
                            ->leftJoin("product_config_groups e", "d.gid = e.id")
                            ->leftJoin("products f", "d.pid = f.id")
                            ->where("a.type", "configoptions")
                            ->where("f.id", $pid)
                            ->select()
                            ->toArray();
                        $currencies = \think\Db::name("currencies")
                            ->field("id,code")
                            ->where("default", 1)
                            ->select()
                            ->toArray();
                        ...
                        if (!empty($dec)) {
                            \think\Db::name("products")
                                ->where("id", $pid)
                                ->update(["stock_control" => 1, "qty" => 0]);
                            $info = implode("\n", $dec) ?? "";
                            $exist = \think\Db::name("info_notice")
                                ->where("relid", $pid)
                                ->where("type", "product")
                                ->where("admin", 1)
                                ->find();
                            if ($exist) {
                                \think\Db::name("info_notice")
                                    ->where("relid", $pid)
                                    ->where("type", "product")
                                    ->where("admin", 1)
                                    ->update(["info" => $info, "update_time" => time()]);
                            } else {
                                \think\Db::name("info_notice")->insert([
                                    "relid" => $pid,
                                    "type" => "product",
                                    "info" => $info,
                                    "admin" => 1,
    ...
    public function updateUpstreamBw()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.id,a.dcimid,b.zjmf_api_id")
            ->leftJoin("products b", "a.productid=b.id")
            ->whereIn("b.api_type", "zjmf_api")
            ->where("a.dcimid", ">", 0)
            ->where("a.domainstatus", "Active")
            ->where("a.nextduedate=0 OR a.nextduedate>" . time())
            ->select()
        ...
        foreach ($host as $v) {
            ...
            $data[$v["id"]] = [
                "url" => $token[$v["zjmf_api_id"]]["url"] . "/host/header?host_id=" . $v["dcimid"],
                "header" => ["Authorization: Bearer " . $token[$v["zjmf_api_id"]]["jwt"]],
            ];
        }
    ...
    public function updateDcim()
    {
        ...
        $server = \think\Db::name("servers")
            ->alias("a")
            ->field("a.id,a.gid,b.area,b.os")
            ->leftJoin("dcim_servers b", "a.id=b.serverid")
            ->where("a.server_type", "dcim")
            ->select()
            ->toArray();
        if (empty($server)) {
            return false;
        ...
        foreach ($server as $k => $v) {
            ...
            if (!empty($v["area"])) {
                foreach ($v["area"] as $vv) {
                    $group_area[$v["gid"]][] = [
                        "id" => $vv["id"],
                        "option_name" => $vv["id"] . "|" . $vv["area"] . "^" . ($vv["name"] ?: $vv["area"]),
                    ];
                }
            }
            ...
            foreach ($v["os"]["os"] as $vv) {
                $group_os[$v["gid"]][] = [
                    "id" => $vv["id"],
                    "option_name" => $vv["id"] . "|" . $svg[$os_group[$vv["group_id"]]] . "^" . $vv["name"],
                ];
            }
        }
        $products = \think\Db::name("products")
            ->field("id,server_group")
            ->where("type", "dcim")
            ->whereIn("server_group", array_column($server, "gid"))
            ->whereIn("api_type", ["", "normal"])
            ->select()
            ->toArray();
        if (empty($products)) {
            return false;
        ...
        foreach ($products as $k => $v) {
            ...
            $configid = \think\Db::name("product_config_options")
                ->where("gid", $gid)
                ->where("option_type", 12)
                ->value("id");
            if (empty($configid)) {
                $configid = \think\Db::name("product_config_options")->insertGetId([
                    "gid" => $gid,
                    "option_name" => "area|区域",
                    "option_type" => 12,
                    "qty_minimum" => 0,
            ...
            foreach ($group_area[$v["server_group"]] as $vv) {
                $is_add = \think\Db::name("product_config_options_sub")
                    ->where("config_id", $configid)
                    ->whereLike("option_name", $vv["id"] . "|%")
                    ->value("id");
                if ($is_add) {
                    continue;
                }
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.id,a.productid,a.dcim_area,a.os,b.gid")
            ->leftJoin("servers b", "a.serverid=b.id")
            ->whereIn("a.productid", $product_ids)
            ->select()
            ->toArray();
        foreach ($host as $v) {
            if (!empty($product_area_config[$v["productid"]])) {
                $sub_id = \think\Db::name("product_config_options_sub")
                    ->where("config_id", $product_area_config[$v["productid"]])
                    ->whereLike("option_name", $v["dcim_area"] . "|%")
                    ->value("id");
                if (!empty($sub_id)) {
                    $r = \think\Db::name("host_config_options")
                        ->where("relid", $v["id"])
                        ->where("configid", $product_area_config[$v["productid"]])
                        ->value("id");
                    if (!empty($r)) {
                        \think\Db::name("host_config_options")
                            ->where("id", $r)
                            ->update(["optionid" => $sub_id, "qty" => 0]);
                    } else {
                        \think\Db::name("host_config_options")->insert([
                            "relid" => $v["id"],
                            "configid" => $product_area_config[$v["productid"]],
                            "optionid" => $sub_id,
                            "qty" => 0,
                        ]);
                    }
            ...
            if (!empty($product_os_config[$v["productid"]])) {
                $sub = \think\Db::name("product_config_options_sub")
                    ->field("id,option_name")
                    ->where("config_id", $product_os_config[$v["productid"]])
                    ->whereLike("option_name", "%" . $v["os"] . "%")
                    ->find();
                if (!empty($sub)) {
                    $r = \think\Db::name("host_config_options")
                        ->where("relid", $v["id"])
                        ->where("configid", $product_os_config[$v["productid"]])
                        ->value("id");
                    if (!empty($r)) {
                        \think\Db::name("host_config_options")
                            ->where("id", $r)
                            ->update(["optionid" => $sub["id"], "qty" => 0]);
                    } else {
                        \think\Db::name("host_config_options")->insert([
                            "relid" => $v["id"],
                            "configid" => $product_os_config[$v["productid"]],
                            "optionid" => $sub["id"],
                            "qty" => 0,
                        ]);
                    }
    ...
    private function ad_log($cron_type = 0, $active_type = 0, $status = 0, $unique_id = 0)
    {
        $datetime = date("Ymd");
        $idata = [
            "cron_type" => \intval($cron_type),
            "active_type" => \intval($active_type),
            "status" => \intval($status),
            "create_time" => time(),
            "datetime" => $datetime,
            "unique_tab" => $datetime . $cron_type . $unique_id,
        ];
    ...
    private function getAppHeat()
    {
        ...
        foreach ($products as $key => $value) {
            $count = \think\Db::name("host")
                ...
                ->count();
            $count2 = \think\Db::name("evaluation")
                ->field("id")
                ->where("rid", $value["id"])
                ->where("type", "products")
                ->where("score", ">", "4")
        ...
        $products = \think\Db::name("products")
            ...
            ->where("retired", 0)
            ->where("app_status", 1)
            ->where("app_hot_lock!=1")
            ->order("app_hot_heat", "desc")
            ->select()
            ->toArray();
        foreach ($products as $key => $value) {
            \think\Db::name("products")
                ->where("id", $value["id"])
                ->update(["app_hot_order" => $key + 1]);
        }
        $unlocks = \think\Db::name("products")
            ->where("p_uid", ">", 0)
            ->where("hidden", 0)
            ->where("retired", 0)
            ->where("app_status", 1)
            ->where("app_hot_lock!=1")
            ->where("app_hot_order!=0")
            ->order("app_hot_heat", "desc")
            ->select()
            ->toArray();
        $unlocks = array_column($unlocks, "id", "app_hot_order");
        $locks = \think\Db::name("products")
            ->where("p_uid", ">", 0)
            ->where("hidden", 0)
            ->where("retired", 0)
            ->where("app_status", 1)
    ...
    private function getHotOrder()
    {
        ...
        foreach ($products as $key => $value) {
            ...
            \think\Db::name("products")
                ->where("id", $value["id"])
                ->update(["app_hot_order" => $app_hot_order]);
        }
    }
    private function generateRepaymentBill()
    {
        ...
        $clients = \think\Db::name("clients")
            ->field("id")
            ->whereIn("bill_generation_date", $whereIn)
            ->select()
            ->toArray();
        if (count($clients) == 0) {
            return false;
    ...
    public function creditLimitInvoice($config)
    {
        if ($config["cron_credit_limit_invoice_unpaid_email"] > 0) {
            ...
            $host = \think\Db::name("invoices")
                ->alias("b")
                ->field("b.id,c.email,c.phone_code,c.phonenumber,b.total,d.suffix,b.uid")
                ->leftJoin("clients c", "b.uid=c.id")
                ->leftJoin("currencies d", "d.id = c.currency")
                ->withAttr("total", function ($value, $data) {
                    return $value . $data["suffix"];
                ...
                ->where("b.status", "Unpaid")
                ->where("b.due_time", ">=", strtotime(date("Y-m-d")) + $day * 24 * 3600)
                ->where("b.due_time", "<=", strtotime(date("Y-m-d")) + $day * 24 * 3600 + 86400 - 1)
                ->where("b.delete_time", 0)
                ->where("b.type", "credit_limit")
                ->select()
                ->toArray();
            if (!empty($host[0])) {
                foreach ($host as $vv) {
                    if (!cancelRequest($vv["id"])) {
                        ...
                        $sms = new \app\common\logic\Sms();
                        $client = check_type_is_use(
                            $message_template_type[strtolower("credit_limit_invoice_notice")],
                            $vv["uid"],
                            $sms,
                        );
                        if ($client) {
                        ...
                        if ($result) {
                            active_log_final(
                                "信用额账单未付款提醒 -  User ID:" . $vv["uid"] . "发送邮件成功",
                                $vv["uid"],
                                5,
                            );
                            $this->ad_log(10, 0, 1);
                            ...
                            active_log_final(
                                "信用额账单未付款提醒 -  User ID:" . $vv["uid"] . "发送邮件失败",
                                $vv["uid"],
                                5,
                            );
                        }
    ...
    private function creditLimitInvoiceDueSend($config, $times = 0)
    {
        ...
        if (!empty($host[0])) {
            ...
            foreach ($host as $v) {
                ...
                $sms = new \app\common\logic\Sms();
                $client = check_type_is_use(
                    $message_template_type[strtolower("credit_limit_invoice_payment_reminder")],
                    $v["uid"],
                    $sms,
                );
                if ($client) {
    ...
    public function creditLimit($config)
    {
        ...
        $shd_credit_limit_liquidated_damages_percent =
            configuration("shd_credit_limit_liquidated_damages_percent") ?? 0;
        if ($shd_credit_limit_liquidated_damages == 1) {
            $invoices = \think\Db::name("invoices")
                ->alias("b")
                ->field("b.id,b.total,b.uid,c.amount")
                ->leftJoin("invoice_items c", "c.invoice_id=b.id")
                ->where("b.status", "Unpaid")
                ->where("b.due_time", ">=", 0)
                ->where("b.due_time", "<=", time())
                ->where("b.delete_time", 0)
                ->where("b.type", "credit_limit")
            ...
            if (count($invoices) > 0) {
                foreach ($invoices as $key => $value) {
                    \think\Db::name("invoice_items")->insertGetId([
                        "invoice_id" => $value["id"],
                        "uid" => $value["uid"],
                        "type" => "liquidated_damages",
                        "rel_id" => 0,
                        "description" => date("Y-m-d") . "逾期违约金",
                        "amount" => round(($value["amount"] * $shd_credit_limit_liquidated_damages_percent) / 100, 2),
                    ]);
                    \think\Db::name("invoices")
                        ->where("id", $value["id"])
                        ->update([
                            "subtotal" =>
                                $value["total"] +
                                round(($value["amount"] * $shd_credit_limit_liquidated_damages_percent) / 100, 2),
                            "total" =>
                                $value["total"] +
                                round(($value["amount"] * $shd_credit_limit_liquidated_damages_percent) / 100, 2),
                        ]);
        ...
        if ($time > 0) {
            $invoices = \think\Db::name("invoices")
                ...
                ->where("b.status", "Unpaid")
                ->where("b.due_time", ">=", 0)
                ->where("b.due_time", "<=", time() - $time * 24 * 3600)
                ->where("b.delete_time", 0)
                ->where("b.type", "credit_limit")
                ->select()
                ->toArray();
            if (count($invoices) > 0) {
                $host = \think\Db::name("host")
                    ->alias("b")
                    ->field("b.id,b.uid,b.domain,b.nextinvoicedate,e.name")
                    ->leftJoin("orders c", "c.id = b.orderid")
                    ->leftJoin("invoices d", "d.id=c.invoiceid")
                    ->leftJoin("products e", "e.id = b.productid")
                    ->whereIn("b.domainstatus", "Active")
                    ->whereIn("d.invoice_id", array_column($invoices, "id"))
                    ->select()
                    ->toArray();
                $reason = lang("SERVICE_HAS_EXPIRED");
                if (!empty($host[0])) {
                    foreach ($host as $v) {
                        ...
                        if ($result["status"] == 200) {
                            ...
                            $data_i[
                                "description"
                            ] = "信用额账单未支付 - 暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                            if ($is_zjmf) {
                                $logic_run_map->saveMap($data_i, 0, 400, 2);
                            }
                            if (!$is_zjmf) {
                        ...
                        $sms = new \app\common\logic\Sms();
                        $client = check_type_is_use(
                            $message_template_type[strtolower("credit_limit_invoice_payment_reminder_host_suspend")],
                            $v["uid"],
                            $sms,
                        );
                        if ($client) {
                            $params = [
                                "product_name" => $v["name"],
                                "hostname" => $v["domain"],
                                "product_end_time" => date("Y-m-d H:i:s", $v["nextinvoicedate"]),
                                "product_terminate_time" => date("Y-m-d H:i:s", $v["nextinvoicedate"]),
                            ];
                            $sms->sendSms(
                                $message_template_type[
                                    strtolower("credit_limit_invoice_payment_reminder_host_suspend")
                                ],
                                $client["phone_code"] . $client["phonenumber"],
                                $params,
                                false,
                        ...
                        if ($result["status"] == 200) {
                            active_log_final(
                                "信用额账单未支付产品到期暂停 - 暂停产品 Host ID:" . $v["id"] . "成功",
                                $v["uid"],
                                5,
                            );
                        } else {
                            active_log_final(
                                "信用额账单未支付产品到期暂停 - 暂停产品 Host ID:" .
                                    $v["id"] .
                                    "失败,失败原因:" .
                                    $result["msg"],
                                $v["uid"],
    ...
    public function contractHostSuspended()
    {
        ...
        foreach ($contracts as $contract) {
            $signed = \think\Db::name("contract_pdf")
            ...
            if ($contract["base"]) {
                ...
                if ($contract["suspended_type"] == "suspended" && !empty($unsigned)) {
                    $hosts = \think\Db::name("host")
                        ->whereIn("uid", $unsigned)
                        ->where("domainstatus", "Active")
                        ->where("regdate", "<", time() - $day * 24 * 3600)
                        ->select()
                        ->toArray();
                }
            } else {
            ...
            if (!empty($hids)) {
                foreach ($hosts as $v) {
                    ...
                    if ($result["status"] == 200) {
                        ...
                        active_log_final(
                            "强制合同未签订暂停 - 暂停产品 Host ID:" . $v["id"] . "失败,失败原因:" . $result["msg"],
                            $v["uid"],
                            5,
                        );
                        $data_i["host_id"] = $v["id"];
