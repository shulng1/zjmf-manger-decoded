...
class CreditLimitController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $user = \think\Db::name("clients")
            ->alias("a")
            ->field(
                "username,phonenumber,email,is_open_credit_limit,credit_limit,repayment_date,bill_generation_date,bill_repayment_period,credit_limit_create_time,b.prefix,b.suffix",
            )
        ...
        $user["amount_to_be_settled"] = \think\Db::name("invoices")
            ->where("status", "Paid")
        ...
        $user["credit_limit_balance"] = number_format(
            $user["credit_limit"] - $user["credit_limit_used"] > 0
                ? $user["credit_limit"] - $user["credit_limit_used"]
                : 0,
            2,
            ".",
            "",
        ...
        $user["this_month_bill"] = \think\Db::name("invoices")
            ->where("type", "credit_limit")
            ->where("create_time", ">=", strtotime(date("Y-m")))
            ->where("is_delete", 0)
            ->where("credit_limit_prepayment", 0)
            ->where("uid", $uid)
        ...
        $credit_limit_config["shd_credit_limit_bill_generation_date"] = configuration(
            "shd_credit_limit_bill_generation_date",
        );
        $credit_limit_config["shd_credit_limit_bill_repayment_period"] = configuration(
            "shd_credit_limit_bill_repayment_period",
        );
        $credit_limit_config["shd_credit_limit_liquidated_damages"] = configuration(
            "shd_credit_limit_liquidated_damages",
        );
        $credit_limit_config["shd_credit_limit_liquidated_damages_percent"] = configuration(
    ...
    public function log()
    {
        ...
        $res = \think\Db::name("credit_limit")
            ->alias("a")
            ->field("a.id,a.uid,a.create_time,a.description,type,a.ip,b.user_login,b.user_nickname")
            ->leftJoin("user b", "a.handle_id = b.id")
            ->where("a.uid", $uid)
            ->where($where)
            ->withAttr("type", function ($value, $data) use ($en, $cn) {
    ...
    public function save()
    {
        ...
        $data = [
            ...
            "bill_generation_date" => $bill_generation_date > 0 ? $bill_generation_date : 0,
            "bill_repayment_period" => $bill_repayment_period > 0 ? $bill_repayment_period : 0,
            "credit_limit_create_time" => time(),
        ];
        $flag = true;
        \think\Db::startTrans();
        try {
    ...
    public function update()
    {
        ...
        try {
            if (!is_null($credit_limit)) {
                ...
                if (!is_null($repayment_date)) {
                    ...
                    Db("invoices")
                        ->where("uid", $uid)
                        ->where("type", "credit_limit")
                        ->where("due_time", $cli["repayment_date"])
                        ->update(["due_time" => $data["repayment_date"]]);
                    $type = "Change Repayment Date";
                    $msg = "还款日修改成功";
                } else {
                    $description = sprintf(
                        "账单生成日%s调整到%s,还款期限%s调整到%s",
                        $cli["bill_generation_date"],
                        $data["bill_generation_date"],
                        $cli["bill_repayment_period"],
                        $data["bill_repayment_period"],
                    );
            ...
            Db("clients")->where("id", $uid)->update($data);
            Db("credit_limit")->insertGetId([
                "description" => $description,
                "type" => $type,
                "uid" => $uid,
                "create_time" => time(),
                "handle_id" => cmf_get_current_admin_id(),
                "ip" => get_client_ip6(),
            ]);
            \think\Db::commit();
        } catch (\Exception $e) {
    ...
    public function list()
    {
        ...
        $invoices = \think\Db::name("invoices")
            ...
            ->leftJoin("currencies cu", "cu.id = c.currency")
            ->field(
                "i.id,i.create_time,i.due_time,i.paid_time,i.subtotal,i.subtotal as sub,i.total,i.credit,i.payment,i.status,cu.prefix,cu.suffix,i.type,i.use_credit_limit,c.username,i.uid",
            )
            ->where($where)
            ->where("i.use_credit_limit", 1)
        ...
        $invoice_items = \think\Db::name("invoice_items")
            ->alias("t")
            ->field("p.name,t.type,t.invoice_id,t.id,t.description")
            ->leftJoin("host h", "t.rel_id = h.id")
            ->leftJoin("products p", "p.id = h.productid")
            ->whereIn("invoice_id", array_column($invoices, "id"))
        ...
        foreach ($invoice_items as $key => $value) {
            $desc =
                isset($type_arr[$value["type"]]) && !is_null($value["name"])
                    ? "【" . $type_arr[$value["type"]] . "】" . $value["name"]
                    : $value["description"];
    ...
    public function userInvoice()
    {
        ...
        $invoices = \think\Db::name("invoices")
            ...
            ->leftJoin("currencies cu", "cu.id = c.currency")
            ->field(
                "i.id,i.create_time,i.due_time,i.paid_time,i.subtotal,i.subtotal as sub,i.total,i.credit,i.payment,i.status,cu.prefix,cu.suffix,i.type,i.use_credit_limit,c.username,i.uid,i.credit_limit_prepayment",
            )
            ->where($where)
            ->where("i.type", "credit_limit")
            ->withAttr("payment", function ($value, $data) use ($gateways) {
        ...
        return jsonrule([
            ...
            "count" => $count,
            "invoices" => $invoices,
            "invoice_status" => config("invoice_payment_status"),
            "credit_limit_invoice_status" => $status,
        ]);
    }
    /**
     * @title 信用额账单列表
    ...
    public function creditLimitInvoice()
    {
        ...
        $invoices = \think\Db::name("invoices")
            ...
            ->leftJoin("products p", "p.id = h.productid")
            ->field(
                "i.id,i.create_time,i.due_time,i.paid_time,i.subtotal,i.subtotal as sub,i.total,i.credit,i.payment,i.status,cu.prefix,cu.suffix,i.type,i.use_credit_limit,p.name",
            )
            ->where($where)
            ->where("i.invoice_id", $invoice_id)
            ->withAttr("payment", function ($value, $data) use ($gateways) {
    ...
    public function get_search($param)
    {
        ...
        if (isset($param["date"])) {
            ...
            $end =
                mktime(0, 0, 0, $date[1] + 1 > 12 ? 1 : $date[1] + 1, 1, $date[1] + 1 > 12 ? $date[0] + 1 : $date[0]) -
                1;
    ...
    public function clientList()
    {
        ...
        if (
            ...
            $order != "amount_to_be_settled" &&
            $order != "credit_limit_unpaid" &&
            $order != "credit_limit_balance"
        ) {
            $order = "a." . $order;
        }
        ...
        if (!empty($payment_status)) {
            if ($payment_status == "Prepayment") {
                $uid = \think\Db::name("invoices")
                    ->where("type", "credit_limit")
                    ->where("create_time", ">=", strtotime(date("Y-m")))
                    ->where("is_delete", 0)
                    ->where("status", "Paid")
                    ->where("credit_limit_prepayment", 1)
                    ->order("create_time", "desc")
                    ->group("uid")
                    ->column("uid");
        ...
        if (
            ...
            $list = \think\Db::name("clients")
                ->alias("a")
                ->field("a.id,a.username,a.phonenumber,a.email,a.is_open_credit_limit,a.credit_limit,b.prefix,b.suffix")
                ->leftJoin("currencies b", "a.currency = b.id")
                ->where($fun)
                ->order($order, $sort)
                ->page($page)
        ...
        foreach ($list as $key => &$value) {
            ...
            $value["credit_limit_balance"] = number_format(
                $value["credit_limit"] - $value["credit_limit_used"] > 0
                    ? $value["credit_limit"] - $value["credit_limit_used"]
                    : 0,
                2,
                ".",
                "",
            );
            $this_month_bill = \think\Db::name("invoices")
                ->where("type", "credit_limit")
                ->where("create_time", ">=", strtotime(date("Y-m")))
                ->where("is_delete", 0)
                ->where("uid", $value["id"])
    ...
    public function getConfig()
    {
        ...
        $res["shd_credit_limit_liquidated_damages_percent"] = configuration(
            "shd_credit_limit_liquidated_damages_percent",
        );
        return jsonrule($res);
    }
    ...
    public function postConfig()
    {
        if ($this->request->isPost()) {
            ...
            updateConfiguration(
                "shd_credit_limit_bill_generation_date",
                $params["shd_credit_limit_bill_generation_date"],
            );
            ...
            updateConfiguration(
                "shd_credit_limit_bill_repayment_period",
                $params["shd_credit_limit_bill_repayment_period"],
            );
            ...
            updateConfiguration(
                "shd_credit_limit_liquidated_damages",
                $params["shd_credit_limit_liquidated_damages"] ?? 0,
            );
            ...
            if ($company_name != $params["shd_credit_limit_liquidated_damages_percent"]) {
                $dec .=
                    ...
                    $company_name .
                    "改为" .
                    $params["shd_credit_limit_liquidated_damages_percent"];
            }
            updateConfiguration(
                "shd_credit_limit_liquidated_damages_percent",
                $params["shd_credit_limit_liquidated_damages_percent"],
