...
class DcimCloudController extends AdminBaseController
{
    ...
    public function editServer()
    {
        ...
        $server_info = \think\Db::name("servers")
            ->alias("a")
            ->field(
                "a.id,a.gid,a.name,a.hostname,a.username,a.password,a.port,a.secure,a.disabled,a.accesshash,b.reinstall_times,b.buy_times,b.reinstall_price,b.auth,b.bill_type,b.area,b.id dcim_server_id",
            )
            ->leftJoin("dcim_servers b", "a.id=b.serverid")
            ->where("a.server_type", "dcimcloud")
            ->where("a.id", $id)
            ->find();
        ...
        if ($params["account_type"] != $old_accesshash["account_type"]) {
            $dec .=
                " - 账号类型" .
                $account_type[$old_accesshash["account_type"]] .
                "改为" .
    ...
    public function serverDetail($id)
    {
        $data = \think\Db::name("servers")
            ->alias("a")
            ->field(
                "a.id,a.name,a.hostname,a.username,a.password,a.port,a.secure,a.disabled,a.accesshash,b.reinstall_times,b.buy_times,b.reinstall_price,b.area",
            )
            ->leftJoin("dcim_servers b", "a.id=b.serverid")
            ->where("a.server_type", "dcimcloud")
            ->where("a.id", $id)
    ...
    public function serverList()
    {
        ...
        $data = \think\Db::name("servers")
            ->alias("a")
            ->field(
                "a.id,a.name,a.hostname,a.username,a.password,a.port,a.secure,a.disabled,a.accesshash,count(DISTINCT b.id) server_num,c.reinstall_times,c.buy_times,c.reinstall_price,c.api_status,d.api_type,count(DISTINCT d.id) product_num",
            )
            ...
            ->leftJoin("products d", "a.gid=d.server_group AND d.api_type not in ('zjmf_api')")
            ->where("a.name LIKE '%{$search}%' OR a.hostname LIKE '%{$search}%'")
            ->where("a.server_type", "dcimcloud")
            ->group("a.id")
            ->order($orderby, $sort)
