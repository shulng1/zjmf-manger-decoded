...
class ConfigMessageController extends AdminBaseController
{
    ...
    public function templateList()
    {
        ...
        $order_method =
            isset($data["order_method"]) && !empty($data["order_method"])
                ? strtolower(trim($data["order_method"]))
                : "asc";
        $sms_operator = \think\Db::name("configuration")->where("setting", "sms_operator")->value("value");
        $smsoperator =
            isset($data["sms_operator"]) && !empty($data["sms_operator"])
                ? strtolower(trim($data["sms_operator"]))
                : $sms_operator;
        $count = \think\Db::name("message_template")
            ->where("sms_operator", $smsoperator)
            ...
            ->count();
        $templates = \think\Db::name("message_template")
            ->where("sms_operator", $smsoperator)
            ->where(function (\think\db\Query $query) use ($data) {
                if (!empty($data["template_id"])) {
                    $template_id = strtolower(trim($data["template_id"]));
    ...
    protected function updateStatus($smsoperator)
    {
        $message_template = \think\Db::name("message_template")
            ->field("template_id,range_type,status")
            ->where("sms_operator", $smsoperator)
            ->select();
    ...
    public function createTemplatePage()
    {
        ...
        return jsonrule([
            ...
            "combine" => $argsarray,
            "msg" => lang("SUCCESS MESSAGE"),
            "data" => config("message_template_type"),
            "allowedSmsOperator" => $this->sms_operator_cn1(),
        ]);
    }
    /**
     * @title 创建模板页面可用参数
    ...
    public function getTemplateDesc()
    {
        ...
        return jsonrule([
            ...
            "descriptionTemplate" => $descriptionTemplate ?: ["cn" => "", "global" => "", "cnpro" => ""],
            "msg" => lang("SUCCESS MESSAGE"),
        ]);
    }
    /**
    ...
    public function updateTemplate()
    {
        ...
        $temp = \think\Db::name("message_template")
            ->field("id,template_id,range_type,sms_operator,title,content,remark,status")
            ->where("id", $id)
            ->find();
                ...
                if (!empty($template)) {
                    ...
                    if ($result["status"] == "success") {
                        ...
                        if ($template["template_id"] && empty($result["template"]["template_status"])) {
                            ...
                            $template["status"] = $result["template"]["template_status"]
                                ? $result["template"]["template_status"]
                                : 0;
                        }
            ...
            foreach ($ids as $id) {
                ...
                if (!empty($temp)) {
                    try {
                        ...
                        \think\Db::name("message_template_link")
    ...
    public function SetSmsTemplate()
    {
        ...
        if ($range_type == 0) {
            $data["shd_allow_sms_send"] = configuration("shd_allow_sms_send") ? 1 : 0;
            $sms_operator = !empty($param["sms_operator"])
                ? strtolower(trim($param["sms_operator"]))
                : configuration("sms_operator");
            ...
            $sms_operator = !empty($param["sms_operator"])
                ? strtolower(trim($param["sms_operator"]))
                : configuration("sms_operator_global");
        ...
        $select_temps = \think\Db::name("message_template_link")
        ...
        return json([
            "status" => 200,
            "msg" => lang("SUCCESS MESSAGE"),
            "message_template_lists" => $message_template_lists,
            "range_type" => $range_type,
            "templates" => $templatesfilter,
            "select_temp" => $select_temps_filter,
            "sms_setting" => $data,
        ]);
    }
    /**
    ...
    public function SetSmsTemplatePost()
    {
        if ($this->request->isPost()) {
            ...
            $invoice_payment_confirmation = isset($param["invoice_payment_confirmation"])
                ? $param["invoice_payment_confirmation"]
                : 0;
            $second_renew_product_reminder = isset($param["second_renew_product_reminder"])
                ? $param["second_renew_product_reminder"]
                : 0;
            $renew_product_reminder = isset($param["renew_product_reminder"]) ? $param["renew_product_reminder"] : 0;
            $third_invoice_payment_reminder = isset($param["third_invoice_payment_reminder"])
                ? $param["third_invoice_payment_reminder"]
                : 0;
            $second_invoice_payment_reminder = isset($param["second_invoice_payment_reminder"])
                ? $param["second_invoice_payment_reminder"]
                : 0;
            $first_invoice_payment_reminder = isset($param["first_invoice_payment_reminder"])
                ? $param["first_invoice_payment_reminder"]
                : 0;
            $new_order_notice = isset($param["new_order_notice"]) ? $param["new_order_notice"] : 0;
            $admin_product_suspension_faild = isset($param["admin_product_suspension_faild"])
                ? $param["admin_product_suspension_faild"]
                : 0;
            ...
            $zjmf_dcim_rebuild_system_success = isset($param["zjmf_dcim_rebuild_system_success"])
                ? $param["zjmf_dcim_rebuild_system_success"]
                : 0;
            $service_termination_notification = isset($param["service_termination_notification"])
                ? $param["service_termination_notification"]
                : 0;
            $service_unsuspension_notification = isset($param["service_unsuspension_notification"])
                ? $param["service_unsuspension_notification"]
                : 0;
            $service_suspension_notification = isset($param["service_suspension_notification"])
                ? $param["service_suspension_notification"]
                : 0;
            $zjmf_dcim_product_welcome = isset($param["zjmf_dcim_product_welcome"])
                ? $param["zjmf_dcim_product_welcome"]
                : 0;
            $support_ticket_auto_close_notification = isset($param["support_ticket_auto_close_notification"])
                ? $param["support_ticket_auto_close_notification"]
                : 0;
            ...
            $credit_limit_invoice_notice = isset($param["credit_limit_invoice_notice"])
                ? $param["credit_limit_invoice_notice"]
                : 0;
            $credit_limit_invoice_payment_reminder = isset($param["credit_limit_invoice_payment_reminder"])
                ? $param["credit_limit_invoice_payment_reminder"]
                : 0;
            $credit_limit_invoice_payment_reminder_host_suspend = isset(
                $param["credit_limit_invoice_payment_reminder_host_suspend"],
            )
                ? $param["credit_limit_invoice_payment_reminder_host_suspend"]
                : 0;
            ...
            try {
                ...
                if (!empty($credit_limit_invoice_payment_reminder_host_suspend[0])) {
                    $this->updateOrInsert(
                        ...
                        $range_type,
                        36,
                        $credit_limit_invoice_payment_reminder_host_suspend,
                    );
                }
    ...
    {
        $exist = \think\Db::name("message_template_link")
            ->where("sms_operator", $sms_operator)
            ->where("range_type", $range_type)
            ->where("type", $type)
            ->find();
        if (!empty($exist)) {
            $exist1 = \think\Db::name("message_template")
                ->field("title,id")
                ->where("id", $exist["sms_temp_id"])
                ->find();
            $res = \think\Db::name("message_template_link")
                ->where("sms_operator", $sms_operator)
                ->where("range_type", $range_type)
                ->where("type", $type)
                ->update(["sms_temp_id" => $temp_id[0], "is_use" => $temp_id[1]]);
            if ($temp_id != $exist1["id"]) {
                active_log(
                    sprintf(
                        $this->lang["ConfigMessage_admin_SetSmsTemplatePost"],
                        $exist["id"],
                        $temp_id[0],
                        $exist1["title"],
                    ),
            ...
            $res = \think\Db::name("message_template_link")->insertGetId([
                "type" => $type,
                "sms_temp_id" => $temp_id[0],
                "range_type" => $range_type,
    ...
    public function testMessageTemplate()
    {
        if ($this->request->isPost()) {
            ...
            $temp = \think\Db::name("message_template")
                ->field("template_id,range_type,content,status,sms_operator")
                ->where("id", $id)
                ->find();
            if (empty($temp)) {
            ...
            $params = [
                ...
                "mobile" => $Sms->mobile86($temp["range_type"], $phone),
            ];
    ...
    public function mobiletemplateList()
    {
        ...
        $param = $this->request->param();
        $templates = \think\Db::name("message_template")
            ->field("id,title,template_id,range_type,content,status,sms_operator")
            ->where("sms_operator", $sms_operator)
            ->where("status", 2)
            ->select();
        $templatesfilter = [];
        ...
        foreach ($templatesfilter as $key => $val) {
            $message_template_link = \think\Db::name("message_template_link")
                ->where("sms_temp_id", $val["id"])
                ->where("is_use", 1)
                ->find();
            ...
            if (
                $mty != "default_product_welcome" &&
                $mty != "service_termination_notification" &&
                $mty != "host_suspend" &&
                $mty != "service_unsuspension_notification"
            ) {
    ...
    public function emailtemplateList()
    {
        ...
        $host = \think\Db::name("host")
            ...
            ->field("p.type")
            ->find();
        $results = \think\Db::name("email_templates")
            ->field("id,type,disabled,name,custom")
        ...
        if ($this->request->isPost()) {
            ...
            if (!empty($param["msgtype"]) && $param["msgtype"]) {
                ...
                $host = \think\Db::name("host")
                    ...
                    ->field("a.port")
                    ->field(
                        "a.id,a.uid,a.productid,a.domainstatus,a.regdate,b.welcome_email,b.type,c.email,a.billingcycle,b.pay_type,b.name,a.nextduedate,a.billingcycle,a.dedicatedip,a.username,a.password,a.os,a.assignedips,a.create_time,a.nextinvoicedate,a.domain,a.suspendreason",
                    )
                    ->leftJoin("products b", "a.productid=b.id")
                    ->leftJoin("clients c", "a.uid=c.id")
                    ->where("a.id", $param["hid"])
                    ->find();
                ...
                $newvar = [
                    ...
                    "product_passwd" => cmf_decrypt($host["password"]),
                    "product_dcimbms_os" => $host["os"],
                    "product_addonip" => $host["assignedips"],
                    "product_first_time" => date("Y-m-d H:i:s", $host["create_time"]),
                    "product_end_time" => $host["nextduedate"] > 0 ? date("Y-m-d H:i:s", $host["nextduedate"]) : "",
                    "product_binlly_cycle" => $billing_cycle[$host["billingcycle"]],
                    "hostname" => $host["domain"],
                    "description" => $host["suspendreason"],
                    "product_terminate_time" => date("Y-m-d H:i:s", time()),
                ];
    ...
    private function sendMessageSms($param, $data, $user)
    {
        ...
        $message_template_link = \think\Db::name("message_template_link")
            ->where("sms_temp_id", $param["msgid"])
            ->where("sms_operator", $data["sms"]["type"])
            ->where("is_use", 1)
            ->find();
        if (empty($message_template_link)) {
        ...
        if ($result["status"] != 200) {
            throw new \Exception(
                $param["ematype"]
                    ? "邮件未发送，" . lang("短信发送失败,失败原因:") . $result["msg"]
                    : lang("短信发送失败,失败原因:") . $result["msg"],
            );
        }
    }
