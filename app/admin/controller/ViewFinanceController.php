...
class ViewFinanceController extends ViewAdminBaseController
{
    ...
    public function transactions(\think\Request $request)
    {
        ...
        $paymentsList = [
            ["label" => "", "value" => ""],
            ["label" => "付款已收到", "value" => "amount_in"],
            ["label" => "付款已发出", "value" => "amount_out"],
        ];
        $result["seachList"] = [
            ["type" => "select", "label" => "显示", "name" => "payments", "content" => "", "list" => $paymentsList],
            ["type" => "text", "label" => "描述", "name" => "description", "content" => ""],
            ["type" => "text", "label" => "金额", "name" => "amount", "content" => ""],
            ["type" => "text", "label" => "付款流水号", "name" => "turnover", "content" => ""],
            ["type" => "date", "label" => "开始时间", "name" => "startTime", "content" => ""],
            ["type" => "date", "label" => "结束时间", "name" => "endTime", "content" => ""],
            ["type" => "select", "label" => "支付方式", "name" => "payment", "content" => "", "list" => $gatewa],
            ["type" => "select", "label" => "销售", "name" => "sale_id", "content" => "", "list" => $salelist],
        ];
        ...
        $result["modalList"] = [
            ...
            ["type" => "text", "label" => "付款流水号", "name" => "turnover", "content" => ""],
            ["type" => "select", "label" => "账单编号", "name" => "billNumber", "content" => ""],
            ["type" => "select", "label" => "支付方式", "name" => "payment", "content" => "", "list" => $gateway],
            ["type" => "text", "label" => "收入", "name" => "amount_in", "content" => ""],
            ["type" => "text", "label" => "支出", "name" => "amount_out", "content" => ""],
            ["type" => "select", "label" => "货币类型", "name" => "currency", "content" => "", "list" => $currencyList],
            [
                "type" => "checkbox",
                "label" => "预付款",
                "name" => "prepayments",
                "content" => "",
                "list" => [["label" => "添加到用户余额", "value" => "1"]],
            ],
        ];
        $param = $request->param();
    ...
    public function invoices(\think\Request $request)
    {
        ...
        $result["seachList"] = [
            ["type" => "select", "label" => "客户", "name" => "username", "content" => "", "list" => $list],
            ["type" => "towDate", "label" => "账单生成日", "name" => "createTime", "content" => ""],
            ["type" => "towDate", "label" => "账单逾期日", "name" => "dueTime", "content" => ""],
            ["type" => "select", "label" => "支付方式", "name" => "payment", "content" => "", "list" => $gateway],
            ["type" => "date", "label" => "账单支付日", "name" => "paidTime", "content" => ""],
            ["type" => "select", "label" => "账单类型", "name" => "type", "content" => "", "list" => $type],
            ["type" => "select", "label" => "销售", "name" => "sale_id", "content" => "", "list" => $salelist],
        ];
        $result["tip"] = "详细统计收入、支出记录及总额。";
        $result["btnText"] = "添加交易流水";
    ...
    public function withdrawdeposits(\think\Request $request)
    {
        ...
        $result["seachList"] = [
            ["type" => "text", "label" => "用户名", "name" => "user_nickname", "content" => ""],
            [
                "type" => "select",
                "label" => "提现方式",
                "name" => "status",
    ...
    public function configgateways(\think\Request $request)
    {
        ...
        $result["tip"] =
            "选择支付网关点击安装，填写接口相关配置信息后，用户在前台可选择支付方式进行结算付款。<a href='#'>查看帮助文档</a>";
    ...
    {
        $result["navTabs"] = [
            ["label" => "激活的优惠码", "name" => "active"],
            ["label" => "过期的优惠码", "name" => "expired"],
            ["label" => "所有优惠码", "name" => "all"],
        ];
        $result["titleList"] = [
            "ID",
            "优惠码",
        ...
        $result["tip"] =
            "添加优惠码可对不同商品进行对应折扣，并可设置优惠码到期时间及使用次数，便于管理。<a href='#'>帮助文档</a>";
        ...
        foreach ($data as $key => &$item) {
            ...
            $item["expirationTime"] =
                $item["expiration_time"] == "-"
                    ? $item["expiration_time"]
                    : date("Y-m-d H:i:s", $item["expiration_time"]);
    ...
    public function configfund(\think\Request $request)
    {
        ...
        $result["modalList"] = [
            [
                "type" => "checkbox",
                "label" => "是否启用充值",
                "name" => "addfunds_enabled",
                "content" => "",
                "tip" => "选中择,客户可以在用户中心添加余额至账户中",
            ],
            [
                "type" => "text",
                "label" => "最小金额",
                "name" => "addfunds_minimum",
                "content" => "",
                "tip" => "单笔充值的最小金额",
            ],
            [
            ...
            [
                "type" => "text",
                "label" => "最高余额",
                "name" => "addfunds_maximum_balance",
                "content" => "",
                "tip" => "输入客户可以添加进余额的最大金额",
            ],
            [
                "type" => "checkbox",
                "label" => "需要已激活的订单",
                ...
                "content" => "",
                "tip" =>
                    "允许添加余额之前必须有一个已激活的订单(用于防止欺诈，意味着在允许添加余额之前管理员必须手动审核用户并批准订单)",
            ],
        ];
        ...
        if ($result["seleTab"] == "cw") {
            $result["modalList"] = [
                [
                    "type" => "checkbox",
                    "label" => "降级退款至余额",
                    "name" => "upgrade_down_product_config",
                    "content" => "",
                ],
                [
                    "type" => "checkbox",
                    "label" => "起始账单号自定义",
                    "name" => "allow_custom_invoice_id",
                    "content" => "",
                ],
            ];
        }
    ...
    public function configreceipt(\think\Request $request)
    {
        ...
        $result["modalList"] = [
            ...
            ["type" => "text", "label" => "发票费率(%) ", "name" => "rate", "content" => "", "required" => true],
        ];
