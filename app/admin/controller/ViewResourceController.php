...
class ViewResourceController extends ViewAdminBaseController
{
    ...
    public function munualresource(\think\Request $request)
    {
        ...
        $result["tip"] =
            "详细统计收入、支出记录及总额。<a href='https://bbs.idcsmart.com/forum.php?mod=viewthread&tid=354&fromuid=39' target='_blank'>帮助文档</a>";
        if ($result["seleTab"] == "zy") {
            ...
            $result["seachList"] = [
                ["type" => "text", "label" => "主IP", "name" => "in_ip", "content" => ""],
                ["type" => "select", "label" => "上游", "name" => "pid", "content" => "", "list" => $list],
            ];
            $result["titleList"] = [
                "重装/破解",
                "电源",
            ...
            foreach ($res["data"] as &$item) {
                $item["names"] = str_replace(
                    "#/customer-view/product-innerpage",
                    "/admin/clientsviewservices",
                    $item["names"],
                );
