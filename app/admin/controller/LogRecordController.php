...
class LogRecordController extends AdminBaseController
{
                ...
                if (!empty($name)) {
                    foreach ($name as $k => $v) {
                        ...
                        if ($v == "Invoice ID") {
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" target=\"blank\" href=\"#/edit-configurable-option-group?groupId=" .
                                $pco["gid"] .
                                "\"><span class=\"el-link--inner\" style=\"display: block;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/add-product-group?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;\">" .
                                $str .
                                "</span></a>";
    ...
    public function getAdminLog(\think\Request $request)
    {
        ...
        $count = \think\Db::name("admin_log")
            ...
            ->count();
        $log_list = \think\Db::name("admin_log")
            ->where(function (\think\db\Query $query) use ($param) {
                if (!empty($param["search_time"])) {
                    $start_time = strtotime(date("Y-m-d", $param["search_time"]));
                    $end_time = strtotime(date("Y-m-d", $param["search_time"])) + 86400;
            ...
            ->select()
            ->toArray();
        $user_list = \think\Db::name("admin_log")
    ...
    public function getNotifyLog(\think\Request $request)
    {
        ...
        $count = \think\Db::name("notify_log")
            ...
            ->count();
        $log_list = \think\Db::name("notify_log")
            ->where(function (\think\db\Query $query) use ($param) {
                if (!empty($param["search_time"])) {
                    $start_time = strtotime(date("Y-m-d", $param["search_time"]));
                    $end_time = strtotime(date("Y-m-d", $param["search_time"])) + 86400;
    ...
    public function getEmailLog()
    {
        ...
        $orderby = strval($param["orderby"]) ? strval($param["orderby"]) : "id";
        $sorting = $param["sorting"] ?? "DESC";
        $count = \think\Db::name("email_log")
            ->alias("a")
            ->field("a.id,a.to,a.create_time,a.subject,b.username,a.status,a.fail_reason,a.ip")
            ->leftJoin("clients b", "b.id = a.uid")
            ->where(function (\think\db\Query $query) use ($param) {
                if (!empty($param["uid"])) {
            ...
            ->count();
        $email_lists = \think\Db::name("email_log")
            ->alias("a")
            ->field("a.id,a.to,a.create_time,a.subject,b.username,a.status,a.fail_reason,a.ip,a.port")
            ->leftJoin("clients b", "b.id = a.uid")
            ->where(function (\think\db\Query $query) use ($param) {
                if (!empty($param["uid"])) {
                    $query->where("uid", $param["uid"])->where("a.is_admin", 0);
    ...
    public function getSmsLog()
    {
        ...
        $orderby = strval($param["orderby"]) ? strval($param["orderby"]) : "id";
        $sorting = $param["sorting"] ?? "DESC";
        $count = \think\Db::name("message_log")
            ->alias("a")
            ->field(
                "a.id,a.uid,a.create_time,a.content,a.fail_reason,a.status,a.phone,a.phone_code,b.username,a.ip,a.port",
            )
            ->leftJoin("clients b", "b.id = a.uid")
            ->where(function (\think\db\Query $query) use ($param) {
                if (!empty($param["uid"])) {
            ...
            ->count();
        $email_lists = \think\Db::name("message_log")
            ->alias("a")
            ->field(
                "a.id,a.uid,a.create_time,a.content,a.fail_reason,a.status,a.phone,a.phone_code,b.username,a.ip,a.port",
            )
    ...
    public function getSmsLogM()
    {
        ...
        $count = \think\Db::name("message_log")
            ->alias("a")
            ->field("a.id,a.uid,a.create_time,a.content,a.fail_reason,a.status,a.phone,a.phone_code,b.username,a.ip")
            ->leftJoin("clients b", "b.id = a.uid")
            ->where(function (\think\db\Query $query) use ($uid, $param) {
                $query->where("uid", $uid);
                if (!empty($param["search_time"])) {
                    $start_time = strtotime(date("Y-m-d", $param["search_time"]));
                    $end_time = strtotime(date("Y-m-d", $param["search_time"])) + 86400;
        ...
        $email_lists = \think\Db::name("message_log")
            ->alias("a")
            ->field("a.id,a.uid,a.create_time,a.content,a.fail_reason,a.status,a.phone,a.phone_code,b.username")
            ->leftJoin("clients b", "b.id = a.uid")
            ->where(function (\think\db\Query $query) use ($uid, $param) {
                $query->where("uid", $uid);
                if (!empty($param["search_time"])) {
        ...
        $user_list = \think\Db::name("message_log")
            ->alias("a")
            ->leftJoin("clients b", "b.id = a.uid")
    ...
    public function getSystemMessageLog(\think\Request $request)
    {
        ...
        $orderby = strval($param["orderby"]) ? strval($param["orderby"]) : "id";
        $sorting = $param["sorting"] ?? "DESC";
        $count = \think\Db::name("system_message")
            ->alias("sm")
            ->join("clients c", "c.id = sm.uid")
            ->where(function (\think\db\Query $query) use ($param) {
                if (intval($param["uid"])) {
            ...
            ->count();
        $list = \think\Db::name("system_message")
            ->alias("sm")
            ->field("sm.*,c.username,c.phonenumber,c.email")
            ->join("clients c", "c.id = sm.uid")
            ->where(function (\think\db\Query $query) use ($param) {
                if (intval($param["uid"])) {
                    $query->where("sm.uid", intval($param["uid"]));
    ...
    public function getApiLog()
    {
        ...
        $count = \think\Db::name("api_resource_log")
            ->alias("a")
            ->field("a.id,a.create_time,a.description,a.ip,b.username")
            ->leftJoin("clients b", "a.uid = b.id")
            ->where($where)
            ->count();
        $list = \think\Db::name("api_resource_log")
            ->alias("a")
            ->field("a.id,a.create_time,a.description,a.ip,b.username,a.port,a.uid")
            ->leftJoin("clients b", "a.uid = b.id")
            ->where($where)
        ...
        return jsonrule([
            "status" => 200,
            "msg" => lang("SUCCESS MESSAGE"),
            "data" => ["count" => $count, "list" => $list, "user" => $user],
        ]);
    }
    /**
     * @title 删除日志页面
