...
class ViewWorkorderController extends ViewAdminBaseController
{
    ...
    public function supportticket(\think\Request $request)
    {
        ...
        $result["seachList"] = [
            ...
            ["type" => "select", "label" => "部门", "name" => "dptid", "content" => ""],
            ["type" => "select", "label" => "状态", "name" => "status", "content" => ""],
            [
                "type" => "select",
                "label" => "优先级",
                "name" => "priority",
            ...
            ["type" => "text", "label" => "主题/内容", "name" => "content", "content" => ""],
            ["type" => "text", "label" => "工单编号", "name" => "tid", "content" => ""],
        ];
    ...
    public function configticketdepartments(\think\Request $request)
    {
        ...
        $result["tip"] =
            "这是您配置的支持工单部门。您输入的邮件地址将用于检测发送到该部门，所有该部门的邮件也将用此邮件地址发出。邮件管道（Email Piping）允许通过邮件回复或开启工单，并可以使用以下方式之一设置。";
    ...
    public function configticketstatuses(\think\Request $request)
    {
        ...
        $result["btnText"] = "添加";
        $result["tip"] =
            "在此处您可以自定义工单状态。无法删除或重命名 5 种默认状态：开启中、已回复、客户回复、已关闭、等待中。";
