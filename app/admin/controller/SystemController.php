...
class SystemController extends AdminBaseController
{
    ...
    public function getUpdateContent()
    {
        ...
        return jsonrule([
            "status" => 200,
            "data" => mb_convert_encoding(iconv("utf-8", "gbk//IGNORE", $str), "utf-8", "GBK"),
        ]);
    }
    /**
     * @title 系统信息
    ...
    public function getInfo()
    {
        ...
        $mysql_version = $mysql_version[0]["VERSION()"]
            ? str_replace("-log", "", $mysql_version[0]["VERSION()"])
            : "获取数据库版本失败";
        $auth_status = "Active";
        $auth_suspend_reason = "";
        $auth_app = [];
        ...
        $data = [
            ...
            "processor_identifier" => ini_get("memory_limit"),
            "system_root" => CMF_ROOT,
            "http_accept_language" => $_SERVER["HTTP_ACCEPT_LANGUAGE"] ?? "",
            "system_token" => \think\Db::name("configuration")->where("setting", "system_token")->value("value") ?? "",
            "install_version" => getZjmfVersion(),
            "mysql_version" => $mysql_version,
            "system_version_type" => configuration("system_version_type") ?? "stable",
            "zjmf_system_version_type_last" => configuration("zjmf_system_version_type_last") ?? "stable",
            "system_license" => configuration("system_license") ?? "",
            "auth_status" => $auth_status,
            "auth_suspend_reason" => $auth_suspend_reason,
            "auth_app" => $auth_app,
            "auth_due_time" => $auth_due_time,
            "service_due_time" => $service_due_time,
        ];
        return jsonrule(["status" => 200, "data" => $data]);
    }
    /**
    ...
    public function putLicense()
    {
        ...
        try {
            \think\Db::name("configuration")
                ->where("setting", "system_license")
                ->update(["value" => $license]);
            active_log("授权码修改成功");
            \think\Db::commit();
