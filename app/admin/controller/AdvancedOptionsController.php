...
class AdvancedOptionsController extends AdminBaseController
{
    ...
    public function page()
    {
        ...
        $options = \think\Db::name("product_config_options")
            ->alias("a")
            ->field("a.id,a.option_name,a.option_type")
        ...
        foreach ($options as &$option) {
            $sub_options = \think\Db::name("product_config_options_sub")
                ->field("id,option_name,qty_minimum,qty_maximum")
        ...
        $tmp = \think\Db::name("product_config_options_links")
            ->field("id,config_id,relation,sub_id")
    ...
    public function create()
    {
        ...
        if (isset($params["link"]) && !empty($params["link"])) {
            ...
            foreach ($link as $j => $h) {
                ...
                foreach ($h["sub_id"] as $jj => $hh) {
                    $tmp3 = \think\Db::name("product_config_options_sub")
                        ->where("config_id", $h["config_id"])
                        ->where("id", $jj)
                        ->find();
                    if (empty($tmp3)) {
                        return jsonrule(["status" => 400, "msg" => "参数错误"]);
                    ...
                    if (judgeQuantity($tmp2["option_type"])) {
                        ...
                        if (
                            $hh_qty_maximum < $hh_qty_minimum ||
                            $hh_qty_minimum < $tmp3["qty_minimum"] ||
                            $tmp3["qty_maximum"] < $hh_qty_maximum
                        ) {
                            return jsonrule(["status" => 400, "msg" => "参数错误"]);
                        }
                    }
                ...
                foreach ($tmp_result as $jjj => $hhh) {
                    ...
                    foreach ($hhh["sub_id"] as $jjjj => $hhhh) {
                        $tmp55 = \think\Db::name("product_config_options_sub")
                            ->where("config_id", $hhh["config_id"])
                            ->where("id", $jjjj)
                            ->find();
                        if (empty($tmp55)) {
                            return jsonrule(["status" => 400, "msg" => "参数错误"]);
                        ...
                        if (judgeQuantity($tmp5["option_type"])) {
                            ...
                            if (
                                $hhhh_qty_maximum < $hhhh_qty_minimum ||
                                $hhhh_qty_minimum < $tmp55["qty_minimum"] ||
                                $tmp55["qty_maximum"] < $hhhh_qty_maximum
                            ) {
                                return jsonrule(["status" => 400, "msg" => "参数错误"]);
                            }
                        }
                ...
                if (isset($h["new_result"]) && is_array($h["new_result"]) && !empty($h["new_result"])) {
                    ...
                    foreach ($new_result_tmp as $jjjjj => $hhhhh) {
                        ...
                        foreach ($hhhhh["sub_id"] as $j6 => $h6) {
                            $new_tmp2 = \think\Db::name("product_config_options_sub")
                                ->where("config_id", $hhhhh["config_id"])
                                ->where("id", $j6)
                                ->find();
                            if (empty($new_tmp2)) {
                                return jsonrule(["status" => 400, "msg" => "参数错误"]);
                            ...
                            if (judgeQuantity($new_tmp1["option_type"])) {
                                ...
                                if (
                                    $h6_qty_maximum < $h6_qty_minimum ||
                                    $h6_qty_minimum < $new_tmp2["qty_minimum"] ||
                                    $new_tmp2["qty_maximum"] < $h6_qty_maximum
                                ) {
                                    return jsonrule(["status" => 400, "msg" => "参数错误"]);
                                }
                            }
        ...
        if (isset($params["new_cid"]) && !empty($params["new_cid"])) {
            if (
                empty($params["new_relation"]) ||
                empty($params["new_subid"]) ||
                empty($params["result"]) ||
                !is_array($params["result"]) ||
                !is_array($params["new_subid"])
            ) {
                return jsonrule(["status" => 400, "msg" => "参数错误"]);
            }
            ...
            foreach ($new_tmp_subid as $u => $w) {
                $tmp7 = \think\Db::name("product_config_options_sub")
                    ->where("config_id", $new_cid)
                    ->where("id", $u)
                    ->find();
                if (empty($tmp7)) {
                    return jsonrule(["status" => 400, "msg" => "参数错误"]);
                ...
                if (judgeQuantity($tmp6["option_type"])) {
                    ...
                    if (
                        $w_qty_maximum < $w_qty_minimum ||
                        $w_qty_minimum < $tmp7["qty_minimum"] ||
                        $tmp7["qty_maximum"] < $w_qty_maximum
                    ) {
                        return jsonrule(["status" => 400, "msg" => "参数错误"]);
                    }
                }
            ...
            foreach ($new_tmp_result as $ww) {
                ...
                foreach ($ww["new_subid"] as $uuu => $www) {
                    $tmp9 = \think\Db::name("product_config_options_sub")
                        ->where("config_id", $ww["new_cid"])
                        ->where("id", $uuu)
                        ->find();
                    if (empty($tmp9)) {
                        return jsonrule(["status" => 400, "msg" => "参数错误"]);
                    ...
                    if (judgeQuantity($tmp8["option_type"])) {
                        ...
                        if (
                            $www_qty_maximum < $www_qty_minimum ||
                            $www_qty_minimum < $tmp9["qty_minimum"] ||
                            $tmp9["qty_maximum"] < $www_qty_maximum
                        ) {
                            return jsonrule(["status" => 400, "msg" => "参数错误"]);
                        }
                    }
            ...
            $tmp_arr2 = [
                "config_id" => $new_cid,
                "relation" => $params["new_relation"],
                "sub_id" => json_encode($new_tmp_subid),
            ];
        ...
        try {
            if ($link) {
                foreach ($link as $k => $v) {
                    ...
                    \think\Db::name("product_config_options_links")
                        ->where("id", $k)
                        ->update(["config_id" => $v["config_id"], "relation" => $v["relation"], "sub_id" => $sub_id]);
                    $result = $v["result"];
                    foreach ($result as $kk => $vv) {
                        ...
                        \think\Db::name("product_config_options_links")
                            ->where("id", $kk)
                            ->update([
                                "config_id" => $vv["config_id"],
                                "relation" => $vv["relation"],
                                "sub_id" => $res_sub_id,
                            ]);
                    }
                    if ($new_result_tmp) {
                        foreach ($new_result_tmp as $kkk => $vvv) {
                            ...
                            \think\Db::name("product_config_options_links")->insert([
                                "config_id" => $vvv["config_id"],
                                "sub_id" => $sub_id,
                                "relation" => $vvv["relation"],
                                "type" => "result",
                                "relation_id" => $k,
                            ]);
                        }
            ...
            if ($new_cid) {
                ...
                $insert = [
                    "config_id" => intval($params["new_cid"]),
                    "relation" => $params["new_relation"],
                    "type" => "condition",
                    "sub_id" => $new_sub_id,
                    "relation_id" => 0,
                ...
                foreach ($new_result as $value) {
                    ...
                    $res_insert = [
                        "config_id" => $value["new_cid"],
                        "relation" => $value["new_relation"],
                        "type" => "result",
                        "relation_id" => $new_id,
                        "sub_id" => $new_res_sub_id,
                    ];
                    $uniq = \think\Db::name("product_config_options_links")
                        ->where("config_id", $value["new_cid"])
                        ->where("sub_id", $new_res_sub_id)
                        ->where("relation", $value["new_relation"])
                        ->where("type", "result")
                        ->where("relation_id", $new_id)
                        ->find();
                    if (empty($uniq)) {
    ...
    public function addCondition()
    {
        ...
        foreach ($sub_id as $k => $v) {
            ...
            if (judgeQuantity($tmp2["option_type"])) {
                ...
                if (
                    $qty_maximum < $qty_minimum ||
                    $qty_minimum < $tmp3["qty_minimum"] ||
                    $tmp3["qty_maximum"] < $qty_maximum
                ) {
                    return jsonrule(["status" => 400, "msg" => "参数错误"]);
                }
            }
        ...
        foreach ($result as $vv) {
            ...
            foreach ($res_sub_id as $kkk => $vvv) {
                ...
                if (judgeQuantity($tmp4["option_type"])) {
                    ...
                    if (
                        $qty_maximum < $qty_minimum ||
                        $qty_minimum < $tmp5["qty_minimum"] ||
                        $tmp5["qty_maximum"] < $qty_maximum
                    ) {
                        return jsonrule(["status" => 400, "msg" => "结果参数错误"]);
                    }
                }
        ...
        $uniq = \think\Db::name("product_config_options_links")
            ...
            ->where("sub_id", json_encode($sub_id))
        ...
        try {
            ...
            foreach ($result as $value) {
                \think\Db::name("product_config_options_links")->insert([
                    ...
                    "sub_id" => json_encode($value["sub_id"]),
                    "relation" => $value["relation"] ?: "seq",
                    "type" => "result",
    ...
    public function addResult()
    {
        ...
        $uniq = \think\Db::name("product_config_options_links")
            ...
            ->find();
        if (empty($uniq)) {
            $res = \think\Db::name("product_config_options_links")->insert([
                "config_id" => $config_id,
