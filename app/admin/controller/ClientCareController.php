...
class ClientCareController extends AdminBaseController
{
    ...
    public function searchCondition()
    {
        return jsonrule([
            ...
            "trigger" => config("app.client_care_trigger"),
        ]);
    }
    /**
     * @title 客戶关怀首页(搜索)
    ...
    public function careList()
    {
        ...
        $ordermethod =
            isset($data["order_method"]) && !empty($data["order_method"])
                ? strtoupper(trim($data["order_method"]))
                : "DESC";
        $total = \think\Db::name("client_care")->count("id");
        $results = \think\Db::name("client_care")
            ->alias("cc")
            ->field(
                "cc.id as care_id,cc.name,cc.trigger,cc.time,cc.method,et.name as email_template,mt.title as message_template,cc.range_type
            ,cc.status,cc.create_time,cc.update_time",
            )
            ->leftJoin("email_templates et", "et.id = cc.email_template_id")
            ->leftJoin("message_template mt", "mt.id = cc.message_template_id")
    ...
    public function editCare()
    {
        ...
        $linkproducts = \think\Db::name("client_care_product_links")
            ->field("product_id")
            ->where("care_id", $id)
            ->select();
        $link = [];
