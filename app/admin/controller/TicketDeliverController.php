...
class TicketDeliverController extends AdminBaseController
{
    ...
    public function add()
    {
        ...
        if (count($params["departments"]) > 0 && is_array($params["departments"])) {
            $departments = \think\Db::name("ticket_department")
                ->field("id")
                ->where("is_related_upstream", 1)
                ->whereIn("id", $params["departments"])
                ->select()
                ->toArray();
    ...
    {
        $data = \think\Db::name("ticket_deliver")
            ->field("id,is_open_auto_reply,bz,mask_keywords")
            ->order("id", "DESC")
            ->select()
            ->toArray();
        $departments = \think\Db::name("ticket_deliver_department")
            ->alias("a")
            ->field("a.id,a.tdid,a.dptid,b.name")
            ->leftJoin("ticket_department b", "a.dptid=b.id")
            ->select()
            ->toArray();
        $products = \think\Db::name("ticket_deliver_products")
            ->alias("a")
            ->field("a.id,a.tdid,a.pid,b.name")
