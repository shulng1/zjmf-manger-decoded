...
{
    public $control_mode = [
        ["name" => "不支持", "value" => "not_support", "disabled" => false],
        ["name" => "IPMI", "value" => "ipmi", "disabled" => false],
        ["name" => "魔方DCIM客户端", "value" => "dcim_client", "disabled" => false],
        ["name" => "OVH(开发中)", "value" => "ovh", "disabled" => true],
    ];
        ...
        try {
            ...
            if ($type == "all") {
                $data = \think\Db::name("zjmf_finance_api")
                    ->field("id,name,contact_way as phone,des as bz,create_time")
                    ->where($where)
                    ->where("is_resource", 0)
                    ->order($order, $sort)
                    ->select()
    ...
    public function upperIndex(\think\Request $request)
    {
        try {
            ...
            $type = !empty($params["type"]) ? trim($params["type"]) : "";
            if ($type == "all") {
                $data = \think\Db::name("upper_reaches_res")
                    ->alias("urr")
                    ->join("zjmf_finance_api ur", "ur.id=urr.pid")
                    ->leftJoin("host h", "h.id=urr.hid")
                    ->leftJoin("clients c", "c.id=h.uid")
                    ->leftJoin("products p", "p.id=h.productid")
                    ->where("ur.is_resource", 0)
                    ->field(
                        "urr.*,p.name,c.username as cname,ur.name as uname,h.nextduedate,h.domainstatus,h.id as hostid,c.id as uid",
                    )
            ...
            $total = \think\Db::name("upper_reaches_res")
                ->alias("urr")
                ->join("zjmf_finance_api ur", "ur.id=urr.pid")
                ->leftJoin("host h", "h.id=urr.hid")
                ->leftJoin("clients c", "c.id=h.uid")
            ...
            foreach ($data as $k => $v) {
                ...
                href=\"#/customer-view/product-innerpage?hid=" .
                        ...
                        $v["uid"] .
                        "\">
                <span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;color:" .
                        $v["domainstatus"]["color"] .
                        "\">" .
                        $str .
                        "</span></a>";
                } else {
            ...
            $apis = \think\Db::name("zjmf_finance_api")
                ->whereIn("type", ["manual", "zjmf_api"])
                ->where("is_resource", 0)
                ->select()
                ->toArray();
    ...
    {
        $data = \think\Db::name("zjmf_finance_api")
            ->field("id,name,contact_way as phone,des as bz")
            ->where("is_resource", 0)
            ->select()
            ->toArray();
    ...
    public function addUpperPost(\think\Request $request)
    {
        try {
            ...
            $data["password"] = !empty($params["password"])
                ? html_entity_decode(trim($params["password"]), ENT_QUOTES)
                : "";
    ...
    public function editUpperPost(\think\Request $request)
    {
        try {
            ...
            $exist = \think\Db::name("upper_reaches_res")
                ->where("in_ip", $params["in_ip"])
            ...
            if (!empty($res1["hid"])) {
                \think\Db::name("host")
                    ->where("id", $res1["hid"])
                    ->update([
                        "dedicatedip" => $data["in_ip"],
                        "assignedips" => implode(",", $ip),
                        "username" => $data["username"],
                        "password" => cmf_encrypt($data["password"]),
                    ]);
            }
            if ($data) {
                return jsonrule(["status" => 200, "msg" => lang("SUCCESS MESSAGE")]);
    ...
    public function allotUpper(\think\Request $request)
    {
        ...
        \think\Db::name("products")
            ->where("id", $host["productid"])
            ->update(["upper_reaches_id" => $re["pid"]]);
        $ip = \think\Db::name("upper_reaches_ip")->field("ip")->where("resid", $id)->select()->toArray();
        $ip = array_column($ip, "ip");
        \think\Db::name("host")
            ->where("id", $hid)
            ->update([
                "dedicatedip" => $re["in_ip"],
                "assignedips" => implode(",", $ip),
                "username" => $re["username"],
                "password" => cmf_encrypt($re["password"]),
            ]);
    ...
    public function emptyUpper(\think\Request $request)
    {
        ...
        \think\Db::name("host")
            ->where("id", $re["hid"])
            ->update(["dedicatedip" => "", "assignedips" => "", "username" => "", "password" => ""]);
