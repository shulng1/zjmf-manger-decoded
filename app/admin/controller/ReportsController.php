...
class ReportsController extends AdminBaseController
{
    ...
    public function getYearIncomeStatistics()
    {
        ...
        if ($currencies) {
            foreach ($currencies as $currency) {
                ...
                $data_temp["year_count"] = \think\Db::name("accounts")
                    ->where("delete_time", 0)
                    ->where("currency", $currency["code"])
                    ->field("FROM_UNIXTIME(pay_time,'%Y年%c月') as date")
                    ->field("sum(amount_in) as income")
                    ->field("sum(amount_out) as expenses")
                    ->field("sum(amount_in - amount_out) as last")
                    ->group("date")
                    ->limit($limit)
                    ->page($page)
                    ->order("pay_time desc")
                    ->select()
                    ->toArray();
                $data_temp["year_count_num"] = \think\Db::name("accounts")
                    ->where("delete_time", 0)
                    ->where("currency", $currency["code"])
                    ->field("FROM_UNIXTIME(pay_time,'%Y年%c月') as date")
                    ->field("sum(amount_in) as income")
    ...
    private function getStatisticYears($currency_code = "", $prefix = "")
    {
        $data["years"] = [];
        $data["list"] = [];
        $years = \think\Db::name("accounts")
            ->where("delete_time", 0)
            ->where("refund", 0)
            ->distinct("year")
            ->column("FROM_UNIXTIME(pay_time,'%Y') as year");
        if ($years) {
            $data["years"] = $years;
            foreach ($years as $item) {
                ...
                $end_time = $item . "-12-31";
                $years_month_data = \think\Db::name("accounts")
                    ->where("delete_time", 0)
                    ->where("currency", $currency_code)
                    ->whereTime("pay_time", [$start_time, $end_time])
                    ->field("FROM_UNIXTIME(pay_time,'%c') as month")
                    ->field("sum(amount_in) as income")
                    ->field("sum(amount_out) as expenses")
                    ->field("sum(amount_in - amount_out) as last")
                    ->group("month")
    ...
    private function getYearOrderInfoGroupByMonth($year = "", $currency_code = "CNY")
    {
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $rows = \think\Db::name("orders")
                ...
                ->where("i.paid_time", "between", [$start_time, $end_time])
                ->where("i.status", "Paid")
                ->where("o.delete_time", 0)
                ->field("o.id,o.ordernum,c.sale_id,o.invoiceid,cu.prefix,cu.suffix,o.amount as am")
                ->field("i.status as invoice_status,i.type as invoice_type")
                ->select()
                ->toArray();
            if ($rows) {
                $discount_sum = 0;
    ...
    public function getNewClientStatistics()
    {
        ...
        for ($i = 1; $i <= $month_day; $i++) {
            ...
            $client_lists = \think\Db::name("clients")
        ...
        return jsonrule([
            ...
            "data" => ["client_data" => $client_data, "new_client_years_group" => $new_client_years_group],
        ]);
    }
    private function getNewClientYearsGroup()
    {
        $year = \think\Db::name("clients")
            ->where("status", 1)
            ->order("create_time asc")
            ->value("FROM_UNIXTIME(create_time,'%Y') as year");
        $years_arr = [];
        $now_year = date("Y");
        if (!$year) {
    ...
    private function getNowYearClientMonths($year = "")
    {
        ...
        $month_groups = \think\Db::name("clients")
            ->where("status", 1)
            ->where("create_time", "between", [$start_time, $end_time])
            ->field("FROM_UNIXTIME(create_time,'%m') as month")
            ->group("month")
            ->order("month desc")
            ->select()
            ->toArray();
    ...
    private function getClientNewOrderNum($start_time = "", $end_time = "", $paid = 0)
    {
        if (!$paid) {
            ...
            $num = \think\Db::name("orders")
                ->alias("o")
                ->join("invoices nv o.invoiceid = nv.id")
                ->where("delete_time", 0)
                ->where("status", "Active")
                ->whereBetweenTime("create_time", $start_time, $end_time)
                ->count();
    ...
    public function rankForwardClient()
    {
        $clients = \think\Db::name("clients")
            ->alias("c")
            ->join("accounts a", "a.uid = c.id")
            ->leftJoin("currencies cu", "cu.code = a.currency")
            ->where("c.status", 1)
            ->where("a.delete_time", 0)
            ->field("c.id,c.username,c.companyname")
            ->field("sum(`a`.`amount_in`) as income_sum, sum(a.amount_out) as  expense_sum,cu.prefix,cu.suffix")
            ->order("expense_sum", "desc")
            ->group("c.id")
            ->select()
            ->toArray();
        if ($clients) {
    ...
    public function productIncome()
    {
        ...
        $product_groups = \think\Db::name("product_groups")
            ...
            ->select()
            ->toArray();
        $product_groups_counts = \think\Db::name("product_groups")
            ->where("hidden", 0)
            ->field("id,name")
            ->order("order", "ASC")
            ->count();
            ...
            foreach ($product_groups as &$item) {
                ...
                if ($products) {
                    foreach ($products as &$p_item) {
                        ...
                        $renew_order_num = 0;
                        $new_order = \think\Db::name("host")
                            ->alias("h")
                            ->leftJoin("invoice_items im", "im.rel_id = h.id")
                            ->leftJoin("invoices i", "im.invoice_id = i.id")
                            ->whereIn("i.status", ["Paid", "Refunded"])
                            ->where("i.type", "product")
                            ->where("h.productid", $p_item["id"])
                            ->where("i.paid_time", "between", [$start_time, $end_time])
                            ->field(
                                "sum(im.amount) as amount,count(distinct h.id) as num,group_concat(distinct i.id) as ids ",
                            )
                            ->find();
                        ...
                        if ($new_order["ids"]) {
                            ...
                            $new_order_refund = \think\Db::name("accounts")
                                ->where("invoice_id", "in", $ids)
                                ->where("refund", ">", 0)
                                ->sum("amount_out");
                            $credit_refund = \think\Db::name("credit")
                                ->whereIn("relid", $ids)
                        ...
                        $renew = \think\Db::name("host")
                            ...
                            ->where("i.type", "renew")
                            ->where("im.type", "renew")
                            ->where("p.id", $p_item["id"])
                            ->whereBetweenTime("paid_time", $start_time, $end_time)
                            ->field("count( distinct im.id) as num,group_concat( distinct i.id) as ids ")
                            ->find();
                        ...
                        if ($renew["ids"]) {
                            $renew_order_refund = \think\Db::name("accounts")
                                ->where("invoice_id", "in", $ids)
                                ->where("refund", ">", 0)
                                ->sum("amount_out");
        ...
        $year = \think\Db::name("orders")
            ...
            ->group("year")
            ->value("FROM_UNIXTIME(i.create_time,'%Y') as year");
        $years = [];
        $now_year = date("Y");
        if ($year) {
            for ($i = $year; $i <= $now_year; $i++) {
        ...
        return jsonrule([
            ...
            "msg" => lang("SUCCESS MESSAGE"),
            "years" => $years,
            "groups_count" => $product_groups_counts,
            "data" => $product_groups,
        ]);
    }
    /**
    ...
    {
        $product_group = \think\Db::name("product_groups")
            ->field("id,name")
            ->where("hidden", 0)
            ->order("create_time", "desc")
            ->order("order", "desc")
            ...
            ->select()
            ->toArray();
        $product_group_count = \think\Db::name("product_groups")
            ->field("id,name")
            ->where("hidden", 0)
            ->order("create_time", "desc")
            ->order("order", "desc")
        ...
        if ($product_group) {
            foreach ($product_group as &$item) {
                $item["total_amount"] = \think\Db::name("orders")
                    ->alias("o")
                    ->join("invoices i", "i.id = o.invoiceid")
                    ->join("invoice_items im", "im.invoice_id = i.id")
                    ->join("host h", "im.rel_id = h.id")
                ...
                $new_order = \think\Db::name("orders")
                    ...
                    ->whereBetweenTime("paid_time", $start_time, $end_time)
                    ->where("i.type", "product")
                    ->field("sum(o.amount) as new_order_amount")
                    ->field("count(o.id) as new_order_num")
                    ->find();
                $renew = \think\Db::name("orders")
                    ->alias("o")
                    ->join("invoices i", "i.id = o.invoiceid")
                    ->join("invoice_items im", "im.invoice_id = i.id")
                    ->join("host h", "im.rel_id = h.id")
                    ...
                    ->whereBetweenTime("paid_time", $start_time, $end_time)
                    ->where("i.type", "renew")
                    ->field("sum(o.amount) as renew_order_amount")
                    ->field("count(o.id) as renew_order_num")
                    ->find();
    ...
    private function getNameProductIncome($year, $month, $page, $limit)
    {
        ...
        $products = \think\Db::name("products")
            ...
            ->select()
            ->toArray();
        $products_count = \think\Db::name("products")
            ->field("id,name")
            ->where("hidden", 0)
            ->order("create_time", "desc")
            ->order("order", "desc")
