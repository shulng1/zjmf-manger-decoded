...
class ClientsServicesController extends GetUserController
{
    ...
    public function index(\think\Request $request)
    {
        ...
        if ($product["api_type"] == "zjmf_api" && $product["upstream_pid"] > 0) {
            ...
            if (!empty($res)) {
                $returndata["upstream_host"] = [
                    ...
                    "domainstatus_desc" => $res["domainstatus_desc"],
                    "firstpaymentamount" => $res["firstpaymentamount"],
                    "firstpaymentamount_desc" => $res["firstpaymentamount_desc"],
                    "amount" => $host_data["upstream_cost"] != "-" ? $host_data["upstream_cost"] : $res["amount"],
                    "amount_desc" => $res["amount_desc"],
                    "promo_code" => $res["promo_code"],
                    "payment" => $res["payment"],
                    "payment_zh" => $res["payment_zh"],
                    "billingcycle" => $res["billingcycle"],
                    "billingcycle_desc" => $res["billingcycle_desc"],
                ];
            } else {
                $returndata["upstream_host"] = [
                    "amount" => $host_data["upstream_cost"] ?: "-",
                    "amount_desc" => "-",
                    "firstpaymentamount" => "-",
                    "firstpaymentamount_desc" => "-",
                ];
            }
        } else {
        ...
        if ($product["api_type"] == "zjmf_api" || $product["api_type"] == "resource") {
            ...
            $upstream_product = zjmfCurl(
                $product_data["zjmf_api_id"],
                "/cart/get_product_config",
                ["pid" => $product_data["upstream_pid"]],
                15,
                "GET",
            );
            $returndata["zjmf_api"] = [
                "id" => $product_data["zjmf_api_id"],
                "name" => $zjmf_api["name"] . "-" . $upstream_product["data"]["products"]["name"],
            ];
        } elseif ($product["api_type"] == "resource") {
        } elseif ($product["api_type"] == "whmcs") {
            $upper_reaches = \think\Db::name("zjmf_finance_api")
                ->where("type", "whmcs")
                ->where("id", $product_data["zjmf_api_id"])
                ->find();
            ...
            $upper_reaches = \think\Db::name("zjmf_finance_api")
                ->where("type", "manual")
                ->where("id", $product_data["upper_reaches_id"])
                ->find();
        ...
        if ($product["api_type"] == "zjmf_api" || $product["api_type"] == "resource") {
            if (!empty($host_data["dcimid"])) {
                ...
                $returndata["module_button"] = array_merge(
                    $returndata["module_button"],
                    $upstream_data["module_button"]["control"],
                );
                $returndata["module_button"] = array_merge(
                    $returndata["module_button"],
                    $upstream_data["module_button"]["console"],
                );
        ...
        if (!empty($custom_field)) {
            ...
            $user_custom_field_value = \think\Db::name("customfieldsvalues")
                ->field("fieldid,value")
        ...
        $returndata["domain_status_list"] =
            $product["type"] == "ssl" ? config("public.sslDomainStatus") : config("public.domainstatus");
        $admin_auth = [
            "create" => "on",
            "suspend" => "on",
            "unsuspend" => "on",
        ...
        if ($product["type"] == "dcim") {
            if ($product["api_type"] == "whmcs") {
                ...
                if ($host_data["dcimid"] == 0) {
                    ...
                    if ($product_data["api_type"] == "zjmf_api") {
                        if (!empty($zjmf_api)) {
                            $returndata["dcim"]["url"] =
                                trim($zjmf_api["hostname"], "/") . "/#/server/billing?id=" . $host_data["dcimid"];
                        } else {
                            $returndata["dcim"]["url"] = "";
                        }
                    } else {
                        ...
                        if ($product_data["config_option1"] == "bms") {
                            $returndata["dcim"]["url"] =
                                $dcim->url . "/bare/#/business/details?id=" . $host_data["dcimid"];
                        } else {
            ...
            $dcim_server_group = \think\Db::name("host_config_options")
                ->field("a.id,a.qty,b.option_name,b.option_type,c.option_name sub_option")
                ->alias("a")
                ...
                ->where("a.relid", $host_data["id"])
                ->whereLike("b.option_name", "server_group|%")
                ->find();
        ...
        if ($product["type"] == "dcimcloud") {
            if ($host_data["dcimid"] > 0) {
                if ($product_data["api_type"] == "zjmf_api") {
                    if (!empty($zjmf_api)) {
                        $returndata["dcim"]["url"] =
                            trim($zjmf_api["hostname"], "/") . "/#/cloud-server-info?id=" . $host_data["dcimid"];
                    } else {
                        $returndata["dcim"]["url"] = "";
                    }
                } else {
    ...
    public function postInfo(\think\Request $request)
    {
        ...
        $udata = [
            ...
            "firstpaymentamount" => $param["firstpaymentamount"],
            "amount" => $param["amount"],
            "nextduedate" => $param["nextduedate"],
            "termination_date" => $param["termination_date"] ?? 0,
            "username" => $param["username"] ?? "",
            "password" => $param["password"] ? cmf_encrypt($param["password"]) : "",
            ...
            "overideautosuspend" => $param["overideautosuspend"] ?? 0,
            "overidesuspenduntil" => $param["overidesuspenduntil"] ?? 0,
            "auto_terminate_end_cycle" => $param["auto_terminate_end_cycle"] == "false" ? 0 : 1,
            "auto_terminate_reason" => $param["auto_terminate_reason"] ?? "",
            "notes" => $param["notes"] ?? "",
            "port" => $param["port"] ?? 0,
            "initiative_renew" => $param["initiative_renew"] ?? 0,
            "upstream_cost" => $param["upstream_cost"] ?? 0,
        ];
        ...
        if ($host_data["regdate"] != $param["regdate"]) {
            $log_desc .=
                "开通时间由“" .
                Date("Y-m-d H:i:s", $host_data["regdate"]) .
                "”改为“" .
                Date("Y-m-d H:i:s", $param["regdate"]) .
                "”，";
        }
        if ($host_data["firstpaymentamount"] != $param["firstpaymentamount"]) {
            $log_desc .=
                "首付金额从由“" . $host_data["firstpaymentamount"] . "”改为“" . $param["firstpaymentamount"] . "”，";
        }
        ...
        if (date("Y", $param["nextduedate"]) > 2099) {
            ...
            if ($host_data["nextduedate"] != $param["nextduedate"]) {
                $log_desc .=
                    "下次到期时间由“" .
                    Date("Y-m-d H:i:s", $host_data["nextduedate"]) .
                    "“改为”" .
                    Date("Y-m-d H:i:s", $param["nextduedate"]) .
                    "”，";
            }
        }
        if (date("Y", $param["termination_date"]) > 2099) {
            ...
            if ($host_data["termination_date"] != $param["termination_date"]) {
                $log_desc .=
                    "终止时间由“" .
                    Date("Y-m-d H:i:s", $host_data["termination_date"]) .
                    "”改为“" .
                    Date("Y-m-d H:i:s", $param["termination_date"]) .
                    "”，";
            }
        }
        ...
        if ($host_data["billingcycle"] != $param["billingcycle"]) {
            $log_desc .=
                "付款周期由“" . $arr[$host_data["billingcycle"]] . "”改为“" . $arr[$param["billingcycle"]] . "”，";
        }
        unset($arr);
        ...
        if ($host_data["overideautosuspend"] != $param["overideautosuspend"]) {
            $log_desc .=
                "暂停时间由“" . $host_data["overideautosuspend"] . "”改为“" . $param["overideautosuspend"] . "”，";
        }
        if ($host_data["overidesuspenduntil"] != $param["overidesuspenduntil"]) {
            $log_desc .=
                "暂停时间戳由“" . $host_data["overidesuspenduntil"] . "”改为“" . $param["overidesuspenduntil"] . "”，";
        }
        ...
        if ($host_data["auto_terminate_reason"] != $param["auto_terminate_reason"]) {
            $log_desc .=
                "到期自动删除原因由“" .
                $host_data["auto_terminate_reason"] .
                "”更改为“" .
                $param["auto_terminate_reason"] .
                "”，";
        }
        ...
        if ($oldproductid != $productid) {
            ...
            if (!empty($new_product_data)) {
                ...
                $log_desc .=
                    "产品/服务由“" . $new_product_data1["name"] . "”更改为“" . $new_product_data["name"] . "”，";
        ...
        if ($product_info["api_type"] == "zjmf_api") {
            ...
            if ($oldserver != $server) {
                $server_data = \think\Db::name("servers")
                    ->field("name")
                    ->where("id={$server}")
                    ->find();
                $server_data1 = \think\Db::name("servers")
                    ->field("name")
                    ->where("id={$oldserver}")
                    ->find();
        ...
        if ($param["domainstatus"] != $host_data["domainstatus"]) {
            ...
            $log_desc .=
                "状态由“" . $status[$host_data["domainstatus"]] . "”更改为“" . $status[$param["domainstatus"]] . "”";
        }
        if ($product_info["api_type"] != "zjmf_api" && $product_type == "dcimcloud") {
            $old_traffic_bill_type = \think\Db::name("host_config_options")
                ->field("a.id,a.qty,a.configid,a.optionid,b.option_name,b.option_type,b.upgrade")
                ->alias("a")
                ->leftJoin("product_config_options b", "a.configid=b.id")
                ->where("a.relid", $hostid)
                ->whereLike("b.option_name", "traffic_bill_type|%")
                ->find();
        }
        ...
        $os = \think\Db::name("host_config_options")
            ->field("a.id,c.option_name")
            ->alias("a")
            ->leftJoin("product_config_options b", "a.configid=b.id")
            ->leftJoin("product_config_options_sub c", "a.configid=c.config_id and a.optionid=c.id")
            ->where("a.relid", $hostid)
            ...
            if (in_array($host_data["domainstatus"], ["Cancelled", "Fraud", "Deleted"])) {
                if (in_array($param["domainstatus"], ["Pending", "Active", "Suspended"])) {
                    ...
                    if ($product_info["qty"] < 1) {
                        return jsonrule([
                            "status" => 400,
                            "msg" => "此产品库存不足，无法切换产品到此状态，请前往添加库存",
                        ]);
                    }
        ...
        if (
            !empty($old_traffic_bill_type) &&
            $configoption[$old_traffic_bill_type["configid"]] != $old_traffic_bill_type["optionid"]
        ) {
            $dcimcloud = new \app\common\logic\DcimCloud();
    ...
    public function postTransfer(\think\Request $request)
    {
        ...
        if (empty($trans_client_data)) {
            active_log(
                sprintf($this->lang["ClientsServices_admin_postTransfer_fail2"], $hostid, $uid, $transfer_uid),
                $uid,
            );
        ...
        if (empty($host_data)) {
            active_log(
                sprintf($this->lang["ClientsServices_admin_postTransfer_fail1"], $hostid, $uid, $transfer_uid),
                $uid,
            );
        ...
        if ($uid == $transfer_uid) {
            active_log(
                sprintf($this->lang["ClientsServices_admin_postTransfer_fail"], $hostid, $uid, $transfer_uid),
                $uid,
            );
        ...
        active_log(
            sprintf($this->lang["ClientsServices_admin_postTransfer_suffer"], $hostid, $uid, $transfer_uid),
            $transfer_uid,
        );
    ...
    public function deleteHost(\think\Request $request)
    {
        ...
        try {
            ...
            foreach ($hostids as $hostid) {
                ...
                $custom_data = \think\Db::name("customfields")
                    ->field("id")
                    ->where("type", "product")
                ...
                \think\Db::name("customfieldsvalues")
                    ->whereIn("fieldid", $custom_data_ids)
                    ->where("relid", $hostid)
                    ->delete();
            ...
            foreach ($hostids as $hostid) {
                ...
                $uid = $host_data["uid"];
                active_log_final(
                    sprintf($this->lang["ClientsServices_admin_deleteHost_success"], $hostid, $uid, $productid),
                    $uid,
                    2,
                    $hostid,
                );
	...
	*/
    ...
    public function applyCredit(\think\Request $request)
    {
        ...
        if ($paid_invoice_total == 0) {
            $update_invoice = [
                "paid_time" => $time,
                "credit" => $paid_invoice_credit,
                "total" => $paid_invoice_total,
                "status" => "Paid",
            ];
            ...
            $result["data"]["hostid"] = \think\Db::name("invoice_items")
                ...
                ->column("rel_id");
            if (
                (strpos($param["downstream_url"], "https://") === 0 ||
                    strpos($param["downstream_url"], "http://") === 0) &&
                strlen($param["downstream_token"]) == 32 &&
                is_numeric($param["downstream_id"])
            ) {
                $stream_info = \think\Db::name("host")
                    ->where("id", \intval($result["data"]["hostid"][0]))
                    ->value("stream_info");
            ...
            \think\Db::name("invoices")
                ->where("id", $invoiceid)
                ->update(["total" => $paid_invoice_total]);
    ...
    public function postSearchClient(\think\Request $request)
    {
        ...
        $client_list = \think\Db::name("clients")
            ->field("email,username,id")
            ->where(function ($query) use ($client_id) {
                if ($client_id) {
                    $query
                        ->where("email", "LIKE", $client_id)
                        ->whereOr("id", $client_id)
                        ->whereOr("username", $client_id);
                }
            })
            ->select()
            ->toArray();
    ...
    private function getHostList($uid)
    {
        $host_list = \think\Db::name("host")
            ->field("h.id,p.name,h.domainstatus,h.dedicatedip,p.type")
            ->alias("h")
    ...
    public function getRefundPage()
    {
        ...
        $items = \think\Db::name("invoice_items")
            ->alias("a")
            ->field("a.invoice_id,a.amount")
            ->leftJoin("invoices b", "a.invoice_id = b.id")
            ->where("a.rel_id", $hid)
            ->whereIn("a.type", ["host", "renew"])
            ->where("b.status", "Paid")
            ->select()
            ->toArray();
        $amount = 0;
        ...
        $invoices = \think\Db::name("invoices")
            ->field("id,subtotal,type,type as type_zh")
            ->whereIn("id", $invoice_ids)
            ->where("status", "Paid")
            ->withAttr("type_zh", function ($value, $data) use ($item_type) {
        ...
        $data = [
            "invoices" => $invoices,
            "currency" => getUserCurrency($tmp["uid"]),
    ...
    public function refund()
    {
        ...
        if ($method == "day") {
            $refund =
                ((($tmp["nextduedate"] - time()) / 24) * 60 * 60 * $amount) /
                (($tmp["nextduedate"] - $tmp["regdate"]) / 86400);
        } elseif ($method == "full") {
            $refund = $amount;
        } else {
