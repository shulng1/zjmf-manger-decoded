...
class TestController extends \think\Controller
{
    ...
    public function index()
    {
        ...
        foreach ($news_list as $key => $value) {
            $id = \think\Db::name("news_menu")->insertGetId([
                "admin_id" => 32,
                "parent_id" => 3,
                "title" => $value["title"],
        ...
        $host = \think\Db::name("host")
            ...
            ->leftJoin("customfields d", "p.id=d.relid AND d.type='product' AND d.fieldname='hostid'")
            ->leftJoin("customfieldsvalues e", "e.fieldid=d.id AND e.relid=h.id")
            ->leftJoin("host f", "f.id=e.value")
            ->where("p.app_status", 1)
            ->where("p.retired", 0)
            ->where("p.hidden", 0)
            ->where("f.domain", "<>", "")
            ->where("o.invoiceid=0 OR i.status='Paid'")
            ->where("h.create_time", "<", time() - 604800)
            ->limit(10)
            ->page(2)
            ->select()
            ->toArray();
        ...
        foreach ($host as $key => $value) {
            $evaluation = \think\Db::name("evaluation")
                ...
                ->find();
            if (empty($evaluation)) {
                \think\Db::name("evaluation")->insertGetId([
                    "type" => "products",
                    "rid" => $v["id"],
                    "uid" => $v["uid"],
                    "content" => "默认好评",
            ...
            $evaluations = \think\Db::name("evaluation")
                ->where("rid", $v["id"])
                ->where("type", "products")
                ->where("eid", 0)
                ->select()
        ...
        try {
            ...
            $orders = \think\Db::name("invoices")
                ->alias("c")
                ->field("c.id,c.uid,f.productid")
                ->leftJoin("invoice_items b", "c.id=b.invoice_id")
                ->leftJoin("host d", "b.rel_id=d.id AND b.type in ('host','renew')")
                ->leftJoin("upgrades u", "b.rel_id=u.id AND b.type in ('upgrade')")
                ->leftJoin("host v", "u.relid=v.id")
                ->leftJoin("products e", "d.productid=e.id OR v.productid=e.id")
                ->leftJoin("res_products f", "e.id=f.productid")
                ->leftJoin("res_sign_for sf", "sf.invoiceid = c.id")
                ->leftJoin("res_evaluation re", "re.rid=c.id")
                ->where("sf.status", 1)
                ->where("f.productid", ">", 0)
                ->where("re.id is null")
                ->where("c.create_time", "<", time() - $shd_resource_auto_evaluate * 24 * 3600)
                ->group("c.id")
                ->select()
                ->toArray();
            foreach ($orders as $key => $value) {
                \think\Db::name("res_evaluation")->insertGetId([
                    "type" => "great",
                    "rid" => $value["id"],
                    "pid" => $value["productid"],
                    "uid" => $value["uid"],
            ...
            foreach ($productid as $key => $value) {
                $evaluations = \think\Db::name("res_evaluation")
                    ->where("pid", $value)
                    ->where("eid", 0)
                    ->select()
                    ->toArray();
            ...
            $evaluations = \think\Db::name("res_evaluation")
                ->alias("a")
                ->field("a.id,a.score,a.netword_score,a.hardware_score,a.pid,b.shop_id")
                ->leftJoin("res_products b", "b.productid=a.pid")
                ->select()
                ->toArray();
            $shops = [];
            foreach ($evaluations as $key => $value) {
                if (!isset($shops[$value["shop_id"]])) {
                    $shops[$value["shop_id"]] = [
                        "score" => $value["score"] + $value["netword_score"] + $value["hardware_score"],
                        "count" => 1,
                    ];
                } else {
                    $shops[$value["shop_id"]]["score"] +=
                        $value["score"] + $value["netword_score"] + $value["hardware_score"];
                    $shops[$value["shop_id"]]["count"]++;
                }
            }
            foreach ($shops as $key => $value) {
                $score = round($value["score"] / $value["count"] / 3, 1);
                \think\Db::name("res_shop")
                    ->where("id", $key)
                    ->update(["score" => $score]);
            }
            \think\Db::commit();
        ...
        $currency = 1;
        $qty = 35;
        $arr = \think\Db::name("product_config_options_sub")
            ->alias("a")
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.id,d.value domain2,a.domain,e.ip_address,e.hostname,e.secure,e.port,e.username,e.password")
            ->leftJoin("products b", "b.id=a.productid")
            ->leftJoin("customfields c", "c.relid=b.id AND c.type='product' AND c.fieldname='domain'")
            ->leftJoin("customfieldsvalues d", "d.relid=a.id AND d.fieldid=c.id")
            ->leftJoin("servers e", "e.gid=b.server_group")
            ->where("b.config_option1", "finance")
            ->whereIn("b.config_option2", ["free", "professional"])
            ->where("d.value", "<>", "")
            ->where("a.uid", $uid)
            ->where("a.id", $hid)
            ->find();
        ...
        $res = commonCurl(
            $url,
            ["token" => $host["username"], "license" => $host["domain"], "secure_password" => $password],
            30,
            "PUT",
        );
        if ($res["status"] == 200) {
            if (stripos($host["domain2"], "https://") === false && stripos($host["domain2"], "http://") === false) {
                $result = commonCurl(
                    "https://" . $host["domain2"] . "/" . $adminAddress . "/app_store/check_token",
                    [],
                    5,
                    "GET",
                );
            ...
            $res = commonCurl(
                $host["domain2"] . "/" . $adminAddress . "/app_store/app/" . $pid . "/install",
        ...
        $cookiename = "xxxx.txt";
        $re = $this->localGet(
            "https://my.ddos-guard.net/attack?AttackSearch%5Bip%5D=&AttackSearch%5Bstart_attack_date%5D=&AttackSearch%5Bend_attack_date%5D=&AttackSearch%5Bprotocol%5D=6",
            null,
            10,
            $cookiename,
        );
        ...
        exit();
        $re = $this->localGet(
            "https://my.ddos-guard.net/attack?AttackSearch%5Bip%5D=&AttackSearch%5Bstart_attack_date%5D=&AttackSearch%5Bend_attack_date%5D=&AttackSearch%5Bprotocol%5D=17",
            null,
            10,
            $cookiename,
        );
        ...
        foreach ($re as $rv) {
            ...
            foreach ($arr as $k => $v) {
                if (
                    $v["ip"] == $ip &&
                    (($v["start_time"] <= $start_time && $start_time <= $v["end_time"]) ||
                        ($v["start_time"] <= $end_time && $end_time <= $v["end_time"]))
                ) {
        ...
        $group_id = $re[0]["id"];
        $re = commonCurl(
            "https://adbos.nsfocus.cloud/api/v1/scrubbingmonitorgroups/" .
                $group_id .
                "/monitor/ads/events/?chart=table&limit=100&offset=5&orderby=-time",
            [],
            90,
            "GET",
            $header,
    ...
    public function index1()
    {
        ...
        exit();
        header(
            "Set-Cookie: testcookie=中文; path=/; domain=w2.test.idcsmart.com; expires=" .
        ...
        foreach ($products as &$product) {
            $pid = $product["id"];
            $configs = \think\Db::name("product_config_options")
                ->alias("a")
        ...
        exit();
        $uniq_arr = [
            ["config_id" => "41440", "relation" => "eq", "sub_id" => "255230"],
            ["config_id" => 41440, "relation" => "eq", "sub_id" => "255230"],
        ];
        ...
        \think\Db::name("clients")
            ->where("email", "like", "%default.email%")
            ->update(["email" => ""]);
        var_dump("success");
        exit();
        $ids = [1, 2, 3, 4];
        ...
        foreach ($hids as $hid) {
            foreach ($cids as $cid) {
                ...
                if ($count > 1) {
                    $config = \think\Db::name("host_config_options")
                        ->where("relid", $hid)
                        ->where("configid", $cid)
                        ->find();
        ...
        $arr1 = [
            ["relid" => 1, "configid" => 1, "optionid" => 1, "upstream_oid" => 1, "upstream_subid" => 1],
            ["relid" => 2, "configid" => 1, "optionid" => 1, "upstream_oid" => 1, "upstream_subid" => 1],
            ["relid" => 2, "configid" => 2, "optionid" => 2, "upstream_oid" => 2, "upstream_subid" => 2],
            ["relid" => 3, "configid" => 3, "optionid" => 3, "upstream_oid" => 3, "upstream_subid" => 3],
        ];
        ...
        $rows = \think\Db::name("orders")
            ...
            ->field(
                "c.username,o.id,o.uid,o.status,o.ordernum,o.create_time,o.invoiceid,o.amount,o.payment,cu.prefix,cu.suffix,u.user_nickname",
            )
            ->select()
            ->toArray();
        var_dump($rows);
