...
class TicketDepartmentController extends AdminBaseController
{
    ...
    public function addPage()
    {
        ...
        $zjmf_finance_api = \think\Db::name("zjmf_finance_api")
            ->field("id,name")
            ->where("status", 1)
            ->select()
            ->toArray();
        return jsonrule([
            "status" => 200,
            "data" => ["admin_list" => $admin_list, "zjmf_finance_api" => $zjmf_finance_api],
        ]);
    }
    /**
     * @title 添加新部门
    ...
    public function save()
    {
        ...
        if (!empty($params["addfieldname"])) {
            $add_field = [
                "type" => "ticket",
                "relid" => $id,
                "fieldname" => $params["addfieldname"],
                "fieldtype" => $params["addfieldtype"] ?: "text",
                "description" => $params["addcfdesc"] ?: "",
                "fieldoptions" => $params["addfieldoptions"] ?: "",
                "regexpr" => $params["addregexpr"] ?: "",
                "adminonly" => $params["addadminonly"] ?: "",
                "required" => $params["addrequired"] ?: "",
                "sortorder" => $params["addsortorder"] ?: 0,
                "create_time" => time(),
            ];
    ...
    public function moveDown()
    {
        ...
        $next = \think\Db::name("ticket_department")
            ->field("id,order")
            ->where("order", ">", $info["order"])
            ->order("order", "asc")
            ->find();
        if (empty($next)) {
            $result["status"] = 200;
        ...
        \think\Db::startTrans();
        try {
            $r1 = \think\Db::name("ticket_department")
                ->where("id", $id)
                ->update(["order" => $next["order"]]);
            $r2 = \think\Db::name("ticket_department")
                ->where("id", $next["id"])
                ->update(["order" => $info["order"]]);
            if ($r1 && $r2) {
                \think\Db::commit();
                $result["status"] = 200;
                $result["msg"] = "操作成功";
    ...
    public function moveUp()
    {
        ...
        $prev = \think\Db::name("ticket_department")
            ->field("id,order")
            ->where("order", "<", $info["order"])
            ->order("order", "desc")
            ->find();
    ...
    public function getList()
    {
        $data = \think\Db::name("ticket_department")
            ->field("id,name,description,email,hidden,order,is_open_auto_reply as auto_reply")
            ->order("order", "DESC")
            ->select()
            ->toArray();
        $result["status"] = 200;
    ...
    public function getDetail($id)
    {
        ...
        $ticket_department_upstream = \think\Db::name("ticket_department_upstream")
            ->where("dptid", $id)
            ->select()
            ->toArray();
        $data["upstreams"] = array_column($ticket_department_upstream, "upstream_dptid", "api_id") ?: [];
        $custom_list = \think\Db::name("customfields")
            ->where("relid", $id)
            ->where("type", "ticket")
            ->order("id", "desc");
    ...
    public function addTicketCustomParam()
    {
        try {
            ...
            $msg = [
                ...
                "fieldtype.require" => "字段类型不能为空",
                "description.require" => "字段描述不能为空",
                "ticketId.require" => "工单部门ID不能为空",
            ];
            $validate = new \think\Validate($rule, $msg);
            $validate_result = $validate->check($params);
            if (!$validate_result) {
            ...
            $model = \think\Db::name("customfields")
                ->where(["fieldname" => $params["fieldname"], "type" => "ticket"])
                ->find();
            if ($model) {
