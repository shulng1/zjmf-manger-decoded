...
class NewsController extends AdminBaseController
{
    ...
    public function getList(\think\Request $request)
    {
        ...
        $news_menu = \think\Db::name("news_menu")
            ->where($where)
    ...
    public function getCatsPage()
    {
        ...
        return jsonrule([
            "status" => 200,
            "data" => ["list" => $list, "meta" => ["total" => $count, "page" => $this->page, "limit" => $this->limit]],
        ]);
    }
    /**
     * @title 新闻分类所有数据
    ...
    public function postEditCat(\think\Request $request)
    {
        ...
        if (!empty($param["id"])) {
            ...
            if ($param["hidden"] == 1) {
                active_log(
                    sprintf($this->lang["News_admin_postEditCat1"], $param["id"], $menu_type["title"], $param["title"]),
                );
            } else {
                active_log(
                    sprintf($this->lang["News_admin_postEditCat2"], $param["id"], $menu_type["title"], $param["title"]),
                );
            }
        } else {
    ...
    public function postEditContent(\think\Request $request)
    {
        ...
        $news_menu_data = [
            ...
            "hidden" => $param["hidden"] ?? 0,
            "sort" => $param["sort"] ?? 0,
            "push_time" => $param["push_time"] ?? time(),
        ];
    ...
    final private function checkCustomParam(\think\Request $request, $e = "")
    {
        ...
        $model = \think\Db::name("customfields")->where([
