...
class BatchsyncController extends AdminBaseController
{
    public function execute()
    {
        ...
        if (count($hostarr) > 0) {
            $dbhost = \think\Db::name("host")
                ->alias("h")
                ->field("h.id")
                ->where("h.domainstatus='Active' AND serverid=1 AND  h.id not in (" . implode(",", $hostarr) . ")")
                ->limit($param["limit"])
                ->select()
                ->toArray();
        } else {
            $dbhost = \think\Db::name("host")
                ->alias("h")
                ->field("h.id")
                ->where("h.domainstatus='Active' AND serverid=1")
                ->limit($param["limit"])
                ->select()
                ->toArray();
        }
        $dbhostcount = \think\Db::name("host")
            ->alias("h")
            ->where("h.domainstatus='Active' AND serverid=1")
            ->count("h.id");
