...
class OrderController extends GetUserController
{
    ...
    public function searchPage()
    {
        ...
        $other_pay = [
            ["id" => 0, "name" => "creditPay", "title" => "余额支付", "status" => 1],
            ["id" => -1, "name" => "creditLimitPay", "title" => "信用额支付", "status" => 1],
        ];
        array_unshift($pay_status, lang("NO_INVOICE"));
        return jsonrule([
            "status" => 200,
            "msg" => lang("SUCCESS MESSAGE"),
            "gateway" => array_merge($gateway, $other_pay),
            "user" => $users,
            "type" => $order_status,
    ...
    public function index()
    {
        ...
        $result = $this->validate(
            ["page" => $page, "limit" => $limit, "order" => $order, "sort" => $sort],
            "app\\admin\\validate\\OrderValidate",
        );
        if ($result !== true) {
        ...
        $price_total = \think\Db::name("orders")
            ->alias("o")
            ->leftjoin("clients c", "o.uid=c.id")
        ...
        $rows = \think\Db::name("orders")
            ->alias("o")
            ->leftjoin("clients c", "o.uid=c.id")
            ->leftjoin("currencies cu", "cu.id = c.currency")
            ->leftJoin("user u", "u.id = c.sale_id")
            ->leftJoin("invoices i", "o.invoiceid = i.id")
            ->leftJoin("upgrades up", "up.order_id = o.id")
            ->field("o.notes as order_notes")
            ->field(
                "up.id as upid,i.type,i.subtotal,c.username,c.companyname,c.notes,o.id,o.uid,c.sale_id,o.status,o.ordernum,i.status as i_status,i.subtotal as sub,i.credit,i.use_credit_limit,o.create_time,o.invoiceid,o.amount,o.amount as am,o.payment,cu.prefix,cu.suffix,u.user_nickname,i.delete_time,i.payment as i_payment",
            )
            ...
            if (!isset($rows_filter[$row["id"]])) {
                ...
                if ($invoice_type == "zjmf_flow_packet") {
                    $hosts[] = [
                        "hostid" => 0,
                        "name" => "流量包",
                        "firstpaymentamount" => $row["prefix"] . $row["subtotal"] . $row["suffix"],
                        "billingcycle" => " -",
                    ];
                    ...
                    $hosts = \think\Db::name("host")
                        ...
                        ->leftJoin("cancel_requests cr", "cr.relid = a.id")
                        ->field(
                            "a.uid,a.initiative_renew,a.id as hostid,b.id as invoice_type,b.name,a.domain,a.dedicatedip,a.billingcycle,a.firstpaymentamount,a.productid",
                        )
                        ->distinct(true)
        ...
        $arr = [
            ...
            "price_total_page" => $price_total_page,
        ];
        session_write_close();
        return jsonrule($arr);
    }
    ...
    public function index11()
    {
        ...
        $price_total = \think\Db::name("orders")
            ...
            ->leftJoin("user u", "u.id = c.sale_id")
            ->leftJoin("invoices i", "o.invoiceid = i.id")
            ->field("o.id,c.sale_id,o.invoiceid,cu.prefix,cu.suffix,o.amount")
            ->where($fun)
            ->whereBetweenTime("o.create_time", $start_time, $end_time)
            ->sum("o.amount");
        ...
        $rows = \think\Db::name("orders")
            ...
            ->leftJoin("invoices i", "o.invoiceid = i.id")
            ->field(
                "c.username,c.companyname,o.id,o.uid,c.sale_id,o.status,o.ordernum,i.status as i_status,i.subtotal as sub,i.credit,i.use_credit_limit,o.create_time,o.invoiceid,o.amount,o.amount as am,o.payment,cu.prefix,cu.suffix,u.user_nickname",
            )
            ->withAttr("payment", function ($value, $data) use ($gateways) {
                if ($data["i_status"] == "Paid") {
                    if ($data["use_credit_limit"] == 1) {
        ...
        foreach ($rows as $k => $row) {
            ...
            if ($invoice_type == "zjmf_flow_packet") {
                $hosts[] = [
                    "hostid" => 0,
                    "name" => "流量包",
                    "firstpaymentamount" => $row["prefix"] . $invoices["subtotal"] . $row["suffix"],
                    "billingcycle" => " -",
                ];
            } elseif ($invoice_type == "zjmf_reinstall_times") {
                $hosts[] = [
                ...
                $hosts = \think\Db::name("host")
                    ...
                    ->leftJoin("cancel_requests cr", "cr.relid = a.id")
                    ->field(
                        "cr.type,cr.reason,a.initiative_renew,a.id as hostid,b.id as invoice_type,b.name,a.domain,a.dedicatedip,a.billingcycle,a.firstpaymentamount,a.productid",
                    )
                    ->distinct(true)
                    ->where(function (\think\db\Query $query) use ($row) {
                        $query->where("a.orderid", $row["id"]);
    ...
    public function indexPost()
    {
        ...
        $rows = \think\Db::name("orders")
            ...
            ->leftJoin("invoices i", "o.invoiceid = i.id")
            ->field(
                "i.status,i.type,o.id,c.sale_id,o.invoiceid,cu.prefix,cu.suffix,o.amount as am,i.use_credit_limit,i.invoice_id",
            )
            ...
            ->where("o.amount", "like", "%{$amount}%")
            ->where("o.status", "like", "%{$status}%")
            ->where(function (\think\db\Query $query) use ($params, $str) {
                if (isset($params["payment"]) && !empty($params["payment"])) {
                    if ($params["payment"] == "creditLimitPay") {
                        $query->where("i.use_credit_limit", 1);
                    } elseif ($params["payment"] == "creditPay") {
                ...
                if (isset($params["product_name"]) && !empty($params["product_name"])) {
                    $pro_ids = \think\Db::name("products")
                        ->whereLike("name", "%" . $params["product_name"] . "%")
                        ->column("id");
                    $_host_id = \think\Db::name("host")->whereIn("productid", $pro_ids)->column("orderid");
                    $query->whereIn("o.id", $_host_id);
                }
            ...
            ->select()
            ->toArray();
        $row1 = \think\Db::name("orders")
            ->alias("o")
            ->leftjoin("clients c", "o.uid=c.id")
            ->leftjoin("currencies cu", "cu.id = c.currency")
            ->leftJoin("user u", "u.id = c.sale_id")
            ->leftJoin("invoices i", "o.invoiceid = i.id")
            ->field("o.id,c.sale_id,o.invoiceid,cu.prefix,cu.suffix,o.amount,i.use_credit_limit,i.invoice_id")
            ->where($where)
            ->where("o.id", "like", "%{$id}%")
            ->where("c.username", "like", "%{$username}%")
            ->where("o.create_time", ">=", $start_time)
        ...
        foreach ($rows as $k => &$row) {
            ...
            if ($invoice_type == "upgrade") {
                $hosts = \think\Db::name("host")
                    ->alias("a")
                    ->leftJoin("products b", "a.productid = b.id")
                    ->leftJoin("sale_products c", "c.pid = b.id")
                    ->leftJoin("sales_product_groups d", "d.id = c.gid")
                    ->leftJoin("upgrades u", "u.relid = a.id")
                    ->field(
                        "a.id as hostid,b.name,a.billingcycle,u.id as upid,a.firstpaymentamount,a.productid,d.bates,d.renew_bates,d.upgrade_bates,d.is_renew,d.updategrade",
                    )
                    ->distinct(true)
                    ->where(function (\think\db\Query $query) use ($row) {
                        $query->where("a.orderid", $row["id"]);
                ...
                $hosts = \think\Db::name("invoices")
                    ->alias("i")
                    ->leftJoin("invoice_items im", "im.invoice_id = i.id")
                    ->leftJoin("host a", "a.id = im.rel_id")
                    ->leftJoin("products b", "a.productid = b.id")
                    ->leftJoin("sale_products c", "c.pid = b.id")
                    ->leftJoin("sales_product_groups d", "d.id = c.gid")
                    ->field("a.id as hostid,b.name,a.billingcycle,a.firstpaymentamount")
                    ->field("a.productid,d.bates,d.renew_bates,d.upgrade_bates,d.is_renew,d.updategrade")
                    ->distinct(true)
                    ->where("i.id", $row["invoiceid"])
                    ->withAttr("billingcycle", function ($value) {
                        return config("billing_cycle")[$value];
                    })
                    ->withAttr("firstpaymentamount", function ($value) use ($row) {
            ...
            if (
                ...
                $amount_out = \think\Db::name("accounts")
                    ->field("id,amount_out")
                    ->where("invoice_id", $row["invoiceid"])
                    ->where("refund", ">", 0)
                    ->sum("amount_out");
    ...
    public function indexPostone()
    {
        ...
        $hosts = \think\Db::name("host")
            ...
            ->leftJoin("upgrades u", "u.relid = a.id")
            ->field(
                "a.id as hostid,b.name,a.billingcycle,u.id as upid,a.firstpaymentamount,a.productid,d.bates,d.is_renew,d.updategrade",
            )
            ->distinct(true)
    ...
    public function check()
    {
        ...
        $invoiceids = \think\Db::name("orders")
            ->field("invoiceid,id,uid")
            ->where("delete_time", 0)
            ->whereIn("id", $ids)
            ->select()
            ->toArray();
        $hosts = \think\Db::name("invoice_items")
            ->alias("a")
            ->field("b.id,c.auto_setup")
            ->leftJoin("host b", "a.rel_id = b.id")
            ->leftJoin("products c", "b.productid = c.id")
            ->where("a.type", "host")
            ->where("a.delete_time", 0)
            ->whereIn("a.invoice_id", array_column($invoiceids, "invoiceid"))
            ->select()
            ->toArray();
        $update = \think\Db::name("orders")
            ->where("delete_time", 0)
            ->whereIn("id", $ids)
            ...
            foreach ($hosts as $host) {
                if ($host["auto_setup"] == "on") {
                    ...
                    if ($result["status"] == 200) {
                        ...
                        $data_i[
                            "description"
                        ] = " 手动审核后 - 开通 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                        if ($is_zjmf) {
    ...
    public function cancel()
    {
        ...
        $hostids = \think\Db::name("invoice_items")
            ->field("rel_id,uid,id")
            ->where("type", "host")
            ->where("delete_time", 0)
            ...
            ->select()
            ->toArray();
        $productids_qty = \think\Db::name("orders")
            ->alias("o")
            ->leftJoin("host h", "o.id = h.orderid")
            ->whereIn("o.id", $ids)
            ->whereIn("o.status", ["Pending", "Active", "Suspend"])
            ->column("h.productid");
        \think\Db::startTrans();
        try {
            \think\Db::name("orders")
        ...
        foreach ($invoiceidss as $ids1) {
            ...
            active_log(
                sprintf($this->lang["Order_admin_cancel_success"], $ids1["uid"], $ids1["id"]),
                $ids1["uid"],
                "",
                2,
            );
    ...
    public function getMultiTotal()
    {
        ...
        $currency_client = \think\Db::name("currencies")
            ->field("id,code,prefix,suffix,rate")
            ->where("id", $currencyid)
            ->find();
        ...
        foreach ($pids as $kp => $pid) {
            ...
            $product = \think\Db::name("products")
                ->alias("a")
                ->field(
                    "a.name,a.pay_type,b.*,a.api_type,a.upstream_version,a.upstream_price_type,a.upstream_price_value",
                )
            ...
            if ($product[$billingcycle] < 0) {
                return jsonrule([
                    "status" => 400,
                    "msg" => "请配置产品'" . $product["name"] . "'在货币'" . $currency_client["code"] . "'下的价格",
                ]);
            }
            ...
            $price_setup =
                $price_override > 0
                    ...
                    : bcsub(
                        $billingcycle == "free"
                            ? number_format(0, 2)
                            : ($product[$setupfeecycle] > 0
                                ? $product[$setupfeecycle] * $rate
                                : number_format(0, 2)),
                        0,
                        2,
                    );
            $price_cycle =
                $price_override > 0
                    ? bcsub($price_override, 0, 2) * $rate
                    : bcsub(
                        $billingcycle == "free"
                            ? number_format(0, 2)
                            : ($product[$billingcycle] > 0
                                ? $product[$billingcycle] * $rate
                                : number_format(0, 2)),
                        0,
                        2,
                    );
            ...
            foreach ($configoption as $k => $v) {
                if ($k == $kp) {
                    foreach ($v as $kk => $vv) {
                        $option_types = \think\Db::name("product_config_options")
                            ->where("id", $kk)
                            ->field("option_type,is_discount")
                            ->find();
                        ...
                        if ($option_type) {
                            ...
                            if (!judgeQuantity($option_type)) {
                                ...
                                $option = \think\Db::name("product_config_options_sub")
                                    ->alias("pcos")
                                    ->field(
                                        "pcos.id,pcos.option_name as suboption_name,pco.option_type,pco.option_name as option_name,pco.is_discount,pco.id as oid,pco.is_rebate",
                                    )
                                    ->leftJoin("product_config_options pco", "pco.id = pcos.config_id")
                                    ->where("pcos.id", $vv)
                                    ->where("pcos.config_id", $kk)
                                    ->find();
                                if (!$option) {
                                    active_log(
                                        sprintf(
                                            $this->lang["Order_admin_getMultiTotal_fail"],
                                            $uid,
                                            lang("ERROR_OPERATE"),
                                        ),
                                        $uid,
                                    ...
                                    $option_filter["option_name"] = explode("|", $optionname)[1]
                                        ...
                                        : $optionname;
                                    $option_filter["suboption_name"] = $suboptionname_deal = explode(
                                        "|",
                                        $suboptionname,
                                    )[1]
                                    ...
                                    if (in_array($suboptionname_deal, $this->allowSystem)) {
                                        $option_filter["suboption_name"] = implode(
                                            " ",
                                            explode("^", $suboptionname_deal),
                                        );
                                    }
                                    $option_filter["suboption_setup_fee"] = $optionupfee =
                                        is_numeric($price_override) && $price_override >= 0
                                            ? $format_zero
                                            : bcsub($optionupfee, 0, 2) * $rate;
                                    $option_filter["suboption_price"] = $optionprice =
                                        is_numeric($price_override) && $price_override >= 0
                                            ? $format_zero
                                            : bcsub($optionprice, 0, 2) * $rate;
                                ...
                                $options = \think\Db::name("product_config_options_sub")
                                    ->alias("pcos")
                                    ->field(
                                        "pco.id as oid,pcos.id,pcos.option_name as suboption_name,pcos.qty_minimum,pcos.qty_maximum,pco.option_type,pco.option_name as option_name,pco.qty_minimum as min,pco.qty_maximum as max,pco.is_discount,pco.is_rebate",
                                    )
                                    ->leftJoin("product_config_options pco", "pco.id = pcos.config_id")
                                    ->where("pcos.config_id", $kk)
                                    ->select()
                                ...
                                if (!empty($options[0])) {
                                    foreach ($options as $option) {
                                        ...
                                        if (
                                            ...
                                            $optionprice =
                                                $pricing[$billingcycle] <= 0 ? 0 : $pricing[$billingcycle] * $vv;
                                            $optionupfee = $pricing[$setupfeecycle] <= 0 ? 0 : $pricing[$setupfeecycle];
                                            if (
                                                $product["api_type"] == "zjmf_api" &&
                                                $product["upstream_version"] > 0 &&
                ...
                if ($promo["type"] == "percent") {
                    ...
                    foreach ($configoptions_base_sale as $h) {
                        if ($h["is_discount"] == 1) {
                            ...
                            $discount_recurring +=
                                ($h["config_base_sale"] - $h["config_base_sale_setupfee"]) * (1 - $promo_value / 100);
                        }
                    }
                    ...
                    $discount_pricing =
                        $product_total_price_sale < $promo["value"] ? $product_total_price_sale : $promo["value"];
    ...
    public function save()
    {
        ...
        foreach ($param["ops"] as $k => $v) {
            $checkProcut = (new \app\common\model\ProductModel())->checkProductPrice(
                $v["pid"],
                $v["billingcycle"],
                $currency,
            );
            ...
            if ($pay_type["pay_type"] == "free") {
                ...
                $is_interior_price =
                    isset($v["interior_price"]) && is_numeric($v["interior_price"]) && $v["interior_price"] >= 0
                        ? true
                        : false;
                if ($price_setup > 0) {
                ...
                $product_item["invoices_items"][] = [
                    ...
                    "amount" => $is_interior_price ? $v["interior_price"] : $price_cycle,
                    "due_time" => $nextduedate,
                    "payment" => $payment,
                ];
                ...
                if ($is_interior_price) {
                    ...
                    $product_item["firstpaymentamount"] =
        ...
        try {
            ...
            if (
                !empty($param["admingenerateinvoice"]) &&
                (count($pay_type_count) != 1 || !in_array("free", $pay_type_count))
            ) {
                $invoices_data = [
                    "uid" => $uid,
                    "create_time" => time(),
            ...
            if (!empty($invoiceid)) {
                ...
                try {
                    ...
                    if ($credit > 0) {
                        ...
                        $inc_credit = \think\Db::name("clients")
                            ->where("id", $uid)
                            ->where("credit", ">=", $credit)
                            ->setDec("credit", $credit);
                        if (empty($inc_credit)) {
                        ...
                        credit_log([
                            "uid" => $uid,
                            "desc" => "Credit Applied to Invoice #" . $invoiceid,
                            "amount" => $credit,
                            "relid" => $invoiceid,
                        ]);
                    }
    ...
    public function read($id)
    {
        ...
        $gateways = gateway_list("gateways", 0);
        $row = \think\Db::name("orders")
            ->alias("o")
            ->join("clients c", "c.id=o.uid")
            ->leftJoin("currencies cu", "cu.id = c.currency")
            ->leftJoin("invoices i", "o.invoiceid = i.id")
            ->field("o.notes as order_notes")
            ->field(
                "i.subtotal,i.status as i_status,i.subtotal as sub,i.credit,i.use_credit_limit,i.payment as i_payment,c.username,c.ip,c.country,o.id,o.uid,o.status,o.create_time,o.amount,o.promo_code,o.payment,o.invoiceid,cu.prefix,cu.suffix,o.invoiceid",
            )
            ->withAttr("amount", function ($value, $data) {
                return $data["prefix"] . $value . $data["suffix"];
            })
        ...
        $currency = \think\Db::name("clients")
            ->alias("a")
            ->join("currencies b", "a.currency = b.id")
            ->field("b.prefix,b.suffix")
            ->where("a.id", $row["uid"])
            ->find();
        $servers = \think\Db::name("host")
            ->alias("h")
            ->join("products p", "p.id=h.productid")
            ->join("orders o", "o.id = h.orderid")
            ->distinct(true)
            ->field(
                "h.id,h.billingcycle,h.firstpaymentamount as amount,h.serverid,h.username,h.password,p.type,p.name,p.description,p.welcome_email,h.domainstatus,o.invoiceid as invoice_id",
            )
            ->withAttr("billingcycle", function ($value) {
                return config("billing_cycle")[$value];
            })
            ...
            ->select()
            ->toArray();
        $upgrades_product = \think\Db::name("upgrades")
            ->alias("a")
            ->leftJoin("orders b", "a.order_id = b.id")
            ->leftJoin("host c", "a.relid = c.id")
            ->leftJoin("products d", "a.new_value = d.id")
            ->distinct(true)
            ->field(
                "d.type,d.welcome_email,a.type as up_type,c.id,b.invoiceid as invoice_id,a.amount,a.new_cycle as billingcycle,c.serverid,c.username,c.password,d.name,a.description,a.status as domainstatus",
            )
            ->where("b.id", $id)
            ...
            ->select()
            ->toArray();
        $upgrades_configoptions = \think\Db::name("upgrades")
            ->alias("a")
            ->leftJoin("orders b", "a.order_id = b.id")
            ->leftJoin("host c", "a.relid = c.id")
            ->leftJoin("products d", "c.productid = d.id")
            ->distinct(true)
            ->field(
                "d.type,d.welcome_email,a.type as up_type,c.id,b.invoiceid as invoice_id,a.amount,a.new_cycle as billingcycle,c.serverid,c.username,c.password,d.name,a.description,a.status as domainstatus",
            )
        ...
        foreach ($servers as $k => $v) {
            ...
            $servers[$k]["domainstatus"] =
                config("domainstatus")[$servers[$k]["domainstatus"]] ?? $upgrad_status[$servers[$k]["domainstatus"]];
    ...
    public function active()
    {
        ...
        \think\Db::name("orders")
            ->where("id", $id)
            ->update(["status" => "Active", "update_time" => time()]);
        $st = config("order_status");
        active_log(
            sprintf($this->lang["Order_admin_active"], $id, $order["uid"], $st["Active"]["name"]),
            $order["uid"],
        );
        active_log(
            sprintf($this->lang["Order_admin_active"], $id, $order["uid"], $st["Active"]["name"]),
        ...
        foreach ($hosts as $host) {
            $host_data = \think\Db::name("host")
                ->alias("a")
                ->field("a.*,b.welcome_email,b.auto_setup")
            ...
            \think\Db::name("host")
                ...
                ->update([
                    ...
                    "password" => cmf_encrypt($host["password"]),
    ...
    public function changeStatus()
    {
        ...
        active_log(
            sprintf($this->lang["Order_admin_ordersttaus"], $id, $orderss["uid"], $st[$status]["name"]),
            $orderss["uid"],
        );
        active_log(
            sprintf($this->lang["Order_admin_ordersttaus"], $id, $orderss["uid"], $st[$status]["name"]),
    ...
    public function customPromo()
    {
        if ($this->request->isPost()) {
            ...
            $rule = [
                "code" => "require|max:10",
                "type" => "in:percent,fixed,override,free",
                "value" => "float",
                "recurring" => "in:0,1",
                "recurfor" => "number",
            ];
            $msg = [
                "code.require" => lang("PROMO_CODE_REQUIRE"),
                "type.in" => lang("PROMO_CODE_TYPE_ERROR"),
                "value.float" => lang("PROMO_CODE_VALUE_ERROR"),
                "recurfor.number" => lang("PROMO_CODE_CYCLE_TIMES_ERROR"),
            ];
            ...
            if (!empty($code_exist)) {
                active_log(
                    sprintf($this->lang["Order_admin_create_customPromo_success"], lang("PROMO_CODE_ALLREADY_EXIST")),
                );
                $result["status"] = 400;
