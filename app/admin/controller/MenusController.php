...
class MenusController extends AdminBaseController
{
    ...
    protected $menuType = [
        "client" => [
            "field" => "client",
            "name" => "会员中心导航",
            "desc" => "此处可对会员中心导航进行修改，并放置在会员中心左侧",
        ],
        "www_top" => [
            "field" => "www_top",
            "name" => "官网顶部导航",
            "desc" => "此处可对官网顶部导菜单航进行修改，并放置在官网页面顶部",
        ],
        "www_bottom" => [
            "field" => "www_bottom",
            "name" => "官网底部导航",
            "desc" => "此处可对官网底部导菜单航进行修改，并放置在官网页面底部",
        ],
    ];
    ...
    public function getMenuList()
    {
        ...
        return $this->tryCatch(function () use ($param) {
            ...
            \think\Db::name($this->menu_table)
                ->whereNotIn("id", $active_menu)
                ->update(["sort" => 0]);
            \think\Db::name($this->menu_table)
                ->whereIn("id", $active_menu)
                ->update(["sort" => 1]);
            $model = $model
                ->page(max(1, $param["page"]), $this->getLimit())
        ...
        return $this->tryCatch(function () use ($param, $db_param) {
            ...
            foreach ($nav_list as $key => $val) {
                if ($nav_list[$key]["lang"]) {
                    $nav_list[$key]["lang"] = is_array($nav_list[$key]["lang"])
                        ? json_encode($nav_list[$key]["lang"])
                        : $nav_list[$key]["lang"];
                }
            ...
            return \think\Db::name($this->menu_table)
                ->where("id", $param["menu_id"])
                ->update(["nav_list" => json_encode($this->getTree($nav_list)), "name" => $param["menu_name"]]);
        });
    }
    /**
    ...
    protected function setNavListCheck($param)
    {
        ...
        $menu_model = \think\Db::name($this->menu_table)
            ->where("name", $param["menu_name"])
            ->where("id", "<>", $param["menu_id"])
            ->find();
        if ($menu_model) {
    ...
    public function getDefaultSenior()
    {
        return $this->tryCatch(function () {
            $param = $this->request->param();
            return [
                "orderFucUrl" => $this->getOrderFucUrl($param["pIds"]),
                "templatePage" => $this->getTemplatePage($param["pIds"]),
            ];
        });
    }
    /**
    ...
    public function addCustomPage()
    {
        ...
        return $this->tryCatch(function () use ($param) {
            ...
            $data = [
                ...
                "pid" => 0,
                "order" => 0,
                "fa_icon" => $param["fa_icon"] ?: "",
                "nav_type" => 1,
                "relid" => $param["relid"] ?: "",
                "menuid" => 0,
                "lang" => $param["lang"] ?: "",
                "menu_type" => $menu_model["type"] ?: 1,
            ];
    ...
    public function addProductPage()
    {
        ...
        return $this->tryCatch(function () use ($param) {
            ...
            \think\Db::name("nav")
                ->where("id", $pid)
                ->update(["url" => "service?groupid=" . $pid]);
            return $pid;
        });
    }
    /**
    ...
    public function getSystemNav()
    {
        $param = $this->request->param();
        return $this->tryCatch(function () use ($param) {
            $nav = \think\Db::name("nav")
                ->field("id,name,pid")
                ->where("menu_type", intval($param["menu_type"]))
                ->where("nav_type", intval($param["nav_type"]))
                ->where("menuid", 1)
                ->order("order")
                ->select()
                ->toArray();
    ...
    public function getProductList()
    {
        $param = $this->request->param();
        return $this->tryCatch(function () use ($param) {
            $p_arr = \think\Db::name($this->menu_table)
                ->field("nav_list")
                ->where("id", intval($param["m_id"]))
                ->column("nav_list");
            $relid = [0];
                ...
                if ($val) {
                    ...
                    foreach ($info_arr as $key => $info) {
                        if ($info["nav_type"] == 2 && $info["relid"]) {
                            $relid =
                                $info["id"] != intval($param["id"])
                                    ? array_merge($relid, explode(",", $info["relid"]))
                                    : $relid;
                        }
                    }
                }
            ...
            $model = \think\Db::name("products")
                ->field("id,gid,name,type")
                ->whereNotIn("id", array_unique($relid))
                ->select()
                ->toArray();
            if (!$model) {
            ...
            $model_group = \think\Db::name("product_groups")
                ->field("id,name")
            ...
            return [
                "list" => array_values($model_group),
                "id" => $menu_id ? $menu_id["id"] : 0,
    ...
    public function createWebPage()
    {
        return $this->tryCatch(function () {
            ...
            return \think\Db::name("nav")->insertGetId([
                "name" => $param["name"],
                "url" => $param["url"],
                "pid" => 0,
                "nav_type" => $param["type"],
    ...
    public function getMenuType()
    {
        return $this->tryCatch(function () {
            ...
            $list_data = \think\Db::name($this->menu_table)
                ->field("id,name,type")
                ->order(["sort" => "desc", "id" => "desc"])
                ->select()
                ->toArray();
    ...
    public function getOtherMenu()
    {
        return $this->tryCatch(function () {
            ...
            $data = \think\Db::name($this->menu_table)
                ->where("id", "<>", $param["id"])
    ...
    public function addMenu()
    {
        ...
        return $this->tryCatch(function () use ($param) {
            ...
            return \think\Db::name($this->menu_table)->insertGetId([
                "name" => $param["name"],
                "type" => intval($param["menu_type"]),
                "nav_list" => json_encode($this->getTree($nav_arr)),
                "active" => 1,
            ]);
        });
    }
    ...
    public function editMenu()
    {
        ...
        return $this->tryCatch(function () use ($param) {
            ...
            return \think\Db::name($this->menu_table)->update([
                "name" => $param["name"],
                "type" => intval($param["menu_type"]),
                "nav_list" => "",
                "active" => 1,
        ...
        return $this->tryCatch(function () use ($param) {
            return \think\Db::transaction(function () use ($param) {
                ...
                \think\Db::name("menu_active")
                    ->where("menuid", intval($param["old_menu_id"]))
                    ->update(["menuid" => intval($param["menu_id"])]);
                $this->directDel($param);
    ...
    public function getCreateWebData()
    {
        return $this->tryCatch(function () {
            ...
            return [
                "all_tpl" => $tpl,
                "topGroups" => $topGroups ? array_column($topGroups, "name", "id") : [],
                "groups" => $groups ? array_column($groups, "name", "id") : [],
                "sys_tpl" => $sys_tpl,
            ];
        });
    }
    ...
    protected function clentareaDefaultNav(&$nav_arr)
    {
        $data = [
            ...
            "relid" => "",
            "menuid" => 1,
            "lang" => json_encode(["chinese" => "所有产品", "chinese_tw" => "所有產品", "english" => "All products"]),
            "plugin" => "",
            "menu_type" => 1,
        ];
        ...
        \think\Db::name("nav")
            ->where("id", $all_p_id)
            ->update(["url" => "service?groupid=" . $all_p_id]);
        ...
        $other_data = [
            ...
            "order" => $all_p_id,
            "senior" => 0,
            "templatePage" => $this->getTemplatePage($relid),
            "orderFuc" => 1,
            "is_custom" => 0,
            "orderFucUrl" => $this->getOrderFucUrl($relid),
        ];
    ...
    private function delWebCustomMenu($param)
    {
        ...
        return \think\Db::name("nav")
            ->whereIn("id", array_unique($param["del_ids"]))
            ->where("menuid", 0)
    ...
    public function addHookMenu($data)
    {
        return $this->tryCatch(function () use ($data) {
            ...
            $arr = [
                ...
                $this->hook_name => $data[$this->hook_name],
            ];
    ...
    private function getTopGroupData($data)
    {
        ...
        $first_group = \think\Db::name("product_first_groups")
            ->field("id, name, order")
            ->whereIn("id", $data["relid"])
            ->select();
        $result = [];
        foreach ($first_group as $k => $v) {
            $result_val = [
                ...
                "nav_type_name" => "一级分组",
                "is_display" => 1,
                "son" => $this->getGroupsData([["gid", "=", $v["id"]]], true),
            ];
            $result[] = $result_val;
        }
        return $result;
