...
class ConfigOptionsController extends AdminBaseController
{
	...
	*/
    public function groupsList()
    {
        ...
        $list = \think\Db::name("product_config_groups")
            ->where("global", 1)
        ...
        foreach ($list as $key => $value) {
            $products = \think\Db::name("product_config_links")
                ->alias("a")
                ->field("b.id,b.name,b.type")
                ->leftJoin("products b", "a.pid = b.id")
                ->where("a.gid", $value["id"])
                ->where(function (\think\db\Query $query) use ($value) {})
                ->select()
                ->toArray();
    ...
    public function createGroups()
    {
        ...
        $result = \think\Db::name("products")
            ->alias("p")
            ->field("pg.name as pg_name,p.name as p_name,p.id as p_id,CONCAT(pg.name,\"：\",p.name) as link,pg.type")
    ...
    public function editGroups()
    {
        ...
        if ($gid) {
            ...
            $prolink = \think\Db::name("products")
                ->alias("p")
                ->field(
                    "pg.name as pg_name,p.name as p_name,p.id as p_id,CONCAT(pg.name,\"：\",p.name) as link,pg.type",
                )
                ->leftJoin("product_groups pg", "p.gid = pg.id")
                ->where($where)
                ->select();
	...
	*/
    public function editGroupsPost()
    {
        if ($this->request->isPost()) {
            ...
            $product = \think\Db::name("product_config_links")
                ->alias("a")
                ->field("b.api_type")
                ->leftJoin("products b", "a.pid=b.id")
            ...
            if ($product["api_type"] == "zjmf_api" && $pcg["upstream_id"] > 0) {
                $product = \think\Db::name("products")
                    ->alias("a")
                    ->leftJoin("product_config_links b", "a.id = b.pid")
                    ->where("b.gid", $gid)
                    ->find();
                $param["productlinks"] = [];
                if ($product["upstream_price_type"] == "custom") {
                    $location_options = \think\Db::name("product_config_options")
                        ->where("gid", $gid)
                        ->select()
                        ->toArray();
                            ...
                            foreach ($config_groups as $config_group) {
                                ...
                                foreach ($options1 as $option) {
                                    foreach ($location_options as $location_option) {
                                        if (
                                            $location_option["upstream_id"] == $option["id"] &&
                                            $option["upgrade"] == 0
                                        ) {
            ...
            try {
                    ...
                    if (!empty($param["productlinks"]) && is_array($param["productlinks"])) {
                        ...
                        if ($gids != $gids1) {
                            $product = \think\Db::name("products")
                                ...
                                ->select()
                                ->toArray();
                            $product1 = \think\Db::name("products")
                                ->field("name")
                                ->where("id", "in", $gids1)
                                ->select()
                                ->toArray();
                ...
                foreach ($orders as $key => $value) {
                    ...
                    $pco = \think\Db::name("product_config_options")
                        ->field("option_name,order,hidden,upgrade,is_discount,option_type")
                        ->where("id", intval($key))
                        ->find();
                    if ($pco["option_type"] == 20) {
                        \think\Db::name("product_config_options")
                            ->where("linkage_top_pid", intval($key))
                            ->update($options);
                    }
        ...
        if (!empty($pid)) {
            ...
            if ($paytype == "onetime") {
                ...
                foreach ($currencies as $v) {
                    $product_pricing = \think\Db::name("pricing")
                        ->where("type", "product")
                        ->where("currency", $v["id"])
                        ->where("relid", $pid)
                        ->find();
            ...
            $data = [
                "paytype" => $paytype,
                "pay_type_recurring" => $pay_type_recurring,
                "pay_ontrial_status" => $pay_ontrial_status,
                "cycle" => $field,
        ...
        if ($this->request->isPost()) {
            ...
            try {
                ...
                if ($optionfilter["linkage_pid"] == 0) {
                    ...
                    $linkage_top_data = \think\Db::name("product_config_options")
                        ->where("id", $optionfilter["linkage_pid"])
                        ->find();
                ...
                $suboption["sort_order"] =
                    isset($param["addsortorder"]) && !empty($param["addsortorder"]) ? ceil($param["addsortorder"]) : 0;
    ...
    public function deleteSubOptions()
    {
        ...
        if ($subid) {
            ...
            try {
                ...
                $sub_ids = \think\Db::name("product_config_options_sub")
                    ->where("id", $subid)
                    ->whereOr("linkage_level", "like", "%-" . $subid . "-%")
                    ->column("id");
                $sub_ids = $sub_ids ?: [0];
    ...
    public function deleteOptions()
    {
        ...
        if ($cid) {
            ...
            try {
                ...
                $subids = \think\Db::name("product_config_options_sub")
                    ->field("id")
                    ->where("config_id", $cid)
                    ->select();
                if (!empty($subids)) {
            ...
            try {
                ...
                if (!empty($cids)) {
                    foreach ($cids as $cid) {
                        $subids = \think\Db::name("product_config_options_sub")
                            ->field("id")
                            ->where("config_id", $cid["id"])
                            ->select();
                        if (!empty($subids)) {
                            foreach ($subids as $subid) {
                                \think\Db::name("pricing")
                                    ->where("type", "configoptions")
                                    ->where("relid", $subid["id"])
                                    ->delete();
                            }
                        }
    ...
    {
        $result = \think\Db::name("product_config_groups")
            ->alias("pcg")
            ->field("id,name,description,CONCAT(name,\"--\",description) as links")
            ->where("global", 1)
            ->select();
        if ($result) {
            ...
            try {
                ...
                if (!empty($oldoptions[0])) {
                    foreach ($oldoptions as $oldoption) {
                        ...
                        $oldcid = $oldoption["id"];
                        $oldsuboptions = \think\Db::name("product_config_options_sub")
                            ->where("config_id", $oldcid)
                            ->select()
                            ->toArray();
                        if (!empty($oldsuboptions[0])) {
                            foreach ($oldsuboptions as $oldsuboption) {
                                ...
                                $oldpricings = \think\Db::name("pricing")
                                    ->field("id,relid", true)
                                    ->where("type", "configoptions")
                                    ->where("relid", $oldsubid)
                                    ->select()
                                    ->toArray();
    ...
    public function configOptionsCheckOs()
    {
        ...
        $products = \think\Db::name("products")
            ->alias("a")
            ->leftJoin("servers s", "s.gid=a.server_group")
            ->leftJoin("product_config_links b", "a.id=b.pid")
            ->leftJoin("shd_product_config_options c", "b.gid=c.gid")
            ->field("a.id as pid,a.name,s.server_type as type")
            ->where(
                "(s.server_type='dcimcloud' OR s.server_type='dcim') AND s.gid>0 AND a.server_group>0 AND c.option_type=5 AND (a.api_type='normal' OR a.api_type='')",
            )
            ->order("a.id", "desc")
            ->select()
            ->toArray();
    ...
    public function configOptionsOs()
    {
        ...
        $products = \think\Db::name("products")
            ...
            ->leftJoin("product_config_links b", "a.id=b.pid")
            ->leftJoin("shd_product_config_options c", "b.gid=c.gid")
            ->field("a.server_group,c.id as cid,a.api_type,a.type")
            ->where(["a.id" => $pid, "c.option_type" => 5])
            ->find();
        $cid = $products["cid"];
        if (empty($cid)) {
            return jsonrule(["status" => 400, "pid" => $pid, "msg" => lang("ID_ERROR")]);
        ...
        if (
            ($products["type"] != "dcim" && $products["type"] != "dcimcloud") ||
            ($products["api_type"] != "normal" && $products["api_type"] != "")
        ) {
            return jsonrule(["status" => 400, "pid" => $pid, "msg" => "只有本地接口（魔方云或者DCIM）才能拉取"]);
        }
        $server_info = \think\Db::name("servers")
            ->field("id,server_type")
            ->where("gid", $products["server_group"])
            ->where("server_type='dcimcloud' OR server_type='dcim'")
            ->find();
        ...
        if (is_array($getOs_name)) {
            $config_options_sub = \think\Db::name("product_config_options_sub")
                ->field("id,option_name,hidden")
                ->where("config_id", $cid)
                ->select()
                ->toArray();
            ...
            foreach ($hidden as $hidden_k => $hidden_v) {
                if ($hidden_v == 1 && in_array($config_options_sub_name[$hidden_k], $getOs_name)) {
                    \think\Db::name("product_config_options_sub")
                        ->where("id", $hidden_k)
                        ->update(["hidden" => 0]);
                    $num_update++;
                }
                ...
                foreach ($sub_name_diff as $sub_k => $sub_diff) {
                    ...
                    if ($hidden[$sub_k] == 1) {
                        if ($getOs_name_new[$id]) {
                            \think\Db::name("product_config_options_sub")
                                ->where("id", $sub_k)
                                ->update(["option_name" => $getOs_name_new[$id], "hidden" => 0]);
                            $num_update++;
                        }
                    } else {
                        if ($getOs_name_new[$id]) {
                            ...
                            \think\Db::name("product_config_options_sub")
                                ->where("id", $sub_k)
                                ->update(["option_name" => $sub_diff, "hidden" => 1]);
                        }
                        $num_update++;
                    }
                }
        ...
        if ($num_update == 0 && $num_insert == 0) {
            ...
            $desc = lang("CONFIG_GETOS_ACTIVE_LOG", [
                "cid" => $cid,
            ...
            return jsonrule([
                "status" => 200,
                "pid" => $pid,
                "msg" => lang("CONFIG_GETOS_SUCCESS_NUM", ["num_update" => $num_update, "num_insert" => $num_insert]),
            ]);
            exit();
        }
    }
    ...
    public function editConfig()
    {
        ...
        if ($optiontype == 3) {
            ...
            foreach ($suboptions as $key => $suboption) {
                ...
                $pricings = \think\Db::name("pricing")
                    ->alias("p")
                    ->field("c.code,p.currency" . $field)
                    ->leftJoin("currencies c", "c.id = p.currency")
        ...
        if ($option["upstream_id"] > 0) {
            $product = \think\Db::name("products")
                ->alias("a")
                ->field("a.upstream_price_type,a.api_type,a.zjmf_api_id,a.upstream_pid")
                ->leftJoin("product_config_links b", "a.id = b.pid")
                ->leftJoin("product_config_groups c", "b.gid = c.id")
                ->leftJoin("product_config_options d", "d.gid = c.id")
                ->where("d.id", $cid)
                ->find();
            if (!empty($product)) {
                $res = getZjmfUpstreamProductConfig($product["zjmf_api_id"], $product["upstream_pid"]);
                                ...
                                if ($is_upstream_percent_product) {
                                    foreach ($upstream_config_pricings as $k4 => $v4) {
                                        foreach ($price_type as $v5) {
                                            $upstream_config_pricings[$k4][$v5[0]] =
                                                ($v4[$v5[0]] * $upstream_product["upstream_price_value"]) / 100;
                                            $upstream_config_pricings[$k4][$v5[1]] =
                                                ($v4[$v5[1]] * $upstream_product["upstream_price_value"]) / 100;
                                        }
                                    }
                                }
    ...
    public function getNextLinkAgeList()
    {
        try {
            $param = $this->request->param();
            $config_id = \think\Db::name("product_config_options_sub")
                ->where("linkage_pid", $param["id"])
                ->value("config_id");
            if (!$config_id) {
                return jsonrule([
                    "status" => 200,
                    "msg" => lang("SUCCESS MESSAGE"),
                    "data" => ["sub_pid" => $param["id"]],
                ]);
            }
            $option_data = \think\Db::name("product_config_options")->where("id", $config_id)->find();
            $option_sub = \think\Db::name("product_config_options_sub")
    ...
    {
        if ($type == "options") {
            $groups = \think\Db::name("product_config_groups")
                ->alias("a")
                ->field("b.pid")
                ->leftJoin("product_config_links b", "a.id = b.gid")
                ->leftJoin("product_config_options c", "a.id = c.gid")
                ->where("c.id", $id)
                ->select()
                ->toArray();
            ...
            $groups = \think\Db::name("product_config_groups")
                ...
                ->leftJoin("product_config_links b", "a.id = b.gid")
                ->leftJoin("product_config_options c", "a.id = c.gid")
                ->leftJoin("product_config_options_sub d", "c.id = d.config_id")
                ->where("d.id", $id)
                ->select()
                ->toArray();
        } else {
    ...
    public function saveLinkAgeLevel()
    {
        try {
            ...
            $option_sub_data = [
                "option_name" => $param["option_sub_name"],
                "qty_minimum" => 0,
                "qty_maximum" => 0,
                "hidden" => $param["hidden"] ?: 0,
                "config_id" => $param["option_id"],
                "linkage_pid" => $param["sub_linkage_pid"] ?: 0,
            ];
            if ($param["sub_option_id"]) {
                \think\Db::name("product_config_options_sub")
                    ->where("id", $param["sub_option_id"])
                    ->update($option_sub_data);
            } else {
            ...
            if ($option_sub_data["linkage_pid"] == 0) {
                ...
                $sub_linkage_top_data = \think\Db::name("product_config_options_sub")
                    ->where("id", $option_sub_data["linkage_pid"])
                    ->find();
                ...
                $sub_update_option["linkage_level"] =
                    $sub_linkage_top_data["linkage_level"] . "-" . $param["sub_option_id"];
            }
            \think\Db::name("product_config_options_sub")
                ->where("id", $param["sub_option_id"])
                ->update($sub_update_option);
            return jsonrule([
                "status" => 200,
                "msg" => "success",
                "option_id" => $param["option_id"],
                "sub_option_id" => $param["sub_option_id"],
            ]);
        } catch (\Throwable $e) {
            return jsonrule(["status" => 400, "msg" => $e->getMessage()]);
        }
    ...
    private function saveConfigOption()
    {
        ...
        $option_data = [
            ...
            "option_type" => $param["option_type"] ?: 1,
            "notes" => $param["notes"] ?: "",
            "qty_stage" => $param["qty_stage"] ?: 0,
            "linkage_pid" => $param["linkage_pid"] ?: 0,
            "is_discount" => $param["is_discount"] ?: 0,
            "unit" => $param["unit"] ?: "",
            "senior" => $param["senior"] ?: 0,
            "upgrade" => 0,
        ];
        if ($param["option_id"]) {
        ...
        if ($option_data["linkage_pid"] == 0) {
            ...
            $linkage_top_data = \think\Db::name("product_config_options")
                ->where("id", $option_data["linkage_pid"])
                ->find();
            $update_option["linkage_top_pid"] = $linkage_top_data["linkage_top_pid"];
            if ($linkage_top_data["linkage_pid"] == 0) {
    ...
    public function delLinkAgeSub()
    {
        try {
            ...
            $sub_ids = \think\Db::name("product_config_options_sub")
                ->where("id", $param["sub_id"])
                ->whereOr("linkage_level", "like", "%-" . $param["sub_id"] . "-%")
                ->column("id");
            $sub_ids = $sub_ids ?: [0];
            \think\Db::name("product_config_options_sub")->whereIn("id", $sub_ids)->delete();
            \think\Db::name("pricing")->where("type", "configoptions")->whereIn("relid", $sub_ids)->delete();
