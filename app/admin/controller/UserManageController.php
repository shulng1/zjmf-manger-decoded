...
class UserManageController extends GetUserController
{
    ...
    public function getUser()
    {
        ...
        $client = \think\Db::name("clients")
            ->field("id,currency,username,companyname,activation,is_open_credit_limit")
            ->where("id", $uid)
            ->find();
        $developer = \think\Db::name("developer")
            ->where("uid", $uid)
            ->where("status", "Active")
            ->order("id", "desc")
            ->find();
    ...
    public function clientList()
    {
        ...
        $fun = function (\think\db\Query $query) use ($data) {
            if (!empty($data["username"])) {
                $certifi_uid = \think\Db::name("certifi_log")
                    ->where("certifi_name", "like", "%" . trim($data["username"]) . "%")
                    ->where("status", 1)
                    ->column("uid");
        ...
        $obj1 = \think\Db::name("clients")
            ->alias("cli")
            ->leftJoin("certifi_company cc", "cc.auth_user_id = cli.id")
            ->leftJoin("certifi_person cp", "cp.auth_user_id = cli.id")
            ->leftJoin("wechat_user wu", "cli.wechat_id = wu.id")
            ->leftJoin("client_groups cg", "cg.id = cli.groupid")
            ->leftJoin("user u", "cli.sale_id=u.id");
        if (isset($params["custom"]) && !empty($params["custom"])) {
            $obj1 = $obj1
                ->leftJoin("customfieldsvalues cfv", "cfv.relid = cli.id")
                ->leftJoin("customfields cf", "cf.id = cfv.fieldid")
                ->where("cf.type", "client")
                ->where("cf.id", array_keys($params["custom"])[0])
                ->where("cfv.value", "like", "%" . array_values($params["custom"])[0] . "%");
        }
        ...
        $obj = \think\Db::name("clients")
            ->alias("cli")
            ->field(
                "cli.id,cli.username,cli.sale_id,cg.group_colour,cli.companyname,cli.phonenumber,cli.email,cli.create_time,cli.qq,
            cc.status as company_status,cp.status as person_status,wu.id as wechat_id,cli.lastlogin,cli.status as client_status,
            u.user_nickname,cli.credit,cu.prefix,cu.id as currency_id,cli.track_status,cg.group_name,cg.group_colour,cli.api_open,cli.credit_limit,cli.api_create_time",
            )
            ->leftJoin("currencies cu", "cli.currency=cu.id")
            ->leftJoin("certifi_company cc", "cc.auth_user_id = cli.id")
            ->leftJoin("certifi_person cp", "cp.auth_user_id = cli.id")
            ->leftJoin("wechat_user wu", "cli.wechat_id = wu.id")
            ->leftJoin("client_groups cg", "cg.id = cli.groupid")
        ...
        if (isset($params["custom"]) && !empty($params["custom"])) {
            ...
            foreach ($params["custom"] as $key => $val) {
                $relid = \think\Db::name("customfieldsvalues")
                    ->where("fieldid", $key)
                    ->whereLike("value", "%" . $val . "%")
                    ->column("relid");
                $cli_ids[] = $relid;
        ...
        foreach ($list as $key => $value) {
            ...
            $value["amount_to_be_settled"] = \think\Db::name("invoices")
                ->where("status", "Paid")
        ...
        return jsonrule([
            ...
            "seachData" => ["sale" => get_sale(), "client_groups" => $clientGroupsFilter],
            "allow_resource_api" => configuration("allow_resource_api") ?? 0,
        ]);
    }
    /**
     * @title 单个客户资料详情
    ...
    public function summary()
    {
        ...
        $summary = \think\Db::name("clients")
            ->alias("c")
            ->leftJoin("client_groups cg", "c.groupid = cg.id")
            ->field(
                "username,companyname,country,province,city,address1,know_us,phone_code,phonenumber,email,currency,credit
            ,status,create_time,lastlogin,lastloginip,host,cg.group_name,qq,sale_id,notes,api_open",
            )
            ->where("c.id", $clientId)
            ->withAttr("phonenumber", function ($value, $data) {
                if (empty($value)) {
                    return "";
                } else {
        ...
        $accounts = \think\Db::name("accounts")
            ->field("amount_in,amount_out,fees")
            ->where("uid", $clientId)
            ->where("delete_time", 0)
            ->select();
        ...
        $accounts_count = \think\Db::name("accounts")
            ->where("uid", $clientId)
            ->where("amount_in", ">", 0)
            ->where("delete_time", 0)
            ->count();
        $re["accounts_count"] = $accounts_count;
        $accounts_out_count = \think\Db::name("accounts")
            ->where("uid", $clientId)
            ->where("amount_out", ">", 0)
            ->where("delete_time", 0)
            ->count();
        ...
        $otherInfo["groupname"] = !empty($summary["group_name"])
            ? $summary["group_name"]
            : config("custom_brokerage")["none"];
        $otherInfo["create_time"] = $summary["create_time"];
        $otherInfo["register_time"] =
            floor((time() - $summary["create_time"]) / 24 / 60 / 60) . config("billing_cycle")["day"];
        ...
        $due_host_total = \think\Db::name("host")
            ->field("sum(firstpaymentamount) as firstpaymentamount,count(id) as due_host_count")
            ->where("uid", $clientId)
            ->where("nextduedate", ">", 0)
            ->where("nextduedate", "<=", time() + 604800)
            ->find();
        $re["being_due_host"] = [
            "count" => $due_host_total["due_host_count"] ?? 0,
            "total" => $currency["prefix"] . number_format(round($due_host_total["firstpaymentamount"] ?? 0.0, 2), 2),
        ];
        $due_total = \think\Db::name("host")
            ->field("sum(firstpaymentamount) as firstpaymentamount,count(id) as due_host_count")
            ->where("uid", $clientId)
            ->where("nextduedate", ">", 0)
            ->where("nextduedate", "<=", time())
            ->where("domainstatus", "<>", "Pending")
            ->find();
        $re["due_host"] = [
            "count" => $due_total["due_host_count"] ?? 0,
            "total" => $currency["prefix"] . number_format(round($due_total["firstpaymentamount"] ?? 0.0, 2), 2),
        ];
        $hostserver = [];
        ...
        $contacts = \think\Db::name("contacts")
            ->field("username,email,concat(username,\"-\",email) as contact")
            ->where("uid", $clientId)
            ->limit(5)
            ->select();
        ...
        $customfields = \think\Db::name("customfields")
            ->alias("a")
            ->field("a.fieldname,b.value")
            ->leftJoin("customfieldsvalues b", "a.id = fieldid")
            ->where("a.type", "client")
            ->where("b.relid", $clientId)
            ->select()
            ->toArray();
        $re["customfields"] = $customfields;
        $plugins_oauth = [];
        $list = \think\Db::name("plugin")
            ->where(["module" => "oauth", "status" => 1])
            ->order("order", "asc")
            ->select();
        $clients_oauth = \think\Db::name("clients_oauth")
            ->where(["uid" => $clientId])
            ->select()
            ->toArray();
        ...
        foreach ($list as $k => $plugin) {
            ...
            if (!empty($clients_oauth[$plugin["name"]])) {
                ...
                if (stripos($oauth_img, ".svg") === false) {
                    $plugins_oauth[$k]["img"] =
                        "<img width=30 height=30 src=\"" . base64EncodeImage($oauth_img) . "\" />";
                } else {
    ...
    public function clientListRe()
    {
        ...
        $total = \think\Db::name("clients")
            ...
            ->where(["cli.sale_id" => 0])
            ->count();
        $list = \think\Db::name("clients")
            ->alias("cli")
            ->field("cli.id,cli.credit,cli.username,cli.create_time")
            ->where(function (\think\db\Query $query) use ($data) {
                if (!empty($data["username"])) {
                    $query->where("cli.username", "like", "%" . trim($data["username"]) . "%");
                }
    ...
    public function hostByUid()
    {
        ...
        if ($params["source"] == "ticket") {
            ...
            $host_list = \think\Db::name("host")
                ->field(
                    "h.id,h.initiative_renew,h.domain,h.uid,h.regdate as create_time,h.productid,h.billingcycle,h.domain,h.payment,h.nextduedate,h.firstpaymentamount,h.amount,h.domainstatus,c.username,p.name as productname,c.currency,p.type,h.serverid",
                )
                ->alias("h")
                ->leftJoin("products p", "p.id=h.productid")
                ->leftJoin("clients c", "c.id=h.uid")
                ->where($fun)
                ->where("h.uid", $uid)
                ->order("h.id", "desc")
                ->page($page)
            ...
            foreach ($host_list as &$v) {
                ...
                $v["firstpaymentamount"] =
                    $currency[$v["currency"]]["prefix"] .
                    $v["firstpaymentamount"] .
                    $currency[$v["currency"]]["suffix"];
            }
            $result["status"] = 200;
        ...
        $hosts = \think\Db::name("host")
            ->alias("h")
            ->field(
                "h.id as hid,p.name,h.domain,h.amount,h.firstpaymentamount,h.dedicatedip,h.billingcycle,h.regdate,h.nextduedate,h.domainstatus,p.type as host_type,p.type as ptype",
            )
            ->leftJoin("products p", "h.productid = p.id")
            ->where("uid", $uid)
            ->withAttr("host_type", function ($value) use ($product_type) {
        ...
        $count = \think\Db::name("host")
            ->alias("h")
            ->field(
                "h.id as hid,concat(p.name,\"-\",h.domain) as hostname,h.amount,h.firstpaymentamount,h.dedicatedip,h.billingcycle,h.regdate,h.nextduedate,h.domainstatus,p.type as host_type",
            )
            ->leftJoin("products p", "h.productid = p.id")
            ->where($where)
            ->where("uid", $uid)
    ...
    {
        $total = \think\Db::name("host")
            ->alias("h")
            ->leftJoin("products p", "p.id = h.productid")
            ->where("h.uid", $clientId)
            ->where("p.type", $type)
            ->count();
        $active = \think\Db::name("host")
            ->alias("h")
            ->leftJoin("products p", "p.id = h.productid")
            ->where("h.uid", $clientId)
            ->where("p.type", $type)
    ...
    {
        $active1 = \think\Db::name("host")
            ->alias("h")
            ->field("count(h.id) as count,p.type")
            ->leftJoin("products p", "p.id = h.productid")
            ->where("h.uid", $clientId)
            ->group("p.type")
            ->select()
        ...
        $active = \think\Db::name("host")
            ...
            ->leftJoin("products p", "p.id = h.productid")
            ->where("h.uid", $clientId)
            ->whereIn("h.domainstatus", ["Active", "Verifiy_Active", "Issue_Active"])
            ->group("p.type")
            ->select()
            ->toArray();
    ...
    private function getInvoice2($clientId, $status, $currency)
    {
        ...
        $subtotals = \think\Db::name("invoices")
            ->field("count(id) as count,sum(subtotal) as subtotal,status")
            ->where("uid", $clientId)
            ->where("status", $status)
            ->where("delete_time", 0)
            ->find();
    ...
    public function profile()
    {
        ...
        $customs = \think\Db::name("customfields")
            ->field("id,fieldname,fieldtype,description,fieldoptions,regexpr,required,sortorder")
            ->where("type", "client")
            ->select()
            ->toArray();
        $client_customs = \think\Db::name("customfields")
            ...
            ->select()
            ->toArray();
        $profile = \think\Db::name("clients")
            ->field(
                "username,sex,profession,signature,companyname,country,province,address1,postcode,phone_code,phonenumber,email,currency,
            defaultgateway,notes,groupid,status,language,know_us,qq,sale_id,marketing_emails_opt_in,send_close",
            )
            ->where("id", $clientId)
            ->find();
        ...
        if ($aff_id > 0) {
            ...
            $profileFilter["aff_id_uid"] = \think\Db::name("affiliates_user_temp")
                ->where("uid", $clientId)
                ->value("affid_uid");
        }
        if ($profileFilter["aff_id_uid"] > 0) {
            $profileFilter["aff_id_username"] = \think\Db::name("clients")
                ->where("id", $profileFilter["aff_id_uid"])
                ->value("username");
        }
    ...
    public function profilePost()
    {
        if ($this->request->isPost()) {
            ...
            if (!empty($params["phonenumber"])) {
                $phone_exist = \think\Db::name("clients")
                    ->where("id", "<>", $uid)
            ...
            if (!empty($params["email"])) {
                $email_exist = \think\Db::name("clients")
                    ->where("id", "<>", $uid)
            ...
            if ($result) {
                ...
                if (!empty($params["defaultgateway"]) && $params["defaultgateway"] != $resultuid["defaultgateway"]) {
                    ...
                    $dec .=
                        "选择默认支付接口由“" .
                        $arr[$resultuid["defaultgateway"]] .
                        "”改为“" .
                        $arr[$params["defaultgateway"]] .
                        "”，";
                }
                ...
                if (!empty($params["groupid"]) && $params["groupid"] != $resultuid["groupid"]) {
                    $clientGroups = \think\Db::name("client_groups")
                        ->field("id,group_name")
                        ->where("id", $resultuid["groupid"])
                        ->find();
                    $clientGroups1 = \think\Db::name("client_groups")
                        ->field("id,group_name")
                        ->where("id", $params["groupid"])
                        ->find();
                    $dec .=
                        "客户分组由“" . $clientGroups["group_name"] . "”改为“" . $clientGroups1["group_name"] . "”，";
                }
                ...
                if (!empty($params["sale_id"]) && $params["sale_id"] != $resultuid["sale_id"]) {
                    $uspre = \think\Db::name("user")
                        ->where("id", $resultuid["sale_id"])
                        ->field("user_nickname")
                        ->find();
                ...
                active_log(
                    sprintf($this->lang["UserManage_user_profilePost_success"], $uid, $dec),
                    $uid,
                    "",
                    1,
                    1,
    ...
    public function createClientPost()
    {
        if ($this->request->isPost()) {
            ...
            $hookreturns = hook("client_details_validate", [
                "username" => $params["username"],
                "sex" => $params["sex"],
                "avatar" => $params["avatar"],
                "profession" => $params["profession"],
            ...
            if (is_array($hookreturns) && !empty($hookreturns[0])) {
                active_log(
                    sprintf($this->lang["UserManage_user_createClientPost_fail"], implode("\r\n", $hookreturns)),
                );
            ...
            if (empty($params["email"]) && empty($params["phonenumber"])) {
                active_log(
                    sprintf($this->lang["UserManage_user_createClientPost_fail"], lang("SERVER_USER_PHONE_EMAIL")),
                );
                return jsonrule(["status" => 400, "msg" => lang("SERVER_USER_PHONE_EMAIL")]);
            }
            if (!empty($params["password"])) {
            ...
            hook("client_add", [
                ...
                "notes" => $params["notes"],
                "groupid" => $params["groupid"],
                "initiative_renew" => $params["initiative_renew"],
            ]);
            if ($userid) {
                if ($customs) {
                    $custom_model = new \app\common\model\CustomfieldsModel();
                ...
                active_log(
                    sprintf($this->lang["UserManage_user_createClientPost_success"], $params["username"], $userid),
                    $userid,
                );
                active_log(
                    sprintf($this->lang["UserManage_user_createClientPost_success"], $params["username"], $userid),
    ...
    public function closeClient()
    {
        ...
        if ($type == "close") {
            ...
            try {
                ...
                \think\Db::commit();
                active_log(
                    sprintf($this->lang["UserManage_user_closeClient_success"], $clients["username"], $uid),
                    $uid,
                );
        ...
        try {
            ...
            foreach ($customfields as $customfield) {
                if (is_array($customfield)) {
                    \think\Db::name("customfieldsvalues")
            ...
            foreach ($customfields2 as $customfield2) {
                $hosts2 = \think\Db::name("host")
                    ->field("id")
                    ->where("uid", $uid)
                    ->where("productid", $customfield2["relid"])
                    ->select();
                foreach ($hosts2 as $host2) {
                    \think\Db::name("customfieldsvalues")
                        ->where("fieldid", $customfield2["id"])
                        ->where("relid", $host2["id"])
                        ->delete();
                }
            }
            ...
            \think\Db::name("customfieldsvalues")
                ->whereIn("fieldid", array_column($customs, "id"))
                ->where("relid", $uid)
                ->delete();
            $custom_model = new \app\common\model\CustomfieldsModel();
            ...
            active_log(
                sprintf($this->lang["UserManage_user_deleteClient_success"], $clients["username"], $uid),
                $uid,
                "",
                2,
            );
    ...
    public function logRecord()
    {
        ...
        $limit = isset($param["limit"])
            ? intval($param["limit"])
            : (configuration("NumRecordstoDisplay") ?:
            config("limit"));
        ...
        $logs = \think\Db::name("activity_log")
            ->field("create_time,id,ipaddr,description,uid,user,port")
            ->where($fun)
            ->withAttr("description", function ($value, $data) {
                ...
                if (!empty($name)) {
                    foreach ($name as $k => $v) {
                        ...
                        if ($v == "Invoice ID") {
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/bill-detail?id=" .
                                $relid .
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/dcim-traffic?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/customer-view/product-innerpage?hid=" .
                                $relid .
                                "&id=" .
                                $host["uid"] .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/promo-code-add?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/order-detail?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/admin-edit?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/support-ticket?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/edit-product?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/add-server?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/balance-details?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/permissions-edit?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/customer-group?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/currency-settings?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                                $str .
                                "</span></a>";
    ...
    public function cerify_list()
    {
        ...
        return jsonrule([
            "status" => 200,
            "msg" => lang("SUCCESS MESSAGE"),
            "data" => ["list" => $list, "total" => $count, "page" => $page, "limit" => $limit],
        ]);
    }
    /**
     * @title 实名认证日志列表
    ...
    public function cerifyLogList22()
    {
        ...
        $list = \think\Db::name("certifi_log")
            ...
            ->select()
            ->toArray();
        $count = \think\Db::name("certifi_log")
            ->alias("a")
            ->leftJoin("clients b", "a.uid = b.id")
            ->field("a.*,b.username")
            ->where($where)
        ...
        foreach ($list as &$v) {
            ...
            $name = \think\Db::name("plugin")
                ->where("module", "certification")
                ->where("name", $v["certifi_type"])
                ->value("title");
            $v["certype"] = $name ?: "人工审核";
                ...
                foreach ($customfields as $customfield) {
                    ...
                    $i <= 10 &&
                        ($v["custom_fields_log_arr"][] = [
                            ...
                            "val" => $v["custom_fields_log"]["custom_fields" . $i] ?? "",
                        ]);
                }
            }
        }
    ...
    public function cerifyLogList()
    {
        ...
        return jsonrule([
            ...
            "msg" => lang("SUCCESS MESSAGE"),
            "data" => [
                "list" => $this->getCerifyPackData($model),
                "total" => $count,
                "page" => max(1, $param["page"]),
                "limit" => max(1, $param["limit"]),
            ],
        ]);
    }
    /**
    ...
    public function certifiPersonDetail()
    {
        ...
        if (isset($log["id"]) && $log["type"] > 1) {
            ...
            $status = \think\Db::name("certifi_company")
                ->where("auth_user_id", intval($params["client_id"]))
                ->value("status");
            if ($status == 2) {
                $type = 1;
                $table = "certifi_person";
    ...
    public function certifiPersonModify()
    {
        if ($this->request->isPost()) {
            ...
            if ($cardtype == 0) {
                ...
                if (
                    !preg_match("/^((\\s?[A-Za-z])|([A-Za-z]{2}))\\d{6}(\\([0−9aA]\\)|[0-9aA])\$/", $idcard) &&
                    !preg_match("/^[a-zA-Z][0-9]{9}\$/", $idcard) &&
                    !preg_match("/^[1|5|7][0-9]{6}\\([0-9Aa]\\)/", $idcard)
                ) {
            ...
            if ($res) {
                $description =
                    ...
                    $params["client_id"] .
                    " - 修改前身份证:" .
                    $certifi["auth_card_number"] .
                    " - 真实姓名:" .
                    $certifi["auth_real_name"] .
                    " - 卡类型:" .
                    $certifi["auth_card_type"] .
                    " -- 修改后身份证:" .
                    $data["idcard"] .
                    " - 真实姓名:" .
                    $data["real_name"] .
    ...
    public function certifiCompanyDetail()
    {
        ...
        $company = \think\Db::name("certifi_company")
            ->field(
                "auth_user_id,auth_real_name,auth_card_type,auth_card_number,
            company_name,company_organ_code,
            status,img_one,img_two,img_three,auth_fail,certify_id,create_time,update_time",
            )
            ->where("auth_user_id", intval($params["client_id"]))
            ->find();
    ...
    public function certifiStatus()
    {
        ...
        if ($params["status"] == 1) {
            ...
            if ($certifimodel->checkOtherUsed($certifi["auth_card_number"], $uid)) {
                $description =
                    ...
                    $params["status"] .
                    " - 错误:" .
                    lang("CERTIFI_CARD_HAS_USED_BY_OTHER");
        ...
        try {
            \think\Db::name($table)
                ->where("id", $certifi["id"])
                ->update(["status" => $params["status"], "auth_fail" => $error]);
            $tmp = \think\Db::name("certifi_log")
                ->where(["uid" => $uid, "type" => $params["type"]])
                ->order("id", "DESC")
                ->find();
            \think\Db::name("certifi_log")
                ->where("id", $tmp["id"])
                ->update(["status" => $params["status"], "error" => $error]);
            if ($params["status"] == 1) {
            ...
            if ($params["status"] == 1 && configuration("certifi_realname") == 1) {
                \think\Db::name("clients")
                    ->where("id", $tmp["uid"])
                    ->update(["username" => $tmp["certifi_name"]]);
            }
            \think\Db::commit();
        } catch (\Exception $e) {
            \think\Db::rollback();
    ...
    public function userInvoice()
    {
        ...
        $count = \think\Db::name("invoices")
            ->alias("i")
            ->leftjoin("clients c", "c.id=i.uid")
            ->leftJoin("currencies cu", "cu.id = c.currency")
            ->leftJoin("invoice_items t", "t.invoice_id = i.id")
            ->field(
                "i.id,i.create_time,i.due_time,i.paid_time,i.subtotal,i.payment,i.status,cu.prefix,cu.suffix,i.type",
            )
            ->where($where)
            ->group("i.id")
            ->count();
        ...
        $invocies = \think\Db::name("invoices")
            ->alias("i")
            ->leftjoin("clients c", "c.id=i.uid")
            ->leftJoin("currencies cu", "cu.id = c.currency")
            ->leftJoin("invoice_items t", "t.invoice_id = i.id")
            ->field(
                "i.id,i.create_time,i.due_time,i.paid_time,i.subtotal,i.subtotal as sub,i.total,i.credit,i.payment,i.status,cu.prefix,cu.suffix,i.type,i.use_credit_limit",
            )
            ->where($where)
    ...
    public function userProductaccounts()
    {
        ...
        $sale = array_column(get_sale(), "user_nickname", "id");
        $rows = \think\Db::name("accounts")
            ->alias("a")
            ->leftJoin("orders o", "o.invoiceid=a.invoice_id")
            ->leftJoin("host h", "h.orderid=o.id")
            ->leftjoin("clients c", "c.id=a.uid")
            ->leftjoin("currencies cu", "cu.code = a.currency")
            ->where($fun)
            ->whereBetweenTime("a.pay_time", $start_time, $end_time)
            ->field(
                "a.id,c.id as uid,c.sale_id,c.username,a.currency,cu.prefix,cu.suffix,a.pay_time,a.update_time,a.gateway,a.description,a.amount_in,a.fees,a.amount_out,a.trans_id",
            )
            ->withAttr("amount_in", function ($value, $data) {
                return $data["prefix"] . $value . $data["suffix"];
            })
        ...
        foreach ($currencys as $item) {
            ...
            $total[$item["code"]] = [
                "amount_in" => $item["prefix"] . bcsub($amount_in, 0, 2) . $item["suffix"],
                "amount_out" => $item["prefix"] . bcsub($amount_out, 0, 2) . $item["suffix"],
                "fees" => $item["prefix"] . bcsub($fees, 0, 2) . $item["suffix"],
                "surplus" => $item["prefix"] . bcsub($surplus, 0, 2) . $item["suffix"],
            ];
        }
        $pages = ["total" => $count];
        return jsonrule([
    ...
    public function userProductInvoice()
    {
        ...
        $count1 = \think\Db::name("invoices")
            ...
            ->where($where)
            ->count();
        $count2 = \think\Db::name("invoices")
            ->alias("i")
            ->leftJoin("invoice_items in", "in.invoice_id=i.id")
            ->leftJoin("upgrades up", "up.id=in.rel_id")
            ->leftJoin("host h", "h.id=up.relid")
            ->leftjoin("clients c", "c.id=i.uid")
            ->leftJoin("currencies cu", "cu.id = c.currency")
            ->leftJoin("invoice_items t", "t.invoice_id = i.id")
            ->field(
        ...
        $gateways = gateway_list();
        $invocies1 = \think\Db::name("invoices")
            ->alias("i")
            ->leftJoin("invoice_items in", "in.invoice_id=i.id")
            ->leftJoin("host h", "h.id=in.rel_id")
            ->leftjoin("clients c", "c.id=i.uid")
            ...
            ->leftJoin("invoice_items t", "t.invoice_id = i.id")
            ->field(
                "i.status,i.id,i.create_time,i.due_time,i.paid_time,i.subtotal,i.subtotal as sub,i.total,i.credit,i.payment,i.status,cu.prefix,cu.suffix,i.type,i.use_credit_limit",
            )
            ->where($where)
            ->group("i.id")
            ->withAttr("payment", function ($value, $data) use ($gateways) {
            ...
            ->select()
            ->toArray();
        $invocies2 = \think\Db::name("invoices")
            ->alias("i")
            ->leftJoin("invoice_items in", "in.invoice_id=i.id")
            ->leftJoin("upgrades up", "up.id=in.rel_id")
            ->leftJoin("host h", "h.id=up.relid")
    ...
    public function get_search($param)
    {
        ...
        if (!empty($param["lineitem_desc"])) {
            $invoice_id = \think\Db::name("invoice_items")
                ->whereLike("description", "%" . $param["lineitem_desc"] . "%")
                ->column("invoice_id");
            if (empty($invoice_id)) {
                $invoice_id = [0];
            }
    ...
    public function addUserInvoice()
    {
        ...
        $invoice_data = [
            "uid" => $uid,
            "status" => "Unpaid",
            "payment" => $gateway ?? gateway_list()[0]["name"],
            "create_time" => time(),
            "due_time" => strtotime("+1 month"),
        ];
        $r = \think\Db::name("invoices")->insertGetId($invoice_data);
        if ($r) {
            active_log(
                sprintf($this->lang["UserManage_user_addUserInvoice_success"], $params["uid"], $r),
                $params["uid"],
            );
            return jsonrule(["status" => 200, "msg" => lang("ADD SUCCESS"), "invoice_id" => $r]);
        } else {
            active_log(
                sprintf($this->lang["UserManage_user_addUserInvoice_fail"], $params["uid"], lang("ADD FAIL")),
                $params["uid"],
            );
    ...
    public function addRechargeInvoice()
    {
        ...
        $data = [
            ...
            "total" => $amount,
            "status" => "Unpaid",
            "payment" => $clients["defaultgateway"],
            "type" => "recharge",
    ...
    public function requestCancelList()
    {
        ...
        $order = !empty($params["order"]) ? trim($params["order"]) : "id";
        $sort = !empty($params["sort"]) ? trim($params["sort"]) : "DESC";
        $count = \think\Db::name("cancel_requests")
            ->alias("a")
            ->field(
                "a.id,b.uid,b.id as hid,c.username,c.companyname,a.create_time,d.name,b.domain,b.dedicatedip,a.type,a.reason,b.domainstatus,a.delete_time as nextduedate",
            )
            ->leftJoin("host b", "a.relid = b.id")
            ->leftJoin("clients c", "b.uid = c.id")
            ->join("products d", "b.productid = d.id");
        if ($this->user["id"] != 1 && $this->user["is_sale"]) {
        ...
        $list = \think\Db::name("cancel_requests")
            ->alias("a")
            ->field(
                "a.id,b.uid,b.id as hid,c.username,c.companyname,a.create_time,d.name,b.domain,b.dedicatedip,a.type,a.reason,b.domainstatus,a.delete_time as nextduedate,cg.group_colour,cc.status as company_status,cp.status as person_status,a.status as cancel_status",
            )
            ->leftJoin("host b", "a.relid = b.id")
            ->leftJoin("clients c", "b.uid = c.id")
            ->leftJoin("client_groups cg", "cg.id = c.groupid")
            ->leftJoin("certifi_company cc", "cc.auth_user_id = c.id")
            ->leftJoin("certifi_person cp", "cp.auth_user_id = c.id")
            ->join("products d", "b.productid = d.id")
            ->withAttr("type", function ($value) use ($type) {
                return $type[$value] ?? "";
            })
        ...
        foreach ($list as &$v) {
            $hosts = \think\Db::name("host")
                ...
                ->leftJoin("upgrades u", "u.relid = a.id")
                ->leftjoin("clients c", "a.uid = c.id")
                ->leftjoin("currencies cu", "cu.id = c.currency")
                ->field(
                    "a.uid,a.initiative_renew,a.id as hostid,b.id as invoice_type,b.name,a.domain,a.dedicatedip,a.billingcycle,a.firstpaymentamount,a.productid,cu.prefix,cu.suffix",
                )
                ->distinct(true)
                ->where("a.id", $v["hid"])
    ...
    public function requestCancelReasonPost()
    {
        ...
        foreach ($data as $key => $val) {
            if ($val["id"] == 0 || $val["id"] == "") {
                $id = \think\Db::name("cancel_reason")->insertGetId([
                    "reason" => htmlspecialchars_decode($val["reason"]),
                ]);
    ...
    public function deleteCancelRequest()
    {
        $param = $this->request->param();
        $id = $param["id"];
        $cancel = \think\Db::name("cancel_requests")
            ->alias("a")
            ->field("b.domainstatus")
            ->leftJoin("host b", "a.relid = b.id")
            ->where("a.id", $id)
            ->find();
    ...
    public function addRecordLog()
    {
        ...
        $id = \think\Db::name("clients_track_record")->insertGetId([
            "uid" => $uid,
            "des" => $record,
    ...
    public function addRemarkLog()
    {
        ...
        \think\Db::name("clients_track_remark")->insert([
            "track_id" => $id,
    ...
    public function getTrackRecord()
    {
        ...
        $list = \think\Db::name("clients_track_record")
        ...
        foreach ($list as &$v) {
            $v["remark"] =
                \think\Db::name("clients_track_remark")
                    ->field("id,remark,create_time")
                    ->where("track_id", $v["id"])
                    ->select()
                    ->toArray() ?:
                [];
    ...
    public function clientTrackStatus()
    {
        ...
        \think\Db::name("clients")
            ->where("id", $uid)
            ->update(["track_status" => $track_status]);
        return jsonrule(["status" => 200, "msg" => lang("UPDATE SUCCESS")]);
    }
    /**
     * @title  获取备注
    ...
    public function postClientNotes()
    {
        ...
        \think\Db::name("clients")
            ->where("id", $uid)
            ->update(["notes" => $notes, "update_time" => time()]);
        return jsonrule(["status" => 200, "msg" => lang("SUCCESS MESSAGE")]);
    }
    /**
     * @title  后台实名认证
    ...
    public function authorInfo()
    {
        try {
            ...
            if ($log) {
                ...
                if ($log["custom_fields_log"]) {
                    ...
                    foreach ($customfields as $customfield) {
                        $i++;
                        $i <= 10 &&
                            ($log["custom_fields_log_arr"][$customfield["field"]] = [
                                "type" => $customfield["type"],
                                "title" => $customfield["title"],
                                "val" => $log["custom_fields_log"]["custom_fields" . $i] ?? "",
                            ]);
                    }
                }
            }
            ...
            return jsons([
                ...
                "data" => [
                    "log" => $log,
                    "company" => $this->getPluginsInfo("enterprises"),
                    "person" => $this->getPluginsInfo("personal"),
                    "config" => $config,
                ],
            ]);
        } catch (\Throwable $e) {
    ...
    protected function passSendSms($param)
    {
        ...
        if ($client) {
            $sms->sendSms(
                $message_template_type[$sms_template],
    ...
    {
        $data = [
            "uid" => $arr["auth_user_id"],
            "certifi_name" => $arr["auth_real_name"],
            "card_type" => $arr["auth_card_type"],
            "idcard" => $arr["auth_card_number"],
            "company_name" => $arr["company_name"] ?? "",
            "bank" => $arr["bank"] ?? "",
            "phone" => $arr["phone"] ?? "",
            "company_organ_code" => $arr["company_organ_code"] ?? "",
            "pic" => $arr["pic"],
            "status" => $status,
            "type" => $type,
            "certifi_type" => $certifi_type,
            "custom_fields_log" => json_encode($custom_fields_log),
        ];
    ...
    protected function getPluginsInfo($type = "personal")
    {
        ...
        if (empty($all_plugins[0])) {
            ...
            foreach ($all_plugins as $all_plugin) {
                $arr[] = [
                    "name" => $all_plugin["title"],
                    "value" => $all_plugin["name"],
                    "custom_fields" => $all_plugin["custom_fields"] ?: [],
                ];
            }
        }
        return $arr ?? [];
    ...
    protected function getConfInfo()
    {
        $conf = configuration([
            "certifi_open",
            "artificial_auto_send_msg",
            "certifi_is_upload",
            "certifi_business_btn",
            "certifi_business_open",
            "certifi_business_is_upload",
            "certifi_business_is_author",
        ...
        $data = [
            "artificial_auto_send_msg" => $conf["artificial_auto_send_msg"],
            "certifi_isbindphone" => $conf["certifi_isbindphone"],
            "certifi_is_upload" => $conf["certifi_open"] ? $conf["certifi_is_upload"] : 0,
            "certifi_business_is_author" => $conf["certifi_business_open"] ? $conf["certifi_business_is_author"] : 0,
            "certifi_business_author_path" => $conf["certifi_business_author_path"]
                ? config("author_attachments_url") . $conf["certifi_business_author_path"]
                : "",
        ];
        $data["certifi_business_is_upload"] = $conf["certifi_business_open"]
            ? $conf["certifi_business_is_upload"]
            : $data["certifi_is_upload"];
        return $data;
    }
}
