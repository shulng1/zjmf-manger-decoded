...
class ReportController extends AdminBaseController
{
    ...
    public function baseInfo()
    {
        ...
        if (
            configuration("cron_last_run_time_over") == configuration("cron_last_run_time") &&
            configuration("cron_last_run_time_over_in") != configuration("cron_last_run_time")
        ) {
    ...
    public function getSystemInfoModulesList()
    {
        ...
        $lists = \think\Db::name("base_info")
            ->field("id,name,desc,enable,sort")
            ->where("delete_time", 0)
            ->order("sort", "asc")
            ->order("id", "asc")
            ->select()
        ...
        if ($admin_id != 1 && $user["role_id"] != 1) {
            $accesses = \think\Db::name("auth_access")
                ->alias("a")
                ->field("a.rule_name")
                ->leftJoin("role_user b", "a.role_id = b.role_id")
                ->where("b.user_id", $admin_id)
                ->select()
                ->toArray();
    ...
    private function get_waiting_ticket()
    {
        ...
        $ticket = \think\Db::name("ticket")
            ->alias("t")
            ->field("t.id, t.tid,t.uid,t.title,c.username,t.create_time")
            ->leftJoin("clients c", "c.id = t.uid")
            ->where("t.status", "in", "1,3")
            ->where("t.merged_ticket_id", 0)
            ->where("dptid", "in", $dptids)
            ->limit(5)
            ->order("id desc")
            ->select()
            ->toArray();
    ...
    {
        $sale_list = \think\Db::name("user")
            ->field("id,user_nickname as sale_username")
            ->where("is_sale", 1)
            ->select()
            ->toArray();
        $rank = [];
    ...
    private function get_online_admin()
    {
        $user = \think\Db::name("user")
            ->field("user_login as  admin_name,avatar,last_act_time")
            ->where("user_status", 1)
            ->where("last_act_time", ">=", time() - 3600)
            ->order("last_act_time", "desc")
            ->select()
            ->toArray();
        if ($user) {
    ...
    {
        $log_list = \think\Db::name("activity_log")
            ->field("create_time,id,ipaddr,description as new_desc,uid,user")
                    ...
                    foreach ($name as $k => $v) {
                        ...
                        if ($v == "Invoice ID") {
                            $url =
                                ...
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                ...
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: \">" .
                                $str .
                                "</span></a>";
                            $value = str_replace($str, $url, $value);
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/add-news?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/new-work-order-dept?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;\">" .
                                $str .
                                "</span></a>";
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" target=\"blank\" href=\"#/edit-configurable-option1?groupId=" .
                                $pco["gid"] .
                                "&optionId=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;\">" .
                            ...
                            if ($server["server_type"] == "dcimcloud") {
                                $url =
                                    "<a class=\"el-link el-link--primary is-underline\" href=\"#/zjmfcloud\"><span class=\"el-link--inner\" style=\"display: block;\">" .
                                    $str .
                                    "</span></a>";
                                $value = str_replace($str, $url, $value);
                            } elseif ($server["server_type"] == "dcim") {
                                $url =
                                    "<a class=\"el-link el-link--primary is-underline\" href=\"#/dcim\"><span class=\"el-link--inner\" style=\"display: block;\">" .
                                    $str .
                                    "</span></a>";
                                $value = str_replace($str, $url, $value);
                            } else {
                            ...
                            $url =
                                "<a class=\"el-link el-link--primary is-underline\" href=\"#/business-statement?id=" .
                                $relid .
                                "\"><span class=\"el-link--inner\" style=\"display: block;\">" .
                                $str .
                                "</span></a>";
    ...
    private function get_sales_ranking()
    {
        $data = \think\Db::name("orders")
            ->alias("o")
            ->join("host h", "h.orderid = o.id")
            ->join("products p", "h.productid = p.id")
            ->field("p.id,p.name,sum(o.amount) as total_amount")
    ...
    private function get_expiring()
    {
        ...
        $expiring = [];
        $expiring = \think\Db::name("host")
            ->alias("h")
            ->field("count(*) as expiring_count,p.type")
            ->join("products p", "p.id = h.productid")
            ->where("h.nextduedate", "between", [time(), strtotime("+7 days")])
    ...
    private function get_client()
    {
        ...
        $today_add_client_count = \think\Db::name("clients")
            ->whereBetweenTime("create_time", $today_start, $today_end)
            ->count();
        $now_online_count = \think\Db::name("clients")
            ->where("lastlogin", ">=", time() - 900)
            ->count();
        $total_client_count = \think\Db::name("clients")->count();
        $latest_online_clinet = \think\Db::name("clients")
            ->field("id,username,lastloginip,lastlogin")
            ->where("lastlogin", ">=", time() - 10800)
            ->limit(20)
    ...
    private function get_trade($start_time = "", $end_time = "")
    {
        ...
        $pre_list = \think\Db::name("orders")
            ->alias("a")
            ->field("from_unixtime(a.create_time, \"%Y-%m-%d\") date,count(a.id) count,sum(amount) amount_total")
            ->leftJoin("invoices b", "a.invoiceid=b.id")
            ->whereBetweenTime("a.create_time", $pre_start_time, $pre_end_time)
            ->where("b.status", "Paid")
            ->where("a.status", "<>", "Cancelled")
            ->where("b.delete_time", 0)
            ->where("a.delete_time", 0)
        ...
        if ($now_list) {
            foreach ($now_list as $item) {
                ...
                $data["tomonth_order_amount_count"] = bcadd(
                    $item["amount_total"],
                    $data["tomonth_order_amount_count"],
                    2,
                );
                $data["tomonth"][] = $item;
            }
        ...
        if ($pre_list) {
            foreach ($pre_list as $item) {
                ...
                $data["pre_month_order_amount_count"] = bcadd(
                    $item["amount_total"],
                    $data["pre_month_order_amount_count"],
                    2,
                );
                $data["pre_month"][] = $item;
            }
    ...
    private function get_trade_latest_7_days($start_time = "", $end_time = "")
    {
        ...
        $pre_month_group_day = \think\Db::name("orders")
            ->alias("a")
            ->field("from_unixtime(a.create_time, \"%Y-%m-%d\") date,count(a.id) count,sum(amount) amount_total")
            ->leftJoin("invoices b", "a.invoiceid = b.id")
            ->where("a.create_time", "between", [$pre_start_time, $pre_end_time])
            ->where("b.status", "Paid")
            ->where("b.delete_time", 0)
            ->where("a.status", "<>", "Cancelled")
            ->where("a.delete_time", 0)
    ...
    private function get_todo()
    {
        ...
        $waiting_check_order_count = \think\Db::name("orders")
            ->where("status", "Pending")
            ->where("delete_time", 0)
            ->count();
        $verified_count = \think\Db::name("certifi_log")
            ->whereIn("status", [3])
            ->count();
        $withdraw_count = \think\Db::name("affiliates_withdraw")->where("status", 1)->count();
        return [
            "waiting_ticket_count" => $waiting_ticket_count,
            "waiting_check_order_count" => $waiting_check_order_count,
    ...
    private function get_order()
    {
        ...
        $list = \think\Db::name("orders")
            ->alias("o")
            ->leftJoin("invoices nv", "nv.id = o.invoiceid")
            ->field("o.*,nv.status as nv_status,nv.id as in_id,nv.delete_time as in_delete_time")
            ->where("o.create_time", ">=", $month_start_time)
        ...
        if ($list) {
            ...
            foreach ($list as $item) {
                if (
                    $today_start <= $item["create_time"] &&
                    $item["nv_status"] == "Paid" &&
                    $item["in_delete_time"] == 0
                ) {
