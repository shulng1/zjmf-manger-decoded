...
class SaleController extends GetUserController
{
    ...
    public function groupList()
    {
        ...
        $total = \think\Db::name("sales_product_groups")
            ...
            ->count();
        $list = \think\Db::name("sales_product_groups")
            ->where(function (\think\db\Query $query) use ($data) {
                if (!empty($data["group_name"])) {
                    $query->where("group_name", "like", "%" . trim($data["group_name"]) . "%");
                }
    ...
    public function saleStatistics()
    {
        ...
        if (!empty($params["time"])) {
            ...
            return json([
                ...
                "today" => $this->getLaddersaleStatistics("today", $id),
                "week" => $this->getLaddersaleStatistics("week", $id),
                "month" => $month,
            ...
            return json([
                ...
                "today" => $this->getLaddersaleStatistics("today", $id),
                "week" => $this->getLaddersaleStatistics("week", $id),
                "month" => $this->getLaddersaleStatistics("month", $id),
                "last_month" => $this->getLaddersaleStatistics("last_month", $id),
            ]);
        }
    }
    ...
    public function saleRecords()
    {
        ...
        try {
            $count = \think\Db::name("invoice_items")
                ->alias("in")
                ->join("host h", "h.id=in.rel_id")
                ->join("products p", "p.id=h.productid")
                ->leftJoin("sale_products sp", "p.id=sp.pid")
                ->leftJoin("sales_product_groups spg", "spg.id=sp.gid")
                ->join("invoices i", "i.id=in.invoice_id")
                ->join("clients c", "i.uid=c.id")
                ->join("currencies cu", "cu.id = c.currency")
                ->field("i.uid,in.invoice_id")
                ->where("i.status", "=", "Paid")
                ->where("c.sale_id", $id)
                ->where("in.type", "neq", "upgrade")
                ->where($where)
                ->select()
                ->toArray();
            ...
            $count1 = \think\Db::name("invoice_items")
                ->alias("in")
                ->join("upgrades u", "u.id=in.rel_id")
                ->join("host h", "h.id=u.relid")
                ->join("products p", "p.id=h.productid")
                ->leftJoin("sale_products sp", "p.id=sp.pid")
                ->leftJoin("sales_product_groups spg", "spg.id=sp.gid")
                ->join("invoices i", "i.id=in.invoice_id")
                ...
                ->where("i.status", "=", "Paid")
                ->where("c.sale_id", $id)
                ->where("in.type='upgrade' OR in.type='discount'")
                ->where($where)
                ->select()
                ->toArray();
        ...
        $list = \think\Db::name("invoice_items")
            ->alias("in")
            ->join("host h", "h.id=in.rel_id")
            ->join("products p", "p.id=h.productid")
            ->leftJoin("sale_products sp", "p.id=sp.pid")
            ...
            ->join("currencies cu", "cu.id = c.currency")
            ->field(
                "i.uid,h.id as hostid,in.id,in.invoice_id,in.amount,spg.bates,p.name,c.username,c.companyname,in.type,i.type as typess,spg.is_renew,spg.updategrade,spg.renew_bates,spg.upgrade_bates,cu.suffix,h.domain,h.dedicatedip",
            )
            ->where("i.status", "=", "Paid")
            ->where("c.sale_id", $id)
            ->where("in.type", "neq", "upgrade")
        ...
        $list1 = \think\Db::name("invoice_items")
            ->alias("in")
            ->join("upgrades u", "u.id=in.rel_id")
            ->join("host h", "h.id=u.relid")
            ->join("products p", "p.id=h.productid")
        ...
        foreach ($list as $key => $val) {
            ...
            href=\"#/customer-view/abstract?id=" .
                $val["uid"] .
                "\">
            ...
            if (!empty($ladder["turnover"]["turnover"])) {
                if (
                    ($val["is_renew"] == 0 && $val["type"] == "renew") ||
                    ($val["is_renew"] == 0 && $val["typess"] == "renew" && $val["type"] == "discount")
                ) {
                    $list[$key]["bates"] = 0;
                    ...
                    ($val["updategrade"] == 0 && $val["type"] == "upgrade") ||
                    ($val["updategrade"] == 0 && $val["typess"] == "upgrade" && $val["type"] == "discount")
                ) {
                    $list[$key]["bates"] = 0;
                    $list[$key]["batesamount"] = "0.00+" . bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 2);
                    if (bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 2) == 0) {
                    ...
                    ($val["is_renew"] == 1 && $val["type"] == "renew") ||
                    ($val["is_renew"] == 1 && $val["typess"] == "renew" && $val["type"] == "discount")
                ) {
                    $list[$key]["batesamount"] =
                        round(bcmul($val["amount"], $val["renew_bates"] / 100, 2), 2) .
                        "+" .
                        round(bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 2), 2);
                    if (
                        round(bcmul($val["amount"], $val["renew_bates"] / 100, 2), 2) == 0 &&
                        round(bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 2), 2) == 0
                    ) {
                        $list[$key]["batesamount"] = "0.00";
                    }
                } elseif (
                    ($val["updategrade"] == 1 && $val["type"] == "upgrade") ||
                    ($val["updategrade"] == 1 && $val["typess"] == "upgrade" && $val["type"] == "discount")
                ) {
                    $list[$key]["batesamount"] =
                        round(bcmul($val["amount"], $val["upgrade_bates"] / 100, 2), 2) .
                        "+" .
                        round(bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 2), 2);
                    if (
                        round(bcmul($val["amount"], $val["upgrade_bates"] / 100, 2), 2) == 0 &&
                        round(bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 2), 2) == 0
                    ) {
                        $list[$key]["batesamount"] = "0.00";
                    }
                    ...
                    $list[$key]["batesamount"] =
                        round(bcmul($val["amount"], $val["bates"] / 100, 2), 2) .
                        "+" .
                        round(bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 2), 2);
                    if (
                        round(bcmul($val["amount"], $val["bates"] / 100, 2), 2) == 0 &&
                        round(bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 2), 2) == 0
                    ) {
                        $list[$key]["batesamount"] = "0.00";
                    }
        ...
        foreach ($arr1 as $key => $val) {
            ...
            $accounts = \think\Db::name("accounts")
                ->field("id,amount_out")
                ->where("invoice_id", $val["invoice_id"])
                ->where("refund", ">", 0)
                ->select()
                ->toArray();
            if (!empty($accounts)) {
                foreach ($accounts as $val2) {
                    ...
                    if (!empty($ladder["turnover"]["turnover"])) {
                        $refund1 = bcadd(
                            $refund1,
                            bcmul($ladder["turnover"]["bates"] / 100, $val2["amount_out"], 4),
                            4,
                        );
                    }
                }
                if (bcadd($refund, $refund1, 4) > 0) {
                    ...
                    if (bcsub(bcadd($refund, $refund1, 4), bcadd($count[1], $count[0], 2), 2) > 0) {
                        $arr1[$key]["refound"] =
                            "-" . round($refunds, 2) . $val["suffix"] . ",提成-" . round($count[0], 2);
                        $arr1[$key]["batesamount"] = "0.00+0.00";
                    } else {
                        $arr1[$key]["refound"] =
                            "-" . round($refunds, 2) . $val["suffix"] . ",提成-" . round($refund, 2);
                        if (empty($count[1])) {
                            $arr1[$key]["batesamount"] = round(bcsub($count[0], $refund, 2), 2);
                        } else {
                            $arr1[$key]["batesamount"] =
                                round(bcsub($count[0], $refund, 2), 2) . "+" . round(bcsub($count[1], $refund1, 2), 2);
                        }
                    }
                }
            }
        ...
        $arrss = [
            ["label" => "续费", "name" => "renew"],
            ["label" => "新订购", "name" => "host"],
            ["label" => "升级", "name" => "upgrade"],
            ["label" => "流量包订购", "name" => "zjmf_flow_packet"],
            ["label" => "重装次数", "name" => "zjmf_reinstall_times"],
        ];
        return jsonrule([
            "status" => 200,
            "msg" => lang("SUCCESS MESSAGE"),
    ...
    public function saleRecordsNew()
    {
        ...
        return jsonrule([
            ...
            "record" => $data,
            "total" => $record_count,
            "this_month_commission_total" => $this_month_commission_total,
            "this_month_sale_total" => $this_month_sale_total,
            "type" => $this->getCommisionInvoice(),
            "last" => $this->getLastLadder($this_month_sale_total),
            "now_ladder" => $this->getLastLadder($this_month_sale_total, "now"),
        ]);
    }
    /**
     * 获取阶梯提成信息
    ...
    private function getLastLadder($this_month_sale_total = 0, $now_or_last = "last")
    {
        $ladder["turnover"] = 0;
        $ladder["bates"] = 0;
        $sale_ladder = \think\Db::name("sale_ladder")
            ->order("turnover", $now_or_last == "last" ? "asc" : "desc")
            ->select()
            ->toArray();
        if ($sale_ladder) {
    ...
    {
        $info = \think\Db::name("accounts")
            ->alias("a")
            ->leftJoin("currencies cu", "cu.code = a.currency")
            ->where("a.invoice_id", $item["invoice_id"])
            ->field("suffix,prefix")
            ->find();
        return $info;
    }
    ...
    private function getProductInoviceDetail($item, $this_month_sale_total = 0)
    {
        ...
        if ($item["type"] != "upgrade") {
            $invoice_item_list = \think\Db::name("invoice_items")
                ->alias("im")
                ->join("host h", "h.id = im.rel_id")
                ->join("products p", "p.id = h.productid")
                ->leftJoin("accounts ac", "ac.invoice_id = im.invoice_id")
                ->leftJoin("currencies cu", "ac.currency = cu.code")
                ->field("h.id as host_id,h.domain")
                ->field("p.id as productid,p.name as product_name")
                ->field("im.invoice_id,im.type,im.amount")
                ->field("cu.prefix,cu.suffix")
                ->group("im.id")
                ->where("im.invoice_id", $item["invoice_id"])
                ->where("im.delete_time", 0)
                ->select()
                ->toArray();
        } else {
            if ($item["type"] == "upgrade") {
                $invoice_item_list = \think\Db::name("invoice_items")
                    ->alias("im")
                    ->join("upgrades ug", "im.rel_id = ug.id")
                    ->join("host h", "h.id = ug.relid")
                    ->join("products p", "p.id = h.productid")
                    ->leftJoin("accounts ac", "ac.invoice_id = im.invoice_id")
                    ->leftJoin("currencies cu", "ac.currency = cu.code")
                    ->field("h.id as host_id,h.domain")
        ...
        if ($invoice_item_list) {
            foreach ($invoice_item_list as $key => $child_item) {
                ...
                $child_invoice["commision_amount"] = $this->calculateCommisionAmount(
                    $child_item,
        ...
        $refund_info = \think\Db::name("accounts")
        ...
        if ($refund_info) {
            ...
            $item["refund"] =
                "-" . $item["refund_sum"] . $last_suffix . "，" . "提成-" . $refund_commission . $last_suffix;
     ...
     */
    private function searchSaleRecordInfo(
        $search_type = "get_list",
        $start_time = "",
        $end_time = "",
        $params = [],
        $statistics = false
        ...
        $allow_invoice_types = array_column($allow_invoice, "name");
        $search_obj = \think\Db::name("invoices")
            ->alias("i")
            ->join("invoice_items im", "im.invoice_id = i.id")
            ->join("host h", "h.id = im.rel_id")
            ->join("products p", "p.id = h.productid")
            ->join("clients c", "c.id = i.uid")
            ->field("i.id as invoice_id,i.subtotal as total,i.type,i.paid_time")
            ->field("p.id as productid,p.name as product_name")
            ->field("h.id as host_id,h.domain")
            ->field("c.id as client_id,c.username  as  client_username,c.companyname")
            ->group("invoice_id")
            ->where($where)
            ->where("i.status", "Paid")
            ->where("i.delete_time", 0)
            ->where("c.sale_id", $id)
            ->where("im.type", "in", $allow_invoice_types);
        if ($name) {
        ...
        if ($search_type === "get_list") {
            ...
            if ($page == 1) {
                $invoice_upgrade_lists = \think\Db::name("invoices")
                    ->alias("i")
                    ->join("invoice_items im", "im.invoice_id = i.id")
                    ->join("upgrades ug", "ug.id = im.rel_id")
                    ->join("host h", "ug.relid = h.id")
    ...
    public function adminList()
    {
        ...
        $count = \think\Db::name("user")
            ->where("user_nickname LIKE '%{$search}%' OR user_login LIKE '%{$search}%'")
            ->count();
        $data = \think\Db::name("user")
            ->alias("a")
            ->leftJoin("role_user b", "b.user_id = a.id")
            ->leftJoin("role c", "c.id = b.role_id")
            ->field("a.cat_ownerless")
            ->field(
                "a.id,a.user_login,a.user_nickname,a.user_email,a.create_time,a.user_status,a.last_login_time,a.last_login_ip,c.name as role,a.is_sale,a.sale_is_use,a.only_mine,a.all_sale,a.only_oneself_notice",
            )
            ->where("user_nickname LIKE '%{$search}%' OR user_login LIKE '%{$search}%'")
            ->page($page)
            ->limit($limit)
    ...
    public function editAdminList()
    {
        if ($this->request->isPost()) {
            ...
            $data["only_oneself_notice"] = isset($param["only_oneself_notice"])
                ? intval($param["only_oneself_notice"])
                : 0;
