...
class HostController extends GetUserController
{
    ...
    public function getList(\think\Request $request)
    {
        ...
        $total_arr = \think\Db::name("host")
            ->alias("h")
            ->field("h.firstpaymentamount,h.amount,h.billingcycle")
        ...
        $host_list = \think\Db::name("host")
            ->field("h.firstpaymentamount")
            ->field(
                "cr.type as crtype,cr.reason,h.id,h.initiative_renew,h.domain,h.uid,h.dedicatedip,h.productid,h.billingcycle,h.domain,h.payment,h.nextduedate,h.assignedips,h.regdate,h.dedicatedip,h.amount,h.domainstatus,c.username,p.name as productname,c.currency,p.type,h.serverid,u.user_nickname",
            )
            ->alias("h")
            ->leftJoin("products p", "p.id=h.productid")
            ->leftJoin("clients c", "c.id=h.uid")
        ...
        foreach ($host_list as &$v) {
            ...
            $v["domainstatus"] = $v["domainstatus"]
                ? config("public.domainstatus")[$v["domainstatus"]]
                : ["name" => "未知状态", "color" => "#FF5722"];
    ...
    {
        $info = \think\Db::name("clients")
            ->field("usertype,username,companyname,email,qq,lastloginip as ip,address1,phone_code,phonenumber,notes")
            ->where("id", $req->uid)
            ->find();
        if (empty($info)) {
