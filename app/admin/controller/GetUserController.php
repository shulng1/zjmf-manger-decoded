...
class GetUserController extends AdminBaseController
{
    ...
    public function initialize()
    {
        ...
        $user = \think\Db::name("user")
            ->field("id,is_sale,sale_is_use,only_mine,cat_ownerless")
    ...
    public function getAdminSale($uid)
    {
        ...
        $sales_client_ids = \think\Db::name("user")
            ->alias("u")
            ->leftJoin("clients c", "c.sale_id = u.id")
            ->field("u.id as uid,u.user_login,u.is_sale,u.sale_is_use,u.only_mine")
            ->where("u.id", $sessionAdminId)
            ->where("u.is_sale", 1)
            ->where("c.sale_id", $sessionAdminId)
            ->column("c.id");
    ...
    public function check($uid)
    {
        $sessionAdminId = session("ADMIN_ID");
        $user = \think\Db::name("user")
            ->field("is_sale,sale_is_use,only_mine,cat_ownerless")
            ->where("id", $sessionAdminId)
            ->find();
    ...
    public function getLadder($uid)
    {
        ...
        if (!empty($str)) {
            $rows = \think\Db::name("invoices")
                ...
                ->field("i.id as invoiceid,cu.suffix")
                ->where("i.status", "=", "Paid")
                ->whereTime("i.paid_time", "month")
                ->where("c.sale_id", $uid)
                ->select()
                ->toArray();
            $sum = \think\Db::name("invoices")
                ->alias("i")
            ...
            if (is_array($rows) && count($rows) > 0) {
                ...
                $accounts_sum_amount_out = \think\Db::name("accounts")
                    ->field("id,amount_in,amount_out,refund")
                    ->where("invoice_id", "IN", $invoiceid_str)
                    ->where("refund", ">", 0)
                    ->sum("amount_out");
            ...
            $sale_ladder = \think\Db::name("sale_ladder")
                ...
                ->order("turnover", DESC)
                ->find();
            $sale_ladder1 = \think\Db::name("sale_ladder")
                ->field("id,turnover,bates")
                ->where("turnover", ">", $sum)
                ->order("turnover", ASC)
                ->find();
    ...
    public function getLadderforall($uid)
    {
        ...
        $data = [
            "total" => floatval($sum),
            "totals" => round(floatval($this->getSum($uid, $sale_ladder)), 2),
            "turnover" => $sale_ladder,
            "last" => $sale_ladder1,
            "suffix" => $rows[0]["suffix"] ?? $default_currency["suffix"],
        ];
        return $data;
    }
    /**
    ...
    public function getLaddersaleStatistics($time, $uid, $start_time = 1)
    {
        ...
        $rows = \think\Db::name("invoices")
            ->alias("i")
            ->join("orders o", "i.id=o.invoiceid")
            ->join("clients c", "i.uid=c.id")
        ...
        $data = [
            "ordercount" => count($rows1),
            "ordercountun" => count($rowsun),
            "ordercountsum" => count($rowsm),
            "total" => floatval($sum),
        ];
        return $data;
    }
    ...
    public function getLaddersaleStatisticsOnlyTotalAccount($time, $uid)
    {
        ...
        $rows2 = \think\Db::name("invoices")
            ->alias("i")
            ->join("clients c", "i.uid=c.id")
            ->leftJoin("accounts a", "a.invoice_id=i.id")
            ->field("i.id as invoiceid,sum(a.amount_out) as amount_outs,a.refund")
            ->where("i.status", "=", "Paid")
            ->whereTime("i.paid_time", $time)
            ->where($wheres2)
            ->where("i.delete_time", 0)
    ...
    public function getSum($uid, $ladder)
    {
        ...
        $rows1 = \think\Db::name("invoice_items")
            ->alias("in")
            ->join("host h", "h.id=in.rel_id")
            ...
            ->join("currencies cu", "cu.id = c.currency")
            ->field(
                "i.uid,h.id as hostid,in.id,in.invoice_id,in.amount,spg.bates,p.name,c.username,c.companyname,in.type,spg.is_renew,spg.updategrade,cu.suffix,h.domain,h.dedicatedip",
            )
            ->where("i.status", "=", "Paid")
            ->where("c.sale_id", $uid)
            ->where($where)
