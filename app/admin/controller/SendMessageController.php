...
class SendMessageController extends AdminBaseController
{
    ...
    public function postEmailPage(\think\Request $request)
    {
        ...
        if (!empty($load_message_id)) {
            $template_data = \think\Db::name("email_templates")
                ->field("id,name,subject,message,fromname,fromemail")
                ->where("type", $type)
                ->where("id", $load_message_id)
                ->find();
        ...
        if ($type == "general") {
            $client_data = \think\Db::name("clients")
            ...
            $client_data = \think\Db::name("host")
                ->field("c.id,c.username,c.email")
                ->alias("h")
                ->leftJoin("clients c", "c.id=h.uid")
                ->whereIn("h.id", $selected)
                ->select()
                ->toArray();
            $returndata["clients"] = $client_data;
        }
        $email_temp_list = \think\Db::name("email_templates")
            ->field("id,name")
            ->where("type", $type)
            ->select()
            ->toArray();
    ...
    public function postSendEmail(\think\Request $request)
    {
        $param = $request->param();
        $rule = [
            "type" => "require|in:product,general",
            "selected" => "require|array",
            "fromname" => "require",
            "fromemail" => "require|email",
            "subject" => "require",
            "message" => "require",
        ];
        $msg = [
            ...
            "type.in" => "发送类型错误",
            "selected.require" => "接收人不能为空",
            "fromname.require" => "发送人信息必填",
            "fromemail.require" => "发送人信息必填",
            "fromemail.email" => "发送人邮箱格式错误",
            "subject.require" => "邮件主题不能为空",
            "message.require" => "邮件内容不能为空",
        ];
        $param = $request->param();
        ...
        if (!empty($savename)) {
            ...
            try {
                ...
                $langs = \think\Db::name("email_languages")
                    ->where("disabled", 1)
                    ->field("language")
                    ->group("language")
                    ->select();
