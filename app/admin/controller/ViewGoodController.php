...
class ViewGoodController extends ViewAdminBaseController
{
    ...
    {
        $result["tip"] =
            "在这里管理您的所有商品和服务。每个商品都必须分配到一个在前台订购页面客户可见的或隐藏的分组里。商品也可以单独设为隐藏。隐藏的商品仍然可以直接使用链接地址访问和订购。";
    ...
    {
        $result["tip"] =
            "在这里配置与我们通信的所有接口。每个模块的默认接口都标有星号（*）。为保证接口自动设置正常运行，您必须选择一个默认的接口。";
    ...
    {
        $result["tip"] =
            "查看取消的订单，添加客户端取消原因。<a href='https://www.idcsmart.com/wiki_list/338.html' target='_blank'>帮助文档</a>";
        $result["page"] = $_GET["page"] ? $_GET["page"] : 1;
    ...
    {
        $result["tip"] =
            "查看取消的订单，添加客户端取消原因。<a href='https://www.idcsmart.com/wiki_list/358.html' target='_blank'>帮助文档</a>";
        $result["page"] = $_GET["page"] ? $_GET["page"] : 1;
        $result["titleList"] = ["ID", "刷新", "名称", "IP", "服务器数量", "区域", "操作"];
        $result["btnText"] = "新增接口";
        if ($_GET) {
