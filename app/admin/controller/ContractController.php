...
class ContractController extends AdminBaseController
{
    ...
    public function initialize()
    {
        ...
        $this->_arg = [
            [
                "title" => lang("CONTRACT_CLIENT"),
                "data" => [
                    ["name" => lang("CONTRACT_CLIENT_USERNAME"), "arg" => "{\$client_username}"],
                    ["name" => lang("CONTRACT_CLIENT_COMPANY_NAME"), "arg" => "{\$client_company_name}"],
                    ["name" => lang("CONTRACT_CLIENT_TELEPHONE"), "arg" => "{\$client_telephone}"],
                    ["name" => lang("CONTRACT_CLIENT_EMAIL"), "arg" => "{\$client_email}"],
                    ["name" => lang("CONTRACT_CLIENT_ADDRESS"), "arg" => "{\$client_address}"],
                ],
            ],
            [
                "title" => lang("CONTRACT_CLIENT_PRODUCT"),
                "data" => [
                    ["name" => lang("CONTRACT_CLIENT_PRODUCT_NAME"), "arg" => "{\$client_product_name}"],
                    ["name" => lang("CONTRACT_CLIENT_PRODUCT_DOMAIN"), "arg" => "{\$client_product_domain}"],
                    ["name" => lang("CONTRACT_CLIENT_PRODUCT_STATUS"), "arg" => "{\$client_product_status}"],
                    ["name" => lang("CONTRACT_CLIENT_PRODUCT_STRART_DATE"), "arg" => "{\$client_product_startdate}"],
                    ["name" => lang("CONTRACT_CLIENT_PRODUCT_END_DATE"), "arg" => "{\$client_product_enddate}"],
                    [
                        "name" => lang("CONTRACT_CLIENT_PRODUCT_BILLING_CYCLE"),
                        "arg" => "{\$client_product_billingcycle}",
                    ],
                    ["name" => lang("CONTRACT_CLIENT_PRODUCT_PRICE"), "arg" => "{\$client_product_price}"],
                    ["name" => lang("CONTRACT_CLIENT_PRODUCT_PRICE_WRITE"), "arg" => "{\$client_product_pricewrite}"],
                    ["name" => lang("CONTRACT_CLIENT_PRODUCT_VALID_TIME"), "arg" => "{\$client_product_validtime}"],
                ],
            ],
        ];
    }
    ...
    public function setting()
    {
        ...
        if (!empty($data["contract_pdf_logo"])) {
            $data["contract_pdf_logo"] =
                request()->domain() . config("contract_get") . configuration("contract_pdf_logo");
        }
        if (!empty($data["contract_company_logo"])) {
            $data["contract_company_logo"] =
                request()->domain() . config("contract_get") . configuration("contract_company_logo");
        }
	...
	*/
    ...
    public function tpl()
    {
        ...
        $contracts = \think\Db::name("contract")
            ->field("id,name,status,force,notes,product_id,remark,create_time")
            ->where($where)
            ->limit($limit)
            ->page($page)
            ->order($order, $sort)
        ...
        foreach ($contracts as $key => $contract) {
            ...
            $contract["signed"] = \think\Db::name("contract_pdf")
                ->where("contract_id", $contract["id"])
                ->whereIn("status", [1, 3, 4, 5])
                ->count();
            $contract["sign"] = \think\Db::name("contract_pdf")
                ->where("contract_id", $contract["id"])
                ->where("status", 2)
                ->count();
    ...
    public function contract()
    {
        ...
        $total = \think\Db::name("contract_pdf")
            ->alias("cp")
            ->leftJoin("host h", "cp.host_id = h.id")
            ->leftJoin("clients c", "h.uid = c.id")
            ->leftJoin("products p", "h.productid = p.id")
            ->leftJoin("orders o", "h.orderid=o.id")
            ->leftJoin("voucher_post vp", "cp.uid=vp.uid")
            ->leftJoin("invoices i", "o.invoiceid=i.id")
            ->leftJoin("contract con", "cp.contract_id=con.id")
            ->where($where)
            ->count();
        $contracts = \think\Db::name("contract_pdf")
            ->alias("cp")
            ->field(
                "i.paid_time,cp.id,c.username,c.email,c.phonenumber,c.companyname,p.name,
            ...
            vp.username as vp_username,vp.detail,vp.phone,h.id as hostid,cp.uid,con.force",
            )
            ->leftJoin("host h", "cp.host_id = h.id")
            ->leftJoin("clients c", "cp.uid = c.id")
            ->leftJoin("products p", "h.productid = p.id")
            ->leftJoin("orders o", "h.orderid=o.id")
            ->leftJoin("voucher_post vp", "cp.uid=vp.uid")
            ->leftJoin("invoices i", "o.invoiceid=i.id")
            ->leftJoin("contract con", "cp.contract_id=con.id")
            ->where($where)
            ->withAttr("domainstatus_zh", function ($value) {
    ...
    public function contractPost()
    {
        ...
        \think\Db::name("contract_pdf")
            ...
            ->update([
                "is_post" => intval($param["is_post"]),
                "express_company" => trim($param["express_company"]),
                "express_order" => trim($param["express_order"]),
                "status" => 4,
            ]);
    ...
    public function contractPage()
    {
        ...
        $client = \think\Db::name("clients")
            ->field("phonenumber,username,email,address1,companyname")
            ->where("id", $uid)
            ->find();
        ...
        $contract = \think\Db::name("contract")
            ->field("id,name,represent,phonenumber,content,email,inscribe_custom")
            ->where("id", $tpl_id)
            ->where("status", 1)
            ->find();
        if (empty($contract)) {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("b.ordernum,d.name,c.description,a.amount,a.create_time,a.nextduedate")
            ->leftJoin("orders b", "a.orderid=b.id")
            ->leftJoin("products d", "a.productid=d.id")
            ->leftJoin("invoice_items c", "a.id=c.rel_id")
            ->where("c.type", "host")
            ->where("a.id", $hid)
            ->find();
        if ($contract["inscribe_custom"]) {
            $party_b = [
                "institutions" => $contract["represent"],
                "addr" => configuration("contract_address"),
                "username" => configuration("contract_username"),
                "phone" => $contract["phonenumber"],
                "email" => $contract["email"],
            ];
        } else {
            $party_b = [
                "institutions" => configuration("contract_institutions"),
                "addr" => configuration("contract_address"),
                "username" => configuration("contract_username"),
                "phone" => configuration("contract_phonenumber"),
                "email" => configuration("contract_email"),
            ];
        }
        $data = [
            "party" => [
                "institutions" => $client["companyname"],
    ...
    public function contractPagePost()
    {
        ...
        if ($param["type"] == "I") {
            \think\Db::name("contract_pdf")
                ->where("id", $id)
                ->update(["pdf_address" => $pdf_address]);
        } else {
            \think\Db::name("contract_pdf")
                ->where("id", $id)
                ->update(["sign_addr" => $img ?: "", "pdf_address" => $pdf_address, "status" => 1]);
        }
