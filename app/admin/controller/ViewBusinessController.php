...
class ViewBusinessController extends ViewAdminBaseController
{
    ...
    public function orders(\think\Request $request)
    {
        ...
        $result["seachList"] = [
            ["type" => "text", "label" => "订单ID", "name" => "id", "content" => ""],
            ["type" => "text", "label" => "客户", "name" => "username", "content" => ""],
            ["type" => "date", "label" => "开始时间", "name" => "start_time", "content" => ""],
        ...
        if ($searchPageRes["status"] == 200) {
            if ($searchPageRes["pay_status"]) {
                ...
                $result["seachList"][count($result["seachList"])] = [
                    "type" => "select",
                    "label" => "付款状态",
                    "name" => "pay_status",
                    "content" => "",
        ...
        $result["navTabs"] = [
            ["label" => "全部", "name" => ""],
            ["label" => "待核验", "name" => "Pending"],
            ["label" => "已激活", "name" => "Active"],
        ...
        $result["tip"] =
            "在这里查看所有订单，生成新订单。<a href='https://www.idcsmart.com/wiki_list/396.html' target='_blank'>帮助文档</a>";
    ...
    {
        $result["tip"] =
            "查看流量包订单。<a href='https://www.idcsmart.com/wiki_list/395.html' target='_blank'>帮助文档</a>";
        $result["page"] = $_GET["page"] ? $_GET["page"] : 1;
    ...
    public function productlist(\think\Request $request)
    {
        ...
        $result["tip"] =
            "系统中所有销售的产品，包括已删除或暂停的产品。<a href='https://www.idcsmart.com/wiki_list/394.html' target='_blank'>帮助文档</a>";
        $result["modalTitle"] = $result["btnText"];
        $result["page"] = $_GET["page"] ? $_GET["page"] : 1;
        $result["titleList"] = [
            "ID",
        ...
        $pageSize =
            \intval($res["data"]["pagination"]["count"] / $pagecount) +
            ($res["data"]["pagination"]["count"] % $pagecount == 0 ? 0 : 1);
        for ($i = 1; $i <= $pageSize; $i++) {
    ...
    public function cancelrequests(\think\Request $request)
    {
        ...
        $result["tip"] =
            "查看取消的订单，添加客户端取消原因。<a href='https://www.idcsmart.com/wiki_list/393.html' target='_blank'>帮助文档</a>";
