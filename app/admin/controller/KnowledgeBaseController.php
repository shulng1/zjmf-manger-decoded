...
class KnowledgeBaseController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $categories = \think\Db::name("knowledge_base_links")
            ->alias("kbl")
            ->field("kbc.id,name,description,hidden,count(kbl.category_id) as num")
            ->leftJoin("knowledge_base_cats kbc", "kbl.category_id = kbc.id")
            ->group("kbl.category_id")
            ->order($order, $sort)
            ->select();
    ...
    public function categoryList()
    {
        ...
        $article = \think\Db::name("knowledge_base_links")
            ->alias("kbl")
            ->field("kb.id,title,article,views,useful,kb.hidden,login_view,host_view,order,public_by,public_time")
            ->leftJoin("knowledge_base_cats kbc", "kbc.id = kbl.category_id")
            ->leftJoin("knowledge_base kb", "kb.id = kbl.article_id")
    ...
    public function tagsList()
    {
        $params = $this->request->param();
        $tag = trim($params["tag"]);
        $article = \think\Db::name("knowledge_base_tags")
            ->alias("kbt")
            ->field("kb.id,title,article,views,useful,kb.hidden,login_view,host_view,order,public_by,public_time")
            ->leftJoin("knowledge_base kb", "kb.id = kbt.article_id")
            ->where("tag", $tag)
            ->select();
        foreach ($article as $key => $value) {
            $value["tag"] = $this->getTagString($value["id"]);
    ...
    public function editArticle()
    {
        ...
        $article = \think\Db::name("knowledge_base")
            ->field("id,title,article,views,useful,hidden,login_view,host_view,order,public_by,public_time")
            ->where("id", $id)
            ->find();
    ...
    public function editArticlePost()
    {
        if ($this->request->isPost()) {
            $data = $this->request->only(
                "id,title,article,categories,views,useful,hidden,login_view,host_view,order,tag,public_by,public_time",
            );
            $id = isset($data["id"]) && !empty($data["id"]) ? intval($data["id"]) : "";
            if (!$id) {
                return jsonrule(["status" => 400, "msg" => lang("ID_ERROR")]);
    ...
    public function deleteCategory()
    {
        ...
        try {
            ...
            $articleids = \think\Db::name("knowledge_base_links")
                ->field("article_id")
