...
class EmailTemplateController extends AdminBaseController
{
            ...
            foreach ($all_controller as $controller) {
                ...
                foreach ($all_action as $action) {
                    ...
                    if (!empty($module) && !empty($controller) && !empty($action)) {
                        ...
                        $rule = db("auth_rule")
                            ->where("lower(name)=\"" . strtolower($rule_name) . "\"")
                            ->find();
                        if (empty($rule)) {
                            $idata = [];
    ...
    public function emailList()
    {
        ...
        $total = \think\Db::name("email_templates")
    ...
    public function createTemplatePost()
    {
        if ($this->request->isPost()) {
            ...
            return jsonrule([
                "status" => 200,
                "msg" => lang("ADD SUCCESS"),
                "id" => $templateid,
                "new" => $new,
                "send" => configuration("company_name"),
                "send_email" => configuration("company_email"),
            ]);
        }
    ...
    public function manageLanguages()
    {
        ...
        $langsused = \think\Db::name("email_templates")
            ->field("language")
            ->where("language", "<>", "")
            ->group("language")
            ->select();
        ...
        if ($this->request->isPost()) {
            ...
            try {
                ...
                foreach ($defaults as $default) {
                    $name = $default["name"];
                    $existlang = \think\Db::name("email_templates")
                        ->where("name", $name)
    ...
    public function editTemplate()
    {
        ...
        $emailtemplate = \think\Db::name("email_templates")
            ->field("id,type,name,attachments,fromname,fromemail,disabled,custom,copyto,blind_copy_to,plaintext")
            ->where("id", $templateid)
            ->find();
        $emailtemplate["fromemail"] = !empty($emailtemplate["fromemail"])
            ? $emailtemplate["fromemail"]
            : configuration("company_email");
        $emailtemplate["fromname"] = !empty($emailtemplate["fromname"])
        ...
        $name = $email["name"];
        $difflangs = \think\Db::name("email_templates")
            ->field("id,subject,message,language")
            ->where("name", $name)
            ->select();
    ...
    protected function uploadHandle($files)
    {
        ...
        foreach ($files as $file) {
            ...
            $info = $file
                ->rule("uniqid")
                ->move($this->imagesave, md5(uniqid()) . time() . $this->split . $originalName);
            if ($info) {
                if (!isset($savename)) {
    ...
    public function disabledTemplate()
    {
        if ($this->request->isPost()) {
            ...
            \think\Db::name("email_templates")
                ->where("id", $id)
                ->update(["disabled" => intval($params["disabled"]), "update_time" => time()]);
