...
{
    const MODE = [
        1 => ["name" => "平均分配", "value" => 1, "desc" => "产品优先分配给产品数量最少的接口"],
        2 => ["name" => "逐个分配", "value" => 2, "desc" => "按最初创建的接口开始分配，满额后切换下一接口"],
    ];
    protected $imagesave;
    ...
    public function serverList()
    {
        ...
        $servers = $servers
            ->order("id", "desc")
            ->page(max(1, $data["page"]), min(50, max(1, $data["limit"])))
            ->select()
            ->toArray();
    ...
    public function addServers()
    {
        ...
        return jsonrule([
            ...
            "data" => ["modules" => $provision->getModules(), "groups" => []],
        ]);
    }
    /**
     * @title 获取模块对应服务器组
    ...
    public function getModulesGroup()
    {
        ...
        $gid = \think\Db::name("servers")
            ->where("server_type", "normal")
            ->where("type", $params["modules"])
            ->column("gid");
    ...
    public function addServersPost()
    {
        if ($this->request->isPost()) {
            ...
            if (
                \think\Db::name("servers")->where("server_type", "normal")->where("name", trim($params["name"]))->find()
            ) {
                return jsonrule(["status" => 400, "msg" => lang("该接口已存在")]);
            }
            if ($params["gid"]) {
                $modules_type = \think\Db::name("servers")
                    ->where("server_type", "normal")
                ...
                if (count(array_unique($modules_type)) != 1) {
                    return jsonrule([
                        "status" => 400,
                        "msg" => lang("同一个接口分组下的接口，服务器模块类型应保持一致！"),
                    ]);
                }
            }
            ...
            if ($params["gid"]) {
                $group = \think\Db::name("server_groups")
                    ->where("id", $params["gid"])
                    ->where("system_type", "normal")
                    ->find();
                if (!$group) {
    ...
    public function groupsList()
    {
        try {
            ...
            foreach ($groups as $k => $v) {
                ...
                $use_num = \think\Db::name("host")
                    ->whereIn("serverid", $servers_ids)
                    ->where("serverid", "<>", 0)
                    ->count();
    ...
    public function createGroups()
    {
        ...
        return jsonrule([
            "status" => 200,
            "msg" => lang("SUCCESS MESSAGE"),
            "data" => ["servers" => $servers, "select_servers" => [], "mode" => array_values(self::MODE)],
        ]);
    }
    /**
     * @title 创建服务器组提交
    ...
    public function editServerGroups()
    {
        ...
        return jsonrule([
            ...
            "data" => [
                ...
                "servers" => $servers,
                "select_servers" => $select_servers ? array_column($select_servers, "id") : [],
                "mode" => array_values(self::MODE),
            ],
        ]);
    }
    /**
            ...
            try {
                return \think\Db::transaction(function () {
                    ...
                    if (!empty($data["mode"]) && $result1["mode"] != $data["mode"]) {
                        $dec .=
                            "分配方式由“" .
                            self::MODE[$result1["mode"]]["name"] .
                            "”改为“" .
                            self::MODE[$data["mode"]]["name"] .
                            "”，";
                    }
                    if (isset($data["sid"])) {
                        \think\Db::name("servers")
    ...
    public function testLink($id)
    {
        ...
        $data = \think\Db::name("servers")
            ->alias("a")
            ->field(
                "a.*,b.name group_name,b.type module_type,b.system_type system_module_type,a.type server_module_type,a.server_type servers_module_type",
            )
            ->leftJoin("server_groups b", "a.gid=b.id")
            ->where("a.id", $id)
            ->find();
        if (empty($data)) {
            return jsonrule(["status" => 400, "msg" => lang("ID_ERROR")]);
