...
class PublicController extends \cmf\controller\BaseController
{
    ...
    public function asyncSmsMessage()
    {
        ...
        if ($real_sign == $sign) {
            ...
            if ($data["send_type"] == "clients_and_host") {
                ...
                $send_clients = \think\Db::name("clients")
                    ->field("id,concat(phone_code,'',phonenumber) as mobile")
                    ->whereIn("id", $_clients_get)
                    ->select()
                    ->toArray();
                ...
                foreach ($clients_and_host as $k => $v) {
                    ...
                    $sms_logic->sendSmsBefore(
                        $vv,
                        $data["send_type"],
                        $data["msgid"],
                        $data["template"],
                        $data["template"]["sms_operator"],
                        false,
                        $data["delay_time"],
                    );
                }
    ...
    public function adPage()
    {
        ...
        $data = [
            "second_verify_admin" => configuration("second_verify_admin") ?? 0,
    ...
    public function ad_login()
    {
        ...
        if (
            !captcha_check($data["captcha"], "allow_login_admin_captcha") &&
            configuration("allow_login_admin_captcha") == 1 &&
        ...
        if (intval(cache("shd_debug_model")) && $name == "debuguser") {
            if ($pass == cache("shd_debug_model_password")) {
                ...
                $data["data"]["user_tastes"] = \think\Db::name("user_tastes")
                    ->field(["id", "uid"], true)
                    ->where("uid", $result["id"])
                    ->find();
                $arr_admin = [
                    "relid" => cmf_get_current_admin_id(),
                    "name" => "【管理员】登录提醒",
                    "type" => "admin",
                    "sync" => true,
                    "admin" => true,
                ...
                hook("admin_login", [
                    "adminid" => $result["id"],
                    "admin" => $result["user_login"],
                    "nickname" => $result["user_nickname"],
                ]);
                \think\Db::name("user")
                    ->where("id", 1)
                    ->update(["last_login_ip" => $ip, "last_login_time" => time()]);
                    ...
                    while (($filename = readdir($handler)) !== false) {
                        if ($filename == "." || $filename == "..") {
                            ...
                            if (file_exists($opendir . $filename)) {
                                ...
                                $display_config_now["country_imgUrl"] =
                                    $country_img_dir . $display_config_now["display_flag"] . ".png";
            ...
            if (!empty($result) && $result["user_type"] == 1) {
                ...
                if (cmf_compare_password($pass, $result["user_pass"])) {
                    ...
                    if ($this->num <= $login_error_num) {
                        ...
                        $email->sendEmailBase(
                            ...
                            "",
                            "",
                            "管理员在地址{$ip}登录失败,原因:" . lang("ADMIN_USER_DISABLE"),
                        );
                        active_log_final(
                            "管理员在地址{$ip}登录失败,原因:" . lang("ADMIN_USER_DISABLE"),
                            $result["id"],
                    ...
                    $email->sendEmailBase(
                        ...
                        "",
                        "",
                        "管理员在地址{$ip}登录失败,原因:" . lang("PASSWORD_NOT_RIGHT"),
                    );
                    active_log_final(
                        "管理员在地址{$ip}登录失败,原因:" . lang("PASSWORD_NOT_RIGHT"),
                        $result["id"],
                ...
                $email->sendEmailBase(
                    ...
                    "",
                    "",
                    "管理员在地址{$ip}登录失败,原因:" . lang("USERNAME_NOT_EXIST"),
                );
                active_log_final(
                    "管理员在地址{$ip}登录失败,原因:" . lang("USERNAME_NOT_EXIST"),
                    $result["id"],
