...
class AgentController extends GetUserController
{
    ...
    {
        $info = \think\Db::name("zjmf_finance_api")
            ->field("id,username,password,ticket_open")
            ->where("is_resource", 1)
            ->where("is_using", 1)
            ->order("id", "desc")
            ->find();
    ...
    public function postResourceInfo()
    {
        ...
        $api = \think\Db::name("zjmf_finance_api")
            ...
            ->find();
        $data = [
            "hostname" => $this->resource_url,
            "username" => trim($param["username"]),
            "password" => $param["password"] ? aesPasswordEncode($param["password"]) : "",
            "name" => "资源池",
            "is_using" => 1,
        ];
        \think\Db::startTrans();
    ...
    public function postResourceTicketOpen()
    {
        ...
        \think\Db::name("zjmf_finance_api")
            ->where("id", $id)
            ->update(["ticket_open" => intval($param["ticket_open"])]);
        return jsonrule(["status" => 200, "msg" => "请求成功"]);
    }
    /**
     * 时间 2021-09-02
    ...
    public function postLinkToResource()
    {
        ...
        $post_data = [
            ...
            "password" => aesPasswordDecode($info["password"]),
            "type" => "agent",
        ];
        $id = $info["id"];
    ...
    public function getProducts()
    {
        ...
        if ($res["status"] == 200) {
            ...
            $products = \think\Db::name("product_first_groups")
                ->field(
                    "c.zjmf_api_id,c.id,c.name,b.name as gname,b.id as gid,a.id as fgid,a.name as fgname,c.qty,
                c.upstream_qty,c.product_shopping_url,c.upstream_product_shopping_url,c.type,c.pay_type,c.api_type,
                c.upstream_version,c.upstream_price_type,c.upstream_price_value,c.upstream_pid",
                )
                ->alias("a")
            ...
            foreach ($products as &$v) {
                ...
                $v["product_price"] = $v["product_count"] = $v["product_price_show"] = bcadd(
                    $v["product_price"],
                    $config_total,
                    2,
                );
                ...
                $v["host_active"] =
                    \think\Db::name("host")->where("productid", $v["id"])->where("domainstatus", "Active")->count() ?:
                    0;
                ...
                $v["resource_price"] =
                    $resource_currency["prefix"] .
                    bcdiv($v["product_price_show"], $v["current_rate"], 2) .
                    $resource_currency["suffix"];
                $local_qty += $v["qty"];
    ...
    public function getOrder()
    {
        ...
        if ($res["status"] == 200) {
            ...
            foreach ($res["data"]["rows"] as $row) {
                ...
                $local_order = \think\Db::name("orders")
                    ->field(
                        "a.amount,a.id,b.uid,b.domain,c.name,d.username,a.payment,e.payment as i_payment,
                    ...
                    ->alias("a")
                    ->leftJoin("upgrades i", "i.order_id=a.id")
                    ->leftJoin("host b", "a.id=b.orderid OR i.relid=b.id")
                    ->leftJoin("products c", "b.productid=c.id")
                    ->leftJoin("clients d", "a.uid=d.id")
                    ->leftJoin("invoices e", "a.invoiceid=e.id")
                    ->where($where)
                    ->whereIn("b.dcimid", $hids)
                    ->where("c.zjmf_api_id", $id)
                ...
                if ($row["invoice_type"] == "product") {
                    $amount = \think\Db::name("invoice_items")
                        ->where("invoice_id", $local_order["invoiceid"])
                        ->whereIn("rel_id", $local_order["hostid"])
                        ->where("type", "<>", "upgrade")
                        ->sum("amount");
                } elseif ($row["invoice_type"] == "upgrade") {
                    $amount = bcmul(
                        bcdiv(bcmul($row["amount"], $row["current_rate"], 20), $row["agent_grade"] / 100, 2),
                        $local_order["percent_value"] / 100,
                        2,
                    );
                }
    ...
    public function getRenew()
    {
        ...
        if ($res["status"] == 200) {
            ...
            $currency = \think\Db::name("currencies")->where("default", 1)->find();
            foreach ($res["data"]["rows"] as &$row) {
                $local_invoices = \think\Db::name("invoices")
                    ->field(
                        "b.amount,i.id,b.uid,b.domain,c.name,d.username,i.payment,i.use_credit_limit,i.subtotal as sub,i.credit,b.id as hostid,i.status",
                    )
                    ->alias("i")
                    ->leftJoin("invoice_items ii", "ii.invoice_id = i.id")
                    ->leftJoin("host b", "ii.rel_id = b.id")
                    ->leftJoin("products c", "b.productid=c.id")
                    ->leftJoin("clients d", "i.uid=d.id")
                    ->where("b.dcimid", $row["hostid"])
                    ->where("c.zjmf_api_id", $id)
                    ->withAttr("payment", function ($value, $data) use ($gateways) {
    ...
    public function getRefundDetail()
    {
        ...
        if ($res["status"] == 200) {
            ...
            $invoice = \think\Db::name("invoices")
                ...
                ->leftJoin("upgrades c", "b.rel_id=c.id")
                ->leftJoin(
                    "host d",
                    "(b.rel_id=d.id AND b.type in ('host','renew')) OR (c.relid=d.id AND b.type in ('upgrade'))",
                )
                ->leftJoin("products e", "d.productid=e.id")
                ->where("e.zjmf_api_id", $id)
                ->whereIn("d.dcimid", $hids)
                ->find();
    ...
    public function getHost()
    {
        ...
        if ($res["status"] == 200) {
            ...
            foreach ($res["data"]["rows"] as &$row) {
                $dcimid = $row["id"];
                $local_host = \think\Db::name("host")
                    ->field("a.id,a.uid,a.amount,a.firstpaymentamount")
                    ->alias("a")
                    ->leftJoin("products b", "a.productid=b.id")
                    ->where("b.zjmf_api_id", $id)
                    ->where("a.dcimid", $dcimid)
                    ->find();
                ...
                $row["firstpaymentamount"] =
                    $currency["prefix"] . ($local_host["firstpaymentamount"] ?: "0.00") . $currency["suffix"];
            }
        }
        return jsonrule($res);
    }
    ...
    public function getBaseInfo()
    {
        ...
        if ($res["status"] == 200) {
            ...
            $host_total = \think\Db::name("host")
                ->alias("a")
                ->leftJoin("products b", "a.productid=b.id")
                ->whereIn("b.upstream_pid", $pids)
                ->where("b.api_type", "zjmf_api")
                ->where("b.zjmf_api_id", $id)
                ->count();
            ...
            $data = [
                "total" => bcsub($total, $total_cost, 2),
                "month" => bcsub($month, $month_cost, 2),
                "host_total" => $host_total,
                "host_active" => $host_active,
                "currency" => $currency,
                "client" => [],
                "resource_url" => $this->resource_url . "/credit",
    ...
    private function getTotal($pids, $start = "", $end = "")
    {
        ...
        $ids = \think\Db::name("invoices")
            ...
            ->leftJoin("products e", "d.productid=e.id")
            ->whereIn("e.upstream_pid", $pids)
            ->where("e.api_type", "zjmf_api")
            ->where("e.zjmf_api_id", $id)
            ->column("a.id");
        ...
        $total += $sum["total"];
        if (!empty($ids[0])) {
            $apply = \think\Db::name("credit")
                ->alias("a")
                ->where(function (\think\db\Query $query) use ($start, $end) {
                    if ($start && $end) {
            ...
            foreach ($ids as $v) {
                $renew_apply =
                    \think\Db::name("credit")
                        ->alias("a")
                        ->where(function (\think\db\Query $query) use ($start, $end, $v) {
                            if ($start && $end) {
                                $query->where("a.create_time", ">=", $start)->where("a.create_time", "<=", $end);
                            }
            ...
            $remove = \think\Db::name("credit")
                ...
                ->where("a.description", "like", "%Credit Removed from Invoice #%")
                ->whereIn("a.relid", $ids)
                ->sum("a.amount");
        } else {
            $apply = 0;
    ...
    public function getConsumption()
    {
        ...
        if ($res["status"] == 200) {
            ...
            $count = \think\Db::name("invoices")
                ->alias("a")
                ->leftJoin("invoice_items b", "a.id=b.invoice_id")
                ->leftJoin("host c", "b.rel_id=c.id AND b.type in ('host','renew')")
                ->leftJoin("upgrades e", "b.rel_id=e.id AND b.type in ('upgrade')")
                ->leftJoin("host f", "e.relid=f.id")
                ->leftJoin("products d", "f.productid=d.id OR c.productid=d.id")
                ->where($where)
                ->group("a.id")
                ->count();
            ...
            $rows = \think\Db::name("invoices")
                ->field(
                    "a.id,a.subtotal,a.payment,a.paid_time,b.type,b.type as type_zh,a.notes,a.uid,a.use_credit_limit,a.credit",
                )
                ->alias("a")
                ->leftJoin("invoice_items b", "a.id=b.invoice_id")
                ->leftJoin("host c", "b.rel_id=c.id AND b.type in ('host','renew')")
                ...
                ->where($where)
                ->withAttr("payment", function ($value, $data) use ($gateways) {
                    $count = \think\Db::name("accounts")
                        ->where("invoice_id", $data["id"])
                        ->where("delete_time", 0)
                        ->count();
                    if (
                        $data["use_credit_limit"] == 1 &&
                        $this->getInterfacePay($data["id"], $data["uid"]) == 0 &&
                        $data["credit"] == 0
                    ) {
                        return "信用额支付";
                    }
            ...
            $data = [
                ...
                "rows" => $rows,
                "gateways" => $gateways,
                "type" => ["all" => "全部", "host" => "产品", "renew" => "续费", "upgrade" => "升降级"],
            ];
            return jsonrule(["status" => 200, "msg" => "请求成功", "data" => $data]);
        }
        return jsonrule($res);
    ...
    public function getHostLists()
    {
        ...
        if ($res["status"] == 200) {
            ...
            $count = \think\Db::name("host")
                ->alias("a")
                ->leftJoin("products b", "a.productid=b.id")
                ->leftJoin("invoice_items d", "a.id=d.rel_id AND d.type in ('host','renew') ")
                ->leftJoin("upgrades u", "a.id=u.relid AND d.rel_id=u.id AND d.type in ('upgrade')")
                ->leftJoin("invoices e", "d.invoice_id=e.id")
                ->where($where)
                ->count();
            $rows = \think\Db::name("host")
                ->field(
                    "a.id,b.name,a.dedicatedip,e.paid_time,a.nextduedate,a.billingcycle,a.billingcycle as billingcycle_zh,b.upstream_pid,
            a.amount,a.amount as cost,a.amount as profit,a.notes,b.upstream_price_value,b.upstream_price_type",
                )
                ->alias("a")
                ->leftJoin("products b", "a.productid=b.id")
                ->leftJoin("invoice_items d", "a.id=d.rel_id AND d.type in ('host','renew') ")
    ...
    public function getRunMapLists()
    {
        ...
        $list = \think\Db::name("run_maping")
            ->alias("r")
            ->leftJoin("host h", "r.host_id = h.id")
            ->leftJoin("products p", "h.productid = p.id")
            ->leftJoin("zjmf_finance_api z", "p.zjmf_api_id = z.id")
            ->field("r.*,h.domain")
            ->where($where)
            ->withAttr("last_execute_time", function ($value) {
