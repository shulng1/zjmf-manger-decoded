...
class AnnounceController extends AdminBaseController
{
    ...
    public function getList()
    {
        ...
        $list = \think\Db::name("announce_ments")
            ->field("id,date,title,hidden")
            ->where("parentid", 0)
            ->order($order, $sort)
            ->select()
            ->toArray();
    ...
    public function getManage(\think\Request $request)
    {
        ...
        if (!empty($id)) {
            $announce_data = \think\Db::name("announce_ments")
            ...
            $mulil_data = \think\Db::name("announce_ments")
                ->field("create_time,update_time", true)
                ->where("parentid", $id)
                ->select()
                ->toArray();
            ...
            if (!empty($multilang_title) && is_array($multilang_title)) {
                ...
                foreach ($multilang_title as $lang => $val) {
                    if (array_key_exists($lang, $language_list)) {
                        $exists_data = \think\Db::name("announce_ments")
                            ->where("parentid", $id)
                            ->where("language", $lang)
                            ->find();
                        if (!empty($exists_data)) {
                            \think\Db::name("announce_ments")
                                ->where("id", $exists_data["id"])
                                ->update([
                                    "title" => $val,
                                    "announcement" => $multilang_announcement[$lang],
                                    "update_time" => $time,
                                ]);
                        } else {
                            \think\Db::name("announce_ments")->insert([
                                "title" => $val,
                                "announcement" => $multilang_announcement[$lang],
                                "language" => $lang,
                                "parentid" => $id,
