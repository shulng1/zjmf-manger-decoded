...
class ProductController extends AdminBaseController
{
    ...
    public function getProuductlistPage()
    {
        ...
        $first_group_data = \think\Db::name("product_first_groups")
            ->where($group_where)
            ->order("order", "asc")
            ->select()
            ->toArray();
        ...
        foreach ($first_group_data as $key => $value) {
            $re["data"][$key] = $value;
            $re["data"][$key]["groups"] = [];
            $group_data = \think\Db::name("product_groups")
                ->where("gid", $value["id"])
            ...
            foreach ($group_data as $k => $v) {
                $product_data = \think\Db::name("products")
                    ->alias("a")
                    ->field(
                        "count(h.id) as count,a.id,a.name,a.gid,a.type,a.pay_method as type_zh,a.pay_type,a.qty,a.auto_setup,a.hidden,b.name as api_name,a.zjmf_api_id,a.stock_control",
                    )
                    ->leftJoin("zjmf_finance_api b", "a.zjmf_api_id = b.id")
                    ->leftJoin("host h", "h.productid = a.id")
                    ->group("a.id")
                    ->where("a.gid", $v["id"])
                    ->whereNotIn("a.type", array_keys($developer_app_product_type))
                    ->where($where)
                    ->withAttr("type_zh", function ($value, $data) {
                    ...
                    ->select()
                    ->toArray();
                $product_data1 = \think\Db::name("products")
                    ->alias("a")
                    ->field("count(h.id) as count_active,a.id")
                    ->leftJoin("zjmf_finance_api b", "a.zjmf_api_id = b.id")
                    ->leftJoin("host h", "h.productid = a.id")
                    ->group("a.id")
                    ->where("a.gid", $v["id"])
    ...
    public function updateFirstGroupsort(\think\Request $request)
    {
        if ($request->isPost()) {
            ...
            if ($pre_gid) {
                ...
                $product_first_groups = \think\Db::name("product_first_groups")
                    ->field("id")
                    ->where("order", "<=", $pre_order)
                    ->select()
                    ->toArray();
                ...
                \think\Db::name("product_first_groups")
                    ->where("id", $gid)
                    ->update(["order" => $pre_order]);
            } else {
                $min_order = \think\Db::name("product_first_groups")->min("order");
                \think\Db::name("product_first_groups")
                    ->where("id", $gid)
                    ->update(["order" => $min_order - 1]);
            }
            $re["status"] = 200;
            $re["msg"] = lang("修改排序成功");
            return jsonrule($re);
        ...
        if ($request->isPost()) {
            ...
            if ($gid == $current_gid) {
                ...
                if ($pre_pid) {
                    ...
                    \think\Db::name("product_groups")
                        ->where("id", $pid)
                        ->update(["order" => $pre_order, "gid" => $current_gid]);
                } else {
                    $min_order = \think\Db::name("product_groups")->where("gid", $current_gid)->min("order");
                    \think\Db::name("product_groups")
                        ->where("id", $pid)
                        ->update(["order" => $min_order - 1, "gid" => $current_gid]);
                }
            }
            $re["status"] = 200;
            $re["msg"] = lang("修改排序成功");
    ...
    public function editGroupPage(\think\Request $request)
    {
        ...
        if (is_numeric($id) && !empty($id)) {
            $product_groups = \think\Db::name("product_groups")
                ->field(
                    "id,name,headline,tagline,order_frm_tpl,hidden,order,disabled_gateways,type,gid,tpl_type,alias,is_upstream,zjfm_api_id",
                )
                ->where("id", $id)
                ->where($where)
                ->find();
            $product_groups["customfields"] = \think\Db::name("product_groups_customfields")
                ->field("name,value")
                ->where("relid", $id)
                ->select()
                ->toArray();
        ...
        $resource = \think\Db::name("zjmf_finance_api")
            ...
            ->find();
        if ($resource) {
            $re["data"]["resourceFirstGroups"] = \think\Db::name("product_first_groups")
                ->where(["zjmf_api_id" => $resource["id"], "is_upstream" => 1])
                ->select()
                ->toArray();
    ...
    public function saveProductGroup(\think\Request $request)
    {
        if ($request->isPost()) {
            $rule = [
                ...
                "tagline" => "max:255",
                "type" => "require|number",
                "order_frm_tpl" => "require",
                "gid" => "require|number",
                "tpl_type" => "require|in:default,custom",
            ];
            $msg = [
                "name.max" => lang("THE_NAME_CANNOT_EXCEED_CHARAC"),
                "headline.max" => lang("PRODUCT_GROUP_TITLE_CANNOT_EXC"),
                "tagline.max" => lang("PRODUCT_GROUP_SLOGAN_CANNOT_EX"),
                "order_frm_tpl.require" => lang("ORDER_FORM_TEMPLATE_CANNOT_BE"),
                "gid.require" => "一级分组不能为空",
            ];
            $des = "";
            $param = $request->param();
            ...
            if (!empty($param["id"])) {
                ...
                $old_customfields = \think\Db::name("product_groups_customfields")
            ...
            if (!empty($param["customfields"])) {
                if (
                    count(array_filter(array_unique(array_column($param["customfields"], "name")))) !=
                    count($param["customfields"])
                ) {
                ...
                if (isset($customfields_change) && $customfields_change === true) {
                    ...
                    if (!empty($param["customfields"])) {
                        foreach ($param["customfields"] as $k => $v) {
                            \think\Db::name("product_groups_customfields")->insertGetId([
                                "relid" => $id,
                                "name" => $v["name"],
                                "value" => $v["value"],
                                "create_time" => time(),
    ...
    public function editFirstGroupPage(\think\Request $request)
    {
        ...
        if (is_numeric($id) && !empty($id)) {
            $product_first_groups = \think\Db::name("product_first_groups")
                ->field("id,name,hidden,order,is_upstream,zjmf_api_id")
                ->where("id", $id)
                ->find();
            $product_first_groups["customfields"] = \think\Db::name("product_first_groups_customfields")
                ->field("name,value")
                ->where("relid", $id)
                ->select()
                ->toArray();
    ...
    public function saveProductFirstGroup(\think\Request $request)
    {
        if ($request->isPost()) {
            ...
            if (!empty($param["id"])) {
                ...
                $old_customfields = \think\Db::name("product_first_groups_customfields")
                    ->where("relid", $param["id"])
                    ->select()
                    ->toArray();
            }
            ...
            if (!empty($param["id"])) {
                ...
                if (isset($customfields_change) && $customfields_change === true) {
                    ...
                    if (!empty($param["customfields"])) {
                        foreach ($param["customfields"] as $k => $v) {
                            \think\Db::name("product_first_groups_customfields")->insertGetId([
                                "relid" => $id,
                                "name" => $v["name"],
                                "value" => $v["value"],
                                "create_time" => time(),
    ...
    public function duplicate()
    {
        ...
        $product_data = \think\Db::name("products")
            ->field("id,name,create_time,update_time,order", true)
            ->where("id", $existingproduct)
            ->find();
        ...
        try {
            ...
            $gids = \think\Db::name("product_config_links")
                ->alias("a")
                ->leftJoin("product_config_groups b", "a.gid = b.id")
                ->where("a.pid", $existingproduct)
                ->where("b.global", 1)
                ->column("a.gid");
            $links = [];
            if (!empty($gids)) {
            ...
            $groups = \think\Db::name("product_config_links")
                ->alias("a")
                ->field("b.id,b.name,b.description,b.upstream_id")
                ->leftJoin("product_config_groups b", "a.gid = b.id")
                ->where("a.pid", $existingproduct)
                ->where("b.global", 0)
                ->order("a.id", "asc")
            ...
            foreach ($groups as $group) {
                $new_gid = \think\Db::name("product_config_groups")->insertGetId([
                    "name" => $newproductname . "配置项组",
                    "description" => $group["description"] ?? "配置项组描述",
                    "upstream_id" => $group["upstream_id"] ?? 0,
                ]);
                \think\Db::name("product_config_links")->insertGetId(["gid" => $new_gid, "pid" => $newpid]);
                break;
            }
            $options = \think\Db::name("product_config_options")
                ->whereIn("gid", array_column($groups, "id"))
                ->select()
                ->toArray();
            $lingAgeArr = [];
            $map_link = [];
            foreach ($options as $ov) {
                ...
                $lingAgeArr[] = $new_oid;
                $sub_options = \think\Db::name("product_config_options_sub")
                    ->where("config_id", $oid)
                    ->select()
                    ->toArray();
                $map = [];
                ...
                $advance_links = \think\Db::name("product_config_options_links")
                    ->where("config_id", $oid)
                    ->order("id", "asc")
                    ->select()
                    ->toArray();
                foreach ($advance_links as $advance_link) {
                    ...
                    $link_id = \think\Db::name("product_config_options_links")->insertGetId([
                        "config_id" => $new_oid,
                        "sub_id" => json_encode($new_advance_sub_ids),
                        "relation" => $advance_link["relation"],
                        "type" => $advance_link["type"],
                        "relation_id" => 0,
                        "upstream_id" => 0,
                    ]);
            ...
            foreach ($map_link as $k1 => $v1) {
                ...
                if ($tmp["type"] == "result") {
                    ...
                    \think\Db::name("product_config_options_links")
                        ->where("id", $v1)
                        ->where("type", "result")
                        ->update(["relation_id" => $new_relation_id]);
                }
            }
            (new \app\common\model\ProductModel())->setLinkAge("copy")->handleLingAge($lingAgeArr);
            $pricing = \think\Db::name("pricing")
                ->field("id,relid", true)
                ->where("relid", $existingproduct)
                ->where("type", "product")
                ->select()
                ->toArray();
            if (!empty($pricing)) {
            ...
            $customfields = \think\Db::name("customfields")
                ->field("id,relid,create_time,update_time", true)
                ->where("relid", $existingproduct)
                ->where("type", "product")
                ->select()
                ->toArray();
            ...
            $upgrade_products = \think\Db::name("product_upgrade_products")
                ->field("product_id, upgrade_product_id")
                ->where("product_id", $existingproduct)
                ->select()
                ->toArray();
            ...
            $download = \think\Db::name("product_downloads")
                ->field("product_id,download_id")
                ->where("product_id", $existingproduct)
                ->select()
                ->toArray();
            if (!empty($download)) {
    ...
    public function addPage()
    {
        $param = $this->request->param();
        $where = [];
        $groupdata = \think\Db::name("product_groups")
            ->field("id,name")
            ->where($where)
            ->order("order", "asc")
            ->select();
    ...
    public function create(\think\Request $request)
    {
        if ($request->isPost()) {
            $rule = [
                "type" => "require|in:" . getProductTypeString(),
                "gid" => "require|number",
                "productname" => "require|max:255",
            ];
            $msg = [
                "type.require" => lang("PRODUCT_TYPE_CANNOT_BE_EMPTY"),
                "type.in" => lang("WRONG_PRODUCT_TYPE"),
                "gid.require" => lang("PRODUCT_GROUP_CANNOT_BE_EMPTY"),
                "gid.number" => lang("PRODUCT_GROUP_MUST_BE_A_NUMBER"),
                "productname.require" => lang("PRODUCT_NAME_CANNOT_BE_EMPTY"),
                "productname.max" => lang("PRODUCT_NAME_CANNOT_EXCEED_CH"),
            ];
            $param = $request->param();
            $validate = new \think\Validate($rule, $msg);
            $result = $validate->check($param);
            ...
            try {
                ...
                if (isset($param["upstream_price_value"])) {
                    \think\Db::name("products")
                        ->where("id", $id)
                        ->update(["upstream_price_value" => $param["upstream_price_value"]]);
                }
                \think\Db::commit();
    ...
    public function editPage()
    {
        ...
        $product = \think\Db::name("products")
            ->field(
                "id,type,gid,groupid,name,description,hidden,welcome_email,stock_control,qty,tax
            ...
            ,config_option23,config_option24,is_truename,clientscount,product_shopping_url,product_group_url,upper_reaches_id,cancel_control,upstream_auto_setup,upstream_ontrial_status",
            )
            ->where("id", $id)
            ->find();
        if (empty($product)) {
        ...
        if (!empty($product["zjmf_api_id"])) {
            $product["is_resource"] = \think\Db::name("zjmf_finance_api")
                ->where("id", $product["zjmf_api_id"])
                ->value("is_resource");
        }
        $where = [];
        ...
        $product["auto_terminate_email"] = empty($product["auto_terminate_email"])
            ? ""
            : $product["auto_terminate_email"];
        ...
        if (!empty($pay_type)) {
            ...
            $re["data"]["product"]["pay_type"] = [
                "pay_type" => !empty($pay_type["pay_type"]) ? $pay_type["pay_type"] : "free",
                "pay_hour_cycle" => !empty($pay_type["pay_hour_cycle"]) ? $pay_type["pay_hour_cycle"] : 720,
                "pay_day_cycle" => !empty($pay_type["pay_day_cycle"]) ? $pay_type["pay_day_cycle"] : 30,
                "pay_ontrial_status" => !empty($pay_type["pay_ontrial_status"]) ? 1 : 0,
                "pay_ontrial_cycle" => !empty($pay_type["pay_ontrial_cycle"]) ? $pay_type["pay_ontrial_cycle"] : 0,
                "pay_ontrial_num" => !empty($pay_type["pay_ontrial_num"]) ? $pay_type["pay_ontrial_num"] : 1,
                "pay_ontrial_condition" => $pay_type["pay_ontrial_condition"] ?? [],
                "pay_ontrial_cycle_type" => $pay_type["pay_ontrial_cycle_type"] ?: "day",
                "pay_ontrial_num_rule" => getEdition() ? ($pay_type["pay_ontrial_num_rule"] ?: 0) : 0,
                "clientscount_rule" => getEdition() ? ($pay_type["clientscount_rule"] ?: 0) : 0,
            ];
        } else {
            $re["data"]["product"]["pay_type"] = [
                "pay_type" => "free",
        ...
        $customfields = \think\Db::name("customfields")
            ->field("type,relid,create_time,update_time", true)
            ->where("type", "product")
            ->where("relid", $id)
            ->select()
            ->toArray();
        ...
        $re["data"]["config_groups"] = \think\Db::name("product_config_groups")
            ->field([0 => "id", "concat(name, \" - \", description)" => "name_desc"])
            ->select()
            ->toArray();
        ...
        $upgrade_product_data = \think\Db::name("product_upgrade_products")
        ...
        if ($product["api_type"] == "zjmf_api" && $product["upstream_pid"] > 0) {
            ...
            if ($res["status"] == 200) {
                ...
                if (
                    $upstream_product["api_type"] == "zjmf_api" &&
                    $upstream_product["upstream_pid"] > 0 &&
                    $upstream_product["upstream_price_type"] == "percent"
                ) {
                    foreach ($upstream_product_pricings as $k => $v) {
                        foreach ($price_type as $kk => $vv) {
                            $upstream_product_pricings[$k][$vv[0]] =
                                ($v[$vv[0]] * $upstream_product["upstream_price_value"]) / 100;
                            $upstream_product_pricings[$k][$vv[1]] =
                                ($v[$vv[1]] * $upstream_product["upstream_price_value"]) / 100;
                        }
                    }
                }
                $re["data"]["upstream_product_pricings"] = $upstream_product_pricings;
                $re["data"]["upstream_pay_ontrial_condition"] = json_decode($res["data"]["products"]["pay_type"], true)[
                    "pay_ontrial_condition"
                ];
            }
        }
    ...
    public function edit(\think\Request $request)
    {
        if ($request->isPost()) {
            ...
            if ($api_type != "zjmf_api" && $api_type != "whmcs") {
                ...
                if ($product["auto_create_config_options"] == 0 && $param["type"] == "dcim") {
                    if (!empty($param["server_group"])) {
                        $server_group = \think\Db::name("server_groups")
                            ->where("id", $param["server_group"])
                            ->where("system_type", "dcim")
                            ->find();
            ...
            if ($param["upper_reaches_id"] > 0) {
                $upper_reaches = \think\Db::name("zjmf_finance_api")
                    ->whereIn("type", ["manual", "whmcs"])
                    ->where("id", \intval($param["upper_reaches_id"]))
                    ->find();
            ...
            if (!empty($basedata["clientscount"]) && $basedata["clientscount"] != $product["clientscount"]) {
                $dec .=
                    "单个客户购买此产品的数量由“" .
                    $product["clientscount"] .
                    "”改为“" .
                    $param["clientscount"] .
                    "”，";
            }
            $basedata["product_shopping_url"] =
                $param["product_shopping_url"] ?: request()->domain() . "/cart?action=configureproduct&pid=" . $id;
            if (
                !empty($basedata["product_shopping_url"]) &&
                $basedata["product_shopping_url"] != $product["product_shopping_url"]
            ) {
                $dec .=
                    "快速订购连接由“" .
                    $product["product_shopping_url"] .
                    "”改为“" .
                    $param["product_shopping_url"] .
                    "”，";
            }
            $first_gid = \think\Db::name("product_groups")->where("id", $basedata["gid"])->value("gid");
            $basedata["product_group_url"] =
                $param["product_group_url"] ?:
                request()->domain() . "/cart?gid=" . $basedata["gid"] . "&fid=" . $first_gid;
            if (
                !empty($basedata["product_group_url"]) &&
                $basedata["product_group_url"] != $product["product_group_url"]
            ) {
                $dec .=
                    "产品组连接由“" . $product["product_group_url"] . "”改为“" . $param["product_group_url"] . "”，";
            }
            ...
            if (!empty($basedata["pay_type"]) && $basedata["pay_type"] != $product["pay_type"]) {
                ...
                if ($pay_type["pay_hour_cycle"] != $payArr["pay_hour_cycle"]) {
                    $dec .=
                        "收费周期小时由“" . $pay_type["pay_hour_cycle"] . "”改为“" . $payArr["pay_hour_cycle"] . "”，";
                }
                ...
                if ($pay_type["pay_ontrial_cycle"] != $payArr["pay_ontrial_cycle"]) {
                    $dec .=
                        "试用的时间由“" .
                        $pay_type["pay_ontrial_cycle"] .
                        "”改为“" .
                        $payArr["pay_ontrial_cycle"] .
                        "”，";
                }
            ...
            if (!empty($basedata["server_group"]) && $basedata["server_group"] != $product["server_group"]) {
                $server_data = \think\Db::name("server_groups")
                    ->field("name")
                    ->where("id=" . $basedata["server_group"])
                    ->find();
                $server_data1 = \think\Db::name("server_groups")
                    ->field("name")
                    ->where("id=" . $product["server_group"])
                    ->find();
            ...
            if (
                !empty($basedata["recurring_cycles"]) &&
                $basedata["recurring_cycles"] != $product["recurring_cycles"]
            ) {
            ...
            if (
                !empty($basedata["auto_terminate_days"]) &&
                $basedata["auto_terminate_days"] != $product["auto_terminate_days"]
            ) {
                $dec .=
                    "自动删除/固定周期由“" .
                    $product["auto_terminate_days"] .
                    "”改为“" .
                    $param["auto_terminate_days"] .
                    "”，";
            }
            $basedata["auto_terminate_email"] = intval($param["auto_terminate_email"]);
            if (
                !empty($basedata["auto_terminate_email"]) &&
                $basedata["auto_terminate_email"] != $product["auto_terminate_email"]
            ) {
                $dec .=
                    "自动终止邮件ID由“" .
                    $product["auto_terminate_email"] .
                    "”改为“" .
                    $param["auto_terminate_email"] .
                    "”，";
            }
            $basedata["config_options_upgrade"] = !empty($param["config_options_upgrade"]) ? 1 : 0;
            if (
                !empty($basedata["config_options_upgrade"]) &&
                $basedata["config_options_upgrade"] != $product["config_options_upgrade"]
            ) {
            ...
            if (
                !empty($basedata["affiliate_pay_type"]) &&
                $basedata["affiliate_pay_type"] != $product["affiliate_pay_type"]
            ) {
                $dec .=
                    "自定义佣金设置由“" .
                    $product["affiliate_pay_type"] .
                    "”改为“" .
                    $param["affiliate_pay_type"] .
                    "”，";
            }
            $basedata["affiliate_pay_amount"] = $param["affiliate_pay_amount"] ?: 0.0;
            if (
                !empty($basedata["affiliate_pay_amount"]) &&
                $basedata["affiliate_pay_amount"] != $product["affiliate_pay_amount"]
            ) {
                $dec .=
                    "金额百分比由“" .
                    $product["affiliate_pay_amount"] .
                    "”改为“" .
                    $param["affiliate_pay_amount"] .
                    "”，";
            }
            $basedata["affiliateonetime"] = !empty($param["affiliateonetime"]) ? 1 : 0;
            if (
                !empty($basedata["affiliateonetime"]) &&
                $basedata["affiliateonetime"] != $product["affiliateonetime"]
            ) {
            ...
            if ($host["show"]) {
                if (
                    empty($param["host_rule_upper"]) &&
                    empty($param["host_rule_lower"]) &&
                    empty($param["host_rule_num"])
                ) {
            ...
            if ($api_type == "manual") {
                $basedata["upper_reaches_id"] = $param["upper_reaches_id"]
                    ? intval($param["upper_reaches_id"])
                    : intval($param["server_group"]);
            } else {
            ...
            $upstream_price_value = isset($param["upstream_price_value"])
                ? floatval($param["upstream_price_value"])
                : 120;
            ...
            if ($api_type == "zjmf_api") {
                $sync_param = [
                    ...
                    "upstream_pid" => $param["upstream_pid"],
                    "page_type" => "edit_product",
                    "upstream_price_type" => $upstream_price_type,
                    "upstream_price_value" => $upstream_price_value,
                ];
                \think\Db::name("products")->where("id", $id)->update($basedata);
                $res = (new \app\common\logic\Product())->syncProduct($sync_param);
                \think\Db::name("products")
                    ->where("id", $id)
                    ->update(["upstream_price_type" => $upstream_price_type]);
                if ($res["status"] != 200) {
                    return jsonrule($res);
                }
                ...
                \think\Db::name("products")
                    ...
                    ->update([
                        ...
                        "is_truename" => intval($param["is_truename"]),
                        "is_bind_phone" => intval($param["is_bind_phone"]),
                        "stock_control" => 0,
                        "qty" => 0,
                        "pay_type" => $pay_type_new,
                        "upstream_pid" => intval($param["upstream_pid"]),
                    ]);
                if ($upstream_price_type == "custom") {
                    ...
                    $product_pricings = \think\Db::name("pricing")
                        ->alias("a")
                        ->field("b.upstream_pid,c.code,a.currency")
                        ->leftJoin("products b", "a.relid = b.id")
                        ->leftJoin("currencies c", "a.currency = c.id")
                ...
                try {
                    ...
                    \think\Db::name("products")->where("id", $id)->update($basedata);
                    if ($api_type == "whmcs") {
                        $field_exist = \think\Db::name("customfields")
                            ->where("type", "product")
                            ->where("relid", $id)
                    ...
                    if ($api_type != "zjmf_api") {
                        ...
                        if ($auto_create_config_options == 1 && $param["type"] == "dcim") {
                            $server_info = \think\Db::name("servers")
                                ->field("id,hostname,username,password,secure,port,accesshash")
                                ->where("gid", $param["server_group"])
                                ->where("server_type", "dcim")
                                ->find();
                            ...
                            if ($basedata["config_option1"] == "rent") {
                                $config_group_id = \think\Db::name("product_config_groups")->insertGetId([
                                    "name" => $config_group_name,
                                    "description" => $config_group_name,
                                ]);
                                \think\Db::name("product_config_links")->insert([
                                    "gid" => $config_group_id,
                                    "pid" => $id,
                                ]);
                                $config_options = [
                                    ["option_name" => "area|区域", "option_type" => 12, "upgrade" => 0, "option" => []],
                                    [
                                        "option_name" => "os|操作系统",
                                        "option_type" => 5,
                                        "upgrade" => 0,
                                    ...
                                    [
                                        ...
                                        "upgrade" => 1,
                                        "option" => [
                                            ["name" => "1|1个", "monthly" => 10],
                                            ["name" => "3|3个", "monthly" => 30],
                                            ["name" => "5|5个", "monthly" => 50],
                                            ["name" => "NO_CHANGE|不变更", "monthly" => 0],
                                        ],
                                    ],
                                    [
                                        "option_name" => "bw|带宽",
                                        ...
                                        "upgrade" => 1,
                                        "option" => [
                                            ["name" => "10,10|10Mbps", "monthly" => 100],
                                            ["name" => "20,20|20Mbps", "monthly" => 200],
                                            ["name" => "50,50|50Mbps", "monthly" => 500],
                                            ["name" => "NO_CHANGE|不变更", "monthly" => 0],
                                        ],
                                    ],
                                ];
                                ...
                                foreach ($dcim_area as $v) {
                                    $config_options[0]["option"][] = [
                                        "name" => $v["id"] . "|" . $v["area"] . "^" . $v["area"],
                                    ];
                                }
                                ...
                                foreach ($config_options as $v) {
                                    ...
                                    foreach ($v["option"] as $vv) {
                                        $config_option_sub = [
                                            "config_id" => $config_options_id,
                                            "option_name" => $vv["name"],
                                        ];
                                        $sub_id = \think\Db::name("product_config_options_sub")->insertGetId(
                                            $config_option_sub,
                                        );
                                        if ($vv["monthly"] > 0) {
                                            $pricing[] = [
                                ...
                                \think\Db::name("products")
                                    ->where("id", $id)
                                    ->update(["auto_create_config_options" => 1]);
                                ...
                                $config_options = [
                                    [
                                        "name" => "group_id|计算节点分组",
                                        "option_type" => 1,
                                        "upgrade" => 0,
                                        "option" => [],
                                    ],
                                    ["name" => "os|操作系统", "option_type" => 5, "upgrade" => 0, "option" => []],
                                    [
                                        "name" => "ip_num|IP数量",
                                        "option_type" => 4,
                                        "hidden" => 0,
                                        ...
                                        "qty_maximum" => 20,
                                        "upgrade" => 1,
                                        "option" => [["name" => "IP数量", "qty_minimum" => 1, "qty_maximum" => 20]],
                                    ],
                                    [
                                        "name" => "system_disk_size|系统盘",
                                        "option_type" => 13,
                                        "hidden" => 1,
                                        "upgrade" => 0,
                                        "option" => [["name" => "50|50G"]],
                                ...
                                foreach ($config_options as $v) {
                                    ...
                                    foreach ($v["option"] as $vv) {
                                        $config_option_sub = [
                                            "config_id" => $config_options_id,
                                            "option_name" => $vv["name"],
                                            "qty_minimum" => $vv["qty_minimum"] ?: 0,
                                            "qty_maximum" => $vv["qty_maximum"] ?: 0,
                                        ];
                                        $sub_id = \think\Db::name("product_config_options_sub")->insertGetId(
                                            $config_option_sub,
                                        );
                        ...
                        if ($auto_create_config_options == 1 && $param["type"] == "dcimcloud") {
                            ...
                            $config_options = [
                                ...
                                [
                                    ...
                                    "upgrade" => 1,
                                    "option" => [
                                        ["name" => "1024|1G", "monthly" => 1],
                                        ["name" => "2048|2G", "monthly" => 2],
                                        ["name" => "4096|4G", "monthly" => 4],
                                        ["name" => "6144|6G", "monthly" => 6],
                                        ["name" => "8192|8G", "monthly" => 8],
                                        ["name" => "12288|12G", "monthly" => 12],
                                        ["name" => "24576|24G", "monthly" => 24],
                                        ["name" => "32768|32G", "monthly" => 32],
                                        ["name" => "65536|64G", "monthly" => 64],
                                        ["name" => "131072|128G", "monthly" => 128],
                                    ],
                                ],
                                [
                                    "name" => "system_disk_size|系统盘",
                                    ...
                                    "hidden" => 1,
                                    "upgrade" => 0,
                                    "option" => [["name" => "lin:30,win:50|Lin30G,Win50G"]],
                                ],
                                [
                                    ...
                                    "hidden" => 1,
                                    "upgrade" => 0,
                                    "option" => [["name" => "vpc|VPC网络"], ["name" => "normal|经典网络"]],
                                ],
                                [
                                    "name" => "bw|带宽",
                                    "option_type" => 11,
                                    ...
                                    "upgrade" => 1,
                                    "option" => [
                                        ["name" => "带宽", "qty_minimum" => 1, "qty_maximum" => 100, "monthly" => 1],
                                    ],
                                ],
                                [
                                    "name" => "in_bw|流入带宽",
                                ...
                                [
                                    ...
                                    "upgrade" => 0,
                                    "option" => [
                                        ["name" => "数据盘", "qty_minimum" => 0, "qty_maximum" => 1000, "monthly" => 1],
                                    ],
                                ],
                                [
                                    "name" => "snap_num|快照数量",
                                ...
                                [
                                    "name" => "system_disk_io_limit|系统盘性能",
                                    "option_type" => 1,
                                    "hidden" => 1,
                                    "upgrade" => 0,
                                    "option" => [
                                        ["name" => "0,0,0,0|不限制性能"],
                                        ["name" => "500,500,2000,2000|500持续2000IOPS"],
                                    ],
                                ],
                                [
                                    "name" => "data_disk_io_limit|数据盘性能",
                                    "option_type" => 1,
                                    "hidden" => 1,
                                    "upgrade" => 0,
                                    "option" => [
                                ...
                                [
                                    "name" => "traffic_bill_type|流量计费方式",
                                    "option_type" => 1,
                                    "hidden" => 1,
                                    "upgrade" => 0,
                                    "option" => [["name" => "month|自然月"], ["name" => "last_30days|订购日至下月"]],
                                ],
                            ];
                            $server_info = \think\Db::name("servers")
                                ->field("id")
                                ->where("gid", $param["server_group"])
                                ->where("server_type", "dcimcloud")
                                ->find();
                        ...
                        if ($auto_create_config_options == 2) {
                            ...
                            $config_options = [
                                [
                                    ...
                                    "option_type" => 5,
                                    "upgrade" => 0,
                                    "option" => ["1|centos^CentOS 7.6 64位"],
                                ],
                                [
                                    "name" => "Location|数据中心",
                                    "option_type" => 12,
                                ...
                                ["name" => "CPU|处理器核心", "option_type" => 6, "upgrade" => 1, "option" => ["4|4核"]],
                                ["name" => "Memory|内存", "option_type" => 8, "upgrade" => 1, "option" => ["2048|2G"]],
                                [
                                    "name" => "Disk Space|数据盘",
                                    "option_type" => 13,
                                    "upgrade" => 0,
                    ...
                    if (empty($this->request->afpid)) {
                        ...
                        $data["affiliate_enabled"] = isset($param["affiliate_enabled"])
                            ? floatval($param["affiliate_enabled"])
                            : 0;
                        $data["affiliate_bates"] = isset($param["affiliate_bates"])
                            ? floatval($param["affiliate_bates"])
                            : 0;
                        $data["affiliate_type"] = isset($param["affiliate_type"])
                            ? intval($param["affiliate_type"])
                            : 0;
                        $data["affiliate_is_reorder"] = isset($param["affiliate_is_reorder"])
                            ? intval($param["affiliate_is_reorder"])
                            : 0;
                        $data["affiliate_reorder"] = isset($param["affiliate_reorder"])
                            ? intval($param["affiliate_reorder"])
                            : 0;
                        $data["affiliate_reorder_type"] = isset($param["affiliate_reorder_type"])
                            ? intval($param["affiliate_reorder_type"])
                            : 0;
                        $data["affiliate_is_renew"] = isset($param["affiliate_is_renew"])
                            ? intval($param["affiliate_is_renew"])
                            : 0;
                        $data["affiliate_renew"] = isset($param["affiliate_renew"])
                            ? intval($param["affiliate_renew"])
                            : 0;
                        $data["affiliate_renew_type"] = isset($param["affiliate_renew_type"])
                            ? intval($param["affiliate_renew_type"])
                            : 0;
                        $data["create_time"] = time();
                        ...
                        $spg = \think\Db::name("affiliates_products_setting")
                            ->where("id", $this->request->afpid)
                            ->find();
                        $param = $this->request->param();
                        $data["affiliate_enabled"] = isset($param["affiliate_enabled"])
                            ? floatval($param["affiliate_enabled"])
                        ...
                        if ($data["affiliate_enabled"] == 1) {
                            ...
                            if ($spg["affiliate_bates"] != $param["affiliate_bates"]) {
                                $desc .=
                                    " 产品推介计划比例" . $spg["affiliate_bates"] . "改为" . $param["affiliate_bates"];
                            }
                            ...
                            if ($spg["affiliate_reorder"] != $param["affiliate_reorder"]) {
                                $desc .=
                                    "二次订购比例由“" .
                                    $spg["affiliate_reorder"] .
                                    "”改为“" .
                                    $param["affiliate_reorder"] .
                                    "”，";
                            }
                        ...
                        \think\Db::name("affiliates_products_setting")
                            ->where("id", $this->request->afpid)
                            ->update($data);
                        if (empty($desc)) {
                            $dec .= "什么都没修改";
    ...
    public function editResProduct(\think\Request $request)
    {
        ...
        if (!empty($basedata["clientscount"]) && $basedata["clientscount"] != $product["clientscount"]) {
            $dec .=
                "单个客户购买此产品的数量由“" . $product["clientscount"] . "”改为“" . $param["clientscount"] . "”，";
        }
        $basedata["product_shopping_url"] =
            $param["product_shopping_url"] ?: request()->domain() . "/cart?action=configureproduct&pid=" . $id;
        if (
        ...
        $first_gid = \think\Db::name("product_groups")->where("id", $basedata["gid"])->value("gid");
        $basedata["product_group_url"] =
            $param["product_group_url"] ?: request()->domain() . "/cart?gid=" . $basedata["gid"] . "&fid=" . $first_gid;
        ...
        if (!empty($basedata["pay_type"]) && $basedata["pay_type"] != $product["pay_type"]) {
            ...
            if ($pay_type["pay_ontrial_cycle"] != $payArr["pay_ontrial_cycle"]) {
                $dec .=
                    "试用的时间由“" . $pay_type["pay_ontrial_cycle"] . "”改为“" . $payArr["pay_ontrial_cycle"] . "”，";
            }
            if ($pay_type["pay_ontrial_condition"] != $payArr["pay_ontrial_condition"]) {
                $arr = config("pay_ontrial_condition_cn_new");
                $pay_type_pay_ontrial_condition = array_flip($pay_type["pay_ontrial_condition"]);
        ...
        if (
            ...
            $dec .=
                "自定义佣金设置由“" . $product["affiliate_pay_type"] . "”改为“" . $param["affiliate_pay_type"] . "”，";
        }
        $basedata["affiliate_pay_amount"] = $param["affiliate_pay_amount"] ?: 0.0;
        if (
            !empty($basedata["affiliate_pay_amount"]) &&
            ...
            $dec .=
                "金额百分比由“" . $product["affiliate_pay_amount"] . "”改为“" . $param["affiliate_pay_amount"] . "”，";
        }
        $basedata["affiliateonetime"] = !empty($param["affiliateonetime"]) ? 1 : 0;
        ...
        if ($api_type == "zjmf_api" && $product["upstream_version"] > 0) {
            ...
            if ($upstream_res["status"] == 200) {
                ...
                if (!empty($upstream_product)) {
                    ...
                    if (
                        !!array_diff($upstream_paytype["pay_ontrial_condition"], $param["pay_ontrial_condition"] ?? [])
                    ) {
        ...
        try {
            if (
                ...
                !empty($upstream_res["data"]["product_pricings"][0]) &&
                $upstream_price_type &&
                ...
                $res_upstream = $product_model->syncProductInfoToResource(
                    $id,
                    $api_type,
                    $zjmf_finance_api_id,
                    $product["upstream_pid"],
                    true,
                    false,
                    false,
                    $rate,
            ...
            if (
                ($api_type == "zjmf_api" &&
                    $product["zjmf_api_id"] > 0 &&
                    $product["zjmf_api_id"] == $zjmf_finance_api_id &&
                    $product["upstream_pid"] > 0 &&
                    $product["upstream_pid"] != $basedata["upstream_pid"]) ||
                ($api_type == "zjmf_api" &&
                    $product["zjmf_api_id"] > 0 &&
                    $product["zjmf_api_id"] != $zjmf_finance_api_id)
            ) {
            ...
            if ($api_type != "zjmf_api") {
                ...
                if ($auto_create_config_options == 1 && $param["type"] == "dcim") {
                    ...
                    if ($basedata["config_option1"] == "rent") {
                        ...
                        $config_options = [
                            ["option_name" => "area|区域", "option_type" => 12, "upgrade" => 0, "option" => []],
                            ["option_name" => "os|操作系统", "option_type" => 5, "upgrade" => 0, "option" => []],
                            [
                                "option_name" => "server_group|硬件配置",
                                "option_type" => 1,
                                "upgrade" => 0,
                        ...
                        \think\Db::name("product_config_links")->insert(["gid" => $config_group_id, "pid" => $id]);
                        $config_options = [
                            ["name" => "group_id|计算节点分组", "option_type" => 1, "upgrade" => 0, "option" => []],
                            ["name" => "os|操作系统", "option_type" => 5, "upgrade" => 0, "option" => []],
                            [
                                "name" => "ip_num|IP数量",
                                "option_type" => 4,
                ...
                if ($auto_create_config_options == 1 && $param["type"] == "dcimcloud") {
                    ...
                    \think\Db::name("product_config_links")->insert(["gid" => $config_group_id, "pid" => $id]);
                    $config_options = [
                        ["name" => "area|区域", "option_type" => 12, "hidden" => 0, "option" => [], "upgrade" => 0],
                        ["name" => "os|操作系统", "option_type" => 5, "hidden" => 0, "option" => [], "upgrade" => 0],
                        [
                            "name" => "cpu|CPU",
                            "option_type" => 6,
                            "hidden" => 0,
                        ...
                        [
                            ...
                            "qty_maximum" => 100,
                            "upgrade" => 1,
                            "option" => [["name" => "带宽", "qty_minimum" => 1, "qty_maximum" => 100, "monthly" => 1]],
                        ],
                        [
                            "name" => "in_bw|流入带宽",
                            "option_type" => 10,
                        ...
                        [
                            ...
                            "hidden" => 1,
                            "upgrade" => 0,
                            "option" => [["name" => "-1|不支持"], ["name" => "0|不限制"], ["name" => "10|10个"]],
                        ],
                        [
                            "name" => "nat_web_limit|共享建站",
                            "option_type" => 1,
                ...
                if ($auto_create_config_options == 2) {
                    ...
                    $config_options = [
                        ...
                        ["name" => "Location|数据中心", "option_type" => 12, "upgrade" => 0, "option" => ["1|CN^香港"]],
                        ["name" => "CPU|处理器核心", "option_type" => 6, "upgrade" => 1, "option" => ["4|4核"]],
                        ["name" => "Memory|内存", "option_type" => 8, "upgrade" => 1, "option" => ["2048|2G"]],
                        ["name" => "Disk Space|数据盘", "option_type" => 13, "upgrade" => 0, "option" => ["40|40G"]],
                        ["name" => "Network Speed|带宽", "option_type" => 10, "upgrade" => 1, "option" => ["1280|10M"]],
                        ["name" => "Snapshot|快照数量", "option_type" => 1, "upgrade" => 1, "option" => ["10|10份"]],
                        ["name" => "Backups|备份数量", "option_type" => 1, "upgrade" => 1, "option" => ["10|10份"]],
                        [
                            "name" => "Extra IP Address|额外IP数量",
                            "option_type" => 1,
                            "upgrade" => 1,
    ...
    public function editStock(\think\Request $request)
    {
        ...
        $re = \think\Db::name("products")
            ->where("id", $id)
            ->update(["qty" => $param["qty"]]);
        (new \app\common\logic\Product())->updateCache([$id]);
    ...
    public function managedownloads(\think\Request $request)
    {
        if ($request->isPost()) {
            ...
            $msg = [
                "id.require" => lang("PRODUCT_ID_CANNOT_BE_EMPTY"),
                "id.number" => lang("PRODUCT_ID_MUST_BE_A_NUMBER"),
                "remdl.number" => lang("DISASSOCIATION_ID_MUST_BE_A_NU"),
                "adddl.number" => lang("ASSOCIATION_ID_MUST_BE_A_NUMBE"),
            ];
            $param = $request->param();
            $validate = new \think\Validate($rule, $msg);
            $result = $validate->check($param);
            ...
            if ($adddl) {
                ...
                $data["create_time"] = time();
                $file_exists = \think\Db::name("product_downloads")
                    ->where("product_id", $id)
                    ->where("download_id", $adddl)
                    ->find();
    ...
    public function downloadcates(\think\Request $request)
    {
        ...
        $productid = $param["productid"] ? intval($param["productid"]) : 0;
        $download_data = [];
        $download_data = \think\Db::name("downloads")
            ->field("d.id,d.location,d.title,d.clientsonly,d.hidden,d.productdownload")
            ->alias("d")
            ->leftJoin("product_downloads p", "p.download_id=d.id")
            ->where("p.product_id", $productid)
            ->where("d.productdownload", 1)
            ->select()
            ->toArray();
    ...
    public function addDownloadcats(\think\Request $request)
    {
        ...
        $msg = [
            "catid.number" => lang("GROUP_ID_MUST_BE_A_NUMBER"),
            "title.require" => lang("GROUP_NAME_CANNOT_BE_EMPTY"),
            "title.max" => lang("THE_TITLE_CANNOT_EXCEED_CHARA"),
    ...
    public function addDownloadFlie(\think\Request $request)
    {
        ...
        $msg = [
            "id.require" => lang("PRODUCT_ID_CANNOT_BE_EMPTY"),
            "id.number" => lang("PRODUCT_ID_MUST_BE_A_NUMBER"),
            "catid.require" => lang("GROUP_NAME_CANNOT_BE_EMPTY"),
            "catid.number" => lang("GROUP_ID_MUST_BE_A_NUMBER"),
            "title.require" => lang("THE_TITLE_NOT_EMPTY"),
            "title.max" => lang("THE_TITLE_CANNOT_EXCEED_CHARA"),
            "title.description" => lang("DESCRIPTION_CANNOT_EXCEED__CHA"),
        ];
        $param = $request->param();
    ...
    public function groupList()
    {
        ...
        $total = \think\Db::name("user_product_groups")
            ...
            ->count();
        $list = \think\Db::name("user_product_groups")
            ->where(function (\think\db\Query $query) use ($data) {
                if (!empty($data["group_name"])) {
                    $query->where("group_name", "like", "%" . trim($data["group_name"]) . "%");
                }
    ...
    public function zklistPage()
    {
        ...
        foreach ($upg as $key => $value) {
            foreach ($cg as $k => $v) {
                $spg = \think\Db::name("user_product_bates")
                    ->alias("u")
                    ->join("client_groups c", "c.id=u.user")
                    ->field("u.id,u.user,u.type,u.bates,c.group_name")
                    ->where("u.products", $value["products"])
                    ->where("u.user", $v["user"])
                    ->find();
                $cg[$k]["id"] = $spg["id"];
                $cg[$k]["type"] = $spg["type"];
    ...
    public function editUserProductgroup()
    {
        if ($this->request->isPost()) {
            ...
            if (empty($param["id"])) {
                ...
                active_log(
                    sprintf($this->lang["Product_admin_UserProductgroup"], $res, $data["user"], $data["products"]),
                );
                return json(["status" => 200, "msg" => lang("SUCCESS MESSAGE")]);
            } else {
                $desc = "";
                ...
                if ($data["type"] == 0) {
                    ...
                    try {
                        ...
                        active_log(
                            sprintf(
                                $this->lang["Product_admin_UserProductgroupedit"],
                                $param["id"],
                                $desc,
                                $data["user"],
                                $data["products"],
