...
class PromoCodeController extends AdminBaseController
{
    ...
    public function addPage()
    {
        ...
        $configoptions = \think\Db::name("product_config_options")
            ->alias("a")
            ->field("a.id,a.option_name,b.name")
        ...
        return jsonrule([
            ...
            "type" => config("promo_code_type"),
            "products" => $groups,
            "cycles" => config("coupon_cycle_promo"),
            "config_options" => $configoptions,
            "type_upgrade" => config("promo_code_type_upgrade"),
        ]);
    }
    /**
     * @title 添加优惠码
    ...
    public function add()
    {
        ...
        $msg = [
            ...
            "start_time.number" => lang("PROMO_CODE_START_TIME_FORMAT_ERROR"),
            "expiration_time.number" => lang("PROMO_CODE_EXPIRE_TIME_FORMAT_ERROR"),
        ];
        ...
        if (!empty($params["appliesto"]) && is_array($params["appliesto"])) {
            ...
            if (!empty($params["appliesto"])) {
                $appliesto = \think\Db::name("products")
                    ->field("id")
                    ->whereIn("id", $params["appliesto"])
                    ->select()
                    ->toArray();
        ...
        if (!empty($params["requires"]) && is_array($params["requires"])) {
            ...
            if (!empty($params["requires"])) {
                $requires = \think\Db::name("products")
                    ->field("id")
                    ->whereIn("id", $params["requires"])
                    ->select()
                    ->toArray();
        ...
        $data["recurfor"] =
            !empty($params["recurring"]) && !empty($params["recurfor"]) ? intval($params["recurfor"]) : 0;
        ...
        if (!empty($data["upgrades"])) {
            ...
            if (
                !empty($params["upgrade_value_type"]) &&
                in_array($params["upgrade_value_type"], ["percent", "fixed"])
            ) {
            ...
            if (!empty($params["upgrade_options"]) && is_array($params["upgrade_options"])) {
                ...
                if (!empty($upgrade_options)) {
                    $upgrade_options = \think\Db::name("product_config_options")
                        ->field("id")
                        ->whereIn("id", $upgrade_options)
                        ->select()
                        ->toArray();
    ...
    public function save()
    {
        ...
        if (isset($params["start_time"])) {
            ...
            if ($data["start_time"] != $code_info["start_time"]) {
                $dec .=
                    "开始时间由“" .
                    date("Y-m-d", $code_info["start_time"]) .
                    "”改为“" .
        ...
        if (isset($params["expiration_time"])) {
            ...
            if ($data["expiration_time"] != $code_info["expiration_time"]) {
                $dec .=
                    "失效时间由“" .
                    date("Y-m-d", $code_info["expiration_time"]) .
                    "”改为“" .
                    date("Y-m-d", $data["expiration_time"]) .
                    "”，";
            }
        }
        if (isset($params["appliesto"])) {
        ...
        if (isset($params["recurfor"])) {
            $data["recurfor"] =
                (!empty($data["recurring"]) || (!isset($data["recurring"]) && !empty($code_info["recurring"]))) &&
                !empty($params["recurfor"])
                    ? intval($params["recurfor"])
                    : 0;
        }
    ...
    public function expireImmediately()
    {
        ...
        \think\Db::name("promo_code")
            ->where("id", $id)
            ->update(["expiration_time" => strtotime("-1 day")]);
    ...
    {
        $type = input("get.type");
        $db = \think\Db::name("promo_code")->field(
            "id,code,type,value,recurring,max_times,used,start_time,expiration_time",
        );
        $now = time();
        if ($type == "all") {
        } elseif ($type == "expired") {
        ...
        foreach ($data as $k => $v) {
            ...
            $data[$k]["expiration_time"] =
                $v["expiration_time"] == 0 ? "-" : date("Y-m-d H:i:s", $v["expiration_time"]);
        }
        $result["status"] = 200;
        $result["data"] = $data;
        $result["msg"] = lang("SUCCESS MESSAGE");
