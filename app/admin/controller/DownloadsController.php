...
class DownloadsController extends AdminBaseController
{
    ...
    public function getList(\think\Request $request)
    {
        ...
        if (empty($id)) {
            ...
            if (!empty($cats_data[0])) {
                $returndata["files"] = \think\Db::name("downloads")
                    ->where("category", $cats_data[0]["id"])
    ...
    public function getEdit()
    {
        ...
        $cats_data = \think\Db::name("downloadcats")
            ->field("id,parentid,name,description,hidden")
            ->where("id", $id)
            ->find();
        if (empty($cats_data)) {
            return jsonrule(["status" => 406, "msg" => "未找到该分类"]);
        ...
        if ($request->isPost()) {
            try {
                ...
                if ($type == 1) {
                    ...
                    if (
                        $pre_order[0] == $cat_order[0] &&
                        \intval($pre_order[0]) === 0 &&
                        \intval($cat_order[0]) === 0
                    ) {
                        \think\Db::name("downloadcats")
                            ->where("id", $pre_id)
                            ->update(["sort" => $pre_id]);
    ...
    public function deleteCat(\think\Request $request)
    {
        if ($request->isDelete()) {
            ...
            try {
                ...
                $delete_down = \think\Db::name("downloads")
                    ->field("id,type,location")
                    ->whereIn("category", $delete_ids)
                    ->select()
                    ->toArray();
        ...
        try {
            if ($request->isPost()) {
                $rule = [
                    ...
                    "type" => "require",
                    "title" => "require",
                    "filetype" => "require|in:manual,upload,remote",
                    "clientsonly" => "in:0,1",
                    "productdownload" => "in:0,1",
                    "hidden" => "in:0,1",
                ];
                $msg = [
                    "cateid.require" => "分类id不能为空",
                    "cateid.number" => "分类id必须为数字",
                    "type.require" => "文件类型不能为空",
                    "title.require" => "标题不能为空",
                    "filetype.require" => "请选择上传还是通过FTP手动上传至downloads 目录",
                    "filetype.in" => "请选择上传还是通过FTP手动上传至downloads 目录",
                ];
                $param = $request->param();
                $validate = new \think\Validate($rule, $msg);
                $result = $validate->check($param);
                ...
                if ($filetype == "manual") {
                    ...
                    if (!file_exists(UPLOAD_PATH_DWN . "support/" . $filename)) {
                        return jsonrule([
                            "status" => 400,
                            "data" => "public/upload/support目录下不存在此文件，请检查文件名是否一致",
                        ]);
                    }
                ...
                if (
                    strpos(config("download_ext")[$type], pathinfo($idata["location"])["extension"]) === false &&
                    $filetype != "remote"
                ) {
        ...
        if ($request->isPost()) {
            ...
            if ($filetype == "manual") {
                ...
                if (!file_exists(UPLOAD_PATH_DWN . "support/" . $filename)) {
                    return jsonrule([
                        "status" => 400,
                        "data" => "downloads/support目录下不存在此文件，请检查文件名是否一致",
                    ]);
                }
            } else {
        ...
        if ($request->isPost()) {
            ...
            if (is_dir(UPLOAD_PATH_DWN . "support")) {
                ...
                if (!$arr) {
                    return jsonrule([
                        "status" => 400,
                        "msg" => "public/upload/support目录下已经存在此文件,不可重复上传",
                    ]);
                }
            }
    ...
    public function getUserDownList(\think\Request $request)
    {
        ...
        $uid = $params["uid"];
        $uid = intval($uid) ?: 0;
        $userdownloadcount = \think\Db::name("userdownloads")
            ->alias("u")
            ->field("u.id")
            ->where("u.uid", $uid)
            ->leftJoin("user c", "c.id=u.adminid")
            ->count();
        $userdownloads = \think\Db::name("userdownloads")
            ->alias("u")
            ->field("u.*,c.user_nickname")
            ->leftJoin("user c", "c.id=u.adminid")
            ->where("u.uid", $uid)
            ->order($order, $sort)
            ->page($page)
    ...
    public function deleteUserFile(\think\Request $request)
    {
        ...
        foreach ($file_data as $k => $v) {
            ...
            active_logs(
                sprintf(
                    $this->lang["Download_admin_deleteuserfile"],
                    $v["id"],
                    $v["name"] . "文件名:" . explode("^", $filename)[1] . "备注:" . $v["remarks"],
                ),
                $v["uid"],
            );
            active_logs(
    ...
    public function postAddUserFile(\think\Request $request)
    {
        if ($request->isPost()) {
            ...
            if (!file_exists(UPLOAD_PATH_DWN . "clients/" . $uid . "/" . $filename)) {
                return jsonrule([
                    "status" => 400,
                    "data" => "public/upload/clients/" . $uid . "/" . "目录下不存在此文件，请检查文件名是否一致",
                ]);
            }
            if (empty($idata["url"])) {
                return jsonrule(["status" => 400, "data" => "文件未上传成功，请等待"]);
            ...
            if ($res) {
                active_logs(
                    sprintf(
                        $this->lang["Download_admin_postadduserfile"],
                        $res,
                        $name . "文件名:" . $idata["downame"] . "备注:" . $remarks,
                    ),
                    $uid,
                );
                active_logs(
