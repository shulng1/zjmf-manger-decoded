...
class VoucherController extends GetUserController
{
    ...
    public function getRate()
    {
        $data = [
            ...
            "rate" => configuration("voucher_rate") ?? "",
        ];
    ...
    public function getVoucherList()
    {
        ...
        $voucher = \think\Db::name("voucher")
            ->alias("a")
            ->field(
                "a.id,e.username,a.create_time,b.title,b.issue_type,b.issue_type as issue_type_zh,a.amount,a.status,a.status as status_zh,c.province,c.city,c.region,c.detail,d.name,a.notes,a.check_time,f.prefix,f.suffix",
            )
            ...
            ->leftJoin("currencies f", "f.id = e.currency")
            ->where($where)
        ...
        foreach ($voucher as $k => &$v) {
            ...
            $subtotal = \think\Db::name("invoices")
    ...
    public function getVoucherDetail()
    {
        ...
        $voucher = \think\Db::name("voucher")
            ->alias("a")
            ->field(
                "a.id,a.invoice_id,a.create_time,b.title,b.issue_type,b.issue_type as issue_type_zh,
            e.subtotal as amount,a.status,a.status as status_zh,c.province,c.city,c.region,c.detail,d.name,
            a.notes,d.price,g.prefix,g.suffix,b.bank,b.account,b.address,b.tax_id,b.phone,b.voucher_type,b.voucher_type as voucher_type_zh",
            )
            ->leftJoin("voucher_type b", "a.type_id = b.id")
            ->leftJoin("voucher_post c", "a.post_id = c.id")
            ->leftJoin("express d", "a.express_id = d.id")
            ->leftJoin("invoices e", "a.invoice_id = e.id")
            ->leftJoin("clients f", "a.uid = f.id")
            ->leftJoin("currencies g", "g.id = f.currency")
            ->withAttr("issue_type_zh", function ($value, $data) {
                return $this->type[$value];
            })
            ->withAttr("status_zh", function ($value, $data) use ($voucher_status) {
        ...
        $data = [
            ...
            "uid" => $tmp["uid"],
            "status" => $status,
            "status_zh" => config("invoice_payment_status")[$status],
        ];
        return jsonrule(["status" => 200, "msg" => lang("SUCCESS MESSAGE"), "data" => $data]);
    }
    /**
