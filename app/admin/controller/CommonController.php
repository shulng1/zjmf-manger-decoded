...
class CommonController extends GetUserController
{
    ...
    public function common()
    {
        ...
        if ($version_data < date("Ymd")) {
            \think\Db::startTrans();
            try {
                $res = \think\Db::name("configuration")
                    ->where("setting", "upgrade_send_template_version")
                    ->where("value", $version)
                    ->update(["value" => microtime(true)]);
    ...
    public function getProductConfigOptions()
    {
        ...
        foreach ($groups as &$group) {
            $group["options"] = \think\Db::name("product_config_options")
