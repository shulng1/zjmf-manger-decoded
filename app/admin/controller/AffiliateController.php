...
class AffiliateController extends GetUserController
{
    ...
    public function index(\think\Request $request)
    {
        ...
        $where = [];
        $data = \think\Db::name("affiliates")
            ->alias("a")
            ->field("a.*,c.username,c.companyname")
            ->join("clients c", "a.uid=c.id")
            ->where($where)
        ...
        if ($this->request->isPost()) {
            ...
            if (!empty($data)) {
                ...
                try {
                    ...
                    active_log_final(
                        sprintf($this->lang["Aff_admin_useraffibalance"], $spg["id"], $desc),
                        $param["uid"],
                    );
                    \think\Db::commit();
                } catch (\Exception $e) {
        ...
        if ($this->request->isPost()) {
            ...
            if (empty($id)) {
                ...
                $data["affiliate_reorder_type"] = isset($param["affiliate_reorder_type"])
                    ? floatval($param["affiliate_reorder_type"])
                    : 0;
                $data["affiliate_is_renew"] = isset($param["affiliate_is_renew"])
                    ? floatval($param["affiliate_is_renew"])
                    : 0;
                ...
                $data["affiliate_renew_type"] = isset($param["affiliate_renew_type"])
                    ? floatval($param["affiliate_renew_type"])
                    : 0;
                ...
                $data["affiliate_type"] = isset($param["affiliate_type"]) ? floatval($param["affiliate_type"]) : 0;
                $data["affiliate_is_reorder"] = isset($param["affiliate_is_reorder"])
                    ? floatval($param["affiliate_is_reorder"])
                    : 0;
                $data["affiliate_reorder"] = isset($param["affiliate_reorder"])
                    ? floatval($param["affiliate_reorder"])
                    : 0;
                $data["affiliate_reorder_type"] = isset($param["affiliate_reorder_type"])
                    ? floatval($param["affiliate_reorder_type"])
                    : 0;
                ...
                if ($spg["affiliate_bates"] != $param["affiliate_bates"]) {
                    $desc .=
                        "用户推介计划比例由“" . $spg["affiliate_bates"] . "”改为“" . $param["affiliate_bates"] . "”，";
                }
                ...
                if ($spg["affiliate_reorder"] != $param["affiliate_reorder"]) {
                    $desc .=
                        " 二次订购比例“" . $spg["affiliate_reorder"] . "”改为“" . $param["affiliate_reorder"] . "”，";
                }
    ...
    public function useraffilist()
    {
        ...
        $rows = \think\Db::name("affiliates_user")
            ->alias("a")
            ->join("clients c", "a.uid=c.id")
            ->field("c.id,c.create_time,c.username,c.companyname,c.lastlogin")
            ->where($where)
            ->where("a.affid", $data["id"])
            ->where(function (\think\db\Query $query) use ($params) {
            ...
            ->select()
            ->toArray();
        $total = \think\Db::name("affiliates_user")
            ->alias("a")
            ->join("clients c", "a.uid=c.id")
            ->field("c.id,c.create_time,c.username,c.lastloginip")
            ->where($where)
            ->where("a.affid", $data["id"])
            ->where(function (\think\db\Query $query) use ($params) {
                if (!empty($params["username"])) {
        ...
        foreach ($rows as $key => $value) {
            $str = $rows[$key]["companyname"]
                ? $rows[$key]["username"] . "(" . $rows[$key]["companyname"] . ")"
                : $rows[$key]["username"];
            $rows[$key]["username"] =
    ...
    public function useraffirecord(\think\Request $request)
    {
        ...
        $rows = \think\Db::name("affiliates_withdraw")
            ...
            ->leftJoin("user u", "a.admin_id=u.id")
            ->field("a.id,a.num,a.type,a.create_time,a.status,a.reason,u.user_nickname,cu.suffix,c.companyname")
            ->where(function (\think\db\Query $query) use ($params) {
        ...
        $total = \think\Db::name("affiliates_withdraw")
            ...
            ->leftJoin("currencies cu", "cu.id = c.currency")
            ->leftJoin("user u", "a.admin_id=u.id")
            ->field("a.id,a.num,a.type,a.create_time,a.status,a.reason,u.user_nickname,cu.suffix")
            ->where(function (\think\db\Query $query) use ($params) {
                if (!empty($params["user_nickname"])) {
                    $search_desc = $params["user_nickname"];
                    $query->where("u.user_nickname LIKE '%{$search_desc}%'");
    ...
    public function useraffibuyrecord(\think\Request $request)
    {
        ...
        $total = \think\Db::name("invoices")
            ->alias("i")
            ->join("clients c", "i.uid=c.id")
            ->leftJoin("currencies cu", "cu.id = c.currency")
            ->leftJoin("orders o", "i.id=o.invoiceid")
            ->field(
                "i.status,i.type,i.subtotal,i.paid_time,c.username,o.id,c.id as uid,o.status,i.create_time,i.id as invoiceid,cu.prefix,cu.suffix",
            )
        ...
        $rows = \think\Db::name("invoices")
            ...
            ->leftJoin("orders o", "i.id=o.invoiceid")
            ->field(
                "o.invoiceid,i.status,i.type,i.subtotal,i.paid_time,c.username,o.id,c.id as uid,o.status,i.create_time,i.id as invoiceid,cu.prefix,cu.suffix,i.aff_sure_time,i.aff_commission,i.aff_commmission_bates,i.aff_commmission_bates_type,i.is_aff,c.companyname",
            )
            ->where("i.delete_time", 0)
            ->where("i.status", "=", "Paid")
            ->where("c.id", "in", $uids)
        ...
        foreach ($rows as $k => $value) {
            if (
                $rows[$k]["is_aff"] == 1 &&
                $value["paid_time"] + configuration("affiliate_delay_commission") * 24 * 60 * 60 <= time()
            ) {
            ...
            foreach ($value["child"] as $key => $val) {
                if ($val["is_aff"] == 1) {
                    ...
                    $rows[$k]["child"][$key]["commission_bates_type"] =
                        $rows[$k]["child"][$key]["aff_commmission_bates_type"];
                }
            }
        ...
        foreach ($rows as $k => $value) {
            ...
            if (
                $rows[$k]["commission_bates"] == "0.00" ||
                $rows[$k]["commission_bates"] == 0 ||
                $rows[$k]["commission_bates"] == ""
            ) {
                unset($rows[$k]);
            }
        }
    ...
    public function productaffiPost()
    {
        if ($this->request->isPost()) {
            ...
            if (empty($id)) {
                ...
                if (
                    $data["affiliate_is_reorder"] == 1 &&
                    $spg["affiliate_is_reorder"] != $param["affiliate_is_reorder"]
                ) {
                    if ($spg["affiliate_is_reorder"] == 1) {
                        $desc .= "二次订购由“关闭”改为“开启”，";
                    } else {
    ...
    public function affiwithdrawrecord(\think\Request $request)
    {
        ...
        $rows = \think\Db::name("affiliates_withdraw")
            ...
            ->leftJoin("user u", "a.admin_id=u.id")
            ->field(
                "c.id as uid,a.id,a.num,a.type,a.create_time,a.status,a.reason,u.user_nickname,cu.suffix,c.username,c.companyname",
            )
            ->where(function (\think\db\Query $query) use ($params) {
                if (!empty($params["user_nickname"])) {
                    $search_desc = $params["user_nickname"];
        ...
        foreach ($rows as $key => $value) {
            ...
            $rows[$key]["username"] =
                "<a class=\"el-link el-link--primary is-underline\"
            href=\"#/customer-view/promotion_plan?id=" .
                $value["uid"] .
                "\">
            <span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
                $str .
            ...
            if ($aw["status"] == 1) {
                if ($status == 2) {
                    ...
                    try {
                        ...
                        $affi = \think\Db::name("affiliates")
                            ->field("balance,withdrawn,withdraw_ing")
                            ->where("uid", $aw["uid"])
                            ->find();
                        \think\Db::name("affiliates")
                            ->where("uid", $aw["uid"])
                            ->update([
                                "withdraw_ing" => bcsub($affi["withdraw_ing"], $aw["num"], 2),
                                "withdrawn" => bcadd($affi["withdrawn"], $aw["num"], 2),
                                "updated_time" => time(),
                            ]);
                        $desc .=
                            " 正在提现余额变化" . $affi["balance"] . "改为" . bcsub($affi["balance"], $aw["num"], 2);
                        $desc .= " 已提现变化" . $affi["withdrawn"] . "改为" . bcadd($affi["withdrawn"], $aw["num"], 2);
                        active_log_final(
                            sprintf($this->lang["Aff_admin_withdrawsh"], $param["id"], $aw["uid"], $desc),
                            $aw["uid"],
                        );
                        \think\Db::commit();
                    } catch (\Exception $e) {
                    ...
                    \think\Db::name("affiliates")
                        ->where("uid", $aw["uid"])
                        ->update([
                            "balance" => bcadd($data["balance"], $aw["num"], 2),
                            "withdraw_ing" => bcsub($data["withdraw_ing"], $aw["num"], 2),
                            "updated_time" => time(),
                        ]);
                    active_log_final(
                        sprintf($this->lang["Aff_admin_withdrawsh"], $param["id"], $aw["uid"], $param["reason"]),
                        $aw["uid"],
                    );
                } else {
    ...
    public function test()
    {
        $rows = \think\Db::name("orders")
            ...
            ->leftJoin("currencies cu", "cu.id = c.currency")
            ->rightJoin("invoices i", "i.id=o.invoiceid")
            ->field(
            ...
            if (!empty($af)) {
                ...
                if (!empty($affi)) {
                    ...
                    try {
                        ...
                        active_log_final(
                            sprintf(
                                $this->lang["Aff_cron_update_balance"],
                                $affi["uid"],
                                $affi["balance"],
