...
class OauthController extends AdminBaseController
{
    ...
    public function listing()
    {
        ...
        foreach ($pluginsa as $key => &$val) {
            ...
            if (
                version_compare(
                    $result_data[$val["dirName"]]["app_version"],
                    $result_data[$val["dirName"]]["old_version"],
                    ">",
                )
            ) {
                $val["update_btn"] = 1;
    ...
    public function active(\think\Request $request)
    {
        ...
        $pluginCount = \think\Db::name("plugin")
            ->where(["name" => $dirName, "module" => $this->modules])
            ->count();
        if ($pluginCount > 0) {
        ...
        $plugin = [
            ...
            "url" => $meta["logo_url"] ? $meta["logo_url"] : "",
            "author" => $meta["author"],
    ...
    {
        $pluginDbAll = \think\Db::name("plugin")
            ->where(["module" => $this->modules])
            ->select();
    ...
    public function configSave(\think\Request $request)
    {
        ...
        $pluginDb = \think\Db::name("plugin")
            ->where(["module" => $this->modules, "name" => $dirName])
            ->field("id")
            ->find();
        if (!$pluginDb) {
