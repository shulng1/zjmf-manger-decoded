...
class AccountController extends GetUserController
{
    ...
    public function searchPage()
    {
        ...
        return jsonrule([
            ...
            "gateway" => array_merge(gateway_list(), $other_pay),
            "salelist" => $list,
        ]);
    }
    /**
    ...
    public function index()
    {
        ...
        $rows = \think\Db::name("accounts")
            ...
            ->leftjoin("invoices ii", "a.invoice_id = ii.id")
            ->where($fun)
            ->whereBetweenTime("a.pay_time", $start_time, $end_time)
            ->field(
                "ii.type,c.companyname,a.id,a.invoice_id,c.id as uid,c.sale_id,c.username,a.currency,cu.prefix,cu.suffix,a.pay_time,a.update_time,a.gateway,a.description,a.amount_in,a.fees,a.amount_out,a.trans_id",
            )
        ...
        foreach ($currencys as $item) {
            ...
            $amount_in = db("accounts")
                ->alias("a")
                ->leftjoin("clients c", "c.id=a.uid")
                ->where($fun1)
                ->where($where)
                ->sum("a.amount_in");
            $amount_out = db("accounts")
                ->alias("a")
                ->leftjoin("clients c", "c.id=a.uid")
                ->where($fun1)
                ->where($where)
        ...
        return jsonrule([
            ...
            "page" => $pages,
            "count" => $total,
            "amount_in_totals" => $amount_in_totals,
        ...
        if ($param["amount_in"] || $param["amount_out"] || $param["fees"]) {
            ...
            try {
                ...
                if (!empty($param["refund"])) {
                    ...
                    credit_log([
                        "uid" => $param["uid"],
                        "desc" => $param["description"] . "{交易流水编号： {$trans_id}}",
                        "amount" => $credit,
                    ]);
                }
                \think\Db::commit();
	...
	*/
    public function update($id)
    {
        ...
        if ($res) {
            ...
            active_log_final(
                sprintf($this->lang["Account_admin_update"], $id, $data["uid"], $data["invoice_id"], $dec),
                $data["uid"],
            );
            unset($dec);
    ...
    public function delete($id)
    {
        ...
        if ($res) {
            active_log_final(
                sprintf(
                    $this->lang["Account_admin_delete"],
                    $id,
                    $accont["uid"],
                    $accont["invoice_id"],
                    $accont["trans_id"],
