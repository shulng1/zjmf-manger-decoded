...
class SetController extends AdminBaseController
{
    ...
    public function site()
    {
        ...
        $adminThemesDir =
            WEB_ROOT .
            config("template.cmf_admin_theme_path") .
        ...
        if ($this->request->isPost()) {
            ...
            if (!empty($adminSettings["admin_theme"])) {
                $result = cmf_set_dynamic_config([
                    "template" => ["cmf_admin_default_theme" => $adminSettings["admin_theme"]],
                ]);
                if ($result === false) {
                    $this->error("配置写入失败!");
                }
    ...
    {
        $keys = [
            "daily_email_backup_status",
            "daily_email_backup",
            "daily_ftp_backup_status",
            "ftp_backup_hostname",
            "ftp_backup_port",
            "ftp_backup_username\n            ",
            "ftp_backup_password",
            "ftp_backup_destination",
            "ftp_secure_mode",
            "ftp_passive_mode",
        ];
        $config_data = getConfig($keys);
        if ($config_data["ftp_backup_password"]) {
            $config_data["ftp_backup_password"] = str_pad(
                "",
                strlen(cmf_decrypt($config_data["ftp_backup_password"])),
                "*",
            );
        }
        $returndata = [];
    ...
    public function backupDatabaseFtp(\think\Request $request)
    {
        $param = $request->param();
        $rule = [
            "ftp_backup_hostname" => "require",
            "ftp_backup_port" => "require|number",
            "ftp_backup_username" => "require",
            "ftp_backup_password" => "require",
            "ftp_backup_destination" => "require",
            "ftp_secure_mode" => "in:0,1",
            "ftp_passive_mode" => "in:0,1",
            "type" => "in:test,save",
        ];
        $msg = [
            "ftp_backup_hostname.require" => "FTP主机名不能为空",
            "ftp_backup_port.require" => "FTP端口号不能为空",
            "ftp_backup_port.number" => "FTP端口号必须为数字",
            "ftp_backup_username.require" => "FTP用户名不能为空",
            "ftp_backup_password.require" => "FTP密码不能为空",
            "ftp_backup_destination.require" => "FTP路径不能为空",
        ];
