...
class TicketController extends GetUserController
{
    ...
    public function createPage()
    {
        ...
        $data["client"] = \think\Db::name("clients")
            ->field("id,username,email,companyname")
            ->where("id", $ticket["uid"])
            ->find();
        $allow = model("TicketDepartmentAdmin")->getAllow();
        $data["department"] = \think\Db::name("ticket_department")
            ->field("id,name")
            ->where("hidden", 0)
            ->whereIn("id", $allow)
            ->select()
            ->toArray();
        $custom_arr = \think\Db::name("customfieldsvalues")
            ->field("A.value,B.fieldname")
            ->alias("A")
            ->join("customfields B", "A.fieldid = B.id")
            ->where("A.relid", $ticket["id"])
            ->select()
            ->toArray();
    ...
    public function add()
    {
        ...
        $rule = [
            ...
            "content" => "require|length:0,10000",
            "email" => "email",
        ];
        $msg = [
            "title.require" => lang("TICKET_TITLE_EMPTY"),
            "dptid.require" => lang("TICKET_DEPARTMENT_EMPTY"),
            "priority.in" => lang("TICKET_PRIORITY_ERROR"),
            "content.require" => lang("TICKET_CONTENT_EMPTY"),
            "email.email" => lang("EMAIL_FORMAT_ERROR"),
        ];
        ...
        $data["content"] = model("Ticket")->parse($params["content"], [
            "name" => $params["name"],
            "email" => $params["email"],
        ]);
        $data["status"] = 1;
        ...
        if ($r) {
            ...
            $client = check_type_is_use(
                $message_template_type[strtolower("Support_Ticket_Opened")],
                $data["uid"],
                $sms,
            );
            if ($client) {
                $params = [
                    "ticket_createtime" => date("Y-m-d H:i:s", time()),
                    "ticketnumber_tickettitle" => $data["title"],
                ];
                $sms->sendSms(
                    $message_template_type[strtolower("Support_Ticket_Opened")],
            ...
            $hook_data = [
                ...
                "uid" => \intval($data["uid"]),
                "dptid" => $data["dptid"],
                "dptname" => \think\Db::name("ticket_department")->where("id", $data["dptid"])->value("name"),
    ...
    public function getList()
    {
        ...
        $data = \think\Db::name("ticket")
            ->alias("a")
            ->field(
                "a.id,a.tid,a.uid,a.admin_unread,a.title,a.name,a.status,a.create_time,a.is_deliver,a.is_receive,a.handle,t.color as statusColor,t.title as status_title,a.last_reply_time,b.user_login flag_admin,c.name department_name,d.username user_name,e.user_login handle_name",
            )
    ...
    public function getClientTicketPage()
    {
        ...
        $ticket_data = \think\Db::name("ticket")
            ->field("t.id,t.tid,t.dptid,t.title,t.status,t.create_time,t.last_reply_time,d.name as depart_name")
            ->alias("t")
            ->leftJoin("ticket_department d", "d.id=t.dptid")
            ->where($where)
            ->page($page, $limit)
            ->order($order, $sort)
            ->select()
        ...
        $returndata["opened_this_month"] = \think\Db::name("ticket")
            ->where(array_merge($where, [["create_time", ">=", $opened_this_month]]))
            ->count();
        if (date("m") != 1) {
        ...
        $returndata["opened_last_month"] = \think\Db::name("ticket")
            ->where(
                array_merge($where, [
                    ["create_time", "<", $opened_this_month],
                    ["create_time", ">=", $opened_last_month],
                ]),
            )
            ->count();
        $opened_this_year = mktime(0, 0, 0, 0, 0, date("Y"));
        $returndata["opened_this_year"] = \think\Db::name("ticket")
            ->where(array_merge($where, [["create_time", ">=", $opened_this_year]]))
            ->count();
        $opened_last_year = mktime(0, 0, 0, 0, 0, date("Y") - 1);
        $returndata["opened_last_year"] = \think\Db::name("ticket")
            ->where(
                array_merge($where, [
                    ["create_time", "<", $opened_this_year],
                    ["create_time", ">=", $opened_last_year],
                ]),
            )
            ->count();
        ...
        foreach ($ticket_data as $key => $val) {
            ...
            $replay_data = \think\Db::name("ticket_reply as t")
                ->field("create_time")
                ->where("tid", $tid)
        ...
        if (isset($data["productid"]) && $data["productid"] > 0) {
            if (isset($data["uid"]) && $data["uid"] > 0) {
                $tmp = \think\Db::name("host")
                    ->where([["productid", "", $data["productid"]], ["uid", "=", $data["uid"]]])
                    ->select();
                unset($data["uid"]);
            } else {
    ...
    public function reply()
    {
        ...
        if ($result["status"] == 200) {
            ...
            if ($client) {
                ...
                $arr = [
                    "name" => $message_template_type[strtolower("ticket_reply")],
            ...
            $hook_data = [
                ...
                "admin" => $data["admin"],
                "status" => 2,
                "status_title" => \think\Db::name("ticket_status")->where("id", 2)->value("title"),
            ];
    ...
    public function saveReply()
    {
        ...
        if ($type == "r") {
            $r = \think\Db::name("ticket_reply")
                ->where("id", $id)
                ->update(["content" => $content]);
        } elseif ($type == "t") {
            $r = \think\Db::name("ticket")
                ->where("id", $id)
                ->update(["content" => $content]);
        }
        if ($r) {
        ...
        active_log(
            sprintf($this->lang["Ticket_admin_saveReply"], $ticket["uid"], $id, $ticket["title"]),
            $ticket["uid"],
        );
        $result["status"] = 200;
    ...
    public function mergeTicket()
    {
        ...
        $r = \think\Db::name("ticket")
            ->whereIn("id", $id)
            ->update(["merged_ticket_id" => $ticketid, "client_unread" => 0, "admin_unread" => 0]);
        if ($r) {
            \think\Db::name("ticket_note")
                ->whereIn("tid", $id)
                ->update(["tid" => $ticketid]);
            ...
            model("SystemLog")->log(
                lang("TICKET_MERGE_SUCCESS", [implode(",", $tickets), $merged]),
                "ticket",
                $ticketid,
            );
            foreach ($tickets as $tic => $value) {
                active_log(
                    sprintf($this->lang["Ticket_admin_mergeTicket"], $value["uid"], $value["id"]),
                    $value["uid"],
                );
            }
        }
    ...
    public function closeTicket()
    {
        ...
        $allow = model("TicketDepartmentAdmin")->getAllow();
        $change_status_ticket = \think\Db::name("ticket")
            ->whereIn("id", $id)
        ...
        $r = \think\Db::name("ticket")
            ...
            ->where("merged_ticket_id", 0)
            ->whereIn("dptid", $allow)
            ->update(["status" => $status["id"], "update_time" => time()]);
        if ($r) {
            ...
            foreach ($tickets as $tic => $value) {
                active_log(
                    sprintf($this->lang["Ticket_admin_closeTicket"], $value["uid"], $value["id"], $status["title"]),
                    $value["uid"],
                );
            }
    ...
    public function deleteTicket()
    {
        ...
        $allow = model("TicketDepartmentAdmin")->getAllow();
        $ticket = \think\Db::name("ticket")
            ->field("id,tid,attachment,dptid,uid,title")
            ->whereIn("id", $id)
            ->whereIn("dptid", $allow)
            ->where("merged_ticket_id", 0)
            ->select()
        ...
        $merged_ticket = \think\Db::name("ticket")
            ->field("id,attachment,dptid")
            ->whereIn("merged_ticket_id", $id)
            ->select()
            ->toArray();
        ...
        $attachment = array_filter(
            array_merge($attachment, $merged_ticket_attachment, $note_attachment, $reply_attachment),
        );
        ...
        try {
            ...
            foreach ($ticket as $tic => $value) {
                active_log(
                    sprintf($this->lang["Ticket_admin_deleteTicket"], $value["uid"], $value["id"], $value["title"]),
                    $value["uid"],
                );
            }
            if (!empty($fields)) {
                \think\Db::name("customfieldsvalues")
                    ->whereIn("fieldid", array_column($fields, "id"))
                    ->whereIn("relid", $delete)
                    ->delete();
            }
            if ($r) {
                \think\Db::commit();
    ...
    {
        $id = input("post.id");
        $info = \think\Db::name("ticket_note")
            ->field("t.uid,tn.id,tn.attachment,t.tid,t.dptid,t.id ticketid")
            ->alias("tn")
            ->leftJoin("ticket t", "tn.tid=t.id")
            ->where("tn.id", $id)
        ...
        if ($r) {
            ...
            model("SystemLog")->log(
                lang("TICKET_NOTE_DELETE_SUCCESS_LOG", [$info["tid"]]),
                "ticket",
                $info["ticketid"],
            );
        }
    ...
    {
        $id = input("post.id");
        $info = \think\Db::name("ticket_reply")
            ->field("t.uid,tn.id,tn.attachment,t.tid,t.dptid,t.id ticketid")
            ->alias("tn")
            ->leftJoin("ticket t", "tn.tid=t.id")
            ->where("tn.id", $id)
        ...
        if ($r) {
            $this->deleteAttachments(explode(",", $info["attachment"]));
            model("SystemLog")->log(
                lang("TICKET_REPLY_DELETE_SUCCESS_LOG", [$info["tid"]]),
                "ticket",
                $info["ticketid"],
            );
        ...
        active_log(
            sprintf($this->lang["Ticket_admin_deleteReply"], $info["uid"], $info["tid"], $t["title"]),
            $info["uid"],
        );
        $result["status"] = 200;
        $result["msg"] = lang("DELETE SUCCESS");
    ...
    public function deleteAttachment()
    {
        ...
        $msg = [
            "id.require" => lang("ID_ERROR"),
            "type.require" => lang("TYPE_ERROR"),
            "type.in" => lang("TYPE_ERROR"),
            "index.require" => lang("TICKET_ATTACHMENT_DELETE_EMPTY"),
            "index.number" => lang("TICKET_ATTACHMENT_ERROR"),
        ];
        $validate = new \think\Validate($rule, $msg);
        $validate_result = $validate->check($params);
        if (!$validate_result) {
        ...
        if ($table == "ticket") {
            ...
            $ticket_info = \think\Db::name($table)
                ->field("b.id,b.tid,b.dptid,a.attachment")
                ->alias("a")
                ->leftJoin("ticket b", "a.tid=b.id")
                ->where("a.id", $id)
                ->where("b.merged_ticket_id", 0)
                ->find();
        }
        ...
        model("SystemLog")->log(
            lang("TICKET_ATTACHMENT_DELETE_SUCCESS_LOG", [$attachment[$index]]),
            "ticket",
            $ticket_info["id"],
        );
        ...
        \think\Db::name($table)
            ->where("id", $id)
            ->update(["attachment" => implode(",", $attachment)]);
        $result["status"] = 200;
        $result["msg"] = lang("DELETE SUCCESS");
        return jsonrule($result);
    }
    ...
    public function downloadAttachment()
    {
        ...
        $msg = [
            ...
            "type.require" => lang("TYPE_ERROR"),
            "type.in" => lang("TYPE_ERROR"),
            "index.require" => lang("TICKET_ATTACHMENT_DOWNLOAD_EMPTY"),
            "index.number" => lang("TICKET_ATTACHMENT_ERROR"),
        ];
        $validate = new \think\Validate($rule, $msg);
        $validate_result = $validate->check($params);
        ...
        if ($table == "ticket") {
            ...
            $ticket_info = \think\Db::name($table)
                ->field("b.tid,b.dptid,a.attachment")
                ->alias("a")
                ->leftJoin("ticket b", "a.tid=b.id")
                ->where("a.id", $id)
                ->where("b.merged_ticket_id", 0)
    ...
    public function ticketDetail($id = 0)
    {
        ...
        $list[] = [
            "id" => $id,
            "uid" => $ticket["uid"],
            "admin_id" => \intval($ticket["admin_id"]),
            "type" => "t",
            "content" => $ticket["content"],
            "attachment" => isset($ticket["attachment"][0])
                ? array_map(function ($v) use ($url) {
                    return $url . $v;
                }, explode(",", $ticket["attachment"]))
                : [],
            "time" => $ticket["create_time"],
            "format_time" => date("Y-m-d H:i:s", $ticket["create_time"]),
            "is_deliver" => $ticket["is_deliver"],
            "is_receive" => $ticket["is_receive"],
        ];
        if (!empty($ticket["uid"])) {
        ...
        foreach ($note as $v) {
            ...
            $list[] = [
                ...
                "type" => "n",
                "content" => $v["content"],
                "attachment" => isset($v["attachment"][0])
                    ? array_map(function ($v1) use ($url) {
                        return $url . $v1;
                    }, explode(",", $v["attachment"]))
                    : [],
                "time" => $v["create_time"],
                "format_time" => date("Y-m-d H:i:s", $v["create_time"]),
                "user" => $v["admin"],
                "realname" => isset($user["user_nickname"][0]) ? $user["user_nickname"] : $v["admin"],
        ...
        foreach ($reply as $v) {
            $v["type"] = "r";
            $v["attachment"] = isset($v["attachment"][0])
                ? array_map(function ($v1) use ($url) {
                    return $url . $v1;
                }, explode(",", $v["attachment"]))
                : [];
        ...
        foreach ($ticket_transfer_log as &$v) {
            ...
            $list[] = [
                ...
                "type" => "log",
                "mode" => $v["mode"],
                "mode_zh" => config("app.ticket_transfer_mode." . $v["mode"]),
                "desc" => $v["desc"],
                "remarks" => $v["remarks"],
                "time" => $v["create_time"],
                "format_time" => date("Y-m-d H:i:s", $v["create_time"]),
        ...
        $result["data"] = [
            ...
            "ticket" => [
                "dptid" => $ticket["dptid"],
                "dpt_name" => $ticket["dpt_name"],
                "handle" => $ticket["handle"],
                "handle_name" => $ticket["handle_name"],
                "title" => html_entity_decode($ticket["title"], ENT_QUOTES),
                ...
                "is_deliver" => $ticket["is_deliver"],
                "is_receive" => $ticket["is_receive"],
                "deliver_status" => $ticket["deliver_status"],
                "deliver_status_title" => $ticket["deliver_status_title"],
            ],
            "user" => $user,
    ...
    public function getTicketDetailHost()
    {
        ...
        $host_list = \think\Db::name("host")
            ->field(
                "h.id,h.initiative_renew,h.domain,h.uid,h.regdate as create_time,h.productid,h.billingcycle,h.domain,h.payment,h.nextduedate,h.amount,h.domainstatus,c.username,p.name as productname,c.currency,p.type,h.serverid",
            )
            ->alias("h")
            ->leftJoin("products p", "p.id=h.productid")
            ->leftJoin("clients c", "c.id=h.uid")
    ...
    public function saveTicket()
    {
        ...
        if (!isset($data["dptid"]) || $data["dptid"] == $ticket["dptid"]) {
            if (isset($params["customfield"]) && is_array($params["customfield"])) {
                (new \app\common\model\CustomfieldsModel())->updateCustomValue(
                    $ticket["dptid"],
                    $id,
                    $params["customfield"],
                    "ticket",
    ...
    public function ticketStatistics()
    {
        ...
        $admins = \think\Db::name("user")
            ->field("id,user_login,user_nickname")
            ->whereIn("id", $admins_id)
            ->select()
            ->toArray();
        $tickets = \think\Db::name("ticket")
            ...
            ->select()
            ->toArray();
        $ticket_replys = \think\Db::name("ticket_reply")
            ->where("create_time", ">=", $start)
            ->where("create_time", "<=", $end)
            ->select()
            ->toArray();
    ...
    public function ticketReceive()
    {
        ...
        $r = \think\Db::name("ticket")
            ->where("id", $id)
            ->update(["handle" => $admin, "handle_time" => time(), "status" => 5]);
        if ($r) {
    ...
    public function ticketTransferList()
    {
        ...
        $id = intval($params["id"]);
        $ticket = \think\Db::name("ticket")->where("id", $id)->find();
        $ticket_department = \think\Db::name("ticket_department")
            ->field("id,name")
            ->where("id", "<>", $ticket["dptid"])
            ->select()
            ->toArray();
        foreach ($ticket_department as &$value) {
            $ticket_department_admin = \think\Db::name("ticket_department_admin")
                ->field("admin_id")
                ->where("dptid", $value["id"])
                ->select()
                ->toArray();
            $value["admins"] = \think\Db::name("user")
                ->field("id,user_login")
                ->whereIn("id", array_column($ticket_department_admin, "admin_id"))
                ->where("id", "<>", $ticket["handle"])
                ->select()
                ->toArray();
        }
        $ticket_department_admin = \think\Db::name("ticket_department_admin")
            ->field("admin_id")
            ->where("dptid", $ticket["dptid"])
            ->select()
            ->toArray();
        $admins = \think\Db::name("user")
            ->field("id,user_login")
        ...
        if ($mode == 1) {
            ...
            if (empty($ticket["handle"])) {
                $r = \think\Db::name("ticket")
                    ...
                    ->update(["dptid" => $dptid]);
                if ($r) {
                    $desc = lang("TICKET_TRANSFER_SUCCESS_LOG", [
                        $id,
                        $old_ticket_department["name"],
                        $ticket_department["name"],
                        config("app.ticket_transfer_mode." . $mode),
                    ]);
                    \think\Db::name("ticket_transfer_log")->insertGetId([
                        "tid" => $id,
                        "desc" => $desc,
                        "remarks" => $params["remarks"] ?? "",
                        "mode" => $mode,
                        "old_dptid" => $ticket["dptid"],
                        "dptid" => $dptid,
                        "admin" => $admin,
                        "create_time" => time(),
                    ]);
                ...
                $r = \think\Db::name("ticket")
                    ->where("id", $id)
                    ->update(["dptid" => $dptid, "handle" => $handle, "handle_time" => time()]);
                if ($r) {
                    $desc = lang("TICKET_TRANSFER_SUCCESS_LOG", [
                        $id,
                        $old_ticket_department["name"] . "(" . $old_handle_info["user_login"] . ")",
                        $ticket_department["name"] . "(" . $handle_info["user_login"] . ")",
                        config("app.ticket_transfer_mode." . $mode),
                    ]);
                    \think\Db::name("ticket_transfer_log")->insertGetId([
                        "tid" => $id,
            ...
            $r = \think\Db::name("ticket")
                ->where("id", $id)
                ->update(["handle" => $handle, "handle_time" => time()]);
            if ($r) {
                $desc = lang("TICKET_TRANSFER_SUCCESS_LOG", [
                    $id,
                    $old_handle_info["user_login"],
                    $handle_info["user_login"],
                    config("app.ticket_transfer_mode." . $mode),
                ]);
                \think\Db::name("ticket_transfer_log")->insertGetId([
                    "tid" => $id,
