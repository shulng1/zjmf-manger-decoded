...
class ViewPluginsController extends ViewAdminBaseController
{
    ...
    public function index(\think\Request $request)
    {
        ...
        $data["svg_menu"] = svgBase64EncodeImage(
            request()->domain() .
                "/" .
                config("database.admin_application") .
                "/themes/default/assets/images/arrow.svg",
        );
        $languagesys = request()->languagesys;
        $status = [
            "chinese" => [1 => "开启", 0 => "禁用", 3 => "未安装"],
            "english" => [1 => "Open", 0 => "Disabled", 3 => "Uninstall"],
            "chinese_tw" => [1 => "開啟", 0 => "禁用", 3 => "未安裝"],
        ];
        if ($languagesys == "CN") {
            $lang = "chinese";
        ...
        $data["pageInfo"] = [
            "count" => $count,
            "limit" => $limit,
            "pages" => $this->ajaxPages(array_slice($plugins, ($page - 1) * $limit, $limit), $limit, $page, $count),
        ];
