...
class ZjmfFinanceApiController extends GetUserController
{
    ...
    public function createApi()
    {
        ...
        if ($params["type"] == "manual") {
            ...
            $insert = [
                ...
                "password" => aesPasswordEncode(html_entity_decode($params["password"], ENT_QUOTES)),
                "create_time" => time(),
                "contact_way" => $params["contact_way"] ?? "",
                "auto_update" => 1,
            ];
            ...
            if ($res["status"] == 200) {
                \think\Db::name("zjmf_finance_api")
                    ->where("id", $id)
                    ->update(["product_num" => $res["data"]["count"]]);
            }
            $data = ["id" => $id];
    ...
    public function modifyApi()
    {
        ...
        if ($params["type"] == "manual") {
            ...
            $count = \think\Db::name("zjmf_finance_api")
                ->where("id", "<>", $id)
            ...
            if ($params["name"] != $api["name"]) {
                $exist = \think\Db::name("zjmf_finance_api")
                    ->where("id", "<>", $id)
                    ->where("is_resource", 0)
                    ->where("name", $params["name"])
                    ->find();
    ...
    public function index()
    {
        ...
        $count = \think\Db::name("zjmf_finance_api")
            ->alias("a")
            ->whereLike("a.hostname|a.username", "%{$search}%")
            ->where($where)
            ->count();
        $data = \think\Db::name("zjmf_finance_api")
            ->alias("a")
            ->field(
                "a.id,a.type,a.type as type_zh,a.contact_way,a.name,a.des,a.hostname,a.username,a.password,a.status,a.product_num,count(DISTINCT b.id) set_product_num,count(DISTINCT c.id) active_host_num,count(DISTINCT d.id) host_num",
            )
            ->leftJoin("products b", "a.id=b.zjmf_api_id OR a.id=b.upper_reaches_id")
            ->leftJoin("host c", "b.id=c.productid AND domainstatus=\"Active\"")
            ->leftJoin("host d", "b.id=d.productid")
            ->whereLike("a.hostname|a.username", "%{$search}%")
            ->where($where)
    ...
    public function refreshStatus($id)
    {
        ...
        if ($data["is_resource"] == 1) {
            ...
            $post_data = [
                ...
                "password" => aesPasswordDecode($data["password"]),
                "type" => "agent",
            ];
        } else {
            if ($data["type"] == "whmcs") {
    ...
    public function resetApiPwd()
    {
        ...
        \think\Db::name("clients")
            ->where("id", $uid)
            ->update(["api_password" => aesPasswordEncode(randStrToPass(12, 0))]);
    ...
    public function apiProducts()
    {
        ...
        $where = function (\think\db\Query $query) use ($param) {
            ...
            if (!empty($param["id"])) {
                ...
                $query->where(function (\think\db\Query $query) use ($param) {
                    $query
                        ->where("a.is_upstream", 1)
                        ->whereOr("b.is_upstream", 1)
                        ->whereOr("c.zjmf_api_id|c.upper_reaches_id", ">", 0);
                });
            }
        };
        $products = \think\Db::name("product_first_groups")
            ->field(
                "d.id as zjmf_api_id,c.id,c.name,b.name as gname,b.id as gid,a.id as fgid,a.name as fgname,c.qty,c.upstream_qty,c.product_shopping_url,c.upstream_product_shopping_url,c.type,c.pay_type,c.api_type,c.upstream_version,c.upstream_price_type,c.upstream_price_value",
            )
            ->alias("a")
            ->leftJoin("product_groups b", "a.id=b.gid")
            ->leftJoin("products c", "b.id=c.gid")
            ->leftJoin("zjmf_finance_api d", "c.zjmf_api_id=d.id or c.upper_reaches_id=d.id")
            ->where("d.id", ">", 0)
            ->where($where)
            ->select()
            ->toArray();
        ...
        foreach ($products as &$v) {
            ...
            $v["host_active"] =
                \think\Db::name("host")->where("productid", $v["id"])->where("domainstatus", "Active")->count() ?: 0;
    ...
    public function apiOrder()
    {
        ...
        $price_total = \think\Db::name("orders")
            ...
            ->leftJoin("zjmf_finance_api d", "p.zjmf_api_id=d.id OR p.upper_reaches_id=d.id")
            ->where("d.is_resource", 0)
            ->field("o.amount")
            ->where($fun)
    ...
    public function apiOrderCom()
    {
        ...
        $rows = \think\Db::name("orders")
            ...
            ->where("d.is_resource", 0)
            ->field(
                "i.status,i.type,o.id,c.sale_id,o.invoiceid,cu.prefix,cu.suffix,o.amount as am,i.use_credit_limit,i.id as invoice_id",
            )
        ...
        $row1 = \think\Db::name("orders")
            ...
            ->leftJoin("zjmf_finance_api d", "p.zjmf_api_id=d.id OR p.upper_reaches_id=d.id")
            ->where("d.is_resource", 0)
            ->field("o.id,c.sale_id,o.invoiceid,cu.prefix,cu.suffix,o.amount,i.use_credit_limit,i.id as invoice_id")
            ->where($where)
            ->where("o.id", "like", "%{$id}%")
            ->where("c.username", "like", "%{$username}%")
            ->where("o.create_time", ">=", $start_time)
    ...
    public function apiHost()
    {
        ...
        $count = \think\Db::name("host")
            ...
            ->leftJoin("zjmf_finance_api d", "b.zjmf_api_id=d.id OR b.upper_reaches_id=d.id")
            ->where($where)
            ->count();
        $hosts = \think\Db::name("host")
            ->field(
                "a.id,b.name,a.dedicatedip,c.id as uid,c.username,c.companyname,a.domain,a.password,a.create_time,a.nextduedate,a.billingcycle,a.firstpaymentamount,a.dcimid,a.port,a.initiative_renew,a.assignedips,
            a.amount,a.domainstatus,a.notes,a.remark,b.pay_type,b.api_type,b.upstream_version,b.upstream_price_type,b.upstream_price_value,b.id as pid,a.promoid,c.sale_id,b.type,b.zjmf_api_id,b.upper_reaches_id,a.upstream_cost",
            )
            ->alias("a")
            ->leftJoin("products b", "a.productid = b.id")
            ->leftJoin("clients c", "a.uid = c.id")
            ->leftJoin("zjmf_finance_api d", "b.zjmf_api_id=d.id")
            ->union(function (\think\db\Query $query) use ($where) {
                $query
                    ->field(
                        "a.id,b.name,a.dedicatedip,c.id as uid,c.username,c.companyname,a.domain,a.password,a.create_time,a.nextduedate,a.billingcycle,a.firstpaymentamount,a.dcimid,a.port,a.initiative_renew,a.assignedips,
            a.amount,a.domainstatus,a.notes,a.remark,b.pay_type,b.api_type,b.upstream_version,b.upstream_price_type,b.upstream_price_value,b.id as pid,a.promoid,c.sale_id,b.type,b.zjmf_api_id,b.upper_reaches_id,a.upstream_cost",
                    ...
                    ->leftJoin("products b", "a.productid = b.id")
                    ->leftJoin("clients c", "a.uid = c.id")
                    ->leftJoin("zjmf_finance_api d", "b.upper_reaches_id=d.id")
                    ->where($where);
            })
            ->where($where)
        ...
        foreach ($hosts as &$v) {
            $pricing = \think\Db::name("pricing")
                ...
                ->where("relid", $v["pid"])
                ->find();
            $v["product_price"] = bcadd(
                $pricing[$v["billingcycle"]],
                $pricing[config("price_type")[$v["billingcycle"]][1]],
            );
            $cart_logic = new \app\common\logic\Cart();
            $rebate_total = 0;
            $config_total = $cart_logic->getProductDefaultConfigPrice(
            ...
            if ($v["api_type"] == "manual") {
                $v["server_name"] = \think\Db::name("zjmf_finance_api")
                    ->where("id", $v["upper_reaches_id"])
                    ->value("name");
            } else {
                $v["server_name"] = \think\Db::name("zjmf_finance_api")->where("id", $v["zjmf_api_id"])->value("name");
            }
    ...
    public function downstreamSummary()
    {
        ...
        if ($api["type"] == "manual") {
            $client = [];
            $client["host_count"] = \think\Db::name("host")
                ->alias("a")
                ->leftJoin("products b", "a.productid=b.id")
                ->where("b.upper_reaches_id", $id)
                ->count();
            $client["active_count"] = \think\Db::name("host")
                ->alias("a")
                ->leftJoin("products b", "a.productid=b.id")
                ->where("b.upper_reaches_id", $id)
    ...
    public function apiLog()
    {
        ...
        $logs = \think\Db::name("api_resource_log")
            ->alias("a")
            ->field("a.id,a.create_time,a.description,a.ip,b.username,a.port")
            ...
            ->select()
            ->toArray();
        $count = \think\Db::name("api_resource_log")
            ->alias("a")
            ->leftJoin("clients b", "a.uid = b.id")
            ->where($where)
            ->count();
        $user_list = \think\Db::name("api_resource_log")
            ->alias("a")
            ->leftJoin("clients b", "a.uid = b.id")
            ->field("a.uid,b.username")
            ->group("a.uid")
            ->select()
            ->toArray();
    ...
    public function postManualHost()
    {
        ...
        if (!empty($exist)) {
            \think\Db::name("upper_manual_info")
                ->where("hid", $param["hid"])
                ->update([
                    "regate" => $param["regate"],
                    "amount" => $param["amount"],
                    "billingcycle" => $param["billingcycle"],
                    "dedicatedip" => $param["dedicatedip"],
                    "assignedips" => $param["assignedips"],
                    "create_time" => $param["create_time"],
                ]);
        } else {
            \think\Db::name("upper_manual_info")->insert([
                "hid" => $param["hid"],
                "regate" => $param["regate"],
                "amount" => $param["amount"],
                "billingcycle" => $param["billingcycle"],
    ...
    public function getRenew()
    {
        ...
        $where = function (\think\db\Query $query) use ($param) {
            ...
            $query
                ->where("d.type", "renew")
                ->where("f.zjmf_api_id|f.upper_reaches_id", ">", 0)
                ->where("a.paid_time", ">=", $start_time)
                ->where("a.paid_time", "<=", $end_time);
        ...
        $rows = \think\Db::name("invoices")
            ->alias("a")
            ->field(
                "a.id,b.username,b.companyname,f.name,e.domain,e.dedicatedip,e.assignedips,e.nextduedate,e.amount,
            a.payment,c.prefix,c.suffix,a.use_credit_limit,a.credit,a.subtotal as sub,a.status,e.upstream_cost as cost",
            )
            ->leftJoin("clients b", "a.uid=b.id")
            ->leftJoin("currencies c", "b.currency=c.id")
            ->leftJoin("invoice_items d", "a.id=d.invoice_id")
            ->leftJoin("host e", "d.rel_id=e.id")
            ->leftJoin("products f", "f.id=e.productid")
            ->leftJoin("zjmf_finance_api g", "f.zjmf_api_id=g.id OR f.upper_reaches_id=g.id")
            ->where($where)
            ->withAttr("payment", function ($value, $data) use ($gateways) {
                if ($data["status"] == "Paid") {
                    if ($data["use_credit_limit"] == 1) {
