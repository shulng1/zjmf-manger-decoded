...
class CancelRequestController extends AdminBaseController
{
    ...
    public function getList()
    {
        ...
        $data = \think\Db::name("cancel_requests")
            ->field(
                "c.id,c.relid,c.type,c.reason,cl.username,h.id as hostid,h.uid,h.domainstatus,h.nextduedate,
                    h.auto_terminate_end_cycle, h.auto_terminate_reason,p.name as productname, 
                    g.name as groupname",
            )
            ->alias("c")
            ->leftJoin("host h", "h.id=c.relid")
            ->leftJoin("clients cl", "cl.id=h.uid")
