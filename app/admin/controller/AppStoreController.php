...
class AppStoreController extends \cmf\controller\BaseController
{
    ...
    {
        $res = commonCurl(
            $this->market_url . "/market/app_version",
            ["request_time" => time(), "license" => configuration("system_license")],
            30,
            "GET",
        );
        if (isset($res["status"]) && $res["status"] == 200) {
            foreach ($res["data"] as $key => $value) {
                ...
                $res["data"][$key]["app_version"] = !empty($value["app_version"])
                    ? str_replace("V", "", $value["app_version"])
                    : "1.0.0";
            }
            $result = $res;
        } else {
    ...
    public function install($id)
    {
        ...
        if ($res["data"]["product"]["app_type"] == "systems" || empty($res["data"]["product"]["app_type"])) {
            ...
            if ($res["data"]["product"]["app_type"] == "templates") {
                ...
                $content = curl_download(
                    $this->market_url . "/download/app_file?id={$id}&jwt=" . cache("market_jwt"),
                    $dir,
                );
                if ($content) {
                    $file = CMF_ROOT . "/public/themes";
                    ...
                    if ($res["status"] == 200) {
                        ...
                        return jsonrule([
                            "status" => 400,
                            "msg" =>
                                "应用文件解压失败,失败code:" . $res["msg"] . ";请到网站目录下解压下载的文件" . $dir,
                        ]);
                    }
                } else {
                ...
                $dir =
                    "/public/plugins/" .
                    $res["data"]["product"]["app_type"] .
                    "/" .
                    $res["data"]["product"]["uuid"] .
                    ".zip";
                $content = curl_download(
                    $this->market_url . "/download/app_file?id={$id}&jwt=" . cache("market_jwt"),
                    $dir,
    ...
    private function marketRequest($app = "", $data = [], $timeout = 30, $method = "POST")
    {
        ...
        $result = commonCurl($this->market_url . "/" . $app, $data, $timeout, $method, [
            "Authorization: JWT " . cache("market_jwt"),
        ]);
        if ($result["status"] == 405) {
            $username = cache("market_username");
            $password = cache("market_password");
            if (cmf_check_mobile($username)) {
                $res = commonCurl($this->market_url . "/market/login_pass_phone", [
                    "phone_code" => "+86",
                    "phone" => $username,
                    "password" => $password,
                ...
                $res = commonCurl($this->market_url . "/market/login_pass_email", [
                    "email" => $username,
                    "password" => $password,
                    "license" => configuration("system_license"),
                    "request_time" => time(),
