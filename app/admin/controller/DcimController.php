...
class DcimController extends AdminBaseController
{
    ...
    public function addServer()
    {
        ...
        $auth = [
            ...
            "rescue" => "off",
            "crack_pass" => "off",
            "enable_ip_custom" => "off",
        ];
        $is_certifi = $this->is_certifi;
        \think\Db::startTrans();
        try {
            $group = \think\Db::name("server_groups")->insertGetId([
                "name" => $params["name"],
                "system_type" => "dcim",
            ]);
            $insert["gid"] = $group;
            ...
            \think\Db::name("dcim_servers")->insert([
                "serverid" => $id,
                "auth" => json_encode($auth),
                "area" => "",
                "bill_type" => "month",
                "flow_remind" => "",
                "is_certifi" => json_encode($is_certifi),
            ]);
            \think\Db::commit();
    ...
    public function editServer()
    {
        ...
        $server_info = \think\Db::name("servers")
            ->alias("a")
            ->field(
                "a.id,a.gid,a.name,a.hostname,a.username,a.password,a.port,a.secure,a.disabled,b.reinstall_times,b.buy_times,b.reinstall_price,b.auth,b.bill_type,b.area,b.is_certifi",
            )
        ...
        if ($close_ip_custom) {
            $change_host = \think\Db::name("host")
                ->field("id,dedicatedip,assignedips")
                ->where("serverid", $id)
                ->select()
                ->toArray();
        }
        ...
        \think\Db::startTrans();
        try {
            \think\Db::name("server_groups")
                ->where("id", $server_info["gid"])
            ...
            if (!empty($change_host)) {
                foreach ($change_host as $v) {
                    ...
                    \think\Db::name("host")
                        ->where("id", $v["id"])
                        ->update(["assignedips" => $v["assignedips"]]);
                }
            }
            \think\Db::commit();
            if (empty($dec)) {
    ...
    public function serverDetail($id)
    {
        $data = \think\Db::name("servers")
            ->alias("a")
            ->field(
                "a.id,a.name,a.hostname,a.username,a.password,a.port,a.secure,a.disabled,a.accesshash,b.reinstall_times,b.buy_times,b.reinstall_price,b.auth,b.area,b.bill_type,b.flow_remind,b.ip_customid,b.is_certifi",
            )
            ->leftJoin("dcim_servers b", "a.id=b.serverid")
            ->where("a.server_type", "dcim")
            ->where("a.id", $id)
    ...
    public function delServer()
    {
        ...
        $server_group = \think\Db::name("host")
            ->alias("a")
            ->leftJoin("servers b", "a.serverid = b.id")
            ->leftJoin("server_groups c", "c.id = b.gid")
            ->where("c.system_type", "dcim")
            ->where("b.id", $id)
            ->find();
    ...
    public function serverList()
    {
        ...
        $count = \think\Db::name("servers")
            ->alias("a")
            ->where("a.name LIKE '%{$search}%' OR a.hostname LIKE '%{$search}%'")
            ->where("a.server_type", "dcim")
            ->count();
        $data = \think\Db::name("servers")
            ->alias("a")
            ->field(
                "a.id,a.name,a.hostname,count(DISTINCT b.id) server_num,c.api_status,count(DISTINCT d.id) product_num",
            )
            ->leftJoin("host b", "b.serverid=a.id AND (b.domainstatus=\"Active\" OR b.domainstatus=\"Suspended\")")
            ->leftJoin("dcim_servers c", "c.serverid=a.id")
            ->leftJoin("products d", "a.gid=d.server_group")
            ->where("a.name LIKE '%{$search}%' OR a.hostname LIKE '%{$search}%'")
            ->where("a.server_type", "dcim")
            ->group("a.id")
            ->order($orderby, $sort)
    ...
    {
        $server_info = \think\Db::name("servers")
            ->field("id,hostname,username,password,secure,port")
            ->where("server_type", "dcim")
            ->select()
            ->toArray();
        if (empty($server_info)) {
        ...
        foreach ($server_info as $v) {
            ...
            $data[$v["id"]] = [
                "url" => $url . "/index.php?m=api&a=getHouse",
    ...
    public function listBuyRecord()
    {
        ...
        $count = \think\Db::name("dcim_buy_record")
            ...
            ->whereLike("a.name|b.username|b.phonenumber|b.email", "%{$search}%");
        if ($getUserCtol->user["id"] != 1 && $getUserCtol->user["is_sale"]) {
            $count->whereIn("b.id", $getUserCtol->str);
        }
        $count = $count->count();
        $data = \think\Db::name("dcim_buy_record")
            ->alias("a")
            ->field(
                "a.id,a.uid,a.name,a.price,a.status,a.create_time,a.pay_time,b.username,c.status as invoice_status,c.payment",
            )
            ->leftJoin("clients b", "a.uid=b.id")
            ->leftJoin("invoices c", "a.invoiceid = c.id")
            ->whereLike("a.name|b.username|b.phonenumber|b.email", "%{$search}%")
    ...
    public function listFlowPacket()
    {
        ...
        $count = \think\Db::name("dcim_flow_packet")
            ->whereLike("name", "%{$search}%")
            ->count();
        $max_page = ceil($count / $page);
        $data = \think\Db::name("dcim_flow_packet")
            ->field("id,name,capacity,price,status,sale_times,stock,create_time")
            ->whereLike("name", "%{$search}%")
            ->order($orderby, $sort)
            ->page($page)
            ->limit($limit)
    ...
    {
        $data = \think\Db::name("dcim_flow_packet")
            ->field("id,name,capacity,price,allow_products,status,create_time,sale_times,stock")
            ->where("id", $id)
            ->find();
        if (empty($data)) {
            return jsonrule(["status" => 400, "msg" => lang("ID_ERROR")]);
    ...
    public function reinstall()
    {
        ...
        $data = [
            "rootpass" => $params["password"],
            "action" => $params["action"],
            "mos" => $params["os"],
            "mcon" => $params["mcon"],
            "port" => $params["port"],
            "disk" => $params["disk"] ?? 0,
            "check_disk_size" => $params["check_disk_size"] ?? 0,
            "part_type" => $params["part_type"] ?? 0,
        ];
        $dcim = new \app\common\logic\Dcim();
        $dcim->is_admin = true;
    ...
    public function assignServer()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.id,a.dcimid,b.api_type,b.zjmf_api_id")
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("dcim_servers c", "a.serverid=c.serverid")
            ->where("a.id", $id)
            ->where("b.type", "dcim")
