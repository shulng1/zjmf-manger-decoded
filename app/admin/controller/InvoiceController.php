...
class InvoiceController extends GetUserController
{
    ...
    public function searchPage()
    {
        ...
        return jsonrule([
            ...
            "gateway" => array_merge(gateway_list1(), $other_pay),
    ...
    public function index()
    {
        ...
        $fun = function (\think\db\Query $query) use ($params) {
            ...
            if (isset($params["create_time"])) {
                $create_time = is_string($params["create_time"])
                    ? strtotime($params["create_time"])
                    : $params["create_time"];
            ...
            if (!empty($params["lineitem_desc"])) {
                $invoice_id = \think\Db::name("invoice_items")
                    ->whereLike("description", "%" . $params["lineitem_desc"] . "%")
                    ->column("invoice_id");
                if (empty($invoice_id)) {
                    $invoice_id = [0];
                }
    ...
    private function getInvoice($fun, $where, $page, $limit, $order, $sort)
    {
        ...
        $rows = \think\Db::name("invoices")
            ->alias("i")
            ->field(
                "c.companyname,c.username,c.sale_id,i.id,i.uid,i.invoice_num,i.paid_time,
            i.create_time,i.due_time,i.subtotal,i.subtotal as sub,i.total,cu.prefix,cu.suffix,
            i.status,i.payment,i.credit,i.type,i.use_credit_limit",
            )
            ->leftjoin("clients c", "c.id=i.uid")
            ->leftJoin("currencies cu", "cu.id = c.currency")
            ->group("i.id")
    ...
    public function paid()
    {
        ...
        foreach ($ids as $id) {
            $invoice = \think\Db::name("invoices")
                ->field("uid,status,payment,type,subtotal,total,credit,notes")
                ->where("id", $id)
                ->find();
            ...
            $account_data = [
                "uid" => $uid,
                "currency" => \think\Db::name("currencies")->where("id", $currency)->value("code"),
                "gateway" => $invoice["payment"],
                "create_time" => time(),
                "pay_time" => time(),
                "description" => "人工入账",
                "amount_in" => $client_surplus,
                "fees" => 0,
                "amount_out" => 0,
                "invoice_id" => $id,
            ];
            ...
            try {
                ...
                if ($virtual_credit > 0) {
                    $virtual = \think\Db::name("clients")
                        ->where("id", $invoice["uid"])
                        ->where("credit", ">=", $virtual_credit)
                        ->setDec("credit", $virtual_credit);
                    if (empty($virtual)) {
                        \think\Db::name("accounts")
                            ->where("id", $acc)
                            ->update(["amount_in" => $invoice["subtotal"]]);
                        \think\Db::name("invoices")
                            ->where("id", $id)
                            ->update(["total" => $invoice["subtotal"]]);
                    } else {
                        credit_log([
                            "uid" => $invoice["uid"],
                            "desc" => "Credit Applied to Invoice #" . $id,
                            "amount" => $virtual_credit,
                            "relid" => $id,
                            "notes" => $invoice["notes"],
                        ]);
                    }
                ...
                active_log_final(
                    sprintf($this->lang["Invoice_admin_addPay"], $invoice["uid"], $id),
                    $invoice["uid"],
                    6,
                    $id,
                );
    ...
    public function duplicate()
    {
        ...
        try {
            ...
            foreach ($invoices as $invoice) {
                ...
                $invoice_items = \think\Db::name("invoice_items")
                ...
                active_log_final(
                    sprintf($this->lang["Invoice_admin_duplicate"], $invoice["uid"], $invoice["id"]),
                    $invoice["uid"],
                    6,
                    $invoice["id"],
                );
    ...
    public function summary()
    {
        ...
        $invoices = \think\Db::name("invoices")
            ->alias("a")
            ->field("b.credit as user_credit,b.is_open_credit_limit,b.credit_limit_balance,a.notes,a.uid")
            ->field(
                "a.subtotal as sub,a.use_credit_limit,a.id,a.uid,a.type,b.username,a.create_time,a.due_time,a.paid_time,a.subtotal,a.total,a.status,a.credit,a.payment,a.last_capture_attempt",
            )
            ...
            ->withAttr("payment", function ($value, $data) use ($gateways, $id) {
                ...
                if (
                    $data["use_credit_limit"] == 1 &&
                    $this->getInterfacePay($id, $data["uid"]) == 0 &&
                    $data["credit"] == 0
                ) {
                    return "信用额支付";
                }
                $gateway = \think\Db::name("accounts")
                    ->where("invoice_id", $id)
                    ->where("refund", 0)
                    ->order("id", "desc")
                    ->value("gateway");
        ...
        if ($invoices["status"] != "Paid" || $invoices["status"] == "Refunded") {
            ...
            $exists_pay = [
                "credit" => [
                    "is_pay" => ($invoices["use_credit_limit"] == 1 ? 0 : $invoices["user_credit"] > 0) ? 1 : 0,
                    "money" => $invoices["user_credit"] > 0 ? $invoices["user_credit"] : 0,
                ],
                "credit_limit" => [
                    "is_pay" =>
                        $invoices["is_open_credit_limit"] && $invoices["sub"] <= $credit_limit && $virtual_credit == 0
                            ? $credit_limit_is_pay
                            : 0,
                    "money" => $credit_limit ?: 0,
                ],
            ];
        }
        ...
        $invoice_items = \think\Db::name("invoice_items")
            ->field("id,description,amount,taxed,rel_id,type")
            ->where("invoice_id", $id)
            ->where("delete_time", 0)
            ->select()
            ->toArray();
        ...
        foreach ($invoice_items as &$m) {
            ...
            if ($m["type"] == "host") {
                $host_data = \think\Db::name("host")
                    ->field("domain,dedicatedip,billingcycle")
                    ->where("id", $m["rel_id"])
                    ->find();
                $new_des = explode("\n", $m["description"]);
                if (count(explode("(", $new_des[0])) <= 2) {
                    $new_des[0] =
                        ...
                        "(" .
                        $host_data["domain"] .
                        "),IP({$host_data["dedicatedip"]}),购买时长：" .
                        $billing_cycle[$host_data["billingcycle"]] .
                        "(" .
                        explode("(", $new_des[0])[1];
                    $new_des = implode("\n", $new_des);
        ...
        $accounts = \think\Db::name("accounts")
            ->alias("a")
            ->field(
                "a.id,a.pay_time,a.gateway,a.trans_id,a.amount_in,a.amount_in as _amount_in,a.amount_out,a.fees,c.prefix,c.suffix,a.invoice_id",
            )
            ->leftJoin("currencies c", "a.currency = c.code")
            ->withAttr("amount_in", function ($value, $data) use ($currency) {
                if ($value > 0) {
                    if (!$data["prefix"]) {
            ...
            ->select()
            ->toArray();
        $refund_accounts = \think\Db::name("accounts")
            ->field("id,currency,pay_time as create_time,trans_id,amount_in")
            ->where("invoice_id", $id)
            ->where("delete_time", 0)
            ->where("refund", 0)
            ->where("gateway", "<>", "")
        ...
        $credit_log_uses = \think\Db::name("credit")
            ->field("id,create_time as pay_time,amount,amount as amount_in,description as gateway,relid")
            ->withAttr("amount_in", function ($value) use ($currency) {
                return $currency["prefix"] . $value . $currency["suffix"];
            })
            ...
            ->where("description", "like", "%Credit Applied to Invoice #{$id}")
            ->whereOr("description", "like", "%Credit Removed from Invoice #{$id}")
            ->whereOr("description", "like", "%Credit Applied to Renew Invoice #{$id}")
            ->whereOr(function (\think\db\Query $query) use ($id) {
        ...
        if ($invoices["use_credit_limit"] && $invoices["status"] == "Paid") {
            array_push($accounts, [
                "pay_time" => $invoices["paid_time"],
                "gateway" => "信用额支付",
                "trans_id" => "",
                "amount_in" => $invoices["subtotal"],
                "type" => "credit_limit",
            ]);
        }
        foreach ($accounts as $key => $val) {
            ...
            if ($val["type"] == "credit") {
                ...
                $accounts[$key]["is_refund"] =
                    $val["id"] && $val["diff_amount"] > 0 && in_array($val["id"], $refund_accounts) ? 1 : 0;
            }
        ...
        foreach ($invoice_items as &$invoice_item) {
            if ($invoice_item["type"] == "host") {
                ...
                $customfields = \think\Db::name("customfields")
                    ->where([["relid", "=", $host["productid"], ["type", "=", "product"]]])
                    ->select()
                    ->toArray();
                $tmp_arr = [];
                if (!empty($customfields[0])) {
                    foreach ($customfields as $v) {
                        if ($v["showorder"]) {
                            $customfieldsvalues = \think\Db::name("customfieldsvalues")
                                ->where([["fieldid", "=", $v["id"]], ["relid", "=", $host["id"]]])
                                ->value("value");
    ...
    private function getInterfacePay($id, $uid)
    {
        ...
        $accounts = \think\Db::name("accounts")
            ->alias("a")
            ->field("a.id,a.pay_time,a.gateway,a.trans_id,a.amount_in,a.amount_out,a.fees")
            ->withAttr("gateway", function ($value) use ($gateways) {
                foreach ($gateways as $v) {
                    if ($value == $v["name"]) {
                        return $v["title"];
    ...
    public function addPay()
    {
        ...
        $time = time();
        $invoice = \think\Db::name("invoices")
            ->field("uid,type,subtotal,total,credit,use_credit_limit,status")
            ->where("id", $id)
            ->where("delete_time", 0)
            ->find();
        if (empty($invoice)) {
        ...
        try {
            ...
            $invoice_data = [
                "paid_time" => $params["pay_time"] ?? $time,
                "status" => "Unpaid",
                "update_time" => time(),
            ];
            if ($invoice_type == "recharge") {
                ...
                if ($subtotal <= $sum) {
                    ...
                    \think\Db::name("accounts")
                        ->where("invoice_id", $id)
                        ->where("refund_credit", 0)
                        ->where("delete_time", 0)
                        ->update(["refund_credit" => 1]);
                }
            }
            \think\Db::name("invoices")->where("id", $id)->where("delete_time", 0)->update($invoice_data);
            if ($virtual_credit > 0) {
                ...
                if (empty($virtual)) {
                    active_log_final(
                        sprintf($this->lang["Order_admin_clients_updatecredit_fail"], $invoice["uid"]),
                        $invoice["uid"],
                        6,
                        $id,
                    );
                ...
                \think\Db::name("invoices")
                    ->where("id", $id)
                    ->update(["credit" => $invoice["subtotal"] - $invoice["total"]]);
            }
            ...
            active_log_final(
                sprintf($this->lang["Invoice_admin_addPay"], $invoice["uid"], $id) . $text,
                $invoice["uid"],
                6,
                $id,
            );
        ...
        if ($update && !empty($params["email"])) {
            ...
            if ($hostid["rel_id"]) {
                ...
                $client = check_type_is_use(
                    $message_template_type[strtolower("Invoice_Payment_Confirmation")],
                    $invoice["uid"],
                    $sms,
                );
                if ($client) {
                    $host = \think\Db::name("host")
                        ->alias("a")
                        ->field("a.id,a.orderid,o.amount")
                        ->join("orders o", "a.orderid=o.id")
                        ->where("a.id", $hostid["rel_id"])
                        ->find();
    ...
    public function optionPage()
    {
        ...
        $invoice = \think\Db::name("invoices")
            ->field("create_time,due_time,invoice_num,taxrate,taxrate2,status,payment,paid_time")
            ->where("id", $id)
            ->find();
        return jsonrule([
            "status" => 200,
        ...
        try {
            return \think\Db::transaction(function () {
                ...
                $option_data = [
                    ...
                    "payment" => $params["gateway"],
                    "taxrate" => $params["taxrate"],
                    "taxrate2" => $params["taxrate2"],
                    "status" => $params["status"],
                    "notes" => $params["notes"],
                    "paid_time" => $paid_time,
                    "update_time" => time(),
                ...
                active_log_final(
                    sprintf($this->lang["Invoice_admin_option"], $invoce["uid"], $id) . $text,
                    $invoce["uid"],
                    6,
                    $id,
                );
    ...
    public function addPayInvoicePage()
    {
        ...
        return jsonrule([
            ...
            "surplus" => $surplus,
            "invoice_credit" => $prefix . bcsub($invoices["subtotal"], $invoices["total"], 2) . $suffix,
        ]);
    }
    /**
     * @title 添加付款金额到账单
    ...
    public function addPayInvoice()
    {
        ...
        $invoices = \think\Db::name("invoices")
            ->field("uid,credit,subtotal,total,status")
            ->where("id", $id)
            ->where("delete_time", 0)
            ->find();
        ...
        try {
            ...
            \think\Db::commit();
            active_log_final(
                sprintf($this->lang["Invoice_admin_addPayInvoice"], $invoices["uid"], $id),
                $invoices["uid"],
                6,
                $id,
            );
        ...
        try {
            ...
            try {
                ...
                if ($invoices["total"] > 0) {
                    if ($this->getUseCreditLimit($uid) < $invoices["total"]) {
                        active_log_final(
                            sprintf($this->lang["Order_admin_clients_updatecreditlimit_fail"], $uid),
                            $uid,
                            6,
                            $invoiceid,
                        );
            ...
            $result["status"] = 200;
            $result["msg"] = "支付成功";
            $result["data"]["hostid"] = \think\Db::name("invoice_items")
    ...
    private function getUseCreditLimit($uid)
    {
        ...
        $amount_to_be_settled = \think\Db::name("invoices")
            ->where("status", "Paid")
        ...
        return bcsub($credit_limit, $credit_limit_used, 2) > 0
            ? bcsub($credit_limit, $credit_limit_used, 2)
            : number_format(0, 2);
    }
    /**
    ...
    public function deletePayInvoice()
    {
        ...
        $invoices = \think\Db::name("invoices")
            ->field("uid,credit,subtotal,total")
            ->where("id", $id)
            ->where("delete_time", 0)
            ->find();
        $credit = $invoices["credit"];
        ...
        try {
            ...
            credit_log([
                "uid" => $invoices["uid"],
                "desc" => "Credit Removed from Invoice #" . $id,
                "amount" => -$amount,
                "relid" => $id,
            ]);
            \think\Db::commit();
            active_log_final(
                sprintf($this->lang["Invoice_admin_deletePayInvoice"], $invoices["uid"], $id),
                $invoices["uid"],
                6,
                $id,
            );
    ...
    public function refund()
    {
        ...
        if ($type == "addascredit") {
            $data = [
                ...
                "create_time" => time(),
                "pay_time" => time(),
                "description" => "产品退款 Transaction ID " . $accounts["trans_id"],
                "amount_out" => $amount,
            ...
            $account_credit = [
                ...
                "create_time" => time(),
                "pay_time" => time(),
                "description" => "退款至余额入账 Invoice ID {$invoice_id}",
                "amount_in" => $amount,
            ];
            ...
            credit_log([
                "uid" => $uid,
                "desc" => "Credit from Refund of Invoice ID {$invoice_id}",
                "amount" => $amount,
            ]);
        } elseif ($type == "only") {
            $data = [
                ...
                "description" => "产品退款 Transaction ID " . $accounts["trans_id"],
                "amount_out" => $amount,
                "trans_id" => isset($params["trans_id"]) ? mb_substr($params["trans_id"], 0, 20) : "",
                "invoice_id" => $invoice_id,
                "refund" => $id,
            ];
            $res = \think\Db::name("accounts")->insertGetId($data);
        ...
        $subs = \think\Db::name("invoice_items")
            ->field("rel_id,amount")
        ...
        $host = \think\Db::name("orders")
            ->alias("a")
            ->field("b.id,a.uid,a.id as orderid,a.amount")
            ->leftJoin("host b", "a.id = b.orderid")
        ...
        if ($client) {
            ...
            $sms->sendSms(
                $message_template_type[strtolower("order_refund")],
                $client["phone_code"] . $client["phonenumber"],
                $params,
                false,
                $host["uid"],
        ...
        $text = " 交易明细处";
        active_log_final(
            sprintf($this->lang["Invoice_admin_refund"], $uid, $invoice_id, $amount) . $text,
            $uid,
            6,
            $invoice_id,
        );
    ...
    public function notes()
    {
        ...
        if (!empty($params["notes"]) && $params["notes"] != $invo["notes"]) {
            active_log_final(
                sprintf($this->lang["Invoice_admin_notes"], $invo["uid"], $id, $invo["notes"], $params["notes"]),
                $invo["uid"],
                6,
                $id,
            );
        ...
        \think\Db::name("invoices")
            ->where("id", $id)
            ->update(["notes" => $params["notes"], "update_time" => time()]);
        $hook_data = ["invoiceid" => $id, "content" => html_entity_decode($params["notes"], ENT_QUOTES)];
        hook("invoice_notes", $hook_data);
        return jsonrule(["status" => 200, "msg" => lang("UPDATE SUCCESS")]);
    }
    ...
    public function invoceEmail()
    {
        ...
        $eamil->is_admin = true;
        $invoice = \think\Db::name("invoice_items")
            ->where([["invoice_id", "=", $invoice_id]])
            ->find();
        ...
        if ($res) {
            active_log_final(
                "账单 Invoice ID:{$invoice_id}发送邮件 - " . $name . " - 发送成功",
                $invoice["uid"],
                6,
                $invoice_id,
            );
            ...
            active_log_final(
                "账单 Invoice ID:{$invoice_id}发送邮件 - " . $name . " - 发送失败",
                $invoice["uid"],
                6,
                $invoice_id,
            );
    ...
    public function editItem()
    {
        ...
        if (
            (isset($params["add_description"]) && !empty($params["add_description"])) ||
            (isset($params["add_amount"]) && !empty($params["add_amount"]))
        ) {
        ...
        \think\Db::name("invoices")
            ->where("id", $id)
            ->update(["subtotal" => $subtotal, "total" => $credit < 0 ? $subtotal : $subtotal - $credit]);
        if ($split) {
    ...
    public function deleteItems()
    {
        ...
        try {
            ...
            $subtotal = \think\Db::name("invoice_items")
                ->where("invoice_id", $invoice_id)
                ->where("delete_time", 0)
                ->sum("amount");
            \think\Db::name("invoices")
                ...
                ->update([
                    "subtotal" => $subtotal,
                    "total" => $invoice_credit < 0 ? $subtotal : bcsub($subtotal, $invoice_credit, 2),
                ]);
            active_log_final(
                sprintf($this->lang["Invoice_admin_deleteItems"], $id, $uid, $invoice_id),
                $uid,
                6,
                $invoice_id,
            );
    ...
    private function getInvoiceIncome()
    {
        ...
        foreach ($currency as $v) {
            $paid_total = db("invoices")
                ->alias("i")
                ->leftjoin("clients c", "i.uid=c.id")
                ->where("c.currency", $v["id"])
                ->where("i.delete_time", 0)
                ->where("i.status", "Paid")
                ->sum("subtotal");
            $arr[$v["code"]]["Paid"] = $v["prefix"] . bcsub($paid_total, 0, 2) . $v["suffix"];
            $unpaid_total = \think\Db::name("invoices")
                ->alias("i")
                ->leftjoin("clients c", "i.uid=c.id")
                ->whereBetweenTime("i.create_time", $start_time, $end_time)
                ->where("c.currency", $v["id"])
                ->where("i.delete_time", 0)
            ...
            $overdue_total = db("invoices")
                ->alias("i")
                ->leftjoin("clients c", "i.uid=c.id")
                ->where("i.due_time", "<=", $start_time)
                ->where("c.currency", $v["id"])
                ->where("i.delete_time", 0)
                ->where("i.status", "Unpaid")
                ->sum("subtotal");
            $accounts_overdue = \think\Db::name("accounts")
                ->alias("a")
                ->leftJoin("invoices b", "a.invoice_id = b.id")
                ->leftjoin("clients c", "a.uid=c.id")
                ->where("a.pay_time", "<=", $start_time)
                ->where("b.due_time", "<=", $start_time)
                ->where("c.currency", $v["id"])
                ->where("a.delete_time", 0)
                ->where("b.delete_time", 0)
                ->where("b.status", "Unpaid")
            ...
            if ($v["type"] == "host") {
                if (get_product_condition($v["rel_id"]) == "payment") {
                    ...
                    if ($result["status"] == 200) {
                        ...
                        $data_i[
                            "description"
                        ] = " 账单付款后 - 开通 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                        if ($is_zjmf) {
                ...
                credit_log([
                    "uid" => $invoiceInfo["uid"],
                    "desc" => sprintf($lang["recharge_ok"], $invoiceId),
        ...
        if (get_product_condition($id) == "payment") {
            ...
            if ($res == "Suspended") {
                ...
                if ($result["status"] == 200) {
                    ...
                    $data_i[
                        "description"
                    ] = " 成功续费产品 - 解除暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                    if ($is_zjmf) {
                ...
                $data = [
                    "nextduedate" => $nextTime,
                    "nextinvoicedate" => $nextTime,
                    "update_time" => time(),
                    "id" => $res["id"],
                ];
                $up = db("host")->update($data);
                active_log_final(
                    sprintf($this->lang["Invoice_admin_renewHandle"], $res["uid"], $id),
                    $res["uid"],
                    6,
                    $id,
                );
    ...
    public function getCombineInvoices()
    {
        ...
        foreach ($invoices as &$v) {
            $total += $v["subtotal"];
            $items = \think\Db::name("invoice_items")
                ->field("id,type,description,amount,type as type_zh")
                ->where("invoice_id", $v["id"])
                ->where("uid", $uid)
                ->where("delete_time", 0)
                ->withAttr("type_zh", function ($value, $data) use ($items) {
    ...
    public function combineInvoices()
    {
        ...
        try {
            ...
            foreach ($invoices as $vv) {
                $insert_all[] = [
                    ...
                    "type" => "combine",
                    "rel_id" => $vv["id"],
                    "description" => "合并账单#Invoice ID " . $vv["id"],
                    "amount" => $vv["subtotal"],
                    "taxed" => 0,
                    "due_time" => 0,
                    "payment" => $vv["payment"],
        ...
        if ($subtotal == 0) {
            \think\Db::name("invoices")
                ->where("id", $invoice_id)
                ->update(["status" => "Paid", "paid_time" => time(), "update_time" => time()]);
            $invoice_logic = new \app\common\logic\Invoices();
    ...
    public function renew()
    {
        ...
        $price_total = \think\Db::name("invoices")
            ->alias("i")
            ->leftjoin("clients c", "i.uid=c.id")
            ->leftjoin("currencies cu", "cu.id = c.currency")
            ->leftJoin("invoice_items ii", "ii.invoice_id = i.id")
            ->leftJoin("host h", "ii.rel_id = h.id")
            ->leftJoin("products p", "h.productid = p.id")
            ->field("ii.amount")
            ->where($fun)
            ->where("i.paid_time", ">=", $start_time)
            ->where("i.paid_time", "<=", $end_time)
            ->where("ii.type", "renew")
            ->where("i.status", "Paid");
        if ($this->user["id"] != 1 && $this->user["is_sale"]) {
        ...
        $rows = \think\Db::name("invoices")
            ...
            ->leftJoin("products p", "h.productid = p.id")
            ->field(
                "i.type,i.subtotal,c.username,c.companyname,i.id,i.uid,i.status,i.subtotal as sub,i.credit,i.use_credit_limit,i.paid_time,ii.amount,ii.amount as am,i.payment,cu.prefix,cu.suffix,i.delete_time,h.id as hostid,p.name,h.domain,h.dedicatedip,h.productid",
            )
            ->withAttr("payment", function ($value, $data) use ($gateways) {
                if ($data["status"] == "Paid") {
                    if ($data["use_credit_limit"] == 1) {
    ...
    {
        $params = $this->request->param();
        $list = \think\Db::name("activity_log")
            ->field("create_time,id,ipaddr,description as new_desc,uid,user,port")
            ->where("type", 6)
            ->where("uid", $params["uid"])
            ->where("type_data_id", $params["invoice_id"])
