...
class UserController extends AdminBaseController
{
    ...
    public function adminList()
    {
        ...
        $data = \think\Db::name("user")
            ...
            ->field(
                "a.id,a.user_login,a.user_nickname,a.user_email,a.create_time,a.user_status,a.last_login_time,a.last_login_ip,c.name as role,a.is_sale,a.sale_is_use",
            )
            ->where("user_nickname LIKE '%{$search}%' OR user_login LIKE '%{$search}%'")
            ->page($page)
            ->limit($limit)
    ...
    public function create()
    {
        if ($this->request->isPost()) {
            $params = $this->request->param();
            $rule = [
                "user_login" => "require|max:15",
                "user_nickname" => "require|max:15",
                "user_pass" => "require|alphaDash|max:32|min:8",
            ...
            $msg = [
                ...
                "user_pass.max" => "密码长度为8-32位!",
                "user_pass.min" => "密码长度为8-32位!",
                "role_id.require" => "没有指定角色",
            ];
            ...
            $insert["language"] = array_key_exists($params["language"], get_language_list())
                ? $params["language"]
                : "zh-cn";
    ...
    public function updatePage()
    {
        ...
        $user = \think\Db::name("user")
            ->field(
                "id,user_login,user_status,user_login,user_pass,user_nickname,user_email,signature,language,sale_is_use,is_sale,only_mine,is_receive",
            )
            ->where("id", $id)
            ->find();
    ...
    public function update()
    {
        if ($this->request->isPost()) {
            ...
            $exist1 = \think\Db::name("user")
                ->where("id", "<>", $uid)
                ->where("user_login", $params["user_login"])
                ->find();
            if (!empty($exist1)) {
                return jsonrule(["status" => 406, "msg" => "用户名已存在"]);
            }
    ...
    public function editSelfInfoPage()
    {
        ...
        $user = \think\Db::name("user")
            ->field("id,user_login,user_nickname,user_email,language")
            ->where("id", $id)
            ->find();
        $lang = get_language_list();
	...
	*/
    public function editSelfInfo()
    {
        if ($this->request->isPost()) {
            ...
            $rule = [
                "user_login" => "max:15",
                "user_nickname" => "max:15",
                "user_email" => "require|email",
                "language" => "require",
                "user_pass" => "max:20",
                "re_user_pass" => "max:20|confirm:user_pass",
            ];
            $msg = [
