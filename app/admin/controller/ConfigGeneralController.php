...
class ConfigGeneralController extends AdminBaseController
{
    ...
    public function getNewLoginPage()
    {
        ...
        return jsonrule([
            ...
            "data" => ["templates" => $templates, "allow_new_login_template" => $allow],
        ]);
    }
    ...
    public function emailIndexPost()
    {
        if ($this->request->isPost()) {
            ...
            updateConfiguration(
                "email_smtpsecure",
                isset($param["smtpsecure"]) ? strtolower($param["smtpsecure"]) : "ssl",
            );
    ...
    public function supportIndex()
    {
        ...
        $result["data"] = [
            "nologin_send_ticket" => $config["nologin_send_ticket"] ?: 0,
            "ticket_reply_order" => $config["ticket_reply_order"] ?: "asc",
            "evaluate_ticket" => $config["evaluate_ticket"] ?: 0,
            "product_download" => $config["product_download"] ?: 0,
        ];
    ...
    public function postGeneral()
    {
        if ($this->request->isPost()) {
            ...
            if ($param["main_tenance_mode_url"] != $company_name) {
                $dec .=
                    "维护模式重定向到的链接由“" . $company_name . "”改为“" . $param["main_tenance_mode_url"] . "”，";
            }
            ...
            if ($param["cart_product_description"] != $cart_product_description) {
                $dec .=
                    ...
                    $cart_product_description .
                    "”改为“" .
                    $param["cart_product_description"] .
                    "”，";
            }
            updateConfiguration(
                "cart_product_description",
                $param["cart_product_description"] ? trim($param["cart_product_description"]) : "",
            );
            updateConfiguration(
                "clientarea_default_themes",
                $param["clientarea_default_themes"] ? trim($param["clientarea_default_themes"]) : "default",
            );
    ...
    public function getAffiliate()
    {
        ...
        $config_files = [
            "affiliate_enabled",
            "affiliate_bonusde_posit",
            "affiliate_bates",
            "affiliate_type",
            "affiliate_cookie",
            "affiliate_withdraw",
            "affiliate_is_authentication",
            "affiliate_delay_commission",
            "affiliate_is_reorder",
            "affiliate_reorder",
            "affiliate_reorder_type",
            "affiliate_is_renew",
            "affiliate_renew",
            "affiliate_renew_type",
            "affiliate_url",
            "affiliate_invited",
            "affiliate_invited_money",
            "affiliate_invited_type",
        ];
            ...
            if ($param["affiliate_enabled"] == 1) {
                ...
                if ($param["affiliate_bonusde_posit"] != null) {
                    ...
                    if (configuration("affiliate_bonusde_posit") != $param["affiliate_bonusde_posit"]) {
                        $desc .=
                            "推介计划激活赠送金额由“" .
                            configuration("affiliate_bonusde_posit") .
                            "”改为“" .
                            $param["affiliate_bonusde_posit"] .
                            "”，";
                    }
                ...
                if ($param["affiliate_invited"] == 1) {
                    if ($param["affiliate_invited_money"] != null) {
                        ...
                        if (configuration("affiliate_invited_money") != $param["affiliate_invited_money"]) {
                            $desc .=
                                "推介计划应邀返利金额由“" .
                                configuration("affiliate_invited_money") .
                                "”改为“" .
                                $param["affiliate_invited_money"] .
                                "”，";
                        }
                ...
                if ($param["affiliate_cookie"] != null) {
                    if (configuration("affiliate_cookie") != $param["affiliate_cookie"]) {
                        $desc .=
                            "推荐链接cookie有效期由“" .
                            configuration("affiliate_cookie") .
                            "”改为“" .
                            $param["affiliate_cookie"] .
                            "”，";
                    }
                ...
                if ($param["affiliate_withdraw"] != null) {
                    ...
                    if (configuration("affiliate_withdraw") != $param["affiliate_withdraw"]) {
                        $desc .=
                            "提现最低金额由“" .
                            configuration("affiliate_withdraw") .
                            "”改为“" .
                            $param["affiliate_withdraw"] .
                            "”，";
                    }
                ...
                if ($param["affiliate_delay_commission"] != null) {
                    if (configuration("affiliate_delay_commission") != $param["affiliate_delay_commission"]) {
                        $desc .=
                            "延迟订单支付的天数由“" .
                            configuration("affiliate_delay_commission") .
                            "”改为“" .
                            $param["affiliate_delay_commission"] .
                            "”，";
                    }
    ...
    public function ladderList()
    {
        ...
        $list = \think\Db::name("affiliate_ladder")
            ->order($order, $sort)
            ->page($page)
            ->limit($limit)
            ->select()
    ...
    public function getSafe()
    {
        ...
        $returndata["config_value"] = $config_value;
        $returndata["pass_strength_list"] = [
            "none" => "禁用",
            "alpha_num" => "字母+数字",
            "catipal_alpha_num" => "大小写字母+数字",
            "alpha_num_special" => "字母+数字+特殊字符",
        ];
        return jsonrule(["status" => 200, "data" => $returndata]);
    }
    /**
    ...
    public function getOther()
    {
        $returndata = [];
        $config_files = [
            "clients_profoptional",
            "clients_profuneditable",
            "show_cancel",
            "aff_report",
            "display_errors",
            "sql_error_reporting",
        ...
        $returndata["config_value"] = $config_value;
        $returndata["clients_profoptional_list"] = [
            "username" => "姓名",
            "companyname" => "公司",
            "qq" => "QQ",
            "address1" => "地址",
        ];
        $returndata["clients_profuneditable_list"] = [
            "username" => "姓名",
            "companyname" => "公司",
            "qq" => "QQ",
            "address1" => "地址",
    ...
    public function getRecharge()
    {
        ...
        $config_files = [
            ...
            "addfunds_minimum",
            "addfunds_maximum",
            "addfunds_maximum_balance",
            "addfunds_require_order",
            "no_auto_apply_credit",
            "credit_on_downgrade",
        ];
        $config_data = \think\Db::name("configuration")->whereIn("setting", $config_files)->select()->toArray();
        $config_value = [];
    ...
    public function getInvoice()
    {
        ...
        $config_files = [
            "in_batch_pay",
            "in_circulation_create",
            "in_pdf",
            "in_save_user_info",
            "in_select_payment",
            "in_unpaid_tick",
            "in_continuous_pay_num",
            "in_continuous_pay_num_type",
            "in_overdue_fine",
            "in_overdue_fine_min",
        ];
        $config_data = \think\Db::name("configuration")->whereIn("setting", $config_files)->select()->toArray();
    ...
    public function registerLoginPage()
    {
        ...
        $data["clients_profoptional_checked"] = !!configuration("clients_profoptional")
            ? explode(",", configuration("clients_profoptional"))
            : [];
        ...
        $data["allow_register_phone"] =
            configuration("allow_register_phone") == null
                ? $allow_phone
                : intval(configuration("allow_register_phone"));
        $data["allow_register_email"] =
            configuration("allow_register_email") == null
                ? $allow_email
                : intval(configuration("allow_register_email"));
        $data["allow_register_wechat"] =
            configuration("allow_register_wechat") == null
                ? $allow_wechat
                : intval(configuration("allow_register_wechat"));
        $data["allow_login_phone"] =
            configuration("allow_login_phone") == null ? $allow_phone : intval(configuration("allow_login_phone"));
        $data["allow_login_email"] =
            configuration("allow_login_email") == null ? $allow_email : intval(configuration("allow_login_email"));
        $data["allow_login_wechat"] =
            configuration("allow_login_wechat") == null ? $allow_wechat : intval(configuration("allow_login_wechat"));
        $data["login_register_custom_require"] =
            json_decode(configuration("login_register_custom_require"), true) ?? [];
    ...
    public function postregister_login_captcha()
    {
        ...
        if ($params["captcha_combination"] != $captcha_combination) {
            ...
            $dec .=
                "验证码组合由:“" . $arr[$captcha_combination] . "”改为“" . $arr[$params["captcha_combination"]] . "”，";
        }
    ...
    public function invoicePost()
    {
        ...
        if ($custom_invoice_id_start != $params["custom_invoice_id_start"]) {
            ...
            if (
                configuration("allow_custom_invoice_id") &&
                $params["custom_invoice_id_start"] &&
                $params["custom_invoice_id_start"] <= $id
            ) {
        ...
        if ($this->request->isPost()) {
            ...
            if ($type == 1) {
                ...
                foreach ($data as $key => $val) {
                    if ($val["id"] == 0) {
                        $data1 = [
                            "groupname" => htmlspecialchars_decode($val["groupname"]),
                            "fa_icon" => $val["fa_icon"],
                        ];
    ...
    public function postSecondVerify()
    {
        ...
        if (
            !!array_diff(
                ...
                array_column(config("second_verify_action_home_type"), "name"),
            )
        ) {
            return jsonrule(["status" => 400, "msg" => "参数错误"]);
        }
    ...
    {
        $data = [
            "shd_debug_model" => intval(cache("shd_debug_model")),
            "shd_debug_model_auth" => configuration("shd_debug_model_auth") ?: "",
            "shd_debug_model_expire_time" => configuration("shd_debug_model_expire_time") ?: time(),
        ];
        return jsonrule(["status" => 200, "msg" => lang("SUCCESS MESSAGE"), "data" => $data]);
    }
    /**
