...
class SendMessageBatchController extends GetUserController
{
    ...
    {
        $server_interface = \think\Db::name("servers")
            ->field("id,name")
            ->where("disabled", 0)
            ->order("id", "desc")
            ->select()
        ...
        $DCIMCloud_interface = \think\Db::name("zjmf_finance_api")
            ->field("id,name")
            ->order("id", "desc")
            ->select()
            ->toArray();
    ...
    public function mobiletemplateList()
    {
        ...
        $templates = \think\Db::name("message_template")
            ->alias("mt")
            ->field("mt.id,mt.title,mt.template_id,mt.range_type,mt.content,mt.status,mt.sms_operator")
            ->where("mt.status", 2)
            ->where("mt.range_type", 2)
            ->whereIn("mt.sms_operator", $pluginsFilter)
            ->select()
            ->toArray();
    ...
    public function searchList()
    {
        ...
        $clients = \think\Db::name("clients")
            ->alias("c")
            ->field("c.id ,c.username,c.phonenumber as mobile,c.email")
        ...
        if ($params["send_type"] == "clients") {
            ...
            $host = \think\Db::name("host")
                ->alias("h")
                ->field("h.id as host_id,h.uid as client_id,h.domain as host_domain,p.name as product_name,h.productid")
        ...
        return jsonrule([
            ...
            "client_count" => count($search_user_list),
    ...
    private function checkFinalSendUser($param)
    {
        ...
        if ($param["is_market"] == 1) {
            $system_clients = \think\Db::name("clients")
                ->where("status", 1)
                ->where("marketing_emails_opt_in", 0)
                ->field("id")
                ->select()
                ->toArray();
    ...
    private function checkForbiddenSmsPlate($send_type = "clients", $message_template_link_name = "")
    {
        if ($send_type == "clients") {
            $forbidden_sms_name = [
                ...
                "second_renew_product_reminder",
                "renew_product_reminder",
                "third_invoice_payment_reminder",
                "second_invoice_payment_reminder",
                "first_invoice_payment_reminder",
                "new_order_notice",
                "default_product_welcome",
                "service_termination_notification",
                "service_unsuspension_notification",
                "support_ticket_auto_close_notification",
    ...
    public function sendSmsBefore(
        ...
        if ($send_type == "clients_and_host") {
            $host = \think\Db::name("host")
                ...
                ->field(
                    "b.pay_type,b.name,a.dedicatedip,a.username,a.password,a.os,a.assignedips,a.create_time,a.nextinvoicedate",
                )
        ...
        $result = $sms->sendSmsForMarerking(
            $send_client["mobile"],
            $msgid,
            $newvar,
            $send_sync,
