...
class UnitEvent
{
    ...
    public function headCount()
    {
        ...
        $cancell_count = db("cancel_requests")
            ->alias("c")
            ->join("host h", "h.id=c.relid")
            ->where("auto_terminate_end_cycle", "<>", 1)
            ->where("domainstatus", "<>", "Cancelled")
            ->count();
        $module_queue = db("module_queue")->where("completed", 1)->count();
        $data = [
            "count" => $count,
            "stickt_unread" => $stickt_unread,
            "cancell_count" => $cancell_count,
    ...
    {
        $rows = db("ticket")
            ->whereIn("status", "Open,CustomerReply")
            ->field("count(id) as count,id,title,last_reply_time")
    ...
    {
        $rows = db("clients")
            ->whereTime("lastlogin", "-90 day")
            ->field("count(id) as count,id,username,lastloginip,lastlogin")
            ->limit(50)
            ->order("id", "desc")
            ->select();
        return $rows;
    ...
    {
        $rows = db("accounts")
            ->field("id,uid,currency,gateway,create_time,amount_in,amount_out")
            ->limit(30)
            ->order("id", "desc")
            ->select();
        return json($rows);
    ...
    public function systemInfo()
    {
        $data = [
            ...
            "processor_identifier" => $_SERVER["PROCESSOR_IDENTIFIER"],
