...
class HostController
{
    ...
    public function editDcimHost()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.id,a.productid,a.uid,a.serverid")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.dcimid", $id)
            ->where("a.id", $params["hostid"])
            ->find();
        if (empty($host)) {
            $result["status"] = 400;
            $result["msg"] = lang("ID_ERROR");
    ...
    public function syncInfo()
    {
        ...
        $update = [
            ...
            "password" => cmf_encrypt($params["password"]),
            "os" => $params["os"],
            "os_url" => $params["os_url"] ?: "",
            "dedicatedip" => $params["dedicatedip"],
            "assignedips" => $params["assignedips"],
        ...
        if (
            isset($params["domainstatus"]) &&
            in_array($params["domainstatus"], ["Pending", "Active", "Cancelled", "Fraud", "Deleted", "Suspended"])
        ) {
        ...
        if ($params["type"] == "create") {
            ...
            if ($params["domainstatus"] == "Active" && $host["domainstatus"] == "Pending") {
                ...
                if ($client) {
                    ...
                    if (
                        $billing_cycle[$host["billingcycle"]] == "免费" ||
                        $billing_cycle[$host["billingcycle"]] == "一次性"
                    ) {
                        $time = "不到期";
                    } else {
    ...
    public function syncTicketReply()
    {
        ...
        try {
            \think\Db::name("ticket")
                ->where("id", $id)
                ->update(["last_reply_time" => time(), "client_unread" => 1, "status" => 2]);
