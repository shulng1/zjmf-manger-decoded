...
class InstallController extends \think\Controller
{
    public function initialize()
    {
        ...
        if (!is_writable(CMF_DATA)) {
            exit(
                json_encode([
                    "status" => 500,
                    "msg" => "目录" . realpath(CMF_ROOT . "data") . "无法写入！",
                    "data" => ["url" => cmf_get_root() . "/"],
                ])
            );
        }
    ...
    public function envMonitor()
    {
        ...
        $folders = [
            ...
            realpath("../uploads") . DIRECTORY_SEPARATOR,
            realpath("./upload") . DIRECTORY_SEPARATOR,
            realpath("../app/config") . "/database.php",
        ];
        $newFolders = [];
        ...
        for ($x = 0; $x < $i; $x++) {
            ...
            if (count($sql) <= $index) {
                ...
                return json([
                    "status" => 200,
                    "msg" => "安装完成！",
                    "data" => ["done" => 1, "error" => $install_error],
                ]);
            }
            ...
            if (!empty($result["error"])) {
                ...
                return json([
                    "status" => 400,
                    "msg" => $result["message"],
                    "data" => ["sql" => $sql_to_exec, "exception" => $result["exception"]],
                ]);
            }
        }
        $index = $sql_index + 1;
    ...
    public function setSite()
    {
        ...
        try {
            ...
            $role_id = \think\Db::name("role")->insert([
                "id" => 1,
                "parent_id" => 0,
                "status" => 1,
                "create_time" => time(),
