...
function createJwt($userinfo, $expire = 7200)
{
    ...
    $token = [
        "userinfo" => $userinfo,
        "iss" => "www.idcSmart.com",
        "aud" => "www.idcSmart.com",
        "ip" => get_client_ip(),
        "iat" => $time,
        "nbf" => $time,
        "exp" => $time + $expire,
...
function crossProductUpgrade($hostid, $new_productid)
{
    ...
    $new_config = \think\Db::name("product_config_options_sub")
        ->alias("o_sub")
        ->field(
            "o.hidden,o.option_name,o.qty_minimum,o.qty_maximum,o_sub.option_name as option_name_sub,o_sub.qty_minimum as qty_minimum_sub,o_sub.qty_maximum as qty_maximum_sub,o_sub.id,o_sub.config_id,o.option_type,o.is_discount,o.option_name,o.is_rebate",
        )
        ->leftJoin("product_config_options o", "o_sub.config_id=o.id")
        ->leftJoin("product_config_links links", "o.gid=links.gid")
        ->where("links.pid", $new_productid)
        ->order("o_sub.sort_order ASC")
        ->order("o_sub.id ASC")
        ->select()
        ->toArray();
    $old_config = \think\Db::name("host_config_options")
        ->alias("h")
        ->field(
            "o.option_name,o.qty_minimum,o.qty_maximum,o_sub.option_name as option_name_sub,o_sub.qty_minimum as qty_minimum_sub,o_sub.qty_maximum as qty_maximum_sub,h.qty,o.option_type",
        )
        ->leftJoin("product_config_options o", "o.id=h.configid")
        ->leftJoin("product_config_options_sub o_sub", "o_sub.id=h.optionid")
        ->where("h.relid", $hostid)
        ->select()
        ->toArray();
    $old_con = [];
    $qty = [];
...
function admin_dept($admin_id)
{
    ...
    $depts = \think\Db::name("ticket_department")
        ->alias("a")
        ->field("a.name")
        ->leftJoin("ticket_department_admin b", "a.id = b.dptid")
        ->leftJoin("user c", "c.id = b.admin_id")
        ->where("c.id", $admin_id)
        ->select()
        ->toArray();
...
function view_tpl_common($content, $include = "")
{
    ...
    if (!empty($view_html_tagdata)) {
        ...
        if ($view_html_tag) {
            $_SESSION["view_tpl_tagdata"] = $_SESSION["view_tpl_tagdata"]
                ? array_merge($_SESSION["view_tpl_tagdata"], $view_html_tagdata[0])
                : $view_html_tagdata[0];
        }
    }
    ...
    if ($debug[0]) {
        $_SESSION["view_tpl_debug"] = $_SESSION["view_tpl_debug"]
            ? array_merge($_SESSION["view_tpl_debug"], $debug[0])
            : $debug[0];
        if (!empty($include)) {
    ...
    if ($tagdata) {
        ...
        foreach ($tagdata as $k => $v) {
            if (stripos($content, $v) !== false) {
                if ($tag == "debug") {
                    if (
                        ($_SESSION["view_tpl_debug_include"] == $templateName &&
                            !empty($_SESSION["view_tpl_debug_include"])) ||
                        empty($_SESSION["view_tpl_debug_include"])
                    ) {
                        ...
                        $templateName =
                            stripos($templateName, "." . VIEW_TEMPLATE_SUFFIX) !== false
                                ? $_SESSION["view_tpl_data"]["TplName"]
                                : $templateName;
                        ...
                        if (VIEW_TEMPLATE_WEB === true) {
                            $hn =
                                "<h1>当前调试模板debug标签所在文件地址:\"" .
                                configuration(VIEW_TEMPLATE_SETTING_NAME) .
                                "/" .
                                $templateName .
                                "." .
                                VIEW_TEMPLATE_SUFFIX .
                                "\" </h1><h2 style=\"background:green;\">模板可以输出的数据:" .
                                $paramsData .
                                " </h2>";
                        } else {
                            $hn =
...
";
						...
						idcsmart_debug_console = window.open(\"\", \"" .
                            md5(time()) .
                            "\", \"width=1024,height=600,left=50,top=50,resizable,scrollbars=yes\");
						idcsmart_debug_console.document.write(\"" .
                            addslashes($debugHtml) .
                            "\");
						</script>
						";
...
function getCountryCode()
{
    ...
    if ($lang == "zh-cn") {
        $sms = \think\Db::name("sms_country")
            ->field("concat(\"+\",phone_code) as phone_code,concat(\"+\",phone_code,\"(\",name_zh,\")\") as link")
            ->order("phone_code asc")
            ->select()
            ->toArray();
    } else {
        $sms = \think\Db::name("sms_country")
            ->field("concat(\"+\",phone_code) as phone_code,concat(\"+\",phone_code,\"(\",name,\")\") as link")
            ->order("phone_code asc")
            ->select()
            ->toArray();
    }
...
function updateConfiguration($setting, $value)
{
    $data["value"] = $value ?? "";
    $result = \think\Db::name("configuration")
        ->whereRaw("setting = :setting", ["setting" => $setting])
        ->find();
    if (!empty($result)) {
...
function gateway_list($module = "gateways", $status = 1)
{
    ...
    $rows = \think\Db::name("plugin")
        ->field("id,name,title,status,module,url,author_url")
        ->where($where)
        ...
        ->withAttr("author_url", function ($value, $data) {
            if (
                file_exists(
                    CMF_ROOT . "modules/gateways/" . cmf_parse_name($data["name"], 0) . "/" . $data["name"] . ".png",
                )
            ) {
...
function gateway_list1($module = "gateways", $status = 1)
{
    ...
    $rows = db("plugin")
        ->field("id,name,title,status,module")
        ->where($where)
        ->order("order", "asc")
        ->order("id", "asc")
        ->select()
...
function get_product_allowcycle($pid)
{
    ...
    $pricing_data = \think\Db::name("pricing")
        ->where("type", "product")
        ->where("relid", $pid)
...
function search_users($name)
{
    return db("clients")
        ->field("id,username")
        ->where("username|phonenumber|email", "like", "%{$name}%")
        ->limit(50)
        ->select();
}
/**
...
function getUserCurrency($uid)
{
    ...
    if (!empty($user_data["currency"])) {
        $currency_data = \think\Db::name("currencies")
            ->field("id,code,prefix,suffix")
            ->where("id", $user_data["currency"])
            ->find();
    }
...
{
    $currencyId = db("invoices")
        ->alias("i")
        ->join("clients c", "c.id=i.uid")
...
{
    $re = [];
    $personalUsed = \think\Db::name("certifi_person")
        ->whereRaw("auth_user_id !=:userid", ["userid" => $clientid])
        ->whereRaw("auth_card_number =:idcard", ["idcard" => $idcard])
        ->find();
    $companyUsed = \think\Db::name("certifi_company")
        ->whereRaw("auth_user_id !=:userid", ["userid" => $clientid])
        ->whereRaw("auth_card_number =:idcard", ["idcard" => $idcard])
        ->find();
...
function getSupportedCycles($pid, $currency, $mon_pack_year = 1)
{
    ...
    $cycle_data = [];
    $cycle_condition = [];
    $price_data = \think\Db::name("pricing")
        ->where("type", "product")
        ->where("relid", $pid)
...
function getCurrencyRateByUid($uid)
{
    ...
    $client = \think\Db::name("clients")
        ->alias("a")
        ->field("b.rate")
        ->leftJoin("currencies b", "a.currency = b.id")
        ->where("a.id", $uid)
        ->find();
    if ($client["rate"]) {
    ...
    if (!empty($name)) {
        foreach ($name as $k => $v) {
            ...
            if ($v == "Invoice ID") {
                ...
                $url =
                    "<a class=\"el-link el-link--primary is-underline\" href=\"#/customer-view/abstract?id=" .
                    $relid .
                ...
                $url =
                    "<a class=\"el-link el-link--primary is-underline\" href=\"#/customer-view/product-innerpage?id=" .
                    $uid .
                    "\"><span class=\"el-link--inner\">" .
                    $str .
                    "</span></a>";
                ...
                $url =
                    "<a class=\"el-link el-link--primary is-underline\" href=\"#/admin-management?id=" .
                    $relid .
                    "\"><span class=\"el-link--inner\">" .
                    $str .
                    "</span></a>";
                ...
                $url =
                    "<a class=\"el-link el-link--primary is-underline\" href=\"#/permissions-managment?id=" .
                    $relid .
                    "\"><span class=\"el-link--inner\">" .
                    $str .
                    "</span></a>";
...
function admin_log()
{
    ...
    if (!empty($exists_data) && empty($exists_data["logouttime"])) {
        \think\Db::name("admin_log")
            ->where("sessionid", $session_id)
            ->update(["lastvisit" => time(), "logouttime" => 0]);
    } else {
        $idata = [
...
function admin_log1()
{
    ...
    if (!empty($exists_data)) {
        \think\Db::name("admin_log")
            ->where("sessionid", $session_id)
            ->update(["sessionid" => $session_id . "-" . time(), "lastvisit" => time(), "logouttime" => time()]);
    } else {
        $idata = [
            "admin_username" => $username ?? "",
            "logintime" => time(),
...
function credit_log($data)
{
    ...
    return \think\Db::name("credit")->insert([
        "uid" => $data["uid"],
...
{
    $clientCares = \think\Db::name("client_care")
        ->field("id,name,trigger,time,method,email_template_id,message_template_id,wechat_template_id,range_type")
        ->where("trigger", "register_surpass")
        ->where("status", 1)
        ->order("id desc")
        ->select();
...
function getAllPromoData($expiration = false)
{
    ...
    $promo_data = \think\Db::name("promo_code")
        ->field("id,code,type,recurring,value,recurring,expiration_time")
        ->where(function (\think\db\Query $query) use ($expiration) {
            $time = time();
            if (!$expiration) {
...
function getProductList($type = 0, $id = 0)
{
    ...
    foreach ($groups as $k => $v) {
        $groupid = $v["id"];
        $groups[$k]["product"] = db("products")
            ->field("id,name,type,api_type")
...
{
    try {
        $groups = db("product_groups")
            ->alias("c")
            ->field("c.id,c.name")
            ->join("products p", "p.gid=c.id")
            ->where(function (\think\db\Query $query) use ($ids) {
                if (!empty($ids)) {
        ...
        foreach ($groups as $k => $v) {
            ...
            $groups[$k]["children"] = db("products")
                ->alias("c")
                ->field("c.id,c.name,c.gid")
                ->join("product_groups p", "p.id=c.gid")
                ->where("p.id", $v["id"])
                ->where(function (\think\db\Query $query) use ($ids) {
                    if (!empty($ids)) {
...
function login_sms_remind($data)
{
    ...
    if ($data["phone_code"] > 0 && isset($data["phonenumber"][0]) && $data["is_login_sms_reminder"] == 1) {
        ...
        if ($client) {
            $tmp = $sms->sendSms(
                $message_template_type[strtolower("login_sms_remind")],
                $phone,
                $params,
                false,
                $data["id"],
...
function email_log(
    ...
    return \think\Db::name("email_log")->insert([
        "uid" => $uid,
        "subject" => $subject,
        "message" => $message,
        "create_time" => time(),
        ...
        "bcc" => $bcc ?? "",
        "status" => $status,
        "fail_reason" => empty($fail_reason) ? "" : $fail_reason,
        "is_admin" => $is_admin,
        "attachments" => $attachments,
        "ip" => !empty($ip) ? $ip : get_client_ip6(),
        "port" => get_remote_port(),
    ]);
}
...
{
    $host = \think\Db::name("host")
        ->alias("a")
        ->field(
            "a.domainstatus,a.reinstall_info,a.uid,a.dcimid,b.api_type,b.zjmf_api_id,c.buy_times,c.reinstall_times,c.reinstall_price,c.auth",
        )
            ...
            if (empty($au)) {
                ...
                if ($type == 1) {
                    ...
                    if (configuration("affiliate_invited")) {
                        ...
                        credit_log([
                            "uid" => $uid,
                            "desc" => "Promotion program activity reward",
                            "amount" => $affiliate_invited_money,
                        ]);
                    }
                } elseif ($type == 2) {
            ...
            if ($aff_id) {
                ...
                if (empty($affiliates_user_temp)) {
                    ...
                    if ($aff_id_uid != $affiliates_user_temp) {
                        \think\Db::name("affiliates_user_temp")
                            ->where("uid", $uid)
                            ->update(["affid_uid" => $aff_id_uid]);
                    }
                }
            }
        } else {
...
{
    $log = \think\Db::name("certifi_log")
        ->where([["uid", "=", $uid], ["type", "=", $type]])
        ->order("id", "DESC")
        ->find();
    if (isset($log["id"])) {
        \think\Db::name("certifi_log")
            ->where("id", $log["id"])
            ->data(["status" => $status])
            ->find();
    }
...
function get_buy_reinstall_times($uid, $hostid)
{
    ...
    $count = \think\Db::name("dcim_buy_record")
        ->where("uid", $uid)
        ->where("hostid", $hostid)
        ->where("type", "reinstall_times")
        ->where("status", 1)
...
function dcim_callback($invoiceid = 0)
{
    ...
    if ($record["type"] == "reinstall_times") {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.dcimid,a.domainstatus,a.nextduedate,a.suspendreason,a.serverid,b.bill_type,c.type product_type,c.api_type,c.zjmf_api_id",
            )
            ->leftJoin("dcim_servers b", "a.serverid=b.serverid")
            ->leftJoin("products c", "a.productid=c.id")
            ->where("a.id", $record["hostid"])
            ->find();
        if ($host["api_type"] == "zjmf_api") {
            ...
            if ($res["status"] == 200) {
                ...
                if ($res["status"] == 1001) {
                    $description = sprintf(
                        "上游购买重装次数成功,上游账单ID:%d - Invoice ID:%d - Host ID:%d",
                        $post_data["invoiceid"],
                        $invoiceid,
                        $record["hostid"],
                    );
                    ...
                    $description = sprintf(
                        "上游购买重装次数失败,上游账单ID:%d - Invoice ID:%d - Host ID:%d - 原因:余额不足",
                        $post_data["invoiceid"],
                        $invoiceid,
                        $record["hostid"],
                    );
                    ...
                    $description = sprintf(
                        "上游购买重装次数失败,上游账单ID:%d - Invoice ID:%d - Host ID:%d - 原因:%s",
                        $post_data["invoiceid"],
                        $invoiceid,
                        $record["hostid"],
                        $res["msg"],
                ...
                $description = sprintf(
                    "上游购买重装次数失败 - Invoice ID: %d - Host ID:%d - 原因:%s",
                    $invoiceid,
                    $record["hostid"],
                    $res["msg"],
                );
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            if ($res["status"] == 200) {
                ...
                if ($res["status"] == 1001) {
                    $description = sprintf(
                        "上游购买流量包成功,上游账单ID:%d - Invoice ID:%d - Host ID:%d",
                        $post_data["invoiceid"],
                        $invoiceid,
                        $record["hostid"],
                    );
                    ...
                    $description = sprintf(
                        "上游购买流量包失败,上游账单ID:%d - Invoice ID:%d - Host ID:%d - 原因:余额不足",
                        $post_data["invoiceid"],
                        $invoiceid,
                        $record["hostid"],
                    );
                    ...
                    $description = sprintf(
                        "上游购买流量包失败,上游账单ID:%d - Invoice ID:%d - Host ID:%d - 原因:%s",
                        $post_data["invoiceid"],
                        $invoiceid,
                        $record["hostid"],
                        $res["msg"],
                ...
                $description = sprintf(
                    "上游购买流量包失败 - Invoice ID: %d - Host ID:%d - 原因:%s",
                    $invoiceid,
                    $record["hostid"],
                    $res["msg"],
                );
...
function jsonrule($data = [], $code = 200, $header = [], $options = [])
{
    ...
    $user = \think\Db::name("user")
        ->field("is_sale,sale_is_use,only_mine,all_sale")
...
function get_dcim_traffic_usage_table($hostid, $uid, $bill_type, $bwusage, $bwlimit = 0)
{
    ...
    $res = \think\Db::name("dcim_buy_record")
        ->field("name,capacity,price,pay_time")
...
function getProductOs($pid)
{
    ...
    if (!empty($products) && $products["type"] == "dcim") {
        $os = \think\Db::name("server_groups")
            ->alias("a")
            ->leftJoin("servers b", "a.id = b.gid")
            ->leftJoin("dcim_servers c", "b.id = c.serverid")
            ->where("a.system_type", "dcim")
            ->where("b.disabled", 0)
            ->where("a.id", $products["server_group"])
            ->value("c.os");
        if ($os) {
...
function cartCheckOs($pid, $os)
{
    ...
    $products = \think\Db::name("products")->where("id", $pid)->find();
    if (!empty($products) && $products["type"] == "dcim") {
        $prodtuc_os = \think\Db::name("server_groups")
            ->alias("a")
            ->leftJoin("servers b", "a.id = b.gid")
            ->leftJoin("dcim_servers c", "b.id = c.serverid")
            ->where("a.system_type", "dcim")
...
function invoiceTypeToDescription($type, $invoiceid = null)
{
    if ($type == "product") {
        ...
        $ids = \think\Db::name("invoice_items")
...
function get_order_num_by_date(int $days = 7, $status = "")
{
    ...
    if (empty($status)) {
        $count = \think\Db::name("orders")
            ->field("from_unixtime(create_time, \"%Y-%m-%d\") date,count(id) count,sum(amount) amount_total")
        ...
        $count = \think\Db::name("orders")
            ->alias("a")
            ->field("from_unixtime(a.create_time, \"%Y-%m-%d\") date,count(a.id) count,sum(amount) amount_total")
            ->leftJoin("invoices b", "a.invoiceid=b.id")
...
function getTimeLine($startMonth)
{
    ...
    while ($ToStartMonth <= $ToEndMonth) {
        $NewMonth = !$i
            ? date("Y-m", strtotime("+0 Month", $ToStartMonth))
            : date("Y-m", strtotime("+1 Month", $ToStartMonth));
...
function check_type_is_use($type, $uid, $sms)
{
    ...
    $count = \think\Db::name("message_template_link")
        ->where("type", $type)
        ->where("range_type", $rangeType)
        ->where("sms_operator", $sms_operator)
        ->where("is_use", 1)
        ->find();
    if (!empty($count)) {
        return $client;
...
{
    $tmp = \think\Db::name("invoice_items")
        ->alias("a")
        ->leftJoin("invoices b", "a.invoice_id = b.id")
        ->where("a.rel_id", $id)
        ->where("b.status", "Paid")
        ->where("a.type", "combine")
        ->find();
    if (!empty($tmp)) {
        return number_format(0, 2);
    }
    $invoices = \think\Db::name("invoices")
        ->alias("a")
        ->field(
            "a.id,b.username,a.create_time,a.due_time,a.paid_time,a.total,a.subtotal,a.status,a.credit,a.payment,a.last_capture_attempt",
        )
        ->leftJoin("clients b ", "a.uid = b.id")
        ->where("a.id", $id)
        ->where("a.delete_time", 0)
        ->find();
    $paids = \think\Db::name("accounts")
        ->field("amount_in,fees,amount_out")
        ->where("invoice_id", $id)
        ->where("delete_time", 0)
        ->select()
        ->toArray();
...
function getProductCommission($oid, $type, $uid, $create_time, $ladder, $invoiceid = 0)
{
    ...
    $hosts = \think\Db::name("host")
        ...
        ->join("invoice_items in", "in.rel_id = a.id")
        ->field(
            "a.domainstatus,a.domain,b.id,a.id as hostid,a.uid,b.name,in.type,in.amount,in.invoice_id,in.id as inid,in.aff_sure_time,in.aff_commission,in.aff_commmission_bates,in.aff_commmission_bates_type,in.is_aff",
        )
    ...
    if ($type == "renew") {
        foreach ($hosts as $key => $val) {
            ...
            if ($affiliates["affiliate_enabled"] == 1) {
                if ($affiliates["affiliate_is_renew"] == 1) {
                    ...
                    $nums = \think\Db::name("invoice_items")
                        ->where("rel_id", $val["hostid"])
                        ->where("type", "in", "renew,promo,discount")
                        ->sum("amount");
                    if ($affiliates["affiliate_renew_type"] == 1) {
                        if ($val["type"] != "discount" && $val["type"] != "setup" && $val["type"] != "promo") {
                            if (!empty($ladder["turnover"]["turnover"])) {
                                $sum = bcadd(
                                    bcadd(
                                        $affiliates["affiliate_renew"],
                                        bcmul($ladder["turnover"]["bates"] / 100, $nums, 4),
                                        4,
                                    ),
                                    $sum,
                                    4,
                        ...
                        if (!empty($ladder["turnover"]["turnover"])) {
                            $sum = bcadd(
                                bcadd(
                                    bcmul($affiliates["affiliate_renew"] / 100, $nums, 4),
                                    bcmul($ladder["turnover"]["bates"] / 100, $nums, 4),
                                    4,
                                ),
                                $sum,
                ...
                if (configuration("affiliate_is_renew") == 1) {
                    ...
                    if (configuration("affiliate_renew_type") == 1) {
                        ...
                        if (!empty($ladder["turnover"]["turnover"])) {
                            $sum = bcadd(
                                bcadd(
                                    bcmul(configuration("affiliate_renew") / 100, $nums, 4),
                                    bcmul($ladder["turnover"]["bates"] / 100, $nums, 4),
                                    4,
                                ),
                                $sum,
        ...
        $ordercount = \think\Db::name("orders")
            ->alias("o")
            ->join("clients c", "o.uid=c.id")
            ->join("invoices i", "i.id=o.invoiceid")
            ->field("o.id")
                        ...
                        if (configuration("affiliate_reorder_type") == 1) {
                            if ($val["type"] != "discount" && $val["type"] != "setup" && $val["type"] != "promo") {
                                if (!empty($ladder["turnover"]["turnover"])) {
                                    $sum = bcadd(
                                        bcadd(
                                            configuration("affiliate_reorder"),
                                            bcmul($ladder["turnover"]["bates"] / 100, $nums, 4),
                                            4,
                                        ),
                                        $sum,
                            ...
                            if (!empty($ladder["turnover"]["turnover"])) {
                                $sum = bcadd(
                                    bcadd(
                                        bcmul(configuration("affiliate_reorder") / 100, $nums, 4),
                                        bcmul($ladder["turnover"]["bates"] / 100, $nums, 4),
                                        4,
                                    ),
                                    $sum,
                ...
                if (!empty($affiliates) && $affiliates["affiliate_enabled"] == 1) {
                    ...
                    if ($affiliates["affiliate_type"] == 1) {
                        ...
                        if (!empty($ladder["turnover"]["turnover"])) {
                            $sum = bcadd(
                                bcadd(
                                    bcmul($affiliates["affiliate_bates"] / 100, $nums, 4),
                                    bcmul($ladder["turnover"]["bates"] / 100, $nums, 4),
                                    4,
                                ),
                                $sum,
                    ...
                    if ($affiliates["affiliate_enabled"] == 2) {
                        ...
                        if (configuration("affiliate_type") == 1) {
                            ...
                            if (!empty($ladder["turnover"]["turnover"])) {
                                $sum = bcadd(
                                    bcadd(
                                        bcmul(configuration("affiliate_bates") / 100, $nums, 4),
                                        bcmul($ladder["turnover"]["bates"] / 100, $nums, 4),
                                        4,
                                    ),
                                    $sum,
...
function getProductCount($oid, $type, $uid, $create_time)
{
    $hosts = \think\Db::name("host")
        ->alias("a")
        ->join("products b", "a.productid = b.id")
        ->field("b.id,a.id as hostid,a.uid")
        ->where("a.orderid", $oid)
        ->order("a.id", "desc")
        ->select()
        ->toArray();
        ...
        if ($affiliate_enabled == 1) {
            if ($value["type"] == "renew") {
                ...
                $ordercount = \think\Db::name("orders")
                    ...
                    ->field("o.id")
                    ->where("i.status", "in", "Paid,Refunded")
                    ->where("o.create_time", "<", $value["create_time"])
                    ->where("c.id", $value["uid"])
                    ->count();
                if ($ordercount > 0) {
...
function isAffInvoiceProduct($invoice_id)
{
    ...
    return \think\Db::name("affiliates_products_setting")
        ->where("affiliate_enabled", "<>", 2)
        ->whereIn("pid", $pro_ids)
        ->count();
}
...
function getLadder($uid, $uids)
{
    $user = \think\Db::name("clients")
        ->alias("c")
        ->field("cu.suffix")
        ->join("currencies cu", "cu.id = c.currency")
        ->where("c.id", $uid)
        ->find();
    $rows = \think\Db::name("invoices")
        ->alias("i")
        ->join("clients c", "i.uid=c.id")
        ->join("currencies cu", "cu.id = c.currency")
        ->field("i.id as invoiceid")
        ->where("i.status", "=", "Paid")
        ->whereTime("i.paid_time", "month")
        ->where("c.id", "in", $uids)
        ->select()
        ->toArray();
    $rows1 = \think\Db::name("invoices")
        ->alias("i")
        ->join("invoice_items in", "in.invoice_id=i.id")
        ->join("host h", "h.id=in.rel_id")
        ->join("clients c", "i.uid=c.id")
        ->field("i.id,in.amount")
        ->where("i.status", "=", "Paid")
        ->whereTime("i.paid_time", "month")
        ->where("c.id", "in", $uids)
        ->select()
    ...
    foreach ($rows as $key => $val) {
        $accounts = \think\Db::name("accounts")
            ->field("id,amount_in,amount_out,refund")
            ->where("invoice_id", $val["invoiceid"])
            ->where("refund", ">", 0)
            ->select()
            ->toArray();
    ...
    $sale_ladder = \think\Db::name("affiliate_ladder")
        ->field("id,turnover,bates")
        ->where("turnover", "<=", $sum)
        ->order("turnover", DESC)
        ->find();
    $sale_ladder1 = \think\Db::name("affiliate_ladder")
        ->field("id,turnover,bates")
        ->where("turnover", ">", $sum)
        ->order("turnover", ASC)
        ->find();
...
function getCommissioninvoice($oid, $type, $aff_type, $ladder, $bates)
{
    $hosts = \think\Db::name("host")
        ...
        ->where("in.invoice_id", $oid)
        ->where("in.type", "in", $type)
        ->order("a.id", "asc")
        ->select()
        ->toArray();
    foreach ($hosts as $key => $val) {
        if ($aff_type == 1) {
            if (!empty($ladder["turnover"]["turnover"])) {
                if ($val["type"] != "discount" && $val["type"] != "setup" && $val["type"] != "promo") {
                    $hosts[$key]["commission"] = round(
                        bcadd($bates, bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 4), 4),
                        2,
                    );
                } else {
            ...
            if (!empty($ladder["turnover"]["turnover"])) {
                $hosts[$key]["commission"] = round(
                    bcadd(
                        bcmul($bates / 100, $val["amount"], 4),
                        bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 4),
                        4,
                    ),
                    2,
                );
        ...
        $hosts[$key]["names"] =
            "<a class=\"el-link el-link--primary is-underline\" 
                href=\"#/customer-view/product-innerpage?hid=" .
            $val["hostid"] .
            "&id=" .
            $val["uid"] .
            "\">
                <span class=\"el-link--inner\" style=\"display: block;height: 24px;line-height: 24px;\">" .
            $str .
            "</span></a>";
        switch ($val["type"]) {
            case "renew":
        ...
        foreach ($hosts as $key => $val) {
            ...
            if ($affiliates["affiliate_enabled"] == 1) {
                if ($affiliates["affiliate_is_renew"] == 1) {
                    ...
                    $arrs = getCommissioninvoicebyhost(
                        $hosts[$key],
                        $affiliates["affiliate_renew_type"],
                        $ladder,
                        $affiliates["affiliate_renew"],
                    );
                    $arr[] = $arrs;
                ...
                if (configuration("affiliate_is_renew") == 1) {
                    ...
                    $arrs = getCommissioninvoicebyhost(
                        $hosts[$key],
                        configuration("affiliate_renew_type"),
                        $ladder,
                        configuration("affiliate_renew"),
                    );
                    $arr[] = $arrs;
            ...
            foreach ($hosts as $key => $val) {
                ...
                if (!empty($affiliates) && $affiliates["affiliate_enabled"] == 2) {
                    ...
                    if (configuration("affiliate_is_reorder") == 1) {
                        ...
                        $arrs = getCommissioninvoicebyhost(
                            $hosts[$key],
                            configuration("affiliate_reorder_type"),
                            $ladder,
                            configuration("affiliate_reorder"),
                        );
                        $arr[] = $arrs;
    ...
    if ($aff_type == 1) {
        if (!empty($ladder["turnover"]["turnover"])) {
            if ($hosts["type"] != "discount" && $hosts["type"] != "setup" && $hosts["type"] != "promo") {
                $hosts["commission"] = round(
                    bcadd($bates, bcmul($ladder["turnover"]["bates"] / 100, $hosts["amount"], 4), 4),
                    2,
                );
            } else {
        ...
        if (!empty($ladder["turnover"]["turnover"])) {
            $hosts["commission"] = round(
                bcadd(
                    bcmul($bates / 100, $hosts["amount"], 4),
                    bcmul($ladder["turnover"]["bates"] / 100, $hosts["amount"], 4),
                    4,
                ),
                2,
            );
        ...
        if (configuration("affiliate_enabled") == 1) {
            if ($value["type"] == "renew") {
                if ($affiliates["affiliate_is_renew"] == 1) {
                    if ($affiliates["affiliate_renew_type"] == 1) {
                        if (!empty($ladder["turnover"]["turnover"])) {
                            $rows[$k]["commission"] = bcadd(
                                $affiliates["affiliate_renew"],
                                bcmul($ladder["turnover"]["bates"] / 100, $value["subtotal"], 4),
                                4,
                            );
                        } else {
                        ...
                        if (!empty($ladder["turnover"]["turnover"])) {
                            $rows[$k]["commission"] = bcadd(
                                bcmul($affiliates["affiliate_renew"] / 100, $value["subtotal"], 2),
                                bcmul($ladder["turnover"]["bates"] / 100, $value["subtotal"], 4),
                                4,
                            );
                        } else {
                            $rows[$k]["commission"] = bcmul(
                                $affiliates["affiliate_renew"] / 100,
                                $value["subtotal"],
                                4,
                            );
                        }
                    ...
                    $commission = getproductcommission(
                        $value["id"],
                        $value["type"],
                        $value["uid"],
                        $value["create_time"],
                ...
                if ($ordercount > 0) {
                    ...
                    if ($affiliates["affiliate_is_reorder"] == 1) {
                        if ($affiliates["affiliate_reorder_type"] == 1) {
                            ...
                            if (!empty($ladder["turnover"]["turnover"])) {
                                $rows[$k]["commission"] = bcadd(
                                    bcmul($affiliates["affiliate_reorder"] / 100, $value["subtotal"], 4),
                                    bcmul($ladder["turnover"]["bates"] / 100, $value["subtotal"], 4),
                                    4,
                                );
                            } else {
                                $rows[$k]["commission"] = bcmul(
                                    $affiliates["affiliate_reorder"] / 100,
                                    $value["subtotal"],
                                    4,
                                );
                            }
                    ...
                    if ($affiliates["affiliate_enabled"] == 1) {
                        if ($affiliates["affiliate_type"] == 1) {
                            ...
                            if (!empty($ladder["turnover"]["turnover"])) {
                                $rows[$k]["commission"] = bcadd(
                                    bcmul($affiliates["affiliate_bates"] / 100, $value["subtotal"], 4),
                                    bcmul($ladder["turnover"]["bates"] / 100, $value["subtotal"], 4),
                                    4,
                                );
                            } else {
                                $rows[$k]["commission"] = bcmul(
                                    $affiliates["affiliate_bates"] / 100,
                                    $value["subtotal"],
                                    4,
                                );
                            }
            ...
            if ($rows[$k]["is_aff"] == 1) {
                $amount_outs = \think\Db::name("accounts")
                    ->field("id,amount_out")
                    ->where("invoice_id", $value["invoiceid"])
                    ->where("refund", ">", 0)
                    ->where("aff_refund", "=", 0)
                    ->select()
                    ->toArray();
                $credit = \think\Db::name("credit")
                    ->field("id,amount")
                    ->where("relid", $value["invoiceid"])
                ...
                if ($refo > 0) {
                    if ($rows[$k]["is_aff"] == 1) {
                        ...
                        $affcomm =
                            bcsub($rows[$k]["aff_commission"], $refo, 2) > 0
                                ? bcsub($rows[$k]["aff_commission"], $refo, 2)
                                : 0.0;
                        ...
                        \think\Db::name("invoices")
                            ->where("id", $value["invoiceid"])
                            ->update(["aff_commission" => $affcomm]);
                        $res = \think\Db::name("affiliates")
                            ->where("id", $affi["id"])
                            ->update(["balance" => $affbala, "updated_time" => time()]);
                    }
                }
            }
            $amount_out3 = \think\Db::name("credit")
                ->field("id,amount")
                ->where("relid", $value["invoiceid"])
                ->where("description", "like", "%Removed%")
                ->sum("amount");
            $amount_out2 = \think\Db::name("accounts")
                ->field("id,amount_out")
                ->where("invoice_id", $value["invoiceid"])
                ->where("refund", ">", 0)
                ->sum("amount_out");
...
function dealCommissionaffs($rows, $ladder, $id)
{
    ...
    foreach ($rows as $k => $value) {
        if ($affiliate_enabled == 1) {
            if ($value["type"] == "renew") {
                if ($affiliates["affiliate_is_renew"] == 1) {
                    if ($affiliates["affiliate_renew_type"] == 1) {
                        ...
                        if (!empty($ladder["turnover"]["turnover"])) {
                            $rows[$k]["commission"] = bcadd(
                                bcmul($affiliates["affiliate_renew"] / 100, $value["subtotal"], 4),
                                bcmul($ladder["turnover"]["bates"] / 100, $value["subtotal"], 4),
                                4,
                            );
                        } else {
                    ...
                    $rows[$k]["child"] = getcommissioninvoice(
                        $value["invoiceid"],
                        "renew,promo,discount",
                        $affiliates["affiliate_renew_type"],
                        $ladder,
                        $affiliates["affiliate_renew"],
                        $value["invoiceid"],
                    ...
                    $commission = getproductcommissioninvoice(
                        $value["id"],
                        $value["type"],
                        $value["uid"],
                        $value["create_time"],
                ...
                if ($ordercount > 0) {
                    ...
                    if ($affiliates["affiliate_is_reorder"] == 1) {
                        ...
                        $rows[$k]["child"] = getcommissioninvoice(
                            $value["invoiceid"],
                            "host,setup,promo,discount",
                            $affiliates["affiliate_reorder_type"],
                            $ladder,
                            $affiliates["affiliate_reorder"],
                        );
                    } else {
            ...
            $rows[$k]["child"] = getcommissioninvoice(
                $value["invoiceid"],
                "renew,host,setup,promo,discount",
                $affiliates["affiliate_renew_type"],
                $ladder,
                $affiliates["affiliate_renew"],
            );
...
{
    $tmp = \think\Db::name("user_product_bates")
        ->alias("a")
        ->field("a.type,a.bates")
        ->leftJoin("user_products b", "a.products = b.gid")
        ->leftJoin("clients c", "a.user = c.groupid")
        ->where("c.id", $uid)
        ->where("b.pid", $pid)
        ->find();
    return $tmp ?: false;
        ...
        if ($host_data["flag"] == 0) {
            ...
            if ($flag) {
                if ($flag["type"] == 1) {
                    ...
                    $v["amount"] =
                        round(bcsub($amount, $flag["bates"]), 2) > 0 ? round(bcsub($amount, $flag["bates"]), 2) : 0;
                } else {
                    $v["saleproducts"] = $flag["bates"];
                    $v["amount"] =
                        round(bcsub($amount, $flag["bates"]), 2) > 0 ? round(bcsub($amount, $flag["bates"]), 2) : 0;
...
function getDiscounts($pid, $uid, $amount)
{
    ...
    if ($flag) {
        if ($flag["type"] == 1) {
            ...
            $v["amount"] =
                bcsub($amount, $v["saleproducts"], 4) > 0
                    ? sprintf("%.2f", bcsub($amount, $v["saleproducts"], 4))
                    : 0.0;
        } elseif ($flag["type"] == 2) {
            $bates = $flag["bates"];
            $v["saleproducts"] = $bates;
...
function judgeOntrialNum($pid, $uid, $qty, $admin = false, $cart = false, $edit = -1)
{
    ...
    $host_ontrial_count = \think\Db::name("host")
        ->alias("a")
        ->where("a.uid", $uid)
        ->where("a.productid", $pid)
        ->where("a.billingcycle", "ontrial")
        ->where($whereMap)
        ->count();
...
function unsuspendAfterCertify($uid)
{
    if (checkcertify($uid)) {
        $hosts = \think\Db::name("host")
            ->field("id,suspendreason,nextduedate,billingcycle")
        ...
        foreach ($hosts as $host) {
            if (
                explode("-", $host["suspendreason"])[1] &&
                (explode("-", $host["suspendreason"])[0] == "uncertifi" ||
                    explode("-", $host["suspendreason"])[0] == "其他")
            ) {
                if ($host["billingcycle"] == "free" || $host["billingcycle"] == "onetime") {
                    ...
                    if ($res["status"] == 200) {
                        ...
                        $data_i[
                            "description"
                        ] = " 实名认证完成 - 解除暂停 Host ID:{$data_i["host_id"]}的产品失败：{$res["msg"]}";
                        if ($is_zjmf) {
...
{
    $ngu = \think\Db::name("nav_group_user")
        ->where("uid", $uid ?? 0)
        ->select()
        ->toArray();
    if ($uid != 0 && $uid != "" && !empty($uid)) {
        if (empty($ngu[0])) {
            ...
            $data = \think\Db::name("nav_group")
                ->alias("ng")
                ->join("nav_group_user agu", "ng.id=agu.groupid")
                ->where("agu.uid", "=", $uid)
                ->field("ng.*")
                ->select()
                ->toArray();
        ...
        if ($td["is_open_auto_reply"] == 1) {
            ...
            if ($value["create_time"] + $td["minutes"] <= time()) {
                ...
                try {
                    ...
                    $r2 = \think\Db::name("ticket")
                        ->where("id", $data["tid"])
                        ->update(["is_auto_reply" => 1, "admin_unread" => 1, "last_reply_time" => time()]);
                    if ($r1 && $r2) {
                        active_logs(
                            sprintf("回复工单#User ID:%d - Ticket ID:%s - %s", $uid, $value["tid"], $value["title"]),
                            $uid,
                        );
                        active_logs(
                            sprintf("回复工单#User ID:%d - Ticket ID:%s - %s", $uid, $value["tid"], $value["title"]),
...
{
    \think\Db::name("developer_app_log")->insert([
        "pid" => $pid ?? 0,
        "desc" => $desc ?? "",
        "reason" => $reason ?? "",
        "type" => $type ?? 0,
...
function quantityStagePrice($cid, $currency, $quantity, $billingcycle, $last_price = 0, $last_setup = 0)
{
    ...
    $subs = \think\Db::name("product_config_options_sub")
        ->alias("a")
        ->field("a.qty_minimum,a.qty_maximum")
        ->leftJoin("pricing b", "a.id = b.relid")
        ->where("b.type", "configoptions")
    ...
    $pricing = \think\Db::name("product_config_options_sub")
        ->alias("a")
        ->leftJoin("pricing b", "a.id = b.relid")
        ->where("b.type", "configoptions")
        ->where("b.currency", $currency)
        ->where("a.config_id", $cid)
        ->where("a.qty_minimum", "<=", $quantity)
        ->where("a.qty_maximum", ">=", $quantity)
        ->find();
    ...
    if ($quantity > 0) {
        if ($pricing["qty_minimum"] > 1) {
            $sum = quantityStagePrice(
                $cid,
                $currency,
                intval($subs[$min - 1]["qty_maximum"]),
                $billingcycle,
                floatval($pricing[$price_type[0]]),
                floatval($pricing[$price_type[1]]),
            );
        } else {
...
{
    \think\Db::name("sendmsglimit")
        ->where("ip", get_client_ip6())
        ->where("time", "lt", strtotime(date("Y-m-d", time())))
        ->delete();
    $sendmsgtimes = 30;
    $sendmsgphone = 5;
    $number = \think\Db::name("sendmsglimit")
        ->where("ip", get_client_ip6())
        ->group("phone")
        ->where("time", "lt", strtotime(date("Y-m-d", strtotime("+1 day"))))
        ->count();
...
{
    $developer_app = \think\Db::name("products")
        ->field("type")
        ->where("id", $pid)
        ->where("hidden", 0)
        ->where("p_uid", ">", 0)
...
function unifyConfigOption()
{
    ...
    foreach ($gids as $gid) {
        ...
        if ($p_count == 0 || $p_count > 1) {
            \think\Db::name("product_config_groups")
                ->where("id", $gid)
                ->update(["global" => 1]);
        }
    }
...
{
    $data = [
        ["type" => 8, "title" => "发送验证码", "content" => "验证码{code}，勿泄漏于他人"],
        [
            "type" => "",
            "title" => "生成账单",
            "content" => "您好{name},您有一笔账单产生:账单号#{invoiceid},金额{total},请及时付款,以免账单失效。",
        ],
        [
            "type" => 2,
            "title" => "账单支付",
        ...
        ["type" => 5, "title" => "工单回复", "content" => "您提交的工单{subject}有新的回复,请注意查收."],
        [
            "type" => 6,
            "title" => "产品暂停",
            "content" =>
                "您好,您购买的产品{description}由于{description}的缘故,现已被暂停所有功能.如需恢复使用,请尽快处理.",
        ],
        [
            "type" => 7,
            "title" => "未支付账单",
            "content" => "您好,您已成功支付账单号#{invoiceid},账单金额{total}没有支付",
        ],
        [
            "type" => 9,
            "title" => "登录短信提醒",
            "content" =>
                "您好,您的账号{account}于{time}时间在以下{address}地址登录.如您未曾尝试登录,请立即更改登录密码,以防账号被盗.",
        ],
        ["type" => 10, "title" => "订单退款", "content" => "订单{order_id},金额{order_total_fee}已退款"],
        [
            "type" => 13,
            "title" => "订单支付提醒",
            "content" => "您的订单(编号{order_id})已经完成付款,付款金额为：{order_total_fee}",
        ],
        [
            "type" => 14,
            "title" => "订单未付款提醒",
            "content" =>
                "建议模板: 您购买的产品{product_name}(主机名{hostname})将于{product_end_time}到期.为了保证届时可以正常使用,请在产品到期之前先行续费",
        ],
        [
            "type" => 15,
            "title" => "自动生成续费账单提醒",
            "content" =>
                "您购买的产品{product_name}(主机名{hostname})将于{product_end_time}到期.为了保证届时可以正常使用,请在产品到期之前先行续费",
        ],
        [
            "type" => 16,
            "title" => "第三次逾期提醒",
            "content" =>
                "您在{product_first_time}订购的{product_name}产品(主机名：{hostname})支付尚未完成.暂时无法开通.为了避免订单过期,请您及时付款。",
        ],
        [
            "type" => 17,
            "title" => "第二次逾期提醒",
        ...
        [
            ...
            "title" => "下单提醒",
            "content" =>
                "您已成功下单{product_name}产品,为期{product_binlly_cycle}.请及时付款,以免订单失效.以下为账单信息产品名称:{product_name}产品单价:{product_price}付款周期:{product_binlly_cycle}订单创建时间:{order_create_time}",
        ],
        [
            "type" => 24,
            "title" => "产品开通提醒",
            "content" =>
                "您购买的产品{product_name}现已开通,感谢使用!ip地址:{product_mainip},操作系统用户名:{product_user},操作系统密码:{product_passwd},操作系统:{product_dcimbms_os},其他附加ip地址:{product_addonip},购买时间:{product_first_time},到期时间:{product_end_time},付款周期:{product_binlly_cycle}",
        ],
        [
            "type" => 26,
            "title" => "未续期产品删除提醒",
            "content" =>
                "您购买的产品{product_name}({hostname})由于未能在指定时间内续费,已于{product_terminate_time}自动删除.对因此而造成的不便我们表示歉意,希望您可以选择我们的其它产品.",
        ],
        [
            "type" => 27,
            "title" => "续费成功提醒",
            "content" => "您购买的产品({product_name})现已续费成功,服务将持续至{product_end_time}.感谢您对我们的信赖!",
        ],
        [
            "type" => "",
            "title" => "工单关闭提醒",
            "content" =>
                "您在{ticket_createtime}提交的工单{ticketnumber_tickettitle}由于长时间未获回复,已于{ticket_reply_time}自动关闭.",
        ],
        [
            "type" => 31,
            "title" => "工单开通提醒",
            "content" =>
                "我们已经收到您在{ticket_createtime}(时间)提交的工单:({ticketnumber_tickettitle}).团队将火速处理您的问题.请耐心等待.",
        ],
        [
            "type" => 32,
            "title" => "绑定提醒",
            "content" => "您的账号{username}与此{epw_type}:({epw_account})已成功进行绑定.如有疑问,请联系客服.",
        ],
        [
            "type" => 33,
            "title" => "注册成功",
            "content" =>
                "您已成功注册{system_companyname}账号,感谢您的使用.请完善账号个人信息并妥善保管.切勿向他人透漏登录密码!",
        ],
    ];
...
function checkDefaultProductGroup()
{
    ...
    if (empty($tmp)) {
        \think\Db::name("product_first_groups")->insert([
            "id" => 1,
            "name" => "默认分组",
            "hidden" => 0,
            "order" => 0,
...
function advancedConfigOptionFilter($config_id, $configoptions = [])
{
    ...
    $need_sub_id = \think\Db::name("product_config_options_sub")
    ...
    foreach ($configoptions as $k => $v) {
        ...
        $conditions = \think\Db::name("product_config_options_links")
            ->where("config_id", $k)
            ->where("type", "condition")
            ->where("relation_id", 0)
            ->withAttr("sub_id", function ($value) {
        ...
        foreach ($conditions as $kk => $vv) {
            ...
            $sub_id = $vv["sub_id"];
            $result = \think\Db::name("product_config_options_links")
                ->where("relation_id", $vv["id"])
                ->where("type", "result")
                ->withAttr("sub_id", function ($value) {
                    return json_decode($value, true);
                })
                                                ...
                                                foreach ($need_sub_id as $r4 => $s4) {
                                                    if ($rrr == $s4) {
                                                        if (judgequantity($need_option_type)) {
                                                            $need_sub_id_filter[] =
                                                                $sss["qty_minimum"] > 0 ? $sss["qty_minimum"] : 0;
                                                        } else {
                                            ...
                                            foreach ($result_sub_id4 as $r9 => $s9) {
                                                foreach ($need_sub_id as $r10 => $s10) {
                                                    if ($r9 == $s10) {
                                                        if (judgequantity($need_option_type)) {
                                                            $need_sub_id_filter[] =
                                                                $s9["qty_minimum"] > 0 ? $s9["qty_minimum"] : 0;
                                                        } else {
...
function userLevel($uid)
{
    ...
    $renew_times = \think\Db::name("invoices")
        ->alias("a")
        ...
        foreach ($rules as $rule) {
            ...
            if ($rule1 && $rule2 && $rule3 && $rule4) {
                ...
                if ($last_login_times_rule["min"] != 0 || $last_login_times_rule["max"] != 0) {
                    ...
                    $last_login_times = \think\Db::name("activity_log")
                        ->where("create_time", ">=", $login_day_before)
                        ->where("uid", $uid)
                        ->where("type", 1)
                        ->count();
                    if (
                        $last_login_times_rule["min"] < $last_login_times &&
                        $last_login_times <= $last_login_times_rule["max"]
                    ) {
                        $rule5 = true;
                    } else {
                        $rule5 = false;
                ...
                if ($rule5) {
                    ...
                    if ($last_renew_times_rule["min"] != 0 || $last_renew_times_rule["max"] != 0) {
                        ...
                        $last_renew_times = \think\Db::name("invoices")
                            ->alias("a")
                            ->leftJoin("invoice_items b", "a.id = b.invoice_id")
                            ->where("b.type", "renew")
                            ->where("b.uid", $uid)
                            ->where("a.status", "Paid")
                            ->where("paid_time", ">=", $renew_day_before)
                            ->count();
                        if (
                            $last_renew_times_rule["min"] < $last_renew_times &&
                            $last_renew_times <= $last_renew_times_rule["max"]
                        ) {
                            $rule6 = true;
                        } else {
                            $rule6 = false;
...
function ticketDeliver($id)
{
    ...
    $ticket = \think\Db::name("ticket_deliver")
        ->alias("a")
        ->leftJoin("ticket_deliver_products b", "b.tdid = a.id")
        ->leftJoin("ticket_deliver_department c", "c.tdid=b.tdid")
        ->leftJoin("ticket d", "d.dptid=c.dptid")
        ->leftJoin("host e", "e.id=d.host_id")
        ->leftJoin("products f", "f.id=e.productid")
        ->leftJoin("ticket_department_upstream g", "g.dptid=c.dptid AND f.zjmf_api_id=g.api_id")
        ->field(
            "f.zjmf_api_id,e.dcimid,d.title,d.content,a.mask_keywords,d.id,g.upstream_dptid,d.attachment,f.api_type,f.id as productid,d.priority",
        )
        ->where("e.productid=b.pid")
        ->where("e.dcimid", ">", 0)
        ->where("d.id", $id)
        ->find();
    if (empty($ticket)) {
    ...
    $token = [];
    $api = \think\Db::name("zjmf_finance_api")
        ->field("hostname,username,password,is_resource,ticket_open")
        ->where("id", $ticket["zjmf_api_id"])
        ->find();
    ...
    if ($api["is_resource"] == 1) {
        ...
        $login_data = [
            ...
            "password" => aespassworddecode($api["password"]),
            "type" => "agent",
        ];
    } else {
    ...
    if (!empty($ticket["attachment"])) {
        ...
        foreach ($ticket["attachment"] as $key => $value) {
            $res = curlUpload(
                $token["url"] . "/upload_image",
                config("ticket_attachments") . $value,
    ...
    $data = [
        "url" => $token["url"] . "/ticket/create",
        "data" => [
            "dptid" => intval($ticket["upstream_dptid"]),
            "hostid" => intval($ticket["dcimid"]),
            "title" => $ticket["title"],
    ...
    if ($res["status"] == 200) {
        \think\Db::name("ticket")
            ->where("id", $id)
            ->update(["upstream_tid" => $res["data"]["tid"], "is_deliver" => 1]);
        return true;
    } else {
        return false;
    }
...
function ticketReplyDeliver($id)
{
    ...
    $ticket_reply = \think\Db::name("ticket_reply")
        ->alias("a")
        ->leftJoin("ticket b", "b.id=a.tid")
        ->leftJoin("host c", "c.id=b.host_id")
        ->leftJoin("products d", "d.id=c.productid")
        ->field("d.zjmf_api_id,c.dcimid,a.content,a.id,b.upstream_tid,a.attachment,d.api_type,d.id as productid")
        ->where("b.upstream_tid", "<>", "")
        ->where("a.id", $id)
        ->find();
    ...
    $token = [];
    $api = \think\Db::name("zjmf_finance_api")
        ->field("hostname,username,password")
        ->where("id", $ticket_reply["zjmf_api_id"])
        ->find();
    $url = rtrim($api["hostname"], "/");
    $login_url = $url . "/zjmf_api_login";
    $login_data = ["username" => $api["username"], "password" => aespassworddecode($api["password"])];
    ...
    $data = [
        "url" => $token["url"] . "/ticket/reply",
        "data" => [
            "tid" => $ticket_reply["upstream_tid"],
...
function setTicketHandle()
{
    ...
    foreach ($tickets as $v) {
        ...
        if (!empty($ticket_reply)) {
            \think\Db::name("ticket")
                ->where("id", $v["id"])
                ->update(["handle" => $ticket_reply["admin_id"]]);
        }
    }
}
    ...
    if (!configuration("change_product_cycle_to_recurring")) {
        ...
        foreach ($products as $product) {
            ...
            if ($pay_type["pay_type"] == "day" || $pay_type["pay_type"] == "hour") {
                ...
                \think\Db::name("products")
                    ->where("id", $product["id"])
                    ->update(["pay_type" => $pay_type_new]);
            }
        }
...
function fieldsBatchUpdate($table, $arrayKey = [], $array2 = [], $fields = "id", $where = "")
{
    ...
    foreach ($arrayKey as $val) {
        ...
        foreach ($array2 as $k => $v) {
            $when .=
                " WHEN '" . $v[$fields] . "' THEN '" . (get_magic_quotes_gpc() ? $v[$val] : addslashes($v[$val])) . "'";
            if ($ke == 0) {
                $in_id .= "'" . $v[$fields] . "',";
            }
        }
...
function getPluginsList($module = "certification", $action = null)
{
    ...
    $plugins = \think\Db::name("plugin")
        ->field("id,name,title,config")
        ->where($where)
        ->order("order", "asc")
        ->order("id", "asc")
        ->select()
...
function CompatibleOldVersionThree()
{
    if (!configuration("certification_compatible_old")) {
        ...
        if (in_array("three", $arr)) {
            ...
            $threehc_config = [
                "module_name" => "华辰--三要素",
                "app_code" => configuration("certifi_appcode"),
                "type" => $certifi_three_type,
                "amount" => 0,
                "free" => 0,
            ];
            $data[] = [
                ...
                "config" => json_encode($threehc_config),
        ...
        if (in_array("phonethree", $arr)) {
            $phonethree_config = [
                "module_name" => "手机三要素",
                "app_code" => configuration("certifi_phonethree_appcode"),
                "amount" => 0,
                "free" => 0,
            ];
            $data[] = [
                ...
                "version" => "1.0",
                "description" => "手机三要素",
                "config" => json_encode($phonethree_config),
                "module" => "certification",
                "order" => 0,
            ];
        }
        if (in_array("ali", $arr)) {
            $ali_config = [
                "module_name" => "芝麻信用",
                "app_id" => configuration("certifi_app_id"),
                "biz_code" => configuration("certifi_alipay_biz_code"),
                "public_key" => configuration("certifi_alipay_public_key"),
                "private_key" => configuration("certifi_merchant_private_key"),
                "amount" => 0,
                "free" => 0,
            ];
            $data[] = [
        ...
        if (configuration("smsbao_account")) {
            $sms["smsbao"] = [
                "title" => "短信宝",
                "description" => "短信宝",
                "help_url" => "http://www.smsbao.com/",
                "config" => [
                    "user" => configuration("smsbao_account"),
                    "pass" => configuration("smsbao_pwd"),
                    "sign" => configuration("smsbao_mobile_signature"),
                ],
            ];
        }
        if (
            configuration("aliyun_mobile_accesskeyid") != "Your Ali Key ID" &&
            configuration("aliyun_mobile_accesskeyid")
        ) {
            $sms["aliyun"] = [
                "title" => "阿里云",
                "description" => "阿里云",
                "help_url" => "https://www.aliyun.com/product/sms",
                "config" => [
                    "AccessKeyId" => configuration("aliyun_mobile_accesskeyid"),
                    "AccessKeySecret" => configuration("aliyun_mobile_accesskeysecret"),
                    "SignName" => configuration("aliyun_mobile_signature"),
                ],
            ];
        }
        if (configuration("submail_app_id") != "Your Submail APP ID" && configuration("submail_app_id")) {
            $sms["submail"] = [
                "title" => "赛邮",
                "description" => "赛邮",
                "help_url" => "https://www.mysubmail.com/",
                "config" => [
                    "app_id" => configuration("submail_app_id"),
                    "app_key" => configuration("submail_app_key"),
                    "app_sign" => configuration("submail_sign_name"),
                    "international_app_id" => configuration("submail_international_app_id"),
                    "international_app_key" => configuration("submail_international_app_key"),
                    "international_app_sign" => configuration("submail_international_sign_name"),
                ],
            ];
        }
        $insertAll = [];
...
function pay($invoiceid)
{
    ...
    return $paymodal .
        "<a href=\"javascript: payamount({$invoiceid});\" class=\"text-primary mr-2\" id=\"{$invoiceid}\"><i class=\"fas fa-check-circle\"></i>支付</a>";
}
        ...
        return \think\Db::transaction(function () use ($table_name) {
            ...
            foreach ($model as $k => $v) {
                if (isset($ratio_model[$k]) && $v["message"] == $ratio_model[$k]["message"]) {
                    isset($replace_model[$k]) &&
                        \think\Db::name("email_templates")
                            ->where("id", $k)
                            ->update(["message" => $replace_model[$k]["message"]]);
                    continue;
                }
    ...
    try {
        return \think\Db::transaction(function () {
            ...
            foreach ($servers as $key => $val) {
                ...
                \think\Db::name("servers")
                    ->where("id", $val["id"])
                    ->update(["type" => $servers_group[$val["gid"]]["type"]]);
            }
        });
    } catch (Throwable $e) {
        return null;
...
function manualHostOld()
{
    ...
    try {
        ...
        foreach ($upper_reaches as $v) {
            ...
            \think\Db::name("upper_reaches_res")
                ->where("pid", $v["id"])
                ->update(["pid" => $id]);
            \think\Db::name("products")
                ->where("upper_reaches_id", $v["id"])
                ->update(["upper_reaches_id" => $id]);
        }
        \think\Db::commit();
        return true;
    } catch (Exception $e) {
...
function upgradeSmsTemplate()
{
    ...
    foreach ($data_install as $v) {
        $check = \think\Db::name("message_template")
            ->where("sms_operator", $now_sms_plugin)
            ->where("title", $v["type"])
            ->value("id");
        if ($check) {
            continue;
...
function upgradeEmilTemplate()
{
    $data = [
        [
            ...
            "type" => "product",
            "custom" => 0,
            "subject" => "[{SYSTEM_COMPANYNAME}]解除暂停成功，谢谢支持",
...
function updateConfigOptionUnit()
{
    ...
    array_walk($options, function ($value) use ($new_type) {
        \think\Db::name("product_config_options")
            ->where("option_type", $value)
            ->update(["unit" => $new_type[$value]]);
    });
    return true;
}
function addWebDefaultNav()
{
    ...
    $data = [
        [
            ...
            "",
            1,
            json_encode(["chinese" => "首页", "chinese_tw" => "首頁", "english" => "home page"]),
            "",
            2,
        ],
        [
            ...
            "",
            1,
            json_encode(["chinese" => "产品服务", "chinese_tw" => "產品服務", "english" => "Product service"]),
            "",
            2,
        ],
    ];
    ...
    $data2 = [
        ...
        "",
        1,
        json_encode(["chinese" => "服务支持", "chinese_tw" => "服務支援", "english" => "Service support"]),
        "",
        2,
    ];
    ...
    $data2_son = [
        [
            ...
            "",
            1,
            json_encode(["chinese" => "帮助中心", "chinese_tw" => "幫助中心", "english" => "Help center"]),
            "",
            2,
        ],
        [
            ...
            "",
            0,
            json_encode(["chinese" => "管家服务", "chinese_tw" => "管家服務", "english" => "butler service"]),
            1,
            "",
            "",
            2,
        ...
        [
            ...
            "",
            1,
            json_encode(["chinese" => "新闻资讯", "chinese_tw" => "新聞資訊", "english" => "News information"]),
            "",
            2,
        ],
    ];
    ...
    $data1 = [
        ...
        "",
        1,
        json_encode(["chinese" => "关于我们", "chinese_tw" => "關於我們", "english" => "About us"]),
        "",
        2,
    ];
    $about_son = [
        ...
        "",
        1,
        json_encode(["chinese" => "联系我们", "chinese_tw" => "聯繫我們", "english" => "contact us"]),
        "",
        2,
    ];
...
function addWebFootDefaultNav()
{
    ...
    $data1 = [
        ...
        "",
        1,
        json_encode(["chinese" => "主要产品", "chinese_tw" => "主要產品", "english" => "main products"]),
        "",
        3,
    ];
    ...
    $data1_son = [
        [
            ...
            "",
            1,
            json_encode(["chinese" => "云服务器", "chinese_tw" => "雲服務器", "english" => "Cloud server"]),
            "",
            3,
        ],
        [
            ...
            "",
            1,
            json_encode(["chinese" => "独立服务器", "chinese_tw" => "獨立服務器", "english" => "Stand alone server"]),
            "",
            3,
        ],
        [
    ...
    $data3 = [
        ...
        "",
        1,
        json_encode(["chinese" => "其他", "chinese_tw" => "其他", "english" => "other"]),
        "",
        3,
    ];
    ...
    $data3_son = [
        ...
        [
            ...
            "",
            1,
            json_encode(["chinese" => "服务条款", "chinese_tw" => "服務條款", "english" => "Terms of service"]),
            "",
            3,
        ],
        [
            ...
            "",
            1,
            json_encode(["chinese" => "隐私政策", "chinese_tw" => "隱私政策", "english" => "Privacy policy"]),
            "",
            3,
        ],
    ];
...
function createWebDefaultMenu()
{
    ...
    \think\Db::name("menu_active")
        ->where("id", 2)
        ->update(["menuid" => $www_top_id]);
    \think\Db::name("menu_active")
        ->where("id", 3)
        ->update(["menuid" => $www_bom_id]);
}
/**
 * 会员中心导航，产品 高级设置 默认值
 * @return array|int|string
...
function editEmailTplNullLink()
{
    return \think\Db::transaction(function () {
        $str =
            "<span style=\"margin: 0; padding: 0; display: inline-block; margin-top: 55px;\">查看订单详情：<span style=\"color: blue;\">链接</span></span>";
        $str = htmlspecialchars($str);
        $data = \think\Db::name("email_templates")
            ->whereLike("message", "%" . $str . "%")
            ->cursor();
        foreach ($data as $key => $val) {
            if (strpos($val["message"], $str) === false) {
                continue;
...
{
    $template = \think\Db::name("message_template_link")
        ->field("mt.*")
        ->alias("mtl")
        ->leftJoin("message_template mt", "mt.id = mtl.sms_temp_id")
        ->where("mtl.type", 8)
        ->select()
        ->toArray();
...
function gateway_list_openapi($module = "gateways", $status = 1)
{
    ...
    $rows = \think\Db::name("plugin")
        ->field("id,name,title,url,author_url")
        ->where($where)
        ->withAttr("url", function ($value, $data) {
            return "upload/pay/" . $data["name"] . ".png";
        })
