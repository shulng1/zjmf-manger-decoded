...
class DownloadsModel extends \think\Model
{
    ...
    {
        $data = \think\Db::name("downloads")
            ->field("id,category,type,title,description,downloads")
    ...
    public static function getAllowDownListHome($cate_id = 0, $uid = 0)
    {
        ...
        $downloads = \think\Db::name("downloads")
            ->field(
                "id,category,type,title,description,downloads,location,locationname,clientsonly,update_time,create_time",
            )
            ->where("hidden", 0)
            ->where("category", $cate_id)
        ...
        foreach ($downloads as $key => $val) {
            ...
            $downloads[$key]["update_time"] = date(
                "Y-m-d H:i:s",
                $downloads[$key]["create_time"] ?: $downloads[$key]["update_time"],
            );
    ...
    public static function getAssociatedfiles($pid)
    {
        $data = \think\Db::name("downloads")
            ->alias("d")
            ->field("d.id,d.title")
            ->leftJoin("product_downloads pl", "pl.download_id=d.id")
    ...
    {
        $downloads = \think\Db::name("downloads")
            ->field("id,type,title,description,downloads")
