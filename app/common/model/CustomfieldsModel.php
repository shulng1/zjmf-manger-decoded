...
class CustomfieldsModel extends \think\Model
{
    ...
    public function getCustomValue($relid, $value_relid, $type = "ticket", $user_type = "admin")
    {
        ...
        $data = \think\Db::name("customfields")
            ->field("a.id,a.fieldname,a.fieldtype,a.description,a.fieldoptions,a.required,a.regexpr,b.value,b.id vid")
            ->alias("a")
            ->leftJoin("customfieldsvalues b", "a.id=b.fieldid AND b.relid=" . $value_relid)
            ->where("a.relid", $relid)
            ->where("a.type", $type);
        if ($user_type != "admin") {
        ...
        if (!empty($update)) {
            ...
            foreach ($data as $v) {
                if (isset($update[$v["id"]])) {
                    if (
                        !empty($v["regexpr"]) &&
                        !preg_match("/" . str_replace("/", "\\/", $v["regexpr"]) . "/", $update[$v["id"]])
                    ) {
                        continue;
                    }
                    ...
                    $hook_res = hook("custom_field_save", [
                        "fieldid" => $v["id"],
                        "relid" => $value_relid,
                        "value" => $update[$v["id"]],
                    ]);
                    ...
                    if ($v["vid"] > 0) {
                        \think\Db::name("customfieldsvalues")
                            ->where("id", $v["vid"])
                            ->update(["value" => $update[$v["id"]], "update_time" => $time]);
                    } else {
    ...
    public function check($customfields, $value = [])
    {
        foreach ($customfields as $k => $v) {
            ...
            if (
                $value[$v["id"]] &&
                !empty($v["regexpr"]) &&
                !preg_match("/" . str_replace("/", "\\/", $v["regexpr"]) . "/", $value[$v["id"]])
            ) {
