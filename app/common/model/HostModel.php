...
class HostModel extends \think\Model
{
    public function getProvisionParams($hostid = 0)
    {
        $data = \think\Db::name("host")
            ->field(
                "a.id hostid,a.productid,a.uid,a.serverid,a.regdate,a.domain,a.payment,a.amount,a.billingcycle,
					...
					c.hostname server_host,c.username server_username,c.password server_password,c.accesshash,c.secure,c.port,c.type module_type",
            )
            ->alias("a")
        ...
        $config_options = \think\Db::name("host_config_options")
            ...
            ->leftJoin(
                "product_config_options_sub c",
                "a.configid=c.config_id and a.optionid=c.id and b.option_type!=4",
            )
    ...
    public function getConfigOption($host, $key)
    {
        ...
        $res = \think\Db::name("host_config_options")
            ->field("a.id,a.qty,a.configid,a.optionid,b.option_name,b.option_type,b.upgrade,c.option_name sub_option")
            ->alias("a")
            ->leftJoin("product_config_options b", "a.configid=b.id")
            ->leftJoin("product_config_options_sub c", "a.configid=c.config_id and a.optionid=c.id")
            ->where("a.relid", $host)
            ->whereLike("b.option_name", $key . "|%")
            ->find();
        if (empty($res)) {
            return [];
        }
