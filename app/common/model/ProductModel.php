...
class ProductModel extends \think\Model
{
    ...
    {
        $data = \think\Db::name("product_config_links")
            ->alias("a")
            ->field("b.*")
        ...
        if (!empty($data)) {
            ...
            $sub = \think\Db::name("product_config_options_sub")
                ->alias("a")
                ->field("a.*" . $price_type_field)
                ->leftJoin("pricing b", "b.type=\"configoptions\" and a.id=b.relid and b.currency=" . $currency)
                ->whereIn("a.config_id", $config_id)
                ...
                foreach ($sub as $kk => $vv) {
                    if ($v["id"] == $vv["config_id"]) {
                        $data[$k]["sub"][$vv["id"]] = [
                            ...
                            "price_setup" => $vv[$price_type[1]],
                            "price_cycle" => $vv[$price_type[0]],
                            "qty_minimum" => $vv["qty_minimum"],
                            "qty_maximum" => $vv["qty_maximum"],
                            "hidden" => $v["hidden"],
                        ];
                    }
                }
    ...
    public function getProductCycle(
        ...
        if (
            !empty($pay_type["pay_ontrial_status"]) &&
            isset($pricing["ontrial"]) &&
            $pricing["ontrial"] >= 0 &&
            !$upgrade
        ) {
            ...
            foreach ($cycle_filter as &$v) {
                ...
                if ($flag) {
                    if ($flag["type"] == 1) {
                        ...
                        $bates = $flag["bates"];
                        $v["price"] =
                            $v["price"] + $config_total - $bates > 0 ? $v["price"] + $config_total - $bates : 0;
                    }
    ...
    public function syncProductInfo(
        ...
        if ($api_type == "zjmf_api") {
            ...
            if ($upstream_res["status"] == 200) {
                ...
                if (!$change_to_percent || $change_pid || $force) {
                    if (
                        (!empty($upstream_product) &&
                            $product["upstream_version"] < $upstream_product["location_version"]) ||
                        $change_pid ||
                        $force
                    ) {
        ...
        try {
            ...
            if (
                (!empty($upstream_product) && $product["upstream_version"] < $upstream_product["location_version"]) ||
                $change_to_percent ||
                $change_pid ||
                $force
            ) {
                                    ...
                                    foreach ($pricings as $pricing) {
                                        if ($pricing["code"] == $currency["code"]) {
                                            ...
                                            if (
                                                ...
                                                foreach ($price_type as $v) {
                                                    $pricing[$v[0]] =
                                                        ($pricing[$v[0]] * $upstream_product["upstream_price_value"]) /
                                                        100;
                                                    $pricing[$v[1]] =
                                                        ($pricing[$v[1]] * $upstream_product["upstream_price_value"]) /
                                                        100;
                                                }
                                            }
    ...
    public function syncProductInfoToResource(
        $id,
        $api_type,
        $zjmf_finance_api_id,
        $upstream_pid,
    ...
    public function handleLingAge($lingAgeArr)
    {
        ...
        $option_arr = \think\Db::name("product_config_options")
            ->whereIn("id", $lingAgeArr)
            ->where("option_type", 20)
            ->select()
            ->toArray();
        if (!$option_arr) {
            return null;
        ...
        $sub_arr = \think\Db::name("product_config_options_sub")
            ->whereIn("config_id", $option_ids)
            ->select()
            ->toArray();
        if (!$sub_arr) {
            return null;
    ...
    public function handleSubLevel($tree, $linkage_pid = 0, $linkage_top_pid = 0, $linkage_level = "")
    {
        foreach ($tree as $k => $v) {
            $data = [
                "linkage_pid" => $v["linkage_pid"] == 0 ? 0 : $linkage_pid,
                "linkage_top_pid" => $linkage_pid == 0 ? 0 : $linkage_top_pid,
                "linkage_level" => $linkage_level ? $linkage_level . "-" . $v["id"] : "0-" . $v["id"],
            ];
