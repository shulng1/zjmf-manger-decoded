...
class SeniorConfModel
{
    ...
    {
        $use_conf_ids = \think\Db::name("product_config_options")
            ->alias("pco")
            ->leftJoin("product_config_links pcl", "pcl.gid = pco.gid")
            ->where("pcl.pid", $product_id)
            ->where("pco.hidden", 0)
            ->column("pco.id");
    ...
    {
        $res_data = \think\Db::name("product_config_options_links")
            ->field("id,config_id,relation,sub_id")
            ->whereIn("config_id", $config_id)
    ...
    {
        $res_data = \think\Db::name("product_config_options_links")
            ->field("id,config_id,relation,sub_id,relation_id")
            ->whereIn("config_id", $config_id)
            ->where("type", "result")
            ->withAttr("sub_id", function ($value) {
                return json_decode($value, true);
