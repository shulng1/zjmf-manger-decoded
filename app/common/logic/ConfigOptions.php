...
class ConfigOptions
{
    ...
    public function getConfigInfo($pid, $admin = false)
    {
        ...
        $configgroups = \think\Db::name("products")
            ->alias("p")
            ->field(
                "pcg.id as pcgid,p.id as pid,pcg.name,pcg.description,pco.id,pco.is_discount,pco.option_name,pco.linkage_pid,pco.linkage_top_pid,pco.option_type,pco.qty_minimum,pco.qty_maximum,pco.order,pco.hidden,pco.upstream_id,pco.upgrade,pco.auto,pco.notes,pco.qty_stage,pco.unit",
            )
            ->join("product_config_links pcl", "pcl.pid = p.id")
            ->join("product_config_groups pcg", "pcg.id = pcl.gid")
            ->leftJoin("product_config_options pco", "pco.gid = pcg.id")
            ->where("p.id", $pid)
            ->where(function (\think\db\Query $query) use ($admin) {
                if (!$admin) {
        ...
        $product = \think\Db::name("products")
            ->field("api_type,upstream_price_type,upstream_price_value")
            ->where("id", $pid)
            ->find();
        $alloption = [];
                    ...
                    if ($option["option_type"] == 5) {
                        foreach ($suboptions as $subkey => $suboption) {
                            if (!empty($suboption)) {
                                ...
                                $original_name = $suboption["option_name"] = explode("|", $suboption_name)[1]
                                    ? explode("|", $suboption_name)[1]
                                    : $suboption_name;
                                $suboption["option_name_first"] = explode("|", $suboption_name)[1]
                                    ? explode("|", $suboption_name)[0]
                                    : $suboption_name;
                    ...
                    if ($billingcycle != "free") {
                        foreach ($pricing as $s => $t) {
                            if ($t["currency"] == $user_currcy_id) {
                                if ($flags) {
                                    $t[$price_type[0]] = bcsub(
                                        $t[$price_type[0]],
                                        bcmul($t[$price_type[0]], $bates, 2),
                                        2,
                                    );
                                }
                                if ($t[$price_type[0]] > 0) {
    ...
    public function showInfo($pid, $host_id, $currency, $billingcycle, $show_price = true)
    {
        ...
        $configgroups = \think\Db::name("products")
            ...
            ->join(["shd_product_config_links" => "pcl"], "pcl.pid = p.id")
            ->join(["shd_product_config_groups" => "pcg"], "pcg.id = pcl.gid")
            ->where("p.id", $pid)
            ->select()
            ->toArray();
        ...
        foreach ($configgroups as $ckey => $configgroup) {
            if (!empty($configgroup)) {
                ...
                foreach ($options as $okey => $option) {
                    ...
                    if (!empty($option)) {
                        $cid = $option["id"];
                        $host_config_data = \think\Db::name("host_config_options")
                            ->where("configid", $cid)
                            ->where("relid", $host_id)
                            ->find();
                        $suboptions = [];
                        ...
                        $config_show[$cid]["name"] = explode("|", $option["option_name"])[1]
                            ? explode("|", $option["option_name"])[1]
                            : $option["option_name"];
                        $config_show[$cid]["name_k"] = explode("|", $option["option_name"])[0]
                            ? explode("|", $option["option_name"])[0]
                            : $option["option_name"];
                        ...
                        if ($option["option_type"] == 3) {
                            ...
                            if (judgeQuantity($option["option_type"])) {
                                ...
                                $config_show[$cid]["sub_name"] = explode("|", $suboptions["option_name"])[1]
    ...
    public function updateConfig($hostid, $configoption, $new_productid = 0, $product_info = [])
    {
        ...
        if ($pid != $new_productid && $new_productid > 0) {
            ...
            $new_config_links = \think\Db::name("product_config_links")
                ->where("pid", $new_productid)
                ->select()
                ->toArray();
            $intersect = array_intersect(array_column($config_links, "gid"), array_column($new_config_links, "gid"));
            if (count($intersect) > 0) {
                $old_config_options = \think\Db::name("product_config_options")
                    ->alias("o")
                    ->field("o_sub.config_id,o_sub.id")
                    ->leftJoin("product_config_options_sub o_sub", "o_sub.config_id=o.id")
                    ->whereIn("o.gid", $intersect)
                    ->select()
                    ->toArray();
            ...
            $configgroups = \think\Db::name("products")
                ->alias("p")
                ->field("pcg.id,pcg.name,pcg.description,p.type")
                ->join(["shd_product_config_links" => "pcl"], "pcl.pid = p.id")
                ->join(["shd_product_config_groups" => "pcg"], "pcg.id = pcl.gid")
                ->where("p.id", $pid)
                ->select()
                    ...
                    foreach ($options as $okey => $option) {
                        ...
                        if (!empty($option)) {
                            ...
                            if (judgeQuantity($option_type)) {
                                ...
                                $config_value =
                                    $config_value <= $option_qty_minimum ? $option_qty_minimum : $config_value;
                                $config_value =
                                    $option_qty_maximum <= $config_value ? $option_qty_maximum : $config_value;
                                $suboptions = \think\Db::name("product_config_options_sub")
                                    ->where("qty_minimum", "<=", $config_value)
                                    ->where("qty_maximum", ">=", $config_value)
                                    ->where("config_id", $cid)
                                    ->order("sort_order asc")
                                    ->order("id asc")
                                    ->find();
    ...
    private function saveHostConfig($hostid, $cid, $suboptions_id, $qty = 0)
    {
        ...
        if (!empty($exists_data)) {
            \think\Db::name("host_config_options")
                ->where("relid", $hostid)
                ->where("configid", $cid)
                ->update(["optionid" => $suboptions_id, "qty" => $qty]);
        } else {
    ...
    public function getLinkAgeList($req)
    {
        ...
        if ($req->sub_id) {
            ...
            if ($sub_data) {
                ...
                $config_id = \think\Db::name("product_config_options_sub")
                    ->whereIn("id", $sub_data_arr)
                    ->whereOr("linkage_pid", "in", $sub_data_arr)
                    ->column("config_id");
        ...
        $linkage_sub = \think\Db::name("product_config_options_sub")
            ->whereIn("config_id", $all_linkage_ids)
            ->order("sort_order", "asc")
            ->cursor();
    ...
    public function setLinkAgeListDefaultVal($list, $param)
    {
        ...
        if ($check_data && $check_data["linkage_pid"] != 0) {
            $check_data = \think\Db::name("product_config_options")
                ->where("id", $check_data["linkage_top_pid"])
                ->find();
        }
        if (!$check_data) {
            return $list;
        ...
        $sub_data_val = \think\Db::name("product_config_options_sub")
    ...
    public function webGetLinkAgeList($req)
    {
        ...
        if ($sub_data) {
            ...
            $config_id = \think\Db::name("product_config_options_sub")
                ->whereIn("id", $sub_data)
                ->whereOr("linkage_top_pid", "in", $sub_data)
                ->column("config_id");
