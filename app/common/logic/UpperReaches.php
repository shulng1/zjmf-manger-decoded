    ...
    public function dcimClientReinstallStatus($re)
    {
        ...
        if ($res["status"] == "success") {
            ...
            if (!empty($res["sales"]["osusername"])) {
                \think\Db::name("host")
                    ->where("id", $re["hid"])
                    ->update(["username" => $res["sales"]["osusername"]]);
                \think\Db::name("upper_reaches_res")
                    ->where("id", $re["id"])
                    ->update(["username" => $res["sales"]["osusername"]]);
            }
            if (!empty($res["sales"]["ospassword"])) {
                \think\Db::name("host")
                    ->where("id", $re["hid"])
                    ->update(["password" => cmf_encrypt($res["sales"]["ospassword"])]);
                \think\Db::name("upper_reaches_res")
                    ->where("id", $re["id"])
                    ->update(["password" => $res["sales"]["ospassword"]]);
            }
        } else {
            $result["status"] = 400;
