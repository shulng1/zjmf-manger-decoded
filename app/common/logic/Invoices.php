...
class Invoices
{
    ...
    public function createRenew($hostid, $pay = 0, $is_cron = false)
    {
        ...
        $host_data = \think\Db::name("host")
            ->field("i.status,h.domain,h.dedicatedip,h.nextduedate,p.name as productname,i.id as invoice_id")
            ->alias("h")
        ...
        if (!empty($host_data) && $host_data["status"] == "Unpaid") {
            ...
            $host_data = \think\Db::name("host")
                ->field("h.domain,h.nextduedate,h.dedicatedip,p.name as productname")
                ->alias("h")
                ->leftJoin("products p", "p.id=h.productid")
                ->where("h.id", $hostid)
                ->find();
        }
        $cron_renew_invoices = \think\Db::name("host")
            ->field("i.status,h.domain,h.dedicatedip,h.nextduedate,p.name as productname,i.id as invoice_id")
            ->alias("h")
            ->leftJoin("products p", "h.productid = p.id")
            ->leftJoin("invoice_items ii", "ii.rel_id = h.id")
            ->leftJoin("invoices i", "ii.invoice_id = i.id")
            ->where("h.id", $hostid)
        ...
        $renew_invoices = \think\Db::name("host")
            ->field("i.status,h.domain,h.dedicatedip,h.nextduedate,p.name as productname,i.id as invoice_id")
            ->alias("h")
            ->leftJoin("products p", "h.productid = p.id")
            ->leftJoin("invoice_items ii", "ii.rel_id = h.id")
            ...
            ->where("i.type", "renew")
            ->where("i.delete_time", 0)
            ->where(function (\think\db\Query $query) use ($is_cron) {
                if ($is_cron) {
        ...
        $renew_items = [
            ...
            "new_recurring_amount" => $host["amount"],
            "status" => "Pending",
            "paid" => "N",
            "create_time" => time(),
            "expire_time" => $next_time,
        ];
        ...
        try {
            ...
            $end_time = date(
                ...
                getNextTime(
                    ...
                    $host_data["nextduedate"],
                    $pay_type["pay_ontrial_cycle_type"] ?: "day",
                ),
            );
            $dec =
                "续费账单 - " .
                $host_data["productname"] .
                "(" .
                $host_data["domain"] .
            ...
            $r2 = \think\Db::name("invoice_items")->insertGetId([
                "invoice_id" => $r1,
                "uid" => $host["uid"],
                "type" => "renew",
                "rel_id" => $hostid,
        ...
        if ($result["status"] == 200) {
            ...
            $client = check_type_is_use(
                $message_template_type[strtolower("renew_product_reminder")],
                $host["uid"],
                $sms,
            );
            if ($client) {
    ...
    {
        $info = \think\Db::name("invoices")
            ->alias("a")
            ->field(
                "a.id,a.uid,a.subtotal,a.total,b.username,b.email,b.credit,d.billingcycle,d.nextduedate,d.domainstatus,d.id hostid,e.pay_type,e.server_type",
            )
            ->field("d.dcimid,e.zjmf_api_id")
            ->leftJoin("clients b", "a.uid=b.id")
            ->leftJoin("invoice_items c", "a.id=c.invoice_id")
            ->leftJoin("host d", "c.rel_id=d.id")
        ...
        $nextduedate = getNextTime(
            $info["billingcycle"],
            $pay_type["pay_" . $info["billingcycle"] . "_cycle"],
            $info["nextduedate"],
            $pay_type["pay_ontrial_cycle_type"] ?: "day",
        );
        if ($info["subtotal"] == 0) {
            \think\Db::name("invoices")
                ->where("id", $invoice_id)
                ->where("delete_time", 0)
                ->update(["paid_time" => time(), "status" => "Paid", "update_time" => time(), "is_delete" => 0]);
            \think\Db::name("host")
                ->where("id", $info["hostid"])
                ->update(["nextduedate" => $nextduedate, "nextinvoicedate" => $nextduedate]);
            $result["status"] = 200;
            ...
            if ($info["credit"] >= $info["subtotal"] - $info["total"]) {
                \think\Db::startTrans();
                try {
                    $r1 = \think\Db::name("clients")
                        ->where("id", $info["uid"])
                        ->where("credit", ">=", $info["total"])
                        ->setDec("credit", $info["total"]);
                    $r2 = \think\Db::name("invoices")
                        ->where("id", $invoice_id)
                        ->where("delete_time", 0)
                        ->update([
                            "paid_time" => time(),
                            "status" => "Paid",
                            "update_time" => time(),
                            "credit" => $info["subtotal"] - $info["total"],
                            "is_delete" => 0,
                        ]);
                    \think\Db::name("host")
                        ->where("id", $info["hostid"])
        ...
        if ($result["status"] == 200) {
            \think\Db::name("invoices")
                ->where("id", $invoice_id)
                ->update(["credit" => $info["total"]]);
            \credit_log([
                "uid" => $info["uid"],
                "desc" => "Credit Applied to Renew Invoice #" . $invoice_id,
                "amount" => $info["total"],
            ]);
            ...
            if ($info["domainstatus"] == "Suspended" && !!getConfig("cron_host_unsuspend")) {
                ...
                if ($result["status"] == 200) {
                    ...
                    $data_i[
                        "description"
                    ] = " 余额支付续费账单 - 解除暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                    if ($is_zjmf) {
    ...
    public function processPaidInvoiceFinal($invoiceid, $email = true)
    {
        ...
        $invoice_item_data = \think\Db::name("invoice_items")
        ...
        foreach ($invoice_item_data as $key => $val) {
            ...
            if ($type == "host") {
                $host = \think\Db::name("host")
                    ->alias("a")
                    ->field(
                        "a.id,a.uid,a.orderid,o.amount,a.productid,a.domainstatus,a.nextduedate,a.regdate,b.welcome_email,b.type,c.email,a.billingcycle,b.pay_type,b.api_type",
                    )
                ...
                if (!empty($host)) {
                    ...
                    if ($host["billingcycle"] == "onetime" || $host["billingcycle"] == "free") {
                        \think\Db::name("host")
                            ->where("id", $host["id"])
                            ->update(["nextduedate" => 0, "nextinvoicedate" => 0, "regdate" => time()]);
                    } else {
                        $nextduedate = getNextTime(
                            $host["billingcycle"],
                            $pay_type["pay_" . $host["billingcycle"] . "_cycle"],
                            time(),
                            $pay_type["pay_ontrial_cycle_type"] ?: "day",
                        );
                        \think\Db::name("host")
                ...
                if ($client) {
                    ...
                    $arr_sms = [
                        "name" => $message_template_type[strtolower("Invoice_Payment_Confirmation")],
                ...
                if (get_product_condition($productid) == "payment") {
                    if (configuration("shd_allow_auto_create_queue")) {
                        ...
                        $curl_multi_data[count($curl_multi_data)] = [
                            "url" => "async_create",
                            "data" => ["hid" => $relid, "is_admin" => $this->is_admin, "ip" => get_client_ip6()],
                        ];
                    }
                }
                if ($host["domainstatus"] == "Suspended") {
                    ...
                    if ($result["status"] == 200) {
                        ...
                        $data_i[
                            "description"
                        ] = " 合并账单支付成功 - 解除暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                        if ($is_zjmf) {
                            $logic_run_map->saveMap($data_i, 0, 400, 3);
                        }
                        if (!$is_zjmf) {
                ...
                credit_log([
                    "uid" => $uid,
                    "desc" => "Add Funds Invoice #" . $invoiceid,
                    "amount" => $amount,
                    "relid" => $invoiceid,
                ...
                if ($invoice_data["credit_limit_prepayment"] == 1) {
                    $credit_limit_prepayment_invoices = json_decode(
                        $invoice_data["credit_limit_prepayment_invoices"],
                        true,
                    );
                    \think\Db::name("invoices")
                        ->whereIn("id", $credit_limit_prepayment_invoices)
                        ->update(["invoice_id" => $invoice_data["id"]]);
                }
                ...
                $host = \think\Db::name("host")
                    ...
                    ->where("b.domainstatus", "Suspended")
                    ->where("d.invoice_id", $invoice_data["id"])
                    ->select()
                    ->toArray();
                foreach ($host as $hv) {
                    $Host = new Host();
                    ...
                    if ($result["status"] == 200) {
                        ...
                        $data_i[
                            "description"
                        ] = " 账单支付成功 - 解除暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                        if ($is_zjmf) {
                            $logic_run_map->saveMap($data_i, 0, 400, 3);
                        }
                        if (!$is_zjmf) {
                ...
                \think\Db::name("invoices")
                    ->where("id", $relid)
                    ->update(["status" => "Paid", "paid_time" => time(), "update_time" => time(), "is_delete" => 0]);
                credit_log([
                    "uid" => $uid,
                    "desc" => "Credit Applied Invoice #" . $relid,
                    "amount" => $amount,
                    "relid" => $relid,
                ]);
                $this->processPaidInvoice($relid);
            } elseif ($type == "voucher") {
                \think\Db::name("voucher")
                    ->where("id", $relid)
                    ->update(["status" => "Pending", "update_time" => time()]);
            } elseif ($type == "contract") {
                \think\Db::name("contract_pdf")
                    ->where("id", $relid)
                    ->update(["status" => 3]);
            } elseif ($type == "user_grade") {
                \think\Db::name("agent")
                    ->where("uid", $uid)
                    ->update(["user_grade" => $relid]);
            }
        }
        if ($curl_multi_data) {
    ...
    public function creditRefund($invoiceid)
    {
        ...
        $apply = \think\Db::name("credit")
            ->where("description", "like", "%Credit Applied to Invoice #%")
            ->where("relid", $invoiceid)
            ->sum("amount");
        if ($invoice["type"] == "renew") {
            $renew_apply = \think\Db::name("credit")
                ->where("description", "Credit Applied to Renew Invoice #{$invoiceid}")
                ->sum("amount");
        }
        $remove = \think\Db::name("credit")
            ->where("description", "like", "%Credit Removed from Invoice #%")
            ->where("relid", $invoiceid)
            ->sum("amount");
