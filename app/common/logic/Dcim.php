...
class Dcim
{
    ...
    public function setUrlByHost($hostid)
    {
        if (!empty($hostid)) {
            $server_info = \think\Db::name("host")
                ->alias("a")
                ->field("b.id,b.hostname,b.username,b.password,b.secure,b.port,b.accesshash")
                ->leftJoin("servers b", "a.serverid=b.id")
                ->where("a.id", $hostid)
                ->where("b.server_type", "dcim")
                ->find();
            if (empty($server_info)) {
                $this->error = true;
            } else {
    ...
    public function getFormatOs()
    {
        ...
        if ($res["status"] == "success") {
            ...
            foreach ($res["data"]["os"] as $v) {
                $data[] =
                    $v["id"] . "|" . ($svg[$os_group[$v["group_id"]]] ?? $os_name[$v["group_id"]]) . "^" . $v["name"];
            }
        }
        return $data;
    }
    public function on($id, $log = true)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.uid,b.config_option1,b.api_type,b.zjmf_api_id,b.id as productid")
        ...
        if ($product["api_type"] == "whmcs") {
            $dcimid = \think\Db::name("customfieldsvalues")
                ...
                ->where("b.relid", $product["productid"])
    ...
    public function traffic($id, $params = [])
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.uid,b.config_option1,b.api_type,b.zjmf_api_id,b.id productid")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
            ->find();
    ...
    public function kvm($id)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.uid,b.config_option1,b.api_type,b.zjmf_api_id")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
            ->find();
    ...
    public function novnc($id, $restart = false, $is_common = false)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.password,b.config_option1,b.api_type,b.zjmf_api_id,b.id as productid")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
            ->find();
    ...
    public function rescue($id, $system = 1)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($this->is_admin) {
                ...
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("发起救援系统失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
                $result["msg"] = "救援系统发起失败";
    ...
    public function crackPass($id, $params = [])
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.serverid,a.dcimid,a.show_last_act_message,a.uid,b.config_option1,b.api_type,b.zjmf_api_id,b.password,a.productid",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
        ...
        if ($res["status"] == "success") {
            ...
            if ($this->is_admin) {
                ...
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("发起重置密码失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
                $result["msg"] = "发起重置密码失败";
    ...
    public function reinstall($id, $params = [])
    {
        ...
        $product = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.uid,a.serverid,a.productid,a.dcimid,a.reinstall_info,a.show_last_act_message,a.os os_name,b.config_option1,b.api_type,b.zjmf_api_id,b.upstream_price_type,b.upstream_price_value,c.reinstall_times,c.os",
            )
            ->leftJoin("products b", "a.productid=b.id")
        ...
        $r = \think\Db::name("product_config_options_sub")
            ...
            ->where("c.pid", $product["productid"])
            ->where("a.id", $params["mos"])
            ->find();
            ...
            if ($result["status"] == 200) {
                ...
                if ($result["status"] == 400 && !$result["confirm"] && !isset($result["price"])) {
                    if ($this->is_admin) {
                        $result["msg"] = $this->log_prefix_error . $result["msg"];
                        $description =
                            $this->log_prefix_error .
                            sprintf("重装系统发起失败,原因:%s - Host ID:%d", $result["msg"], $id);
                    } else {
                        $result["msg"] = "重装系统发起失败";
            ...
            if ($product["config_option1"] == "rent") {
                ...
                if ($res["status"] == "success") {
                    ...
                    if ($this->is_admin) {
                        ...
                        $description =
                            $this->log_prefix_error .
                            sprintf("重装系统发起失败,原因:%s - Host ID:%d", $res["msg"], $id);
                    } else {
                        $result["msg"] = "重装系统发起失败";
                        $description = sprintf("重装系统发起失败 - Host ID:%d", $id);
                    }
    ...
    public function getMirrorOsConfig($id)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.reinstall_info,b.config_option1,a.uid,c.reinstall_times")
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("dcim_servers c", "a.serverid=c.serverid")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
        ...
        if ($product["config_option1"] == "rent") {
            ...
            if ($res["status"] == "success") {
                ...
                if ($this->is_admin) {
                    $result["msg"] = $this->log_prefix_error . $result["msg"];
                    $description =
                        $this->log_prefix_error . sprintf("重装解除暂停失败,原因:%s - Host ID:%d", $res["msg"], $id);
                } else {
    ...
    public function cancelReinstall($id)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.serverid,a.dcimid,a.reinstall_info,a.uid,b.config_option1,b.api_type,b.zjmf_api_id,c.reinstall_times",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("dcim_servers c", "a.serverid=c.serverid")
            ->where("b.type", "dcim")
        ...
        if ($res["status"] == "success") {
            ...
            if ($this->is_admin) {
                ...
                $description =
                    $this->log_prefix_error .
                    sprintf("取消%s失败,原因:%s - Host ID:%d", $des[$task_type], $res["msg"], $id);
            } else {
    ...
    public function reinstallStatus($id)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.serverid,a.dcimid,a.show_last_act_message,a.domainstatus,a.uid,b.config_option1,b.api_type,b.zjmf_api_id",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
            ...
            if ($product["config_option1"] == "rent") {
                ...
                if ($res["status"] == "success") {
                    if (!empty($res["data"])) {
                        ...
                        $result["data"] = [];
                        if (
                            isset($res["last_result"]) &&
                            $product["show_last_act_message"] == 1 &&
                            ($res["last_result"]["type"] == 1 || $res["last_result"]["type"] == 2)
                        ) {
    ...
    public function free($id)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,b.config_option1,a.uid")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
            ->find();
    ...
    public function refreshPowerStatus($id)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.serverid,a.dcimid,a.reinstall_info,b.config_option1,a.uid,b.api_type,b.zjmf_api_id,c.reinstall_times,b.id productid",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("dcim_servers c", "a.serverid=c.serverid")
            ->where("b.type", "dcim")
        ...
        if ($product["api_type"] == "zjmf_api") {
            $result = zjmfCurl($product["zjmf_api_id"], "/dcim/refresh_power_status", [
                "id" => $product["dcimid"],
                "is_api" => 1,
            ]);
        } elseif ($product["api_type"] == "whmcs") {
    ...
    public function getTrafficUsage($id, $start, $end)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,b.config_option1,b.api_type,b.zjmf_api_id,b.id productid")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
            ->find();
            ...
            if ($product["config_option1"] == "rent") {
                ...
                if ($res["status"] == "success") {
                    ...
                    foreach ($x as $v) {
                        $data[] = [
                            "time" => substr($v, 5),
                            "value" => round($res["data"][$v][$res["direction"]] / $res["decimal"], 2),
                        ];
                    }
                    $result["status"] = 200;
                    $result["data"] = $data;
                    ...
                    if ($detail["code"] == 200) {
                        ...
                        if (!empty($detail["data"]["float_ip"])) {
                            $update["assignedips"] = implode(
                                ",",
                                array_column($detail["data"]["float_ip"], "ipaddress"),
                            );
                        }
    ...
    public function upgrade($id)
    {
        ...
        if ($params["config_option1"] == "rent") {
            if (isset($params["configoptions_upgrade"]["bwt"])) {
                ...
                if ($res["status"] == "success") {
                    $bill_type = \think\Db::name("dcim_servers")
                        ->field("bill_type")
                    ...
                    if (!empty($bill_type)) {
                        ...
                        if (
                            $params["domainstatus"] == "Suspended" &&
                            ($reason == "用量超额" || $reason == "flow") &&
                            $now_percent < 100
                        ) {
            ...
            if (isset($params["configoptions_upgrade"]["ip_num"])) {
                ...
                if (!empty($post_data)) {
                    ...
                    if ($dcim_server["auth"]["enable_ip_custom"] == "on") {
                        $res = $this->post(
                            "getServerIp&id=" . $params["dcimid"],
                            ["ip_customid" => $dcim_server["ip_customid"], "custom_type" => "api"],
                            20,
                        );
                    } else {
                        ...
                        if (!empty($res["data"])) {
                            if ($dcim_server["auth"]["enable_ip_custom"] == "on") {
                                ...
                                foreach ($res["data"] as $v) {
                                    $update["assignedips"][] = !empty($v["custom_field_value"])
                                        ? $v["ipaddress"] . "(" . str_replace(",", "，", $v["custom_field_value"]) . ")"
                                        : $v["ipaddress"];
                                }
            ...
            if (isset($params["configoptions_upgrade"]["ip_num"])) {
                $this->bmsCurl(
                    "/bms/source/" . $params["dcimid"] . "/ip",
                    ["num" => $params["configoptions"]["ip_num"]],
                    "PUT",
                    10,
                );
            }
            if (isset($params["configoptions_upgrade"]["data_disk_size"])) {
                ...
                if ($res["code"] == 200) {
                    ...
                    if (!empty($disk_id)) {
                        $this->bmsCurl(
                            "/bms/disk/" . $disk_id,
                            ["size" => $params["configoptions"]["data_disk_size"]],
                            "PUT",
                            20,
                        );
                    } else {
                        if (!empty($store_id)) {
                            $this->bmsCurl("/bms/source/" . $params["dcimid"] . "/disk", [
                                "size" => $params["configoptions"]["data_disk_size"],
                                "ceph_id" => $store_id,
                            ]);
                        }
    ...
    public function assignServer($id, $dcimid)
    {
        ...
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.regdate,a.dcimid,b.config_option1,a.uid,a.nextduedate,b.host,b.api_type")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
            ->find();
        ...
        $is_assign = \think\Db::name("host")
            ->alias("h")
            ->join("products p", "h.productid=p.id")
            ->where("h.serverid", $product["serverid"])
            ->where("h.dcimid", $dcimid)
            ->where("p.api_type", "normal")
            ->find();
        if (!empty($is_assign)) {
            $result["status"] = 400;
    ...
    public function getPanelPass($hostid, $pid)
    {
        ...
        $value = \think\Db::name("customfields")
            ->alias("a")
            ->leftJoin("customfieldsvalues b", "a.id=b.fieldid")
            ->where("a.type", "product")
            ->where("a.relid", $pid)
            ->where("a.fieldname", "面板管理密码")
            ->where("b.relid", $hostid)
            ->value("value");
    ...
    public function buyFlowPacket($params)
    {
        ...
        if ($res["status"] == "success") {
            ...
            if (
                $res["act"] == 1 &&
                ($params["nextduedate"] == 0 || date("Ymd") < date("Ymd", $params["nextduedate"]))
            ) {
                $host = new Host();
                $host->is_admin = $this->is_admin;
    ...
    public function createSnap($id)
    {
        ...
        if ($product["api_type"] == "zjmf_api") {
            ...
            $res = $this->bmsCurl("/bms/source/" . $product["dcimid"] . "/snap", [
                "disk_id" => $post["id"],
                "remarks" => $post["name"],
            ]);
        }
    ...
    public function deleteSnap($id)
    {
        ...
        if ($product["api_type"] == "zjmf_api") {
            ...
            $list = $this->bmsCurl(
                "/bms/source/" . $product["dcimid"] . "/snap?listpages=999&sorting=asc",
                [],
                "GET",
                30,
            );
    ...
    public function createBackup($id)
    {
        ...
        if ($product["api_type"] == "zjmf_api") {
            ...
            $res = $this->bmsCurl("/bms/source/" . $product["dcimid"] . "/backup", [
                "disk_id" => $post["id"],
                "remarks" => $post["name"],
            ]);
        }
    ...
    public function deleteBackup($id)
    {
        ...
        if ($product["api_type"] == "zjmf_api") {
            ...
            $list = $this->bmsCurl(
                "/bms/source/" . $product["dcimid"] . "/backup?listpages=999&sorting=asc",
                [],
                "GET",
                30,
            );
    ...
    public function moduleClientAreaDetail($id, $key, $api_url = "")
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.uid,b.config_option1")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
            ->find();
        ...
        if ($key == "snapshot") {
            ...
            if ($support_backup) {
                $backup = $this->bmsCurl(
                    "/bms/source/" . $product["dcimid"] . "/backup",
                    ["listpages" => 999],
                    "GET",
                    15,
                );
            ...
            $res = [
                ...
                "vars" => [
                    "snap" => $snap["data"]["list"] ?? [],
                    "backup" => $backup["data"]["list"] ?? [],
                    "disk" => $detail["data"]["disk"],
        ...
        if ($func == "rescue") {
            ...
            if ($res["code"] == 200) {
                ...
                if ($this->is_admin) {
                    ...
                    active_log_final(
                        sprintf("救援系统发起失败#Host ID:%d - 原因:%s", $id, $res["msg"]),
                        $host_data["uid"],
                        2,
                        $id,
                    );
            ...
            if ($res["code"] == 200) {
                ...
                if ($this->is_admin) {
                    ...
                    $res["msg"] = $this->log_prefix_error . $res["msg"];
                    active_log_final(
                        sprintf("退出救援系统失败#Host ID:%d - 原因:%s", $id, $res["msg"]),
                        $host_data["uid"],
                        2,
                        $id,
                    );
    ...
    public function bmsReinstall($id, $os, $os_name)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.uid,a.dcimid,a.password,a.reinstall_info,b.config_option1")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
            ->find();
    ...
    public function sync($id)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.uid,a.productid,a.nextduedate,a.regdate,b.config_option1")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcim")
            ->where("a.id", $id)
            ->find();
            ...
            if ($res["code"] == 200) {
                ...
                if (!empty($configoptions)) {
                    $sub = \think\Db::name("product_config_options_sub")
                        ...
                        ->whereLike("option_name", $res["data"]["os_id"] . "|%")
