...
class DcimCloud
{
    ...
    public function setUrl($serverid)
    {
        if (!empty($serverid)) {
            $server_info = \think\Db::name("servers")
                ...
                ->where("id", $serverid)
                ->where("server_type", $this->server_type)
                ->find();
    ...
    {
        if (!empty($hostid)) {
            $server_info = \think\Db::name("host")
                ->alias("a")
                ->field("a.dcimid,b.id,b.hostname,b.username,b.password,b.secure,b.port,b.accesshash")
                ->leftJoin("servers b", "a.serverid=b.id")
                ->where("a.id", $hostid)
                ->where("b.server_type", $this->server_type)
                ->find();
            if (empty($server_info)) {
                $this->error = true;
            } else {
        ...
        if ($this->account_type == "agent") {
            ...
            if ($res["status"] == "success") {
                ...
                foreach ($res["data"]["data"] as $v) {
                    ...
                    if ($v["status"] == 1) {
                        $result[] = [
                            "name" => $v["id"] . "|" . ($v["group_name"] ?? $v["group"]["name"]) . "^" . $v["name"],
                        ];
                    }
                }
            }
    ...
    public function moduleClientArea($id)
    {
        ...
        if (
            empty($params["configoptions"]["type"]) ||
            $params["configoptions"]["type"] == "host" ||
            $params["configoptions"]["type"] == "lightHost"
        ) {
            if (
                is_numeric($params["configoptions"]["nat_acl_limit"]) &&
                $params["configoptions"]["nat_acl_limit"] >= 0
            ) {
                $data[] = ["name" => "NAT转发", "key" => "nat_acl"];
            }
            if (
                is_numeric($params["configoptions"]["nat_web_limit"]) &&
                $params["configoptions"]["nat_web_limit"] >= 0
            ) {
    ...
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.uid")
        ...
        if ($key == "snapshot") {
            ...
            $res = [
                ...
                "vars" => [
                    "list" => $res["data"]["data"],
                    "disk" => $cloud["data"]["disk"],
                    "support_snap" => $support_snap,
            ...
            if ($v10) {
                return [
                    ...
                    "data" => [
                        ...
                        "support_backup" => $support_backup,
                        "host_type" => $host_type,
                        "backup_num" => isset($params["configoptions"]["backup_num"])
                            ? \intval($params["configoptions"]["backup_num"])
                            : -1,
                        "snap_num" => isset($params["configoptions"]["snap_num"])
                            ? \intval($params["configoptions"]["snap_num"])
                            : -1,
                    ],
                ];
            }
            ...
            $res = [
                ...
                "vars" => [
                    "iso" => $cloud["data"]["iso"],
                    "bootorder" => $cloud["data"]["bootorder"],
                    "iso2" => $iso2,
                    "host_type" => $host_type,
                ],
            ];
            ...
            if (!empty($detail["data"]["user_id"])) {
                $data = $this->curl(
                    "/security_groups",
                    ["list_type" => "all", "user" => $detail["data"]["user_id"], "type" => $detail["data"]["type"]],
                    30,
                    "GET",
                );
                $list = $data["data"]["data"] ?: [];
            ...
            if (empty($protocols)) {
                $protocols = [
                    ["name" => "全部", "port" => "1-65535", "value" => "all"],
                    ["name" => "全部TCP", "port" => "1-65535", "value" => "all_tcp"],
                    ["name" => "全部UDP", "port" => "1-65535", "value" => "all_udp"],
                    ["name" => "自定义TCP", "port" => "", "value" => "tcp"],
                    ["name" => "自定义UDP", "port" => "", "value" => "udp"],
                    ["name" => "ICMP", "port" => "1-65535", "value" => "icmp"],
                    ["name" => "SSH (22)", "port" => "22", "value" => "ssh"],
                    ["name" => "telnet (23)", "port" => "23", "value" => "telnet"],
                    ["name" => "HTTP (80)", "port" => "80", "value" => "http"],
                    ["name" => "HTTPS (443)", "port" => "443", "value" => "https"],
                    ["name" => "MS SQL(1433)", "port" => "1433", "value" => "mssql"],
                    ["name" => "Oracle (1521)", "port" => "1521", "value" => "oracle"],
                    ["name" => "MySQL (3306)", "port" => "3306", "value" => "mysql"],
                    ["name" => "RDP (3389)", "port" => "3389", "value" => "rdp"],
                    ["name" => "PostgreSQL (5432)", "port" => "5432", "value" => "postgresql"],
                    ["name" => "Redis (6379)", "port" => "6379", "value" => "redis"],
                ];
            }
            $res = [
                "template" => "security_groups.html",
                "vars" => [
                    "list" => $list,
                    "used" => $detail["data"]["security"],
                    "protocols" => $protocols,
                    "host_type" => $host_type,
                ],
            ];
            ...
            $res = [
                "template" => "nat_acl.html",
                "vars" => ["list" => $data["data"]["data"] ?: [], "nat_host_ip" => $data["data"]["nat_host_ip"]],
            ];
        } elseif ($key == "nat_web") {
            $data = $this->curl("/clouds/" . $product["dcimid"] . "/nat_web", ["list_type" => "all"], 30, "GET");
            $res = [
                "template" => "nat_web.html",
                "vars" => ["list" => $data["data"]["data"] ?: [], "nat_host_ip" => $data["data"]["nat_host_ip"]],
            ];
        }
        if (file_exists($this->dir . "/" . $res["template"])) {
            ...
            if (!empty($api_url)) {
                ...
                $view->assign(
                    "MODULE_CUSTOM_API",
                    request()->domain() . request()->rootUrl() . "/provision/custom/" . $id,
                );
            }
    ...
    public function getNatInfo($id)
    {
        ...
        if (isset($params["configoptions"]["nat_acl_limit"]) && $params["configoptions"]["nat_acl_limit"] >= 0) {
            $data = $this->curl(
                "/clouds/" . $product["dcimid"] . "/nat_acl",
                ["per_page" => 1, "sort" => "asc"],
                15,
                "GET",
            );
            $result["nat_acl"] = $data["data"]["data"][0]
                ? $data["data"]["nat_host_ip"] . ":" . $data["data"]["data"][0]["ext_port"]
                : "";
        }
        if (isset($params["configoptions"]["nat_web_limit"]) && $params["configoptions"]["nat_web_limit"] >= 0) {
            $data = $this->curl(
                "/clouds/" . $product["dcimid"] . "/nat_web",
                ["per_page" => 1, "sort" => "asc"],
                15,
                "GET",
            );
    ...
    public function listSnapBackup($id)
    {
        ...
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.uid,b.api_type,b.zjmf_api_id")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcimcloud")
            ->where("a.id", $id)
            ->find();
        ...
        if ($product["api_type"] == "zjmf_api") {
            ...
            $res = $this->curl(
                "/clouds/" . $product["dcimid"] . "/snapshots",
    ...
    public function setBootOrder($id)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("启动项设置失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function createSecurityGroup($id)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("安全组创建失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function showSecurityRules($id)
    {
        ...
        if ($product["api_type"] == "zjmf_api") {
            $res = zjmfCurl(
                $product["zjmf_api_id"],
                "/provision/custom/" . \intval($product["dcimid"]),
                input("post."),
            );
        } else {
            $this->setUrl($product["serverid"]);
    ...
    public function createSecurityRule($id)
    {
        ...
        if ($product["api_type"] == "zjmf_api") {
            ...
            $post_data = [
                ...
                "start_port" => $post["start_port"],
                "end_port" => $post["end_port"],
                "start_ip" => $post["start_ip"],
                "end_ip" => $post["end_ip"],
            ...
            if (!empty($detail["data"]["user_id"])) {
                $data = $this->curl(
                    "/security_groups",
                    ["list_type" => "all", "user" => $detail["data"]["user_id"]],
                    30,
                    "GET",
                );
                $list = $data["data"]["data"] ?: [];
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("安全组规则创建失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function delSecurityGroup($id)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("安全组删除失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function delSecurityRule($id)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("安全组规则删除失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function linkSecurityGroup($id)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("安全组应用失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function addNatAcl($id)
    {
        ...
        if ($product["api_type"] == "zjmf_api") {
            ...
            $post_data = [
                ...
                "ext_port" => input("post.ext_port", 0, "intval"),
                "int_port" => input("post.int_port", 0, "intval"),
                "protocol" => input("post.protocol", 0, "intval"),
            ];
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($nat_acl["status"] == "success") {
                ...
                $description = sprintf(
                    "NAT转发创建成功,名称:%s,转发IP及端口:%s,内部端口:%d,协议:%s - Host ID:%d",
                    $post_data["name"],
                    $nat_acl["data"]["nat_host_ip"] . ":" . $nat_acl["data"]["data"][0]["ext_port"],
                    $nat_acl["data"]["data"][0]["int_port"],
                    $protocol[$nat_acl["data"]["data"][0]["protocol"]],
                    $id,
                );
            } else {
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("NAT转发创建失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function delNatAcl($id)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("NAT转发删除失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function addNatWeb($id)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            $result["msg"] = "创建成功";
            $description = sprintf(
                "共享建站创建成功,域名:%s,外部端口:%d,内部端口:%d - Host ID:%d",
                $post_data["domain"],
                $post_data["ext_port"],
                $post_data["int_port"],
                $id,
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("共享建站创建失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function delNatWeb($id)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("共享建站删除失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function exitRescue($id)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            ...
            if ($this->is_admin) {
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("退出救援系统失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function vnc($id)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.password")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcimcloud")
            ->where("a.id", $id)
            ->find();
            ...
            if (!empty($res["data"]["vnc_url_http"]) && !empty($res["data"]["vnc_url_https"])) {
                ...
                if ($this->is_admin) {
                    $result["url"] =
                        request()->domain() .
                        "/" .
                        config("database.admin_application") .
                        "/dcim/novnc?url=" .
                        urlencode(base64_encode($link_url)) .
                        "&password=" .
                        $res["data"]["vnc_pass"] .
                        "&host_token=" .
                        urlencode(aesPasswordEncode(cmf_decrypt($product["password"])));
                } else {
                    $result["url"] =
                        request()->domain() .
                        "/dcim/novnc?url=" .
    ...
    public function reinstall($id, $os, $port = 0)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.uid,a.reinstall_info")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcimcloud")
            ->where("a.id", $id)
            ->find();
        ...
        $system_disk_size_config_options = \think\Db::name("host_config_options")
            ->field("a.id,a.qty,b.option_name,b.option_type,c.option_name sub_option,b.upgrade")
            ->alias("a")
            ->leftJoin("product_config_options b", "a.configid=b.id")
            ->leftJoin("product_config_options_sub c", "a.configid=c.config_id and a.optionid=c.id")
            ->where("a.relid", $id)
            ->whereLike("b.option_name", "system_disk_size|%")
            ->find();
        if (
            $system_disk_size_config_options["option_type"] == 1 ||
            $system_disk_size_config_options["option_type"] == 13
        ) {
        ...
        if ($res["status"] == "success") {
            ...
            if ($this->is_admin) {
                ...
                $result["msg"] = $this->log_prefix_error . $result["msg"];
                $description =
                    $this->log_prefix_error . sprintf("重装系统发起失败,原因:%s - Host ID:%d", $res["msg"], $id);
            } else {
    ...
    public function sync($id)
    {
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.dcimid,a.uid,a.productid")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcimcloud")
            ->where("a.id", $id)
            ->find();
        ...
        if ($res["status"] == "success") {
            ...
            if ($res["data"]["client_show_ip_remark"] == 1) {
                ...
                foreach ($res["data"]["ip"] as $v) {
                    $update["assignedips"][] = !empty($v["remark"])
                        ? $v["ipaddress"] . "(" . str_replace(",", "，", $v["remark"]) . ")"
                        : $v["ipaddress"];
                }
            } else {
                $update["assignedips"] = array_column($res["data"]["ip"], "ipaddress") ?? [];
            }
            $update["assignedips"] = array_merge(
                $update["assignedips"],
                array_column($res["data"]["ipv6"], "ipv6") ?? [],
            );
            $update["assignedips"] = implode(
                ",",
                array_filter($update["assignedips"], function ($x) use ($res) {
            ...
            $configoptions = \think\Db::name("product_config_links")
                ->alias("a")
                ->leftJoin("product_config_options b", "a.gid=b.gid")
                ->where("a.pid", $product["productid"])
                ->where("b.option_type", 5)
                ->value("b.id");
            if (!empty($configoptions)) {
                $sub = \think\Db::name("product_config_options_sub")
                    ->field("id,config_id,option_name")
                    ->whereLike("option_name", $res["data"]["system"] . "|%")
                    ->where("config_id", $configoptions)
                    ->find();
                if (!empty($sub)) {
                    \think\Db::name("host_config_options")
                        ->where("relid", $id)
                        ->where("configid", $configoptions)
    ...
    public function buyFlowPacket($params)
    {
        ...
        if ($res["status"] == "success") {
            $description = sprintf(
                "流量包购买成功，已成功附加到产品。- Invoice ID:%d - Host ID:%d",
                $params["invoiceid"],
                $params["hostid"],
            );
            ...
            if ($params["nextduedate"] == 0 || date("Ymd") < date("Ymd", $params["nextduedate"])) {
                if ($info["status"] == "success" && $info["data"]["info"]["30_day"]["float"] < 100) {
                    ...
                    if ($result["status"] == 200) {
                        ...
                        $data_i[
                            "description"
                        ] = " 购买流量包后 - 解除暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                        if ($is_zjmf) {
            ...
            $description = sprintf(
                "流量包购买成功，附加到产品失败，请手动添加临时流量。- Invoice ID:%d - Host ID:%d",
                $params["invoiceid"],
                $params["hostid"],
            );
        }
    ...
    public function moduleClientButton($id)
    {
        ...
        $button = [
            "control" => [
                ...
                ["type" => "custom", "func" => "exitRescue", "name" => "退出救援系统"],
            ],
            "console" => [["type" => "default", "func" => "vnc", "name" => "VNC"]],
        ];
        return $button;
    }
    public function execCustomButton($id, $func = "")
    {
        ...
        $product = \think\Db::name("host")
            ->alias("a")
            ->field("a.serverid,a.domain,a.dcimid,a.uid")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("b.type", "dcimcloud")
            ->where("a.id", $id)
            ->find();
    ...
    public function upgrade($id, $old)
    {
        ...
        $post_data = [];
        if (
            isset($params["configoptions_upgrade"]["cpu"]) &&
            $params["configoptions_upgrade"]["cpu"] != $detail["cpu"]
        ) {
            if (!$needOff) {
                $needOff = $waitOff();
            }
        ...
        if (
            isset($params["configoptions_upgrade"]["memory"]) &&
            ($params["configoptions_upgrade"]["memory"] / 1024 != $detail["memory"] ||
                $params["configoptions_upgrade"]["memory"] != $old["configoptions"]["memory"])
        ) {
            if (!$needOff) {
                $needOff = $waitOff();
            }
            $post_data["memory"] = \intval($params["configoptions"]["memory"]);
            $memory_config_options = \think\Db::name("host_config_options")
                ->field("a.id,a.qty,b.option_name,b.option_type,c.option_name sub_option,b.upgrade")
                ->alias("a")
                ->leftJoin("product_config_options b", "a.configid=b.id")
                ->leftJoin("product_config_options_sub c", "a.configid=c.config_id and a.optionid=c.id")
                ->where("a.relid", $id)
                ->whereLike("b.option_name", "memory|%")
                ->find();
        ...
        if (
            isset($params["configoptions_upgrade"]["ip_num"]) &&
            ($params["configoptions_upgrade"]["ip_num"] != $old["configoptions"]["ip_num"] ||
                $params["configoptions_upgrade"]["ip_num"] != $detail["ip_num"])
        ) {
            if (!$needOff && $detail["data"]["network_type"] == "normal") {
                $needOff = $waitOff();
            }
            $res3 = $this->curl(
                "/clouds/" . $params["dcimid"] . "/ip",
                ["num" => $params["configoptions"]["ip_num"], "ip_group" => $params["configoptions"]["ip_group"]],
                20,
                "PUT",
            );
        }
        if (
            isset($params["configoptions_upgrade"]["data_disk_size"]) &&
            $params["configoptions_upgrade"]["data_disk_size"] != $old["configoptions"]["data_disk_size"]
        ) {
            $disk_id = 0;
            $store_id = 0;
            $data_disk_size_config_options = \think\Db::name("host_config_options")
                ->field("a.id,a.qty,b.option_name,b.option_type,c.option_name sub_option,b.upgrade")
                ->alias("a")
                ->leftJoin("product_config_options b", "a.configid=b.id")
                ->leftJoin("product_config_options_sub c", "a.configid=c.config_id and a.optionid=c.id")
                ->where("a.relid", $id)
                ->whereLike("b.option_name", "data_disk_size|%")
                ->find();
            if (
                $data_disk_size_config_options["option_type"] == 1 ||
                $data_disk_size_config_options["option_type"] == 13
            ) {
            ...
            if (!empty($disk_id)) {
                ...
                if (!empty($store_id)) {
                    $new_disk = $this->curl(
                        "/clouds/" . $params["dcimid"] . "/disks",
                        ["size" => $data_disk_size, "store" => $store_id, "driver" => "virtio"],
                        20,
                        "POST",
                    );
        ...
        if (
            isset($params["configoptions_upgrade"]["ipv6_num"]) &&
            ($params["configoptions_upgrade"]["ipv6_num"] != $old["configoptions"]["ipv6_num"] ||
                $params["configoptions_upgrade"]["ipv6_num"] != $detail["ipv6_num"])
        ) {
            if (!$needOff) {
                $needOff = $waitOff();
            }
            $res_ipv6 = $this->curl(
                "/clouds/" . $params["dcimid"] . "/ipv6",
                ["num" => $params["configoptions"]["ipv6_num"]],
                20,
                "PUT",
            );
        }
        if (
            isset($params["configoptions_upgrade"]["system_disk_io_limit"]) &&
            $params["configoptions_upgrade"]["system_disk_io_limit"] != $old["configoptions"]["system_disk_io_limit"]
        ) {
        ...
        if (
            isset($params["configoptions_upgrade"]["data_disk_io_limit"]) &&
            $params["configoptions_upgrade"]["data_disk_io_limit"] != $old["configoptions"]["data_disk_io_limit"]
        ) {
        ...
        if ($detail["status"] == "success") {
            ...
            $update["assignedips"] = array_merge(
                $update["assignedips"],
                array_column($detail["data"]["ipv6"], "ipv6") ?? [],
            );
            $update["assignedips"] = implode(
                ",",
                array_filter($update["assignedips"], function ($x) use ($detail) {
    ...
    public function getChartData($id, $get)
    {
        ...
        if ($get["type"] == "cpu") {
            $query = [
                "node_id" => $cloud["data"]["node_id"],
                "kvm" => $cloud["data"]["kvmid"],
                "type" => "kvm_info",
                "st" => $get["start"],
                "et" => $get["end"],
            ];
            ...
            if ($res["status"] == "success") {
                ...
                foreach ($res["data"] as $v) {
                    $result["data"]["list"][0][] = [
                        "time" => date("Y-m-d H:i:s", strtotime($v[0])),
                        "value" => round($v[1], 2),
                    ];
                }
            } else {
    ...
    public function createAccount($hostid)
    {
        ...
        if ($params["type"] == "dcimcloud") {
            ...
            $user_data = [
                ...
                "status" => 1,
                "real_name" => $params["user_info"]["username"] ?: "",
                "password" => randStr(8),
            ];
            if ($this->account_type == "agent") {
            ...
            $post_data["nat_acl_limit"] = isset($params["configoptions"]["nat_acl_limit"])
                ? $params["configoptions"]["nat_acl_limit"]
                : -1;
            $post_data["nat_web_limit"] = isset($params["configoptions"]["nat_web_limit"])
                ? $params["configoptions"]["nat_web_limit"]
                : -1;
            ...
            $post_data["traffic_quota"] = $params["configoptions"]["flow_limit"];
            $post_data["backup_num"] = isset($params["configoptions"]["backup_num"])
                ? $params["configoptions"]["backup_num"]
                : -1;
            $post_data["snap_num"] = isset($params["configoptions"]["snap_num"])
                ? $params["configoptions"]["snap_num"]
                : -1;
            ...
            if (
                ...
                if (strpos($sub_option, ",") !== false) {
                    $sub_option = explode(",", $sub_option);
                    $post_data["other_data_disk"] = [
                        ["size" => \intval($sub_option[0]), "store" => \intval($sub_option[1])],
                    ];
                } else {
                ...
                if ($data_disk_size_config_options["option_type"] == 14) {
                    if (strpos($data_disk_size_config_options["sub_option"], "|") !== false) {
                        ...
                        if (!empty($sub_option) && !empty($params["configoptions"]["data_disk_size"])) {
                            $post_data["other_data_disk"] = [
                                [
                                    "size" => \intval($params["configoptions"]["data_disk_size"]),
                                    "store" => \intval($sub_option),
                                ],
                            ];
                        }
                    } else {
            ...
            if (
                is_numeric($params["configoptions"]["cpu_limit"]) &&
                $params["configoptions"]["cpu_limit"] >= 0 &&
                $params["configoptions"]["cpu_limit"] <= 100
            ) {
                $post_data["cpu_limit"] = \intval($params["configoptions"]["cpu_limit"]);
            }
            if (
                is_numeric($params["customfields"]["port"]) ||
                is_numeric($params["customfields"]["端口"]) ||
                $params["customfields"]["port"] === "auto" ||
                $params["customfields"]["端口"] === "auto"
            ) {
            ...
            $post_data["ipv6_num"] = isset($params["configoptions"]["ipv6_num"])
                ? \intval($params["configoptions"]["ipv6_num"])
                : 0;
            $post_data["type"] = $params["configoptions"]["type"];
            if (
                isset($params["configoptions"]["niccard"]) &&
                in_array($params["configoptions"]["niccard"], [1, 2, 3])
            ) {
    ...
    public function getTrafficUsage($id, $start, $end)
    {
        ...
        $get = [
            "type" => 2,
            "start_time" => strtotime($start . " 00:00:00") . "000",
            "end_time" => strtotime($end . " 23:59:59") . "000",
            "unit" => "GB",
        ];
    ...
    public function savePanelPass($hostid, $pid, $password)
    {
        ...
        $customid = \think\Db::name("customfields")
            ->where("type", "product")
            ->where("relid", $pid)
            ->where("fieldname='面板管理密码' OR fieldname='panel_passwd'")
            ->value("id");
        ...
        if (!empty($customid)) {
            $exist = \think\Db::name("customfieldsvalues")
    ...
    public function resetFlow($hostid, $dcimid, $nextduedate = 0, $domainstatus)
    {
        ...
        if ($res["status"] == "success") {
            ...
            if (
                ...
                ($nextduedate == 0 || date("Ymd") < date("Ymd", $nextduedate)) &&
                $suspendreason_type == "flow"
            ) {
                $host = new Host();
                $host->is_admin = $this->is_admin;
                ...
                if ($result["status"] == 200) {
                    ...
                    $data_i[
                        "description"
                    ] = " 同步流量后 - 解除暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                    if ($is_zjmf) {
                        $logic_run_map->saveMap($data_i, 0, 400, 3);
                    }
                    if (!$is_zjmf) {
    ...
    public function rescue($id, $system, $temp_pass)
    {
        ...
        $res = $this->curl(
            "/clouds/" . $product["dcimid"] . "/rescue",
            ["type" => $system, "temp_pass" => $temp_pass],
            30,
            "POST",
        );
        if ($res["status"] == "success") {
