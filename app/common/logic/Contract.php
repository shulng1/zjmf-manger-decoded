...
class Contract
{
    public function getHostContract($hid)
    {
        ...
        if (!empty($pid) && !empty($tpls)) {
            foreach ($tpls as $tpl) {
                $count = \think\Db::name("contract_pdf")
                    ->where("contract_id", $tpl["id"])
                    ->where("host_id", $hid)
                    ->whereIn("status", [1, 3, 4])
                    ->count();
                $count1 = \think\Db::name("contract_pdf")
                    ->where("contract_id", $tpl["id"])
                    ->where("uid", $uid)
                    ->whereIn("status", [1, 3, 4])
                    ->count();
    ...
    public function replaceArg($content, $hostid, $base = 0, $userid = 0)
    {
        if (empty($base)) {
            $host = \think\Db::name("host")
                ->alias("h")
                ->field(
                    "h.uid,p.name,h.domain,h.domainstatus,h.regdate,h.termination_date,h.nextduedate,h.billingcycle,h.firstpaymentamount,h.amount",
                )
        ...
        $client = \think\Db::name("clients")
            ->field("username,companyname,phonenumber,email,address1")
            ->where("id", $uid)
            ->find();
        $search = [
            "{\$client_username}",
        ...
        $replace = [
            ...
            $host["name"],
            $host["domain"],
            config("domainstatus")[$host["domainstatus"]],
            date("Y-m-d H:i:s", $host["regdate"]),
            date("Y-m-d H:i:s", $host["termination_date"]),
            config("billing_cycle")[$host["billingcycle"]],
            $host["firstpaymentamount"],
            $currency["prefix"] . $host["firstpaymentamount"],
            $this->getMonthNum($host["nextduedate"], $host["regdate"]) . "个月",
        ];
