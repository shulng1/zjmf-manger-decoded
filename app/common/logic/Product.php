...
class Product
{
    ...
    public function syncProduct($param)
    {
        ...
        if (
            $product["upstream_version"] != $info["location_version"] ||
            $product["upstream_price_type"] != $param["upstream_price_type"]
        ) {
            ...
            if (
                $info["qty"] != $product["upstream_qty"] ||
                $info["stock_control"] != $product["upstream_stock_control"]
            ) {
                \think\Db::name("products")
                    ->where("id", $id)
                    ->update(["upstream_qty" => $info["qty"], "upstream_stock_control" => $info["stock_control"]]);
            }
        }
        if (
            $product["upstream_version"] != $info["location_version"] ||
            ...
            if (!empty($param["upstream_price_type"])) {
                \think\Db::name("products")
                    ->where("id", $id)
                    ->update([
                        "upstream_price_type" => $param["upstream_price_type"] ?: "percent",
                        "upstream_price_value" => $param["upstream_price_value"] ?: 120,
                    ]);
            }
            $this->updateCache([$id]);
        ...
        foreach ($apis as $api) {
            ...
            if ($res["status"] == 200) {
                ...
                $products = \think\Db::name("products")
                    ->field(
                        "id,name,description,upstream_pid,upstream_version,upstream_price_type,location_version,zjmf_api_id,gid,pay_type,
                    upstream_stock_control,upstream_qty",
                    )
                    ->where("zjmf_api_id", $id)
                    ->select()
                    ->toArray();
                ...
                foreach ($infos as $info) {
                    foreach ($products as $product) {
                        if ($info["id"] == $product["upstream_pid"]) {
                            ...
                            if (
                                $info["stock_control"] != $product["upstream_stock_control"] ||
                                $info["qty"] != $product["stock_control"]
                            ) {
                                \think\Db::name("products")
                                    ->where("id", $product["id"])
                                    ->update([
                                        "upstream_qty" => $info["qty"],
                                        "upstream_stock_control" => $info["stock_control"],
                                    ]);
                            }
                        }
                    }
                    ...
                    if ($res["status"] == 200) {
                        ...
                        foreach ($detail as $key => $value) {
                            ...
                            if ($res["status"] == 200) {
                                ...
                                $desc =
                                    "定时任务同步供应商'{$api_name}'商品'{$value["name"]}'成功,本地#PRODUCT ID:" .
                                    $local_product["id"];
                                active_log_final($desc, 0, 5);
                            } else {
    ...
    public function baseUpdateProduct($upstream_product, $product, $rate = 1, $is_cron = false)
    {
        ...
        $basedata = [
            "type" => $upstream_product["type"],
            "password" => $upstream_product["password"],
            "auto_setup" => "payment",
            "auto_terminate_days" => $upstream_product["auto_terminate_days"],
            "config_options_upgrade" => $upstream_product["config_options_upgrade"],
            "down_configoption_refund" => $upstream_product["down_configoption_refund"],
            "retired" => $upstream_product["retired"],
            "is_featured" => $upstream_product["is_featured"],
            "groupid" => $upstream_product["groupid"],
            "api_type" => "zjmf_api",
            "location_version" => $product["location_version"] + 1,
            "upstream_version" => $upstream_product["location_version"],
            "zjmf_api_id" => $zjmf_finance_api_id,
            "server_group" => $zjmf_finance_api_id,
            "upstream_pid" => $upstream_pid,
            "hidden" => intval($upstream_product["hidden"]),
            "pay_method" => $upstream_product["pay_method"],
            "rate" => $rate,
            "upstream_stock_control" => $upstream_product["upstream_stock_control"],
            "upstream_qty" => $upstream_product["qty"],
            "stock_control" => 0,
            "qty" => 0,
            "upstream_auto_setup" => $upstream_product["auto_setup"],
            "upstream_ontrial_status" => intval($pay_type["pay_ontrial_status"]),
            "upstream_product_shopping_url" => $upstream_product["product_shopping_url"] ?: "",
        ];
        ...
        try {
                        ...
                        if ($product_pricing["code"] == $currency["code"]) {
                            ...
                            if (
                                ...
                                foreach ($price_type as $v) {
                                    $product_pricing[$v[0]] =
                                        ($product_pricing[$v[0]] * $upstream_product["upstream_price_value"]) / 100;
                                    $product_pricing[$v[1]] =
                                        ($product_pricing[$v[1]] * $upstream_product["upstream_price_value"]) / 100;
                                }
                            }
                                ...
                                foreach ($price_type as $v) {
                                    if ($product_pricing[$v[0]] >= 0) {
                                        $product_pricing[$v[0]] =
                                            ($rate *
                                                $product_pricing[$v[0]] *
                                                $upstream_product["upstream_price_value"]) /
                                            100;
                                    }
                                    $product_pricing[$v[1]] =
                                        ($rate * $product_pricing[$v[1]] * $upstream_product["upstream_price_value"]) /
                                        100;
                                }
                            } else {
                                foreach ($price_type as $v) {
            ...
            $local_customfields = \think\Db::name("customfields")
                ->field("id,upstream_id")
                ->where("type", "product")
                ->where("relid", $id)
                ->select()
                ->toArray();
                    ...
                    foreach ($local_customfields as $local_customfield) {
                        if ($customfield["upstream_id"] == $local_customfield["upstream_id"]) {
                            \think\Db::name("customfieldsvalues")
                                ->where("fieldid", $local_customfield["id"])
                                ->update(["fieldid" => $new_customid]);
                        }
                    }
                }
            }
            $hostids = \think\Db::name("host")->where("productid", $id)->column("id");
            $host_links = \think\Db::name("host_config_options")
                ->alias("a")
                ->field(
                    "a.id,a.relid,a.configid,a.optionid,a.qty,b.upstream_id as upstream_oid,c.upstream_id as upstream_subid",
                )
                ->leftJoin("product_config_options b", "a.configid = b.id")
                ->leftJoin("product_config_options_sub c", "a.optionid = c.id")
                ->whereIn("a.relid", $hostids)
                ->select()
                ->toArray();
            ...
            $advanced_link_ids = \think\Db::name("product_config_options_links")
                ->whereIn("config_id", $oids)
            ...
            \think\Db::name("product_config_options_links")
                ->where("type", "result")
                ->whereIn("relation_id", $advanced_link_ids)
                ->where("upstream_id", ">", 0)
                ->delete();
            \think\Db::name("product_config_options_links")
                ->whereIn("config_id", $oids)
                    ...
                    foreach ($options as $option) {
                        ...
                        foreach ($subs as $sub) {
                            ...
                            foreach ($host_links as $hk => $host_link) {
                                if (
                                    $host_link["upstream_oid"] == $option["upstream_id"] &&
                                    $host_link["upstream_subid"] == $sub["upstream_id"]
                                ) {
                                    \think\Db::name("host_config_options")
                                        ->where("relid", $host_link["relid"])
                                        ->where("configid", $host_link["configid"])
                                        ->update(["configid" => $config_id, "optionid" => $sub_id]);
                                    unset($host_links[$hk]);
                                }
                            }
                            foreach ($currencies as $currency) {
                                foreach ($pricings as $pricing) {
                                    if ($pricing["code"] == $currency["code"]) {
                                        ...
                                        if (
                                            ...
                                            foreach ($price_type as $v) {
                                                $pricing[$v[0]] =
                                                    ($pricing[$v[0]] * $upstream_product["upstream_price_value"]) / 100;
                                                $pricing[$v[1]] =
                                                    ($pricing[$v[1]] * $upstream_product["upstream_price_value"]) / 100;
                                            }
                                        }
    ...
    public function getList($pids = [])
    {
        ...
        $lists = \think\Db::name("products")
            ->field(
                "id,type,gid,name,description,pay_method,tax,order,pay_type,api_type,
            upstream_version,upstream_price_type,upstream_price_value,stock_control,qty",
            )
            ->where($where)
            ->order("order", "asc")
            ->select()
        ...
        foreach ($lists as $v) {
            ...
            $cgs = \think\Db::name("client_groups")
                ->alias("a")
                ->field("a.id,b.type,b.bates")
                ->leftJoin("user_product_bates b", "a.id=b.user")
                ->leftJoin("user_products c", "b.products=c.gid")
                ->where("c.pid", $v["id"])
                ->select()
                ->toArray();
            $cg_f = [];
