...
class Download
{
    ...
    {
        $cate_data = \app\common\model\DownloadcatsModel::where("parentid", $cate_id)
            ->where("hidden", 0)
            ->order("sort", "asc")
            ->select()
            ->toArray();
    ...
    public function getCatsProduct($productid)
    {
        $cats_data = \think\Db::name("downloadcats")
            ->field("id,parentid,name,description")
            ->order("sort", "asc")
            ->select()
            ->toArray();
        if (!empty($cats_data)) {
            $product_downloads = \think\Db::name("product_downloads")
                ->field("download_id")
            ...
            $download_data = \think\Db::name("downloads")
                ->field("id,category,title as name")
                ->where("id", "not in", $str)
                ->select()
                ->toArray();
    ...
    public function getCatsProductselect($pid)
    {
        $product_downloads = \think\Db::name("product_downloads")
            ->alias("p")
            ->leftJoin("downloads d", "d.id=p.download_id")
            ->field("d.*")
    ...
    public function upload()
    {
        $validate = [
            "size" => 52428800,
            "ext" => "rar,zip,png,jpg,jpeg,gif,doc,docx,key,number,pages,pdf,ppt,pptx,txt,rtf,vcf,xls,xlsx",
        ];
