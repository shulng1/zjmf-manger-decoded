...
class Upgrade
{
    ...
    {
        $upgrade_product = \think\Db::name("product_upgrade_products")
            ->alias("a")
            ->leftJoin("host b", "a.product_id = b.productid")
            ->where("b.id", $hid)
            ->find();
    ...
    public function deleteUpgradeInvoices($hid)
    {
        ...
        $upgrades = \think\Db::name("upgrades")
            ->alias("a")
            ->field("a.id,a.order_id,d.id as invoice_id,a.status,d.status as invoice_status,a.amount,a.uid")
            ->leftJoin("host b", "a.relid = b.id")
            ->leftJoin("orders c", "a.order_id = c.id")
            ->leftJoin("invoices d", "c.invoiceid = d.id")
            ->where("a.relid", $hid)
        ...
        foreach ($upgrades as $upgrade) {
            if ($upgrade["status"] != "Completed" || $upgrade["invoice_status"] != "Paid") {
                ...
                \think\Db::name("invoice_items")
                    ->where("invoice_id", $upgrade["invoice_id"])
    ...
    {
        $host_config = \think\Db::name("host_config_options")
            ->field("configid,optionid,qty")
            ->where("relid", $hid)
            ->select()
            ->toArray();
        $host_config_filter = [];
    ...
    public function checkChangeText($hid, $configoptions)
    {
        ...
        foreach ($host_config as $v) {
            ...
            if (judgeQuantity($option_type)) {
                ...
                if ($value != $v["optionid"]) {
                    $option_name_old = \think\Db::name("product_config_options_sub")
                        ->where("id", $v["optionid"])
                        ->value("option_name");
                    $option_name_old = array_pop(explode("|", $option_name_old));
                    $option_name_new = \think\Db::name("product_config_options_sub")
                        ->where("id", $value)
                        ->value("option_name");
    ...
    protected function filterDelConfigOptions($conf, $filter_conf, $hid, $admin = false)
    {
        ...
        if ($admin) {
            $upgrade_config_id = \think\Db::name("product_config_options")
                ->whereIn("id", $configid)
                ->select()
                ->toArray();
        } else {
        ...
        foreach ($upgrade_config_id as $k => $val) {
            if (!isset($conf[$val["id"]]) && $val["option_type"] != 12 && $val["option_type"] != 5) {
                ...
                if ($val["option_type"] == 3) {
                    $filter_conf[$val["id"]] = \think\Db::name("product_config_options_sub")
        ...
        if ($filter_conf) {
            $data = \think\Db::name("product_config_options")
                ->field("id,linkage_top_pid as pid")
                ->whereIn("id", array_keys($filter_conf))
                ->select()
                ->toArray();
    ...
    {
        $allConfigArr = \think\Db::name("product_config_options")
            ->alias("a")
            ->field("a.*")
            ->leftJoin("product_config_links b", "b.gid=a.gid")
            ->where("b.pid", $pid)
            ->where(function (\think\db\Query $query) use ($admin, $configoptions) {
                if (!$admin) {
        ...
        if (!empty($allConfigArr[0])) {
            foreach ($allConfigArr as $k => $v) {
                ...
                if (judgeQuantity($option_type)) {
                    ...
                    if ($option_type == 3) {
                        $exists_data = \think\Db::name("product_config_options_sub")
                            ->where("config_id", $config_id)
                            ->where("id", $configoptions[$config_id])
                            ->find();
                        if (!empty($exists_data)) {
                            $configoptions_filter[$config_id] = $configoptions[$config_id];
                        } else {
                            $sub_data = \think\Db::name("product_config_options_sub")
                                ->where("config_id", $config_id)
                                ->order("sort_order asc")
                                ->find();
                            if (!empty($sub_data)) {
    ...
    public function upgradeConfigAdmin($hid, $configoptions)
    {
        $host = \think\Db::name("host")
            ->alias("h")
            ->field(
                "h.uid,h.regdate,h.billingcycle,p.pay_type,h.firstpaymentamount,h.amount,h.domain,h.nextduedate,h.payment,p.id as pid,p.server_group",
            )
        ...
        foreach ($configoptions_old as $v) {
            $option = \think\Db::name("product_config_options")
                ->field("option_type,option_name")
                ->where("upgrade", 1)
                ->where("id", $v["configid"])
                ->find();
            $suboption = \think\Db::name("product_config_options_sub")
                ->field("option_name")
                ->where("id", $v["optionid"])
                ->find();
            if (!judgeQuantity($option["option_type"])) {
                $params["configoptions"][explode("|", $option["option_name"])[0] ?? $option["option_name"]] = explode(
                    "|",
    ...
    public function upgradeConfigCommon(
        ...
        $host = \think\Db::name("host")
            ->alias("h")
            ->field(
                "concat(pg.name, \"-\" , p.name) as name,p.name as pname,h.uid,h.regdate,h.billingcycle,h.flag,
            p.pay_type,h.firstpaymentamount,h.amount,h.domain,h.nextduedate,h.payment,p.id as pid,p.down_configoption_refund,p.api_type,p.upstream_price_type,p.upstream_price_value,p.upstream_pid",
            )
            ->leftJoin("products p", "h.productid = p.id")
            ->leftJoin("product_groups pg", "pg.id = p.gid")
            ->where("h.id", $hid)
            ->find();
        ...
        $product_pricings = \think\Db::name("pricing")
            ...
            ->where("relid", $pid)
            ->find();
        $product_pricing = bcadd(
            $product_pricings[$billingcycle],
            $product_pricings[config("price_type")[$billingcycle][1]],
            2,
        );
        ...
        foreach ($configoptions as $k => $v) {
            ...
            if (judgeQuantity($option_type)) {
                $old_sub = \think\Db::name("host_config_options")
                    ->field("optionid,qty")
                    ->where("relid", $hid)
                    ->where("configid", $k)
                    ->find();
                if (!empty($old_sub)) {
                    ...
                    $old_sub_option = \think\Db::name("product_config_options_sub")
                        ->where("id", $old_subid)
                        ->value("option_name");
                    $qty = $old_sub["qty"];
                    if (judgeQuantityStage($option_type)) {
                        ...
                        $old_pricing = \think\Db::name("pricing")
                            ->where("type", "configoptions")
                            ->where("currency", $currencyid)
                            ->where("relid", $old_subid)
                            ->find();
                ...
                $options = \think\Db::name("product_config_options_sub")
                    ->alias("pcos")
                    ->field(
                        "pco.id as oid,pcos.id as sub_id,pcos.option_name as suboption_name,pcos.qty_minimum,pcos.qty_maximum,pco.option_type,pco.option_name as option_name,pco.qty_minimum as min,pco.qty_maximum as max,pco.is_discount,p.*,pco.is_rebate,pco.qty_stage,pco.unit",
                    )
                    ->leftJoin("product_config_options pco", "pco.id = pcos.config_id")
                    ->leftJoin("pricing p", "p.relid = pcos.id")
                    ->where("pcos.config_id", $k)
                    ...
                    foreach ($options as $option) {
                        ...
                        if ($v >= 0 && $option["min"] <= $v && $v <= $option["max"] && $min <= $v && $v <= $max) {
                            ...
                            if ($billingcycle == "onetime" || $billingcycle == "free") {
                                ...
                                $diff_sub_surplustime = bcmul(
                                    $diff_sub,
                                    bcdiv(
                                        $surplustime,
                                        $this->cycleToTime(
                                            $billingcycle,
                                            $old_paytype["pay_{$billingcycle}_cycle"],
                                            $regdate,
                                            $old_paytype["pay_ontrial_cycle_type"],
                                        ),
                                    ),
                                );
                            }
                            ...
                            $option_filter["option_name"] = explode("|", $option_name)[1]
                                ? explode("|", $option_name)[1]
                                : $option_name;
                            $option_filter["suboption_name"] = explode("|", $suboption_name)[1]
                                ? explode("|", $suboption_name)[1]
                                : $suboption_name;
                ...
                if (!empty($option_filter)) {
                    if ($v != $qty) {
                        ...
                        $upgrade_item = [
                            "original_value" => $k . "=>" . $old_subid . ":" . $qty,
                            "new_value" => $new_suboption_id . ":" . $v,
                            "recurring_change" => $diff_sub,
                            "paid" => $paid,
                            "amountdue" => $diff_sub_surplustime,
                            "invoices_item_description" =>
                                lang("OPTIONS_UPGRADE") .
                                ":" .
                                $product_name .
                                "(" .
                                $host["domain"] .
                ...
                if ($v) {
                    $option = \think\Db::name("product_config_options_sub")
                        ->alias("pcos")
                        ->field(
                            "pco.id as oid,pcos.option_name as suboption_name,pco.option_type,pco.option_name as option_name,pcos.id as suboption_id,pco.is_discount,p.*,pco.is_rebate,pco.qty_stage,pco.unit",
                        )
                        ->leftJoin("product_config_options pco", "pco.id = pcos.config_id")
                        ->leftJoin("pricing p", "p.relid = pcos.id")
                        ->where("pcos.id", $v)
                    ...
                    if ($option) {
                        ...
                        $diff_sub_surplustime = bcsub($diff_sub_surplustime, 0, 2);
                        $suboption_name = $option["suboption_name"];
                        $suboption_name = explode("|", $suboption_name)[1]
                            ? explode("|", $suboption_name)[1]
                            : $suboption_name;
                        $option["suboption_name"] = $is_no
                            ? "否"
                            : (judgeYesNo($option_type)
                                ? "是"
                                : implode(" ", explode("^", $suboption_name)));
                        $option_name = $option["option_name"];
                        ...
                        if ($v != $old_subid || $is_no || $is_yes) {
                            $paid = $diff_sub_surplustime <= 0 ? "Y" : "N";
                            $upgrade_item = [
                                "original_value" => $k . "=>" . $old_subid,
                                "new_value" => judgeYesNo($option_type) ? $v . ":" . ($is_yes ? 1 : 0) : $v,
                                "recurring_change" => $diff_sub,
                                "paid" => $paid,
                                "amountdue" => $diff_sub_surplustime,
                                "invoices_item_description" =>
                                    ...
                                    $v .
                                    ")",
                                "upgrade_description" => $option_name . ":" . $old_sub_option . "=>" . $suboption_name,
                            ];
                            $option["upgrade_item"] = $upgrade_item;
                            $old_sub_option = explode("|", $old_sub_option)[1]
                                ? explode("|", $old_sub_option)[1]
                                : $old_sub_option;
                            $option["old_suboption_name"] = $is_yes
                                ? "否"
                                : (judgeYesNo($option_type)
                                    ? "是"
                                    : implode(" ", explode("^", $old_sub_option)));
                            $diff_pricing = bcadd($diff_pricing, $diff_sub_surplustime, 2);
                        } else {
                            $option["upgrade_item"] = [];
                        }
                    ...
                    $tmp = \think\Db::name("product_config_options")
                        ->field("option_type,option_name,is_discount,is_rebate,qty_stage,unit")
                        ->where("id", $k)
                        ->find();
                    ...
                    $upgrade_item = [
                        ...
                        "invoices_item_description" =>
                            ...
                            ")",
                        "upgrade_description" =>
                            $option_name . ":" . $old_sub_option . "=>" . $option["suboption_name"],
                    ];
                    $option["upgrade_item"] = $upgrade_item;
        ...
        if ($flag) {
            foreach ($alloption as $km => $m) {
                ...
                if (
                    (judgeQuantity($m["option_type"]) && intval($m["old_qty"]) != intval($m["qty"])) ||
                    (!judgeQuantity($m["option_type"]) && $m["old_suboption_id"] != $m["suboption_id"])
                ) {
                    if ($flag["type"] == 1) {
                        ...
                        if ($m["is_rebate"] || !$edition) {
                            ...
                            if ($old_option_total < $bates && $bates < $new_option_total) {
                                ...
                                $new_sub_pricing_sale =
                                    $m["new_sub_pricing"] - ($bates * $m["new_sub_pricing"]) / $new_option_total;
                                $new_sub_pricing_sale_base =
                                    $m["new_sub_pricing_base"] -
                                    ($bates * $m["new_sub_pricing_base"]) / $new_option_total;
                            } else {
                                if ($new_option_total < $bates) {
                                    $old_sub_pricing_sale = 0;
                                    $new_sub_pricing_sale = 0;
                                } else {
                                    $old_sub_pricing_sale =
                                        $m["old_sub_pricing"] - ($bates * $m["old_sub_pricing"]) / $old_option_total;
                                    $old_sub_pricing_sale_base = $m["old_sub_pricing_base"];
                                    $new_sub_pricing_sale =
                                        $m["new_sub_pricing"] - ($bates * $m["new_sub_pricing"]) / $new_option_total;
                                    $new_sub_pricing_sale_base = $m["new_sub_pricing_base"];
        ...
        $host_promo = \think\Db::name("host")
            ->alias("a")
            ->field("b.code")
            ->leftJoin("promo_code b", "a.promoid = b.id")
            ->where("a.id", $hid)
            ->find();
            ...
            if ($res1["status"] == 200) {
                ...
                if (!empty($alloption)) {
                    foreach ($alloption as &$v6) {
                        if ($v6["is_discount"]) {
                            if (
                                (judgeQuantity($v6["option_type"]) && intval($v6["old_qty"]) != intval($v6["qty"])) ||
                                (!judgeQuantity($v6["option_type"]) && $v6["old_suboption_id"] != $v6["suboption_id"])
                            ) {
                                if ($old_upgrade_value_type == "percent") {
                                    ...
                                    $v6["old_sub_pricing"] =
                                        $v6["old_sub_pricing"] - $old_upgrade_value > 0
                                            ? $v6["old_sub_pricing"] - $old_upgrade_value
                                            : 0;
                                }
                            }
                        }
                        if ($old_promo_data) {
                            if (isset($v6["upgrade_item"]["recurring_change"])) {
                                $v6["upgrade_item"]["recurring_change"] = bcsub(
                                    $v6["new_sub_pricing"],
                                    $v6["old_sub_pricing"],
                                    2,
                                );
            ...
            if ($result["status"] == 200) {
                ...
                if (!empty($alloption)) {
                    foreach ($alloption as &$v7) {
                        if ($v7["is_discount"]) {
                            if (
                                (judgeQuantity($v7["option_type"]) && intval($v7["old_qty"]) != intval($v7["qty"])) ||
                                (!judgeQuantity($v7["option_type"]) && $v7["old_suboption_id"] != $v7["suboption_id"])
                            ) {
                                if ($upgrade_value_type == "percent") {
                                    ...
                                    if ($upgrade_value_type = "fixed") {
                                        $v7["new_promo_price"] =
                                            bcsub($v7["new_sub_pricing"], $upgrade_value) <= 0
                                                ? $v7["new_sub_pricing"]
                                                : $upgrade_value;
                                        $v7["new_sub_pricing"] =
                                            bcsub($v7["new_sub_pricing"], $upgrade_value) <= 0
                                                ? 0
                                                : bcsub($v7["new_sub_pricing"], $upgrade_value);
                                    }
                                }
                            }
                        }
                        if ($promo && $promo["recurring"] == 1) {
                            if (isset($v7["upgrade_item"]["recurring_change"])) {
                                $v7["upgrade_item"]["recurring_change"] = bcsub(
                                    $v7["new_sub_pricing"],
                                    $v7["old_sub_pricing"],
                                    2,
                                );
            ...
            if (!empty($alloption)) {
                foreach ($alloption as $k3 => $v3) {
                    ...
                    if ($billingcycle == "onetime" || $billingcycle == "free") {
                        ...
                        $alloption[$k3]["new_promo_price"] = bcmul(
                            $v3["new_promo_price"],
                            bcdiv(
                                $surplustime,
                                $this->cycleToTime(
                    ...
                    $res_is_discount = \think\Db::name("product_config_options")
                        ->where("id", $v3["oid"])
                        ->where("is_discount", 1)
                        ->value("id");
                    if ($res_is_discount) {
                        if (($promoqualifies || $old_promoqualifies) && !empty($v3["upgrade_item"])) {
                            ...
                            $alloption[$k3]["upgrade_item"]["amount"] =
                                $v3["suboption_price"] - $alloption[$k3]["new_promo_price"];
                            $alloption[$k3]["upgrade_item"]["item_discount"] = $alloption[$k3]["new_promo_price"];
                            $alloption[$k3]["upgrade_item"]["discount_description"] =
                                $v3["option_name"] . lang("OPTIONS_UPGRADE_USE_PROMO") . ":" . $promo_code;
                        }
                    }
                }
            }
        ...
        if ($checkout) {
            ...
            try {
                ...
                if ($promo_code && $promoqualifies) {
                    $upgradepromo = \think\Db::name("promo_code")
                        ->field("upgrade_config")
                ...
                foreach ($alloption_filter as $kkk => $vvv) {
                    ...
                    if (!empty($upgrade_item_tmp)) {
                        $upgrade_data = [
                            ...
                            "date" => time(),
                            "relid" => $hid,
                            "original_value" => $upgrade_item_tmp["original_value"],
                            "new_value" => $upgrade_item_tmp["new_value"],
                            "new_cycle" => "",
                            "amount" => $upgrade_item_tmp["amountdue"],
                            "credit_amount" => "",
                            "days_remaining" => "",
                            "total_days_in_cycle" => "",
                            "new_recurring_amount" => "",
                            "recurring_change" => $upgrade_item_tmp["recurring_change"],
                            "status" => "Pending",
                            "paid" => $upgrade_item_tmp["paid"],
                            "description" =>
                                $host["name"] .
                                "(" .
                                $host["domain"] .
                        ...
                        if ($upgrade_item_tmp["amountdue"] >= 0 && $invoice_id) {
                            $invoice_items_data[] = [
                                ...
                                "uid" => $uid,
                                "type" => "upgrade",
                                "rel_id" => intval($upgrade_id),
                                "description" => $upgrade_item_tmp["invoices_item_description"],
                                "amount" => $upgrade_item_tmp["amountdue"],
                                "due_time" => time(),
                                "payment" => $payment,
                            ];
                            if (
                                $upgrade_item_tmp["item_discount"] &&
                                $upgrade_item_tmp["item_discount"] >= 0 &&
                                $diff_pricing >= 0
                            ) {
                                $invoice_items_data[] = [
                                    "invoice_id" => $invoice_id,
                                    ...
                                    "type" => "promo",
                                    "rel_id" => intval($upgrade_id),
                                    "description" => $upgrade_item_tmp["discount_description"] ?? "",
                                    "amount" => -$upgrade_item_tmp["item_discount"],
                                    "due_time" => time(),
                                    "payment" => $payment,
                                ];
                            }
                ...
                if ($flag && !empty($upgrade_ids[0])) {
                    $invoice_items_data = [
                        ...
                        "uid" => $uid,
                        "type" => "discount",
                        "rel_id" => intval($upgrade_ids[0]),
                        "description" => "客戶折扣",
                        "amount" => -$data["saleproducts"],
                        "due_time" => time(),
                        "payment" => $payment,
                    ];
                ...
                if (configuration("upgrade_down_product_config") && $credit_refund > 0) {
                    ...
                    try {
                        ...
                        $account1 = [
                            ...
                            "refund" => 0,
                            "description" => "产品'{$product_name}'降级退款,充值至余额",
                        ];
                        ...
                        $account2 = [
                            ...
                            "amount_in" => 0,
                            "fees" => "",
                            "amount_out" => $credit_refund,
                            "rate" => 1,
                            "trans_id" => "",
                            "invoice_id" => $invoice_refund_id,
                            "refund" => $aid,
                            "description" => "产品'{$product_name}'降级退款",
                        ];
                        ...
                        credit_log([
                            "uid" => $uid,
                            "desc" => $credit_refund > 0 ? "降级退款至余额" : "减少余额",
                            "amount" => $credit_refund,
                            "relid" => $invoice_id,
                        ]);
                        \think\Db::commit();
    ...
    public function checkUpgradePromo(
        $promo_code,
        $hid,
        $new_pid = "",
        $new_billingcycle = "",
        ...
        $host = \think\Db::name("host")
            ->alias("h")
            ->field("h.uid,h.billingcycle,p.id as pid")
            ->leftJoin("products p", "h.productid = p.id")
            ->leftJoin("product_groups pg", "pg.id = p.gid")
            ->where("h.id", $hid)
            ->find();
        ...
        if ($promo["once_per_client"]) {
            $promocount = \think\Db::name("orders")
                ->where("status", "Active")
                ->where("uid", $uid)
    ...
    public function doUpgrade($upgrade_id)
    {
        ...
        if (!empty($upgrade) && $upgrade["status"] != "Completed") {
            ...
            foreach ($configoptions as $v) {
                ...
                if (!judgeQuantity($option["option_type"])) {
                    $params["configoptions"][
                        explode("|", $option["option_name"])[0] ?? $option["option_name"]
                    ] = explode("|", $suboption["option_name"])[0];
                } else {
                    $params["configoptions"][explode("|", $option["option_name"])[0] ?? $option["option_name"]] =
                        $v["qty"];
                }
            }
            ...
            if ($upgrade["type"] == "product") {
                ...
                if ($old_billingcycle == "free" || $old_billingcycle == "onetime") {
                    $next_time = getNextTime(
                        $new_billingcycle,
                        $pay_type["pay_" . $new_billingcycle . "_cycle"],
                        0,
                ...
                $olds = \think\Db::name("host_config_options")
                    ->alias("a")
                    ->field("b.option_type,b.id as configid,a.optionid,a.qty,c.option_name")
                    ...
                    if (judgeOs($v["option_type"])) {
                        ...
                        foreach ($new_option_prices as &$vv) {
                            if (judgeOs($vv["optiontype"])) {
                                $os_subs = \think\Db::name("product_config_options_sub")
                ...
                foreach ($new_option_prices as $configoption) {
                    $insert_data = [
                        ...
                        "optionid" => $configoption["selsubid"],
                    ];
                ...
                $old_fieldids = \think\Db::name("customfields")
                    ->field("id,fieldname")
                    ->where("type", "product")
                    ->where("relid", $old_pid)
                    ->select()
                ...
                foreach ($old_fieldids as $ov) {
                    $new_fieldid = \think\Db::name("customfields")
                        ->where("type", "product")
                        ->where("relid", $new_pid)
                        ->where("fieldname", $ov["fieldname"])
                        ->value("id");
                    if ($new_fieldid) {
                        \think\Db::name("customfieldsvalues")
                            ->where("fieldid", $ov["id"])
                            ->where("relid", $hid)
                            ->update(["fieldid" => $new_fieldid, "update_time" => time()]);
                    }
                }
                ...
                $count = \think\Db::name("host_config_options")
                    ->where("relid", $hid)
                    ->where("configid", $config_id)
                    ->count();
                if (!$count) {
                    if (judgeQuantity($option_type)) {
                        $host_config_options_data = [
                            "relid" => $hid,
                            "configid" => $config_id,
                            "optionid" => explode(":", $new_value)[0],
                            "qty" => explode(":", $new_value)[1],
                        ];
                    } else {
                    ...
                    if (judgeQuantity($option_type) || judgeYesNo($option_type)) {
                        \think\Db::name("host_config_options")
                            ->where("relid", $hid)
                            ->where("configid", $config_id)
                            ->update(["optionid" => explode(":", $new_value)[0], "qty" => explode(":", $new_value)[1]]);
                    } else {
                        if ($new_value) {
                            \think\Db::name("host_config_options")
                                ->where("relid", $hid)
                                ->where("configid", $config_id)
                                ->update(["optionid" => $new_value]);
                        } else {
                            \think\Db::name("host_config_options")
                                ->where("relid", $hid)
                                ->where("configid", $config_id)
            ...
            if ($promo_code) {
                $promo = \think\Db::name("promo_code")
                    ->field("id,type,recurring,value")
                    ->where("code", $promo_code)
                    ->find();
                \think\Db::name("host")
                    ->where("id", $hid)
                    ->update(["promoid" => $promo["id"]]);
            } else {
                \think\Db::name("host")
                    ->where("id", $hid)
                    ->update(["promoid" => 0]);
    ...
    {
        $pids = [];
        $allowupgradeproducts = \think\Db::name("product_upgrade_products")
    ...
    public function getProductUpgradeConfig($hid, $pid, $billingcycle, $currencyid)
    {
        ...
        foreach ($newoptions as $kkk => $newoption) {
            ...
            if ($newoptiontype == "3") {
                ...
                $newsuboptionpricing = \think\Db::name("pricing")
                    ->where("type", "configoptions")
                    ->where("currency", $currencyid)
                    ->where("relid", $newsuboptionid)
                    ->find();
                $newsuboptionsetupfee = isset($newsuboptionpricing[$billingcycle])
                    ? $newsuboptionpricing[$setupfeecycle]
                    : 0;
                $newsuboptionprice = isset($newsuboptionpricing[$billingcycle])
                    ? $newsuboptionpricing[$billingcycle]
                    : 0;
                ...
                $setupvalue =
                    $newsuboptionsetupfee > 0 ? " + " . $newsuboptionsetupfee . " " . lang("ORDERS_SETUPFEE") : "";
                $suboptionsarray[] = [
                    "id" => $newsuboptionid,
                    "hidden" => $newsuboptionhidden,
                    "name" => $newsuboptionname . $setupvalue,
                    "nameonly" => $opnameonly,
                    "recurring" => $newsuboptionprice,
                ];
                if (!$selectedvalue) {
                    $selectedvalue = 0;
                }
                ...
                if (judgeQuantity($newoptiontype)) {
                    $newsuboptions = \think\Db::name("product_config_options_sub")
                        ->alias("pcos")
                        ->field("")
                        ->leftJoin("pricing p", "p.relid = pcos.id")
                        ->where("p.type", "configoptions")
                        ->where("p.currency", $currencyid)
                        ->where("pcos.config_id", $newoptionid)
                        ->order("pcos.sort_order ASC")
                        ->order("pcos.id asc")
                        ->select();
                    $exist = false;
                    foreach ($newsuboptions as $newsuboption) {
                        ...
                        if (!is_numeric($selectedvalue) || $selectedvalue < 0) {
                            ...
                            if ($newsuboptionqtymin == $newoptionqtymin) {
                                ...
                                $setupvalue =
                                    $newsuboptionsetupfee > 0
                                        ? " + " . $newsuboptionsetupfee . " " . lang("ORDERS_SETUPFEE")
                                        : "";
                                $suboptionsarray[] = [
                                    "id" => $newsuboptionid,
                                    "hidden" => $newsuboptionhidden,
                        ...
                        if ($newsuboptionqtymin <= $selectedvalue && $selectedvalue <= $newsuboptionqtymax) {
                            ...
                            $suboptionsarray[] = [
                                ...
                                "nameonly" => $opnameonly,
                                "recurring" => $newsuboptionprice,
                                "salerecurring" => $newsuboption_saleprice,
                            ];
                            $exist = true;
                        }
                    }
                    if ($newsuboptions[0] && !$exist) {
                        $newsuboptionsetupfee = isset($newsuboptions[0][$billingcycle])
                            ? $newsuboptions[0][$setupfeecycle]
                            : 0;
                        $newsuboptionprice = isset($newsuboptions[0][$billingcycle])
                            ? $newsuboptions[0][$billingcycle]
                            : 0;
                        ...
                        $selsuboptionname = strpos($newsuboptions[0]["option_name"], "|")
                            ? explode("|", $newsuboptions[0]["option_name"])[1]
                            : $newsuboptions[0]["option_name"];
                        $opnameonly = $newsuboptionname;
                        if ($newsuboptionprice > 0) {
                            $newsuboptionname .= " " . $newsuboptionprice;
                        }
                    ...
                    $newsuboptions = \think\Db::name("product_config_options_sub")
                        ->alias("pcos")
                        ->field("pcos.hidden,pcos.option_name,p.*")
                        ->leftJoin("pricing p", "p.relid = pcos.id")
                        ->where("p.type", "configoptions")
                        ->where("p.currency", $currencyid)
                        ->where("pcos.config_id", $newoptionid)
                    ...
                    foreach ($newsuboptions as $newsuboption) {
                        ...
                        if (!$newsuboptionhidden || $newsuboptionid == $selectedvalue) {
                            $suboptionsarray[] = [
                                ...
                                "nameonly" => $opnameonly,
                                "recurring" => $newsuboptionprice,
                                "nameandprice" => $newsuboptionname,
                                "setup" => $newsuboptionsetupfee,
                                "fullprice" => $newsuboptionprice,
                            ];
                        }
            ...
            $newconfigoptions[] = [
                "id" => $newoptionid,
                "hidden" => $newoptionhidden,
                "optionname" => $newoptionname,
                "optiontype" => $newoptiontype,
                "selsubid" => $selsubid,
                "suboptionname" => $selsuboptionname,
                "selsetupfee" => $selsetup,
                "selrecurring" => $selrecurring,
                "selectedqty" => $selectedqty,
                "selname" => $selname,
                "newoptionqtymin" => $newoptionqtymin,
                "newoptionqtymax" => $newoptionqtymax,
                "suboptions" => $suboptionsarray,
                "is_rebate" => $newoption["is_rebate"],
                "is_discount" => $newoption["is_discount"],
            ];
        }
        return $newconfigoptions;
    }
    public function upgradeProductCommon(
        $hid,
        $new_pid,
        $new_billingcycle,
        $currency = "",
        ...
        $old_host = \think\Db::name("product_groups")
            ->alias("pg")
            ->field(
                "concat(pg.name,\"-\",p.name) as host,h.regdate,h.flag,h.uid,h.domain,p.description,p.id as pid,p.name as pname,h.firstpaymentamount,h.amount,h.billingcycle,h.nextduedate,p.pay_type",
            )
            ->leftJoin("products p", "p.gid = pg.id")
            ->leftJoin("host h", "h.productid = p.id")
            ->where("h.id", $hid)
            ->find();
        ...
        if (
            !$cart->validBillingCycle($new_billingcycle) ||
            !$product_model->checkProductPrice($new_pid, $new_billingcycle, $currency_id)
        ) {
        ...
        $is_zjmf_pai = false;
        if (
            $new_product["api_type"] == "zjmf_api" &&
            $new_product["upstream_pid"] > 0 &&
            $new_product["upstream_price_type"] == "percent"
        ) {
            $is_zjmf_pai = true;
        }
        ...
        foreach ($new_option_prices as $new_option_price) {
            if ($new_option_price["hidden"] == 0) {
                if ($is_zjmf_pai) {
                    $new_option_price["selrecurring"] =
                        ($new_option_price["selrecurring"] * $new_product["upstream_price_value"]) / 100;
                }
                if ($new_product["api_type"] == "resource" && function_exists("resourceUserGradePercent")) {
        ...
        if ($old_billingcycle == "onetime" || $old_billingcycle == "free") {
            ...
            if ($new_billingcycle == "onetime" || $new_billingcycle == "free") {
                $old_surplus_amount = bcmul(
                    bcdiv(
                        $old_amount,
                        $this->cycleToTime(
                            $old_billingcycle,
                            $old_paytype["pay_{$old_billingcycle}_cycle"],
                            $old_start,
                            $old_paytype["pay_ontrial_cycle_type"],
                        ),
                        true,
                ...
                $new_surplus_amount = bcmul(
                    bcdiv(
                        $new_amount,
                        $this->cycleToTime(
                            $new_billingcycle,
                            $new_paytype["pay_{$new_billingcycle}_cycle"],
                            $old_start,
                            $new_paytype["pay_ontrial_cycle_type"],
                            true,
                        ),
                    ),
                    $old_surplus_time,
        ...
        if ($checkout) {
            ...
            try {
                ...
                if ($promo_code) {
                    $upgradepromo = \think\Db::name("promo_code")
                        ->field("upgrade_config,type,value")
                        ->where("code", $promo_code)
                        ->find();
                    $upgradeconfig = unserialize($upgradepromo["upgrade_config"]);
                ...
                $last_upgrade_invoice = \think\Db::name("invoices")
                    ->field("id,credit")
                    ->where("uid", $uid)
                    ->where("type", "upgrade")
                    ->where("status", "Unpaid")
                ...
                if (isset($last_upgrade_invoice["credit"]) && $last_upgrade_invoice["credit"] > 0) {
                    ...
                    credit_log([
                        "uid" => $uid,
                        "desc" => "Credit Removed from Invoice #" . $last_upgrade_invoice_id,
                        "amount" => -$credit,
                        "relid" => $last_upgrade_invoice_id,
                    ]);
                }
                ...
                $order_data = [
                    ...
                    "promo_value" => $promovalue,
                    "payment" => $payment,
                    "amount" => $amount_with_discount <= 0 ? 0 : $amount_with_discount,
                ];
                $invoice_data = [
                    "uid" => $uid,
                    "create_time" => time(),
                    "due_time" => time(),
                    "subtotal" => $amount_with_discount,
                    "credit" => "",
                    "total" => $amount_with_discount,
                    "status" => "Unpaid",
                    "payment" => $payment,
                    "type" => "upgrade",
                    "url" => "servicedetail?id=" . $hid,
                ...
                $upgrades = [
                    ...
                    "date" => time(),
                    "relid" => $hid,
                    "original_value" => $old_pid,
                    "new_value" => $new_pid,
                    "new_cycle" => $new_billingcycle,
                    "amount" => $amount_without_discount,
                    "recurring_change" => $difference,
                    "order_id" => $order_id,
                    "credit_amount" => "",
                    "days_remaining" => "",
                    "total_days_in_cycle" => "",
                    ...
                    "status" => "Pending",
                    "paid" => $amount_with_discount <= 0 ? "Y" : "N",
                    "description" => $old_host["host"] . "=>" . $new_product_name . "\n" . $old_host["domain"],
                ];
                ...
                if ($invoice_id) {
                    $invoice_items_data[] = [
                        ...
                        "rel_id" => $upgrade_id,
                        "description" =>
                            lang("UPGRADE_OR_DOWNGRADE_PRODUCT") .
                            ": " .
                            $old_product_name .
                            "\n" .
                            $old_product_name .
                    ...
                    if ($discount > 0) {
                        $invoice_items_data[] = [
                            ...
                            "type" => "promo",
                            "rel_id" => $upgrade_id,
                            "description" => lang("UPGRADE_OR_DOWNGRADE_PRODUCT_PROMO") . $promo_code . "",
                            "amount" => $discount * -1,
                            "due_time" => time(),
                            "payment" => $payment,
                        ];
                ...
                $has = \think\Db::name("invoice_items")
                    ->alias("a")
                    ->field("b.id,b.status,b.subtotal")
                ...
                if (configuration("upgrade_down_product_config") && $credit_refund > 0) {
                    ...
                    try {
                        ...
                        $account1 = [
                            ...
                            "invoice_id" => $invoice_refund_id,
                            "refund" => 0,
                            "description" => "产品'{$old_product_name}'降级退款,充值至余额",
                        ];
                        $aid = \think\Db::name("accounts")->insertGetId($account1);
                        $account2 = [
                            "uid" => $uid,
                            ...
                            "invoice_id" => $invoice_refund_id,
                            "refund" => $aid,
                            "description" => "产品'{$old_product_name}'降级退款",
                        ];
                        \think\Db::name("accounts")->insert($account2);
                        $invoice_refund_item = [
                            "invoice_id" => $invoice_refund_id,
                    ...
                    $invoice_refund_item = [
                        ...
                        "type" => "upgrade",
                        "rel_id" => 0,
                        "description" => "从{$old_product_name}降级至{$new_product_name}",
                        "amount" => 0,
                        "due_time" => time(),
                        "payment" => $payment,
                    ];
        ...
        $data["amount"] =
            !configuration("upgrade_down_product_config") && $amount_show < 0 ? number_format(0, 2) : $amount_show;
        $data["discount"] = bcsub($discount, 0, 2);
        $data["amount_total"] =
            !configuration("upgrade_down_product_config") && $amount_total < 0
                ? number_format(0, 2)
                : bcsub($amount_total, 0, 2);
        $data["currency"] = $currency;
        $data["promo_code"] = $promo_code ?: "";
