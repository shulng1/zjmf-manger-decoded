            ...
            if ($res_condition["final"]) {
                ...
                if (is_array($res_r) && count($res_r) > 0 && $res_r["config_id"] > 0) {
                    ...
                    if ($sub_id) {
                        ...
                        if (!$res_result["final"]) {
                            return $res_condition["describe"] .
                                $this->relation_text[$res_r["relation"]] .
                                $res_result["describe"];
                        }
                    }
                }
            ...
            case 1:
                if ($final_sum) {
                    $describe =
                        ...
                        $config_name .
                        "】在" .
                        $condition_one["qty_minimum"] .
                        "~" .
                        $condition_one["qty_maximum"];
                    $final =
                        $condition_one["qty_minimum"] <= $result_value && $result_value <= $condition_one["qty_maximum"]
                            ? true
                            : false;
                } else {
            ...
            case 2:
                if ($final_sum) {
                    $describe =
                        ...
                        $condition_one["qty_maximum"];
                    $final =
                        $result_value < $condition_one["qty_minimum"] && $condition_one["qty_maximum"] < $result_value
                            ? true
                            : false;
                } else {
    ...
    public function aloneCheckConf($product_id, $conf, $check_id = null, &$data = [])
    {
        ...
        foreach ($response as $k => $res) {
            ...
            if ($res_condition["final"]) {
                $res_r = $res["result"][0];
                $result_value_r = $conf[$res_r["config_id"]];
                $res_result = $this->aloneCheckOne(
                    $res_r,
                    $result_value_r,
                    $this->relation[$res_r["relation"]],
                    $check_id,
                    $data,
                );
            }
