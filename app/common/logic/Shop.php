...
class Shop
{
    ...
    private function save($status = 1)
    {
        ...
        if (!empty($uid)) {
            $udata = [
                ...
                "status" => $status,
                "expire_time" => strtotime("next year"),
                "update_time" => time(),
            ];
    ...
    private function checkProductToArr(
        $pid,
        $billingcycle,
        $serverid,
        $configoption,
        ...
        $product_data = \think\Db::name("products")
            ->field("p.*")
            ->alias("p")
            ->leftJoin("product_groups g", "p.gid=g.id")
            ->where("p.id", $pid)
            ->find();
                    ...
                    if ($option_type == 3) {
                        if (!empty($configoption[$config_id])) {
                            $exists_data = \think\Db::name("product_config_options_sub")
                                ...
                                ->where("hidden", 0)
                                ->where("id", $configoption[$config_id])
                                ->order("id", "asc")
                                ->find();
        ...
        $all_customfield_arr = \think\Db::name("customfields")
            ->where("relid", $pid)
            ->where("type", "product")
            ->select()
            ->toArray();
        ...
        if (!empty($customfield) && is_array($customfield)) {
            foreach ($customfield as $k => $v) {
                ...
                if (in_array($k, $custom_id_arr)) {
                    ...
                    if (
                        $v &&
                        !empty($custom_data["regexpr"]) &&
                        in_array($custom_data["fieldtype"], array_keys(config("customfields")))
                    ) {
    ...
    private function getCurrency($currency)
    {
        ...
        if ($currency == 0) {
            ...
            $currdata = \think\Db::name("currencies")
    ...
    public function getShopPageData($currency = 0, $pos = [])
    {
        ...
        if (!empty($products)) {
            ...
            foreach ($products as $k => $v) {
                ...
                $is_zjmfapi = false;
                if (
                    $product_data["api_type"] == "zjmf_api" &&
                    $product_data["upstream_version"] > 0 &&
                    $product_data["upstream_price_type"] == "percent"
                ) {
                    $is_zjmfapi = true;
                }
                if ($product_data["api_type"] == "resource") {
                    $user_grade = resourceUserGradePercent($uid, $pid);
                    $shop = \think\Db::name("res_products")
                        ->field("b.id,b.name,b.img")
                        ->alias("a")
                        ->leftJoin("res_shop b", "a.shop_id=b.id")
                ...
                if (
                    in_array($product_data["type"], array_keys(config("developer_app_product_type"))) &&
                    $product_data["p_uid"] > 0
                ) {
                    $nav_group = \think\Db::name("host")
                        ->alias("a")
                        ->field("c.id,c.fa_icon,c.groupname,a.id as hostid")
                        ->leftJoin("products b", "a.productid = b.id")
                        ->leftJoin("nav_group c", "c.id = b.groupid")
                        ->where("a.id", $hostid)
                        ->find();
                ...
                if ($billingcycle != "free") {
                    ...
                    $product_pricing =
                        $prodcut_pricing_data[$itself_field] > 0 ? $prodcut_pricing_data[$itself_field] : 0;
                    $product_setup_pricing =
                        $prodcut_pricing_data[$setupfee_field] > 0 ? $prodcut_pricing_data[$setupfee_field] : 0;
                } else {
                    $product_pricing = 0;
                ...
                if (!empty($configoptions)) {
                    ...
                    $config_data = \think\Db::name("product_config_options")
                    ...
                    foreach ($config_data as $j => $vvv) {
                        ...
                        if (judgeQuantity($option_type)) {
                            $option_sub_data = \think\Db::name("product_config_options_sub")
                                ->where("config_id", $config_id)
                                ->where("qty_minimum", "<=", $option_sub)
                                ->where("qty_maximum", ">=", $option_sub)
                                ->find();
                        } else {
                            $option_sub_data = \think\Db::name("product_config_options_sub")
                                ->where("id", $option_sub)
                        ...
                        if ($billingcycle != "free" && $vvv["hidden"] != 1) {
                            $option_sub_pricing = $this->getConfigoptionsPricing(
                                $option_sub_id,
                                $pricing_field_str,
                                $currencyid,
                            );
                            $sub_pricing =
                                $option_sub_pricing[$itself_field] > 0 ? $option_sub_pricing[$itself_field] : 0;
                            $sub_setup_pricing =
                                $option_sub_pricing[$setupfee_field] > 0 ? $option_sub_pricing[$setupfee_field] : 0;
                        } else {
                            $sub_pricing = 0;
                            $sub_setup_pricing = 0;
                        }
                        $config_str = explode("|", $option_name)[1] ? explode("|", $option_name)[1] : $option_name;
                        $sub_option_name = explode("|", $sub_option_name)[1]
                            ? explode("|", $sub_option_name)[1]
                            : $sub_option_name;
                        ...
                        if (judgeQuantity($option_type)) {
                            ...
                            $configoptions_showarr[$config_str]["value"] =
                                $option_sub . configoptionsUnit($option_type);
                        } else {
                            if ($sub_pricing >= 0) {
                                $config_total_pricing += $sub_pricing;
                                $config_setup_total_pricing += $sub_setup_pricing;
                        ...
                        if ($is_zjmfapi) {
                            ...
                            $sub_setup_pricing =
                                bcmul($sub_setup_pricing, $product_data["upstream_price_value"], 2) / 100;
                        }
                        if ($product_data["api_type"] == "resource") {
                ...
                if ($is_zjmfapi) {
                    ...
                    $config_total_pricing =
                        bcmul($config_total_pricing, $product_data["upstream_price_value"], 2) / 100;
                    $product_setup_pricing =
                        bcmul($product_setup_pricing, $product_data["upstream_price_value"], 2) / 100;
                    $config_setup_total_pricing =
                        bcmul($config_setup_total_pricing, $product_data["upstream_price_value"], 2) / 100;
                }
                if ($product_data["api_type"] == "resource") {
                ...
                if ($hostid > 0) {
                    ...
                    if (!empty($auth) && $auth["config_option2"] == "professional") {
                        if (!empty($activity)) {
                            $cart_products["discount_pricing"] = in_array($activity["object"], [0, 1])
                                ? round(($cart_products["discount_pricing"] * (100 - $activity["discount"])) / 100, 2)
                                : $cart_products["discount_pricing"];
                        }
                        $cart_products["discount_pricing"] = round(
                            ($cart_products["discount_pricing"] * (100 - $product["professional_discount"])) / 100,
                            2,
                        );
                        $cart_products["discount_amount"] = bcsub($pricing, $cart_products["discount_pricing"], 2);
                    } else {
                        if (!empty($activity)) {
                            $cart_products["discount_pricing"] = in_array($activity["object"], [0, 2])
                                ? round(($cart_products["discount_pricing"] * (100 - $activity["discount"])) / 100, 2)
                                : $cart_products["discount_pricing"];
                            $cart_products["discount_amount"] = bcsub($pricing, $cart_products["discount_pricing"], 2);
                        }
            ...
            if (isset($cart_data["promo"])) {
                ...
                foreach ($pagedata["cart_products"] as $kk => $vv) {
                    $use_promo_data[] = [
                        ...
                        "billingcycle" => $vv["billingcycle"],
                        "pricing" => $vv["pricing"],
                        "setup_pricing" => $vv["setup_pricing"],
                        "qty" => $vv["qty"],
                        "product_sale" => $vv["product_sale"],
                        "configoptions_base_sale" => $vv["configoptions_base_sale"],
                        "total_sale" => $vv["total_sale"],
                        "total_sale_setupfee" => $vv["total_sale_setupfee"],
                    ];
                }
                $res = $this->checkPromoCode($promo);
                if ($res == 1) {
                    $promo_price_data = $this->handlePromoPrice(
                        $use_promo_data,
                        $promo,
                        $currency_prefix,
                        $currency_suffix,
        ...
        if (!empty($products)) {
            ...
            if (isset($cart_data["promo"])) {
                ...
                if ($res == 1) {
                    ...
                    if ($promo_price_data["status"] == "success") {
                        $pagedata["promo"] = [
                            "promo" => $promo_price_data["data"]["promo"],
                            "promo_desc" => $promo_price_data["data"]["promo_desc_str"],
                            "promo_price" => $promo_price_data["data"]["promo_price"],
                        ];
                        $subtotal = $subtotal - $promo_price_data["data"]["promo_price"];
                    }
                }
    ...
    private function getSaleData($uid, $pos, &$products)
    {
        ...
        foreach ($products as $k5 => &$v5) {
            ...
            if ($flag["type"] == 1) {
                ...
                $bates = $flag["bates"];
                $v5["product_sale"] =
                    ($v5["product_base_sale"] / $v5["product_pricing"]) * ($v5["product_pricing"] - $bates);
                $v5["product_sale_setupfee"] =
                    ($v5["product_base_sale_setupfee"] / $v5["product_pricing"]) * ($v5["product_pricing"] - $bates);
            ...
            foreach ($configoptions_base_sale as &$v6) {
                ...
                if ($v6["is_rebate"] || !$edition) {
                    if ($flag["type"] == 1) {
                        ...
                        $v6["config_rebate_price"] = bcmul(
                            $bates / 100,
                            $v6["config_base_sale"] - $v6["config_base_sale_setupfee"],
                            20,
                        );
                        ...
                        $bates = $flag["bates"];
                        $v6["config_sale"] =
                            ($v6["config_base_sale"] / $v5["product_pricing"]) * ($v5["product_pricing"] - $bates);
                        $v6["config_sale_setupfee"] =
                            ($v6["config_base_sale_setupfee"] / $v5["product_pricing"]) *
                            ($v5["product_pricing"] - $bates);
                        $v6["config_rebate_price"] =
                            (($v6["config_base_sale"] - $v6["config_base_sale_setupfee"]) / $v5["product_pricing"]) *
                            ($v5["product_pricing"] - $bates);
                        $v6["config_rebate_setupfee"] =
                            ($v6["config_base_sale_setupfee"] / $v5["product_pricing"]) *
                            ($v5["product_pricing"] - $bates);
                    }
                } else {
    ...
    private function handlePromoPrice(
        $use_promo_data,
        $promo,
        $currency_prefix,
        $currency_suffix,
        ...
        if ($promo_flag && !empty($use_promo_data)) {
            ...
            if ($promotype == "percent") {
                ...
                foreach ($use_promo_data as $kk => $vv) {
                    if (in_array($kk, $pos)) {
                        $discount_pricing = bcadd(
                            $discount_pricing,
                            bcmul($vv["product_sale"], 1 - $promovalue / 100, 2) * $vv["qty"],
                            2,
                        );
                        foreach ($vv["configoptions_base_sale"] as $kkk => $vvv) {
                            if ($vvv["is_discount"] == 1) {
                                $discount_pricing = bcadd(
                                    $discount_pricing,
                                    bcmul($vvv["config_sale"], 1 - $promovalue / 100, 2) * $vv["qty"],
                                    2,
                                );
                            }
                        }
                ...
                $pagedata["promo_price_str"] =
                    "-" . $currency_prefix . sprintf("%.2f", $discount_pricing) . $currency_suffix;
                ...
                foreach ($use_promo_data as $kk => $vv) {
                    ...
                    if (in_array($kk, $pos)) {
                        $discount_pricing +=
                            ($now_product_pricing < $promovalue ? $now_product_pricing : $promovalue) * $vv["qty"];
                    }
                }
                $promodescription =
                    $currency_prefix . sprintf("%.2f", $promovalue) . $currency_suffix . $promo_curring_str;
    ...
    {
        $data = \think\Db::name("product_groups")
            ->alias("g")
            ->field(
                "g.name as groupname,p.id,p.type,p.gid,p.name as productname,p.pay_type,p.pay_method,p.server_group",
            )
            ->leftJoin("product p", "p.gid=g.id")
            ->where("g.hidden", 0)
            ->where("p.hidden", 0)
            ->where("p.id", $pid)
            ->find();
    ...
    {
        $data = \think\Db::name("servers")
            ->alias("s")
            ->field("s.id,s.gid,s.name")
            ->leftJoin("product p", "p.server_group=s.gid")
            ->where("s.disabled", 0)
            ->where("p.id", $pid)
            ->where("s.id", $serverid)
            ->find();
