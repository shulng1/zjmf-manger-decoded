...
class Cart
{
    ...
    public function getProductCycle($pid, $currencyid)
    {
        $product = \think\Db::name("products")
            ->field("id,name,description,pay_type,host,password,allow_qty,stock_control,qty")
            ->where("id", $pid)
            ->find();
        ...
        $product["host"]["host"] = generateHostName(
            $product["host"]["prefix"],
            $product["host"]["rule"],
            $product["host"]["show"],
        );
    ...
    {
        $product = \think\Db::name("products")
            ->field("id,name,description,pay_type,host,password,allow_qty")
            ->where("id", $pid)
            ->find();
        $product = array_map(function ($v) {
            return is_string($v) ? htmlspecialchars_decode($v, ENT_QUOTES) : $v;
    ...
    public function getProductDefaultConfigPrice($pid, $currency, $billingcycle, &$rebate_total, &$setupfee_total = 0)
    {
        $options = \think\Db::name("product_config_options")
            ->alias("a")
            ->field("a.id as cid,a.option_type,a.qty_minimum,a.is_discount,a.option_name,a.is_rebate")
            ->leftJoin("product_config_groups b", "a.gid = b.id")
            ->leftJoin("product_config_links c", "c.gid = b.id")
            ->leftJoin("products d", "c.pid = d.id")
            ->where("d.id", $pid)
            ->where("a.hidden", 0)
            ->select()
            ->toArray();
        if ($options) {
            ...
            $field_options = \think\Db::name("product_config_options")
                ->whereIn("id", $options_ids)
                ->select()
                ->toArray();
        ...
        foreach ($options as $option) {
            ...
            if (judgeQuantity($option["option_type"])) {
                if (judgeQuantityStage($option["option_type"])) {
                    ...
                    $config_total +=
                        ($pricing[$price_type[$billingcycle][0]] ?? 0) * ($qty_min < 0 ? 0 : intval($qty_min));
                    if ($option["is_rebate"] || !$edition) {
                        $rebate_total +=
                            ($pricing[$price_type[$billingcycle][0]] ?? 0) * ($qty_min < 0 ? 0 : intval($qty_min));
                    }
    ...
    public function getProductPriceArea1($pid)
    {
        ...
        foreach ($cycles as $cycle) {
            ...
            foreach ($options as $option) {
                ...
                $pricings = \think\Db::name("pricing")
                    ->alias("a")
                    ->field("a.*,b.qty_minimum,b.qty_maximum")
                    ->leftJoin("product_config_options_sub b", "a.relid = b.id")
                    ->where("a.type", "configoptions")
                    ->where("a.currency", $currency)
                    ->where("b.hidden", 0)
                ...
                foreach ($pricings as $pricing) {
                    ...
                    if (judgeQuantity($option_type)) {
                        ...
                        if (judgeQuantityStage($option_type)) {
                            ...
                            $other_sub_total +=
                                ($pricing[$price_type[$billingcycle][0]] ?? 0) * ($qty_max < 0 ? 0 : intval($qty_max));
                            if ($qty_max > 0) {
