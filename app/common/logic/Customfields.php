...
class Customfields
{
    public function add($relid, $type = "product", $param = [])
    {
        ...
        if (!empty($param["addfieldname"])) {
            $add_field = [
                "type" => $type,
                "relid" => $relid,
                "fieldname" => $param["addfieldname"],
                "fieldtype" => $param["addfieldtype"] ?: "text",
                "description" => $param["addcustomfielddesc"] ?: "",
                "fieldoptions" => $param["addfieldoptions"] ?: "",
                "regexpr" => $param["addregexpr"] ?: "",
                "adminonly" => $param["addadminonly"] ?: "",
                "required" => $param["addrequired"] ?: "",
                "showorder" => $param["addshoworder"] ?: "",
                "sortorder" => $param["addsortorder"] ?: 0,
                "create_time" => time(),
                "showdetail" => $param["addshowdetail"] ?: 0,
            ];
        ...
        if (is_array($customfieldname) && !empty($customfieldname)) {
            ...
            foreach ($customfieldname as $k => $v) {
                if (in_array($k, $custom_id_arr)) {
                    $update = [
                        "fieldname" => $param["customfieldname"][$k],
                        "fieldtype" => $param["customfieldtype"][$k] ?: "text",
                        "description" => $param["customfielddesc"][$k] ?: "",
                        "fieldoptions" => $param["customfieldoptions"][$k] ?: "",
                        "regexpr" => $param["customfieldregexpr"][$k] ?: "",
                        "adminonly" => $param["customadminonly"][$k] ?: "",
                        "required" => $param["customrequired"][$k] ?: "",
                        "showorder" => $param["customshoworder"][$k] ?: "",
                        "sortorder" => $param["customsortorder"][$k] ?: 0,
                        "showdetail" => $param["customshowdetail"][$k] ?: 0,
                        "update_time" => time(),
                    ];
                    foreach ($customfields as $k1 => $v1) {
                        if ($k == $v1["id"]) {
                            ...
                            if ($update["fieldoptions"] != $v1["fieldoptions"]) {
                                $dec1 .=
                                    "字段选项由“" . $v1["fieldoptions"] . "”改为“" . $update["fieldoptions"] . "”，";
                            }
    ...
    {
        return \think\Db::name("customfields")
            ->field($field)
            ->where("relid", $relid)
            ->where("type", $type)
            ->order("sortorder", "asc")
    ...
    {
        $fields_array = \think\Db::name("customfields")
            ->field("id")
            ->where("relid", $relid)
            ->where("type", $type)
            ->select()
        ...
        if (is_array($customfield_param) && !empty($customfield_param)) {
            foreach ($customfield_param as $k => $v) {
                ...
                if (!empty($field)) {
                    ...
                    $fieldvalue = \think\Db::name("customfieldsvalues")
                        ->field("id")
                        ->where("fieldid", $k)
                        ->where("relid", $relid)
                        ->find();
    ...
    public function getCustomField($pid)
    {
        $custom_data = \think\Db::name("customfields")
            ->field("id,fieldname,fieldtype,fieldoptions,regexpr")
            ->where("type", "product")
            ->where("relid", $pid)
    ...
    {
        $custom_data = \think\Db::name("customfields")
            ->field("id,fieldname,description,fieldtype,fieldoptions,regexpr,required")
            ->where("type", "product")
            ->where("relid", $pid);
        if (!$is_admin) {
    ...
    public function saveCustomField(
        ...
        if ($pid != $new_productid && $new_productid > 0) {
            $old = \think\Db::name("customfields")
                ->where("type", $type)
                ->where("relid", $field_relid)
                ->select()
                ->toArray();
            $oldcustomfieldsvalues = \think\Db::name("customfieldsvalues")
                ->whereIn("fieldid", array_column($old, "id"))
                ->where("relid", $value_relid)
                ->select()
                ->toArray();
            $new = \think\Db::name("customfields")
                ->where("type", $type)
                ->where("relid", $new_productid)
                ->select()
                ->toArray();
            $old_con = [];
            foreach ($old as $v) {
            ...
            $customfields_data = \think\Db::name("customfields")
            ...
            foreach ($customfield as $key => $value) {
                if (in_array($key, $customfields_ids)) {
                    $exist_data = \think\Db::name("customfieldsvalues")
                        ->where("fieldid", $key)
                        ->where("relid", $value_relid)
                        ->find();
                    if (!empty($exist_data)) {
                        if ($exist_data["value"] != $value) {
                            \think\Db::name("customfieldsvalues")
                                ->where("fieldid", $key)
                                ->where("relid", $value_relid)
                                ->update(["value" => $value, "update_time" => time()]);
                        }
                    } else {
                        $icustom_data = [
                            "fieldid" => $key,
