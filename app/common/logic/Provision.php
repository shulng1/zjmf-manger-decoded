...
class Provision
{
    ...
    protected $HostModel;
    protected $support = [
        "CreateAccount" => ["type" => "button", "auth" => "admin", "func" => "create", "des" => "开通"],
        "SuspendAccount" => ["type" => "button", "auth" => "admin", "func" => "suspend", "des" => "暂停"],
        "UnsuspendAccount" => ["type" => "button", "auth" => "admin", "func" => "unsuspend", "des" => "解除暂停"],
        "TerminateAccount" => ["type" => "button", "auth" => "admin", "func" => "terminate", "des" => "删除"],
        "Renew" => ["auth" => "both", "des" => "续费"],
        "ChangePackage" => ["des" => "升降级"],
        "On" => ["type" => "button", "auth" => "both", "place" => "control", "func" => "on", "des" => "开机"],
        "Off" => ["type" => "button", "auth" => "both", "place" => "control", "func" => "off", "des" => "关机"],
        "Reboot" => ["type" => "button", "auth" => "both", "place" => "control", "func" => "reboot", "des" => "重启"],
        "HardOff" => [
            "type" => "button",
            "auth" => "both",
            "place" => "control",
        ...
        "Vnc" => ["type" => "button", "auth" => "both", "place" => "console", "func" => "vnc", "des" => "VNC"],
        "Sync" => ["type" => "button", "auth" => "admin", "place" => "control", "func" => "sync", "des" => "拉取信息"],
        "Status" => ["auth" => "both", "func" => "status", "des" => "获取状态"],
        "ManagePanel" => ["type" => "button", "auth" => "admin", "func" => "panel", "des" => "管理面板"],
    ];
    protected $options = [
        "text" => ["name", "placeholder", "description", "default", "type", "key"],
        "password" => ["name", "placeholder", "description", "default", "type", "key"],
        "yesno" => ["name", "description", "default", "type", "key"],
        "radio" => ["name", "description", "options", "default", "type", "key"],
        "dropdown" => ["name", "description", "options", "default", "type", "key"],
        "textarea" => ["name", "placeholder", "description", "default", "rows", "cols", "type", "key"],
    ];
            ...
            if (function_exists($function)) {
                ...
                if (is_array($data)) {
                    foreach ($data as $k => $v) {
                        if (is_array($v)) {
                            if (
                                file_exists($this->dir . $module . "/" . $v["template"]) ||
                                file_exists($this->dir2 . $module . "/" . $v["template"])
                            ) {
        ...
        if ($this->checkAndRequire($module)) {
            ...
            if (function_exists($function)) {
                ...
                if (isset($data[$key]) && function_exists($detail_func)) {
                    ...
                    if (is_array($res)) {
                        if (
                            file_exists($this->dir . $module . "/" . $res["template"]) ||
                            file_exists($this->dir2 . $module . "/" . $res["template"])
                        ) {
                            $view = new \think\View();
                            $view->init("Think");
                            ...
                            if (!empty($api_url)) {
                                ...
                                $view->assign(
                                    "MODULE_CUSTOM_API",
                                    request()->domain() . request()->rootUrl() . "/provision/custom/" . $hostid,
                                );
                            }
            ...
            foreach ($this->support as $k => $v) {
                ...
                if ($is_admin) {
                    if (($v["auth"] == "admin" || $v["auth"] == "both") && $hide[$k] !== true) {
                        if (
                            ($params["domainstatus"] == "Active" || $params["domainstatus"] == "Suspended") &&
                            $k == "CreateAccount"
                        ) {
                            continue;
                        }
                ...
                if ($handle = opendir($this->dir)) {
                    while (($file = readdir($handle)) !== false) {
                        if (
                            $file != "." &&
                            $file != ".." &&
                            filetype($this->dir . $file) == "dir" &&
                            preg_match("/^[a-z][a-z0-9]+\$/", $file)
                        ) {
    ...
    protected function execSupportFunc($name, $hostid = 0)
    {
        ...
        if (!empty($module) && !empty($params["hostid"]) && $define && $name != "Status") {
            if ($result["status"] == "success") {
                active_log_final(
                    lang("MODULE_EXEC_SUCCESS", [$module, $this->support[$name]["des"], $params["hostid"]]),
                    0,
                    2,
                    $params["hostid"],
                );
                ...
                active_log_final(
                    lang("MODULE_EXEC_FAILED", [
                        $module,
                        $this->support[$name]["des"],
                        $params["hostid"],
                        $result["msg"],
                    ]),
                    0,
    ...
    public function downloadResource($param)
    {
        ...
        if ($module == "dcimcloud") {
            $res = [
                ...
                "data" => [
                    "module" => "mf_finance",
                    "url" => request()->domain() . "/plugins/servers/mf_finance/data/abc.zip",
                    "version" => "1.0.0",
                ],
            ];
        } elseif ($module == "dcim") {
            $res = [
                ...
                "msg" => "模块文件不存在",
                "data" => [
                    "module" => "mf_finance_dcim",
                    "url" => request()->domain() . "/plugins/servers/mf_finance_dcim/data/abc.zip",
                    "version" => "1.0.0",
                ],
            ];
        } else {
