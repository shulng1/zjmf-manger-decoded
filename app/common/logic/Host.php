...
class Host
{
    ...
    public function createFinal($id, $ip = "")
    {
            ...
            if ($module_res["status"] == 200) {
                ...
                if ($module_res2["status"] == 200) {
                    ...
                    if (!empty($module_res2["invoiceid"])) {
                        ...
                        $post_data["qty"] = 1;
                        $configoption = \think\Db::name("host_config_options")
                            ->alias("a")
                            ->field("a.qty,b.option_type,b.upstream_id config_upstream,c.upstream_id")
                            ->leftJoin("product_config_options b", "a.configid=b.id")
                            ->leftJoin("product_config_options_sub c", "a.optionid=c.id")
                            ->where("a.relid", $id)
                            ->where("b.upstream_id", ">", 0)
                            ->where("c.upstream_id", ">", 0)
                            ->select()
                            ->toArray();
                        ...
                        $customfield = \think\Db::name("customfieldsvalues")
                            ->alias("a")
                            ->field("b.upstream_id,a.value")
                            ->leftJoin("customfields b", "a.fieldid=b.id")
                            ->where("a.relid", $id)
                            ->where("b.relid", $host["productid"])
                            ->select()
                            ->toArray();
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200) {
            ...
            $host1 = \think\Db::name("host")
                ->alias("a")
                ->field("a.port")
                ->field(
                    "a.id,a.uid,a.productid,a.domainstatus,a.regdate,b.welcome_email,b.type,c.email,a.billingcycle,b.pay_type,b.name,a.nextduedate,a.billingcycle,a.dedicatedip,a.username,a.password,a.os,a.assignedips,a.create_time",
                )
                ->leftJoin("products b", "a.productid=b.id")
                ->leftJoin("clients c", "a.uid=c.id")
                ->where("a.id", $id)
            ...
            if ($host["api_type"] == "resource") {
                ...
                if ($module_res["status"] == 200) {
                    $current_rate = \think\Db::name("host")
                        ->alias("a")
                        ->leftJoin("res_products b", "a.productid=b.productid")
                        ->where("a.id", $id)
                        ->value("b.current_rate");
                    \think\Db::name("invoices")
                        ->alias("a")
                        ->leftJoin("orders b", "b.invoiceid=a.id")
                        ->where("b.id", $host["orderid"])
                        ->update([
                            "cost" => bcmul($module_res["data"]["host_data"]["order_amount"], $current_rate, 20),
                        ]);
                }
            }
            if (!in_array($host["api_type"], ["zjmf_api", "resource"])) {
                ...
                $client = check_type_is_use(
                    $message_template_type[strtolower("Default_Product_Welcome")],
                    $host1["uid"],
                    $sms,
                );
                if ($client) {
                    ...
                    if (
                        $billing_cycle[$host1["billingcycle"]] == "免费" ||
                        $billing_cycle[$host1["billingcycle"]] == "一次性"
                    ) {
                        $time = "不到期";
                    } else {
                    ...
                    $params = [
                        "product_name" => $host1["name"],
                        "product_mainip" => $host1["dedicatedip"],
                        "product_user" => $host1["username"],
                        "product_passwd" => cmf_decrypt($host1["password"]),
                        "product_dcimbms_os" => $host1["os"],
                        "product_addonip" => $host1["assignedips"],
                        "product_first_time" => date("Y-m-d H:i:s", $host1["create_time"]),
                        "product_end_time" => $time,
                        "product_binlly_cycle" => $billing_cycle[$host1["billingcycle"]],
                    ];
                ...
                $host = \think\Db::name("host")
                    ->alias("a")
                    ->field("a.id,a.uid,a.dedicatedip,a.serverid,b.server_group")
                    ->leftJoin("products b", "a.productid=b.id")
                    ->where("a.id", $id)
                    ->find();
                ...
                active_log_final(
                    sprintf(
                        "开通host - User ID:%d - Host ID:%d - 服务器模块:%s - 接口:%s - IP:%s - 成功",
                        $host["uid"],
                        $id,
                        $server_groups["name"],
                        $servers["name"],
            ...
            if (!empty($host["server_group"])) {
                active_log_final(
                    sprintf(
                        "模块命令:开通host - User ID:%d - Host ID:%d - 失败 - 原因：%s",
                        $host["uid"],
                        $id,
                        $module_res["msg"],
                    ),
    ...
    public function suspend($id, $reason_type = "other", $reason = "", $send = 0)
    {
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.nextduedate,a.id,a.productid,a.domainstatus,a.dcimid,b.server_type,b.type,c.email,b.name,a.uid,a.domain,a.nextinvoicedate,b.api_type,b.zjmf_api_id,b.server_group,a.dedicatedip",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("clients c", "a.uid=c.id")
            ->where("a.id", $id)
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200) {
            ...
            \think\Db::name("host")
                ...
                ->update([
                    ...
                    "suspendreason" => $reason_type . "-" . $reason,
                    "suspend_time" => time(),
                ]);
            if ($send) {
                $email = new Email();
            ...
            if ($client) {
                $params = [
                    ...
                    "description" => $reason,
                    "product_end_time" => date("Y-m-d H:i:s", $host["nextduedate"]),
                    "product_mainip" => $host["dedicatedip"] ?: "",
                ];
                $sms->sendSms(
                    $message_template_type[strtolower("host_suspend")],
                    $client["phone_code"] . $client["phonenumber"],
                    $params,
                    false,
                    $host["uid"],
            ...
            active_log_final(
                sprintf(
                    "模块命令:暂停host - User ID:%d - Host ID:%s - 原因：%s",
                    $host["uid"],
                    $id,
                    $reason_type . "-" . $reason,
                ),
            ...
            active_log_final(
                sprintf(
                    "模块命令:暂停host - User ID:%d - Host ID:%s - 失败：%s",
                    $host["uid"],
                    $id,
                    $module_res["msg"] . "-" . $reason,
                ),
                $host["uid"],
                2,
                $id,
    ...
    public function unsuspend($id, $send = 0, $type = "", $force_success = false)
    {
        $host = \think\Db::name("host")
            ...
            ->field("a.port")
            ->field(
                "a.id,a.productid,a.domainstatus,a.dcimid,b.server_type,b.type,c.email,a.uid,a.domain,a.nextinvoicedate,a.suspendreason,b.api_type,b.zjmf_api_id,b.server_group,a.dedicatedip,b.name",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("clients c", "a.uid=c.id")
            ->where("a.id", $id)
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200 || $force_success) {
            ...
            \think\Db::name("host")
                ->where("id", $id)
                ->update(["domainstatus" => "Active", "suspendreason" => ""]);
            if ($send) {
                $email = new Email();
            ...
            if ($client) {
                $params = [
                    "product_name" => $host["name"],
                    "product_end_time" => date("Y-m-d H:i:s", $host["nextinvoicedate"]),
                    "product_mainip" => $host["dedicatedip"],
                ];
            ...
            active_log_final(
                sprintf("模块命令:解除暂停失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                $host["uid"],
                2,
                $id,
            );
    ...
    public function terminate($id, $cron = "")
    {
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.uid,a.productid,a.domainstatus,a.dcimid,a.domain,a.dedicatedip,a.assignedips,a.username,a.password,a.os,a.port,b.name,b.server_type,b.type,b.api_type,b.zjmf_api_id,c.email,b.auto_terminate_email,b.api_type,b.zjmf_api_id,b.server_group",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("clients c", "a.uid=c.id")
            ->where("a.id", $id)
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200) {
            ...
            $note = [
                ...
                "附加IP：" . implode("/", explode(",", $host["assignedips"])),
                "主机名：" . $host["domain"],
                "用户名：" . $host["username"],
                "密码：" . cmf_decrypt($host["password"]),
                "操作系统：" . $host["os"],
                "端口：" . $host["port"],
                "ID：" . $host["dcimid"],
            ];
            ...
            if ($cron == "cron") {
                active_log_final(
                    sprintf("删除成功#User ID:%d - Host ID:%d - %s", $host["uid"], $id, $str),
                    $host["uid"],
                    2,
                    $id,
                );
                ...
                active_log_final(
                    sprintf("模块命令:删除成功#User ID:%d - Host ID:%d - %s", $host["uid"], $id, $str),
                    $host["uid"],
                    2,
                    $id,
                );
            ...
            if ($cron == "cron") {
                active_log_final(
                    sprintf("删除失败#User ID:%d - Host ID:%d - 原因:%s", $host["uid"], $id, $module_res["msg"]),
                    $host["uid"],
                    2,
                    $id,
                );
                ...
                active_log_final(
                    sprintf(
                        "模块命令:删除失败#User ID:%d - Host ID:%d - 原因:%s",
                        $host["uid"],
                        $id,
                        $module_res["msg"],
                    ),
    ...
    public function sync($id)
    {
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.uid,a.productid,a.domainstatus,a.dcimid,b.server_type,b.type,b.api_type,b.zjmf_api_id,c.email,b.auto_terminate_email,b.server_group",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("clients c", "a.uid=c.id")
            ->where("a.id", $id)
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            if ($module_res["status"] == 200) {
                ...
                if (
                    $host["domainstatus"] == "Pending" &&
                    $module_res["data"]["host_data"]["domainstatus"] == "Active"
                ) {
            ...
            $up_id = \think\Db::name("customfieldsvalues")
                ->alias("a")
                ->leftJoin("customfields b", "a.fieldid=b.id")
                ->where("a.relid", $id)
            ...
            if ($res["status"] == 200) {
                $update = [
                    ...
                    "bwusage" => $res["data"]["bwusage"],
                    "username" => $res["data"]["username"],
                    "password" => cmf_encrypt($res["data"]["password"]),
                    "port" => $res["data"]["port"] ?: "",
                    "os" => $res["data"]["os"] ?: "",
                ];
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200) {
            ...
            active_log_final(
                sprintf("模块命令:同步失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                $host["uid"],
                2,
                $id,
            );
    ...
    public function on($id)
    {
        if (is_array($id)) {
            ...
            $host = \think\Db::name("host")
                ->alias("a")
                ->field(
                    "a.id,a.productid,a.domainstatus,a.uid,a.dcimid,b.server_type,b.type,b.api_type,b.zjmf_api_id,c.email,b.server_group",
                )
                ->leftJoin("products b", "a.productid=b.id")
                ->leftJoin("clients c", "a.uid=c.id")
                ->where("a.id", $id)
            ...
            if ($module_res["status"] == "success" || $module_res["status"] == 200) {
                ...
                active_log_final(
                    sprintf("模块命令:开机失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                    $host["uid"],
                    2,
                    $id,
                );
    ...
    public function off($id)
    {
        if (is_array($id)) {
            ...
            $host = \think\Db::name("host")
                ->alias("a")
                ->field(
                    "a.id,a.productid,a.domainstatus,a.dcimid,b.server_type,b.type,b.api_type,b.zjmf_api_id,c.email,a.uid,b.server_group",
                )
                ->leftJoin("products b", "a.productid=b.id")
                ->leftJoin("clients c", "a.uid=c.id")
                ->where("a.id", $id)
            ...
            if ($module_res["status"] == "success" || $module_res["status"] == 200) {
                ...
                active_log_final(
                    sprintf("模块命令:关机失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                    $host["uid"],
                    2,
                    $id,
                );
    ...
    public function reboot($id)
    {
        if (is_array($id)) {
            ...
            if ($module_res["status"] == "success" || $module_res["status"] == 200) {
                ...
                active_log_final(
                    sprintf("模块命令:重启失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                    $host["uid"],
                    2,
                    $id,
                );
    ...
    public function hardOff($id)
    {
        if (is_array($id)) {
            ...
            if ($module_res["status"] == "success" || $module_res["status"] == 200) {
                ...
                active_log_final(
                    sprintf("模块命令:硬关机失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                    $host["uid"],
                    2,
                    $id,
                );
    ...
    public function hardReboot($id)
    {
        if (is_array($id)) {
            ...
            if ($module_res["status"] == "success" || $module_res["status"] == 200) {
                ...
                active_log_final(
                    sprintf("模块命令:硬重启失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                    $host["uid"],
                    2,
                    $id,
                );
    ...
    public function vnc($id)
    {
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200) {
                ...
                if ($host["type"] == "dcimcloud" && !$module_res["data"]["zjmfcloud_out_vnc"]) {
                    if ($this->is_admin) {
                        $result["data"]["url"] =
                            ...
                            "/dcim/novnc" .
                            substr($result["data"]["url"], strpos($result["data"]["url"], "?url="));
                    } else {
                        $result["data"]["url"] =
                            request()->domain() .
                            "/dcim/novnc" .
            ...
            active_log_final(
                sprintf("模块命令:vnc启动失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                $host["uid"],
                2,
                $id,
            );
    ...
    public function reinstall($id, $os, $port = 0)
    {
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.productid,a.domainstatus,a.dcimid,a.os,a.username,b.server_type,b.type,b.api_type,b.zjmf_api_id,b.upstream_price_value,c.email,a.uid,b.server_group",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("clients c", "a.uid=c.id")
            ->where("a.id", $id)
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            if (
                $module_res["status"] != 200 &&
                isset($module_res["price"]) &&
                $host["upstream_price_type"] == "percent"
            ) {
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200) {
            ...
            $old = \think\Db::name("host_config_options")
                ->where("relid", $id)
                ->where("configid", $r["config_id"])
                ->find();
            if (!empty($old)) {
                \think\Db::name("host_config_options")
                    ->where("id", $old["id"])
                    ->update(["optionid" => $r["id"]]);
            } else {
            ...
            \think\Db::name("host")
                ->where("id", $id)
                ->update(["os" => $os_name, "os_url" => $os_group]);
            active_log_final(
                sprintf("模块命令:重装系统发起成功,新系统:%s#Host ID:%d", $os_name, $id),
                $host["uid"],
                2,
                $id,
            );
            ...
            active_log_final(
                sprintf("模块命令:重装系统发起失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                $host["uid"],
                2,
                $id,
            );
                    ...
                    if ($format && $module_res["status"] == 200) {
                        if (isset($module_res["data"]["power"])) {
                            if ($module_res["data"]["power"] == "not_support") {
                                ...
                                $module_res["data"]["status"] =
                                    $module_res["data"]["power"] == "error" ? "unknown" : $module_res["data"]["power"];
                            }
    ...
    public function changePackage($id, $params, $is_config = false, $is_upstream_product = false, $upgrade_id = 0)
    {
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.productid,a.domainstatus,a.uid,a.dcimid,b.server_type,b.type,b.api_type,b.zjmf_api_id,c.email,a.billingcycle,b.upstream_pid,b.server_group",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("clients c", "a.uid=c.id")
            ->where("a.id", $id)
        ...
        if ($host["api_type"] == "zjmf_api" && $is_config) {
            ...
            if (function_exists("resourceCurl")) {
                $res_products = \think\Db::name("host")
                    ->alias("a")
                    ->field("b.percent_value,b.price_type")
                    ->leftJoin("res_products b", "a.productid=b.productid")
                    ->where("a.id", $id)
                    ->find();
                $post_data = [];
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200) {
            ...
            if ($host["api_type"] == "resource") {
                $orders = \think\Db::name("orders")
                    ->alias("a")
                    ->field("a.id,a.supplier_id,a.invoiceid")
                    ->leftJoin("upgrades b", "a.id=b.order_id")
                    ->where("b.id", $upgrade_id)
                    ->find();
                ...
                if ($module_res["status"] == 200) {
                    $current_rate = \think\Db::name("host")
                        ...
                        ->value("b.current_rate");
                    \think\Db::name("invoices")
                        ->where("id", $orders["invoiceid"])
                        ->update(["cost" => bcmul($module_res["data"]["order_amount"], $current_rate, 20)]);
                }
            }
            pushHostInfo($id);
            $result["status"] = 200;
    ...
    public function crackPass($id, $new_pass = "")
    {
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.uid,a.productid,a.domainstatus,a.uid,a.dcimid,b.server_type,b.type,b.api_type,b.zjmf_api_id,c.email,b.server_group,b.config_option1",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("clients c", "a.uid=c.id")
            ->where("a.id", $id)
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200) {
            hook("after_module_crack_password", [
                "hostid" => $id,
                "oldpassword" => $hook_data["password"],
                "newspassword" => $new_pass,
            ]);
            if (!empty($new_pass)) {
                \think\Db::name("host")
                    ->where("id", $id)
                    ->update(["password" => cmf_encrypt($new_pass)]);
            }
            ...
            active_log_final(
                sprintf("模块命令:重置密码失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                $host["uid"],
                2,
                $id,
            );
    ...
    public function renew($id)
    {
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.productid,a.domainstatus,a.uid,a.dcimid,a.billingcycle,b.server_type,b.type,b.api_type,b.zjmf_api_id,b.server_group",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->where("a.id", $id)
            ->find();
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            $post_data["hostid"] = $host["dcimid"];
            $post_data["billingcycles"] = $host["billingcycle"];
            $price_type = \think\Db::name("host")
                ->alias("a")
                ->leftJoin("res_products d", "a.productid=d.productid")
                ->where("a.id", $id)
                ->value("d.price_type");
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200) {
            ...
            if ($host["api_type"] == "resource") {
                ...
                if ($module_res["status"] == 200) {
                    ...
                    $invoiceid = \think\Db::name("invoice_items")
                        ->alias("a")
                    ...
                    \think\Db::name("invoices")
                        ->where("id", $invoiceid)
                        ->update(["cost" => $amount, "supplier_invoiceid" => intval($supplier_invoiceid)]);
                }
            }
            $result["status"] = 200;
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            if (!empty($host["server_group"])) {
                if ($host["type"] == "dcim") {
                    ...
                    $ticket_data = \think\Db::name("ticket")
                        ->field(
                            "id,tid,dptid,host_id,name,email,create_time,title,content,status,c,priority,attachment,last_reply_time,client_unread,admin_unread,star,is_auto_reply,token",
                        )
                        ->where("id", $ticketid)
                        ->find();
                    $ticket_data["department"] = \think\Db::name("ticket_department")
                        ->field("id,name,description,email")
                        ->where("id", $ticket_data["dptid"])
                        ->find();
                    $ticket_data["status_desc"] = \think\Db::name("ticket_status")
                        ->where("id", $ticket_data["status"])
                        ->value("title");
    ...
    public function replyTicket($id, $reply_data)
    {
        ...
        $params = [];
        $params["ticket"] = \think\Db::name("ticket")
            ->field(
                "id,tid,dptid,host_id,name,email,create_time,title,content,status,priority,attachment,last_reply_time,client_unread,admin_unread,star,is_auto_reply,token",
            )
            ->where("id", $reply_data["tid"])
            ->find();
        if (empty($params["ticket"]["token"])) {
            $params["ticket"]["token"] = md5(uniqid() . randStr() . $ticket_data["tid"] . mt_rand(100, 999));
            \think\Db::name("ticket")
                ->where("id", $reply_data["tid"])
                ->update(["token" => $params["ticket"]["token"]]);
        }
        if ($host["api_type"] == "zjmf_api") {
        } elseif ($host["api_type"] == "normal") {
            if (!empty($host["server_group"])) {
                if ($host["type"] == "dcim") {
                    ...
                    $params["ticket"]["department"] = \think\Db::name("ticket_department")
                        ->field("id,name,description,email")
                        ->where("id", $params["ticket"]["dptid"])
                        ->find();
                    $params["ticket"]["status_desc"] = \think\Db::name("ticket_status")
                        ->where("id", $params["ticket"]["status"])
                        ->value("title");
                    $params["ticket_reply"] = [
                        "id" => \intval($reply_data["id"]),
                        "tid" => $reply_data["tid"],
                        "create_time" => $reply_data["create_time"],
    ...
    public function rescueSystem($id, $system)
    {
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200) {
            ...
            active_log_final(
                sprintf("模块命令:救援系统发起失败#Host ID:%d - 原因:%s", $id, $module_res["msg"]),
                $host["uid"],
                2,
                $id,
            );
