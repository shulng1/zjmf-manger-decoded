...
class Email
{
    ...
    {
        if ($templateId) {
            $template = \think\Db::name("email_templates")
                ->field("type,subject,message,attachments,fromname,fromemail,copyto,blind_copy_to")
    ...
    private function getEmailTemplateByName($name, $type, $uid = "")
    {
        ...
        $email_template = \think\Db::name("email_templates")
            ->where("type", $type)
            ->where(function (\think\db\Query $query) use ($name) {
    ...
    public function sendEmailDiy(
        ...
        $subject = str_replace(
            ["{SYSTEM_COMPANYNAME}", "{ACTION_IP}", "{USERNAME}"],
            [configuration("company_name") ?? "智简魔方", $server_ip, $clients["username"] ?: ""],
            $subject,
        );
    ...
    public function sendEmailBase(
        ...
        return $this->sendEmailBaseFinal(
            $relid,
            $name,
            $type,
            $sync,
    ...
    public function sendEmailBaseFinal(
        ...
        $subject = str_replace(
            ["{SYSTEM_COMPANYNAME}", "{ACTION_IP}", "{USERNAME}"],
            [
                configuration("company_name") ?? "智简魔方",
    ...
    public function sendEmail($templateId, $relid, $ip = "")
    {
        ...
        $clients = \think\Db::name("clients")->where("id", $uid)->find();
        $subject = str_replace(
            ["{SYSTEM_COMPANYNAME}", "{PRODUCT_NAME}", "{USERNAME}"],
            [configuration("company_name") ?? "智简魔方", $host["name"] ?? "", $clients["username"] ?: ""],
            $template["subject"],
        );
        $subject = $this->replaceArg($subject, $uid, $type, $relid);
    ...
    public function sendEmailCode($email, $code, $admin = false, $admin_id = 0, $action = "verification code")
    {
        ...
        $template = \think\Db::name("email_templates")
            ->where("language", $system_language)
            ->where("name", "验证码")
            ->where("disabled", 0)
            ->find();
        if (empty($template)) {
        ...
        $replace = [
            ...
            $company_name,
            date("Y-m-d H:i:s"),
            configuration("domain") . configuration("logo_url"),
        ];
    ...
    public function sendEmailBind($email, $action = "bind email", $sync = false)
    {
        ...
        $subject = str_replace(
            ["{EPW_TYPE}", "{SYSTEM_COMPANYNAME}", "{USERNAME}"],
            [config("bind_email")[$action], $company_name, $username],
            $subject,
        );
    ...
    public function batchSendEmailBase(
        $relids,
        $type,
        $subject = "",
        $cc = "",
    ...
    protected function sendActivationMailBase(
        $relid,
        $subject = "",
        $type = "",
        $cc = "",
        ...
        switch ($type) {
            case "general":
                $args = [
                    ...
                    ["name" => lang("EMAIL_TEMPLATE_CLIENT_LOGIN_DATA_TIME"), "arg" => "{LOGIN_DATA_TIME}"],
                    ["name" => lang("EMAIL_TEMPLATE_CLIENT_ACTION_IP"), "arg" => "{ACTION_IP}"],
                ];
                break;
            case "product":
                $args = [
                    ...
                    ["name" => lang("EMAIL_TEMPLATE_INVOICE_ID"), "arg" => "{INVOICE_ID}"],
                    ["name" => lang("EMAIL_TEMPLATE_INVOICE_TOTAL_FEE"), "arg" => "{INVOICE_TOTAL_FEE}"],
                ];
                break;
            case "invoice":
                $args = [
            ...
            case "support":
                $args = [
                    ...
                    ["name" => lang("工单内容"), "arg" => "{TICKET_CONTNET}"],
                    ["name" => lang("工单回复"), "arg" => "{TICKET_REPLY}"],
                ];
                break;
            case "notification":
                $args = [];
    ...
    protected function replaceArg($content, $uid, $type, $relid)
    {
        ...
        $replaceBase = [
            ...
            $client["companyname"] ?? "",
            $client["email"] ?? "",
            !empty($client["lastlogin"]) ? date("Y-m-d H:i:s", $client["lastlogin"]) : "",
            $client["lastloginip"] ?? "",
            configuration("company_name") ?? "智简魔方",
    ...
    protected function getContent($content, $type, $relid)
    {
        ...
        if ($type == "admin") {
            ...
            $host = \think\Db::name("host")
                ...
                ->field("h.port")
                ->field(
                    "p.name as product_name,o.id,o.create_time,o.amount,h.os,o.pay_time,i.paid_time,h.domain,h.payment,h.firstpaymentamount,h.billingcycle,h.nextduedate,h.dedicatedip,h.assignedips,h.username,h.password,h.os,cu.prefix,cu.suffix",
                )
                ->leftJoin("products p", "p.id = h.productid")
                ->leftJoin("orders o", "o.id = h.orderid")
                ->leftJoin("clients c", "h.uid = c.id")
                ->leftJoin("currencies cu", "c.currency = cu.id")
            ...
            $replaceAdd = [
                ...
                $host["username"],
                $host["dedicatedip"],
                cmf_decrypt($host["password"]),
                $host["os"],
                $host["assignedips"],
                date("Y-m-d", $host["create_time"]),
                $host["nextduedate"] > 0 ? date("Y-m-d", $host["nextduedate"]) : "永久",
                config("billing_cycle")[$host["billingcycle"]],
                date("Y-m-d", $host["create_time"]),
                $host["id"],
                $host["prefix"] . $host["amount"] . $host["suffix"],
                !empty($host["paid_time"]) ? date("Y-m-d", $host["paid_time"]) : date("Y-m-d"),
                date("Y-m-d", $terminate_time),
                $invoice["id"],
                $invoice["total"],
            ];
            ...
            $ticket = \think\Db::name("ticket")
                ->alias("t")
                ->field(
                    "t.id as ticket_id,td.name as ticket_department,t.create_time,t.title,t.content,t.status,t.priority,t.last_reply_time,t.uid,t.admin_id",
                )
                ->leftJoin("ticket_department td", "td.id = t.dptid")
                ->where("t.id", $relid)
                ->find();
            $user_name = "匿名";
            if ($ticket["uid"] > 0) {
            ...
            $cron_reply_time = date(
                "Y-m-d H:i:s",
                $ticket["last_reply_time"] + 3600 * configuration("cron_ticket_close_time"),
            );
            $priority = ["high" => "高", "low" => "低", "medium" => "中"];
            $reply = \think\Db::name("ticket_reply")
                ->field("content")
                ->where("tid", $relid)
                ->order("id", "desc")
                ->find();
            ...
            $replaceAdd = [
                $admin_name,
                $user_name,
                date("Y-m-d", $ticket["last_reply_time"]),
                $ticket["ticket_department"],
                $cron_reply_time,
                $ticket["title"],
                date("Y-m-d", $ticket["create_time"]),
        ...
        foreach ($var as $key => $value) {
            switch ($value) {
                ...
                case "CLIENT_LOGIN_DATA_TIME":
                    $email_content = preg_replace(
                        "/\\{CLIENT_LOGIN_DATA_TIME\\}/",
                        date("Y-m-d H:i:s", $client_info["lastlogin"]),
                        $email_content,
                    );
                    break;
                ...
                case "CLIENT_SIGNUP_DATE":
                    $email_content = preg_replace(
                        "/\\{CLIENT_SIGNUP_DATE\\}/",
                        date("Y-m-d H:i:s", $client_info["create_time"]),
                        $email_content,
                    );
                    break;
                case "CLIENT_CREDIT":
                ...
                case "CLIENT_GROUP_NAME":
                    $group_name = \think\Db::name("client_groups")
                        ->where("id", $client_info["groupid"])
                        ->value("group_name");
                    $email_content = preg_replace("/\\{CLIENT_GROUP_NAME\\}/", $group_name, $email_content);
                    break;
                case "CLIENT_DUE_INVOICES_BALANCE":
                    $amount = \think\Db::name("invoices")
                        ->where("uid", $client_info["id"])
