...
class Sms
{
    ...
    public function sms_log(
        ...
        if (strpos($type, "msgid") !== false) {
            ...
            $message_template = \think\Db::name("message_template")
                ->field("template_id,content")
                ->where("id", $msgid_type[1])
                ->find();
        } else {
            $message_template = \think\Db::name("message_template_link")
                ->alias("a")
                ->field("b.template_id,b.content")
                ->join("message_template b", "b.id = a.sms_temp_id")
                ->where("a.type", $type)
                ->where("a.range_type", $range_type)
                ->where("a.sms_operator", $sms_operator)
                ->find();
        }
        ...
        $log_data = [
            ...
            "phone" => $phone ?? "",
            "template_code" => $message_template["template_id"] ?? "",
            "content" => $message_template["content"] ?? "",
            "status" => $status,
            "create_time" => time(),
            "is_admin" => $admin,
            "is_wx" => $wx,
            "fail_reason" => empty($msg) ? "" : $msg,
            "ip" => get_client_ip6(),
            "port" => get_remote_port(),
        ];
        return \think\Db::name("message_log")->insert($log_data);
    }
    public function sendSmsForMarerking(
        $phone,
        $msgid,
        $param,
        $sync = false,
    ...
    public function sendSmsForMarerkingFinal(
        $phone,
        $msgid,
        $param,
        $sync = false,
        ...
        if ($sync) {
            ...
            $isPushed = \think\Queue::later(
                $delay_time,
                "app\\common\\job\\SendActivationMarketing",
                json_encode($param),
                "SendActivationMarketing",
            );
            if ($isPushed !== false) {
        ...
        $params = [
            "content" => $message_template["content"],
            "template_id" => $message_template["template_id"],
            "sms_operator" => $message_template["sms_operator"],
            "mobile" => $phone,
        ];
        $result = $this->send($params, $templateParam, $rangeType);
        \think\facade\Log::record(
            json_encode($result) . "data" . json_encode($param) . " uid:" . $type . " phone:" . $phone,
            "sms",
        );
        if (isset($result["status"]) && $result["status"] == "success") {
            ...
            return [
                "status" => 400,
                "msg" => lang("SEND FAIL") . ":" . (isset($result["status"]) ? $result["msg"] : ""),
                "data" => $result,
            ];
        }
    }
    ...
    public function sendSmsFinal($type, $phone, $param, $sync = false, $uid = "", $delay_time = 0, $is_market = false)
    {
        ...
        $templateParamSystem = [
            "system_companyname" => configuration("company_name"),
            "system_web_url" => configuration("system_url"),
        ...
        if (!empty($uid)) {
            ...
            $templateParamUserInfo = [
                ...
                "account" => $user_info["username"] . "(" . $user_info["phonenumber"] . ")",
                "user_company" => $user_info["companyname"],
                "account_email" => $user_info["email"],
                "login_data_time" => date("Y-m-d H:i:s", $user_info["lastlogin"]),
                "action_ip" => $user_info["lastloginip"],
                "time" => date("Y-m-d H:i:s", $user_info["lastlogin"]),
                "epw_type" => "手机",
                "epw_account" => $user_info["phonenumber"],
                "address" => $user_info["lastloginip"],
                "register_time" => date("Y-m-d H:i:s", $user_info["create_time"]),
                ...
                foreach ($result as $value) {
                    array_push(
                        ...
                        $value["status"] == "success"
                            ? lang("INLAND_MOBILE_SEND_SUCCESS", ["phone" => $value["to"]])
                            : lang("INLAND_MOBILE_SEND_FAIL", ["phone" => isset($value["to"]) ? $value["to"] : ""]),
                    );
                }
            } else {
                array_push(
                    $submail_msg,
                    lang("INLAND_MOBILE_SEND_FAIL", ["phone" => isset($result["to"]) ? $result["to"] : ""]) .
                        ":" .
        ...
        if (!empty($result2)) {
            if (is_array($result2[0])) {
                foreach ($result2 as $value2) {
                    array_push(
                        $submail_msg,
                        $value2["status"] == "success"
                            ? lang("INTERNATIONAL_MOBILE_SEND_SUCCESS", ["phone" => $value2["to"]])
                            : lang("INTERNATIONAL_MOBILE_SEND_FAIL", [
                                "phone" => isset($value2["to"]) ? $value2["to"] : "",
                            ]),
                    );
                }
            } else {
                array_push(
                    $submail_msg,
                    lang("INTERNATIONAL_MOBILE_SEND_FAIL", ["phone" => isset($result2["to"]) ? $result2["to"] : ""]) .
                        ":" .
                        (isset($result2["msg"]) ? $result2["msg"] : ""),
                );
            }
        }
    ...
    {
        $templateid = \think\Db::name("message_template_link")
            ->field("mt.template_id")
            ->alias("mtl")
            ->leftJoin("message_template mt", "mt.id = mtl.sms_temp_id")
            ->where("mtl.range_type", $rangeType)
            ->where("mtl.sms_operator", $smsOperator)
            ->where("mtl.type", $type)
            ->where("mt.status", 2)
            ->find();
        return $templateid;
    ...
    private function getTemplateCodeByType($type, $smsOperator, $rangeType)
    {
        $templateid = \think\Db::name("message_template_link")
            ->alias("mtl")
            ->field("mt.template_id,mt.content,mt.id")
            ->leftJoin("message_template mt", "mt.id = mtl.sms_temp_id")
            ->where("mtl.range_type", $rangeType)
            ->where("mtl.sms_operator", $smsOperator)
            ->where("mtl.type", $type)
    ...
    {
        return [
            ["name" => lang("EMAIL_TEMPLATE_COMPANY_NAME"), "arg" => "{system_companyname}"],
            ["name" => lang("EMAIL_TEMPLATE_CODE"), "arg" => "{code}"],
            ["name" => lang("EMAIL_TEMPLATE_SEND_TIME"), "arg" => "{send_time}"],
            ["name" => lang("EMAIL_TEMPLATE_SYSTEM_URL"), "arg" => "{system_url}"],
            ["name" => lang("EMAIL_TEMPLATE_SYSTEM_WEB_URL"), "arg" => "{system_web_url}"],
            ["name" => lang("EMAIL_TEMPLATE_COMPANY_LOGO_URL"), "arg" => "{system_email_logo_url}"],
        ];
    }
    public function getReplaceArgphone($type)
    {
        switch ($type) {
            case "general":
                $args = [
                    ["name" => lang("EMAIL_TEMPLATE_CLIENT_NAME"), "arg" => "{username}"],
                    ["name" => lang("EMAIL_TEMPLATE_CLIENT_COMPANY_NAME"), "arg" => "{user_company}"],
                    ["name" => lang("EMAIL_TEMPLATE_CLIENT_EMAIL"), "arg" => "{account_email}"],
                    ["name" => lang("EMAIL_TEMPLATE_CLIENT_EMAIL"), "arg" => "{login_data_time}"],
                    ["name" => lang("EMAIL_TEMPLATE_CLIENT_EMAIL"), "arg" => "{action_ip}"],
                ];
                break;
            case "product":
                $args = [
                    ["name" => lang("EMAIL_TEMPLATE_PRODUCT_NAME"), "arg" => "{product_name}"],
                    ["name" => lang("EMAIL_TEMPLATE_PRODUCT_HOSTNAME"), "arg" => "{hostname}"],
                    ["name" => lang("EMAIL_TEMPLATE_PRODUCT_USER"), "arg" => "{product_user}"],
                    ["name" => lang("EMAIL_TEMPLATE_PRODUCT_PRODUCT_MAINIP"), "arg" => "{product_mainip}"],
                    ["name" => lang("EMAIL_TEMPLATE_PRODUCT_PASSWD"), "arg" => "{product_passwd}"],
                    ["name" => lang("EMAIL_TEMPLATE_PRODUCT_DCIMBMS_OS"), "arg" => "{product_dcimbms_os}"],
                    ["name" => lang("EMAIL_TEMPLATE_PRODUCT_ADDONIP"), "arg" => "{product_addonip}"],
                    ["name" => lang("EMAIL_TEMPLATE_PRODUCT_FIRST_TIME"), "arg" => "{product_first_time}"],
                    ["name" => lang("EMAIL_TEMPLATE_PRODUCT_END_TIME"), "arg" => "{product_end_time}"],
                    ["name" => lang("EMAIL_TEMPLATE_PRODUCT_BINLLY_CYCLE"), "arg" => "{product_binlly_cycle}"],
                    ["name" => lang("EMAIL_TEMPLATE_ORDER_CREATE_TIME"), "arg" => "{order_create_time}"],
                    ["name" => lang("EMAIL_TEMPLATE_ORDER_ID"), "arg" => "{order_id}"],
                    ["name" => lang("EMAIL_TEMPLATE_ORDER_TOTAL_FEE"), "arg" => "{order_total_fee}"],
                    ["name" => lang("EMAIL_TEMPLATE_INVOICE_PAID_TIME"), "arg" => "{invoice_paid_time}"],
                    ["name" => lang("自动删除时间"), "arg" => "{product_terminate_time}"],
                ];
                break;
            case "invoice":
                $args = [
                    ...
                    ["name" => lang("EMAIL_TEMPLATE_ORDER_TOTAL_FEE"), "arg" => "{order_total_fee}"],
                    ["name" => lang("EMAIL_TEMPLATE_INVOICE_PAID_TIME"), "arg" => "{invoice_paid_time}"],
                    ["name" => lang("账单id"), "arg" => "{invoiceid}"],
                    ["name" => lang("自动删除时间"), "arg" => "{product_terminate_time}"],
                ];
                break;
            case "support":
                $args = [
                    ["name" => lang("EMAIL_TEMPLATE_TICKET_REPLY_TIME"), "arg" => "{ticket_reply_time}"],
                    ["name" => lang("EMAIL_TEMPLATE_TICKET_DEPARTMENT"), "arg" => "{ticket_department}"],
                    ["name" => lang("EMAIL_TEMPLATE_TICKET_ATUO_CLOSE_TIME"), "arg" => "{auto_ticket_close_time}"],
                    ["name" => lang("EMAIL_TEMPLATE_TICKET_TITLE"), "arg" => "{ticketnumber_tickettitle}"],
                    ["name" => lang("EMAIL_TEMPLATE_TICKET_CREATETIME"), "arg" => "{ticket_createtime}"],
                    ["name" => lang("EMAIL_TEMPLATE_TICKET_PRIORITY"), "arg" => "{ticket_level}"],
                ];
                break;
            case "notification":
                $args = [];
            ...
            case "admin":
                $args = [
                    ["name" => lang("管理员名称"), "arg" => "{admin_account_name}"],
                    ["name" => lang("登录时间"), "arg" => "{admin_login_data_time}"],
                    ["name" => lang("登录IP"), "arg" => "{admin_action_ip}"],
                ];
                break;
        }
        return $args;
    ...
    public function templateParam($user_info = [], $host = [], $order = [])
    {
        ...
        $var = [
            ...
            "send_time" => date("Y-m-d H:i:s"),
            "system_url" => configuration("system_url") . "/" . $system_dir,
            "system_web_url" => configuration("system_url"),
            "system_email_logo_url" => configuration("logo_url_home"),
            "username" => $user_info["username"],
            "register_time" => date("Y-m-d H:i:s", $user_info["create_time"]),
            "user_address" => $user_info["address1"],
            "qq" => $user_info["qq"],
            ...
            "epw_account" => $user_info["phonenumber"],
            "address" => $user_info["lastloginip"],
            "admin_account_name" => $admin_info["user_login"],
            "admin_login_data_time" => date("Y-m-d H:i:s", $admin_info["last_act_time"]),
            "admin_action_ip" => $admin_info["last_login_ip"],
            "product_name" => $host["name"],
            "hostname" => $host["domain"],
            "product_user" => $host["username"],
            "product_passwd" => aesPasswordDecode($host["password"]),
            "product_mainip" => $host["dedicatedip"],
            "product_dcimbms_os" => $host["os"],
            "product_addonip" => $host["assignedips"],
            "description" => $host["suspendreason"],
            "product_first_time" => date("Y-m-d H:i:s", $host["create_time"]),
            "product_end_time" => date("Y-m-d H:i:s", $host["nextduedate"]),
            "product_binlly_cycle" => config("billing_cycle")[$host["billingcycle"]],
            "order_create_time" => date("Y-m-d H:i:s", $order["create_time"]),
            "order_id" => $order["id"],
            "order_total_fee" => $order["amount"],
            "invoice_paid_time" => date("Y-m-d H:i:s", $order["paid_time"]),
    ...
    public function sendSmsBefore(
        ...
        if ($send_type == "clients_and_host") {
            $host = \think\Db::name("host")
                ->alias("a")
                ->field(
                    "a.id,a.uid,a.productid,a.domain,a.domainstatus,a.regdate,b.welcome_email,b.type,c.email,a.billingcycle,a.nextduedate,a.billingcycle",
                )
                ->field(
                    "b.pay_type,b.name,a.dedicatedip,a.username,a.password,a.os,a.assignedips,a.create_time,a.nextinvoicedate,a.suspendreason",
                )
                ->field("a.termination_date,a.port")
                ->leftJoin("products b", "a.productid=b.id")
                ->leftJoin("clients c", "a.uid=c.id")
                ->where("a.id", $send_client["host_id"])
                ->find();
            $order = \think\Db::name("orders")
                ->alias("o")
                ->leftJoin("invoices i", "i.id = o.invoiceid")
                ->leftJoin("invoice_items im", "i.id = im.invoice_id")
                ->where("im.rel_id", $send_client["host_id"])
                ->field("o.create_time,o.id,o.amount,i.paid_time,o.invoiceid,i.subtotal")
                ->find();
        }
        $newvar = $this->templateParam($user_info, $host, $order);
        $result = $this->sendSmsForMarerking(
            $send_client["mobile"],
            $msgid,
            $newvar,
            $send_sync,
