...
class Menu
{
     ...
     * @var string[]
     */
    protected $displayNav = [
        "allow_resource_api" => "apimanage",
        "affiliate_enabled" => ["url" => "affiliates"],
        "voucher_manager" => ["id" => [20, 21, 22, 23]],
        "credit_limit" => ["url" => "credit"],
        "addfunds_enabled" => ["url" => "addfunds"],
        "certifi_open" => ["url" => "verified"],
    ...
    public function getNavs($type = "client", $domain = "", $language = "", $is_admin = false)
    {
        try {
            ...
            $nav_list = array_column(
                $this->changeTwoArr(json_decode($model["nav_list"], true), "son", false),
                null,
                "id",
            );
            ...
            $uid = request()->uid;
            $is_open_credit_limit = \think\Db::name("clients")
                ->where("id", $uid)
                ->where("status", 1)
                ->value("is_open_credit_limit");
            ...
            foreach ($nav_model as $key => $val) {
                ...
                $nav_list[$val["id"]]["lang"] = $nav_list[$val["id"]]["muti"]
                    ? $nav_list[$val["id"]]["lang"]
                    : $val["lang"];
    ...
    public function getWebNav($type = "www_top", $domain = "", $language = "", $is_admin = false)
    {
        try {
            ...
            $nav_list_ids = array_column($nav_list, "id");
            $nav_model = \think\Db::name("nav")
                ->field("id,url,lang")
                ->whereIn("id", $nav_list_ids)
                ->select()
                ->toArray();
    ...
    private function getProductData($where, $url)
    {
        ...
        foreach ($result as $k => $v) {
            $result_val = [
                ...
                "is_display" => 1,
                "son" => [],
                "url" => config("product_type_web_tpl")[$v["type"]] ?? "",
            ];
    ...
    public function editDefaultNav($data, $type = "client")
    {
        try {
            ...
            return \think\Db::name($this->menu_table)
                ->where("id", $menuid)
                ->update(["nav_list" => json_encode($this->getTree($data))]);
        } catch (\Throwable $e) {
            return [$e->getMessage(), $e->getLine()];
        }
    }
    public function defaultNavUnique($data = [])
    {
        ...
        foreach ($data as $key => $val) {
            $_data[$val["id"]] = [
                "name" => $val["name"] ?? "",
                "pid" => $val["pid"] ?? 0,
                "nav_type" => $val["nav_type"] ?? 0,
                "menu_type" => $val["menu_type"] ?? 1,
            ];
        }
    ...
    public function getNav($menuid, $pid = 0, $domain = "", $is_admin = false, $language = "")
    {
        ...
        $data = \think\Db::name("nav")
            ->field("id,name,url,fa_icon,pid,nav_type,relid,lang")
            ->whereIn("pid", $pid)
            ->where("menuid", $menuid)
            ->where($where)
            ->order("order", "asc")
    ...
    public function moveNavTop($id)
    {
        ...
        $top = \think\Db::name("nav")
            ->where("menuid", $nav["menuid"])
            ->where("pid", 0)
            ->order("order", "asc")
            ->order("id", "asc")
            ->find();
    ...
    public function activeMenu($type, $menuid = 0)
    {
        ...
        if (!empty($exist)) {
            \think\Db::name("menu_active")
                ->where("id", $exist["id"])
                ->update(["type" => $type, "menuid" => $menuid]);
        } else {
        ...
        try {
            return \think\Db::transaction(function () use ($data) {
                ...
                return \think\Db::name($this->menu_table)
                    ->where("id", $menuid)
                    ->update([
                        "nav_list" => json_encode(
                            $this->getTree(array_merge($nav_list, array_column($model, null, "id"))),
                        ),
                    ]);
            });
        } catch (\Throwable $e) {
    ...
    public function productMenu()
    {
        try {
            return \think\Db::transaction(function () {
                ...
                $m_model_arr = array_column(
                    $this->changeTwoArr(json_decode($m_model["nav_list"], true), "son", false),
                    null,
                    "id",
                );
                ...
                foreach ($this->setPlang($nav_group) as $key => $val) {
                    $data = [
                        ...
                        "relid" => "",
                        "menuid" => 0,
                        "lang" => isset($val["lang"]) ? json_encode($val["lang"]) : "",
                        "menu_type" => 1,
                    ];
                    ...
                    if ($is_set) {
                        \think\Db::name("nav")
                            ->where("id", $is_set["id"])
                            ->update(["url" => "service?groupid=" . $is_set["id"]]);
                        $data["id"] = $is_set["id"];
                ...
                return \think\Db::name($this->menu_table)
                    ->where("id", $m_active_model["menuid"])
                    ->update(["nav_list" => json_encode($this->getTree($m_model_arr))]);
            });
        } catch (\Throwable $e) {
            return false;
        }
    ...
    public function setPlang($nav_group)
    {
        ...
        $arr_lang = [
            "Cloud" => ["chinese" => "云服务器", "chinese_tw" => "雲服務器", "english" => "Cloud"],
            "Server" => ["chinese" => "独立服务器", "chinese_tw" => "獨立服務器", "english" => "Server"],
            "Other" => ["chinese" => "其他", "chinese_tw" => "其他", "english" => "Other"],
        ];
    ...
    public function createMenus()
    {
        return \think\Db::transaction(function () {
            ...
            $model_id = \think\Db::name($this->menu_table)->insertGetId([
                "name" => "默认菜单(系统-多语言)",
                "type" => 1,
                "nav_list" => json_encode($this->getTree($nav_arr)),
                "active" => 1,
            ]);
            return \think\Db::name("menu_active")
                ->where("type", "client")
                ->update(["menuid" => $model_id]);
        });
    }
    /**
     * 获取导航产品默认模板页
