...
class UpgradeSystem
{
    private $allowed_local_test_license = [
        "7563DA6198C41976A46F4F57D836C0BE",
        "E54D92AA56A69D8C87DA5B4604ADFCCF",
        "772EC3A11FC87438E9B7E40AC4E86F8D",
        "22EC411DD6931A0275EC0B8887D4CC0F",
    ];
    public $upgrade_log = "";
    ...
    public function updateUnzip()
    {
        ...
        if ($progresslog["setup"] == "unzip") {
            ...
            if ($res["status"] == 200) {
                ...
                $progress_log["progress"] = "40%";
                $progress_log["msg"] =
                    "解压失败,失败code:" . $res["msg"] . ";请到网站目录下解压下载的文件或者重新更新系统";
                $progress_log["status"] = 400;
                $this->update_progress(json_encode($progress_log));
                ...
                if ($res["status"] == 200) {
                    ...
                    if ($admin_application != "admin") {
                        if (is_dir($this->upload_dir . "public/admin")) {
                            $custom_dir = array_diff(
                                scandir($this->upload_dir . "public/" . $admin_application),
                                scandir($this->upload_dir . "public/admin"),
                            );
                            ...
                            usleep(100000);
                            rename(
                                $this->upload_dir . "public/" . $admin_application,
                                $this->upload_dir . "public/" . $admin_application . "_old",
                            );
                        }
                        usleep(100000);
