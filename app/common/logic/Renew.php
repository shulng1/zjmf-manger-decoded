...
class Renew
{
    ...
    {
        $renew_invoices = \think\Db::name("invoices")
            ->alias("a")
            ->field("a.id")
    ...
    {
        $upgrade_invoices = \think\Db::name("invoices")
            ->alias("a")
            ->field("a.id")
            ->leftJoin("invoice_items b", "a.id = b.invoice_id")
            ->where("b.rel_id", $hid)
            ->where("b.type", "upgrade")
            ->where("a.status", "Unpaid")
            ->select()
            ->toArray();
        ...
        try {
            foreach ($hids as $hid) {
                ...
                $host_data = \think\Db::name("host")
                    ->field(
                        "h.id,h.uid,h.orderid,h.productid,h.domain,h.amount,h.promoid,h.payment,h.billingcycle,h.nextduedate,h.nextinvoicedate,h.domainstatus,h.dedicatedip,p.name as productname,p.pay_method,p.pay_type,i.status,h.flag",
                    )
                    ->alias("h")
                ...
                if (!empty($host_data) && $host_data["status"] == "Unpaid") {
                    ...
                    $host_data = \think\Db::name("host")
                        ->field(
                            "h.id,h.uid,h.orderid,h.productid,h.domain,h.amount,h.promoid,h.payment,h.billingcycle,h.nextduedate,h.nextinvoicedate,h.domainstatus,h.dedicatedip,p.name as productname,p.pay_method,p.pay_type,h.flag",
                        )
                        ->alias("h")
                        ->leftJoin("products p", "p.id=h.productid")
                        ->where("h.id", $hid)
                ...
                if (
                    $amount >= 0 &&
                    !in_array($billingcycle, ["free", "ontrial"]) &&
                    $pay_method == "prepayment" &&
                    in_array($domainstatus, ["Active", "Suspended"])
                ) {
                    ...
                    $invoice_items = [
                        ...
                        "description" =>
                            ...
                            $end_time .
                            ")",
                        "amount" => bcadd($amount, $userdiscount),
                    ];
                    ...
                    if (!empty($has) && $has["status"] == "Unpaid") {
                        $invoice_credit = \think\Db::name("invoices")
                            ->where("id", $has["id"])
                            ->where("delete_time", 0)
                            ->value("credit");
            ...
            foreach ($hids as $hid) {
                ...
                active_log_final(
                    sprintf(" 生成续费账单成功 - User ID:%s - Host ID:%s - Invoice ID:%s ", $uid, $hid, $invoice_id),
                    $uid,
                    2,
                    $hid,
                );
    ...
    public function renew($hid, $billingcycle)
    {
        ...
        $host_data = \think\Db::name("host")
            ->field(
                "h.id,h.uid,h.orderid,h.productid,h.domain,h.amount,h.promoid,h.payment,h.billingcycle,
            h.nextduedate,h.nextinvoicedate,h.domainstatus,h.dedicatedip,p.name as productname,p.pay_method,
            p.pay_type,i.status,h.flag,p.api_type,p.upstream_price_value",
            )
            ->alias("h")
            ->leftJoin("products p", "p.id=h.productid")
            ->leftJoin("orders o", "h.orderid = o.id")
        ...
        if (!empty($host_data)) {
            ...
            $host_data = \think\Db::name("host")
                ->field(
                    "h.id,h.uid,h.orderid,h.productid,h.domain,h.amount,h.promoid,h.payment,h.billingcycle,
                h.nextduedate,h.nextinvoicedate,h.domainstatus,h.dedicatedip,p.name as productname,p.pay_method,
                p.pay_type,h.flag,p.api_type,p.upstream_price_value",
                )
                ->alias("h")
                ->leftJoin("products p", "p.id=h.productid")
                ->where("h.id", $hid)
        ...
        if (
            ...
            $invoice_items = [
                ...
                "description" =>
                    ...
                    $end_time .
                    ")",
                "amount" => bcadd($amount, $userdiscount, 2),
            ];
            $renew_items = [
                "uid" => $uid,
                "type" => "normal",
            ...
            $insert_invoices = [
                ...
                "status" => "Unpaid",
                "type" => "renew",
                "payment" => $host_data["payment"],
                "url" => "servicedetail?id=" . $hid,
            ];
            if ($host_data["api_type"] == "resource") {
                $price_type = \think\Db::name("res_products")
                    ->where("productid", $host_data["productid"])
                    ->value("price_type");
            ...
            if ($amount > 0) {
                if (empty($has) || $has["status"] === "Paid") {
                    ...
                    try {
                        ...
                        active_log_final(
                            sprintf(
                                " 生成续费账单成功 - User ID:%s - Host ID:%s - Invoice ID:%s ",
                                $uid,
                                $hid,
                                $invoice_id,
                            ),
    ...
    {
        $host_data = \think\Db::name("host")
            ->field("id,uid,productid,billingcycle,promoid")
            ->where("id", $hostid)
            ->find();
        $uid = $host_data["uid"];
        ...
        foreach ($configgroups as $ckey => $configgroup) {
            if (!empty($configgroup)) {
                ...
                foreach ($options as $okey => $option) {
                    ...
                    $user_config_data = \think\Db::name("host_config_options")
                        ->where("relid", $hostid)
                        ->where("configid", $cid)
                        ->find();
                    ...
                    $config_pricing = \think\Db::name("pricing")
    ...
    protected function renewHandleIn($billingcycle, $hid)
    {
        ...
        $res = \think\Db::name("host")
            ->alias("a")
            ->field("a.port")
            ->field("a.uid,a.id as hostid,a.nextduedate,a.domainstatus,b.name,a.nextinvoicedate,a.dedicatedip")
        ...
        $data = [
            "nextduedate" => $next_time,
            "nextinvoicedate" => $next_time,
            "update_time" => time(),
            "amount" => 0,
        ...
        if ($res["domainstatus"] == "Suspended" && !!configuration("cron_host_unsuspend")) {
            ...
            if ($result["status"] == 200) {
                ...
                $data_i[
                    "description"
                ] = " 产品续费 - 解除暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                if ($is_zjmf) {
        ...
        $client = check_type_is_use(
            $message_template_type[strtolower("Service_Unsuspension_Notification")],
            $res["uid"],
            $sms,
        );
        if ($client) {
            $params = [
                ...
                "product_terminate_time" => date("Y-m-d H:i:s", $next_time),
                "product_mainip" => $res["dedicatedip"],
            ];
        ...
        if ($up) {
            $description =
                "产品续费成功 - Host ID:" . $res["hostid"] . "，到期时间修改为：" . date("Y-m-d H:i:s", $next_time);
        ...
        if ($result["status"] == 200) {
            ...
            $data_i[
                "description"
            ] = " 账单支付成功 - 进行续费 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
            if ($is_zjmf) {
    ...
    public function renewHandle($id)
    {
        ...
        $res = \think\Db::name("invoice_items")
            ...
            ->field("b.port")
            ->field(
                "a.uid,b.id as hostid,c.pay_type,b.nextduedate,b.domainstatus,c.name,b.nextinvoicedate,b.dedicatedip,b.domain",
            )
        ...
        $data = [
            ...
            "nextinvoicedate" => $next_time,
            "update_time" => time(),
            "amount" => $renew_cycle["new_recurring_amount"],
            "billingcycle" => $billingcycle,
        ];
        $up = \think\Db::name("host")->where("id", $res["hostid"])->update($data);
        if ($res["domainstatus"] == "Suspended" && !!configuration("cron_host_unsuspend")) {
        ...
        if ($client) {
            $params = [
                "product_name" => $res["name"],
                "product_end_time" => date("Y-m-d H:i:s", $next_time),
                "product_mainip" => $res["dedicatedip"],
                "hostname" => $res["domain"],
            ];
            $params["product_mainip"] .= $res["port"] ? ":" . $res["port"] : "";
        ...
        if ($up) {
            \think\Db::name("upgrades")
                ->where("type", "configoptions")
                ->where("relid", $res["hostid"])
                ->where("uid", $res["uid"])
                ->where("status", "Pending")
                ->where("paid", "N")
                ->update(["days_remaining" => 1]);
            $description =
                "产品续费成功 - Host ID:" . $res["hostid"] . "，到期时间修改为：" . date("Y-m-d H:i:s", $next_time);
        } else {
            $description = "产品续费失败 - Host ID:" . $res["hostid"];
