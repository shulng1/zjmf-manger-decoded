                ...
                if (!empty($menuAnnotations)) {
                    foreach ($menuAnnotations as $menuAnnotation) {
                        ...
                        if (empty($menuAnnotation->parent)) {
                            ...
                            if ($countParent > 3) {
                                throw new \Exception(
                                    $controllerClass . ":" . $action . "  @adminMenuRoot parent格式不正确!",
                                );
                            }
                            $parentApp = $app;
                            ...
                            $findParentAdminMenu = \app\admin\model\AdminMenuModel::where([
                                "app" => $parentApp,
                                "controller" => $parentController,
                                "action" => $parentAction,
                            ])->find();
                            if (empty($findParentAdminMenu)) {
                                $parentId = \app\admin\model\AdminMenuModel::insertGetId([
                                    "app" => $parentApp,
                                    "controller" => $parentController,
                                    "action" => $parentAction,
                                    "name" => "--new--",
                        ...
                        $findAdminMenu = \app\admin\model\AdminMenuModel::where([
                            "app" => $app,
                            "controller" => $controllerName,
                            "action" => $action,
                        ])->find();
                        if (empty($findAdminMenu)) {
                            \app\admin\model\AdminMenuModel::insert([
                                "parent_id" => $parentId,
                                "type" => $type,
                                "status" => $status,
                            ...
                            if ($findAdminMenu["name"] == "--new--") {
                                \app\admin\model\AdminMenuModel::where([
                                    "app" => $app,
                                    "controller" => $controllerName,
                                    "action" => $action,
                                ])->update([
                        ...
                        $findAuthRuleCount = \app\admin\model\AuthRuleModel::where([
                            "app" => $app,
                            "name" => $authRuleName,
                            "type" => "admin_url",
                        ])->count();
                        if ($findAuthRuleCount == 0) {
                            \app\admin\model\AuthRuleModel::insert([
                                "app" => $app,
                                "name" => $authRuleName,
                                "type" => "admin_url",
                                "param" => $param,
                            ...
                            \app\admin\model\AuthRuleModel::where([
                                "app" => $app,
                                "name" => $authRuleName,
                                "type" => "admin_url",
                            ])->update(["param" => $param, "title" => $menuName]);
                        }
                    }
                }
                ...
                if (!empty($methods)) {
                    foreach ($methods as $method) {
                        if ($method->class == $controllerClass && strpos($method->name, "_") !== 0) {
                            $menuAnnotations = \mindplay\annotations\Annotations::ofMethod(
                                $controllerClass,
                                $method->name,
                                "@adminMenu",
                            );
                            if (!empty($menuAnnotations)) {
                                ...
                                if (empty($menuAnnotation->parent)) {
                                    ...
                                    if ($countParent > 3) {
                                        throw new \Exception(
                                            $controllerClass . ":" . $action . "  @menuRoot parent格式不正确!",
                                        );
                                    }
                                    $parentApp = $app;
                                    $parentController = $controllerName;
