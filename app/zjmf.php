...
function pushTicketReply($id)
{
    $ticket_reply = \think\Db::name("ticket_reply")
        ...
        ->field("a.id as rid,c.password,a.content,c.id,c.stream_info,b.tid,a.attachment")
        ->where("a.id", $id)
        ->find();
    ...
    if (!empty($stream_info["downstream_url"]) && !empty($stream_info["downstream_token"])) {
        ...
        if (!empty($ticket_reply["attachment"])) {
            ...
            foreach ($ticket_reply["attachment"] as $key => $value) {
                $res = curlUpload(
                    rtrim($stream_info["downstream_url"], "/") . "/upload_image",
...
{
    $field =
        "id,productid,domain,username,password,dedicatedip,assignedips,port,os,os_url,stream_info,nextduedate,domainstatus";
