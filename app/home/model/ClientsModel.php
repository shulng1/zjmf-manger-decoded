...
class ClientsModel extends \think\Model
{
        ...
        if (empty($result)) {
            $data = [
                "username" => !empty($user["username"])
                    ? $user["username"]
                    : $user["phonenumber"] ?? substr($user["email"], 0, strpos($user["email"], "@")),
                "email" => $user["email"] ?? "",
                "avatar" => $user["avatar"] ?? "用户头像2-" . rand(10, 20) . ".jpg",
                "phone_code" => $user["phone_code"] ?? "86",
                "phonenumber" => $user["phonenumber"] ?? "",
                "currency" => getDefaultCurrencyId(),
                "password" => !empty($user["password"]) ? cmf_password($user["password"]) : "",
                "lastloginip" => get_client_ip(0, true),
                "create_time" => time(),
                "lastlogin" => time(),
                "status" => $status,
                "defaultgateway" => gateway_list()[0]["name"] ?? "",
                "sale_id" => $user["sale_id"] ?? 0,
                "qq" => $user["qq"] ?? "",
            ...
            if (!empty($user["phonenumber"])) {
                ...
                active_log_final(
                    sprintf($this->lang["User_home_regist"], "手机", $user["phonenumber"]),
                    $userId,
                    0,
                    0,
                    2,
    ...
    public function mobileVerify($user, $is_market = false)
    {
        ...
        if (!empty($result)) {
            ...
            if ($comparePasswordResult) {
                ...
                active_log_final(
                    sprintf($this->lang["User_home_login"], "手机密码", $user["phonenumber"], $userinfo["id"]),
                    $userinfo["id"],
                    1,
                    0,
                    2,
            ...
            $r = $result;
            $result = \think\Db::name("contacts")
                ->field("con.id as contactid,con.uid,con.username,con.avatar,con.password,con.status")
                ->alias("con")
                ->leftJoin("clients c", "c.id=con.uid")
                ->where("con.email", $user["phonenumber"])
                ->where("con.status", 1)
                ->where("c.status", 1)
                ->find();
            if (!empty($result)) {
    ...
    public function idVerify($user, $is_market = false)
    {
        ...
        if (!empty($result)) {
            ...
            if ($comparePasswordResult) {
                ...
                login_sms_remind($result);
                active_log_final(
                    sprintf($this->lang["User_home_login"], "id密码", $user["id"], $userinfo["id"]),
                    $userinfo["id"],
                    1,
                    0,
                    2,
            ...
            if (!empty($result)) {
                ...
                if ($comparePasswordResult) {
                    ...
                    login_sms_remind($result);
                    active_log_final(
                        sprintf($this->lang["User_home_login"], "邮件密码", $user["email"], $userinfo["id"]),
                        $userinfo["id"],
                        1,
                        0,
                        2,
    ...
    public function mobileCodeVerify($user)
    {
        ...
        if (!empty($result)) {
            ...
            if ($user["code"] == cache("logintel" . $mobile)) {
                ...
                login_sms_remind($result);
                active_log_final(
                    sprintf($this->lang["User_home_login"], "手机短信", $user["phonenumber"], $userinfo["id"]),
                    $userinfo["id"],
                    1,
                    0,
                    2,
        ...
        switch ($type) {
            case 1:
                ...
                if (!empty($result)) {
                    ...
                    active_log_final(
                        sprintf($this->lang["User_home_reset_pass"], "手机", $user["phonenumber"]),
                        $result["id"],
                        0,
                        0,
                        2,
                    ...
                    hook("client_reset_password", [
                        "uid" => $result["id"],
                        "password" => html_entity_decode($user["password"], ENT_QUOTES),
                    ]);
            ...
            case 2:
                ...
                if (!empty($result)) {
                    ...
                    active_log_final(
                        sprintf($this->lang["User_home_reset_pass"], "邮箱", $user["email"]),
                        $result["id"],
                        0,
                        0,
                        2,
    ...
    public function getUserCertifi($user_id, $status = 1)
    {
        ...
        if (!$res) {
            $res = \think\Db::name("certifi_company")
