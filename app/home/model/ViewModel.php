...
class ViewModel extends \think\Model
{
            ...
            if ($login_error_max_num) {
                $verify["is_captcha"] =
                    $login_error_max_num < intval(\think\facade\Cookie::get("login_error_log"))
                        ? 1
                        : $verify["is_captcha"];
                $verify["allow_login_phone_captcha"] =
                    $login_error_max_num < intval(\think\facade\Cookie::get("login_error_log"))
                        ? 1
                        : $verify["allow_login_phone_captcha"];
                $verify["allow_login_email_captcha"] =
                    $login_error_max_num < intval(\think\facade\Cookie::get("login_error_log"))
                        ? 1
                        : $verify["allow_login_email_captcha"];
                $verify["allow_login_id_captcha"] =
                    $login_error_max_num < intval(\think\facade\Cookie::get("login_error_log"))
                        ? 1
                        : $verify["allow_login_id_captcha"];
                $verify["allow_login_code_captcha"] =
                    $login_error_max_num < intval(\think\facade\Cookie::get("login_error_log"))
                        ? 1
                        : $verify["allow_login_code_captcha"];
            }
    ...
    public function userinfo($strjwt = "")
    {
        ...
        if ($authInfo) {
            $userinfo = \think\Db::name("clients")
                ->field(
                    "username,sex,avatar,companyname,email,country,province,city,address1,phonenumber,lastloginip,credit,language,status,id",
                )
                ->where("id", $uid)
                ->find();
        } else {
    ...
    {
        $list = \think\Db::name("news_type")
            ->where(["hidden" => 0, "parent_id" => $parent_id])
            ->order("id", "DESC")
            ->select()
            ->toArray();
        $data = [];
        foreach ($list as $k => $v) {
            $data[$k] = $v;
            $data[$k]["count"] = \think\Db::name("news_menu")
                ->where(["parent_id" => $v["id"], "hidden" => 0])
                ->count();
        }
        return $data;
    }
    ...
    public function nList($param, $cate_id)
    {
        ...
        if (isset($parent_id) && $parent_id > 0) {
            ...
            if (is_array($cid)) {
                ...
                $tmp_list = \think\Db::name("news_type")
                    ->where(["parent_id" => $cate_id, "hidden" => 0])
                    ->select()
                    ->toArray();
        ...
        $news_menu = \think\Db::name("news_menu as m")
            ->field("m.*")
            ->leftJoin("news", "m.id=news.relid")
            ->where($where)
            ->withAttr("news.content", function ($value) {
                return mb_substr($value, 0, 50);
            })
            ->order("push_time", $order)
            ->page($page, $pagecount)
            ->select()
            ->toArray();
        ...
        if ($parent_id > 0) {
            $news_type = \think\Db::name("news_type")
                ->where(["id" => $parent_id, "hidden" => 0])
                ->find();
    ...
    public function content($id, $cate_id)
    {
        ...
        $new_content = \think\Db::name("news_menu")
            ->field("m.*,n.content,u.user_nickname as author,nt.title as cate_name")
            ->alias("m")
            ->leftJoin("news n", "n.relid=m.id")
            ->leftJoin("news_type nt", "nt.id=m.parent_id")
            ->leftJoin("shd_user u", "u.id=m.admin_id")
            ->where("m.id", $id)
            ->find();
        ...
        \think\Db::name("news_menu")
            ->where("id", $id)
            ->update(["read" => $new_content["read"] + 1]);
        $returndata["next"] = \think\Db::name("news_menu")
            ->alias("m")
            ->field("m.id,m.title")
            ...
            ->order("id", "desc")
            ->find();
        $returndata["prev"] = \think\Db::name("news_menu")
            ->alias("m")
            ->field("m.id,m.title")
            ->where(array_merge($where, [["id", ">", $id]]))
            ->whereIn("parent_id", $parent)
    ...
    public function firstGroups($param, $tagDataVal)
    {
        ...
        $productgroups = \think\Db::name("product_first_groups")
            ->field("id,name,hidden,order")
            ->where("hidden=0" . $in)
            ->order("order", "asc")
            ->select();
    ...
    {
        $model = \think\Db::name("customfields")
            ->field("fieldname,id")
            ->where("type", "depot")
            ->order("id", "desc")
            ->select()
