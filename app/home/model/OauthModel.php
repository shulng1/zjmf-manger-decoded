...
class OauthModel extends \think\Model
{
    public function bindCheck($params)
    {
        ...
        $clients_uid = \think\Db::name("clients_oauth")
            ->where(["type" => $params["type"], "uid" => $params["uid"]])
            ->find();
        if ($clients_uid["id"]) {
            return ["status" => 400, "msg" => lang("账号已经绑定" . $params["type"])];
        }
        $clients_oauth = \think\Db::name("clients_oauth")
            ->where(["type" => $params["type"], "openid" => $params["openid"]])
            ->find();
    ...
    public function bind($params)
    {
        ...
        if (empty($clients_oauth["id"])) {
            $oauth = [
                ...
                "oauth" => json_encode($params["oauth"]),
            ];
