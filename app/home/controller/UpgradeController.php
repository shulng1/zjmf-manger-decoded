    ...
    public function upgradeProduct()
    {
        try {
            ...
            $oldhost = \think\Db::name("product_groups")
                ->alias("pg")
                ->field(
                    "p.name as host,h.domain,p.description,p.id as pid,h.uid,h.flag,h.firstpaymentamount,h.billingcycle",
                )
            ...
            foreach ($host as $k => $product) {
                if (
                    $product["pid"] != $oldhost["pid"] &&
                    in_array($product["pid"], $pids) &&
                    (!isset($params["need_pids"]) ||
                        (isset($params["need_pids"]) &&
                            is_array($params["need_pids"]) &&
                            in_array($product["pid"], $params["need_pids"])))
                ) {
    ...
    public function upgradeProductPost()
    {
        try {
            if ($this->request->isPost()) {
                ...
                $currency_id =
                    isset($params["currencyid"]) && !empty($params["currencyid"]) ? intval($params["currencyid"]) : "";
                $billingcycle =
                    isset($params["billingcycle"]) && !empty($params["billingcycle"])
                        ? strtolower(trim($params["billingcycle"]))
                        : "";
                $uid = request()->uid;
    ...
    public function addPromoToProduct()
    {
        try {
            if ($this->request->isPost()) {
                ...
                $result = $upgrade_logic->checkUpgradePromo(
                    $promo_code,
                    $hid,
                    $new_pid,
                    $new_billingcycle,
    ...
    public function checkoutProductUpgrade()
    {
        try {
            if ($this->request->isPost()) {
                ...
                $upgrade_logic = new \app\common\logic\Upgrade();
                $percent_value = $params["resource_percent_value"] ?: "";
                $result = $upgrade_logic->upgradeProductCommon(
                    $hid,
                    $newpid,
                    $billingcycle,
                    $currencyid,
