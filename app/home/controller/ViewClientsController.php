...
class ViewClientsController extends ViewBaseController
{
    ...
    public function login(\think\Request $request)
    {
        ...
        if ($request->isPost()) {
            ...
            if ($result["status"] == 200) {
                ...
                $error_num = \think\facade\Cookie::has("login_error_log")
                    ? \think\facade\Cookie::get("login_error_log")
                    : 0;
        ...
        if (configuration("login_error_switch")) {
            ...
            if ($login_error_max_num) {
                $dataLogin["is_captcha"] =
                    $login_error_max_num < intval(\think\facade\Cookie::get("login_error_log"))
                        ? 1
                        : $dataLogin["is_captcha"];
            }
        }
        if (!empty($dataLogin["is_captcha"])) {
            ...
            if (configuration("login_error_switch") && $login_error_max_num) {
                $dataLogin["allow_login_phone_captcha"] =
                    $login_error_max_num < intval(\think\facade\Cookie::get("login_error_log"))
                        ? 1
                        : $dataLogin["allow_login_phone_captcha"];
                $dataLogin["allow_login_email_captcha"] =
                    $login_error_max_num < intval(\think\facade\Cookie::get("login_error_log"))
                        ? 1
                        : $dataLogin["allow_login_email_captcha"];
                $dataLogin["allow_login_id_captcha"] =
                    $login_error_max_num < intval(\think\facade\Cookie::get("login_error_log"))
                        ? 1
                        : $dataLogin["allow_login_id_captcha"];
                $dataLogin["allow_login_code_captcha"] =
                    $login_error_max_num < intval(\think\facade\Cookie::get("login_error_log"))
                        ? 1
                        : $dataLogin["allow_login_code_captcha"];
            }
    ...
    public function register(\think\Request $request)
    {
        ...
        $dataRegister["allow_login_register_sms_global"] =
            $LoginRegisterIndex["data"]["allow_login_register_sms_global"];
    ...
    public function bind(\think\Request $request)
    {
        ...
        $LoginRegisterIndex = $Login->LoginRegisterIndex();
        $data["SmsCountry"] = $mobileLoginVerifyPage["data"];
        $data["Bind"]["allow_login_register_sms_global"] =
            $LoginRegisterIndex["data"]["allow_login_register_sms_global"];
    ...
    public function verified(\think\Request $request)
    {
                ...
                if ($step == "info") {
                    ...
                    if ($uploadImage["status"] !== 200) {
                        ...
                        if ($result["status"] == 200) {
                            ...
                            header(
                                "location:{$this->ViewModel->domain}/verified?action={$action}&step=authstart&plugin={$certifi_type}",
                            );
                            exit();
                        } else {
                            $data["ErrorMsg"] = $result["msg"];
        ...
        if ($plugin != "artificial") {
            ...
            if (floatval($_config["amount"]) > 0 && $pay) {
                $invoice = \think\Db::name("invoice_items")
                    ->alias("a")
                    ->field("b.status,b.id,b.url,b.subtotal")
                ...
                if ($invoice["status"] == "Unpaid") {
                    ...
                    $invoices_data = [
                        ...
                        "notes" => "",
                        "type" => "certifi_person",
                        "url" => "verified?action=personal&step=authstart&type={$plugin}",
                    ];
                    $invoices_items = [
                        "uid" => $uid,
                        "type" => "certifi_person",
            ...
            if ($request->isGet()) {
                ...
                if ($tmp["status"] == 4 && $flag) {
                    $postdata = [
                        "name" => $tmp["auth_real_name"] ?: "",
                        "card" => $tmp["auth_card_number"] ?: "",
                        "phone" => $tmp["auth_card_number"] ?: "",
                        "bank" => $tmp["bank"] ?: "",
                        "company_name" => $tmp["company_name"] ?: "",
                        "company_organ_code" => $tmp["company_organ_code"] ?: "",
                    ];
                            ...
                            if ($customfield["type"] == "file") {
                                if (!empty($tmp["custom_fields" . ($key + 1)])) {
                                    $customfield_plugin[$customfield["field"]] =
                                        config("certificate") . $tmp["custom_fields" . ($key + 1)];
                                } else {
                                    $customfield_plugin[$customfield["field"]] = "";
                                }
                            } else {
        ...
        if (isset($certifi_person["id"])) {
            if (
                $certifi_person["status"] == 1 &&
                isset($certifi_company["status"]) &&
                $certifi_company["status"] == 2
            ) {
    ...
    public function security(\think\Request $request)
    {
        ...
        $arr = [
            $model["password"] ? 1 : 0,
            $model["phonenumber"] ? 1 : 0,
            $model["email"] ? 1 : 0,
            $p_model && $p_model["status"] == 1 ? 1 : 0,
            $model["seond_verify"] ? 1 : 0,
            $model["is_login_sms_reminder"] ? 1 : 0,
            $model["email_remind"] ? 1 : 0,
        ];
        $count = count($arr);
        $number = 0;
    ...
    public function billing(\think\Request $request)
    {
        ...
        foreach ($dataBilling as &$value) {
            $value = [
                ...
                "status" => $value["status"],
                "status_zh" => $value["status_zh"],
                "type" => $value["type"],
                "type_zh" => $value["type_zh"],
                "payment_zh" => $value["payment_zh"],
            ];
        }
        ...
        if ($status == "Paid") {
            ...
            if ($invoice["type"] == "recharge") {
                ...
                $client["is_open_credit_limit"] =
                    configuration("credit_limit") == 1 ? $client["is_open_credit_limit"] : 0;
                ...
                if (!empty($client["is_open_credit_limit"]) && $subtotal <= $client["credit_limit_balance"]) {
                    if (!is_null($param["use_credit_limit"])) {
                        ...
                        $currency = \think\Db::name("currencies")
                            ->field("prefix,suffix")
                ...
                if ($credit <= 0) {
                    ...
                    $data["Pay"]["total_desc"] =
                        $credit_data["currency"]["prefix"] . $credit_data["total"] . $credit_data["currency"]["suffix"];
    ...
    public function submitticket(\think\Request $request)
    {
        ...
        if ($request->dptid) {
            $ticketCustom = \think\Db::name("customfields")
                ->where("relid", $request->dptid)
                ->where("type", "ticket")
                ->select()
                ->toArray();
        ...
        if ($action == "download") {
            ...
            if (is_array($download) && $download["status"] == 200) {
                $url =
                    ...
                    "/downloads?action=download&id={$param["id"]}&cate_id={$param["cate_id"]}&type=0";
    ...
    public function invoicecompany(\think\Request $request)
    {
        ...
        if (!empty($id) && $action == "edit") {
            ...
            $data["Pages"] = $this->ajaxPages(
                $dataVoucherinfolist,
                $limit,
                $page,
                $getVoucherInfoList["data"]["total"],
            );
            $data["Limit"] = $limit;
        }
            ...
            if ($status == "Paid") {
                ...
                if ($invoice["type"] == "recharge") {
                    ...
                    if ($credit <= 0) {
                        ...
                        $data["Pay"]["total"] = $credit_data["total"];
                        $data["Pay"]["total_desc"] =
                            $credit_data["currency"]["prefix"] .
                            $credit_data["total"] .
                            $credit_data["currency"]["suffix"];
                        $data["Pay"]["credit"] = $credit_data["credit"];
                        $data["Pay"]["invoiceid"] = $credit_data["invoiceid"];
                        $data["Pay"]["deduction"] = $credit_data["deduction"];
                        $pay_data = $Pay->startPay($request);
    ...
    public function service(\think\Request $request)
    {
        ...
        if ($nav_info["templatePage"] == "service_ssl") {
            ...
            $data["isSslPage"] = 1;
            $data["certifi_log"] = \think\Db::name("certifi_log")
        ...
        $data["Title"] =
            (new \app\common\logic\Menu())->getOneNavs("client", $param["groupid"] ?: 0, "name") ?:
            get_title_lang("title_other_server");
        $data["Total"] = $getList["data"]["sum"];
    ...
    public function servicedetail(\think\Request $request)
    {
        ...
        if (!empty($tpl) && configuration("contract_open") == 1) {
            $has_contract = \think\Db::name("contract_pdf")
                ->where("host_id", $request->id)
                ->where("contract_id", $tpl["id"])
                ->order("id", "desc")
                ->find();
        ...
        $getHeader["data"]["host_data"]["format_nextduedate"] = format_nextduedate(
            $getHeader["data"]["host_data"]["nextduedate"],
        );
        $tpls = [
        ...
        if (!empty($action) && $request->isGet()) {
            ...
            if ($action == "log_page") {
                ...
                $data["Pages"] = $this->ajaxPages(
                    $getUserLogDcs["data"]["log_list"],
                    $limit,
                    $page,
                    $getUserLogDcs["data"]["count"],
                );
                $data["Limit"] = $limit;
                ...
                $data["Pages"] = $this->ajaxPages(
                    $getHostRecharge["data"]["invoices"],
                    $limit,
                    $page,
                    $getHostRecharge["data"]["count"],
                );
                $data["Limit"] = $limit;
        ...
        if (
            $getHeader["data"]["host_data"]["type"] == "dcim" &&
            $getHeader["data"]["host_data"]["dcim"]["auth"]["traffic"] == "on"
        ) {
        }
        ...
        if ($getHeader["data"]["host_cancel"]["type"] == "Immediate") {
            ...
            $getHeader["data"]["host_data"]["deletedate"] =
                strtotime(date("Y-m-d 00:00:00")) + $cron_day_start_time * 3600;
            ...
            if (configuration("cron_host_terminate") == 1) {
                if (configuration("cron_host_terminate_high") == 1) {
                    $cron_host_terminate_time = configuration(
                        "cron_host_terminate_time_" . $getHeader["data"]["host_data"]["type"],
                    );
                } else {
                    $cron_host_terminate_time = configuration("cron_host_terminate_time");
                }
                $getHeader["data"]["host_data"]["deletedate"] =
                    $getHeader["data"]["host_data"]["nextduedate"] + $cron_host_terminate_time * 24 * 3600;
            }
        }
        if (!empty($getHeader["data"]["host_data"]["assignedips"])) {
            ...
            if (
                count($getHeader["data"]["host_data"]["assignedips"]) == 1 &&
                $getHeader["data"]["host_data"]["assignedips"][0] == $dedicatedip[0]
            ) {
        ...
        if ($action == "recharge") {
            ...
            if ($invoice["type"] != "recharge") {
                ...
                if ($user_credit > 0) {
                    if ($user_credit > 0 && $invoice["subtotal"] == $invoice["total"] && $param["use_credit"] == 1) {
                        $paid_invoice_credit =
                            $user_credit + $invoice["credit"] + $invoice["subtotal"] - $invoice["total"];
            ...
            if ($invoice["type"] == "recharge") {
                ...
                if ($invoice["type"] != "credit_limit") {
                    ...
                    $data["paymt"] = $paymt;
                    if (
                        !empty($client["is_open_credit_limit"]) &&
                        $subtotal <= $client["credit_limit_balance"] &&
                        1 == $invoice["use_credit_limit"]
                    ) {
                        if (!is_null($param["use_credit_limit"])) {
                            if ($param["use_credit_limit"] == 1) {
                                $data["Pay"]["use_credit_limit"] = 1;
    ...
    public function contract(\think\Request $request)
    {
        ...
        $ClientsModel = new \app\home\model\ClientsModel();
        $client_data = $ClientsModel->getClientByField(
            "id",
            $request->uid,
            "companyname,address1,phonenumber,email,username",
        );
        $ClientsModel->replaceClientName($request->uid, $client_data);
