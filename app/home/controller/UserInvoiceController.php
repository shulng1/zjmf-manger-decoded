...
class UserInvoiceController extends CommonController
{
    ...
    public function index()
    {
        ...
        $count = \think\Db::name("invoices")
            ->field("id,paid_time,status,payment,subtotal as total,type as invoice_type,subtotal as sub,credit")
            ->where("delete_time", 0)
            ->where("is_delete", 0)
            ->where("uid", $uid)
        ...
        $invoices = \think\Db::name("invoices")
            ->field(
                "id,paid_time,status,payment,subtotal as total,type as invoice_type,subtotal as sub,credit,use_credit_limit",
            )
            ->where("delete_time", 0)
            ->where("is_delete", 0)
            ->where("uid", $uid)
        ...
        foreach ($invoices as $k => $invoice) {
            ...
            if ($invoice["invoice_type"] == "upgrade") {
                $items = \think\Db::name("invoice_items")
                    ->alias("a")
                    ->field(
                        "a.id,c.id as hostid,a.invoice_id,d.name,c.domain,a.amount as total,c.create_time as paid_time,a.type,a.description as status,a.payment as payment_zh,c.dedicatedip,c.regdate,c.nextduedate,d.type as product_type",
                    )
                    ->join("upgrades b", "a.rel_id = b.id")
                    ->join("host c", "b.relid = c.id")
                    ->leftJoin("products d", "c.productid = d.id")
                    ->where("a.delete_time", 0)
                    ->where("a.type", "upgrade")
                    ->where("a.rel_id", "<>", 0)
                    ->where("a.invoice_id", $invoice["id"])
                    ->whereNotIn("d.type", $developer_app_product_type)
                    ->withAttr("id", function ($value) use ($invoice) {
                ...
                $items = \think\Db::name("invoice_items")
                    ->alias("a")
                    ->field(
                        "a.id,b.id as hostid,a.invoice_id,c.name,b.domain,a.amount as total,b.create_time as paid_time,a.type,a.description as status,a.payment as payment_zh,b.dedicatedip,b.regdate,b.nextduedate,c.type as product_type",
                    )
                    ->join("host b", "b.id = a.rel_id")
                    ->leftJoin("products c", "b.productid = c.id")
                    ->where("a.delete_time", 0)
                    ->where("a.type", "<>", "upgrade")
                    ->where("a.rel_id", "<>", 0)
                    ->where("a.invoice_id", $invoice["id"])
                    ->whereNotIn("c.type", $developer_app_product_type)
                    ->withAttr("id", function ($value) use ($invoice) {
                        return $invoice["id"] . "-" . $value;
                    })
                    ->withAttr("invoice_id", function ($value) {
    ...
    public function deleteOrder()
    {
        ...
        try {
            ...
            \think\Db::name("host")
                ->whereIn("id", $hids)
                ->update(["domainstatus" => "Cancelled", "update_time" => time()]);
            \think\Db::commit();
        } catch (\Exception $e) {
            \think\Db::rollback();
    ...
    public function create()
    {
        ...
        $rows = db("invoice_items")
            ->where($where)
            ->field("id,create_time,due_time,description,amount,type,rel_id,")
    ...
    public function read($id)
    {
        ...
        $row = db("invoices")
            ...
            ->field(
                "i.id,i.uid,c.username,i.create_time,i.due_time,i.payment,i.credit,i.subtotal,i.status,i.payment,c.username,c.country,c.province,c.city",
            )
            ->where($where)
            ->find();
        if (empty($row)) {
        ...
        $host = db("invoice_items")
            ->field("id,rel_id,type,description,amount")
    ...
    public function sendPay()
    {
        ...
        if ($row["status"] == "Unpaid") {
            $payData = [
                "out_trade_no" => $row["id"],
                "product_name" => "智简魔方",
                "total_fee" => $row["subtotal"],
                "attach" => $row["type"] ?? "product@" . $uid . "@" . $row["id"] . "@" . $row["subtotal"],
                "fee_type" => $currency,
            ];
    ...
    public function getInvoices()
    {
        ...
        $total = \think\Db::name("invoice_items")
            ->alias("a")
            ->leftJoin("invoices b", "b.id=a.invoice_id")
            ->where($fun)
            ->group("a.invoice_id")
            ->count();
        ...
        $invoices = \think\Db::name("invoice_items")
            ->alias("a")
            ->field(
                "b.id,b.invoice_num,b.subtotal,b.type,b.paid_time,b.due_time,b.status,b.use_credit_limit,b.subtotal as sub,
            b.payment,b.create_time as status_zh,b.payment as payment_zh,b.delete_time as type_zh,b.credit",
            )
            ->leftJoin("invoices b", "b.id=a.invoice_id")
            ->where($fun)
    ...
    public function getInvoicesDetail()
    {
        ...
        $invoice_payment_status = config("invoice_payment_status");
        $gateways = gateway_list();
        $detail = \think\Db::name("invoices")
            ->alias("a")
            ->field(
                "b.username,b.companyname,a.status,a.status as status_zh,a.paid_time,a.payment,a.payment as payment_zh,a.subtotal,a.total,a.credit,a.id,a.url,b.phonenumber,a.create_time,a.use_credit_limit",
            )
            ...
            ->withAttr("payment_zh", function ($value, $data) use ($gateways) {
                foreach ($gateways as $v) {
                    ...
                    if (
                        $data["subtotal"] == $data["credit"] &&
                        ($data["status"] == "Paid" || $data["status"] == "Refunded")
                    ) {
                        return "余额支付";
                    }
                    if (
                        1 == $data["use_credit_limit"] &&
                        ($data["status"] == "Paid" || $data["status"] == "Refunded")
                    ) {
                        return "信用额支付";
                    }
        ...
        $invoice_items = \think\Db::name("invoice_items")
            ->field("type,description,description2,amount,type as type_zh,rel_id")
            ->where("invoice_id", $id)
            ->where("uid", $uid)
        ...
        $credit_log_uses = \think\Db::name("credit")
            ->field("id,create_time as pay_time,amount as amount_in,description as gateway")
        ...
        foreach ($invoice_items as &$invoice_item) {
            $host = \think\Db::name("host")
                ->alias("a")
                ->field("a.id,b.groupid,a.productid")
                ->leftJoin("products b", "a.productid = b.id")
                ->where("a.id", $invoice_item["rel_id"])
                ->find();
                ...
                if ($host_data) {
                    ...
                    foreach ($invoice_items as $key => $m) {
                        if ($m["type"] == "host" && isset($host_data[$m["rel_id"]])) {
                            ...
                            $new_des[0] .= $host_data[$m["rel_id"]]["dedicatedip"]
                                ? "(" . $host_data[$m["rel_id"]]["dedicatedip"] . ")"
                                : "";
        ...
        $client_currency = getUserCurrency($uid);
        $data = [
            "payee" => configuration("company_name"),
            "detail" => $detail,
    ...
    public function accountsRecord()
    {
        ...
        $accounts = \think\Db::name("accounts")
            ...
            ->field(
                "a.id,a.invoice_id,a.pay_time,a.gateway,a.gateway as payment_zh,a.description,b.type,b.type as type_zh,a.amount_in,a.trans_id",
            )
            ->where($where)
            ->withAttr("type_zh", function ($value, $data) use ($items) {
                if ($value == "combine") {
        ...
        foreach ($accounts as &$vv) {
            $refund = \think\Db::name("accounts")
                ->field("id,amount_out")
                ->where("refund", $vv["id"])
                ->where("invoice_id", $vv["invoice_id"])
                ->where("delete_time", 0)
                ->where("refund", ">", 0)
                ->select()
                ->toArray();
    ...
    public function creditRecord()
    {
        ...
        $where = function (\think\db\Query $query) {
            $query
                ->where("a.description", "like", "%Credit Applied to Invoice #%")
                ->whereOr("a.description", "like", "%Credit Applied to Renew Invoice #%");
        };
        $accounts = \think\Db::name("credit")
            ->alias("a")
            ->field("a.id,a.relid,a.create_time,a.description,a.description as type,a.amount")
            ->where("a.uid", $uid)
            ->where($where)
            ...
            ->toArray();
        foreach ($accounts as &$vv) {
            $refund = \think\Db::name("credit")
                ->alias("a")
                ->field("a.id,a.amount")
                ->where("a.uid", $uid)
                ->where("a.relid", $vv["relid"])
    ...
    public function creditLimitRecord()
    {
        ...
        $gateways = gateway_list();
        $invoice_type = config("invoice_type_all");
        $accounts = \think\Db::name("invoices")
            ->field(
                "id,invoice_num,subtotal,type,paid_time,due_time,status,payment,create_time as status_zh,update_time as payment_zh,delete_time as type_zh",
            )
            ->where($fun)
            ->withAttr("status_zh", function ($value, $data) use ($invoice_payment_status) {
                return $invoice_payment_status[$data["status"]] ?: "";
    ...
    public function rechargeRecord()
    {
        ...
        $accounts = \think\Db::name("accounts")
            ->alias("a")
            ->field(
                "a.id,a.pay_time,a.gateway,a.gateway as payment_zh,a.description,b.type as type_zh,a.amount_in,a.invoice_id,a.trans_id",
            )
            ->leftJoin("invoices b", "a.invoice_id = b.id")
            ->withAttr("type_zh", function ($value, $data) use ($items) {
                return $items[$data["type"]] ?: "";
    ...
    public function refundRecord()
    {
        ...
        $accounts = \think\Db::name("accounts")
            ->alias("a")
            ->field("a.id,a.pay_time,a.description,a.amount_out,a.invoice_id,a.trans_id")
            ->where($where)
            ->order($order, $sort)
            ->order("a.pay_time", "desc")
            ->page($page)
    ...
    public function withdrawRecord()
    {
        ...
        $rows = \think\Db::name("affiliates_withdraw")
            ->alias("a")
            ->field("a.id,a.num,a.type,a.create_time,a.status,a.reason,a.reason as des")
            ->where($where)
            ...
            ->select()
            ->toArray();
        $total = \think\Db::name("affiliates_withdraw")
            ->alias("a")
            ->field("a.id,a.num,a.type,a.create_time,a.status")
            ->where($where)
            ->count();
    ...
    public function getCombineInvoices()
    {
        ...
        $where = function (\think\db\Query $query) use ($uid, $ids) {
            $query
                ...
                ->where("delete_time", 0)
                ->where("is_delete", 0)
                ->whereNotIn("type", ["combine", "recharge"])
                ->where("status", "Unpaid")
                ->whereIn("id", $ids);
        };
