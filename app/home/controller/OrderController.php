    ...
    public function orders()
    {
        ...
        if ($this->request->isPost()) {
            $orders = \think\Db::name("orders")
                ->field("status,pay_time,create_time,balance,payment")
                ->where("uid", $clientId)
    ...
    public function orderPayHandle($data = [])
    {
        ...
        if ($IncoiceInfo["total"] < $total_amount) {
            ...
            if ($credit > 0) {
                ...
                credit_log([
                    "uid" => $IncoiceInfo["uid"],
                    "desc" => "Invoice #{$invoice_id} Overpayment",
                    "amount" => $credit,
                    "relid" => $invoice_id,
                ]);
            }
        ...
        $accountsData = [
            ...
            "refund" => 0,
            "description" => invoiceTypeToDescription($IncoiceInfo["type"], $invoice_id),
        ];
        ...
        if ($aid) {
            \think\Db::startTrans();
            try {
                $res2 = \think\Db::name("invoices")
                    ->where("id", $IncoiceInfo["id"])
                    ->where("delete_time", 0)
                    ->update($upData);
                \think\Db::name("invoice_items")
                    ->where("invoice_id", $IncoiceInfo["id"])
                    ->where("delete_time", 0)
                    ...
                    if ($invoice_credit <= $client_credit + 0.01) {
                        ...
                        if ($invoice_credit <= $client_credit) {
                            \think\Db::name("clients")
                                ->where("id", $IncoiceInfo["uid"])
                                ->setDec("credit", $invoice_credit);
                        } else {
                            \think\Db::name("clients")
                                ->where("id", $IncoiceInfo["uid"])
                                ->setDec("credit", $client_credit);
                        }
                        credit_log([
                            "uid" => $IncoiceInfo["uid"],
        ...
        if ($flag) {
            ...
            if ($IncoiceInfo["type"] == "product") {
                active_logs(
                    sprintf(
                        "产品支付成功 - 账单号#Invoice ID:%d - 交易单号#Transaction ID:%s",
                        $IncoiceInfo["id"],
                        $tans,
                    ),
                    $IncoiceInfo["uid"],
                ...
                active_logs(
                    "续费支付成功 - 账单号#Invoice ID:" . $IncoiceInfo["id"] . " - 交易单号#Transaction ID:" . $tans,
                    $IncoiceInfo["uid"],
                );
                active_logs(
                    "续费支付成功 - 账单号#Invoice ID:" . $IncoiceInfo["id"] . " - 交易单号#Transaction ID:" . $tans,
                ...
                active_logs(
                    "充值支付成功 - 账单号#Invoice ID:" . $IncoiceInfo["id"] . " - 交易单号#Transaction ID:" . $tans,
                    $IncoiceInfo["uid"],
                );
                active_logs(
                    "充值支付成功 - 账单号#Invoice ID:" . $IncoiceInfo["id"] . " - 交易单号#Transaction ID:" . $tans,
