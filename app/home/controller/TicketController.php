...
class TicketController extends CommonController
{
    ...
    public function getOpenTicketPage(\think\Request $request)
    {
        ...
        if ($uid) {
            ...
            $host_list = \think\Db::name("host")
                ->where("uid", $uid)
                ->field("h.id,p.name as product_name,h.id,h.domainstatus,h.domain,h.dedicatedip")
                ->alias("h")
        ...
        if (empty($uid)) {
            $depart = \think\Db::name("ticket_department")
                ->field("id,name")
                ->where("only_reg_client", 0)
                ->where("hidden", 0)
                ->select()
                ->toArray();
        } else {
    ...
    public function getTicketCustom(\think\Request $request)
    {
        ...
        $ticket_custom_data = \think\Db::name("customfields")
            ->field("id,fieldname,fieldtype,description,fieldoptions,regexpr,required,adminonly")
            ->where("relid", $depart_id)
            ->where("adminonly", 0)
            ->where("type", "ticket")
            ->select()
            ->toArray();
    ...
    public function getDepartmentList(\think\Request $request)
    {
        ...
        if (empty($uid)) {
            $data = \think\Db::name("ticket_department")
    ...
    public function createTicket(\think\Request $request)
    {
        ...
        $msg = [
            "dptid.require" => "请选择工单部门",
            "dptid.number" => "类型错误",
            "title.require" => "请输入工单标题",
            "content.require" => "请输入工单内容",
            "email.email" => "邮箱格式错误",
            "hostid.number" => "产品错误",
        ];
        ...
        if (!empty($uid)) {
            ...
            $department = \think\Db::name("ticket_department")
                ->field("id,name,is_product_order,is_certifi")
                ->where("id", $params["dptid"])
                ->where("hidden", 0)
                ->find();
        } else {
        ...
        foreach ($customfields as $k => $v) {
            if (
                !empty($v["required"]) &&
                !isset($params["customfield"][$v["id"]]) &&
                $params["customfield"][$v["id"]] == ""
            ) {
                $result["status"] = 406;
        ...
        if ($r) {
            ...
            foreach ($customfields as $v) {
                if (isset($params["customfield"][$v["id"]])) {
                    ...
                    if (
                        $v["fieldname"] == "select" &&
                        !in_array($params["customfield"][$v["id"]], explode(",", $v["fieldoptions"]))
                    ) {
                        continue;
                    }
                    $insert = [
            ...
            $departmentadmin = \think\Db::name("ticket_department_admin")
                ->field("t.admin_id,u.only_oneself_notice")
                ->alias("t")
                ->leftJoin("User u", "u.id=t.admin_id")
                ->where("t.dptid", $params["dptid"])
                ->select()
                ->toArray();
            $curl_multi_data = [];
            ...
            if ($client) {
                ...
                $arr_sms = [
                    "name" => $message_template_type[strtolower("Support_Ticket_Opened")],
        ...
        if ($r) {
            ...
            if ($td["is_open_auto_reply"] == 1) {
                ...
                active_log(
                    sprintf("自动回复工单成功#User ID:%d - Ticket ID:%s - %s", $uid, $r, $params["title"]),
                    $uid,
                );
            }
        }
    ...
    public function ticketDetail(\think\Request $request)
    {
        ...
        if ($ticket["host_id"] > 0) {
            $host = \think\Db::name("host")
                ->alias("a")
                ->field("b.name,a.domain,a.id")
                ->leftJoin("products b", "a.productid = b.id")
                ->where("a.id", $ticket["host_id"])
                ->find();
        ...
        $result["data"] = [
            ...
            "ticket" => $ticket,
            "evaluate" => $evaluate,
            "feedback_request" => $department["feedback_request"],
        ];
        return jsons($result);
    }
    /**
    ...
    public function replyTicket(\think\Request $request)
    {
        ...
        try {
            ...
            $r2 = \think\Db::name("ticket")
                ->where("id", $data["tid"])
                ->update(["admin_unread" => 1, "last_reply_time" => time(), "status" => 3]);
            if ($r1 && $r2) {
                \think\Db::commit();
        ...
        if ($result["status"] == 200) {
            ...
            active_logs(
                sprintf($this->lang["Ticket_home_replyTicket_success"], $params["tid"], $title["title"]),
                $uid,
                "",
                2,
            );
            ...
            $hook_data = [
                ...
                "uid" => $uid,
                "dptid" => $ticket["dptid"],
                "dptname" => $ticket["dptname"],
                "title" => html_entity_decode($ticket["title"], ENT_QUOTES),
                "content" => html_entity_decode($data["content"], ENT_QUOTES),
                "priority" => $ticket["priority"],
                "status" => 3,
                "status_title" => \think\Db::name("ticket_status")->where("id", 3)->value("title"),
            ];
    ...
    public function closeTicket(\think\Request $request)
    {
        ...
        if ($r) {
            active_logs(
                sprintf($this->lang["Ticket_home_closeTicket_success"], $ticket["tid"], $ticket["title"]),
                $uid,
            );
            active_logs(
                sprintf($this->lang["Ticket_home_closeTicket_success"], $ticket["tid"], $ticket["title"]),
    ...
    public function evaluate(\think\Request $request)
    {
        ...
        if ($r) {
            ...
            active_logs(
                sprintf($this->lang["Ticket_home_evaluate_success"], $ticket["tid"], $ticket["title"]),
                $uid,
                "",
                2,
            );
