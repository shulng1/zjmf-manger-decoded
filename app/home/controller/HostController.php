...
class HostController extends CommonController
{
    ...
    public function getList(\think\Request $request)
    {
        ...
        $where_search_area = "";
        if (!empty($dcim_area)) {
            $search_server = \think\Db::name("dcim_servers")
                ->field("serverid,area")
                ->where("area", "<>", "")
                ->select()
                ->toArray();
        ...
        if (!empty($where_search_area)) {
            $count = \think\Db::name("host")
                ...
                ->where($where)
                ->where($where_search_area)
                ->count();
            $max_page = ceil($count / $page);
            $data = \think\Db::name("host")
                ->field("h.orderid,p.api_type,p.zjmf_api_id")
                ->field(
                    "h.id,h.domain,h.initiative_renew,h.domainstatus,h.regdate,h.dedicatedip,h.assignedips,h.nextduedate,h.remark  notes,h.nextinvoicedate,h.firstpaymentamount,h.amount,h.billingcycle,h.os,h.os_url,h.dcimid,h.dcim_os,h.dcim_area,b.os server_os,p.name as productname,b.area,b.auth,p.type as product_type,p.id as pid,p.pay_type",
                )
                ->alias("h")
                ->leftJoin("products p", "p.id=h.productid")
                ->leftJoin("dcim_servers b", "h.serverid=b.serverid")
                ->where($where)
                ->where($where_search_area)
        ...
        if ($orderby === "domainstatus") {
            $data = $data
                ->orderField("domainstatus", ["Suspended", "Active", "Pending"], $sort)
                ->select()
                ->toArray();
        } else {
        ...
        foreach ($data as $key => $val) {
            ...
            if ($val["product_type"] == "ssl") {
                ...
                if ($val["api_type"] == "zjmf_api") {
                    $zjmf_data = zjmfCurl($val["zjmf_api_id"], "/provision/sslCertFunc", [
                        "func" => "getAllInfo",
                        "id" => $val["dcimid"],
                    ]);
        ...
        foreach ($data as $key => $val) {
            ...
            foreach ($all_options as $k => $v) {
                $ssl_option = \think\Db::name("product_config_options")
                    ->where("id", $v["configid"])
                    ->value("option_name");
                $ssl_option_sub = \think\Db::name("product_config_options_sub")
                    ->where("id", $v["optionid"])
                    ->value("option_name");
            ...
            $hco = \think\Db::name("host_config_options")
                ->alias("h")
                ->field("pco.option_name,pcos.option_name as option_names")
                ->join("product_config_options pco", "pco.id=h.configid")
                ->join("product_config_options_sub pcos", "pcos.id=h.optionid")
                ->where("h.relid", $val["id"])
                ->select()
                ->toArray();
            ...
            if (!empty($hco)) {
                foreach ($hco as $k => $v) {
                    ...
                    if ($hco[$k]["option_names"] == "" || $hco[$k]["option_name"] == "") {
                        $hco = [
                            ["option_name" => "os", "option_names" => ""],
                            ["option_name" => "Memory", "option_names" => ""],
                            ["option_name" => "DiskSpace", "option_names" => ""],
                            ["option_name" => "CPU", "option_names" => ""],
                        ];
                        break;
                    }
                }
    ...
    public function postSaveCate(\think\Request $request)
    {
        ...
        if (!empty($cate_id)) {
            ...
            \think\Db::name("host_category")
                ->where("id", $cate_id)
                ->update(["name" => $cate_name]);
        } else {
    ...
    public function deleteCate(\think\Request $request)
    {
        ...
        \think\Db::name("host")
            ->where("user_cate_id", $cate_id)
            ->update(["user_cate_id" => 0]);
        ...
        active_logs(
            sprintf($this->lang["Host_home_postSaveCate"], $uid, $cate_id, $host["user_cate_id"], 0),
            $uid,
            "",
            2,
        );
    ...
    public function postTransferCate(\think\Request $request)
    {
        ...
        \think\Db::name("host")
            ->where("id", $host_id)
            ->update(["user_cate_id" => $cate_id]);
        active_log_final(
            sprintf(
                $this->lang["Host_home_postTransferCate"],
                $uid,
                $host_id,
                $cate_id,
                $host["user_cate_id"],
    ...
    public function getDetails(\think\Request $request)
    {
        ...
        $returndata = [];
        $host_data = \think\Db::name("host")
            ->field(
                "h.orderid,h.productid,h.serverid,h.regdate,h.domain,h.payment,
                ...
                p.type,p.name as productname,p.pay_method as payment_type,p.config_options_upgrade,p.config_option1,g.name as groupname,o.ordernum,p.api_type",
            )
            ->alias("h")
            ->leftJoin("products p", "p.id=h.productid")
            ->leftJoin("product_groups g", "g.id=p.gid")
            ->leftJoin("orders o", "o.id=h.orderid")
            ->where("h.id", $host_id)
            ->find();
        $domainstatus_config = config("domainstatus");
        $currency = getUserCurrency($uid);
        $billing_cycle = config("billing_cycle");
        $host_data["suspendreason_type"] = explode("-", $host_data["suspendreason"])[0]
            ? explode("-", $host_data["suspendreason"])[0]
            : "";
        $host_data["suspendreason"] = explode("-", $host_data["suspendreason"])[1]
            ? explode("-", $host_data["suspendreason"])[1]
            : "";
        ...
        $host_data["firstpaymentamount_desc"] =
            $currency["prefix"] . $host_data["firstpaymentamount"] . $currency["suffix"];
        ...
        $upgrade_products_data = \think\Db::name("product_upgrade_products")
        ...
        if ($host_data["config_options_upgrade"] == 1) {
            $system_button["upgrade_option"] = [
                "name" => lang("UPGRADE_DOWNGRADE_OPTIONS"),
                "func" => "upgrade_option",
            ];
            if ($domainstatus == "Active") {
        ...
        if (
            $domainstatus == "Active" &&
            $host_data["payment_type"] == "prepayment" &&
            !in_array($host_data["billingcycle"], ["ontrial", "hour", "day"])
        ) {
            $system_button["product_transfer"] = [
                "name" => lang("PRODUCT_TRANSFER"),
                "func" => "product_transfer",
                "disabled" => false,
            ];
        }
        if (in_array($domainstatus, ["Active", "Suspended"])) {
            if (
                $host_data["payment_type"] == "prepayment" &&
                !in_array($host_data["billingcycle"], ["onetime", "free", "hour", "day"])
            ) {
                ...
                if ($host_data["payment_type"] == "postpaid" || in_array($host_data["billingcycle"], ["hour", "day"])) {
                    $system_button["pay_cycle"] = [
                        "name" => lang("PAYMENT_CURRENT_PERIOD"),
                        "func" => "pay_cycle",
                        "disabled" => false,
                    ];
                }
        ...
        $custom_field_data = \think\Db::name("customfields")
            ->field("id,fieldname")
            ->where("type", "product")
        ...
        if ($host_data["bwlimit"] > 0) {
            $flowpacket = \think\Db::name("dcim_flow_packet")
                ->field("id,name,capacity,price,sale_times,stock")
                ->where("status", 1)
                ->whereRaw("FIND_IN_SET('{$host_data["productid"]}', allow_products)")
                ->select()
                ->toArray();
            if (!empty($flowpacket)) {
        ...
        if ($host_data["type"] == "dcim" && $host_data["config_option1"] != "bms") {
            $server = \think\Db::name("servers")
                ...
                ->where("a.id", $host_data["serverid"])
                ->find();
            $returndata["dcim"]["auth"] = json_decode($server["auth"], true);
            if ($host_data["bwlimit"] > 0) {
                $returndata["dcim"]["flow_packet_use_list"] = get_dcim_traffic_usage_table(
                    $host_id,
                    $uid,
                    $server["bill_type"],
                    $host_data["bwusage"],
        ...
        if ($host_data["type"] == "cloud" || $host_data["type"] == "dcimcloud") {
            $os_config_option_id = \think\Db::name("product_config_links")
                ->alias("a")
                ->leftJoin("product_config_options b", "a.gid=b.gid")
                ->where("a.pid", $host_data["productid"])
                ->where("b.option_type", 5)
                ->value("b.id");
            $sub = \think\Db::name("product_config_options_sub")
                ->field("id,option_name")
                ->where("config_id", $os_config_option_id)
                ->where("hidden", 0)
            ...
            $os_info = \think\Db::name("host_config_options")
                ->alias("a")
                ->field("b.option_name")
                ->leftJoin("product_config_options_sub b", "a.optionid=b.id")
                ->where("a.relid", $host_id)
                ->where("a.configid", $os_config_option_id)
                ->find();
    ...
    public function getProduct(\think\Request $request)
    {
        ...
        $host_data = \think\Db::name("host")
            ->field(
                "h.orderid,h.id as host_id,h.uid,h.initiative_renew,h.productid,h.serverid,h.regdate,h.domain,h.payment,p.groupid,h.promoid,
                h.firstpaymentamount,h.amount,h.billingcycle,h.nextduedate,h.nextinvoicedate,
                h.dedicatedip,h.assignedips,h.domainstatus,h.username,h.password,h.suspendreason,p.id as pid,
                h.bwusage,h.bwlimit,h.os,h.remark,h.dcimid,h.dcim_area,h.dcim_os,h.port,p.type,p.name as productname,p.pay_method as payment_type,p.config_options_upgrade,p.api_type,p.zjmf_api_id,p.upper_reaches_id,p.upstream_price_type,p.upstream_price_value,g.name as groupname,o.ordernum",
            )
            ->alias("h")
            ->leftJoin("products p", "p.id=h.productid")
            ->leftJoin("product_groups g", "g.id=p.gid")
        ...
        if ($host_data["api_type"] == "zjmf_api") {
            ...
            $upstream_data = zjmfCurl(
                $host_data["zjmf_api_id"],
                "/host/header",
                ["host_id" => $host_data["dcimid"]],
                30,
                "GET",
            );
    ...
    public function getCancel(\think\Request $request)
    {
        ...
        $host_cancel = \think\Db::name("cancel_requests")
            ->field("type,reason")
            ->where("relid", $host_id)
            ->where("delete_time", 0)
            ->find();
    ...
    public function getMoudle(\think\Request $request)
    {
        ...
        if ($api_type == "zjmf_api") {
            ...
            if ($domainstatus == "Active") {
                if ($type == "dcimcloud") {
                    ...
                    $returndata["control_view"]["reinstall_random_port"] = $dcimcloud->supportReinstallRandomPort(
                        $host_id,
                    );
                } elseif ($type == "dcim") {
                    ...
                    $returndata["module_client_area"] = $module_client_area;
                    $returndata["module_chart"] = $provision_logic->chart($host_id);
                    $returndata["control_view"]["module_power_status"] = $provision_logic->checkDefineFunc(
                        $host_id,
                        "Status",
                    );
        ...
        if ($api_type == "zjmf_api") {
            $returndata["control_view"]["show_traffic_usage"] = $upstream_data["host_data"]["show_traffic_usage"]
                ? true
                : false;
        } elseif ($api_type == "manual") {
        ...
        if ($api_type == "zjmf_api") {
            ...
            if ($upstream_price_type == "percent") {
                foreach ($returndata["dcim"]["flowpacket"] as $k => $v) {
                    $returndata["dcim"]["flowpacket"][$k]["price"] = round(
                        ($v["price"] * $upstream_price_value) / 100,
                        2,
                    );
                }
            }
            ...
            if ($type == "dcim" && $config_option1 != "bms") {
                $returndata["dcim"]["auth"] = $upstream_data["dcim"]["auth"] ?? [
                    "bmc" => "off",
                    "crack_pass" => "off",
                    "ikvm" => "off",
                    "kvm" => "off",
            ...
            if ($bwlimit > 0) {
                $flowpacket = \think\Db::name("dcim_flow_packet")
                    ->field("id,name,capacity,price,sale_times,stock")
                    ->where("status", 1)
                    ->whereRaw("FIND_IN_SET('{$productid}', allow_products)")
                    ->select()
                    ->toArray();
                if (!empty($flowpacket)) {
                    foreach ($flowpacket as $k => $v) {
    ...
    public function getHeader(\think\Request $request)
    {
        ...
        $returndata = [];
        $host_data = \think\Db::name("host")
            ->field("p.cancel_control")
            ->field("o.create_time as ocreate_time,o.amount as order_amount")
            ->field(
                "h.id,h.orderid,h.initiative_renew,h.productid,h.serverid,h.regdate,h.domain,h.payment,p.groupid,h.promoid,
                h.firstpaymentamount,h.amount,h.billingcycle,h.nextduedate,h.nextinvoicedate,
                h.dedicatedip,h.assignedips,h.domainstatus,h.username,h.password,h.suspendreason,p.id as pid,p.api_type,
                h.auto_terminate_end_cycle,h.auto_terminate_reason,h.bwusage,h.bwlimit,h.os,h.remark,h.dcimid,h.dcim_area,h.dcim_os,h.port,p.type,p.name as productname,p.pay_method as payment_type,p.config_options_upgrade,p.api_type,p.zjmf_api_id,p.upstream_price_type,p.upstream_price_value,p.upper_reaches_id,p.config_option1,p.password password_rule,g.name as groupname,o.ordernum",
            )
            ->alias("h")
            ->leftJoin("products p", "p.id=h.productid")
            ->leftJoin("product_groups g", "g.id=p.gid")
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.initiative_renew,a.productid,a.uid,a.firstpaymentamount,a.amount,a.create_time,a.nextduedate,a.billingcycle,a.productid,c.status,c.id,a.domainstatus,a.regdate,a.flag,a.promoid",
            )
            ->leftJoin("orders b", "b.id = a.orderid")
            ->leftJoin("invoices c", "b.invoiceid = c.id")
            ->where("a.id", $host_id)
            ->find();
        if (
            $host["billingcycle"] != "onetime" &&
            $host["billingcycle"] != "free" &&
            $host["billingcycle"] != "hour" &&
            $host["billingcycle"] != "day"
        ) {
        ...
        if ($upgrade_logic->judgeUpgradeConfig($host_id)) {
            ...
            if (
                isset($upgradeConfig["data"]) &&
                ($upgradeConfig["data"]["host_config_options"] || $upgradeConfig["data"]["host"])
            ) {
        ...
        if ($host_data["type"] != "ssl") {
            if ($host_data["api_type"] == "zjmf_api") {
                ...
                if (!$returndata["host_data"]["dedicatedip"] && $upstream_data["host_data"]["dedicatedip"]) {
                    $sync_info = [
                        "dedicatedip" => $upstream_data["host_data"]["dedicatedip"],
                        "assignedips" => implode(",", $upstream_data["host_data"]["assignedips"]) ?? "",
                        "domain" => $upstream_data["host_data"]["domain"] ?? "",
                        "username" => $upstream_data["host_data"]["username"] ?? "",
                        "password" => cmf_encrypt($upstream_data["host_data"]["password"]),
                        "port" => \intval($upstream_data["host_data"]["port"]),
                        "os" => $upstream_data["host_data"]["os"],
                    ];
                    \think\Db::name("host")->where("id", $host_id)->update($sync_info);
                    $sync_info["assignedips"] = !empty($sync_info["assignedips"])
                        ? explode(",", $sync_info["assignedips"])
                        : [];
                ...
                $upper_reaches = \think\Db::name("zjmf_finance_api")
                    ->where("id", $host_data["upper_reaches_id"])
                    ->find();
                $returndata["manual"] = ["id" => $host_data["upper_reaches_id"], "name" => $upper_reaches["name"]];
                $upper_reaches_res = \think\Db::name("upper_reaches_res")->where("hid", $host_id)->find();
                $returndata["host_data"]["upper_reaches_res"] = $upper_reaches_res["id"] ?? "";
                ...
                $dcimid = \think\Db::name("customfieldsvalues")
                    ->alias("a")
                    ->leftJoin("customfields b", "a.fieldid=b.id")
                    ->where("a.relid", $host_id)
                    ->where("b.type", "product")
                    ->where("b.relid", $host_data["pid"])
                    ->where("b.fieldname", "hostid")
                    ->value("value");
        ...
        if ($host_data["api_type"] == "zjmf_api" || $host_data["api_type"] == "resource") {
            $returndata["host_data"]["show_traffic_usage"] = $upstream_data["host_data"]["show_traffic_usage"]
                ? true
                : false;
        } else {
        ...
        if ($host_data["api_type"] == "zjmf_api") {
            ...
            if ($host_data["upstream_price_type"] == "percent") {
                foreach ($returndata["dcim"]["flowpacket"] as $k => $v) {
                    $returndata["dcim"]["flowpacket"][$k]["price"] = round(
                        ($v["price"] * $host_data["upstream_price_value"]) / 100,
                        2,
                    );
                }
            }
            ...
            if ($host_data["type"] == "dcim" && $host_data["config_option1"] != "bms") {
                ...
                if ($host_data["bwlimit"] > 0) {
                    if ($host_data["type"] == "dcimcloud") {
                        $traffic_bill_type_config_options = (new \app\common\model\HostModel())->getConfigOption(
                            $host_id,
                            "traffic_bill_type",
                        );
                        $returndata["dcim"]["flow_packet_use_list"] = get_dcim_traffic_usage_table(
                            $host_id,
                            $uid,
                            $traffic_bill_type_config_options["sub_option_arr"][0],
                            $host_data["bwusage"],
                            $host_data["bwlimit"],
                        );
                    } else {
    ...
    public function setSslConfig(&$host_data)
    {
        ...
        if ($host_data["api_type"] == "zjmf_api") {
            $cert_info = zjmfCurl($host_data["zjmf_api_id"], "/provision/sslCertFunc", [
                "id" => $host_data["dcimid"],
                "func" => "getAllInfo",
            ]);
            ...
            $host_data["certssl_orderinfo"] = \think\Db::name("certssl_orderinfo")
                ->where("hostid", $host_data["id"])
                ->order("id", "desc")
                ->find();
            if ($host_data["certssl_orderinfo"]) {
                $host_data["certssl_orderinfo"]["domainNames_arr"] = explode(
                    PHP_EOL,
                    $host_data["certssl_orderinfo"]["domainNames"],
                );
                $host_data["certssl_orderinfo"]["domainNames_arr"] = array_filter(
                    $host_data["certssl_orderinfo"]["domainNames_arr"],
                );
                $host_data["cert_pinfo"] = \think\Db::name("certssl_product")
                    ->where("p_id", $host_data["certssl_orderinfo"]["cert_pid"])
                    ->where("server_name", $host_data["certssl_orderinfo"]["server_name"])
                    ->find();
            }
        }
        if ($host_data["certssl_orderinfo"]) {
            if ($host_data["certssl_orderinfo"]["country"]) {
                $host_data["certssl_orderinfo"]["country_name"] = \think\Db::name("sms_country")
                    ->where("iso", $host_data["certssl_orderinfo"]["country"])
                    ->value("name_zh");
            }
            $host_data["certssl_orderinfo"]["telephone"] = $host_data["certssl_orderinfo"]["telephone"]
                ? substr_replace($host_data["certssl_orderinfo"]["telephone"], "****", 3, 4)
                : "";
            $host_data["certssl_orderinfo"]["company_phone"] = $host_data["certssl_orderinfo"]["company_phone"]
                ? substr_replace($host_data["certssl_orderinfo"]["company_phone"], "****", 3, 4)
                : "";
            $host_data["certssl_orderinfo"]["issus_time"] = $host_data["certssl_orderinfo"]["issus_time"]
                ? date("Y-m-d H:i", $host_data["certssl_orderinfo"]["issus_time"])
                : "-";
            if ($host_data["certssl_orderinfo"]["due_time"]) {
                if (
                    $host_data["certssl_orderinfo"]["due_time"] > time() &&
                    $host_data["certssl_orderinfo"]["due_time"] < time() + 5184000
                ) {
                ...
                $host_data["certssl_orderinfo"]["due_time_day"] = ceil(
                    ($host_data["certssl_orderinfo"]["due_time"] - time()) / 86400,
                );
            }
            $host_data["certssl_orderinfo"]["due_time"] = $host_data["certssl_orderinfo"]["due_time"]
                ? date("Y-m-d H:i", $host_data["certssl_orderinfo"]["due_time"])
                : "-";
        } else {
    ...
    public function postNameToUser(\think\Request $request)
    {
        ...
        if (\think\facade\Validate::isEmail($tranfer_name)) {
            ...
            if (\think\facade\Validate::isMobile($tranfer_name)) {
                $tranfer_info = \think\Db::name("clients")
                    ->field("id,username")
                    ->where("phonenumber LIKE '%{$tranfer_name}' ")
                    ->find();
            }
        }
        ...
        if ($system_certifi == 1 && empty($user_certifi)) {
            return json([
                "status" => 406,
                "msg" => lang("THE_SYSTEM_REQUIRES_THE_RECEIVER_TO_PASS_THE_REAL_NAME_VERIFICATION"),
            ]);
        }
        if (!empty($tranfer_info)) {
    ...
    public function postTransfer(\think\Request $request)
    {
        ...
        $host_data = \think\Db::name("host")
            ->field("h.id,h.productid,h.domainstatus,h.billingcycle,p.pay_method  as payment_type")
            ->alias("h")
            ->leftJoin("products p", "p.id=h.productid")
            ->where("h.id", $host_id)
            ->where("h.uid", $uid)
        ...
        if (
            in_array($host_data["billingcycle"], ["ontrial", "hour", "day"]) ||
            $host_data["payment_type"] == "postpaid"
        ) {
    ...
    public function postReceiveTranfer(\think\Request $request)
    {
        $uid = $request->uid;
        $transfer_id = $request->transfer_id;
        $transfer_data = \think\Db::name("transfer")
            ->where("transfer_uid", $uid)
    ...
    public function getRenewPage()
    {
        ...
        $cycle = $product_model->getProductCycle(
            $host["productid"],
            $currency_id,
            $hid,
            $host["billingcycle"],
        ...
        if (!$flag) {
            if (
                (new \app\common\logic\Renew())->unchangePrice($hid, $host["billingcycle"], $currency_id) != -1 &&
                round((new \app\common\logic\Renew())->calculatedPrice($hid, $host["billingcycle"]), 2) !=
                    round($host["amount"], 2) &&
                $host["promoid"] == 0
            ) {
                $cycles = [];
            ...
            if (
                $host["billingcycle"] != "ontrial" &&
                !in_array($host["billingcycle"], array_column($cycles, "billingcycle"))
            ) {
                $cycles[] = [
                    "billingcycle" => $host["billingcycle"],
                    "billingcycle_zh" => $host["billingcycle_zh"],
                    "setup_fee" => 0,
                    "price" => 0,
                    "amount" => $host["amount"],
                    "saleproducts" => 0,
        ...
        if (
            !empty($params["billingcycles"]) &&
            in_array($params["billingcycles"], array_keys(config("billing_cycle")))
        ) {
            if ($params["billingcycles"] != $host["billingcycle"]) {
                $renew = new \app\common\logic\Renew();
                if (
                    (new \app\common\model\ProductModel())->checkProductPrice(
                        $host["productid"],
                        $params["billingcycles"],
                        $currency_id,
                    )
                ) {
        ...
        foreach ($cycles as &$vv) {
            $vv["duration"] =
                getNextTime(
                    $vv["billingcycle"],
                    $pay_type["pay_" . $vv["billingcycle"] . "_cycle"],
                    $host["nextduedate"],
    ...
    public function getHostRecharge()
    {
        ...
        $accounts = \think\Db::name("accounts")
            ->alias("a")
            ->field("a.trans_id,a.pay_time,a.gateway,a.refund,a.amount_out,ii.type,a.amount_in")
            ->leftJoin("invoice_items ii", "a.invoice_id = ii.invoice_id")
            ...
            ->where("ii.rel_id", $hostid)
            ->where("a.uid", $uid)
            ->where("a.delete_time", 0)
            ->whereIn("ii.type", ["host", "renew", "upgrade"])
            ->select()
            ->toArray();
        $invoices = \think\Db::name("invoice_items")
            ->distinct(true)
            ->field("invoice_id")
            ->where("rel_id", $hostid)
            ->whereIn("type", ["host", "renew"])
            ->select()
            ->toArray();
        $ids = array_column($invoices, "invoice_id") ?? [];
        if (!empty($ids[0])) {
            $credit_log_uses = \think\Db::name("credit")
                ->alias("f")
                ->leftJoin("invoice_items ii", "f.relid = ii.invoice_id")
                ->field(
                    "f.id as trans_id,f.create_time as pay_time,f.amount as amount_in,f.description as gateway,ii.type",
                )
                ->withAttr("type", function ($value) {
                    if ($value == "renew") {
                        return "续费";
    ...
    public function postAutoRenew()
    {
        ...
        if (strlen($initiative_renew) > 0) {
            \think\Db::name("host")
                ->where("id", $hid)
                ->update(["initiative_renew" => $initiative_renew]);
        }
        $text = ["关闭", "开启"];
        active_log_final(
            "设置产品-Host ID:{$hid} 的自动续费功能为: {$text[$initiative_renew]}",
            $params["uid"],
            2,
            $hid,
            2,
    ...
    public function postBatchRenewPage()
    {
        ...
        $host_data = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.productid,a.dedicatedip,a.uid,a.id,a.domainstatus,b.id as pid,b.name,b.pay_type,b.pay_method,a.nextduedate,a.billingcycle,a.amount,a.flag,b.groupid,a.promoid",
            )
            ->leftJoin("products b", "a.productid = b.id")
            ->whereIn("a.id", $host_ids)
            ->select()
            ->toArray();
        $total = 0;
        $totalsale = 0;
        ...
        foreach ($host_data as $k => $v) {
            if (
                $v["amount"] >= 0 &&
                !in_array($v["billingcycle"], ["free", "onetime"]) &&
                $v["pay_method"] == "prepayment" &&
                in_array($v["domainstatus"], ["Active", "Suspended"])
            ) {
                ...
                $pid = $v["pid"];
                $product_model = new \app\common\model\ProductModel();
                $allow_billingcycle = $product_model->getProductCycle(
                    $pid,
                    $currency_id,
                    $hid,
                    $billingcycle,
                ...
                if (!$flag) {
                    if (
                        (new \app\common\logic\Renew())->unchangePrice($hid, $billingcycle, $currency_id) != -1 &&
                        round((new \app\common\logic\Renew())->calculatedPrice($hid, $billingcycle), 2) !=
                            round($v["amount"], 2) &&
                        $v["promoid"] == 0
                    ) {
                        $cycles = [];
                    ...
                    if (!in_array($billingcycle, array_column($cycles, "billingcycle"))) {
                        $cycles[] = [
                            ...
                            "billingcycle_zh" => $billing_cycle[$billingcycle],
                            "setup_fee" => 0,
                            "price" => 0,
                            "amount" => $v["amount"],
                            "saleproducts" => 0,
                ...
                if ($billingcycle == "onetime" || $billingcycle == "free") {
                    ...
                    $next_time = getNextTime(
                        $billingcycle,
                        $pay_type["pay_" . $billingcycle . "_cycle"],
                        $v["nextduedate"],
                        $pay_type["pay_ontrial_cycle_type"] ?: "day",
                    );
                }
    ...
    public function postHourDayRenew(\think\Request $request)
    {
        ...
        if ($settlement) {
            ...
            \think\Db::startTrans();
            try {
                $exists_data = \think\Db::name("invoice_items")
                    ->where("uid", $uid)
                    ->where("type", "renew")
                    ->where("rel_id", $host_id)
                    ->where("delete_time", "=", 0)
                    ->find();
                if (!empty($exists_data)) {
                    \think\Db::name("invoices")
                        ->where("id", $exists_data["invoice_id"])
                        ->update(["delete_time" => $time]);
                    \think\Db::name("invoice_items")
                        ->where("id", $exists_data["id"])
                        ->update(["delete_time" => $time]);
                }
                $insert_invoice_data = [
                    "uid" => $uid,
                    "create_time" => $time,
                    "due_time" => strtotime("+1 hour"),
                    "subtotal" => $amount,
                    "total" => $amount,
                    "status" => "Unpaid",
                ];
                ...
                $insert_item_data = [
                    ...
                    "type" => "renew",
                    "rel_id" => $host_id,
                    "description" => $product_data["name"] . "续费，时长：" . $returndata["renewal_time"],
                    "amount" => $amount,
                    "due_time" => strtotime("+1 hour"),
                ];
                ...
                try {
                    ...
                    if (!empty($exists_data)) {
                        ...
                        \think\Db::name("invoice_items")
                            ->where("id", $exists_data["id"])
                            ->update(["delete_time" => $time]);
                        \think\Db::name("renew_cycle")
                            ->where("relid", $exists_data["id"])
                            ->where("paid", "=", "N")
                            ->update(["delete_time" => $time]);
                    }
                    $insert_invoice_data = [
                    ...
                    $insert_item_data = [
                        "invoice_id" => $r1,
                        "uid" => $uid,
                        "type" => "cycle_to_mon_year",
                        "rel_id" => $host_id,
                        "description" =>
                            $product_data["name"] . "，付款周期转为" . config("billing_cycle")[$change_cycle],
                        "amount" => $cost,
                        "due_time" => strtotime("+1 hour"),
                    ];
                    $r2 = \think\Db::name("invoice_items")->insertGetId($insert_item_data);
                    $renew_cycle_data = [
                        ...
                        "type" => "cycle_to_mon_year",
                        "relid" => $r2,
                        "new_cycle" => $change_cycle,
                        "new_recurring_amout" => $new_recurring_amout,
                        "status" => "Pending",
                        "paid" => "N",
                        "create_time" => $time,
    ...
    private function checkHostCycle($uid, $host_id)
    {
        ...
        $host_data = \think\Db::name("host")
            ->field("productid,payment,amount,billingcycle,nextduedate,domainstatus,dedicatedip")
            ->where("uid", $uid)
            ->where("id", $host_id)
            ->find();
        if (empty($host_data)) {
    ...
    public function getCancelPage(\think\Request $request)
    {
        ...
        $host_data = \think\Db::name("host")
            ->field("h.productid,h.billingcycle,h.domainstatus,h.dedicatedip,p.name as productname,g.name as groupname")
            ->alias("h")
            ->leftJoin("products p", "p.id = h.productid")
            ->leftJoin("product_groups g", "g.id=p.gid")
            ->where("h.id", $id)
    ...
    public function postCancel(\think\Request $request)
    {
        ...
        $host_data = \think\Db::name("host")
            ->field("id,domainstatus,productid")
            ->where("id", $id)
            ->where("uid", $uid)
            ->find();
        if (empty($host_data)) {
        ...
        \think\facade\Hook::listen("cancellation_request", [
            "uid" => $uid,
            "relid" => $id,
            "reason" => $reason,
            "type" => $type,
    ...
    public function getTrafficUsage(\think\Request $request)
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.regdate,a.serverid,a.dcimid,b.type,b.api_type,b.zjmf_api_id,b.config_option1")
            ->leftJoin("products b", "a.productid=b.id")
    ...
    public function postSetDownStream(\think\Request $request)
    {
        ...
        if (
            (strpos($params["downstream_url"], "https://") === 0 ||
                strpos($params["downstream_url"], "http://") === 0) &&
            strlen($params["downstream_token"]) == 32 &&
            is_numeric($params["downstream_id"])
        ) {
    ...
    public function getDedicatedServer(\think\Request $request)
    {
        ...
        $host_data = \think\Db::name("host")
            ->field(
                "h.orderid,h.initiative_renew,h.productid,h.serverid,h.regdate,h.domain,h.payment,p.groupid,h.promoid,
                h.firstpaymentamount,h.amount,h.billingcycle,h.nextduedate,h.nextinvoicedate,
                h.dedicatedip,h.assignedips,h.domainstatus,h.username,h.password,h.suspendreason,p.id as pid,
                h.auto_terminate_end_cycle,h.auto_terminate_reason,h.bwusage,h.bwlimit,h.os,h.remark,h.dcimid,h.dcim_area,h.dcim_os,h.port,p.type,p.name as productname,p.pay_method as payment_type,p.config_options_upgrade,p.api_type,p.zjmf_api_id,p.upstream_price_type,p.upstream_price_value,p.upper_reaches_id,p.config_option1,g.name as groupname,o.ordernum,p.api_type",
            )
            ->alias("h")
            ->leftJoin("products p", "p.id=h.productid")
            ->leftJoin("product_groups g", "g.id=p.gid")
