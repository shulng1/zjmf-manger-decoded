...
class ContactsController extends CommonController
{
    ...
    public function index(\think\Request $request)
    {
        ...
        $uid = $request->uid;
        $cid = $param["cid"];
        $contact_list = \think\Db::name("contacts")
        ...
        if (!empty($cid)) {
            $contact_data = \think\Db::name("contacts")
    ...
    public function save(\think\Request $request)
    {
        if ($request->isPost()) {
            ...
            $rule = [
                ...
                "postcode" => "number",
                "phonenumber" => "mobile",
                "generalemails" => "in:0,1",
                "invoiceemails" => "in:0,1",
                "productemails" => "in:0,1",
                "supportemails" => "in:0,1",
                "status" => "in:1,0,2",
                "permissions" => "array",
            ];
            $msg = [
                "cid.number" => lang("CONTACTS_SAVE_VERIFY_CID_NUMBER"),
                "username.chsDash" => lang("CONTACTS_SAVE_VERIFY_UNAME_CHADASH"),
                "sex.in" => lang("CONTACTS_SAVE_VERIFY_SEX_IN"),
                "email.require" => lang("CONTACTS_SAVE_EMAIL_REQUIRE"),
                "email.email" => lang("CONTACTS_SAVE_EMAIL_EMAIL"),
                "postcode.number" => lang("CONTACTS_SAVE_POSTCODE_NUMBER"),
                "phonenumber.mobile" => lang("CONTACTS_SAVE_PNUM_MOBILE"),
            ];
            ...
            $udata = [
                ...
                "postcode" => $param["postcode"] ?: 0,
                "phonenumber" => $param["phonenumber"] ?: "",
                "generalemails" => $param["generalemails"] ?: 0,
                "invoiceemails" => $param["invoiceemails"] ?: 0,
                "productemails" => $param["productemails"] ?: 0,
                "supportemails" => $param["supportemails"] ?: 0,
