...
class ContractController extends CommonController
{
    ...
    public function host()
    {
        ...
        $total = \think\Db::name("host")
            ...
            ->leftJoin("clients e", "a.uid=e.id")
            ->leftJoin("contract_pdf d", "a.id=d.host_id")
            ->leftJoin("invoices f", "f.id=b.invoiceid")
            ->where($where)
            ->group("a.id", "desc")
            ->count();
        $hosts = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,c.name,a.dedicatedip,a.domain,a.domainstatus,a.domainstatus as domainstatus_zh,
            a.amount,b.create_time,a.nextduedate,d.pdf_num,e.companyname,e.phone_code,e.phonenumber,
            e.email,e.address1,f.status,f.status as status_zh,d.status as pdf_status,d.id as pdf_id",
            )
            ->leftJoin("orders b", "a.orderid=b.id")
            ->leftJoin("products c", "a.productid=c.id")
            ->leftJoin("clients e", "a.uid=e.id")
    ...
    public function contractCreate()
    {
        ...
        if ($hid) {
            $contract = \think\Db::name("contract_pdf")
                ->alias("a")
                ->leftJoin("contract b", "a.contract_id=b.id")
                ->where("a.uid", $uid)
                ->where("a.host_id", $hid)
                ->where("b.base", "<>", 1)
                ->where("b.status", 1)
    ...
    public function contractPost()
    {
        ...
        if ($param["type"] == "I") {
            ...
            if ($up) {
                ...
                $hosts = \think\Db::name("host")
                    ->where("suspendreason", "like", "contract%")
                ...
                foreach ($hosts as $host) {
                    ...
                    if ($result["status"] == 200) {
                        $data_i[
                            "description"
                        ] = " 满足签订合同解除暂停条件 - 解除暂停 Host ID:{$data_i["host_id"]}的产品成功";
                        if ($is_zjmf) {
                        ...
                        $data_i[
                            "description"
                        ] = " 满足签订合同解除暂停条件 - 解除暂停 Host ID:{$data_i["host_id"]}的产品失败：{$result["msg"]}";
                        if ($is_zjmf) {
    ...
    public function contractList()
    {
        ...
        $total = \think\Db::name("contract_pdf")
            ->alias("a")
            ->leftJoin("host b", "a.host_id=b.id")
            ->leftJoin("products c", "b.productid=c.id")
            ->leftJoin("contract d", "a.contract_id=d.id")
            ->leftJoin("orders e", "b.orderid=e.id")
            ->where($where)
            ->count();
        $lists = \think\Db::name("contract_pdf")
            ->alias("a")
            ->field(
                "a.id,a.pdf_num,c.name,b.domain,b.dedicatedip,b.amount,e.create_time,b.nextduedate,b.domainstatus,d.force,
            b.domainstatus as domainstatus_zh,a.status,a.status as status_zh,a.express_company,a.express_order,a.cancel_post,a.host_id",
            )
            ->leftJoin("host b", "a.host_id=b.id")
            ->leftJoin("products c", "b.productid=c.id")
            ->leftJoin("contract d", "a.contract_id=d.id")
        ...
        foreach ($lists as &$v) {
            ...
            $invoice_id = \think\Db::name("contract_pdf")
                ->alias("a")
                ->leftJoin("invoice_items b", "a.id=b.rel_id")
                ->where("a.id", $v["id"])
                ->where("b.type", "contract")
                ->value("b.invoice_id");
        ...
        $data = [
            ...
            "status" => array_reverse($contract_status),
            "currency" => getUserCurrency(request()->uid),
            "bases" => (new \app\common\logic\Contract())->getUnsignedBaseContract(request()->uid),
        ];
        return jsons(["status" => 200, "msg" => lang("SUCCESS MESSAGE"), "data" => $data]);
    }
    /**
    ...
    public function download()
    {
        ...
        $uid = request()->uid;
        $id = intval($param["id"]);
        $contract_pdf = \think\Db::name("contract_pdf")
            ->field("pdf_address")
            ->where("id", $id)
            ->where("uid", $uid)
            ->find();
    ...
    {
        $uid = request()->uid;
        $voucher = \think\Db::name("voucher_post")
            ->field("id,username,detail,phone")
            ->where("uid", $uid)
            ->where("default", 1)
            ->find();
    ...
    public function postPost()
    {
        ...
        if (!empty($param["voucher_id"])) {
            \think\Db::name("voucher_post")
                ->where("id", intval($param["voucher_id"]))
                ->update([
            ...
            \think\Db::name("voucher_post")->insertGetId([
                "uid" => $uid,
                "username" => trim($param["username"]),
                "phone" => trim($param["phone"]),
                "detail" => trim($param["detail"]),
