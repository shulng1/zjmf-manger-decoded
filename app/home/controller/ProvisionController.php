...
class ProvisionController extends CommonController
{
    ...
    public function postClientAreaContent(\think\Request $request)
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.id,a.productid,a.domainstatus,a.uid,a.dcimid,b.server_type,b.type,b.api_type,b.zjmf_api_id")
            ->leftJoin("products b", "a.productid=b.id")
            ->where("a.id", $id)
            ->find();
        if (empty($host)) {
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            $result = zjmfCurl(
                $host["zjmf_api_id"],
                "/zjmf_api/provision/custom/content?jwt=" . urlencode($post_data["now_jwt"]),
                $post_data,
            );
        } elseif ($host["api_type"] == "resource") {
            $post_data["id"] = $host["dcimid"];
    ...
    public function execCustomButton()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("h")
            ->field("h.id,h.serverid,h.dcimid,h.uid,p.type,p.api_type,p.zjmf_api_id,p.config_option1")
            ...
            if ($res["status"] == "success" || $res["status"] == 200) {
                if ($host["api_type"] == "zjmf_api") {
                    if ($result["data"]["host"]) {
                        \think\Db::name("host")
                            ->where("id", $id)
                            ->update(["domainstatus" => $result["data"]["host"]["domainstatus"]]);
                    }
                }
                $result["status"] = 200;
                $result["msg"] = $res["msg"] ?: "";
    ...
    public function sslCertDown($orderNo)
    {
        ...
        if ($host["api_type"] == "zjmf_api") {
            $res = zjmfCurl($host["zjmf_api_id"], "/provision/sslCertFunc", [
                "id" => $host["dcimid"],
                "func" => "getAllInfo",
            ]);
            if ($res["status"] == 200) {
            ...
            if ($result["orderInfo"]) {
                ...
                $result["downLoad"] = \think\Db::name("certssl_download")
                    ->where("co_id", $result["orderInfo"]["id"])
                    ->find();
            }
        }
