...
class CartController extends CommonController
{
    ...
    {
        $resource = \think\Db::name("zjmf_finance_api")
            ->where("is_resource", 1)
            ->where("is_using", 1)
            ->order("id", "desc")
            ->find();
        $resource_id = $resource["id"];
        $first_groups = \think\Db::name("product_first_groups")
            ->field("id,name")
            ->where("hidden", 0)
            ->where("is_upstream", 1)
            ->where("zjmf_api_id", $resource_id)
            ->order("order", "asc")
            ->order("id", "asc")
            ->select()
            ->toArray();
        $product_groups = \think\Db::name("product_groups")
            ->field("id,gid,name")
            ->where("hidden", 0)
            ->where("is_upstream", 1)
            ->where("zjfm_api_id", $resource_id)
            ->order("order", "asc")
            ->order("id", "asc")
            ->select()
            ->toArray();
    ...
    public function postCreateProducts()
    {
        ...
        try {
            ...
            if (!empty($product_groups_id)) {
                ...
                if (empty($fexist)) {
                    $fgid = \think\Db::name("product_first_groups")->insertGetId([
                        "name" => "一级分组(系统默认)",
                        "hidden" => 0,
                        "order" => 0,
                        "create_time" => time(),
                        "zjmf_api_id" => $resource_id,
                        "is_upstream" => 1,
                    ]);
                } else {
                    $fgid = $fexist["id"];
                ...
                if (empty($gexist)) {
                    $gid = \think\Db::name("product_groups")->insertGetId([
                        "name" => "二级分组(系统默认)",
                        "headline" => "",
                        "tagline" => "",
            ...
            $up = [
                "product_shopping_url" => request()->domain() . "/cart?action=configureproduct&pid=" . $id,
                "product_group_url" => request()->domain() . "/cart?gid=" . $gid . "&fid=" . $fgid,
            ];
                        ...
                        if ($product_pricing["code"] == $currency["code"]) {
                            ...
                            foreach ($price_type as $v) {
                                ...
                                if ($api_type == "zjmf_api" && $upstream_pid > 0 && $upstream_price_type == "percent") {
                                    if ($product_pricing[$v[0]] >= 0) {
                                        $product_pricing[$v[0]] =
                                            ($product_pricing[$v[0]] * $upstream_price_value) / 100;
                                    }
                                    $product_pricing[$v[1]] = ($product_pricing[$v[1]] * $upstream_price_value) / 100;
                                }
                            }
            ...
            foreach ($advanced as $m) {
                if ($m["type"] == "condition") {
                    ...
                    foreach ($advanced_sub_id as $nn => $mm) {
                        $advanced_sub = \think\Db::name("product_config_options_sub")
                            ->field("config_id,id")
                            ->where("upstream_id", $nn)
                            ->order("id", "desc")
                            ->find();
                    ...
                    $new_advanced = [
                        "config_id" => intval($config_id_condition),
                        "sub_id" => json_encode($new_advanced_data),
                        "relation" => $m["relation"],
                        "type" => $m["type"],
                        "relation_id" => 0,
                        "upstream_id" => $m["id"],
                        ...
                        if ($m3["type"] == "result") {
                            if ($m3["relation_id"] == $m["id"]) {
                                ...
                                foreach ($advanced_sub_id_result as $n4 => $m4) {
                                    $new_advanced_sub_result = \think\Db::name("product_config_options_sub")
                                        ->field("config_id,id")
                                        ->where("upstream_id", $n4)
                                        ->order("id", "desc")
                                        ->find();
                                ...
                                $result_advanced = [
                                    "config_id" => intval($config_id_result),
                                    "sub_id" => json_encode($new_advanced_data_result),
                                    "relation" => $m3["relation"],
                                    "type" => $m3["type"],
                                    "relation_id" => $condition_id,
                                    "upstream_id" => $m3["id"],
                                ];
    ...
    public function hostInfo()
    {
        ...
        $hosts = \think\Db::name("host")
            ->field(
                "id,productid,domain,dedicatedip,assignedips,create_time,nextduedate,billingcycle,billingcycle as billingcycle_zh,firstpaymentamount,amount,port,username,password,initiative_renew,domainstatus,domainstatus as domainstatus_zh",
            )
            ->where("uid", $uid)
            ->whereIn("id", $hids)
            ->withAttr("billingcycle_zh", function ($value, $data) {
        ...
        if ($param["all"]) {
            ...
            foreach ($hosts as $host) {
                ...
                $host_option_config = \think\Db::name("host_config_options")
                    ->where("relid", $hostid)
                    ->select()
                    ->toArray();
    ...
    public function ontrialAndMax()
    {
        ...
        $pro = \think\Db::name("api_user_product")
            ->field("ontrial,qty")
            ->where("uid", $uid)
            ->where("pid", $pid)
            ->find();
    ...
    public function summary()
    {
        ...
        $client = \think\Db::name("clients")
            ->field("api_password,api_create_time,api_open,lock_reason,api_lock_time")
            ->where("id", $uid)
            ->find();
        ...
        $agent_pids = \think\Db::name("api_resource_log")
            ->where("uid", $uid)
            ->where("pid", "<>", 0)
            ->field("pid")
            ->distinct(true)
            ->column("pid");
        $host_count = \think\Db::name("host")
            ->whereIn("productid", $agent_pids)
            ->where("uid", $uid)
            ->where("stream_info", "like", "%downstream_url%")
            ->count();
        $active_count = \think\Db::name("host")
        ...
        $api_count = \think\Db::name("api_resource_log")
            ->where("uid", $uid)
            ->whereBetweenTime("create_time", $yesterday_start, $yesterday_end)
            ->count();
        ...
        $api_count2 = \think\Db::name("api_resource_log")
            ->where("uid", $uid)
            ->whereBetweenTime("create_time", $before_yesterday_start, $before_yesterday_end)
            ->count();
        ...
        $api_count3 = \think\Db::name("api_resource_log")
            ->where("uid", $uid)
            ->whereBetweenTime("create_time", $before_yesterday_start2, $before_yesterday_end2)
            ->count();
        ...
        $free_products = \think\Db::name("api_user_product")
            ->field("a.id,b.name,a.ontrial,a.qty")
            ->alias("a")
    ...
    {
        $total = \think\Db::name("api_resource_log")
    ...
    public function getProducts()
    {
        ...
        $count = \think\Db::name("products")
            ->where("hidden", 0)
            ->where("retired", 0)
            ->whereNotIn("type", array_keys($developer_app_product_type))
            ->count();
    ...
    public function getProductConfig()
    {
        ...
        $fields = \think\Db::name("customfields")
            ->field(
                "id,fieldname,description,fieldtype,fieldoptions,regexpr,required,showorder,showinvoice,sortorder,showdetail",
            )
            ->where("type", "product")
            ->where("relid", $pid)
            ->where("adminonly", 0)
        ...
        $product_pricings = \think\Db::name("pricing")
            ->alias("a")
            ->field("a.*,b.code")
        ...
        $config_groups = \think\Db::name("product_config_groups")
            ->alias("a")
            ->leftJoin("product_config_links b", "a.id = b.gid")
        ...
        foreach ($config_groups as $k => $v) {
            ...
            foreach ($options as $kk => $vv) {
                $subs = \think\Db::name("product_config_options_sub")
                    ->where("config_id", $vv["id"])
                    ->where("hidden", 0)
                    ->select()
                    ->toArray();
                foreach ($subs as $kkk => $vvv) {
                    $pricings = \think\Db::name("pricing")
                        ->alias("a")
                        ->field("a.*,b.code")
                        ->leftJoin("currencies b", "a.currency = b.id")
        ...
        $advanced = \think\Db::name("product_config_options_links")
            ->whereIn("config_id", array_unique($oids_all))
            ->order("id", "asc")
            ->select()
            ->toArray();
    ...
    public function globalSearch()
    {
        ...
        $count = \think\Db::name("products")
            ...
            ->leftJoin("product_first_groups c", "b.gid = c.id")
            ->where($where)
            ->count();
        $products = \think\Db::name("products")
            ->alias("a")
            ->field(
                "a.id,a.type,a.gid,a.name,a.description,a.pay_method,a.tax,a.order,a.pay_type,a.api_type,a.upstream_version,a.upstream_price_type,a.upstream_price_value,a.stock_control,a.qty",
            )
            ->leftJoin("product_groups b", "a.gid = b.id")
            ->leftJoin("product_first_groups c", "b.gid = c.id")
            ->where($where)
            ...
            if (!empty($v)) {
                ...
                if ($flag) {
                    if ($flag["type"] == 1) {
                        ...
                        $v["sale_price"] =
                            bcsub($v["product_price"], $rebate, 2) < 0 ? 0 : bcsub($v["product_price"], $rebate, 2);
        ...
        return jsons([
            ...
            "currencies" => $currenciesfilter,
            "default_currency" => $currency,
            "products" => $newfilterproducts ?: [],
            "order_page_style" => intval(configuration("order_page_style")),
        ]);
    }
    /**
     * @title 购物车支付，信用接口
    ...
    public function getGateway()
    {
        ...
        $client["amount_to_be_settled"] = \think\Db::name("invoices")
            ...
            ->where("uid", request()->uid)
            ->sum("total");
        $unpaid = \think\Db::name("invoices")
            ->where("type", "credit_limit")
            ->where("status", "Unpaid")
            ->where("is_delete", 0)
            ->where("uid", request()->uid)
            ->sum("total");
        $client["credit_limit_used"] = round($client["amount_to_be_settled"] + $unpaid, 2);
        $client["credit_limit_balance"] = round(
            $client["credit_limit"] - $client["credit_limit_used"] > 0
                ? $client["credit_limit"] - $client["credit_limit_used"]
                : 0,
            2,
        );
    ...
    public function index()
    {
        ...
        if (isset($params["first_gid"]) && !empty($params["first_gid"])) {
            ...
            $default_first = \think\Db::name("product_first_groups")
                ->field("id")
                ->where("hidden", 0)
                ->order("order", "asc")
                ->order("id", "asc")
        ...
        if ((isset($params["type"]) && $params["type"] == "uuid") || !empty($p_uid)) {
            ...
            $products = \think\Db::name("products")
                ->field(
                    "id,info,type,gid,name,description,pay_method,tax,order,pay_type,api_type,upstream_version,upstream_price_type,upstream_price_value,stock_control,qty,icon",
                )
                ->where("gid", $gid)
                ->where("hidden", 0)
                ->where("retired", 0)
                ->where("p_uid", ">", 0)
                ->where(function (\think\db\Query $query) use ($p_uid) {
                    if (!empty($p_uid)) {
            ...
            if (isset($params["gid"]) && !empty($params["gid"])) {
                ...
                $products = \think\Db::name("products")
                    ->field(
                        "id,type,gid,name,description,pay_method,tax,order,pay_type,api_type,upstream_version,upstream_price_type,upstream_price_value,stock_control,qty",
                    )
                    ->where("gid", $gid)
                    ->where("hidden", 0)
                    ->where("retired", 0)
                ...
                $defaultgroup = \think\Db::name("product_groups")
                    ->where("gid", $first_gid)
                    ->where("hidden", 0)
                    ->where("order_frm_tpl", "<>", "uuid")
                    ->order("order", "asc")
                    ->order("id", "asc")
                    ->find();
    ...
    public function setConfig()
    {
        ...
        if ($pro["api_type"] == "zjmf_api") {
            ...
            if ($api["auto_update"] == 1) {
                $param = [
                    "pid" => $pid,
                    "zjmf_finance_api_id" => $zjmf_finance_api_id,
                    "upstream_pid" => $upstream_pid,
                    "timeout" => 2,
                    "page_type" => "set_config_page",
                    "upstream_price_type" => $pro["upstream_price_type"],
                    "upstream_price_value" => $pro["upstream_price_value"],
                ];
                (new \app\common\logic\Product())->syncProduct($param);
            }
        }
        $servers = \think\Db::name("products")
            ->alias("p")
            ->field("s.id,s.name,s.noc")
            ->leftJoin("server_groups sg", "sg.id = p.server_group")
            ->leftJoin("servers s", "s.gid = sg.id")
            ->where("p.id", $pid)
            ->select()
            ->toArray();
        $serversfilter = [];
        ...
        if (empty($billingcycle)) {
            ...
            $billingcycle =
                $product_model->getProductCycle($pid, $currencyid, "", "", "", "", "", "", 1)[0]["billingcycle"] ?: "";
        }
        ...
        if (!empty($developer_app)) {
            $hosts = \think\Db::name("host")
                ->alias("a")
                ->field("a.id,b.name,a.dedicatedip")
                ->leftJoin("products b", "a.productid = b.id")
                ->leftJoin("customfields c", "c.relid = b.id")
                ->where("c.type", "product")
                ->where("a.domainstatus", "Active")
                ->where(function (\think\db\Query $query) use ($developer_app) {
        ...
        $cids = \think\Db::name("product_config_options")
            ->alias("a")
            ->field("a.id")
            ->leftJoin("product_config_links b", "b.gid = a.gid")
            ->leftJoin("product_config_groups c", "a.gid = c.id")
            ->where("b.pid", $pid)
            ->order("a.order", "asc")
            ->order("a.id", "asc")
            ->column("a.id");
        $links = \think\Db::name("product_config_options_links")
            ->whereIN("config_id", $cids)
        ...
        return jsons([
            ...
            "currency" => $currency,
            "dafault_currencyid" => $currencyid,
            "product" => $product,
            "option" => $alloption,
            "custom_fields" => $fields,
            "allow_qty" => $allow_qty,
            "developer_app" => !empty($developer_app) ? 1 : 0,
            "hosts" => $hosts ?? [],
            "links" => $links ?? [],
        ]);
    }
    ...
    public function getLinkAgeList()
    {
        ...
        $product_model = new \app\common\model\ProductModel();
        $billingcycle =
            $product_model->getProductCycle($req->pid, $currencyid, "", "", "", "", "", "", 1)[0]["billingcycle"] ?: "";
        $config_logic = new \app\common\logic\ConfigOptions();
    ...
    {
        $productgroups = \think\Db::name("product_groups")
            ->field("id,name,headline,tagline,order,gid,order_frm_tpl,tpl_type")
            ->where("hidden", 0)
            ->where("order_frm_tpl", "<>", "uuid")
            ->where(function (\think\db\Query $query) use ($first_gid) {
                if (!empty($first_gid)) {
    ...
    {
        $productfirstgroups = \think\Db::name("product_first_groups")
            ->field("id,name,order")
            ->where("hidden", 0)
            ->order("order", "asc")
            ->select();
    ...
    public function getTotal()
    {
        if ($this->request->isPost() || VIEW_TEMPLATE_WEBSITE === true) {
            ...
            $product = \think\Db::name("products")
                ->alias("a")
                ->field(
                    "a.id as productid,a.name,a.pay_type,b.*,a.api_type,a.upstream_version,a.upstream_price_type,a.upstream_price_value,a.hidden,a.stock_control,a.qty",
                )
                ->leftJoin("pricing b", "a.id = b.relid")
                ->where("a.id", $pid)
                ->where("b.type", "product")
                ->where("b.currency", $currencyid)
                ->find();
            if (!$product) {
            ...
            foreach ($configoption as $key => $value) {
                $option1 = \think\Db::name("product_config_options")
                    ->field("option_type,unit")
                    ->where("id", $key)
                    ->find();
                ...
                if ($option_type && $value) {
                    ...
                    if (!judgeQuantity($option_type)) {
                        $option = \think\Db::name("product_config_options_sub")
                            ->alias("pcos")
                            ->field(
                                "pco.is_discount,pcos.option_name as suboption_name,pco.option_type,pco.option_name as option_name,pco.hidden,p.*,pco.is_rebate",
                            )
                            ->leftJoin("product_config_options pco", "pco.id = pcos.config_id")
                            ->leftJoin("pricing p", "p.relid = pcos.id")
                            ->where("pcos.id", $value)
                            ->where("pcos.config_id", $key)
                        ...
                        if (!$option) {
                            ...
                            $option_filter["option_name"] = explode("|", $optionname)[1]
                                ? explode("|", $optionname)[1]
                                : $optionname;
                            $option_filter["suboption_name"] = $suboptionname_deal = explode("|", $suboptionname)[1]
                                ? explode("|", $suboptionname)[1]
                                : $suboptionname;
                            ...
                            if ($flag && $flag["type"] == 1) {
                                if ($is_ajmf_api) {
                                    $option_filter["suboption_setup_fee"] = bcsub(
                                        bcmul($optionupfee, $product["upstream_price_value"]) / 100,
                                        0,
                                    );
                                    $option_filter["suboption_sale_setup_fee"] = round(
                                        bcsub(
                                            $option_filter["suboption_setup_fee"],
                                            bcmul($option_filter["suboption_setup_fee"], $bates, 2),
                                            2,
                                        ),
                                        2,
                                    );
                                    $option_filter["suboption_sale_setup_fee"] =
                                        $option_filter["suboption_sale_setup_fee"] > 0
                                            ? $option_filter["suboption_sale_setup_fee"]
                                            : 0;
                                    $option_filter["suboption_price"] = bcsub(
                                        bcmul($optionprice, $product["upstream_price_value"]) / 100,
                                        0,
                                    );
                                    $option_filter["suboption_sale_price"] = round(
                                        bcsub(
                                            $option_filter["suboption_price"],
                                            bcmul($option_filter["suboption_price"], $bates, 2),
                                            2,
                                        ),
                                        2,
                                    );
                                    $option_filter["suboption_sale_price"] =
                                        $option_filter["suboption_sale_price"] > 0
                                            ? $option_filter["suboption_sale_price"]
                                            : 0;
                                    $option_filter["suboption_price_total"] = bcadd(
                                        bcmul($optionupfee, $product["upstream_price_value"]) / 100,
                                        bcmul($optionprice, $product["upstream_price_value"]) / 100,
                                    );
                                    $option_filter["suboption_sale_price_total"] = round(
                                        bcsub(
                                            $option_filter["suboption_price_total"],
                                            bcmul($option_filter["suboption_price_total"], $bates, 2),
                                            2,
                                        ),
                                        2,
                                    );
                                    $option_filter["suboption_sale_price_total"] =
                                        $option_filter["suboption_sale_price_total"] > 0
                                            ? $option_filter["suboption_sale_price_total"]
                                            : 0;
                                } else {
                                ...
                                $optionsaleupfee = round(
                                    bcsub($optionsaleupfee, bcmul($optionsaleupfee, $bates, 2), 2),
                                    2,
                                );
                                $optionsaleprice = round(
                                    bcsub($optionsaleprice, bcmul($optionsaleprice, $bates, 2), 2),
                                    2,
                                );
                            } else {
                                if ($is_ajmf_api) {
                        ...
                        $options = \think\Db::name("product_config_options_sub")
                            ->alias("pcos")
                            ->field(
                                "pcos.option_name as suboption_name,pcos.qty_minimum,pcos.qty_maximum,pco.option_type,pco.hidden,pco.option_name as option_name,pco.qty_minimum as min,pco.qty_maximum as max,pco.is_discount,p.*,pco.is_rebate",
                            )
                            ->leftJoin("product_config_options pco", "pco.id = pcos.config_id")
                            ->leftJoin("pricing p", "p.relid = pcos.id")
                            ->where("pcos.config_id", $key)
                        ...
                        if (!empty($options[0])) {
                            foreach ($options as $option) {
                                ...
                                if (
                                    ...
                                    $optionsaleprice =
                                        $option[$billingcycle] <= 0 ? 0 : $option[$billingcycle] * $value;
                                    ...
                                    $option_filter["option_name"] = explode("|", $optionname)[1]
                                        ? explode("|", $optionname)[1]
                                        : $optionname;
                                    $option_filter["suboption_name"] = explode("|", $suboptionname)[1]
                                        ? explode("|", $suboptionname)[1]
                                        : $suboptionname;
                                    if ($flag && $flag["type"] == 1) {
                                        if ($is_ajmf_api) {
            ...
            if ($is_ajmf_api) {
                ...
                $product_filter["product_setup_fee"] =
                    bcmul($product_filter["product_setup_fee"], $product["upstream_price_value"]) / 100;
                $product_filter["product_price"] = round(
                    bcmul($product_filter["product_price"], $product["upstream_price_value"]) / 100,
                    2,
                );
            ...
            if ($pay_ontrial_cycle["pay_type"] == "free") {
                ...
                $res["products"]["duration"] =
                    getNextTime(
                        $billingcycle,
                        $pay_ontrial_cycle["pay_" . $billingcycle . "_cycle"],
                        0,
                        $pay_ontrial_cycle["pay_ontrial_cycle_type"] ?: "day",
                    ) - time();
            }
            if ($flag) {
                if ($flag["type"] == 1) {
                    $res["products"]["sale_setupfee_total"] = bcsub(
                        ($rebate_setupfee * $flag["bates"]) / 100 + $setupfeetotal - $rebate_setupfee,
                        0,
                    );
                    $res["products"]["sale_price"] = bcsub(
                        ($rebate_price * $flag["bates"]) / 100 + $total - $setupfeetotal - $rebate_price,
                        0,
                    );
                    $res["products"]["sale_signal_price"] = bcsub(
                        ($rebate_signal_price * $flag["bates"]) / 100 + $signal_price - $rebate_signal_price,
                        0,
                    );
                    ...
                    if ($bates <= $rebate_price) {
                        ...
                        $res["products"]["sale_setupfee_total"] =
                            $rebate_setupfee - $negative >= 0
                                ? bcsub($rebate_setupfee - $negative + $total - $setupfeetotal - $rebate_price, 0)
                                : bcsub($total - $setupfeetotal - $rebate_price, 0);
    ...
    public function settle(\think\Request $request)
    {
        ...
        if (!empty($pos_param["cart_data"])) {
            \think\Db::name("cart_session")
                ->where("uid", $uid)
                ->update(["cart_data" => "", "update_time" => time()]);
            $cart_data = [];
            $pos_param["cart_data"]["configoptions"] = $shop->configfilter(
        ...
        foreach ($cart_data["products"] as $k => $value) {
            $product = \think\Db::name("products")
                ->field("id,name,is_truename,clientscount,api_type,upstream_pid,zjmf_api_id,pay_type")
                ->where("id", $value["pid"])
                ->find();
            $api = \think\Db::name("api_user_product")
                ->field("ontrial,qty")
                ->where("uid", $uid)
                ->where("pid", $value["pid"])
                ->find();
        ...
        foreach ($prod as $k => $value) {
            if ($value["clientscount"] > 0) {
                ...
                $productcounbt = \think\Db::name("host")
                    ->field("id")
                    ->where("productid", $k)
                    ->where("uid", $uid)
                    ->where($whereMap)
        ...
        foreach ($cart_data["products"] as $k => $value) {
            $product = \think\Db::name("products")
                ->field("name,is_truename,is_bind_phone")
                ->where("id", $value["pid"])
                ->find();
        ...
        if (!empty($cart_data["promo"]) && $checkout == 0) {
            ...
            if (!empty($promo)) {
                ...
                if (
                    ($promo["max_times"] != 0 && $promo["used"] >= $promo["max_times"]) ||
                    ($promo["expiration_time"] > 0 && $promo["expiration_time"] <= time())
                ) {
                    $promo_error = "优惠码已过期";
                    $promo = [];
                }
                ...
                if (!empty($promo["once_per_client"])) {
                    if (
                        \think\Db::name("orders")->where("uid", $uid)->where("promo_code", $cart_data["promo"])->find()
                    ) {
                ...
                if (!empty($promo["requires"])) {
                    ...
                    if (!empty($promo["requires_exist"])) {
                        $has_products = \think\Db::name("host")
                            ->field("productid")
                            ->where("uid", $uid)
                            ->where("domainstatus", "Active")
                            ->select()
        ...
        foreach ($cart_data["products"] as $k => $v) {
            ...
            if ($v["billingcycle"] == "free") {
                ...
                $product = \think\Db::name("products")
                    ->alias("a")
                    ->field("a.*," . $product_price_field)
                    ->leftJoin("pricing b", "b.type=\"product\" and a.id=b.relid and currency=" . $currency)
                    ->where("a.id", $v["pid"])
                    ->find();
            }
            $products[] = $product;
            ...
            $nextduedate = time();
            $customfields = \think\Db::name("customfields")
                ->where("relid", $v["pid"])
                ->where("type", "product")
                ->order("sortorder", "asc")
                ->select()
            ...
            if ($pay_type["pay_type"] == "free") {
                ...
                $product_item = [
                    ...
                    "product_config" => [],
                    "customfields" => [],
                    "dcim_os" => array_keys($v["os"])[0] ?? 0,
                    "os" => array_values($v["os"])[0] ?? "",
                    "host" => $v["host"] ?? "",
                    "password" => $v["password"] ?? "",
                    "qty" => $qty,
                    "percent_value" => $product["upstream_price_value"],
                ];
                $item_desc[] = $product["name"] . " (" . date("Y-m-d H", time()) . " - ) ";
                foreach ($customfields as $ck => $cv) {
                    if (isset($v["customfield"][$cv["id"]])) {
                        $product_item["customfields"][] = [
                            "fieldid" => $cv["id"],
                            "value" => $v["customfield"][$cv["id"]],
                        ];
                    }
                }
                $developer_app = checkDeveloperApp($v["pid"]);
                if (!empty($developer_app)) {
                    $host_custom = \think\Db::name("customfields")
                        ->field("id")
                        ->where("type", "product")
                        ->where("relid", $v["pid"])
                        ->where("fieldname", "hostid")
                        ->order("id", "asc")
                        ->find();
                    ...
                    foreach ($config_price as $kkk => $vvv) {
                        if (isset($v["configoptions"][$vvv["id"]])) {
                            ...
                            if (judgeQuantity($vvv["option_type"])) {
                                ...
                                foreach ($vvv["sub"] as $kkkk => $vvvv) {
                                    ...
                                    if (strpos($vvvv["option_name"], "-") !== false) {
                                        ...
                                        if (
                                            is_numeric($range[0]) &&
                                            is_numeric($range[1]) &&
                                            $range[1] >= $v["configoptions"][$vvv["id"]] &&
                                            $v["configoptions"][$vvv["id"]] >= $range[0]
                                        ) {
                                            $sub_id = $kkkk;
                                            break;
                                        }
                                ...
                                if ($sub_id > 0) {
                                    $product_item["product_config"][] = [
                                        "configid" => $vvv["id"],
                                        "optionid" => $sub_id,
                                        "qty" => $v["configoptions"][$vvv["id"]],
                                    ];
                                }
                            } else {
                                if (isset($vvv["sub"][$v["configoptions"][$vvv["id"]]])) {
                                    $product_item["product_config"][] = [
                                        "configid" => $vvv["id"],
                                        "optionid" => $v["configoptions"][$vvv["id"]],
                                        "qty" => 0,
                                    ];
                                }
                            }
                ...
                $next_time = getNextTime(
                    $v["billingcycle"],
                    $pay_type["pay_" . $v["billingcycle"] . "_cycle"],
                    0,
                    $pay_type["pay_ontrial_cycle_type"] ?: "day",
                );
                ...
                if ($pay_type["pay_type"] == "onetime") {
                    ...
                    $item_desc[] = $item_desc_home[] =
                        $product["name"] . " (" . date("Y-m-d H", time()) . " - " . date("Y-m-d H", $next_time) . ") ";
                }
                foreach ($customfields as $ck => $cv) {
                    if (isset($v["customfield"][$cv["id"]])) {
                        $product_item["customfields"][] = [
                ...
                if ($app_id > 0 && $auth_id > 0) {
                    $product = \think\Db::name("products")
                        ->field("id,professional_discount")
                        ->where("id", $app_id)
                        ->where("p_uid", ">", 0)
                        ->find();
                    $activity = \think\Db::name("app_activity_rel")
                        ->alias("a")
                        ->field("a.id,b.object,b.discount")
                        ->leftJoin("app_activity b", "b.id=a.activity_id")
                        ->where("b.start_time", "<=", time())
                        ->where("b.end_time", ">=", time())
                        ->where("a.pid", $app_id)
                        ->find();
                    $auth = \think\Db::name("host")
                        ->alias("a")
                        ->field("a.id,b.config_option2")
                        ->leftJoin("products b", "b.id=a.productid")
                        ->where("a.id", $auth_id)
                        ->find();
                    if (!empty($auth) && $auth["config_option2"] == "professional") {
                        if (!empty($activity)) {
                            $price_cycle = in_array($activity["object"], [0, 1])
                                ? round(($price_cycle * (100 - $activity["discount"])) / 100, 2)
                                : $price_cycle;
                        }
                        $price_cycle = round(($price_cycle * (100 - $product["professional_discount"])) / 100, 2);
                    } else {
                        if (!empty($activity)) {
                            $price_cycle = in_array($activity["object"], [0, 2])
                                ? round(($price_cycle * (100 - $activity["discount"])) / 100, 2)
                                : $price_cycle;
                        }
                    }
                        ...
                        if (isset($v["configoptions"][$vvv["id"]])) {
                            ...
                            if (judgeQuantity($vvv["option_type"])) {
                                ...
                                foreach ($vvv["sub"] as $kkkk => $vvvv) {
                                    ...
                                    if (
                                        $v["configoptions"][$vvv["id"]] >= $vvvv["qty_minimum"] &&
                                        $v["configoptions"][$vvv["id"]] <= $vvvv["qty_maximum"]
                                    ) {
                                        $sub_price_setup = $vvvv["price_setup"];
                                        $sub_price_cycle = $vvvv["price_cycle"];
                                        $sub_id = $kkkk;
                                ...
                                if ($sub_id > 0) {
                                    ...
                                    if ($vvv["hidden"] != 1) {
                                        if (judgeQuantityStage($vvv["option_type"])) {
                                            ...
                                            $price_cycle = bcadd(
                                                $price_cycle,
                                                bcmul($sub_price_cycle, $v["configoptions"][$vvv["id"]]),
                                            );
                                            $config_base_sale =
                                                (intval($v["configoptions"][$vvv["id"]]) > 0 ? $sub_price_setup : 0) +
                                                bcmul($sub_price_cycle, $v["configoptions"][$vvv["id"]]);
                                            $config_base_sale_setupfee =
                                                intval($v["configoptions"][$vvv["id"]]) > 0 ? $sub_price_setup : 0;
                                        }
                                    }
                                    $product_item["product_config"][] = [
                                ...
                                $configoptions_base_sale[] = [
                                    "config_base_sale" => $config_base_sale,
                                    "config_base_sale_setupfee" => $config_base_sale_setupfee,
                                    "is_discount" => $vvv["is_discount"],
                                    "id" => $vvv["id"],
                                    "is_rebate" => $vvv["is_rebate"],
                                ];
                            } else {
                                $config_base_sale = 0;
                                $config_base_sale_setupfee = 0;
                                if (isset($vvv["sub"][$v["configoptions"][$vvv["id"]]])) {
                                    if (
                                        $vvv["sub"][$v["configoptions"][$vvv["id"]]]["price_setup"] > 0 &&
                                        $vvv["hidden"] != 1
                                    ) {
                                        $price_setup = bcadd(
                                            $price_setup,
                                            $vvv["sub"][$v["configoptions"][$vvv["id"]]]["price_setup"],
                                        );
                                        $config_base_sale +=
                                            $vvv["sub"][$v["configoptions"][$vvv["id"]]]["price_setup"];
                                        $config_base_sale_setupfee =
                                    ...
                                    if (
                                        $vvv["sub"][$v["configoptions"][$vvv["id"]]]["price_cycle"] > 0 &&
                                        $vvv["hidden"] != 1
                                    ) {
                                        $price_cycle = bcadd(
                                            $price_cycle,
                                            $vvv["sub"][$v["configoptions"][$vvv["id"]]]["price_cycle"],
                                        );
                                        $config_base_sale +=
                                            $vvv["sub"][$v["configoptions"][$vvv["id"]]]["price_cycle"];
                                    }
                                    if (
                                        strpos($vvv["sub"][$v["configoptions"][$vvv["id"]]]["option_name"], "|") !==
                                        false
                                    ) {
                                        $item_desc_name .=
                                            ": " .
                                            substr(
                                                $vvv["sub"][$v["configoptions"][$vvv["id"]]]["option_name"],
                                                strpos(
                                                    $vvv["sub"][$v["configoptions"][$vvv["id"]]]["option_name"],
                                                    "|",
                                                ),
                                    ...
                                    $product_item["product_config"][] = [
                                        "configid" => $vvv["id"],
                                        "optionid" => $v["configoptions"][$vvv["id"]],
                                        "qty" => judgeYesNo($vvv["option_type"]) ? 1 : 0,
                                    ];
                                    $configoptions_base_sale[] = [
                                        "config_base_sale" => $config_base_sale,
                                        "config_base_sale_setupfee" => $config_base_sale_setupfee,
                ...
                if (
                    ...
                    $config_base_sale_setupfee =
                        bcmul($config_base_sale_setupfee, $product["upstream_price_value"]) / 100;
                    ...
                    foreach ($configoptions_base_sale as &$m) {
                        ...
                        $m["config_base_sale_setupfee"] =
                            bcmul($m["config_base_sale_setupfee"], $product["upstream_price_value"]) / 100;
                    }
                ...
                if (isset($param["resource_percent_value"])) {
                    ...
                    foreach ($configoptions_base_sale as &$m) {
                        ...
                        $m["config_base_sale_setupfee"] = bcmul(
                            $m["config_base_sale_setupfee"],
                ...
                if ($price_setup > 0) {
                    $product_item["invoices_items"][] = [
                        "uid" => $uid,
                        "type" => "setup",
                        "description" => "初装费",
                        "description2" => "初装费",
                        "amount" => $price_setup,
                        "due_time" => $nextduedate,
                        "payment" => $payment,
                    ];
                }
                $product_item["invoices_items"][] = [
                    ...
                    if ($flag["type"] == 1) {
                        ...
                        foreach ($configoptions_base_sale as &$mm) {
                            if ($mm["is_rebate"] || !$edition) {
                                ...
                                $mm["config_base_sale_setupfee"] = bcmul(
                                    $bates / 100,
                                    $mm["config_base_sale_setupfee"],
                                );
                            }
                        ...
                        $product_base_sale =
                            ($product_base_sale / $product_total_price) * ($product_total_price - $bates);
                        $product_base_sale_setupfee =
                            ($product_base_sale_setupfee / $product_total_price) * ($product_total_price - $bates);
                        $product_base_sale_price = $product_base_sale - $product_base_sale_setupfee;
                        foreach ($configoptions_base_sale as &$mm) {
                            if ($mm["is_rebate"] || !$edition) {
                                $mm["config_base_sale"] =
                                    ($mm["config_base_sale"] / $product_total_price) * ($product_total_price - $bates);
                                $mm["config_base_sale_setupfee"] =
                                    ($mm["config_base_sale_setupfee"] / $product_total_price) *
                                    ($product_total_price - $bates);
                            } else {
                ...
                if (!empty($promo) && $product_total_price_sale > 0) {
                    if ($flag && $promo["is_discount"] == 0) {
                        ...
                        if (
                            (empty($promo["appliesto"][0]) || in_array($v["pid"], $promo["appliesto"])) &&
                            (empty($promo["cycles"]) || in_array($v["billingcycle"], explode(",", $promo["cycles"])))
                        ) {
                            if ($promo["type"] == "percent") {
                                $promo_value =
                                    $promo["value"] > 100 ? 100 : ($promo["value"] > 0 ? $promo["value"] : 0);
                                ...
                                foreach ($configoptions_base_sale as $h) {
                                    if ($h["is_discount"] == 1) {
                                        ...
                                        $discount_recurring +=
                                            ($h["config_base_sale"] - $h["config_base_sale_setupfee"]) *
                                            (1 - $promo_value / 100);
                                    }
                                }
                                if ($promo["recurring"] > 0) {
                                    $product_total_price_sale_price = bcsub(
                                        $product_total_price_sale_price,
                                        $discount_recurring,
                                    );
                                }
                            } elseif ($promo["type"] == "fixed") {
                                $discount_pricing =
                                    $product_total_price_sale < $promo["value"]
                                        ? $product_total_price_sale
                                        : $promo["value"];
                                if ($promo["recurring"] > 0) {
                                    $product_total_price_sale_price =
                                        $product_total_price_sale_price - $promo["value"] > 0
                                            ? bcsub($product_total_price_sale_price, $promo["value"])
                                            : 0;
                                }
                            ...
                            $product_total_price_sale =
                                bcsub($product_total_price_sale, $discount_pricing, 2) > 0
                                    ? bcsub($product_total_price_sale, $discount_pricing, 2)
                                    : 0;
                            if ($promo["one_time"] == 1) {
                                if (empty($one_time)) {
                                    $qty = 1;
                                    ...
                                    $product_item["invoices_items"][] = [
                                        "uid" => $uid,
                                        "type" => "promo",
                                        "description" => promoCodeDesc($promo),
                                        "description2" => promoCodeDesc($promo) ?? "",
                                        "amount" => "-" . $discount_pricing,
                                        "due_time" => $nextduedate,
                                        "payment" => $payment,
                                        "one_time" => 1,
                                    ];
        ...
        if (request()->is_api == 1) {
            ...
            $is_downstream =
                (strpos($downstream_data["downstream_url"], "https://") === 0 ||
                    strpos($downstream_data["downstream_url"], "http://") === 0) &&
                strlen($downstream_data["downstream_token"]) == 32 &&
                is_numeric($downstream_data["downstream_id"]);
        }
        \think\Db::startTrans();
        try {
            ...
            foreach ($product_items as $k => $v) {
                ...
                $r = \think\Db::name("products")
                    ->field("name,stock_control,qty,auto_setup,api_type")
                ...
                if ($r["api_type"] == "resource") {
                    ...
                    $price_model = \think\Db::name("res_products")
                ...
                for ($i = 0; $i < $qtys; $i++) {
                    if ($qtys > 1) {
                        ...
                        $v["domain"] = empty($host)
                            ? generateHostName($host_rule["prefix"], $host_rule["rule"], $host_rule["show"])
                            : $host;
                    }
            ...
            \think\Db::name("cart_session")
                ->where("uid", $uid)
                ->update(["cart_data" => $new_cart_data, "update_time" => time()]);
            if (!empty($promo)) {
                \think\Db::name("promo_code")->where("id", $promo["id"])->setInc("used");
            }
            foreach ($products as $key => $v) {
                if ($v["groupid"] != 0) {
                    $ng = \think\Db::name("nav_group_user")
                        ->where("uid", $uid)
            ...
            if (count($all_host) == 1) {
                \think\Db::name("invoices")
                    ->where("id", $invoiceid)
                    ->update(["url" => "servicedetail?id=" . $all_host[0]]);
            } else {
                if (count($all_host) > 1) {
                    $menu = new \app\common\logic\Menu();
            ...
            if ($is_downstream) {
                $downstream_create = \think\Db::name("host")
                    ->whereLike("stream_info", "%" . $downstream_data["downstream_token"] . "%")
                    ->find();
                if (!empty($downstream_create)) {
                    ...
                    \think\Db::name("host")
                        ->where("id", \intval($all_host[0]))
                        ->update(["stream_info" => json_encode($stream_info)]);
                }
            }
            if ($invoiceid == 0) {
        ...
        if ($subtotal != 0) {
            ...
            foreach ($product_items as $k => $v) {
                ...
                if ($client && $v["billingcycle"] != "free") {
                    ...
                    $params = [
                        "product_name" => $hostre["name"],
                        "product_binlly_cycle" => $b[$v["billingcycle"]],
                        "product_price" => $v["amount"],
                        "order_create_time" => date("Y-m-d H:i:s", $v["create_time"]),
                    ];
                    ...
                    $arr = [
                        "name" => $message_template_type[strtolower("New_Order_Notice")],
                        "phone" => $phone,
        ...
        hook("shopping_cart_settle", [
            "orderid" => $orderid,
            "total" => $total,
            "invoiceid" => \intval($invoiceid),
            "hostid" => array_column($hids, "hid"),
        ]);
        if ($total == 0) {
            if (!empty($invoiceid)) {
                \think\Db::name("invoices")
                    ->where("id", $invoiceid)
                    ->update(["status" => "Paid", "paid_time" => time()]);
                ...
                foreach ($create_after_pay as $vv) {
                    if (configuration("shd_allow_auto_create_queue")) {
                        ...
                        $curl_multi_data[count($curl_multi_data)] = [
                            "url" => "async_create",
                            "data" => ["hid" => $vv, "ip" => get_client_ip6()],
                        ];
                    }
                }
            }
    ...
    public function addToShop(\think\Request $request)
    {
        ...
        if ($request->isPost()) {
            $rule = [
                ...
                "os" => "array",
                "hostid" => "integer",
                "checkout" => "number|in:0,1",
            ];
            $msg = [
                "pid.require" => lang("CART_ADDTOSHOP_VERIFY_PID_REQUIRE"),
                "pid.number" => lang("CART_ADDTOSHOP_VERIFY_PID_NUMBER"),
                "serverid.number" => lang("CART_ADDTOSHOP_VERIFY_SERVERID_REQUIRE"),
                "configoption.array" => lang("CART_ADDTOSHOP_VERIFY_CONFIG_ARRAY"),
                "customfield.array" => lang("CART_ADDTOSHOP_VERIFY_CUSTOM_ARRAY"),
                "qty.number" => lang("CART_ADDTOSHOP_VERIFY_QTY_NUMBER"),
                "os.array" => lang("CART_ADDTOSHOP_VERIFY_OS_ARRAY"),
                "hostid.integer" => lang("CART_ADDTOSHOP_VERIFY_HOSTID_INTEGER"),
            ];
            $param = $request->param();
            ...
            $shop = new \app\common\logic\Shop($uid);
            $product = \think\Db::name("products")
                ->field("host,password,name,is_truename,stock_control,qty,zjmf_api_id,upstream_pid,api_type")
                ->where("id", $pid)
                ->find();
            ...
            if (
                !empty($product["stock_control"]) &&
                isset($param["is_api"]) &&
                $param["is_api"] &&
                $product["qty"] < ($param["qty"] ?? 1)
            ) {
                return jsons(["msg" => lang("CART_SETTLE_PRO_STOCK_CONTROL", [$product["name"]]), "status" => 400]);
            }
                ...
                foreach ($prod as $k => $value) {
                    if ($value["clientscount"] > 0) {
                        ...
                        if ($value["clientscount"] < $productcounbt + $value["qty"]) {
                            return json([
                                "status" => 400,
                                "msg" => lang("CART_SETTLE_CLIENT_COUNT_ERROR", [$value["name"]]),
                            ]);
                        }
                    }
                }
            ...
            if ($product["api_type"] == "zjmf_api" || $product["api_type"] == "resource") {
                ...
                if ($result["status"] == 200) {
                    ...
                    if ($upstream_data["product"]["stock_control"] && $upstream_data["product"]["qty"] <= 0) {
                        return jsons([
                            "status" => 400,
                            "msg" => lang("CART_SETTLE_PRO_STOCK_CONTROL", [$product["name"]]),
                        ]);
                    }
                }
            }
    ...
    public function editToshopPage()
    {
        ...
        return jsons([
            ...
            "option" => $alloption,
            "custom_fields" => $fields,
            "config_options" => $cart_data["configoptions"],
            "custom_fields_value" => $cart_data["customfield"] ?? [],
            "billingcyle" => $billingcycle,
            "host" => $cart_data["host"] ?? "",
            "password" => $cart_data["password"] ?? "",
            "qty" => $cart_data["qty"],
            "hostid" => $cart_data["hostid"],
            "hosts" => $hosts ?? [],
            "links" => $links ?? [],
            "developer_app" => !empty($developer_app) ? 1 : 0,
        ]);
    ...
    public function editToShop(\think\Request $request)
    {
        if ($request->isPost()) {
            ...
            $msg = [
                "i.require" => lang("CART_EDIT_TOSHOP_VERIFY_I_REQUIRE"),
                "i.number" => lang("CART_EDIT_TOSHOP_VERIFY_I_NUMBER"),
                "billingcycle.require" => lang("CART_EDIT_TOSHOP_VERIFY_bill_REQUIRE"),
                "serverid.number" => lang("CART_EDIT_TOSHOP_VERIFY_SID_NUMBER"),
                "configoption.array" => lang("CART_EDIT_TOSHOP_VERIFY_CONF_ARRAY"),
                "customfield.array" => lang("CART_EDIT_TOSHOP_VERIFY_CUSTOM_ARRAY"),
                "qty.number" => lang("CART_ADDTOSHOP_VERIFY_QTY_NUMBER"),
                "os.array" => lang("CART_ADDTOSHOP_VERIFY_OS_ARRAY"),
                "hostid.integer" => lang("CART_ADDTOSHOP_VERIFY_HOSTID_INTEGER"),
            ];
    ...
    public function getOptions($pid, $currencyid, $admin = false, $cycle = "")
    {
        ...
        $configgroups = \think\Db::name("products")
            ->alias("p")
            ->field("pcg.id,pcg.name,pcg.description")
            ->join("product_config_links pcl", "pcl.pid = p.id")
            ->join("product_config_groups pcg", "pcg.id = pcl.gid")
            ->where("p.id", $pid)
            ->select()
            ->toArray();
        $cart = new \app\common\logic\Cart();
        ...
        foreach ($configgroups as $ckey => $configgroup) {
            if (!empty($configgroup)) {
                ...
                foreach ($options as $okey => $option) {
                    if (!empty($option)) {
                        $cid = $option["id"];
                        $option["option_name"] = explode("|", $option["option_name"])[1]
                            ? explode("|", $option["option_name"])[1]
                            : $option["option_name"];
                        if ($option["option_type"] == 3) {
                            $suboptions = \think\Db::name("product_config_options_sub")
                                ->where("config_id", $cid)
                        ...
                        if (
                            in_array(strtolower($option["option_name"]), $this->allowSystem) ||
                            $option["option_type"] == 5
                        ) {
                            ...
                            foreach ($suboptions as $subkey => $suboption) {
                                if (!empty($suboption)) {
                                    ...
                                    if (!empty($cycle)) {
                                        $pricings = \think\Db::name("pricing")
                                            ->field($cycle . " as fee")
                                            ->field($cart->changeCycleToupfee($cycle) . " as setupfee")
                                            ->where("type", "configoptions")
                                            ->where("relid", $subid)
                                            ->select();
                                    } else {
                                    ...
                                    if (!empty($subprice)) {
                                        ...
                                        $original_name = $suboption["option_name"] = explode(
                                            "|",
                                            $suboption["option_name"],
                                        )[1]
                                            ? explode("|", $suboption["option_name"])[1]
                                            : $suboption["option_name"];
                                        ...
                                        $version = explode("^", $original_name)[1]
                                            ? explode("^", $original_name)[1]
                                            : $original_name;
                            ...
                            if (judgeNoc($option["option_type"])) {
                                foreach ($suboptions as $subkey => $suboption) {
                                    if (!empty($suboption)) {
                                        ...
                                        if (!empty($pricings[0])) {
                                            ...
                                            $option["child"][$suboption["option_name"]]["country_code"] =
                                                $suboption["country_code"];
                                ...
                                foreach ($suboptions as $subkey => $suboption) {
                                    if (!empty($suboption)) {
                                        $subid = $suboption["id"];
                                        $suboption["option_name"] = explode("|", $suboption["option_name"])[1]
                                            ? explode("|", $suboption["option_name"])[1]
                                            : $suboption["option_name"];
                                        if (!empty($cycle)) {
                                            $pricings = \think\Db::name("pricing")
    ...
    public function clearCart(\think\Request $request)
    {
        ...
        if ($request->is_api == 1) {
            ...
            $downstream_create = \think\Db::name("host")
                ...
                ->find();
            if (!empty($downstream_create)) {
                $orders = \think\Db::name("orders")
                    ->field("status,invoiceid")
                    ->where("id", $downstream_create["orderid"])
                    ->find();
                if (!empty($orders["invoiceid"])) {
                    ...
                    if ($invoice_status == "Paid") {
                        return json([
                            "status" => 400,
                            "msg" => "该订单已开通,请勿重新开通",
                            "hostid" => $downstream_create["id"],
                            "domainstatus" => $downstream_create["domainstatus"],
                        ]);
                    } else {
                        return json([
                            "status" => 200,
    ...
    public function getDeveloperAppDetail()
    {
        ...
        $product = \think\Db::name("products")
            ->field(
                "name,info,type,description,instruction,icon,pay_type,retired,app_status,uuid,unretired_time,p_uid,app_type,version_description,app_version as version,app_score,app_images,app_file,app_pay_type",
            )
            ->where("id", $id)
            ->where("p_uid", ">", 0)
            ->find();
        ...
        if (
            !empty($product["app_file"]) &&
            file_exists(CMF_ROOT . "/public/upload/common/application/" . $product["app_file"])
        ) {
        ...
        if ($this->request->uid) {
            $evaluation_like = \think\Db::name("evaluation_like")
                ->where("uid", $this->request->uid)
                ->select()
                ->toArray();
            $evaluation_like = array_column($evaluation_like, "eid");
            $product["my_evaluation"] = \think\Db::name("evaluation")
                ->alias("a")
                ->field("b.id as uid,a.id,b.username,a.content,a.score,a.like_num,a.create_time")
                ->leftJoin("clients b", "a.uid = b.id")
                ->where("a.eid", 0)
                ->where("a.rid", $id)
                ->where("a.type", "products")
                ->where("a.status", 1)
            ...
            if (!empty($product["my_evaluation"]["id"])) {
                ...
                $product["my_evaluation"]["reply"] = \think\Db::name("evaluation")
                    ->alias("a")
                    ->field("b.id as uid,a.id,a.aid,b.username,a.content,a.create_time")
                    ->leftJoin("clients b", "a.uid = b.id")
                    ->where("a.eid", $product["my_evaluation"]["id"])
                    ->where("a.status", 1)
            ...
            $app_favorite = \think\Db::name("app_favorite")
                ->where("pid", $id)
                ->where("uid", $this->request->uid)
                ->find();
        ...
        $url = request()->domain() . config("app_file_url");
        $product["icon"] = isset($product["icon"][0])
            ? array_map(function ($v) use ($url) {
                return $url . $v;
            }, explode(",", $product["icon"]))
            : [];
        $product["app_images"] = isset($product["app_images"][0])
            ? array_map(function ($v) use ($url) {
                return $url . $v;
            }, explode(",", $product["app_images"]))
            : [];
        $developer = \think\Db::name("developer")->field("name,desc")->where("uid", $uid)->find();
        $relation_app = \think\Db::name("products")
            ->alias("a")
            ->field(
                "b.id as uid,a.id,a.uuid,c.name as nickname,a.name,a.type,a.info,a.description,a.pay_type,a.icon,a.app_score,a.app_type,a.version_description,a.app_version,a.unretired_time,b.currency",
            )
            ->leftJoin("clients b", "a.p_uid = b.id")
            ->leftJoin("developer c", "b.id = c.uid")
            ->where("a.p_uid", $uid)
            ->where("a.app_status", 1)
            ->where("a.retired", 0)
            ->where("a.hidden", 0)
            ->where("a.type", $product["type"])
            ->withAttr("icon", function ($value) use ($url) {
                $icon = explode(",", $value);
                foreach ($icon as &$vv) {
                    $vv = $url . $vv;
        ...
        $other_app = \think\Db::name("products")
            ->alias("a")
            ->field(
                "b.id as uid,a.id,a.uuid,c.name as nickname,a.name,a.type,a.info,a.description,a.pay_type,a.icon,a.app_score,a.app_type,a.version_description,a.app_version,a.unretired_time,b.currency",
            )
            ...
            ->where("a.retired", 0)
            ->where("a.hidden", 0)
            ->where("a.app_type", $product["app_type"])
            ->where("a.type", $product["type"])
            ->where("a.app_pay_type", $product["app_pay_type"])
            ->withAttr("icon", function ($value) use ($url) {
                $icon = explode(",", $value);
                foreach ($icon as &$vv) {
                    $vv = $url . $vv;
        ...
        $product["app_version"] = \think\Db::name("app_version")
            ->where("pid", $id)
            ->order("version", "desc")
            ->select()
            ->toArray();
        $product["update_time"] = $product["app_version"][0]["create_time"] ?? $product["unretired_time"];
        $product["evaluation"] = \think\Db::name("evaluation")
            ->alias("a")
            ->field("b.id as uid,a.id,b.username,a.content,a.score,a.like_num,a.create_time")
            ->leftJoin("clients b", "a.uid = b.id")
            ->where("a.eid", 0)
        ...
        foreach ($product["evaluation"] as &$v) {
            ...
            $v["reply"] = \think\Db::name("evaluation")
                ->alias("a")
                ->field("b.id as uid,a.id,a.aid,b.username,a.content,a.create_time")
                ->leftJoin("clients b", "a.uid = b.id")
                ->where("a.eid", $v["id"])
        ...
        $product["evaluation_count"] = \think\Db::name("evaluation")
            ->where("eid", 0)
            ->where("rid", $id)
            ->where("type", "products")
            ->where("status", 1)
            ->count();
        $product["purchases"] = \think\Db::name("host")
            ->field("id")
            ->where("productid", $id)
            ->where("domainstatus", "Active")
            ->count();
        $product["scores"]["one"] = \think\Db::name("evaluation")
            ->where("score like '1%' AND rid='{$id}' AND type='products' AND eid=0 AND status=1")
            ->count();
        $product["scores"]["two"] = \think\Db::name("evaluation")
            ->where("score like '2%' AND rid='{$id}' AND type='products' AND eid=0 AND status=1")
            ->count();
        $product["scores"]["three"] = \think\Db::name("evaluation")
            ->where("score like '3%' AND rid='{$id}' AND type='products' AND eid=0 AND status=1")
            ->count();
        $product["scores"]["four"] = \think\Db::name("evaluation")
            ->where("score like '4%' AND rid='{$id}' AND type='products' AND eid=0 AND status=1")
            ->count();
        $product["scores"]["five"] = \think\Db::name("evaluation")
            ->where("score like '5%' AND rid='{$id}' AND type='products' AND eid=0 AND status=1")
            ->count();
        $data = [
            "product" => $product ?? [],
            "developer" => $developer ?? [],
            "product_type" => config("developer_app_product_type"),
            "currency" => getUserCurrency($uid),
            "other_app" => $other_app ?? [],
            "relation_app" => $relation_app ?? [],
        ];
        return jsons(["status" => 200, "msg" => lang("SUCCESS MESSAGE"), "data" => $data]);
    }
    public function indexAppHome()
    {
        ...
        $url = request()->domain() . config("app_file_url");
        $count = \think\Db::name("products")
            ->field("id,info,name,description,pay_type,icon,version_description,app_version")
            ->where("gid", $gid)
            ->where("hidden", 0)
            ->where("retired", 0)
            ->where("p_uid", ">", 0)
    ...
    public function getMarketApp($type = "", $app_type = "")
    {
        ...
        if (empty($app_type)) {
            $hot = \think\Db::name("products")
                ...
                ->where("a.retired", 0)
                ->where("a.hidden", 0)
                ->where(function (\think\db\Query $query) use ($type) {
                    if (!empty($type)) {
            ...
            $recommend = \think\Db::name("products")
                ->alias("a")
                ->field(
                    "b.id as uid,a.id,a.uuid,c.name as nickname,a.name,a.type,a.info,a.description,a.pay_type,a.icon,a.app_score,a.app_type,a.version_description,a.app_version,a.unretired_time,b.currency",
                )
                ...
                ->where("a.retired", 0)
                ->where("a.hidden", 0)
                ->where("a.app_recommend_status", 1)
                ->where(function (\think\db\Query $query) use ($type) {
                    if (!empty($type)) {
                        $query->where("a.type", $type);
                    }
                ...
                ->order("a.app_recommend_order", "asc")
                ->page(1)
                ->limit(8)
                ->select()
                ->toArray();
            ...
            $gateways = \think\Db::name("products")
                ->alias("a")
                ->field(
                    "b.id as uid,a.id,a.uuid,c.name as nickname,a.name,a.type,a.info,a.description,a.pay_type,a.icon,a.app_score,a.app_type,a.version_description,a.app_version,a.unretired_time,b.currency",
                )
            ...
            $templates = \think\Db::name("products")
                ->alias("a")
                ->field(
                    "b.id as uid,a.id,a.uuid,c.name as nickname,a.name,a.type,a.info,a.description,a.pay_type,a.icon,a.app_score,a.app_type,a.version_description,a.app_version,a.unretired_time,b.currency",
                )
            ...
            if ($app_type == "hot") {
                $count = \think\Db::name("products")
                    ...
                    ->where(function (\think\db\Query $query) use ($keywords) {
                        if (!empty($keywords)) {
                            $query
                                ->where("a.info", "like", "%{$keywords}%")
                                ->whereOr("a.name", "like", "%{$keywords}%");
                        }
                    })
                    ->count();
                $products = \think\Db::name("products")
    ...
    public function appEvaluation($id)
    {
        ...
        $evaluation = \think\Db::name("evaluation")
            ->alias("a")
            ->field("b.id as uid,a.id,b.username,a.content,a.score,a.like_num,a.create_time")
            ->leftJoin("clients b", "a.uid = b.id")
            ->where("a.eid", 0)
            ...
            ->where("a.type", "products")
            ->where("a.status", 1)
            ->where(function (\think\db\Query $query) use ($score) {
                if (!empty($score)) {
        ...
        foreach ($evaluation as $key => &$value) {
            ...
            $value["reply"] = \think\Db::name("evaluation")
                ->alias("a")
                ->field("b.id as uid,a.id,a.aid,b.username,a.content,a.create_time")
                ->leftJoin("clients b", "a.uid = b.id")
                ->where("a.eid", $value["id"])
                ->where("a.status", 1)
                ->order("a.create_time", "asc")
                ->select()
                ->toArray();
    ...
    {
        [$currency, $pid, $confs] = $param;
        $data = [];
        $conf = $confs["configoptions"];
        $options = \think\Db::name("product_config_options")
            ->whereIn("id", array_keys($conf))
            ->where("hidden", 0)
            ->select()
            ->toArray();
        foreach ($options as $key => $val) {
            ...
            $suboptions = \think\Db::name("product_config_options_sub")
                ...
                ->where("id", $conf[$val["id"]])
                ->where("hidden", 0)
                ->find();
            $config_show = [
                "option_type" => $val["option_type"],
                "name" => explode("|", $val["option_name"])[1]
                    ? explode("|", $val["option_name"])[1]
                    : $val["option_name"],
            ];
            if ($val["option_type"] == 3) {
                ...
                if (judgeQuantity($val["option_type"])) {
                    ...
                    $config_show["sub_name"] = explode("|", $suboptions["option_name"])[1]
                        ? explode("|", $suboptions["option_name"])[1]
                        : $suboptions["option_name"];
    ...
    public function proList()
    {
        ...
        $fgs = \think\Db::name("product_first_groups")
            ->field("id,name")
            ->where("hidden", 0)
            ->order("order", "asc")
            ->order("id", "asc")
            ...
            ->toArray();
        foreach ($fgs as &$fg) {
            $gs = \think\Db::name("product_groups")
                ->field("id,name,headline,tagline,order,gid,order_frm_tpl,tpl_type")
                ->where("hidden", 0)
                ->where("order_frm_tpl", "<>", "uuid")
                ->where(function (\think\db\Query $query) use ($fg) {
