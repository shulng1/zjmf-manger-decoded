...
class TestController extends \think\Controller
{
    ...
    public function processPaidInvoice($invoiceid, $noemail = "", $date = "")
    {
        ...
        foreach ($invoice_item_data as $key => $val) {
            ...
            if ($type == "host") {
                $info = Db::name("host")
                    ->field("h.id,h.billingcycle,h.domainstatus,h.nextduedate,p.pay_type,p.auto_setup")
                    ->alias("h")
                ...
                Db::name("host")
                    ->where("id", $relid)
                    ->update(["regdate" => time(), "nextduedate" => $nextduedate, "nextinvoicedate" => $nextduedate]);
                if ($auto_setup == "payment") {
                    ...
                    if ($result["status"] == 200) {
                        ...
                        $data_i[
                            "description"
                        ] = " 支付成功后 - 开通 Host ID:{$data_i["host_id"]}的产品失败。原因:{$result["msg"]}";
                        if ($is_zjmf) {
                ...
                $renew_cycle_data = Db::name("renew_cycle")
                ...
                if ($renew_cycle_data) {
                    ...
                    Db::name("host")
                        ->where("id", $relid)
                        ->update(["billingcycle" => $new_cycle, "amount" => $amount]);
                    Db::name("renew_cycle")
                        ->where("id", $renew_cycle_data["id"])
                        ->update(["status" => "Completed", "paid" => "Y"]);
                    $description =
                        "产品周期从" .
                        config("billing_cycle")[$host_data["billingcycle"]] .
                        "更改到" .
                        config("billing_cycle")[$new_cycle] .
                        ",host ID:" .
                        $host_data["id"];
                ...
                $info = Db::name("host")
                    ->field("h.id,h.billingcycle,h.domainstatus,h.nextduedate,p.pay_type")
                    ->alias("h")
                    ->leftJoin("products p", "p.id=h.productid")
                    ->where("h.id", $relid)
                    ->find();
