...
class VoucherController extends CommonController
{
    ...
    public function getVoucherList()
    {
        ...
        $voucher = \think\Db::name("voucher")
            ->alias("a")
            ->field(
                "a.id,a.invoice_id,a.create_time,b.title,b.issue_type,b.issue_type as issue_type_zh,e.subtotal as amount,a.status,a.status as status_zh,c.province,c.city,c.region,c.detail,d.name,a.notes",
            )
            ->leftJoin("voucher_type b", "a.type_id = b.id")
            ->leftJoin("voucher_post c", "a.post_id = c.id")
            ->leftJoin("express d", "a.express_id = d.id")
            ->leftJoin("invoices e", "a.invoice_id = e.id")
            ->where("a.uid", $uid)
        ...
        foreach ($voucher as &$v) {
            ...
            $invoices_subtotal = \think\Db::name("invoices")
    ...
    public function getVoucherDetail()
    {
        ...
        $voucher = \think\Db::name("voucher")
            ->alias("a")
            ->field(
                "a.id,a.create_time,b.title,b.issue_type,b.issue_type as issue_type_zh,a.amount,a.status,a.status as status_zh,c.province,c.city,c.region,c.detail,d.name,a.notes,d.price",
            )
            ->leftJoin("voucher_type b", "a.type_id = b.id")
            ->leftJoin("voucher_post c", "a.post_id = c.id")
            ->leftJoin("express d", "a.express_id = d.id")
        ...
        $invoices = \think\Db::name("invoices")
            ->field("id,tax as taxed,subtotal,subtotal as taxed_amount")
            ->withAttr("taxed", function ($value, $data) use ($taxed) {
                return $taxed . "%";
            })
    ...
    public function getVoucherRequest()
    {
        ...
        $where = function (\think\db\Query $query) use ($uid, $type, $keywords) {
            $query
                ...
                ->where("delete_time", 0)
                ->where("status", "Paid")
                ->whereIn("type", array_keys($type));
            if ($keywords) {
                $query->where("id", $keywords);
            }
        };
        $invoices = \think\Db::name("invoices")
            ->field("id,subtotal,type,type as type_zh,paid_time")
            ->withAttr("type_zh", function ($value, $data) use ($type) {
                return $type[$value];
            })
            ->where($where)
            ->order($order, $sort)
        ...
        $i = 0;
        foreach ($invoices as $invoice) {
            $voucher_ids = \think\Db::name("voucher_invoices")
                ->where("invoice_id", $invoice["id"])
                ->column("voucher_id");
    ...
    public function getIssueVoucher()
    {
        ...
        $post = \think\Db::name("voucher_post")
            ->field("id,province,city,region,default")
            ->where("uid", $uid)
            ->select()
            ->toArray();
        ...
        $taxed = configuration("voucher_rate") > 0 ? floatval(configuration("voucher_rate")) : 0;
        $invoices = \think\Db::name("invoices")
            ->field("id,tax as taxed,subtotal,subtotal as taxed_amount,type")
            ->withAttr("taxed", function ($value, $data) use ($taxed) {
                return $taxed . "%";
            })
            ->withAttr("taxed_amount", function ($value, $data) use ($taxed) {
        ...
        $data = [
            ...
            "title" => $title_filter,
            "invoices" => $invoices,
            "voucher_amount" => bcsub($voucher_amount, 0, 2),
        ];
        return jsons(["status" => 200, "msg" => lang("SUCCESS MESSAGE"), "data" => $data]);
    }
    /**
    ...
    public function getVoucherInfoList()
    {
        ...
        $order = !empty($params["order"]) ? trim($params["order"]) : "id";
        $sort = !empty($params["sort"]) ? trim($params["sort"]) : "DESC";
        $voucher_type = \think\Db::name("voucher_type")
            ->field(
                "id,title,issue_type,issue_type as issue_type_zh,voucher_type,voucher_type as voucher_type_zh,tax_id",
            )
            ->where("uid", $uid)
            ->withAttr("issue_type_zh", function ($value, $data) {
                return $this->type[$value];
    ...
    public function getVoucherInfo()
    {
        ...
        if (isset($param["id"])) {
            $id = intval($param["id"]);
            $tmp = \think\Db::name("voucher_type")
                ->field("id,title,issue_type,voucher_type,tax_id,bank,account,address,phone")
                ->where("id", $id)
                ->find();
            if (empty($tmp)) {
    ...
    public function getVoucherPostList()
    {
        ...
        $order = !empty($params["order"]) ? trim($params["order"]) : "id";
        $sort = !empty($params["sort"]) ? trim($params["sort"]) : "DESC";
        $voucher_post = \think\Db::name("voucher_post")
            ->field("id,username,phone,province,city,region,detail,post,default")
            ->where("uid", $uid)
            ->order("default", "desc")
            ->order($order, $sort)
            ->page($page)
    ...
    public function getVoucherPost()
    {
        ...
        if (isset($param["id"])) {
            $id = intval($param["id"]);
            $tmp = \think\Db::name("voucher_post")
                ->field("id,username,province,city,region,detail,post,default,phone")
                ->where("uid", $uid)
                ->where("id", $id)
                ->find();
            if (empty($tmp)) {
    ...
    public function postVoucherPost()
    {
        ...
        $default = \think\Db::name("voucher_post")
            ->where(function (\think\db\Query $query) use ($id) {
                if ($id) {
                    $query->where("id", "<>", $id);
                }
            })
        ...
        if ($res) {
            if (!empty($default) && $param["default"] == 1) {
                \think\Db::name("voucher_post")
                    ->where("id", $default["id"])
                    ->update(["default" => 0, "update_time" => time()]);
            }
            return jsons(["status" => 200, "msg" => lang("SUCCESS MESSAGE")]);
        } else {
            return jsons(["status" => 400, "msg" => lang("FAIL MESSAGE")]);
    ...
    public function deleteVoucherPost()
    {
        ...
        if (empty($default)) {
            ...
            \think\Db::name("voucher_post")
                ...
                ->update(["default" => 1, "update_time" => time()]);
        }
        return jsons(["status" => 200, "msg" => lang("DELETE SUCCESS")]);
    }
    /**
    ...
    public function postVoucherDefaultPost()
    {
        ...
        $exist = \think\Db::name("voucher_post")
            ->where("id", "<>", $id)
            ->where("default", 1)
            ->where("uid", $uid)
            ->find();
