...
class OauthController extends CommonController
{
    ...
    public function listing()
    {
        ...
        $list = \think\Db::name("plugin")
            ->where(["module" => $this->modules, "status" => 1])
            ->order("order", "asc")
            ->select();
    ...
    public function url(\think\Request $request)
    {
        ...
        $pluginDb = \think\Db::name("plugin")
            ->where(["module" => $this->modules, "name" => $request->dirName])
            ->field("config")
            ->find();
        if (!$pluginDb["config"]) {
        }
    ...
    public function callback(\think\Request $request)
    {
        ...
        $clients_oauth = \think\Db::name("clients_oauth")
            ->where(["type" => $request->dirName, "openid" => $userinfo["openid"]])
            ->field("uid")
            ->find();
        ...
        if ($userStatusinfo["id"]) {
            $bind = [
                "type" => $request->dirName,
                "openid" => $userinfo["openid"],
                "oauth" => $userinfo["data"],
                "uid" => $userStatusinfo["id"],
            ];
            ...
            if ($clients_oauth["uid"]) {
                ...
                if ($jwt["status"] !== 200) {
                    ...
                    $idata = [
                        ...
                        "usertype" => "Client",
                        "uid" => $clients_oauth["uid"],
                        "ipaddr" => get_client_ip6(),
                        "type" => 1,
                        "activeid" => $clients_oauth["uid"],
    ...
    public function bindLoginEmail(\think\Request $request)
    {
        if ($request->isPost()) {
            ...
            $params = [
                "type" => $_SESSION["oauth"]["dirName"],
                "openid" => $_SESSION["oauth"]["openid"],
                "oauth" => $_SESSION["oauth"]["new_data"],
            ];
            if ($clients["id"] > 0) {
                $user["id"] = $clients["id"];
                ...
                $ret = $sms->sendSms(
                    39,
                    $clients["phone_code"] . $clients["phonenumber"],
                    $params,
                    false,
                    $clients["id"],
                );
	...
	*/
    ...
    public function bindEmailSend(\think\Request $request)
    {
        ...
        $data = $request->param();
        if (
            !captcha_check($data["captcha"], "allow_register_email_captcha") &&
            configuration("allow_register_email_captcha") == 1 &&
    ...
    public function bindPhoneSend(\think\Request $request)
    {
        ...
        $data = $request->param();
        if (
            !captcha_check($data["captcha"], "allow_register_phone_captcha") &&
            configuration("allow_register_phone_captcha") == 1 &&
            configuration("is_captcha") == 1
        ) {
            return json(["status" => 400, "msg" => "图形验证码有误"]);
        }
