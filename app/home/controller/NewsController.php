...
class NewsController extends \cmf\controller\HomeBaseController
{
        ...
        if (isset($param["parent_id"]) && $param["parent_id"] > 0) {
            ...
            if (isset($tmp["id"]) && $tmp["parent_id"] == 0) {
                $tmp_list = \think\Db::name("news_type")
                    ->where(["hidden" => 0, "parent_id" => $tmp["id"]])
                    ->select()
                    ->toArray();
    ...
    {
        $list = \think\Db::name("news_type")
            ->where(["parent_id" => 2, "hidden" => 0])
            ->select()
            ->toArray();
        foreach ($list as &$v) {
            $v["list"] = \think\Db::name("news_menu")
    ...
    public function getContent(\think\Request $request)
    {
        ...
        if (isset($parent["parent_id"]) && $parent["parent_id"] == 1) {
            $tmp = \think\Db::name("news_type")
                ->where([["parent_id", "=", 1], ["hidden", "=", 0]])
                ->select();
            if (isset($tmp[0]["id"])) {
        ...
        $returndata["next"] = \think\Db::name("news_menu")
            ->where(array_merge($where, [["push_time", ">", $new_content["push_time"]]]))
            ->order("push_time", "ASC")
            ->find();
        $returndata["prev"] = \think\Db::name("news_menu")
            ...
            ->order("push_time", "DESC")
            ->find();
        $returndata["relevant"] = \think\Db::name("news_menu")
            ->where(array_merge($where, [["id", "<>", $new_content["id"]]]))
            ->order("push_time", "DESC")
            ->limit(3)
            ->page(1)
            ->select()
