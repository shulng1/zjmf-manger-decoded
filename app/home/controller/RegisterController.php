...
class RegisterController extends \cmf\controller\HomeBaseController
{
    ...
    public function registerPhone()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $validate->message([
                ...
                "companyname.max" => "公司名不超过20个字符",
                "address1.max" => "地址不超过20个字符",
            ]);
            $data = $this->request->param();
    ...
    public function resetPhoneSend()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $data = $this->request->param();
            if (
                !captcha_check($data["captcha"], "allow_phone_forgetpwd_captcha") &&
                configuration("allow_phone_forgetpwd_captcha") == 1 &&
                configuration("is_captcha") == 1
            ) {
                return json(["status" => 400, "msg" => "图形验证码有误"]);
            }
    ...
    public function resetEmailSend()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $data = $this->request->param();
            if (
                !captcha_check($data["captcha"], "allow_email_forgetpwd_captcha") &&
                configuration("allow_email_forgetpwd_captcha") == 1 &&
                configuration("is_captcha") == 1
            ) {
                return json(["status" => 400, "msg" => "图形验证码有误"]);
            }
