...
class CreditLimitController extends \cmf\controller\HomeBaseController
{
    ...
    public function index()
    {
        ...
        $user["is_open_credit_limit"] =
            configuration("shd_credit_limit") == 1
    ...
    public function creditLimitInvoice()
    {
        ...
        $detail = \think\Db::name("invoices")
            ->alias("a")
            ->field(
                "b.username,b.companyname,a.status,a.status as status_zh,a.paid_time,a.payment,a.payment as payment_zh,a.subtotal,a.total,a.credit,a.id",
            )
    ...
    public function creditLimitUsed()
    {
        ...
        $uid = request()->uid;
        $invoices1 = \think\Db::name("invoices")
            ->field("id")
            ->where("status", "Paid")
            ->where("use_credit_limit", 1)
            ->where("invoice_id", 0)
        ...
        $invoices = \think\Db::name("invoices")
            ...
            ->whereIN("i.invoice_id", $invoice_id)
            ->where("i.is_delete", 0)
            ->withAttr("payment", function ($value, $data) use ($gateways) {
                if ($data["status"] == "Paid") {
                    if ($data["use_credit_limit"] == 1) {
    ...
    public function creditLimitPrepayment()
    {
        ...
        $data = [
            "uid" => $uid,
            "create_time" => time(),
            "due_time" => time() + $clients["bill_repayment_period"] * 24 * 3600,
            "subtotal" => $amount,
            "total" => $amount,
            "status" => "Unpaid",
            ...
            "notes" => "信用额提前还款",
            "url" => "credit",
            "credit_limit_prepayment" => 1,
            "credit_limit_prepayment_invoices" => json_encode($invoices),
        ];
        $data2 = [
            "uid" => $uid,
            "type" => "credit_limit",
