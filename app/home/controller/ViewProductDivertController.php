...
class ViewProductDivertController extends ViewBaseController
{
    ...
    public function pushserver(\think\Request $request)
    {
        ...
        $product = \think\Db::name("host")
            ...
            ->where(["h.id" => $host_id, "h.uid" => $param["uid"]])
            ->find();
        if (!in_array($product["id"], $system["product_range"])) {
            $this->error("该服务不支持转移");
        }
        $product_divert = \think\Db::name("product_divert")
            ->field("id")
            ->where(["hostid" => $host_id, "status" => 1])
            ->find();
        if ($product_divert) {
    ...
    public function pushpulllist(\think\Request $request)
    {
        ...
        $product_divert = \think\Db::name("product_divert")
            ->field(
                "id,push_invoice_id,pull_invoice_id,product_name,product_domain,product_ip,push_userid,pull_userid,push_username,pull_username,push_cost,pull_cost,status",
            )
            ->whereOr(["push_userid" => $param["uid"], "pull_userid" => $param["uid"]])
        ...
        foreach ($product_divert as $k => $v) {
            if ($v["push_invoice_id"]) {
                $product_divert[$k]["push_pay_status"] = \think\Db::name("invoices")
                    ->where(["id" => $v["push_invoice_id"]])
                    ->value("status");
            }
            if ($v["pull_invoice_id"]) {
                $product_divert[$k]["pull_pay_status"] = \think\Db::name("invoices")
                    ->where(["id" => $v["pull_invoice_id"]])
                    ->value("status");
            }
        }
    ...
    public function pullrefuse(\think\Request $request)
    {
        ...
        \think\Db::startTrans();
        try {
            $invoice = \think\Db::name("product_divert")
                ->alias("p")
                ->leftJoin("invoices i", "p.push_invoice_id=i.id")
                ->where(["p.id" => $param["id"], "p.pull_userid" => $param["uid"]])
                ->field("i.status as pay_status,i.subtotal,p.push_invoice_id,p.push_userid")
                ->find();
            if ($invoice["pay_status"] == "Unpaid") {
                \think\Db::name("product_divert")
                    ->where(["id" => $param["id"], "pull_userid" => $param["uid"]])
                    ->update(["status" => 4]);
                \think\Db::name("invoices")
                    ->where(["id" => $invoice["push_invoice_id"]])
            ...
            if ($invoice["pay_status"] == "Paid") {
                ...
                \think\Db::name("clients")
                    ->where(["id" => $invoice["push_userid"]])
                    ->setInc("credit", $invoice["subtotal"]);
                \think\Db::commit();
            }
        } catch (\Exception $e) {
            \think\Db::rollback();
    ...
    {
        $param = $request->param();
        $divert = \think\Db::name("product_divert")
            ->where(["id" => $param["id"], "push_userid" => $param["uid"], "status" => 1])
            ->find();
        if (!$divert) {
        ...
        try {
            \think\Db::name("product_divert")
                ->where(["id" => $param["id"], "push_userid" => $param["uid"]])
                ->update(["status" => 3]);
            \think\Db::name("invoices")
