...
class IndexController extends CommonController
{
    ...
    public function index()
    {
        ...
        $client = \think\Db::name("clients")
            ->field("username,phonenumber,credit,marketing_emails_opt_in")
            ->where("id", $uid)
            ->find();
        $client["credit"] = $client["credit"] . $suffix;
        $data["client"] = $client;
        $ticket_count = \think\Db::name("ticket")
            ->where("uid", $uid)
            ->where("status", ["<>", 1], ["<>", 4])
            ->count();
        $data["ticket_count"] = $ticket_count;
        $order_count = \think\Db::name("invoices")
            ->where("uid", $uid)
            ->where("delete_time", 0)
            ->where("is_delete", 0)
            ->where("status", "Unpaid")
        ...
        $accounts = \think\Db::name("accounts")
            ...
            ->whereBetweenTime("create_time", $this_month_start, $this_month_end)
            ->where("uid", $uid)
            ->select()
            ->toArray();
        ...
        $invoices_unpaid = \think\Db::name("invoices")
            ->where("status", "Unpaid")
            ->where("uid", $uid)
            ->where("delete_time", 0)
            ->where("is_delete", 0)
        ...
        $datan = [];
        $pid = \think\Db::name("host")
            ->where("uid", $uid)
            ->whereIn("domainstatus", ["Active", "Verifiy_Active", "Issue_Active"])
            ->column("productid");
        ...
        $news_menu = \think\Db::name("news_type")
            ->alias("nt")
            ->join("news_menu nm", "nm.parent_id=nt.id")
    ...
    public function common_list()
    {
        ...
        $ret = [
            "logo_url" => $logo_url,
            "main_tenance_mode" => configuration("main_tenance_mode"),
            "main_tenance_mode_message" => configuration("main_tenance_mode_message"),
            "main_tenance_mode_url" => configuration("main_tenance_mode_url"),
            "allow_user_language" => configuration("allow_user_language") ?? 0,
            "language" => configuration("language") ?? "zh-hk",
            "company_name" => configuration("company_name") ?? "顺戴财务",
            "domain" => configuration("domain") ?? cmf_get_domain(),
            "system_url" => configuration("system_url") ?? cmf_get_domain(),
            "logo_url_home" => $logo_url_home ?? "/img/logo2.png",
            "company_email" => configuration("company_email") ?? "",
            "certifi_open" => configuration("certifi_open") ?? 1,
            "main_phone" => configuration("main_phone") ?? "",
            "main_address" => configuration("main_address") ?? "",
            "record_no" => configuration("record_no") ?? 1,
            "map" => configuration("map") ?? "1,1",
            "company_profile" => configuration("company_profile") ?? "",
            "msfntk" => md5(time() + rand(0, 9999)),
            "is_themes" => configuration("is_themes"),
            "themes_templates" => configuration("themes_templates"),
            "login_header_footer" => $login_header_footer,
            "is_profession" => $is_profession,
            "order_page_style" => \intval(configuration("order_page_style")),
            "custom_login_background_img" => configuration("custom_login_background_img") ?? "",
            "custom_login_background_char" => configuration("custom_login_background_char") ?? "",
            "custom_login_background_description" => configuration("custom_login_background_description") ?? "",
            "enable_file_download" => configuration("enable_file_download") ?? 0,
        ];
        if ($login_header_footer) {
            $ret["login_header"] = configuration("login_header")
                ? htmlspecialchars_decode(configuration("login_header"), ENT_QUOTES)
                : "";
            $ret["login_footer"] = configuration("login_footer")
                ? htmlspecialchars_decode(configuration("login_footer"), ENT_QUOTES)
                : "";
        }
    ...
    {
        $config_data = \think\Db::name("friendly_links")
            ->where("is_open", 1)
