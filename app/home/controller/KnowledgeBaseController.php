...
class KnowledgeBaseController extends CommonController
{
    ...
    public function index()
    {
        ...
        $categories = \think\Db::name("knowledge_base_links")
            ->alias("kbl")
            ->field("kbc.id,name,description,count(kbl.category_id) as num")
        ...
        $article = \think\Db::name("knowledge_base_links")
            ->alias("kbl")
            ->field("kb.id,title,article,views,useful,public_by,public_time")
            ->leftJoin("knowledge_base kb", "kbl.article_id = kb.id")
            ->leftJoin("knowledge_base_cats kbc", "kbl.category_id = kbc.id")
            ->where("kbc.hidden", 0)
            ->where("kb.hidden", 0)
            ->where("kbl.category_id", $cid)
    ...
    public function searchArticle()
    {
        if ($this->request->isPost()) {
            ...
            $result = \think\Db::name("knowledge_base")
                ->field("id,title,article,views,useful,public_by,public_time")
                ->where("title|article", "like", "%{$keyword}%")
                ->where("hidden", 0)
                ->where(function (\think\db\Query $query) {
                    $uid = request()->uid;
                    if (!$uid) {
