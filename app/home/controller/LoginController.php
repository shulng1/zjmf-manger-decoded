...
class LoginController extends \cmf\controller\HomeBaseController
{
    ...
    public function LoginRegisterIndex()
    {
        ...
        $data["fields"] = $fields;
        $data["login_register_custom_require"] = configuration("login_register_custom_require")
    ...
    public function mobileSend()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (
                !captcha_check($data["captcha"], "allow_login_code_captcha") &&
                configuration("allow_login_code_captcha") == 1 &&
    ...
    public function phonePassLogin()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (
                !captcha_check($data["captcha"], "allow_login_phone_captcha") &&
                configuration("allow_login_phone_captcha") == 1 &&
                configuration("is_captcha") == 1
            ) {
                return json(["status" => 400, "msg" => "图形验证码有误"]);
            }
    ...
    public function emailLogin()
    {
        if ($this->request->isPost()) {
            ...
            if (strpos($email, "@") > 0) {
                ...
                if (
                    !captcha_check($data["captcha"], "allow_login_email_captcha") &&
                    configuration("allow_login_email_captcha") == 1 &&
                    configuration("is_captcha") == 1
                ) {
                    return json(["status" => 400, "msg" => "图形验证码有误"]);
                }
    ...
    public function zjmfApiLogin()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (aesPasswordEncode($password) == $result["api_password"]) {
                ...
                $userinfo["is_api"] = 1;
                active_logs(
                    sprintf($this->lang["User_api_login"], "api鉴权登录", $username, $userinfo["id"]),
                    $userinfo["id"],
                    1,
                );
                active_logs(
    ...
    public function getProuductlistPage()
    {
        ...
        foreach ($group_data as $k => $v) {
            $product_data = \think\Db::name("products")
                ->field("id,name,gid,type,pay_method as type_zh,pay_type,qty,auto_setup,hidden")
                ->where("gid", $v["id"])
                ->where($where)
    ...
    public function getSecondVerifyPage()
    {
        ...
        if (strpos($username, "@") === false) {
            $client = \think\Db::name("clients")
                ->field("phone_code,phonenumber,email,password")
