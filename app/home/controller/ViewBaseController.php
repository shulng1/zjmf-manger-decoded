...
class ViewBaseController extends CommonController
{
    ...
    public function __construct()
    {
        ...
        if (VIEW_TEMPLATE_DIRECTORY === "clientarea") {
            ...
            if (
                ($theme && in_array($theme, $path_arr)) ||
                (cookie("clientarea_theme") && in_array(cookie("clientarea_theme"), $path_arr))
            ) {
                if ($theme) {
    ...
    {
        $url = "/" . request()->action();
        $p = \think\paginator\driver\Bootstrap::make($showdata, $listRow, $curpage, $total, false, [
            "var_page" => "page",
            "path" => $url,
            "fragment" => "",
            "query" => $_GET,
    ...
    public function view($tplName, $data = [], $config = [])
    {
        ...
        if (!empty($custom_lang)) {
            ...
            if (
                file_exists(
                    CMF_ROOT . "public/themes/clientarea/" . $theme_name . "/language/" . $custom_language . ".php",
                )
            ) {
        ...
        if ($view_header_and_footer == "web") {
            ...
            if (!empty($custom_lang)) {
                ...
                if (
                    file_exists(
                        CMF_ROOT . "public/themes/web/" . $webtplname . "/language/" . $custom_language . ".php",
                    )
                ) {
    ...
    public function pushParam(\think\Request $request)
    {
        ...
        $nextduedate3 = \think\Db::name("host")
            ->where("domainstatus", "=", "Active")
            ->whereBetweenTime("nextduedate", time(), time() + 259200)
            ->where("uid", $id)
            ->count();
        $nextduedate7 = \think\Db::name("host")
            ->where("domainstatus", "=", "Active")
            ->whereBetweenTime("nextduedate", time(), time() + 604800)
            ->where("uid", $id)
            ->count();
        $createduedate7 = \think\Db::name("host")
            ->where("domainstatus", "=", "Active")
            ->whereBetweenTime("create_time", time() - 604800, time())
            ->where("uid", $id)
            ->count();
        $createduedate30 = \think\Db::name("host")
            ->where("domainstatus", "=", "Active")
            ->whereBetweenTime("create_time", time() - 2592000, time())
            ->where("uid", $id)
            ->count();
        ...
        $accounts7 = \think\Db::name("accounts")
            ->field("amount_in,amount_out,fees")
            ->whereBetweenTime("create_time", time() - 604800, time())
            ->where("uid", $id)
            ->sum("amount_in");
        $accounts30 = \think\Db::name("accounts")
            ->field("amount_in,amount_out,fees")
            ->whereBetweenTime("create_time", time() - 604800, time())
            ->where("uid", $id)
            ->sum("amount_in");
        ...
        return [
            "aff" => $aff,
            "nextduedate3" => $nextduedate3,
            "nextduedate7" => $nextduedate7,
            "_user" => $_user,
            "user_certtfi" => $certifi_person ? $certifi_person[0] : 0,
            "accounts7" => number_format(round($accounts7, 2), 2),
            "accounts30" => number_format(round($accounts30, 2), 2),
            "accounts" => number_format(round($accounts, 2), 2),
            "createduedate7" => $createduedate7,
            "createduedate30" => $createduedate30,
        ];
    }
}
