...
class ViewCartController extends ViewBaseController
{
    ...
    public function cart(\think\Request $request)
    {
        if (trim($request->alias)) {
            $model = \think\Db::name("product_groups")
                ->field("id")
                ->where("alias", trim($request->alias))
                ->whereOr("id", trim($request->alias))
                ->find();
            if (!$model) {
                abort(404, "页面异常");
            }
    ...
    protected function productFun($request, $cart, $param, $products)
    {
        ...
        $gid_is_true = \think\Db::name("product_groups")
            ->where("id", $param["gid"])
            ->whereNotIn("gid", [0])
            ->find();
        if (!$gid_is_true) {
    ...
    protected function viewCartFun($request, $cart, $param)
    {
            ...
            if (!$param["terms"]) {
                ...
                if ($result["status"] == 200 || request()->uid) {
                    ...
                    if ($settle["status"] == 200) {
                        ...
                        header(
                            "location:{$this->ViewModel->domain}/viewbilling?id=" .
                                $settle["data"]["invoiceid"] .
                                "&wakeup=1",
                        );
                        exit();
                        ...
                        $data["ErrorMsg"] =
                            $settle["msg"] . "<a href=\"" . $this->ViewModel->domain . "/verified\">去认证</a>";
                        if ($settle["msg_phone"]) {
                            $data["ErrorMsg"] .=
                                "<br>" .
                                $settle["msg_phone"] .
                        ...
                        $data["ErrorMsg"] =
                            $settle["msg"] . "<a href=\"" . $this->ViewModel->domain . "/security\">去绑定</a>";
                    } else {
        ...
        if (!request()->uid) {
            ...
            $dataRegister["login_register_custom_require"] =
                $LoginRegisterIndex["data"]["login_register_custom_require"];
            ...
            if (empty($billing_cycle_base)) {
                ...
                if (!empty($billing_cycle_base)) {
                    if ($cycle["billingcycle"] == "day") {
                        $cycle_base_multiple =
                            ($billing_cycle_unit["day"] * $cycle["day_cycle"]) /
                            $billing_cycle_unit[$billing_cycle_base];
                    } else {
                        $cycle_base_multiple =
                            $billing_cycle_unit[$cycle["billingcycle"]] / $billing_cycle_unit[$billing_cycle_base];
                    }
    ...
    public function preferentialPeriod($data)
    {
        ...
        foreach ($list as $key => $val) {
            ...
            if (isset($arr[$list[$key]["billingcycle"]])) {
                $list[$key]["discount"] =
                    $list[$key]["product_price"] / $arr[$list[$key]["billingcycle"]] < $monthPrice ? 1 : 0;
            }
    ...
    public function cartGroupTpl($order_frm_tpl = "", $tpl_type = "")
    {
        ...
        if (!empty($theme)) {
            ...
            if (
                ...
                (cookie("cart_theme") && in_array(cookie("cart_theme"), $path_arr))
            ) {
                if ($theme) {
