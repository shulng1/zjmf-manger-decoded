...
class DcimController extends CommonController
{
    ...
    public function buyFlowPacket(\think\Request $request)
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.productid,a.dcimid,a.serverid,a.bwlimit,b.api_type,b.zjmf_api_id,b.upstream_price_type,b.upstream_price_value,b.name as productname,a.domain,a.dedicatedip",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->where("a.uid", $uid)
            ->where("a.id", $id)
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            $flow_packet = \think\Db::name("dcim_flow_packet")
                ->where("id", $fid)
                ->where("status", 1)
                ->whereRaw("FIND_IN_SET('{$host["productid"]}', allow_products)")
                ->find();
            if (empty($flow_packet)) {
                $result["status"] = 400;
                $result["msg"] = lang("DCIM_GET_UPSTREAM_HEADER_ERROR");
        ...
        $invoice_data = [
            "uid" => $uid,
            "price" => $flow_packet["price"],
            "relid" => $id,
            "description" =>
                "流量包订购，大小：" .
                $flow_packet["capacity"] .
                "Gb;" .
                $host["productname"] .
                "(" .
                $host["domain"] .
                "),IP({$host["dedicatedip"]})",
            "type" => "zjmf_flow_packet",
        ];
        ...
        $data = [
            "uid" => $uid,
            "relid" => $fid,
            "name" => $flow_packet["name"],
            "price" => $flow_packet["price"],
            "status" => 0,
            "create_time" => time(),
            "capacity" => $flow_packet["capacity"],
            "invoiceid" => $invoiceid,
            "type" => "flow_packet",
            "hostid" => $id,
        ];
        ...
        if ($record) {
            active_log_final(
                sprintf(
                    $this->lang["Dcim_home_buyFlowPacket"],
                    $flow_packet["name"],
                    $flow_packet["capacity"],
                    $id,
                    $invoiceid,
            ...
            hook("flow_packet_invoice_create", [
                "invoiceid" => $invoiceid,
                "hostid" => $id,
                "price" => $flow_packet["price"],
                "name" => $flow_packet["name"],
    ...
    public function checkReinstall(\think\Request $request)
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.productid,a.serverid,a.reinstall_info,a.dcimid,b.type,b.api_type,b.zjmf_api_id,b.upstream_price_type,b.upstream_price_value,b.config_option1,c.reinstall_times,c.buy_times,c.reinstall_price,c.auth",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("dcim_servers c", "a.serverid=c.serverid")
            ->where("a.uid", $uid)
    ...
    public function crackPass(\think\Request $request)
    {
        ...
        $check_pass = (new \app\common\logic\Shop($product["uid"]))->checkHostPassword(
            $data["crack_password"],
            $product["productid"],
        );
    ...
    public function refreshPowerStatus(\think\Request $request)
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.id,a.dcimid,c.hostname,c.username,c.password,c.secure,c.port,b.api_type,b.zjmf_api_id")
            ->leftJoin("products b", "a.productid=b.id")
        ...
        if (!empty($host)) {
            ...
            foreach ($host as $v) {
                ...
                $data[$v["id"]] = [
                    "url" => $url . "/index.php?m=api&a=ipmiPowerSync",
                    "data" => [
                        "username" => $v["username"],
                        "password" => aesPasswordDecode($v["password"]) ?? "",
                        "id" => $v["dcimid"],
                    ],
                ];
            }
    ...
    public function novncPage()
    {
        ...
        $data = [
            "url" => $url,
            "password" => $password,
            "host_token" => !empty($host_token) ? aesPasswordDecode($host_token) : "",
            "restart_vnc" => "",
            "id" => input("get.id", 0, "intval"),
        ];
        if (!empty($host_token)) {
    ...
    public function checkAllReinstallStatus(\think\Request $request)
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.id,a.dcimid,c.hostname,c.username,c.password,c.secure,c.port")
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("servers c", "a.serverid=c.id")
            ->where("a.uid", $uid)
            ->whereIn("a.id", $id)
        ...
        if (!empty($host)) {
            ...
            foreach ($host as $v) {
                ...
                $data[$v["id"]] = [
                    "url" => $url . "/index.php?m=api&a=getReinstallStatus",
                    "data" => [
                        "username" => $v["username"],
                        "password" => aesPasswordDecode($v["password"]) ?? "",
                        "id" => $v["dcimid"],
            ...
            foreach ($res as $k => $v) {
                if (
                    $v["data"]["status"] == "success" &&
                    $v["http_code"] == 200 &&
                    !empty($v["data"]["data"]) &&
                    !$v["data"]["data"]["windows_finish"]
                ) {
                    $result["data"][] = $k;
                }
            }
    ...
    public function hideLastResult(\think\Request $request)
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field("a.regdate,a.dcimid,b.api_type,b.zjmf_api_id")
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("dcim_servers c", "a.serverid=c.serverid")
            ->where("a.uid", $uid)
            ->where("a.id", $id)
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            \think\Db::name("host")
                ->where("id", $id)
                ->update(["show_last_act_message" => 0]);
            $result["status"] = 200;
        }
        return json($result);
    }
