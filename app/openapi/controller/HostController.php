...
class HostController extends \cmf\controller\HomeBaseController
{
    public function initialize()
    {
        ...
        if (in_array($action, $_action) && request()->id) {
            ...
            $client = \think\Db::name("clients")
                ->field("phone_code,phonenumber,email,second_verify")
            ...
            if ($client["second_verify"] == 1 && !empty($second_verify_home) && !empty($second_verify_action)) {
                if (empty($verification_success_1) && empty($verification_success_2)) {
                    ...
                    echo json_encode([
                        "status" => 400,
                        "data" => $result,
                        "msg" => "This operation requires secondary verification",
                    ]);
                    exit();
                }
            }
            $is_certifi = \think\Db::name("host")
                ->alias("a")
                ->leftJoin("products b", "a.productid=b.id")
                ->leftJoin("dcim_servers c", "a.serverid=c.serverid")
                ->where("a.uid", request()->uid)
                ->where("a.id", intval(request()->id))
                ->value("c.is_certifi");
    ...
    public function getHosts()
    {
        ...
        if (
            !file_exists(
                CMF_ROOT .
                    ...
                    configuration("clientarea_default_themes") .
                    "/" .
                    $nav_info["templatePage"] .
                    ".tpl",
            )
        ) {
        ...
        $data = \think\Db::name("host")
            ->field(
                "h.id,h.domain,h.initiative_renew,h.domainstatus,h.regdate,h.dedicatedip,h.assignedips,h.nextduedate,h.remark,h.firstpaymentamount,h.amount,h.billingcycle,p.name as product_name,p.type,p.id as product_id",
            )
            ->alias("h")
            ->leftJoin("products p", "p.id=h.productid")
    ...
    public function getHostDetail($id = 0)
    {
        ...
        $host = \think\Db::name("host")
            ->field(
                "h.id,h.uid,h.initiative_renew,h.regdate,h.domain,h.payment,p.groupid group_id,
                ...
                h.bwusage,h.bwlimit,h.os,h.remark,h.dcimid,h.port,p.type,p.name as product_name,p.config_options_upgrade",
            )
            ->alias("h")
            ->leftJoin("products p", "p.id=h.productid")
        ...
        $host["suspend_reason"] = explode("-", $host["suspendreason"])[1]
    ...
    public function getBillLog($id = 0)
    {
        ...
        $accounts = \think\Db::name("accounts")
            ->alias("a")
            ->field("a.trans_id,a.pay_time,a.gateway,a.refund,a.amount_out,ii.type,ii.amount as amount_in")
        ...
        if (!empty($ids[0])) {
            $credit_log_uses = \think\Db::name("credit")
                ...
                ->field(
                    "f.id as trans_id,f.create_time as pay_time,ii.amount as amount_in,f.description as gateway,ii.type",
                )
                ->withAttr("type", function ($value) {
                    if ($value == "renew") {
                        return "续费";
    ...
    public function getHostDownloads($id = 0)
    {
        ...
        $download_data = \think\Db::name("downloads")
            ->field("d.id,d.title name,d.create_time,d.downloads amount,d.productdownload")
            ->alias("d")
            ->leftJoin("product_downloads p", "p.download_id=d.id")
            ->where("p.product_id", $host_exists["productid"])
            ->select()
            ->toArray();
    ...
    public function hostDownloadFile($id = 0, $download = 0)
    {
        ...
        if ($productdownload) {
            $product_download_data = \think\Db::name("product_downloads")
                ->field("id,product_id")
                ->where("download_id", $download)
                ->select()
                ->toArray();
            $need_product = array_column($product_download_data, "product_id");
            if (!empty($need_product)) {
                $exists_data = \think\Db::name("host")
                    ->field("id, domainstatus")
                    ->where("uid", $uid)
                    ->whereIn("productid", $need_product[0])
        ...
        if (file_exists(UPLOAD_PATH_DWN . "support/" . $filename)) {
            ...
            return json([
                "status" => 200,
                "data" => $this->download(UPLOAD_PATH_DWN . "support/" . $filename, explode("^", $filename)[1]),
            ]);
            exit();
    ...
    public function upgradeConfigPage()
    {
        ...
        $hosts = \think\Db::name("host")
            ->alias("h")
            ->field("pco.linkage_pid,pco.linkage_top_pid")
            ->field(
                "pco.option_name as option_name,pco.id as oid,pco.option_type,pcos.option_name as suboption_name,pcos.id as subid,hco.qty,h.billingcycle,h.flag,pri.*,pco.qty_stage,pco.unit",
            )
            ->leftJoin("host_config_options hco", "h.id = hco.relid")
            ->leftJoin("product_config_options pco", "pco.id = hco.configid")
            ->leftJoin("product_config_options_sub pcos", "pcos.id = hco.optionid")
            ->leftJoin("pricing pri", "pri.relid = pcos.id")
            ->where("h.id", $hid)
            ->where("h.uid", $uid)
            ->where("pri.currency", $currencyid)
            ->where("pri.type", "configoptions")
            ->where("pco.upgrade", 1)
            ->select()
            ->toArray();
        ...
        foreach ($hosts as $key => $host) {
            ...
            if ($host["option_type"] != 5 && $host["option_type"] != 12 && $option_name[0] != "system_disk_size") {
                ...
                $h["qty"] = $host["qty"];
                $h["suboption_name"] = explode("|", $host["suboption_name"])[1]
                    ? explode("|", $host["suboption_name"])[1]
                    : $host["suboption_name"];
                $h["suboption_name"] = implode(" ", explode("^", $h["suboption_name"]));
                $h["suboption_name_first"] = explode("|", $host["suboption_name"])[1]
                    ? explode("|", $host["suboption_name"])[0]
                    : $host["suboption_name"];
                ...
                foreach ($allOption as $vv) {
                    if ($vv["id"] == $h["oid"]) {
                        ...
                        if (
                            count($vv["sub"]) > 1 ||
                            judgeQuantity($host["option_type"]) ||
                            judgeYesNo($host["option_type"])
                        ) {
                            $sub = $vv["sub"];
        ...
        if (getEdition()) {
            ...
            if ($links) {
                $hostconfigoptions = \think\Db::name("host")
                    ->alias("h")
                    ->field("hco.qty,hco.configid,hco.optionid,pco.hidden,pco.upgrade")
                    ->leftJoin("host_config_options hco", "h.id = hco.relid")
                    ->leftJoin("product_config_options pco", "pco.id = hco.configid")
                    ->where("h.id", $hid)
                    ->where("h.uid", $uid)
    ...
    public function upgradeConfig()
    {
        try {
            ...
            if (!$upgrade_logic->judgeUpgradeConfigError($hid)) {
                return json([
                    "status" => 400,
                    "msg" => "The current product cannot upgrade or downgrade configurable items",
                ]);
            }
    ...
    public function upgradeConfigCheckout()
    {
        try {
            ...
            if (!$upgrade_logic->judgeUpgradeConfigError($hid)) {
                return json([
                    "status" => 400,
                    "msg" => lang("The current product cannot upgrade or downgrade configurable items"),
                ]);
            }
            $data = cache("upgrade_down_config_" . $hid);
            if (!$data) {
            ...
            if (!empty($configoptions) && is_array($configoptions)) {
                $re = $upgrade_logic->upgradeConfigCommon(
                    $hid,
                    $configoptions,
                    $currencyid,
                    false,
    ...
    public function upgradeHostPage()
    {
        try {
            ...
            $oldhost = \think\Db::name("product_groups")
                ->alias("pg")
                ->field("p.name as host,h.domain,p.description,p.id as pid,h.uid,h.flag")
            ...
            $host = \think\Db::name("products")
                ->alias("p")
                ->field("p.id as pid,p.name as host,p.description")
                ->leftJoin("product_groups pg", "p.gid = pg.id")
                ->select()
                ->toArray();
    ...
    public function reinstallBuy()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.productid,a.dcimid,a.serverid,a.reinstall_info,b.type,b.api_type,b.zjmf_api_id,b.upstream_price_type,b.upstream_price_value,b.config_option1,c.reinstall_times,c.buy_times,c.reinstall_price,c.auth",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("dcim_servers c", "a.serverid=c.serverid")
            ->where("a.uid", $uid)
            ->where("a.id", $host_id)
            ->whereIn("b.type", "dcim,dcimcloud")
        ...
        if ($buy) {
            $invoice_data = [
                "uid" => $uid,
                "price" => $host["reinstall_price"],
                "relid" => $host_id,
                "description" => "购买重装次数",
                "type" => "zjmf_reinstall_times",
            ];
            ...
            $data = [
                ...
                "capacity" => 1,
                "invoiceid" => $invoiceid,
                "type" => "reinstall_times",
                "hostid" => $host_id,
            ];
            $record = \think\Db::name("dcim_buy_record")->insertGetId($data);
            if ($record) {
                active_log_final(
                    sprintf($this->lang["Dcim_home_buyReinstallTimes"], $host_id, $invoiceid),
                    $uid,
                    2,
                    $host_id,
                    2,
                ...
                $result["status"] = 400;
                $result["msg"] =
                    "The number of purchases and reloads is wrong, please contact customer service, do not pay the generated bill, the ID is：" .
                    $invoiceid;
            }
        } else {
            $result["status"] = 400;
    ...
    public function repassword()
    {
        ...
        if ($host_data["type"] == "dcim") {
            ...
            $data = [
                "crack_password" => $params["password"],
                "other_user" => intval($params["other_user"]),
                "user" => $params["user"] ?? "",
    ...
    public function getReinstall()
    {
        ...
        if (!empty($cloud_os_group)) {
            foreach ($cloud_os_group as $k => $v) {
                $cloud_os_group[$k] = [
                    "group_name" => $v,
                    "img" => svgBase64EncodeImage(CMF_ROOT . "public/upload/common/system/" . getOsSvg($v) . ".svg"),
                ];
            }
    ...
    public function reinstall()
    {
        ...
        if ($host_data["type"] == "dcim") {
            ...
            $data = [
                "rootpass" => $param["dcim"]["password"],
                "action" => "",
                "mos" => $os_id,
                "mcon" => "",
                "port" => $param["dcim"]["port"],
                "disk" => 0,
                "check_disk_size" => 1,
                "part_type" => $param["dcim"]["part_type"] ?? 0,
            ];
            $dcim = new \app\common\logic\Dcim();
            $dcim->is_admin = false;
    ...
    public function charts()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.productid,a.domainstatus,a.regdate,a.uid,a.dcimid,b.server_type,b.type,b.api_type,b.zjmf_api_id,c.email",
            )
            ->leftJoin("products b", "a.productid=b.id")
    ...
    public function custom()
    {
        ...
        $host = \think\Db::name("host")
            ->alias("a")
            ->field(
                "a.id,a.productid,a.domainstatus,a.uid,a.dcimid,b.server_type,b.type,b.api_type,b.zjmf_api_id,c.email",
            )
            ->leftJoin("products b", "a.productid=b.id")
            ->leftJoin("clients c", "a.uid=c.id")
            ->where("a.id", $id)
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            $res = zjmfCurl(
                $host["zjmf_api_id"],
                "/zjmf_api/provision/custom/content?jwt=" . urlencode($jwt),
                $post_data,
            );
            if ($res["status"] == 200) {
                $html = $res["data"]["html"];
