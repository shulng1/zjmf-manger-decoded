...
class PayController extends \app\home\controller\CommonController
{
    public function pay()
    {
        ...
        if (!$flag) {
            ...
            if (!isset($pay_html["data"][0])) {
                ...
                return json([
                    "status" => 400,
                    "msg" => "Payment interface configuration error! or " . $error,
                    "data" => $returndata,
                ]);
            }
    ...
    public function fund()
    {
        ...
        if ($user_credit < $invoic_subtotal && $use_credit && $param["is_api"] == 1) {
            $result = [
                "status" => 400,
                "msg" => "Insufficient balance: the bill needs {$invoic_subtotal}, current balance {$user_credit}",
            ];
            if ($is_downstream) {
    ...
    public function credit()
    {
        ...
        if ($check_res["status"] == 200) {
            ...
            if ($invoice_data["credit"] > 0) {
                return json([
                    "status" => 400,
                    "msg" => "The current bill uses the balance and cannot be paid with credit",
                ]);
            }
        ...
        if ($inv["subtotal"] > $inv["total"]) {
            return json([
                "status" => 400,
                "msg" => "The balance of the bill has been used, and the credit can no longer be used to pay",
            ]);
        }
        ...
        if ($use_credit_limit < $invoice_data["total"]) {
            return json([
                "status" => 400,
                "msg" => "The current credit balance is insufficient, and the credit cannot be used to pay",
            ]);
        }
        $time = time();
        $update_invoice = [
    ...
    public function status()
    {
        ...
        $invoice = \think\Db::name("invoices")
            ...
            ->where("delete_time", 0)
            ->find();
        $hids = \think\Db::name("invoice_items")
