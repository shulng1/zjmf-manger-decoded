...
class CartController extends \cmf\controller\HomeBaseController
{
    public function products()
    {
        ...
        $fgs = \think\Db::name("product_first_groups")
            ->field("id,name")
            ->where("hidden", 0)
            ->where(function (\think\db\Query $query) use ($fgid) {
                if (isset($fgid)) {
                    $query->where("id", $fgid);
                }
            })
            ...
            ->toArray();
        foreach ($fgs as &$fg) {
            $fg["fields"] = \think\Db::name("product_first_groups_customfields")
                ->field("name,value")
                ->where("relid", $fg["id"])
                ->select()
                ->toArray();
            $gs = \think\Db::name("product_groups")
                ->field("id,name,headline,tagline")
                ->where("hidden", 0)
                ->where("order_frm_tpl", "<>", "uuid")
                ->where(function (\think\db\Query $query) use ($fg, $group_id) {
                ...
                ->toArray();
            foreach ($gs as &$g) {
                $g["fields"] = \think\Db::name("product_groups_customfields")
                    ->field("name,value")
                    ->where("relid", $g["id"])
                    ->select()
                    ->toArray();
                ...
                foreach ($tmp as $v) {
                    if ($v["ontrial"]) {
                        $v["ontrial"] = [
                            "ontrial" => $v["ontrial"],
                            "ontrial_cycle" => $v["ontrial_cycle"],
                            "ontrial_cycle_type" => $v["ontrial_cycle_type"],
                            "ontrial_price" => $v["ontrial_price"],
                            "ontrial_setup_fee" => $v["ontrial_setup_fee"],
                        ];
                    } else {
                        $v["ontrial"] = ["ontrial" => 0];
                    }
                    ...
                    foreach ($alloption as $con_k => $con_v) {
                        if ($con_v["sub"]) {
                            foreach ($con_v["sub"] as $sub_k => $sub_v) {
                                $alloption[$con_k]["sub"][$sub_k]["pricing"][0] = $this->subPricing(
                                    $configoptions_cycle,
                                    $sub_v["pricing"][0],
                                );
                            }
    ...
    public function addProducts()
    {
        ...
        $msg = [
            "product_id.require" => "Product ID is require",
            "product_id.number" => "Product ID is number",
            "configoption.array" => "Configoption is array",
            "customfield.array" => "Custom fields is array",
            "qty.number" => "Quantity is number",
        ];
        $param = $this->request->param();
    ...
    public function cartPage()
    {
        ...
        if ($uid) {
            $client = \think\Db::name("clients")
                ->field("username,email,address1,phonenumber,credit,credit_limit,is_open_credit_limit")
                ->where("id", $uid)
                ->find();
            $client["is_open_credit_limit"] = configuration("shd_credit_limit")
                ? (configuration("credit_limit") == 1
                    ? $client["is_open_credit_limit"]
                    : 0)
                : 0;
    ...
    public function cartCheckout()
    {
        ...
        if (!$flag && count($msg) >= 1) {
            ...
            $res_msg1 = [
                "status" => 410,
                "msg" =>
                    "The product you purchased requires real-name authentication, please complete the real-name authentication before purchasing",
            ];
        }
        if (!$flag1 && count($msg1) >= 1) {
            $res_msg2 = [
                "status" => 415,
                "msg" =>
                    "The product you purchased needs to be bound with a mobile phone number, please bind the mobile phone number before purchasing",
            ];
        }
        ...
        foreach ($cart_data["products"] as $k => $v) {
            ...
            if (!judgeOntrialNum($v["pid"], $uid, $qty, false, true) && $v["billingcycle"] == "ontrial") {
                return json([
                    "status" => 400,
                    "msg" =>
                        "The product you purchased exceeds the trial quantity limit, please re-select this product cycle",
                ]);
            }
            $pay_type = json_decode($product["pay_type"], true);
    ...
    public function goods()
    {
        ...
        foreach ($fgs as &$fg) {
            ...
            $gs = \think\Db::name("product_groups")
                ...
                ->where("hidden", 0)
                ->where("order_frm_tpl", "<>", "uuid")
                ->where(function (\think\db\Query $query) use ($fg, $param) {
                    $query->where("gid", $fg["id"]);
                    if (isset($param["gid"])) {
    ...
    public function cartEditPage()
    {
        ...
        $data = [
            ...
            "qty" => $cart_data["qty"],
            "hostid" => intval($cart_data["hostid"]),
        ];
        if (getEdition()) {
    ...
    public function cartEdit()
    {
        ...
        $msg = [
            "position.require" => lang("CART_EDIT_TOSHOP_VERIFY_I_REQUIRE"),
            "position.number" => lang("CART_EDIT_TOSHOP_VERIFY_I_NUMBER"),
    ...
    public function cartModifyQty()
    {
        ...
        if ($uid) {
            $client = \think\Db::name("clients")
                ->field("credit,credit_limit,is_open_credit_limit")
