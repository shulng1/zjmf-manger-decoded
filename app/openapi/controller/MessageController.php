...
class MessageController extends \cmf\controller\HomeBaseController
{
    ...
    public function message(\think\Request $request)
    {
        ...
        $list = \think\Db::name("system_message")
            ...
            ->page($page)
            ->limit($limit)
            ->field("sm.id,sm.title,sm.content,sm.attachment,sm.type,sm.is_market,sm.create_time,sm.read_time")
            ->order("sm.id", "desc")
            ->select()
            ->toArray();
        ...
        foreach ($system_message_type as $key => $type_item) {
            ...
            $temp_message["count"] = \think\Db::name("system_message")
                ->where("delete_time", 0)
                ->where("read_time", 0)
                ->where("type", $key)
    ...
    public function readMessage()
    {
        ...
        $result = \think\Db::name("system_message")
            ->where("uid", $param["uid"])
            ->where(function (\think\db\Query $query) use ($ids, $type) {
                if ($ids) {
