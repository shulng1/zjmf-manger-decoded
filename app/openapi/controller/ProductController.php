...
class ProductController extends \cmf\controller\HomeBaseController
{
    ...
    public function getProductDetail($id = 0)
    {
        ...
        $currency = getUserCurrency($uid);
        $billing_cycle = config("billing_cycle");
        $product["suspendreason_type"] = explode("-", $product["suspendreason"])[0]
            ? explode("-", $product["suspendreason"])[0]
            : "";
        $product["suspendreason"] = explode("-", $product["suspendreason"])[1]
            ? explode("-", $product["suspendreason"])[1]
            : "";
        ...
        $product["firstpaymentamount_desc"] =
            $currency["prefix"] . $product["firstpaymentamount"] . $currency["suffix"];
    ...
    public function upgradeConfig1()
    {
        try {
            ...
            if (!$upgrade_logic->judgeUpgradeConfigError($hid)) {
                return json([
                    "status" => 400,
                    "msg" => "Configurable items cannot be upgraded or downgraded for the current product",
                ]);
            }
            ...
            if (!empty($configoptions) && is_array($configoptions)) {
                ...
                foreach ($alloptions as $alloption) {
                    $tmp = [
                        "oid" => $alloption["oid"],
                        "option_name" => $alloption["option_name"],
                        "option_type" => $alloption["option_type"],
                        "suboption_name" => $alloption["suboption_name"],
                        "old_suboption_name" => $alloption["old_suboption_name"],
                    ];
                ...
                $return = [
                    ...
                    "name" => $page["name"],
                    "billingcycle" => $page["billingcycle"],
                    "payment" => $page["payment"],
                    "saleproducts" => $page["saleproducts"],
                    "subtotal" => $page["subtotal"],
                    "total" => $page["total"],
                    "promo_code" => $page["promo_code"],
                    "configoptions" => $page["configoptions"],
                    "alloption" => $filter,
                ];
