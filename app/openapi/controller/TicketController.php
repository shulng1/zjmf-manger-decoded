...
class TicketController extends \cmf\controller\HomeBaseController
{
    public function getTickets()
    {
        ...
        $list = \think\Db::name("ticket")
            ->alias("a")
            ->field(
                "a.tid id,a.dptid department_id,a.uid,a.host_id,a.name,a.email,a.create_time,a.title,a.content,a.status,a.priority,a.admin,a.last_reply_time,a.update_time,b.name department_name,p.name product_name",
            )
    ...
    public function getOpenTicketPage()
    {
        $uid = request()->uid;
        $returndata = [];
        $returndata["host"] = \think\Db::name("host")
            ->where("uid", $uid)
            ->field("h.id,p.name as product_name,h.domainstatus status,h.domain name,h.dedicatedip ip")
            ->alias("h")
        ...
        $ticketCustom = \think\Db::name("customfields")
            ->field("id,fieldname name,fieldtype type,fieldoptions options,description,regexpr,required,relid")
            ->whereIn("relid", array_column($depart, "id"))
            ->where("type", "ticket")
            ->select()
            ->toArray();
        $custom = [];
    ...
    public function createTicket()
    {
        ...
        $msg = [
            "department_id.require" => "请选择工单部门",
            "department_id.number" => "类型错误",
        ...
        foreach ($customfields as $k => $v) {
            if (
                !empty($v["required"]) &&
                !isset($params["custom_field"][$v["id"]]) &&
                $params["custom_field"][$v["id"]] == ""
            ) {
                $result["status"] = 400;
        ...
        if ($r) {
            ...
            $departmentadmin = \think\Db::name("ticket_department_admin")
                ...
                ->where("t.dptid", $params["department_id"])
                ->select()
                ->toArray();
            $curl_multi_data = [];
    ...
    public function ticketDetail($id = 0)
    {
        ...
        $ticket = \think\Db::name("ticket")
            ->alias("t")
            ->field(
                "t.tid id,t.dptid department_id,t.uid,t.host_id,t.name,t.email,t.create_time,t.title,t.content,t.status,t.priority,t.admin,t.attachment,t.last_reply_time,t.update_time,td.name department_name",
            )
            ->leftJoin("ticket_department td", "t.dptid=td.id")
            ->where("t.tid", $id)
            ->where("t.uid", $uid)
        ...
        $reply = \think\Db::name("ticket_reply")
            ->field("content,admin,admin_id,attachment,create_time")
