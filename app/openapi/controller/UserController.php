...
class UserController extends \cmf\controller\HomeBaseController
{
    ...
    public function userPage()
    {
        $data["client"] = \think\Db::name("clients")
            ->field(
                "phone_code,phonenumber as phone,email,qq,username,companyname,country,province,city,region,address1 as address,defaultgateway,marketing_emails_opt_in,credit",
            )
    ...
    public function user()
    {
        ...
        if (
            !empty($data["marketing_emails_opt_in"]) &&
            ($data["marketing_emails_opt_in"] != 0 || $data["marketing_emails_opt_in"] != 1)
        ) {
        ...
        $data_clients = [
            "qq" => $data["qq"] ?: $client["qq"],
            "username" => $data["username"] ?: $client["username"],
            "companyname" => $data["companyname"] ?: $client["companyname"],
            "country" => $data["country"] ?: $client["country"],
            "province" => $data["province"] ?: $client["province"],
            "city" => $data["city"] ?: $client["city"],
            "region" => $data["region"] ?: $client["region"],
            "address1" => $data["address"] ?: $client["address"],
            "defaultgateway" => $data["defaultgateway"] ?: $client["defaultgateway"],
            "marketing_emails_opt_in" => intval($data["marketing_emails_opt_in"]) ?: $client["marketing_emails_opt_in"],
        ];
    ...
    public function phoneBind(\think\Request $request)
    {
        ...
        if (!empty($res)) {
            ...
            if ($res["phonenumber"] == $data["phone"]) {
                return json([
                    "status" => 400,
                    "msg" => "You have bound this phone number, no need to repeat the operation",
                ]);
            }
        }
        $code = $data["code"];
        ...
        if ($type == "email") {
            ...
            if ($res) {
                if ($status == 1) {
                    ...
                    active_logs(
                        sprintf($this->lang["User_home_loginSmsReminder_success1"]),
                        $this->request->uid,
                        "",
                        2,
                    );
                    ...
                    active_logs(
                        sprintf($this->lang["User_home_loginSmsReminder_success2"]),
                        $this->request->uid,
                        "",
                        2,
                    );
            ...
            if ($status === 0) {
                ...
                $tmp = \intval(
                    cache("verification_code_login_notice_phone" . $user["phone_code"] . $user["phonenumber"]),
                );
                if ($code !== $tmp) {
                    return json(["status" => 400, "msg" => "Incorrect verification code"]);
                }
    ...
    public function realNameAuth()
    {
        ...
        if (!isset($log["id"])) {
            ...
            if (!isset($certifi["id"])) {
                $return = [
                    ...
                    "method" => $arr,
                    "default" => $default,
                    "upload" => configuration("certifi_is_upload") ?? 1,
                ];
        ...
        $certifi["auth_card_number"] =
            mb_substr($certifi["auth_card_number"], 0, 1) .
            str_repeat("*", strlen($certifi["auth_card_number"]) - 2) .
            mb_substr($certifi["auth_card_number"], strlen($certifi["auth_card_number"]) - 1, 1);
        if ($certifi["status"] == 1) {
            $certifi["auth_fail"] = "";
        }
    ...
    public function personRealNameAuth()
    {
                ...
                foreach ($customfields as $customfield) {
                    if ($customfield["type"] == "text") {
                        ...
                        if (
                            ...
                            return json([
                                "status" => 400,
                                "msg" => "Only numbers, letters and Chinese characters are allowed for file names",
                            ]);
                        }
                        ...
                        if (
                            ...
                            !in_array($data[$customfield["field"]], array_keys($customfield["options"]))
                        ) {
                            return json([
                                "status" => 400,
                                "msg" =>
                                    $customfield["title"] .
                                    " in " .
                                    implode(",", array_values($customfield["options"])),
                            ]);
                        }
                    }
                    $i++;
        ...
        if ($cardtype == 0) {
            ...
            active_logs(
                sprintf($this->lang["Certification_home_personCertifiPost"], $othercard["auth_real_name"]),
                $clientid,
            );
            active_logs(
                sprintf($this->lang["Certification_home_personCertifiPost"], $othercard["auth_real_name"]),
        ...
        if ($cardtype == 1) {
            ...
            if ($flag && $certifi_type != "artificial") {
                ...
                $count = \think\Db::name("certifi_log")
                    ->where("uid", $uid)
                    ->where("card_type", 1)
                    ->where("certifi_type", $certifi_type)
                    ->count();
                $pay = false;
                ...
                if (floatval($_config["amount"]) > 0 && $pay) {
                    ...
                    $invoices_data = [
                        ...
                        "notes" => "",
                        "type" => "certifi_person",
                        "url" => "verified?action=personal&step=authstart&type={$certifi_type}",
                    ];
                    $invoices_items = [
                        "uid" => $uid,
                        "type" => "certifi_person",
    ...
    public function companyRealNameAuth()
    {
        if ($this->request->isPost()) {
                ...
                if (!empty($customfields[0])) {
                    ...
                    foreach ($customfields as $customfield) {
                        if ($customfield["type"] == "text") {
                            ...
                            $image = request()->file($customfield["field"]);
                            $str = explode(
                                pathinfo($image->getInfo()["name"])["extension"],
                                $image->getInfo()["name"],
                            )[0];
                            if (
                                preg_match(
                ...
                if ($res1 && $certifi_type != "artificial") {
                    ...
                    if (floatval($_config["amount"]) > 0 && $pay) {
                        ...
                        $invoices_data = [
                            ...
                            "url" => "verified?action=enterprises&step=authstart&type={$certifi_type}",
                        ];
                        $invoices_items = [
                            "uid" => $uid,
                            "type" => "certifi_company",
            ...
            $this->save_log($certifi, $this->type, $certifi_type, $certifi["status"], $custom_fields_log);
            active_logs(
                sprintf($this->lang["Certification_home_companyCertifiPost"], $certifi["company_name"]),
                $clientid,
            );
            active_logs(
                sprintf($this->lang["Certification_home_companyCertifiPost"], $certifi["company_name"]),
    ...
    protected function companyBusinessUpload()
    {
        ...
        foreach ($attachments as $image) {
            $str = explode(pathinfo($image->getInfo()["name"])["extension"], $image->getInfo()["name"])[0];
            if (
                preg_match("/[ ',:;*?~`!@#\$%^&+=)(<>{}]|\\]|\\[|\\/|\\\\|\"|\\|/", substr($str, 0, strlen($str) - 1))
            ) {
    ...
    public function realNameAuthStatus()
    {
        ...
        if (
            ...
            $log["certifi_type"] != "Ali" &&
            $log["certifi_type"] != "Idcsmartali"
        ) {
            if ($data["status"] == 1) {
                ...
                \think\Db::name($table)
                    ->where("auth_user_id", $log["uid"])
                    ->update(["status" => $status, "auth_fail" => "", "update_time" => time()]);
                if ($status == 1 && configuration("certifi_realname") == 1) {
                    $cl = \think\Db::name("certifi_log")->where("id", $log["id"])->find();
                    \think\Db::name("clients")
                        ->where("id", $cl["uid"])
                        ->update(["username" => $cl["certifi_name"]]);
                }
                unsuspendAfterCertify($uid);
            }
            return json(["status" => 200, "msg" => lang("SUCCESS MESSAGE")]);
        ...
        if (($data["status"] == 4 || $data["status"] == 2) && $data["auth_card_type"] == 1) {
            ...
            if ($res["status"] == 1) {
                ...
                \think\Db::name("certifi_log")
                    ->where("id", $log["id"])
                    ->update(["status" => $status, "error" => $res["msg"]]);
                \think\Db::name($table)
                    ->where("auth_user_id", $log["uid"])
                    ->update(["status" => $status, "update_time" => time(), "auth_fail" => $res["msg"]]);
            }
        }
