...
class InvoicesController extends \app\home\controller\CommonController
{
    public function invoices()
    {
        ...
        $data = [
            "invoices" => [
                "logo" => configuration("logo_url_bill"),
    ...
    public function combineInvoices()
    {
        ...
        $where = function (\think\db\Query $query) use ($uid, $ids) {
            $query
                ...
                ->where("delete_time", 0)
                ->where("is_delete", 0)
                ->whereNotIn("type", ["recharge"]);
            if (!empty($ids)) {
                $query->whereIn("id", $ids);
            }
        };
        $invoices = \think\Db::name("invoices")
            ->field("id,subtotal,total,payment,status,type")
            ->where($where)
            ->select()
            ->toArray();
        if (empty($invoices)) {
        ...
        foreach ($invoices as $v) {
            ...
            if ($v["type"] == "combine" || $v["type"] == "recharge") {
                return json([
                    "status" => 400,
                    "msg" => "Consolidated bills or recharge bills #" . $v["id"] . " cannot be consolidated again",
                ]);
            }
            $total += $v["total"];
            $subtotal += $v["subtotal"];
    ...
    public function fundsInfo()
    {
        ...
        $accounts = \think\Db::name("accounts")
            ->alias("a")
            ->field("a.trans_id,a.amount_in,a.pay_time,a.gateway,a.invoice_id,a.description")
    ...
    public function accountsRecord()
    {
        ...
        $accounts = \think\Db::name("accounts")
            ...
            ->field("a.id,a.invoice_id,a.pay_time,a.gateway as payment_zh,a.description,b.type,a.amount_in,a.trans_id")
            ->where($where)
            ->withAttr("payment_zh", function ($value, $data) use ($gateways) {
                foreach ($gateways as $v) {
                    if ($v["name"] == $value) {
