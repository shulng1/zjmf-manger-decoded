...
class LoginController extends \cmf\controller\HomeBaseController
{
    ...
    public function loginAPI()
    {
        ...
        if ($client["status"] != 1) {
            return json([
                "status" => 400,
                "msg" => "The account has been deactivated/closed, please contact the administrator",
            ]);
        }
    ...
    public function login()
    {
        ...
        if ($param["phone"]) {
            ...
            $validate->message([
                "phone.require" => "Mobile number cannot be empty",
                "phone.length" => "Phone length is 4-11 digits",
            ...
            $client = $clients
                ->field("id,phone_code,phonenumber,email,password,second_verify,status,username")
            ...
            $validate->message([
                "phone.require" => "Mobile number cannot be empty",
                "code.require" => "verification code must be filled",
            ]);
            if (!$validate->check($data)) {
                return json(["status" => 400, "msg" => $validate->getError()]);
            }
        ...
        if ($comparePasswordResult) {
            ...
            if (
                ...
                !empty($second_verify_home) &&
                !empty($second_verify_login)
            ) {
            ...
            active_log_final(
                sprintf($this->lang["User_home_login"], $active_log_final, $account, $client["id"]),
                $client["id"],
                1,
                0,
                2,
    ...
    public function registerPage()
    {
        ...
        $dataRegister["system_fields"] = configuration("login_register_custom_require")
    ...
    public function register(\think\Request $request)
    {
        ...
        if ($param["phone"]) {
            ...
            $validate->message([
                ...
                "password.min" => "Password must be at least 6 characters",
                "password.max" => "Password up to 32 characters",
                "code.require" => "Verification code required",
                "qq.max" => "qq no more than 20 characters",
                "username.max" => "Username must not exceed 20 characters",
                "companyname.max" => "Company name no more than 20 characters",
                "address1.max" => "Address no more than 20 characters",
            ]);
            if (!$validate->check($data)) {
                return json(["status" => 400, "msg" => $validate->getError()]);
            }
            ...
            $client = \think\Db::name("clients")
                ->field("phone_code,phonenumber,email,password,second_verify,status")
                ->where("phonenumber", $data["phone"])
                ->find();
            if (!empty($client)) {
            ...
            $client = \think\Db::name("clients")
                ->field("phone_code,phonenumber,email,password,second_verify,status")
                ->where("phonenumber", $data["email"])
                ->find();
            if (!empty($client)) {
                return json(["status" => 400, "msg" => "This account already exists"]);
            }
        }
        $login_register = configuration("login_register_custom_require")
            ? json_decode(configuration("login_register_custom_require"), true)
            : [];
        ...
        $data_clients = [
            "username" => !empty($data["system_fields"]["username"])
                ? $data["system_fields"]["username"]
                : $data["phone"] ?? substr($data["email"], 0, strpos($data["email"], "@")),
            "email" => !empty($data["email"]) ? $data["email"] : "",
            "avatar" => "用户头像2-" . rand(10, 20) . ".jpg",
            "phone_code" => !empty($data["phone_code"]) ? $data["phone_code"] : "86",
            "phonenumber" => !empty($data["phone"]) ? $data["phone"] : "",
            "currency" => getDefaultCurrencyId(),
            "password" => !empty($data["password"]) ? cmf_password($data["password"]) : "",
            "lastloginip" => get_client_ip(0, true),
            "create_time" => time(),
            "lastlogin" => time(),
            "status" => 1,
            "defaultgateway" => gateway_list()[0]["name"] ?? "",
            "sale_id" => !empty($sale_id) ? $sale_id : 0,
            "qq" => $data["system_fields"]["qq"] ?? "",
            "companyname" => $data["system_fields"]["companyname"] ?? "",
            "address1" => $data["system_fields"]["address1"] ?? "",
            "api_password" => aesPasswordEncode(randStrToPass(12, 0)),
            "marketing_emails_opt_in" => configuration("marketing_emails_opt_in"),
            "api_open" => 0,
        ];
        $userId = \think\Db::name("clients")->insertGetId($data_clients);
        if (!empty($data["custom_fields"])) {
            foreach ($data["custom_fields"] as $k => $v) {
                \think\Db::name("customfieldsvalues")->insert([
                    "fieldid" => $k,
                    "relid" => $userId,
                    "value" => $v,
                    "create_time" => time(),
        ...
        if ($param["email"]) {
            ...
            if ($client) {
                $params = [
                    ...
                    "username" => $data["system_fields"]["username"],
                ];
                $ret = sendmsglimit($client["phonenumber"]);
                if ($ret["status"] !== 400) {
                    $ret = $sms->sendSms(
                        $message_template_type[strtolower("Registration_Success")],
    ...
    public function pwreset()
    {
        ...
        if ($param["phone"]) {
            ...
            if (!configuration("allow_login_phone")) {
                return json([
                    "status" => 400,
                    "msg" =>
                        "The mobile phone login function is not turned on, and the password cannot be retrieved with the mobile phone",
                ]);
            }
            $client = $clients
                ->field("id,phone_code,phonenumber,email,password,second_verify,status,username")
            ...
            if (!configuration("allow_login_email")) {
                return json([
                    "status" => 400,
                    "msg" =>
                        "The email login function is not turned on, and the email cannot be used to retrieve the password",
                ]);
            }
            $client = $clients
                ->field("id,phone_code,phonenumber,email,password,second_verify,status,username")
            ...
            $validate = new \think\Validate(["email" => "require", "password" => "require|min:6|max:32"]);
            $validate->message([
                "email.require" => "Mobile number cannot be empty",
                "password.require" => "Password required",
                "password.min" => "Password must be at least 6 characters",
                "password.max" => "Password up to 32 characters",
                "code.require" => "Verification code required",
        ...
        \think\Db::name("clients")->where("id", $client["id"])->update($_data);
        active_log_final(
            sprintf($this->lang["User_home_reset_pass"], $active_log_final, $account),
            $client["id"],
            0,
            0,
            2,
