...
class AffiliateController extends \cmf\controller\HomeBaseController
{
    ...
    public function affiliateActive(\think\Request $request)
    {
        ...
        if ($userinfo["activation"] == 1) {
            ...
            return json([
                "status" => 400,
                "url" => $url,
                "msg" => "You have been activated. You don't need to activate again",
            ]);
        } else {
            ...
            $affiliates_user_temp = \think\Db::name("affiliates_user_temp")
                ->where("affid_uid", $id)
                ->select()
                ->toArray();
            ...
            \think\Db::name("affiliates")
                ->where("id", $aid)
                ->update(["balance" => bcadd($aff["balance"], $affiliate_bonusde_posit, 2), "url_identy" => $code]);
            Db("clients")
                ->where("id", $id)
                ->update(["activation" => 1]);
            ...
            active_logs(
                sprintf($this->lang["Aff_home_activation"], $clients["id"], $clients["username"], $id),
                $id,
                "",
                2,
            );
    ...
    public function affiliate(\think\Request $request)
    {
        ...
        $data = \think\Db::name("affiliates")
            ->alias("a")
            ->field(
                "a.id,a.visitors,a.registcount,a.payamount,a.audited_balance,a.balance,a.withdrawn,a.url_identy,a.sum,a.withdraw_ing withdrawing,cu.suffix,cu.prefix",
            )
        ...
        if ($aus["affiliate_enabled"] == 1) {
            ...
            if ($affiliate_type == 1) {
                $data1[] = [
                    "name" => "推介收益",
                    "description" => "推介的用户注册购买产品后返佣金额",
                    "type" => "固定金额",
                    "commission" => $data["prefix"] . sprintf("%.2f", $aus["affiliate_bates"]),
                ];
            } else {
                $data1[] = [
                    "name" => "推介收益",
                    "description" => "推介的用户注册购买产品后返佣金额",
                    "type" => "百分比",
                    "commission" => $aus["affiliate_bates"] . "%",
                ];
            }
        } else {
            if ($data["affiliate_enabled"]) {
                ...
                if ($affiliate_type == 1) {
                    $data1[] = [
                        ...
                        "description" => "推介的用户注册购买产品后返佣金额",
                        "type" => "固定金额",
                        "commission" => $data["prefix"] . sprintf("%.2f", configuration("affiliate_bates")),
                    ];
                } else {
                    $data1[] = [
                        "name" => "推介收益",
                        "description" => "推介的用户注册购买产品后返佣金额",
                        "type" => "百分比",
                        "commission" => configuration("affiliate_bates") . "%",
                    ];
                }
            }
        }
        if ($aus["affiliate_is_renew"] == 1) {
            ...
            if ($affiliate_type == 1) {
                $data1[] = [
                    "name" => "续费收益",
                    "description" => "推介的用户产品续费后返佣金额",
                    "type" => "固定金额",
                    "commission" => $data["prefix"] . sprintf("%.2f", $aus["affiliate_renew"]),
                ];
            } else {
                $data1[] = [
                    "name" => "续费收益",
                    "description" => "推介的用户产品续费后返佣金额",
                    "type" => "百分比",
                    "commission" => $aus["affiliate_renew"] . "%",
                ];
            }
        } else {
            ...
            if ($data["affiliate_is_renew"]) {
                ...
                if ($affiliate_type == 1) {
                    $data1[] = [
                        ...
                        "description" => "推介的用户产品续费后返佣金额",
                        "type" => "固定金额",
                        "commission" => $data["prefix"] . sprintf("%.2f", configuration("affiliate_renew")),
                    ];
                } else {
                    $data1[] = [
                        "name" => "续费收益",
                        "description" => "推介的用户产品续费后返佣金额",
                        "type" => "百分比",
                        "commission" => configuration("affiliate_renew") . "%",
                    ];
                }
            }
        }
        if ($aus["affiliate_is_reorder"] == 1) {
            ...
            if ($affiliate_type == 1) {
                $data1[] = [
                    "name" => "二次购买",
                    "description" => "推介的用户二次购买产品后返佣金额",
                    "type" => "固定金额",
                    "commission" => $data["prefix"] . sprintf("%.2f", $aus["affiliate_reorder"]),
                ];
            } else {
                $data1[] = [
                    "name" => "二次购买",
                    "description" => "推介的用户二次购买产品后返佣金额",
                    "type" => "百分比",
                    "commission" => $aus["affiliate_reorder"] . "%",
                ];
            }
        } else {
            ...
            if ($data["affiliate_is_reorder"]) {
                ...
                if ($affiliate_type == 1) {
                    $data1[] = [
                        ...
                        "description" => "推介的用户二次购买产品后返佣金额",
                        "type" => "固定金额",
                        "commission" => $data["prefix"] . sprintf("%.2f", configuration("affiliate_reorder")),
                    ];
                } else {
                    $data1[] = [
                        "name" => "二次购买",
                        "description" => "推介的用户二次购买产品后返佣金额",
                        "type" => "百分比",
                        "commission" => configuration("affiliate_reorder") . "%",
                    ];
                }
            }
        }
        ...
        if ($this->request->isPost()) {
            ...
            try {
                ...
                \think\Db::name("affiliates")
                    ...
                    ->update([
                        "balance" => $now_balance,
                        "withdraw_ing" => bcadd($data["withdraw_ing"], $datas["num"], 2),
                        "updated_time" => time(),
                    ]);
                $res = \think\Db::name("affiliates_withdraw")->insertGetId([
                    "uid" => $id,
                    "create_time" => time(),
                    "type" => 0,
                    "num" => $datas["num"],
                ...
                if ($res) {
                    ...
                    return json([
                        "status" => 200,
                        "msg" => "Withdrawal succeeded, waiting for the background administrator to review",
                    ]);
                } else {
                    \think\Db::rollback();
    ...
    public function withdrawRecord(\think\Request $request)
    {
        ...
        $rows = \think\Db::name("affiliates_withdraw")
            ->alias("a")
            ->field("a.id,a.num,a.type,a.create_time,a.status,a.reason,u.user_nickname admin,cu.suffix")
    ...
    public function affiliateRecord(\think\Request $request)
    {
        ...
        $total = \think\Db::name("invoices")
            ...
            ->field(
                "i.id as invoiceid,i.is_aff,i.aff_commmission_bates_type,i.aff_commission,i.aff_commmission_bates,i.status,i.create_time,i.type,i.subtotal,i.paid_time,c.username,o.id,c.id as uid,cu.prefix,cu.suffix,i.aff_sure_time,i.aff_commission,i.aff_commmission_bates,i.aff_commmission_bates_type,i.is_aff",
            )
            ...
            ->where(function (\think\db\Query $query) use ($param) {
                if (!empty($param["keywords"])) {
                    $search_desc = $param["keywords"];
                    $query->where(
                        "c.username like '%{$search_desc}%' OR i.subtotal like '%{$search_desc}%' OR i.create_time like '%{$search_desc}%'",
                    );
                }
                if (!empty($param["type"])) {
        ...
        $rows = \think\Db::name("invoices")
            ...
            ->leftJoin("orders o", "i.id=o.invoiceid")
            ->field(
                "o.id,i.id as invoiceid,i.is_aff,i.aff_commmission_bates_type,i.aff_commission,i.aff_commmission_bates,i.status,i.create_time,i.type,i.subtotal,i.paid_time,c.username,c.id as uid,cu.prefix,cu.suffix,i.aff_sure_time",
            )
            ->where("i.delete_time", 0)
            ->where("i.status", "in", "Paid,Refunded")
            ->where("c.id", "in", $uids)
    ...
    public function getCommission($rows, $ladder, $id)
    {
        ...
        foreach ($rows as $k => $value) {
            if (
                ...
                $rows[$k]["paid_time"] =
                    $value["paid_time"] + configuration("affiliate_delay_commission") * 24 * 60 * 60;
            }
            if ($rows[$k]["is_aff"] == 1) {
    ...
    public function updateCommission($id, $uids, $ladder)
    {
        $rows1 = \think\Db::name("invoices")
            ...
            ->leftJoin("orders o", "i.id=o.invoiceid")
            ->field(
                "i.status,i.is_aff,i.aff_commmission_bates_type,i.aff_commission,i.aff_commmission_bates,i.type,i.subtotal,i.paid_time,c.username,o.id,c.id as uid,o.status,i.create_time,i.id as invoiceid,cu.prefix,cu.suffix",
            )
            ->where("i.delete_time", 0)
            ->where("i.status", "in", "Paid,Refunded")
            ->where("i.is_aff", 0)
            ->where("i.paid_time", "<=", time() - configuration("affiliate_delay_commission") * 24 * 60 * 60)
            ->where("c.id", "in", $uids)
            ->select()
            ->toArray();
        ...
        try {
            $res = \think\Db::name("affiliates")
                ->where("id", $affi["id"])
                ->update(["audited_balance" => $sum, "updated_time" => time()]);
            $affi1 = \think\Db::name("affiliates")->where("id", $affi["id"])->find();
            $res = \think\Db::name("affiliates")
                ->where("id", $affi["id"])
                ->update([
                    "sum" =>
                        $affi1["audited_balance"] + $affi1["balance"] + $affi1["withdrawn"] + $affi1["withdraw_ing"],
                    "updated_time" => time(),
                ]);
            \think\Db::commit();
        } catch (\Exception $e) {
        ...
        $total = \think\Db::name("orders")
            ->alias("o")
            ->join("clients c", "o.uid=c.id")
            ->join("invoices i", "i.id=o.invoiceid")
            ->field(
                "i.status,i.type,i.subtotal,i.paid_time,c.username,o.id,o.uid,c.id as uid,o.status,i.create_time,i.id as invoiceid",
            )
            ->where("i.delete_time", 0)
            ->where("i.status", "=", "Paid")
            ->where("c.id", "in", $uids)
        ...
        if (!empty($affi)) {
            ...
            try {
                $res = \think\Db::name("affiliates")
                    ->where("id", $affi["id"])
                    ->update(["payamount" => $total, "updated_time" => time()]);
                \think\Db::commit();
            } catch (\Exception $e) {
                \think\Db::rollback();
            }
    ...
    public function user()
    {
        ...
        $rows = \think\Db::name("affiliates_user")
            ...
            ->field(
                "c.id,c.create_time,c.username,c.companyname company_name,c.lastlogin last_login_time,c.email,c.phonenumber",
            )
