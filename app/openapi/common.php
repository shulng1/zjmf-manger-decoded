...
function start_pay($id, $payment)
{
    ...
    if ($type == "recharge") {
        ...
        $info = db("invoice_items")
            ->alias("i")
            ->join("host h", "i.rel_id=h.id")
            ->join("products p", "p.id=h.productid")
            ->where("i.invoice_id", $row["id"])
            ->where("i.rel_id", ">", 0)
            ->field("p.name,i.rel_id")
            ->find();
    }
    ...
    $payData = [
        "out_trade_no" => $row["id"],
        "product_name" => $company_name . $info["name"] ?? "智简魔方",
        "total_fee" => $row["total"],
        "attach" => $row["type"] . "@" . $uid . "@" . $row["id"] . "@" . $row["total"] . "@" . $payment,
        "fee_type" => $currency,
    ];
